...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	уатОбщегоНазначения.ЗаполнитьЗначенияJSONИсторииКонтактнойИнформации(ИсторияКонтактнойИнформацииИзПараметров);
...
Процедура ПроверитьЗаполнениеИстории(Отказ)
	...
	Для Каждого Запись Из ИсторияКонтактнойИнформации Цикл
		...
		Если НЕ ЗначениеЗаполнено(Запись.Период) И ИндексСтроки > 0 Тогда
			СообщениеОбОшибке = НСтр("ru = 'Нужно указать дату начала действия'; en = 'Specify effective date'");
		...
		Иначе
			...
			СообщениеОбОшибке = НСтр("ru='Уже есть запись с указанной датой сведений';en='A record with the specified information date already exists'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, , ПрефиксСообщенияСтроки+"Период", , Отказ);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Запись.Представление) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Поле ""Адрес"" не заполнено'; en = 'The ""Address"" field is empty.'");
