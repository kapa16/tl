...
Процедура ЗаполнитьСписокДоступныхЗначений(МассивПереданыхЗначений, ВидСправочника, ДополнительныеУсловия = Неопределено)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СправочникДляВыбораОтоборов.Ссылка КАК Ссылка,
	|	ВЫБОР
	...
	ТаблицаДоступныхЗначений = Запрос.Выполнить().Выгрузить();
	
	СписокВыбираемыхЗначений.Очистить();
	Для Каждого ДоступноеЗначение Из ТаблицаДоступныхЗначений Цикл
		НовоеВыбираемоеЗначение = СписокВыбираемыхЗначений.Добавить();
		НовоеВыбираемоеЗначение.Представление = ДоступноеЗначение.Ссылка;
		НовоеВыбираемоеЗначение.Пометка       = ДоступноеЗначение.Пометка;
		НовоеВыбираемоеЗначение.Идентификатор = ДоступноеЗначение.Ссылка.УникальныйИдентификатор();
	КонецЦикла;
