	...
	Если Параметры.Назначение = "ПрисоединенныеФайлы" Тогда
		...
		Заголовок = ?(Параметры.ОбъектыПечати.Количество() > 1,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Присоединить к документам (%1)';
					|en = 'Attach to documents (%1)'"), Параметры.ОбъектыПечати.Количество()),
				НСтр("ru = 'Присоединить к документу';
					|en = 'Attach to document'"));
		АвтоЗаголовок = Ложь;
...
Процедура Сохранить(Команда)
	...
	Если Элементы.КаталогСохраненияФайлов.Видимость Тогда
		Если ВариантСохранения = "СохранитьВПапку" И ПустаяСтрока(ВыбраннаяПапка) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите каталог.';
															|en = 'Choose a directory.'"),, "ВыбраннаяПапка");
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ФорматыСохранения.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Укажите один из предложенных форматов.';
									|en = 'Choose one of the suggested formats.'"));
		Возврат;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файл не будет присоединен к документам:
		| %1';
		|en = 'The file will not be attached to the documents:
		|%1'"), СтрокаОшибки);
	...
	ПараметрыВопроса.Заголовок = НСтр("ru = 'Недостаточно прав для присоединения файла';
										|en = 'Insufficient rights to attach the file'");
