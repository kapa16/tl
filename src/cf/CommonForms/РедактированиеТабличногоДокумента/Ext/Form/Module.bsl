...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохранить изменения в %1?';
																				|en = 'Do you want to save the changes to %1?'"), ИмяДокумента);
		...
		Иначе
			...
			Если НЕ МакетДляВыгрузкиОбъекта Тогда
				...
				ТекстВопроса = НСтр("ru = 'Макет заполнен некорректно. Продолжить запись?';
									|en = 'Some entries are invalid. Save the template anyway?'");
				ПоказатьВопрос(
					ОписаниеОповещения,
					ТекстВопроса,
...
Процедура ПроверитьЯчейкиОбъединены(Знач Область, Знач НомерСтроки, Знач НомерКолонки, Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Шаблон = НСтр("ru = 'Объединение ячеек запрещено (стр. %1 кол. %2)';
						|en = 'Cannot merge cells (row %1, column %2)'");
		ИдентификаторОбласти = МодульУправлениеПечатью.ИдентификаторОбласти(Область);
...
Процедура ПроверитьИмяПоляDBF(Знач ТекстЯчейки, Знач НомерСтроки, Знач НомерКолонки, Отказ)
	...
	Если СтрДлина(ТекстЯчейки) > 10 Тогда
		...
		Шаблон = НСтр("ru = 'Имя поля не может превышать 10 знаков (стр. %1 кол. %2)';
						|en = 'Field name must not exceed 10 characters (row %1, column %2)'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Шаблон,
			НомерСтроки, 
			НомерКолонки);
	...
	Для Индекс = 1 По СтрДлина(ТекстЯчейки) Цикл
		...
		Если ОшибкаКодаСимвола Тогда
			...
			Шаблон = НСтр("ru = 'Имена полей могут содержать только латинские буквы, цифры и символ подчеркивания (стр. %1 кол. %2)';
							|en = 'Field name can only contain letters, numbers, and underscores (row %1, column %2)'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Шаблон,
				НомерСтроки,
				НомерКолонки);
		...
		Если НомерСтроки = НомерСтрокиИменПолей Тогда
			...
			Если МассивИменКолонок.Найти(ТекстЯчейки) <> Неопределено Тогда
				...
				Шаблон = НСтр("ru = 'Неуникальное имя колонки (стр. %1 кол. %2)';
								|en = 'Column name is not unique (row %1, column %2)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					НомерСтроки,
					НомерКолонки);
		...
		Иначе
			...
			Если НомерСтроки > НомерСтрокиИменПолей + 1 Тогда
				...
				Шаблон = НСтр("ru = 'Данные выгрузки должны указываться в строке %1 (стр. %2 кол. %3)';
								|en = 'Row %1 must contain export data (row %2, column %3)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
			...
			Если ПустаяСтрока(ТекстЯчейкиИмен) Тогда
				...
				Шаблон = НСтр("ru = 'Не указано имя колонки (кол. %1)';
								|en = 'Column name is required (column %1)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Шаблон,
					НомерКолонки);
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,,,, Отказ);
	...
	Если НомерКолонки > НомерКолонкиЗначений
	   И НЕ ПустаяСтрока(ТекстЯчейки) Тогда
		...
		Шаблон = НСтр("ru = 'Имена полей и их значения должны располагаться в %1 и во %2 колонке соответственно (стр. %3 кол. %4)';
						|en = 'Field names and values must be placed in the columns %1 and %2, respectively (row %3, column %4)'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Шаблон,
	...
	Если НомерКолонки = НомерКолонкиИменПолей Тогда
		...
		Если ПустаяСтрока(ТекстЯчейки) <> ПустаяСтрока(ТекстЗначения) Тогда
			...
			Шаблон = НСтр("ru = 'Имя поля и его значение не сопоставлены (стр. %1)';
							|en = 'Field name is not mapped to field value (row %1)'");
...
Процедура ПроверитьСимволыЯчейки(Знач ТекстЯчейки, Знач НомерСтроки, Знач НомерКолонки, Знач ЭтоXML, Отказ)
	...
	Если ЭтоXML Тогда
		...
		ШаблонНеСоответствуетТребованиям = НСтр("ru = 'Имя поля ""%1"" не соответствует требованиям к именам токенов XML (стр. %2 кол. %3)';
												|en = 'Field name ""%1"" does not comply with XML token naming convention (row %2, column %3)'");
	...
	Иначе
		...
		ШаблонНеСоответствуетТребованиям = НСтр("ru = 'Имя поля ""%1"" не соответствует требованиям к именам переменных (стр. %2 кол. %3)';
												|en = 'Field name ""%1"" does not comply with variable naming convention (row %2, column %3)'");
	...
	Если РезультатПроверки.СодержитЗапрещеннуюПодстроку Тогда 
		...
		ШаблонЗапрещеннаяСтрока = НСтр("ru = 'Имя поля ""%1"" не может начинаться с ""%2"" (стр. %3 кол. %4)';
										|en = 'Field name ""%1"" must not start with ""%2"" (row %3, column %4)'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Асинх Процедура НачатьДиалогУдаленияСвойств(Знач ОбработчикЗавершения, ОсвободитьФайл) 
	...
	ТекстПредупреждения = НСтр("ru = 'Форматирование макета или его содержимое не соответствует требованиям формата. При сохранении макета будут очищены неподходящие настройки.';
								|en = 'Template formatting or content does not meet format requirements. When you save the template, incompatible settings will be cleared.'");
	...
	Для Каждого ОбластьМакета Из ТаблицаОбластей Цикл
		...
		Если ОбластьМакета.ЭтоОбластьУсловияВывода Тогда
			...
			Шаблон = НСтр("ru = 'Область условия должна располагаться ниже области владельца хотя бы на одну строку (стр. %1)';
							|en = 'Condition area must be placed at least one row below the owner area (row %1)'");
...
Процедура ПроверитьИмяОбластей(Знач ТаблицаОбластей, Отказ)
	...
	ШаблонНеСоответствуетТребованиям = НСтр("ru = 'Имя области ""%1"" не соответствует требованиям к именам токенов XML (стр. %2)';
											|en = 'Area name ""%1"" does not comply with XML token naming convention (row %2)'");
	ШаблонЗапрещеннаяСтрока = НСтр("ru = 'Имя области ""%1"" не может начинаться с ""%2"" (стр. %3)';
									|en = 'Area name ""%1"" must not start with ""%2"" (row %3)'");
...
Процедура ПроверитьМассивИменОбласти(Знач ИмяОбласти, Знач МассивИмен, ОбластьМакета, ЗапрещеннаяПодстрока, Отказ)
	...
	ШаблонНеСоответствуетТребованиямДо = НСтр("ru = 'Часть имени области ""%1"" до первого символа ''_'' не соответствует требованиям к именам токенов XML (стр. %2)';
												|en = 'The portion of the area name ""%1"" before the first underscore does not comply with XML token naming conventions (row %2)'");
	ШаблонНеСоответствуетТребованиямПосле = НСтр("ru = 'Часть имени области ""%1"" после первого символа ''_'' не соответствует требованиям к именам токенов XML (стр. %2)';
												|en = 'The portion of the area name ""%1"" after the first underscore does not comply with XML token naming conventions (row %2)'");
	ШаблонЗапрещеннаяСтрока = НСтр("ru = 'Имя области не может начинаться с ""%1"" (стр. %2)';
									|en = 'Area name must not start with ""%1"" (row %2)'");
	ШаблонЗапрещеннаяСтрокаПосле = НСтр("ru = 'Часть имени области ""%1"" после первого символа ''_'' не может начинаться с ""%2"" (стр. %3)';
										|en = 'The portion of the area name ""%1"" after the first underscore must not start with ""%2"" (row %3)'");
...
Процедура ПроверитьПересечениеОбластей(Знач ТаблицаОбластей, Отказ)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Шаблон = НСтр("ru = 'Для типа макета выгрузки ""%1"" не допускается пересечение областей ""%2"" и ""%3""';
						|en = 'Export templates of type ""%1"" do not support overlapping or areas ""%2"" and ""%3""'");
	...
	Для Каждого ТекущаяОбласть Из ТаблицаОбластей Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Шаблон = НСтр("ru = 'В области ""%1"" уже есть таблица ""%2"". Область может содержать только одну таблицу (стр. %3)';
							|en = 'Area ""%1"" already contains table ""%2"". An area can contain only one table (row %3)'");
