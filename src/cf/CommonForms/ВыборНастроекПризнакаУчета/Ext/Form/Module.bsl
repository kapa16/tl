...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Валюта") Тогда
		РежимРаботы = 2;
		Заголовок = НСтр("en='Currency determination method';ru='Способ определения валюты'");
	ИначеЕсли Параметры.Свойство("Количество") Тогда
		РежимРаботы = 1;
		Заголовок = НСтр("en='Method of determining a unit of measure';ru='Способ определения ед. измерения'");
	Иначе
		ВызватьИсключение НСтр("en='Unknown operation mode';ru='Неизвестный режим работы'");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыАналитики(ЭлементыСтрокиРодитель, ОписаниеТипов, КэшКартинок, Уровень = 0)
	...
	ТипЗначенияЕИ = Тип("СправочникСсылка.ЕдиницыИзмерения");
	...
	Для Каждого ТипРеквизита из ТипыРеквизита Цикл
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			ЕстьИскомыйТип = Ложь;
			Если Типы.Количество() = 1 Тогда
				ИскомыйТип = ?(РежимРаботы = 1, ТипЗначенияЕИ, Тип("СправочникСсылка.Валюты"));
...
Процедура ОбработатьВыбор(ВыбраннаяСтрока)
	...
	СтрокаПодходитДляВыбора = СтрокаДерева.Искомый ИЛИ ТипЗнч(СтрокаДерева.Значение) = ТипЗначенияЕИ;
	Если Не СтрокаПодходитДляВыбора Тогда
		ПоказатьПредупреждение(, НСтр("en='Line cannot be selected.';ru='Строка не может быть выбрана!'"));
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(СтрокаДерева.Значение) = ТипЗначенияЕИ Тогда
