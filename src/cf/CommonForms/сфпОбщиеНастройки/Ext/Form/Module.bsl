...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.сфпПланНумерации.Видимость = НЕ ЭтоЖКХ И ИспользуетсяПланНумерации;
	Элементы.ПланНумерации.Видимость = НЕ ЭтоЖКХ И ИспользуетсяПланНумерации;
...
Процедура сфпСрокЖизниСообщенияЖурналаЗвонковПриИзменении(Элемент)
	...
	ШаблонПодсказки = НСтр("ru = 'Будут удалены сообщения журнала звонков Системы взаимодействия, созданные более %1 дней назад'; en = 'Interaction System call log messages created more than %1 days ago will be deleted'");
...
Процедура УдалитьНеиспользуемыеЛинии(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Несуществующие номера будут удалены! Продолжить?'; en = 'Non-existent numbers will be deleted! Continue?'");
...
Процедура ЗаполнитьМаршрутизациюНаСервере()
	...
	Иначе
		...
		|ГДЕ
		|	Настройка = ЗНАЧЕНИЕ(ПланВидовХарактеристик." + ИмяПВХНастроек + ".сфпТекущийВнутреннийНомер)");
	КонецЕсли;
