...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаУведомления.Видимость = уатОбщегоНазначенияПовтИсп.ВариантПоставкиКОРП();
КонецПроцедуры
...
&НаКлиенте
Процедура НастроитьЦелевыеПоказателиОпозданий(Команда)
	ОткрытьФорму("РегистрСведений.уатПоказателиОпозданий.ФормаСписка", , ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура УстановитьФлагФормироватьУведомленияОбОпозданияхВРейсе(Команда)
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрогнозированиеОпозданийОтветНаВопрос", ЭтаФорма);
	ТекстВопроса = НСтр("ru='Установить возможность формирования оповещений о прогнозируемых опозданиях для всех ТС?';
		|en='Allow lateness predicrtion notifications for all vehicles?'");
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьРегламентноеЗаданиеПрогнозированиеОпозданийВручную(Команда)
	ВыполнитьРегламентноеЗаданиеВручную(ПрогнозированиеОпозданийПоМаршрутнымЛистам, "Прогнозирование опозданий по маршрутным листам");
КонецПроцедуры

&НаКлиенте
Процедура НастроитьРегламентноеЗадание(Команда)
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Идентификатор", ПрогнозированиеОпозданийПоМаршрутнымЛистам);
...
Процедура ОбновитьДанныеРеглЗаданийСервер()
	...
	РеглЗаданиеПрогнозированиеОпозданийПоМаршрутнымЛистам = РегламентныеЗаданияСервер.Задание(Метаданные.РегламентныеЗадания.уатПрогнозированиеОпозданийПоМаршрутнымЛистам);
	ПрогнозированиеОпозданийПоМаршрутнымЛистам = РеглЗаданиеПрогнозированиеОпозданийПоМаршрутнымЛистам.УникальныйИдентификатор;
	
	Подсказка =  НСтр("ru='Никогда не выполнялось';en='Never execute'");
	Если РеглЗаданиеПрогнозированиеОпозданийПоМаршрутнымЛистам = Неопределено Тогда
	...
	Иначе
		ПоследнееФоновоеЗадание = РегламентныеЗаданияСлужебный.СвойстваФоновыхЗаданий(Новый Структура("ПолучитьПоследнееФоновоеЗаданиеРегламентногоЗадания,
			|ИдентификаторРегламентногоЗадания", Истина, ПрогнозированиеОпозданийПоМаршрутнымЛистам));
		Подсказка = ?(РеглЗаданиеПрогнозированиеОпозданийПоМаршрутнымЛистам.Использование, 
...
&НаКлиенте
Процедура ПрогнозированиеОпозданийОтветНаВопрос(Результат, ДопПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		уатОбщегоНазначения_проф.УстановитьФлагФормироватьУведомленияОбОпозданияхДляТС();
	КонецЕсли;
