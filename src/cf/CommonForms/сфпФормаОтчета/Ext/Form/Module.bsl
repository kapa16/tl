...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МодульОтчетыКлиентСервер = сфпОбщегоНазначения.сфпОбщийМодуль("ОтчетыКлиентСервер");
	НастройкиОтчета = МодульОтчетыКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию();
	
	сфпОтчеты.ИнициализироватьНастройкиОтчета(НастройкиОтчета);
	...
	ИначеЕсли Параметры.Свойство("Отчет") 
			И сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(Параметры.Отчет) = "ИдентификаторыОбъектовМетаданных" Тогда
		НастройкиОтчета.Вставить("ОтчетСсылка", Параметры.Отчет);
	Иначе
		Попытка    Информация = МодульВариантыОтчетов.СформироватьИнформациюОбОтчетеПоПолномуИмени(НастройкиОтчета.ПолноеИмя);
		Исключение Информация = СформироватьИнформациюОбОтчетеПоПолномуИмени(НастройкиОтчета.ПолноеИмя);
		КонецПопытки;	
		НастройкиОтчета.Вставить("ОтчетСсылка", Информация.Отчет);
	КонецЕсли; 
	НастройкиОтчета.Вставить("Внешний", ТипЗнч(НастройкиОтчета.ОтчетСсылка) = Тип("Строка") ИЛИ сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(НастройкиОтчета.ОтчетСсылка) = "ДополнительныеОтчетыИОбработки");
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(КлючТекущегоВарианта) = "ВариантыОтчетов" Тогда
	...
	Если МодульВариантыОтчетовПовтИсп.Параметры().ОтчетыСНастройками.Найти(НастройкиОтчета.ОтчетСсылка) <> Неопределено Тогда
	...
	Если МодульОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		Если НЕ НастройкиОтчета.Свойство("ВариантСсылка") Тогда
			Если Параметры.Свойство("Ключ") И сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(Параметры.Ключ) = "ВариантыОтчетов" Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		МодульРассылкаОтчетов = сфпОбщегоНазначения.сфпОбщийМодуль("РассылкаОтчетов");
	...
	// События.
	МодульОтчетыПереопределяемый = сфпОбщегоНазначения.сфпОбщийМодуль("ОтчетыПереопределяемый");
	МодульОтчетыПереопределяемый.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	...
	Если сфпСофтФонПроСервер.сфпИспользоватьОграничениеПоказаТелефонныхЗвонков() Тогда
		...
		сфпКонтролируемыеПользователи = сфпСофтФонПроСервер.сфпПреобразоватьМассивЗвонящихВСписокЗначений(МассивПользователей);
	КонецЕсли;
	// -СофтФон

КонецПроцедуры

// АПК:78-выкл отключаем проверку экспортных процедур в модуле форм
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			МодульВариантыОтчетов.ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение,
				сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" не найден в программе, он будет значиться как внешний.'; en = 'Report ""%1"" was not found in the program, it will be listed as external.'"),
					ОтчетПолноеИмя));
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав доступа к отчету ""%1"".'; en = 'Insufficient access rights to report ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для отчета ""%1"" невозможно определить тип (не установлен префикс).'; en = 'The report type cannot be determined for report ""%1"" (no prefix set).'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
	...
	Результат.Отчет = МодульОбщегоНазначения.ИдентификаторОбъектаМетаданных(Результат.ОтчетМетаданные);
	Результат.ТипОтчета = МодульВариантыОтчетов.ТипОтчета(Результат.Отчет);
...
&НаСервере
Функция ПоУмолчаниюВсеПодключеныКХранилищу() Экспорт
...
// АПК:78-вкл включаем проверку экспортных процедур в модуле форм
...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("УправляемаяФорма") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если СуффиксФормыИсточника = ВРег("ФормаНастроекОтчета")
				...
				Если Результат.Свойство("НастройкиКД", НастройкиКД) ИЛИ Результат.Свойство("КомпоновщикНастроекКД", КомпоновщикНастроекКД) Тогда
					Если НастройкиКД<>Неопределено Тогда
						Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиКД);
					Иначе
						Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(КомпоновщикНастроекКД.Настройки);
					КонецЕсли;
					ОбновитьНастройкиНаФорме();
					ОтметитьНеактуальным(); 
				КонецЕсли; 
			КонецЕсли;
		ИначеЕсли сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(Результат) = "ВариантыОтчетов" Тогда
			Вариант = Результат;
	...
	Если МодульОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		МодульРассылкаОтчетовКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("РассылкаОтчетовКлиент");
		МодульРассылкаОтчетовКлиент.ФормаОтчетаОбработкаВыбора(ЭтотОбъект, Результат, ПодчиненнаяФорма, РезультатОбработан);
	КонецЕсли;
	
	МодульОтчетыКлиентПереопределяемый = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОтчетыКлиентПереопределяемый");
	МодульОтчетыКлиентПереопределяемый.ОбработкаВыбора(ЭтотОбъект, Результат, ПодчиненнаяФорма, РезультатОбработан);
	
КонецПроцедуры

&НаКлиенте
Функция ИмяСобытияИзменениеВарианта()
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	МодульВариантыОтчетовКлиентСервер = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ВариантыОтчетовКлиентСервер");
	
	Если ИмяСобытия = ИмяСобытияИзменениеВарианта() Тогда
		ОбновитьНастройкиНаФорме();
		ОтметитьНеактуальным(); 
	...
	МодульОтчетыКлиентПереопределяемый = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОтчетыКлиентПереопределяемый");
	МодульОтчетыКлиентПереопределяемый.ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник, ОповещениеОбработано);
...
Процедура ПриСохраненииВариантаНаСервере(Настройки)
	ЗаполнитьКомандыВыбораВариантов();
	ТекВариант = ПолучитьСсылку(НастройкиОтчета.ОтчетСсылка, КлючТекущегоВарианта);
	Если Элементы.Вариант.СписокВыбора.НайтиПоЗначению(ТекВариант) <> Неопределено Тогда
		Вариант = ТекВариант;
	КонецЕсли;
	УстановитьВидимостьИДоступность("СохраненВариантОтчета");
КонецПроцедуры
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(КлючТекущегоВарианта) = "ВариантыОтчетов" Тогда
		...
		ЗначенияРеквизитов = МодульОбщегоНазначения.ЗначенияРеквизитовОбъекта(Вариант, "КлючВарианта, Пользовательский");
	...
	ИначеЕсли ТипЗнч(КлючТекущегоВарианта)=Тип("Строка") Тогда
		...
		Иначе
			...
			НастройкиОтчета.Вставить("КлючВарианта", КлючТекущегоВарианта);
		КонецЕсли;
		ЗначениеРеквизитаПользовательский = МодульОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДопВариант, "Пользовательский");
		...
		Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ДопВариант) = "ВариантыОтчетов" Тогда
			Вариант = ДопВариант;
		КонецЕсли; 
	Иначе
		НастройкиОтчета.Вставить("ВариантСсылка", Справочники["ВариантыОтчетов"].ПустаяСсылка());
	...
	Если РежимВариантаОтчета И НЕ НовыеНастройкиКД = Неопределено Тогда
		...
		МодульОтчетыПереопределяемый = сфпОбщегоНазначения.сфпОбщийМодуль("ОтчетыПереопределяемый");
		МодульОтчетыПереопределяемый.ПередЗагрузкойВариантаНаСервере(ЭтотОбъект, НовыеНастройкиКД);
	...
	МодульОбщегоНазначенияКлиентСервер = сфпОбщегоНазначения.сфпОбщийМодуль("ОбщегоНазначенияКлиентСервер");
	...
	ИначеЕсли НЕ НовыеНастройкиКД=Неопределено И НовыеНастройкиКД.ДополнительныеСвойства.Свойство("НаименованиеВарианта") Тогда 
		НастройкиОтчета.Вставить("НаименованиеВарианта", МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыеНастройкиКД.ДополнительныеСвойства, "НаименованиеВарианта"));
	Иначе
		НастройкиОтчета.Вставить("НаименованиеВарианта", МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства, "НаименованиеВарианта"));
	КонецЕсли;
	...
	ЗначениеМакетОформления = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("МакетОформления");
	Если ЗначениеМакетОформления<>Неопределено И НЕ ЗначениеМакетОформления.Использование Тогда
		ЗначениеМакетОформления.Значение = "CRM_МакетОформления";
		ЗначениеМакетОформления.Использование = Истина;
	КонецЕсли;
	ЗначениеМакетОформления = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("ВыводитьПараметрыДанных");
	Если ЗначениеМакетОформления<>Неопределено И НЕ ЗначениеМакетОформления.Использование Тогда
		ЗначениеМакетОформления.Значение = ТипВыводаТекстаКомпоновкиДанных.НеВыводить;
		ЗначениеМакетОформления.Использование = Истина;
	КонецЕсли;
	ЗначениеМакетОформления = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор");
	Если ЗначениеМакетОформления<>Неопределено И НЕ ЗначениеМакетОформления.Использование Тогда
		ЗначениеМакетОформления.Значение = ТипВыводаТекстаКомпоновкиДанных.НеВыводить;
		ЗначениеМакетОформления.Использование = Истина;
	КонецЕсли;
	...
	Если Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("РазвернутьДо") Тогда
	...
	// +СофтФон
	Для Каждого ТекЭлемент Из Элементы.ГруппаФильтры.ПодчиненныеЭлементы.ГруппаФильтрЭлементы.ПодчиненныеЭлементы Цикл
		Если ТекЭлемент.Заголовок = "Сотрудник" Тогда
			Если ТекЭлемент.ПодчиненныеЭлементы.Количество() > 0 Тогда
				МассивСвязей = Новый Массив();
				МассивСвязей.Добавить(Новый СвязьПараметраВыбора("Отбор.Ссылка", "сфпКонтролируемыеПользователи"));
				ТекЭлемент.ПодчиненныеЭлементы[0].СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивСвязей);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	// -СофтФон
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		Тип = МодульОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ПользовательскаяНастройкаКД));
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			ИдентификаторЭлемента = ПривестиИдентификаторКИмени(ПользовательскаяНастройкаКД.ИдентификаторПользовательскойНастройки);
			...
			Если ПериодНачало.АвтоОтметкаНезаполненного
				...
				ТекстОшибки = НСтр("ru = 'Не указан период'; en = 'Period is not specified'");
				ПутьКДанным = ПериодНачало.ПутьКДанным;
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ru = 'Конец периода должен быть больше начала'; en = 'Period end should be later than its start'");
			...
			Отказ = Истина;
			сфпОбщегоНазначенияКлиентСервер.сфпСообщитьПользователю(ТекстОшибки, , ПутьКДанным);
...
Процедура ПредставлениеПериодаНажатие(Элемент)
	...
	Если ПараметрПериод=Неопределено Тогда
		ПараметрПериод = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ПериодОтчета");
	КонецЕсли; 
	Если ПараметрПериод=Неопределено Тогда
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НастройкиОтчета.Свойство("ВариантСтандартногоПериода") Тогда
			ПараметрыВыбора = Новый Структура("НачалоПериода,КонецПериода, ЗакрыватьПриВыборе, ВариантПериода",
				Период.ДатаНачала, Период.ДатаОкончания, Ложь, НастройкиОтчета.ВариантСтандартногоПериода);
			ОткрытьФорму("ОбщаяФорма.сфпВыборСтандартногоПериода", ПараметрыВыбора, Элементы.ПредставлениеПериодаКнопка, , , , Оповещение);
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период.ДатаОкончания, НСтр("ru = 'На дату'; en = 'For date'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
Процедура ПредставлениеПериодаНажатиеЗавершение(НовыйПериод, ДополнительныеДанные) Экспорт
	...
	КонецЕсли;
	ПериодПриИзмененииСервер(Истина);
	ОтметитьНеактуальным();
...
Процедура СтрокаОтбораАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если СписокВыбора.Количество()=0 И СтрДлина(Текст)>2 Тогда
		СписокВыбора.Добавить("", Новый ФорматированнаяСтрока(НСтр("ru = 'Совпадений не найдено'; en = 'No matches found'"),, WebЦвета.НейтральноСерый));
	КонецЕсли; 
...
Процедура ПоказатьУточняющееПодменюСтрокиОтбора(ВыбранноеЗначение, Варианты)
	...
	Если ВыбранноеЗначение.Свойство("Поле") Тогда
		...
		ВариантыСравнения = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Варианты);
...
Процедура ОбработатьВыборСтрокаОтбора(СтруктураПараметров)
	...
	ИначеЕсли ТипЗнч(Значение)=Тип("Строка") Тогда
		Реквизиты = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтруктураПараметров.Реквизиты);
...
Процедура СтрокаОтбораПриИзменении(Элемент)
	...
	Если ВРег(СтрокаОтбора)="СКД" Тогда
		ОчиститьСтрокуОтбора();
		РедактироватьСКД();
	КонецЕсли;
...
&НаКлиенте
Процедура СброситьНастройкиНажатие(Элемент)
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
		...
		Если НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.Ресурс Тогда
			...
			Для каждого Область Из ОтчетТабличныйДокумент.ВыделенныеОбласти Цикл
				...
				Если НЕ ПустаяСтрока(Представление) Тогда
					ДробныйРазделитель = НСтр("ru = ','; en = ','");
					ТестДляПроверки = СтрЗаменить(Представление, ДробныйРазделитель, "");
					Если СтрЧислоВхождений(Представление, ДробныйРазделитель)<=1 И сфпСтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ТестДляПроверки) Тогда
	...
	Если Данные.ЭтоСсылка Тогда
	    ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение, НСтр("ru = 'Открыть'; en = 'Open'"));
	КонецЕсли; 
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И НЕ ДоступноеПоле.Ресурс Тогда
		ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать, НСтр("ru = 'Отфильтровать ""'; en = 'Filter ""'")+Данные.Поле+" = "+Данные.Значение+"""");
	КонецЕсли;
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		ДополнительныеДействия.Добавить(СформироватьПодменюОформить(Данные.Поле, Данные.Значение), НСтр("ru = 'Оформить'; en = 'Complete'"));
		ПодменюСортировка = Новый СписокЗначений;
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, Данные.Поле+НСтр("ru = ' по возрастанию'; en = 'ascending'"),, БиблиотекаКартинок.СортироватьСписокПоВозрастанию); 
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, Данные.Поле+НСтр("ru = ' по убыванию'; en = 'in descending order'"),, БиблиотекаКартинок.СортироватьСписокПоУбыванию); 
	    ДополнительныеДействия.Добавить(ПодменюСортировка, НСтр("ru = 'Сортировать'; en = 'Sort'"));
	КонецЕсли;
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	НСтр("ru = 'Редактировать список быстрых настроек'; en = 'Edit the quick settings list'"),, 
	БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
	СписокКоманд.Добавить(
	"РучнаяНастройкаОформления", 
	НСтр("ru = 'Ручная настройка оформления'; en = 'Manual design settings'"),, 
...
Функция ПолучитьДанныеРасшифровки(Расшифровка)
	...
	КонецЕсли;
	МодульОбщегоНазначения = сфпОбщегоНазначения.сфпОбщийМодуль("ОбщегоНазначения");
	СтруктураВозврата.Вставить("ЭтоСсылка", МодульОбщегоНазначения.ЭтоСсылка(ТипЗнч(СтруктураВозврата.Значение)));
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыполненноеДействие="РучнаяНастройкаОформления" Тогда
		...
		ОткрытьФорму("ОбщаяФорма.сфпФормаОформления", СтруктураОткрытия, ЭтаФорма,,,, Оповещение);
	ИначеЕсли ВыполненноеДействие="СписокБыстрыхНастроекОформления" Тогда
		СтруктураОткрытия = Новый Структура;
		СтруктураОткрытия.Вставить("ТипЗначения", Данные.ТипЗначения);
		ОткрытьФорму("ОбщаяФорма.сфпФормаБыстрыхНастроекОформления", СтруктураОткрытия, ЭтаФорма);
...
Процедура ОткрытьФормуОтчета()
	...
	Если СтандартнаяОбработка Тогда
		...
		СтруктураОткрытия.Вставить("ПредставлениеВарианта", Строка(НастройкиОтчета.НаименованиеВарианта)+НСтр("ru = ' (Расшифровка)'; en = '(Transcript)'"));
		ОткрытьФорму(НастройкиОтчета.ПолноеИмя+".Форма", СтруктураОткрытия, ЭтотОбъект,, Неопределено);
	КонецЕсли;
...
Процедура ВыполнитьНестандартнуюРасшифровку(Расшифровка, Данные, СтандартнаяОбработка = Истина)
	...
	МодульОтчетыКлиентПереопределяемый = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОтчетыКлиентПереопределяемый");
	МодульОтчетыКлиентПереопределяемый.ОбработкаРасшифровки(ЭтотОбъект, Элементы.ОтчетТабличныйДокумент, Расшифровка, СтандартнаяОбработка);
...
Процедура ОткрытьОтчетРасшифровку(Расшифровка, Данные, Стр)
	...
	МодульВариантыОтчетовКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ВариантыОтчетовКлиент");
	МодульВариантыОтчетовКлиент.ОткрытьФормуОтчета(ЭтаФорма, Стр.Вариант, ПараметрыОткрытия);
...
Процедура ДобавитьОформление(Поле, Оформление)
	...
	сфпОтчетыКлиентСервер.ДобавитьУсловноеОформление(Отчет.КомпоновщикНастроек.Настройки, Поле, Оформление);
	ОтметитьНеактуальным();
...
Процедура ВидДиаграммыНачалоВыбораЗадержка()
	...
	Меню.Добавить(Неопределено, НСтр("ru = 'Не выводить'; en = 'Не выводить'"),, БиблиотекаКартинок.сфпПустая);
	Меню.Добавить(ТипДиаграммы.График, НСтр("ru = 'Графики'; en = 'Schedules'"),, КартинкаДиаграммы(ТипДиаграммы.График));
	Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ru = 'Обычные гистограммы'; en = 'Regular histograms'"),, КартинкаДиаграммы(ТипДиаграммы.Гистограмма));
	Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ru = 'Горизонт. гистограммы'; en = 'Horizontal histogram'"),, КартинкаДиаграммы(ТипДиаграммы.ГистограммаГоризонтальная));
	Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ru = 'Круговые диаграммы'; en = 'Pie charts'"),, КартинкаДиаграммы(ТипДиаграммы.Круговая));
...
Процедура ВидДиаграммыВыборГруппы(Значение, ДополнительныеДанные) Экспорт
	...
	Если Значение.Значение=ТипДиаграммы.График Тогда
		Меню.Добавить(ТипДиаграммы.График, НСтр("ru = 'Обычный'; en = 'Usual'"));
		Меню.Добавить(ТипДиаграммы.ГрафикПоШагам, НСтр("ru = 'По шагам'; en = 'Step by step'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСНакоплением, НСтр("ru = 'С накоплением'; en = 'With accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластями, НСтр("ru = 'С областями'; en = 'With regions'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиИНакоплением, НСтр("ru = 'С областями и накоплением'; en = 'With areas and accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиНормированный, НСтр("ru = 'Нормированный'; en = 'Standardized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Гистограмма Тогда
		...
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированная, НСтр("ru = 'Нормированная'; en = 'Standardized'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаОбъемная, НСтр("ru = 'Объемная'; en = 'Volumetric'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемОбъемная, НСтр("ru = 'Объемная с накоплением'; en = 'Volumetric with accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяОбъемная, НСтр("ru = 'Объемная нормированная'; en = 'Volumetric normalized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.ГистограммаГоризонтальная Тогда
		...
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальная, НСтр("ru = 'Нормированная'; en = 'Standardized'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальнаяОбъемная, НСтр("ru = 'Объемная'; en = 'Volumetric'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная, НСтр("ru = 'Объемная с накоплением'; en = 'Volumetric with accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная, НСтр("ru = 'Объемная нормированная'; en = 'Volumetric normalized'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Круговая Тогда
		Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ru = 'Обычная'; en = 'Normal'"));
		Меню.Добавить(ТипДиаграммы.КруговаяОбъемная, НСтр("ru = 'Объемная'; en = 'Volumetric'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Изометрическая Тогда
		...
		Меню.Добавить(ТипДиаграммы.ИзометрическаяНепрерывная, НСтр("ru = 'Непрерывная'; en = 'Continuous'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяЛента, НСтр("ru = 'Лента'; en = 'Лента'"));
...
Функция КартинкаДиаграммы(ВидДиаграммы)
	...
	Если ВидДиаграммы=ТипДиаграммы.График Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_График;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикПоШагам Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикПоШагам;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикСНакоплением Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикСНакоплением;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикСОбластями Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикСОбластями;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикСОбластями Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикСОбластями;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикСОбластямиИНакоплением Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикСОбластямиИНакоплением;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГрафикСОбластямиНормированный Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГрафикСОбластямиНормированный;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Гистограмма Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Гистограмма;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаСНакоплением Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаСНакоплением;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаНормированная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаНормированная
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаСНакоплениемОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаСНакоплениемОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаНормированнаяОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаНормированнаяОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаГоризонтальная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаГоризонтальная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаСНакоплениемГоризонтальная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаСНакоплениемГоризонтальная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаНормированнаяГоризонтальная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаНормированнаяГоризонтальная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаГоризонтальнаяОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаГоризонтальнаяОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаСНакоплениемГоризонтальнаяОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ГистограммаНормированнаяГоризонтальнаяОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Круговая Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Круговая;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.КруговаяОбъемная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_КруговаяОбъемная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Изометрическая Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Изометрическая;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ИзометрическаяНепрерывная Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ИзометрическаяНепрерывная;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ИзометрическаяЛента Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ИзометрическаяЛента;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ИзометрическаяПирамида Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ИзометрическаяПирамида;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Водопад Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Водопад;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.КаркаснаяПоверхность Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_КаркаснаяПоверхность;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Поверхность Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Поверхность;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ВыпуклаяПоверхность Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ВыпуклаяПоверхность;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ВогнутаяПоверхность Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ВогнутаяПоверхность;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.ЗатененнаяПоверхность Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_ЗатененнаяПоверхность;
	ИначеЕсли ВидДиаграммы=ТипДиаграммы.Сотовая Тогда
		Возврат БиблиотекаКартинок.сфпТипДиаграммы_Сотовая;
	Иначе
...
Процедура ДиаграммаСерииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение="<Прочие>" Тогда
		...
		ОткрытьФорму("ОбщаяФорма.сфпФормаСпискаПолей", СтруктураПараметров, ЭтаФорма,,,, Оповещение);
...
Функция СписокБыстрыхГруппировок()
	...
	ДобавленныеГруппировки = СписокДобавленныхПолей();
	Список = Новый СписокЗначений;
...
Процедура ДобавитьСтрокуЗавершение(Элемент, ДополнительныеДанные) Экспорт
	...
	Иначе
		СписокПолей = СписокДобавленныхПолей();
...
Функция СписокДополнительныхКолонок()
	...
	Для каждого Показатель Из Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		Если НастройкиОтчета.Свойство("ТолькоРесурсыОстатков") И НастройкиОтчета.ТолькоРесурсыОстатков Тогда
			ПолеСхемы = сфпОтчеты.ПолеСхемы(Схема, ИмяПоля);
...
Процедура ДобавитьКолонкуЗавершение(Элемент, ДополнительныеДанные) Экспорт
	...
	Иначе
		СписокПолей = СписокДобавленныхПолей(Истина, Истина);
		Оповещение = Новый ОписаниеОповещения("ДобавитьПрочиеПоля", ЭтотОбъект);
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("Режим", "ПоляВыбор");
		СтруктураПараметров.Вставить("СуществующиеПоля", СписокПолей);
		СтруктураПараметров.Вставить("АдресСхемы", НастройкиОтчета.АдресСхемы);
		СтруктураПараметров.Вставить("АдресНастроек", ПоместитьВоВременноеХранилище(Отчет.КомпоновщикНастроек.Настройки, УникальныйИдентификатор));
		ОткрытьФорму("ОбщаяФорма.сфпФормаСпискаПолей", СтруктураПараметров, ЭтаФорма,,,, Оповещение);
...
&НаКлиенте
Процедура ДобавитьПрочиеПоля(ПолноеИмя, ИмяГруппировки) Экспорт
	
	Если ПолноеИмя=Неопределено Тогда
		Возврат;
	КонецЕсли;
	ИмяЭлемента = ДобавитьОписаниеПоказателя(ПолноеИмя);
	Если НЕ ПустаяСтрока(ИмяЭлемента) Тогда
		ПриИзмененииПоказателя(ИмяЭлемента);
	КонецЕсли; 
...
Процедура ПриИзмененииПоляСравнения()
	...
	Если НЕ сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ПолеСравнения) = "Периодичность" Тогда
		ПоказыватьАбсолютноеИзменение = Ложь;
		ПоказыватьОтносительноеИзменение = Ложь;
	КонецЕсли;
...
Процедура СвернутьРазвернутьПанельНастроек(Элемент)
	...
	Элементы.СкрытьПоказатьНастройки.Заголовок = ?(НовоеЗначениеВидимость, НСтр("ru = 'Скрыть настройки отчета'; en = 'Hide report settings'"), НСтр("ru = 'Показать настройки отчета'; en = 'Show report settings'"));
...
Процедура Подключаемый_ФильтрПриИзменении(Элемент)
	...
	Иначе
		...
		Если МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Стр.ДополнительныеПараметры, "СвязьПоПредставлению", Ложь) Тогда
...
Процедура Подключаемый_ФильтрНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// +СофтФон
	Если Стр.Поле = "Сотрудник" И сфпКонтролируемыеПользователи.Количество() > 0 Тогда
		ПараметрыОткрытия.Отбор.Вставить("Ссылка", сфпКонтролируемыеПользователи);
	КонецЕсли;
	// -СофтФон
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов=ГруппыИЭлементы.Элементы Тогда
		...
		Элемент.Подсказка = НСтр("ru = 'Используется выбор групп. Нажмите для выбора элементов.'; en = 'Group selection is used. Click to select items.'");
		Элемент.Картинка = БиблиотекаКартинок.сфпРежимВыбораГрупп;
	Иначе
		ЭлементПоле.ВыборГруппИЭлементов = ГруппыИЭлементы.Элементы;
		ЭлементПоле.ПодсказкаВвода = Стр.Заголовок;
		Элемент.Подсказка = НСтр("ru = 'Используется выбор элементов. Нажмите для выбора групп.'; en = 'Element selection is used. Click to select groups.'");
		Элемент.Картинка = БиблиотекаКартинок.сфпРежимВыбораЭлементов;
	КонецЕсли;
...
Процедура ОтправитьПоПочте(Команда)
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ТекстВопроса = НСтр("ru = 'Отчет не сформирован. Сформировать?'; en = 'Report is not generated. Generate?'");
...
Процедура РаскрытьНаВесьЭкран(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.сфпФормаПредпросмотраОтчета", ПараметрыОткрытия, ЭтаФорма);
...
Процедура ДобавитьСтроку(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокСтрок Тогда
		Список.Добавить(Неопределено, НСтр("ru = 'Прочие поля'; en = 'Other fields'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ДобавитьКолонку(Команда)
	...
	Если ДетальныеЗаписиИспользуются И НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокСтрок Тогда
		Список.Добавить(Неопределено, НСтр("ru = 'Прочие поля'; en = 'Other fields'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
	Если Список.Количество()=1 Тогда
...
Процедура Подключаемый_Команда(Команда)
	...
	Если ПостояннаяКоманда <> Неопределено И ЗначениеЗаполнено(ПостояннаяКоманда.Представление) Тогда
		...
		КлиентскийМодуль = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль(МассивПодстрок[0]);
	...
	Иначе
		МодульОтчетыКлиентПереопределяемый = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОтчетыКлиентПереопределяемый");
		МодульОтчетыКлиентПереопределяемый.ОбработчикКоманды(ЭтотОбъект, Команда, Ложь);
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура ПериодПриИзмененииСервер(Интерактивно = Ложь)
	
	ОбновитьПараметрыПериода(Интерактивно);
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ru = 'за все время'; en = 'all time'");
		//ИначеЕсли Элементы.ПредставлениеПериода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет Тогда
		//	ПредставлениеПериода = НСтр("ru = 'за '; en = 'за '")+НРег(ПредставлениеПериода(Отчет.Период.ДатаНачала, КонецДня(Период.ДатаОкончания)));
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ru = 'актуальные данные'; en = 'relevant data'");
		Иначе
...
Процедура ОбновитьЗаголовокОтчета()
	...
	Если РежимРасшифровки Тогда
		ЗаголовокОтчета = ЗаголовокОтчета + " (" + НСтр("ru = 'Расшифровка'; en = 'Explanation'") + ")";
	КонецЕсли; 
...
Процедура ПоказатьДиалогОтправкиПоПочте()
	...
	МодульОбщегоНазначенияКлиентСервер = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОбщегоНазначенияКлиентСервер");
	СписокВложений = МодульОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Вложение);
	
	МодульОбщегоНазначенияКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОбщегоНазначенияКлиент");
	
	Если МодульОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		МодульРаботаСПочтовымиСообщениямиКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("РаботаСПочтовымиСообщениямиКлиент");
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	СохраненВариантОтчета = (Событие="СохраненВариантОтчета");
	...
	Если ПолнаяИнициализация Тогда
		...
		Элементы.ПредставлениеПериодаКнопка.Видимость = НЕ (НастройкиОтчета.РежимПериода="БезПериода");
		Элементы.ПредставлениеПериодаКнопка.Заголовок = ?(НастройкиОтчета.РежимПериода="НаДату", 
		НСтр("ru = 'На дату'; en = 'For date'"), 
		НСтр("ru = 'Период'; en = 'Period'"));
	КонецЕсли;
	...
	Если ИзмененыПоказатели ИЛИ ИзмененыПериодыСравнения Тогда
		...
		Элементы.ГруппаВариантыСравнения.Видимость = ((КоличествоПоказателей=1 ИЛИ НЕ НастройкиОтчета.ПоказыватьПоказателиНаФормеОтчета) И сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ПолеСравнения)="Периодичность" И НастройкиОтчета.ИспользоватьДополнительныеРежимыСравнения);
	КонецЕсли;
	...
	Если ИзмененРежимРедактирования Тогда
		...
		КонецЦикла; 
		Элементы.РежимРедактированияСтрок.Картинка = ?(РежимРедактированияСтрок, БиблиотекаКартинок.сфпРежимРедактированияСпискаЭлементовАктивно, БиблиотекаКартинок.сфпРежимРедактированияСпискаЭлементов);
		Элементы.РежимРедактированияКолонок.Картинка = ?(РежимРедактированияКолонок, БиблиотекаКартинок.сфпРежимРедактированияСпискаЭлементовАктивно, БиблиотекаКартинок.сфпРежимРедактированияСпискаЭлементов);
	КонецЕсли;
	...
	Если СохраненВариантОтчета Тогда
		Элементы.Вариант.Видимость = (Элементы.Вариант.СписокВыбора.Количество()>1) И НЕ РежимРасшифровки;
	КонецЕсли;
...
Функция ТаблицаВариантов(ТолькоВидимые = Истина)
	...
	ИначеЕсли ТолькоВидимые Тогда
		...
		Запрос.Параметры.Вставить("ОтключенныеПредопределенныеВарианты", сфпОтчетыПовтИсп.ПредопределенныеВариантыНедоступныеПоФО());
		Запрос.Параметры.Вставить("ОтключенныеВарианты", сфпОтчетыПовтИсп.ВариантыНедоступныеПоФО());
		Запрос.Параметры.Вставить("ТекущийПользователь", сфпСофтФонПроСервер.сфпТекущийПользователь());
		Запрос.Параметры.Вставить("Контекстный", НастройкиОтчета.Контекстный);
		
		Если Метаданные.Справочники["ВариантыОтчетов"].Реквизиты.Найти("ТолькоДляАвтора") <> Неопределено Тогда
			ДопУсловие = "
...
Функция НавигационнаяСсылкаВарианта(НастройкиОтчета)
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(НастройкиОтчета.ВариантСсылка) <> "ВариантыОтчетов" Тогда
		Возврат "";
	КонецЕсли;
...
Процедура МодифицироватьСхему(СхемаКомпоновкиДанных)
	...
	сфпОтчеты.СтандартизироватьСхему(СхемаКомпоновкиДанных);
	сфпОтчеты.ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных);
	
	// Запрет непосредственного использования поля ДинамическийПериод 
	Поле = сфпОтчеты.ПолеСхемы(СхемаКомпоновкиДанных, "ДинамическийПериод");
...
Процедура ОпределитьПараметрыОтчетаПоУмолчанию()
	...
	Для Каждого Итератор Из ТаблицаВариантов(Ложь) Цикл
		...
		сфпОтчеты.ИнициализироватьНастройкиВарианта(НастройкиВариантов[Итератор.КлючВарианта]);
	КонецЦикла;
	...
	Иначе
		...
		сфпОтчеты.ИнициализироватьНастройкиВарианта(СтруктураНастроек);
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) И НЕ ПараметрыПериодов.Период=Неопределено И НастройкиОтчета.ПользовательскиеНастройки=Неопределено Тогда
		...
		Если ТипЗнч(ПериодИзНастроек)=Тип("СтандартныйПериод") Тогда
			...
			КонецЕсли; 
		Иначе
			Если ЗначениеЗаполнено(ПараметрыПериодов.Период.ИдентификаторПользовательскойНастройки) Тогда
				ПараметрПериод = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ПараметрыПериодов.Период.ИдентификаторПользовательскойНастройки).Значение;
			Иначе
				ПараметрПериод = ПараметрыПериодов.Период.Значение;
			КонецЕсли;
			Если ПараметрПериод<>Неопределено Тогда
				Если ТипЗнч(ПараметрПериод)=Тип("СтандартныйПериод") Тогда
					Период.Вариант = ПараметрПериод.Вариант;
					Период.ДатаНачала = ПараметрПериод.ДатаНачала;
					Период.ДатаОкончания = ПараметрПериод.ДатаОкончания;
				ИначеЕсли ТипЗнч(ПараметрПериод)=Тип("Дата") Тогда
					Период.ДатаОкончания = ПараметрПериод;
				КонецЕсли; 
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли; 
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		Если ТипЗнч(Значение)=Тип("Структура") Тогда
			Если Значение.Свойство("ПолеКомпоновкиДанных") Тогда
				Имя = Значение.ПолеКомпоновкиДанных;
			КонецЕсли;
		...
		Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеОтбораКомпоновкиДанных") Тогда
			Если ВидСравненияКД = ВидСравненияКомпоновкиДанных.Равно 
				И ДоступноеПолеКД.Тип.Типы()[0] = Тип("Дата")
				И ТипЗнч(Значение) = Тип("СтандартныйПериод") Тогда
				
				УстановитьЗначениеФильтра(Имя, Значение, ВидСравненияКД);
			Иначе
				МодульОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ОтборыКД, Имя, Значение, ВидСравненияКД, , Истина, Обычный, "");
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		
		// Фильтр не установлен
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось установить фиксированный отбор ""%1"".'; en = 'Failed to set fixed filter ""%1"".'"), Имя);
		ЗаписьЖурналаРегистрации(
		"CRM_ФормаОтчетаНовая.УстановитьПараметрыИОтборыРасшифровки",
...
Процедура ВыбратьКолонкиРасшифровки()
	...
	Если ЗначениеЗаполнено(ФормаПараметры.Колонки) Тогда
		...
		Если ТипЗнч(ФормаПараметры.Колонки)=Тип("Строка") Тогда
			МассивКолонок = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ФормаПараметры.Колонки);
		Иначе
...
Функция ОпределитьПараметрыПериодовСКД(ОтчетОбъект)
	...
	Если ПараметрПериод=Неопределено Тогда
		ПараметрПериод = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ПериодОтчета");
	КонецЕсли;
	Если ПараметрПериод=Неопределено Тогда
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		МодульДлительныеОперацииКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ДлительныеОперацииКлиент");
		МодульДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчика);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если Отчет.КомпоновщикНастроек.Настройки.Структура.Количество()=0 Тогда
		ПоказатьОшибкиФормирования(НСтр("ru = 'Не выбрано ни одной группировки'; en = 'No groups selected'"));
		Возврат Ложь;
	КонецЕсли; 
	...
	ФормированиеПриОткрытии = МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДопСвойства, "ФормированиеПриОткрытии", Ложь);
	...
	// {УАТ} СофтФон ++
	ПараметрыФормированияОтчета.Вставить("ПараметрыИзменены", Ложь);
	// {УАТ} СофтФон --
	...
	МодульДлительныеОперации = сфпОбщегоНазначения.сфпОбщийМодуль("ДлительныеОперации");
	
	ПараметрыЗапуска = МодульДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Варианты отчетов: Формирование отчета'; en = 'Report Options: Generating a Report'");
	...
	Иначе
		...
		ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.сфпДлительнаяОперация;
...
Функция ФоновоеЗаданиеВыполнено()
	...
	Попытка
		ЗаданиеВыполнено = МодульДлительныеОперации.ЗаданиеВыполнено(ФоновоеЗаданиеИдентификатор);
	Исключение
		ПоказатьОшибкиФормирования(ИнформацияОбОшибке());
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ФоновоеЗаданиеОтменить(ФоновоеЗаданиеИдентификатор)
	МодульДлительныеОперации = сфпОбщегоНазначения.сфпОбщийМодуль("ДлительныеОперации");
	МодульДлительныеОперации.ОтменитьВыполнениеЗадания(ФоновоеЗаданиеИдентификатор);
КонецПроцедуры
...
Процедура ФоновоеЗаданиеЗагрузитьРезультат()
	...
	Успех = МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатФормирования, "Успех");
...
Процедура ОтметитьНеактуальным()
	...
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.'; en = 'The report has not been generated. Click ""Generate"" (F5) to generate the report.'");
	ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
Процедура Сформировать()
	...
	МодульОценкаПроизводительностиКлиентСервер = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОценкаПроизводительностиКлиент");
	...
	ИдентификаторЗамера = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(Истина, ИмяОперации + " (формирование)");
	// СтандартныеПодсистемы.ОценкаПроизводительности	
	
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если ЗначениеЗаполнено(Период.ДатаНачала) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru = 'за '; en = 'за '")+ПредставлениеПериода;
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru = 'текущий момент'; en = 'current moment'");
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НастройкиОтчета.НаименованиеВарианта+НСтр("ru = ' на '; en = ' to'")+ПредставлениеДляЗаголовка);
	Иначе
	...
	Иначе
		...
		Если ТребуетсяОбработчик Тогда
			МодульДлительныеОперацииКлиент = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ДлительныеОперацииКлиент");
			МодульДлительныеОперацииКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчика);
...
Процедура ОперацииПослеФормирования()
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	МодульОценкаПроизводительностиКлиентСервер = сфпОбщегоНазначенияКлиент.сфпОбщийМодуль("ОценкаПроизводительностиКлиент");
	МодульОценкаПроизводительностиКлиентСервер.ЗавершитьЗамерВремени(ИдентификаторЗамера);
	// СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура СвернутьГруппировки()
	...
	Если НЕ ПустаяСтрока(РазвернутьДо) Тогда
		
		КоличествоГруппировок = Элементы.РазвернутьДо.СписокВыбора.Количество()-1;
		
		ЭлементСписка = Элементы.РазвернутьДо.СписокВыбора.НайтиПоЗначению(РазвернутьДо);
		Если ЭлементСписка<>Неопределено Тогда
			КоличествоИтераций = КоличествоГруппировок-Элементы.РазвернутьДо.СписокВыбора.Индекс(ЭлементСписка)+1;
		Иначе
			КоличествоИтераций = 0;
		КонецЕсли;
		Для ии = 1 По КоличествоИтераций Цикл
			ОтчетТабличныйДокумент.ПоказатьУровеньГруппировокСтрок(КоличествоГруппировок-ии);
		КонецЦикла; 
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Ошибка при формировании:'; en = 'Generation error:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	...
	Если Не ПустаяСтрока(ПодробноеПредставлениеОшибки) Тогда
		...
		Если НастройкиОтчета.Свойство("ВариантСсылка") Тогда
			МодульВариантыОтчетов.ЗаписатьВЖурнал(Уровень, ПодробноеПредставлениеОшибки, НастройкиОтчета.ВариантСсылка);
		ИначеЕсли Параметры.Свойство("Ключ") Тогда
			МодульВариантыОтчетов.ЗаписатьВЖурнал(Уровень, ПодробноеПредставлениеОшибки, Параметры.Ключ);
		ИначеЕсли Параметры.Свойство("Отчет") Тогда
			МодульВариантыОтчетов.ЗаписатьВЖурнал(Уровень, ПодробноеПредставлениеОшибки, МодульОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Параметры.Отчет));
		Иначе
			МодульВариантыОтчетов.ЗаписатьВЖурнал(Уровень, ПодробноеПредставлениеОшибки);
		КонецЕсли; 
	КонецЕсли;
		...
		Если ТипЗнч(ЭлементИсточник)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(ЭлементИсточник)=Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			Для каждого ПолеИсточник Из ЭлементИсточник.ПоляГруппировки.Элементы Цикл
				...
				Если ТипЗнч(ПолеИсточник)=Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
					...
					ПолеСхемы = сфпОтчеты.ПолеСхемы(СхемаКомпоновкиДанных, ПолеИсточник.Поле);
...
&НаСервере
Процедура ОбновитьПараметрыПериода(Интерактивно = Ложь)
		...
		Если ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") 
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				...
				Если НЕ ИмяПоля="ДинамическийПериод" И НЕ ИмяПоля="МесяцГода" И НЕ ИмяПоля="ДеньМесяцГода" И НЕ ИмяПоля="НеделяГода" Тогда
					...
					ПолеСхемы = сфпОтчеты.ПолеСхемы(СхемаКД, Поле.Поле);
...
Процедура ОбновитьНастройкиДиаграммыНаФорме()
	...
	Если ИмяТочки="ДинамическийПериод" Тогда
		...
		Если ПараметрДанныхПериодичность=Неопределено ИЛИ НЕ ПараметрДанныхПериодичность.Использование ИЛИ НЕ ЗначениеЗаполнено(ПараметрДанныхПериодичность.Значение) Тогда
			ИмяТочки = сфпОтчеты.ПолучитьЗначениеПериодичности(Период.ДатаНачала, Период.ДатаОкончания);
		Иначе
	...
	Если ЗначениеЗаполнено(ИмяТочки) Тогда
		Если ДоступныеТочки.НайтиПоЗначению(ИмяТочки)=Неопределено Тогда
			Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ИмяТочки)="Периодичность" Тогда
...
Процедура ЗаполнитьВариантыСерийТочекЗначений()
	...
	ДоступныеСерии.Добавить(Неопределено, НСтр("ru = 'Не использовать'; en = 'None'"));
	ДоступныеТочки.Добавить(Неопределено, НСтр("ru = 'Не использовать'; en = 'None'"));
	
	ПоляПериодичностиТочки = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("ПериодГод,ПериодПолугодие,ПериодКвартал,ПериодМесяц,МесяцГода,ПериодНеделя,НеделяГода,ПериодДень,ДеньМесяца,ДеньНедели");
	ПоляПериодичностиСерии = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("ПериодГод,ПериодПолугодие,ПериодКвартал,ПериодМесяц");
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		ДоступныеСерии.Добавить("<Прочие>", НСтр("ru = 'Прочее'; en = 'Other'"),, БиблиотекаКартинок.Выбрать);
		ДоступныеТочки.Добавить("<Прочие>", НСтр("ru = 'Прочее'; en = 'Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ОбновитьНастройкиДиаграммы()
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ДиаграммаТочки)="Периодичность" Тогда
...
Функция ОбновитьДополнениеГруппировки(Группировка, Схема)
	...
	Если ИмяПоля="ДинамическийПериод" Тогда
		...
		Если НЕ ПараметрСхемыПериодичность=Неопределено И ЗначениеЗаполнено(ПараметрСхемыПериодичность.Значение) Тогда
			ИмяПериодичности = МодульОбщегоНазначения.ИмяЗначенияПеречисления(ПараметрСхемыПериодичность.Значение);
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		Для каждого Элемент Из Стр.Реквизиты Цикл
			...
			Если Стр.Тип=ТипФильтр() 
				И МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(Стр.ТипЗначения, Тип("Булево")) 
				И НЕ Стр.ВидЭлемента="Флажок" 
				И НЕ Стр.ВидЭлемента="Булево" Тогда
				ОписаниеТипов = Новый ОписаниеТипов("ПеречислениеСсылка.сфпБулевоТриСостояния");
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Стр.Тип=ТипПараметр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			сфпОтчеты.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, Неопределено);
		ИначеЕсли Стр.Тип=ТипФильтр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			сфпОтчеты.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, СвязиПараметровВыбора);
		КонецЕсли; 
		Стр.Создан = Истина;
	КонецЦикла;
		...
		Если Стр.ВидЭлемента="Тумблер" И Стр.ТипЗначения.СодержитТип(Тип("Булево")) Тогда
			Для каждого Реквизит Из Стр.Реквизиты Цикл
				Если Стр.ЗначениеПоУмолчанию=Истина Тогда
					ЭтаФорма[Реквизит.Ключ] = Перечисления.сфпБулевоТриСостояния.Истина;
				ИначеЕсли Стр.ЗначениеПоУмолчанию=Ложь Тогда
					ЭтаФорма[Реквизит.Ключ] = Перечисления.сфпБулевоТриСостояния.Ложь;
				Иначе
					ЭтаФорма[Реквизит.Ключ] = Перечисления.сфпБулевоТриСостояния.НеИзменять;
				КонецЕсли; 
			КонецЦикла;
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				Если ТипЗнч(Поле)=Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
					Если Поле.Поле=Новый ПолеКомпоновкиДанных("ДинамическийПериод") Тогда
						ПараметрСхемыПериодичность = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Периодичность"));
						Если ТипЗнч(Элемент)=Тип("ГруппировкаДиаграммыКомпоновкиДанных") 
							И ПараметрСхемыПериодичность<>Неопределено 
							И ЗначениеЗаполнено(ПараметрСхемыПериодичность.Значение) Тогда
							// Заменяем периодичность в диаграммах непосредственно на поле нужного периода
							ИмяПериодичности = МодульОбщегоНазначения.ИмяЗначенияПеречисления(ПараметрСхемыПериодичность.Значение);
							Поле.Поле = Новый ПолеКомпоновкиДанных("Период"+ИмяПериодичности);
						КонецЕсли;
						Поле.Использование = Истина;
					КонецЕсли; 
				КонецЕсли; 
...
Процедура ЗаполнитьВариантыГруппировкиКолонок()
	...
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Очистить();
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("", НСтр("ru = 'Не группировать'; en = 'Do not group'"));
	 ...
	 Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		 Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			 ...
			 Если ЗначениеЗаполнено(Стр.Поле) Тогда
				 ПолеГруппировки = сфпОтчеты.ПолеСхемы(Схема, Стр.Поле);
			 ...
			 Иначе
				 Для каждого ПодСтр Из Стр.ПолучитьЭлементы() Цикл
					 Если НЕ ПодСтр.Тип=ТипРеквизит() Тогда
						 Продолжить;
					 КонецЕсли; 
					 ПолеГруппировки = сфпОтчеты.ПолеСхемы(Схема, ПодСтр.Поле);
					 Если ТипЗнч(ПолеГруппировки)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И НЕ ПолеГруппировки.Роль.НомерПериода=0 И ПолеГруппировки.Роль.НомерПериода<ПолеСхемы.Роль.НомерПериода Тогда
						 Индекс = ?(Индекс=Неопределено, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр), Мин(Индекс, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр)));
						 Прервать;
					 КонецЕсли;
...
Функция ПроверитьСовместимостьГруппировокКолонок(ИмяПоляПроверки = "")
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ПолеСравнения)="Периодичность" Тогда
		СтрокаДлительностьПериода = МодульОбщегоНазначения.ИмяЗначенияПеречисления(ПолеСравнения);
	...
	Если ЗначениеЗаполнено(ПолеГруппировкиКолонок) Тогда
		ГруппировкаКолонокСхемы = сфпОтчеты.ПолеСхемы(Схема, ПолеГруппировкиКолонок);
	...
	Если ЗначениеЗаполнено(ИмяСравнения) Тогда
		СравнениеСхемы = сфпОтчеты.ПолеСхемы(Схема, ИмяСравнения);
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		Поле = сфпОтчеты.ПолеСхемы(Схема, ИмяПоля);
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в колонках отчета'; en = 'in the report columns'");
			...
			Иначе
				...
				ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в строках отчета'; en = 'in the report lines'");
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
				...
				КонецЕсли;
			КонецЕсли; 
			сфпОбщегоНазначенияКлиентСервер.сфпСообщитьПользователю(сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1'; en = 'Using different period fields in rows and columns is prohibited. Please disable the %1 field first.'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли НЕ ЭтоСтрока И ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") И НРег(ИмяПоля)=НРег(ДополнительныеСвойства.Сравнение) Тогда
				...
				КонецЕсли; 
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") И ИмяПоля="ДинамическийПериод" И сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ДополнительныеСвойства.Сравнение)="Периодичность" Тогда
				КоличествоПоказателей = КоличествоПоказателейСКД(Отчет.КомпоновщикНастроек.Настройки.Выбор.Элементы);
				Если КоличествоПоказателей>1 Тогда
					// отключение режима сравнения
					ДополнительныеСвойства.Удалить("Сравнение");
	...
	Для каждого ДоступноеПоле Из МассивПолей Цикл
		...
		Для каждого Тип Из ДоступноеПоле.ТипЗначения.Типы() Цикл
			...
			Если Справочники.ТипВсеСсылки().СодержитТип(Тип) Тогда
				...
				ИмяМетаданных = сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(СсылкаТипа);
			КонецЕсли;
			
			Если ИмяМетаданных = "Валюты" ИЛИ
				 ИмяМетаданных = "СтруктураПредприятия" ИЛИ
				 ИмяМетаданных = "Организации" ИЛИ
				 ИмяМетаданных = "НаправленияДеятельности" ИЛИ
...
Функция ПроверитьВозможностьДобавленияГруппировки(Стр, СтрГруппа)
	...
	ПолеСхемы = сфпОтчеты.ПолеСхемы(Схема, Стр.Поле);
	Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		Для каждого ПодСтр Из СтрГруппа.ПолучитьЭлементы() Цикл
			ВложенноеПоле = сфпОтчеты.ПолеСхемы(Схема, ПодСтр.Поле);
...
&НаСервере
Процедура ДобавитьОтступ(ИмяРеквизита, Группа, Первый = Истина)
	
	КартинкаОтступ = Элементы.Найти(ИмяРеквизита+"_Отступ");
	Если КартинкаОтступ = Неопределено Тогда
		...
		Если Первый Тогда
			КартинкаОтступ.Картинка = БиблиотекаКартинок.сфпКоннекторВерхПравоНиз;
		Иначе
			КартинкаОтступ.Картинка = БиблиотекаКартинок.сфпКоннекторВерхНиз;
		КонецЕсли;
	Иначе
		Если Первый Тогда
			КартинкаОтступ.Картинка = БиблиотекаКартинок.сфпКоннекторВерхПравоНиз;
...
&НаСервере
Процедура УдалитьОтступ(ИмяРеквизита)
...
Процедура ДобавитьПереключательИерархииГруппировки(Стр, ИмяРеквизита, Группа)
	...
	Если ДоступноеПоле=Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если НЕ сфпОтчеты.ОписаниеТиповСодержитИерархическийТип(ДоступноеПоле.ТипЗначения) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьКоманды(Стр, ИмяРеквизита, Группа)
	...
	Кнопка.РастягиватьПоГоризонтали = Ложь;
	Кнопка.РастягиватьПоВертикали = Ложь;
	Кнопка.Картинка = БиблиотекаКартинок.сфпПереместитьВверх;
	...
	Кнопка.РастягиватьПоГоризонтали = Ложь;
	Кнопка.РастягиватьПоВертикали = Ложь;
	Кнопка.Картинка = БиблиотекаКартинок.сфпПереместитьВниз;
...
Процедура УстановитьПредставлениеКнопкиИерархииГруппировки(Кнопка, Стр)
	...
	Если Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
		...
		Кнопка.Подсказка = НСтр("ru = 'Отображать только элементы'; en = 'Show only items'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.ТолькоИерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.сфпРежимВыбораГрупп;
		Кнопка.Подсказка = НСтр("ru = 'Отображать только иерархию'; en = 'Show hierarchy only'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Иерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.сфпРежимВыбораГруппИЭлементов;
		Кнопка.Подсказка = НСтр("ru = 'Отображать элементы и иерархию'; en = 'Display elements and hierarchy'");
	КонецЕсли; 
...
Функция СписокПолейГруппировок(Сравнение = Ложь)
	...
	Для ии = 1 По ТекущееКоличество-1 Цикл
		Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(СписокВыбора[ТекущееКоличество-ии].Значение)="Периодичность" Тогда
			Продолжить;
		КонецЕсли; 
	...
	Если (НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок И НЕ Сравнение) 
		...
		СписокВыбора.Добавить("<Прочие>", НСтр("ru = 'Прочее'; en = 'Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Процедура ОбновитьГруппировкиРазворачивания()
	...
	Элементы.РазвернутьДо.СписокВыбора.Добавить("", НСтр("ru = '<По умолчанию>'; en = '<By default>'"));
...
Функция ПроверитьСовместимостьДиаграммы(ИмяПоляПроверки = "")
	...
	Если ЗначениеЗаполнено(ДиаграммаТочки) Тогда
		ПолеТочка = сфпОтчеты.ПолеСхемы(Схема, ДиаграммаТочки);
		...
		Если ЗначениеЗаполнено(ДиаграммаСерии) Тогда
			ПолеСерия = сфпОтчеты.ПолеСхемы(Схема, ДиаграммаСерии);
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в диаграмме'; en = 'in the diagram'");
			...
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ru = ' в структуре отчета'; en = 'in the report structure'");
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					ДиаграммаТочки = Неопределено;
				КонецЕсли; 
			...
			КонецЕсли; 
			сфпОбщегоНазначенияКлиентСервер.сфпСообщитьПользователю(сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1'; en = 'Using different period fields in a report or chart is prohibited. Please disable the %1 field first.'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ДобавитьЭлементГруппаПоказателейПоказатель(Стр)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Для ии = 1 По Отступ Цикл
			ДобавитьОтступ(ИмяРеквизита+ии, Группа, ии = 1);
		КонецЦикла; 
...
Функция СместитьПоказатель(Идентификатор, Шаг)
	...
	ВерхнийУровень = (Стр.ПолучитьРодителя()=Неопределено);
	...
	Если ИндексВМассиве+Шаг<0 ИЛИ ИндексВМассиве+Шаг>=Строки.Количество() Тогда
		Если ВерхнийУровень Тогда
			...
			Возврат Ложь;
		Иначе
			ВерхнийУровень = Стр.ПолучитьРодителя().ПолучитьРодителя();
			Если ВерхнийУровень = Неопределено Тогда
				СтрокиВерхнегоУровня = ПоляНастроек.ПолучитьЭлементы();
			Иначе	
				СтрокиВерхнегоУровня = ВерхнийУровень.ПолучитьЭлементы();
			КонецЕсли;
			Индекс = СтрокиВерхнегоУровня.Индекс(Стр.ПолучитьРодителя());
			ГруппаРодитель = Стр.ПолучитьРодителя().ИмяГруппы;
			Если Шаг > 0 Тогда
				Если СтрокиВерхнегоУровня.Количество() = Индекс+1 Тогда
					Элементы.Переместить(Группа, Элементы[ГруппаРодитель].Родитель); 
					НовСтрока = СтрокиВерхнегоУровня.Добавить();
					ПередЭлементом = Неопределено;
				Иначе
					ПередЭлементом = Элементы[ГруппаРодитель].Родитель.ПодчиненныеЭлементы.Найти(СтрокиВерхнегоУровня[Индекс+1].ИмяГруппы);
					Элементы.Переместить(Группа, Элементы[ГруппаРодитель].Родитель, ПередЭлементом); 
					НовСтрока = СтрокиВерхнегоУровня.Вставить(Индекс+1);
				КонецЕсли;
			Иначе
				ПередЭлементом = Элементы[ГруппаРодитель];
				Элементы.Переместить(Группа, Элементы[ГруппаРодитель].Родитель, ПередЭлементом); 
				НовСтрока = СтрокиВерхнегоУровня.Вставить(Индекс);
			КонецЕсли;
			Если ЭтоГруппа Тогда
				Элементы.Переместить(ГруппаПоказателей, Элементы[ГруппаРодитель].Родитель, ПередЭлементом); 
			КонецЕсли; 
			Отступ = УровеньОтступа(НовСтрока);
			ИмяРеквизита = СтрЗаменить(Группа.Имя, "_Группа", "");
			УдалитьОтступ(ИмяРеквизита+(Отступ+1));
			Для ии = 1 По Отступ Цикл
				ДобавитьОтступ(ИмяРеквизита+ии, Группа, ии = 1);
			КонецЦикла; 
			ЗаполнитьЗначенияСвойств(НовСтрока, Стр);
			СтрокиРодителя.Удалить(Стр);
		КонецЕсли;
	ИначеЕсли ИндексВМассиве+Шаг=Строки.Количество()-1 Тогда
		СтрСоСмещением = Строки[ИндексВМассиве+Шаг];
		Если СтрСоСмещением.Тип = ТипПоказатель() Тогда
			// Добавление в конец
			...
			КонецЕсли; 
		Иначе
			СтрокиГруппы = СтрСоСмещением.ПолучитьЭлементы();
			Элементы.Переместить(Группа, Элементы[СтрЗаменить(СтрСоСмещением.ИмяГруппы, "_Группа", "_Поля")], ?(СтрокиГруппы.Количество()>0, Элементы[СтрокиГруппы[0].ИмяГруппы], Неопределено)); 
			Если ЭтоГруппа Тогда
				Элементы.Переместить(ГруппаПоказателей, Элементы[СтрЗаменить(СтрСоСмещением.ИмяГруппы, "_Группа", "_Поля")], ?(СтрокиГруппы.Количество()>0, Элементы[СтрокиГруппы[0].ИмяГруппы], Неопределено)); 
			КонецЕсли; 
			НовСтрока = СтрокиГруппы.Вставить(0);
			Отступ = УровеньОтступа(НовСтрока);
			ИмяРеквизита = СтрЗаменить(Группа.Имя, "_Группа", "");
			Для ии = 1 По Отступ Цикл
				ДобавитьОтступ(ИмяРеквизита+ии, Группа, ии = 1);
	...
	Иначе
		СтрСоСмещением = Строки[ИндексВМассиве+Шаг];
		Если СтрСоСмещением.Тип = ТипГруппаПоказателей() Тогда
			СтрокиГруппы = СтрСоСмещением.ПолучитьЭлементы();
			Если Шаг > 0 Тогда
				Элементы.Переместить(Группа, Элементы[СтрЗаменить(СтрСоСмещением.ИмяГруппы, "_Группа", "_Поля")], ?(СтрокиГруппы.Количество()>0, Элементы[СтрокиГруппы[0].ИмяГруппы], Неопределено)); 
				Если ЭтоГруппа Тогда
			...
			Иначе
				...
				Элементы.Переместить(Группа, Элементы[СтрЗаменить(СтрСоСмещением.ИмяГруппы, "_Группа", "_Поля")]); 
				Если ЭтоГруппа Тогда
					Элементы.Переместить(ГруппаПоказателей, ГруппаПоказателей.Родитель); 
				КонецЕсли; 
			КонецЕсли;
...
Процедура ЗаполнитьВариантыСравнения()
	...
	Элементы.ПолеСравнения.СписокВыбора.Очистить();
	Элементы.ПолеСравнения.СписокВыбора.Добавить(Неопределено, НСтр("ru = 'Не сравнивать'; en = 'Do not compare'"));
	
	Если НастройкиОтчета.ИспользоватьПериодичность Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.День, НСтр("ru = 'Дни'; en = 'Days'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.Неделя, НСтр("ru = 'Недели'; en = 'Weeks'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.Месяц, НСтр("ru = 'Месяцы'; en = 'Months'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.Квартал, НСтр("ru = 'Кварталы'; en = 'Quarters'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.Полугодие, НСтр("ru = 'Полугодия'; en = 'Half-year'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.сфпПериодичность.Год, НСтр("ru = 'Годы'; en = 'Years'"));
	КонецЕсли;
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихПолейСравнения Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить("<Прочие>", НСтр("ru = 'Прочее'; en = 'Other'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Процедура ДобавитьГруппировкуСравнения(СтруктураКолонок)
	...
	Если сфпСофтФонПроСервер.сфпПолучитьИмяМетаданных(ПолеСравнения)="Периодичность" Тогда
		...
		СхемаКомпоновкиДанных = ПолучитьИзВременногоХранилища(НастройкиОтчета.АдресСхемы);
		Поле = сфпОтчеты.ПолеСхемы(СхемаКомпоновкиДанных, ИмяПоля);
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	Если Стр.ВидЭлемента = "Период" И Стр.Реквизиты.Количество()>0 Тогда 
		Для каждого Реквизит из Стр.Реквизиты Цикл
			Идентификатор = Реквизит.Ключ;
			Прервать;
		КонецЦикла;
	Иначе
		Идентификатор = НовоеИмяРеквизита(ТипФильтр());
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Строка")) Тогда
		...
		Стр.ВидЭлемента = "Поле";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Дата")) ИЛИ Стр.ВидЭлемента = "Период" Тогда
		// Дата
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.Равно Тогда
			Если ТипЗнч(Значение) = Тип("СтандартныйПериод") Тогда
				Стр.Реквизиты[Идентификатор].ДатаНачала = Значение.ДатаНачала;
				Стр.Реквизиты[Идентификатор].ДатаОкончания = Значение.ДатаОкончания;
			Иначе
		...
		ИначеЕсли (ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.БольшеИлиРавно
			ИЛИ ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.Больше) Тогда
		...
		ИначеЕсли (ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.МеньшеИлиРавно
			ИЛИ ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.Меньше) Тогда
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.сфпБулевоТриСостояния.НеИзменять, НСтр("ru = 'Все'; en = 'All'"), Значение=Неопределено);
		Значения.Добавить(Перечисления.сфпБулевоТриСостояния.Ложь, Формат(Ложь, ДоступноеПоле.ФорматРедактирования), Значение=Истина);
		Значения.Добавить(Перечисления.сфпБулевоТриСостояния.Истина, Формат(Истина, ДоступноеПоле.ФорматРедактирования), Значение=Ложь);
		Стр.Реквизиты.Вставить(Идентификатор, Значения);
		Стр.ТипЗначения = Новый ОписаниеТипов("Булево,Неопределено");
	ИначеЕсли МодульОбщегоНазначения.ЭтоСсылка(ТипЗначения.Типы().Получить(0)) Тогда
		// Ссылочный тип
		ПараметрыВыбора = ОпределитьПараметрыВыбора(ДоступноеПоле);
		КоличествоЭлементов = сфпОтчеты.ОпределитьКоличествоЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
		Если (КоличествоЭлементов=0 ИЛИ КоличествоЭлементов=1) И НЕ РучноеДобавление Тогда
			Возврат;
		КонецЕсли;
		Если КоличествоЭлементов<3 Тогда
			Значения = сфпОтчеты.ПолучитьСписокЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
			СтрокаЗаголовка = "";
	...
	ИначеЕсли Стр.ВидЭлемента="Тумблер" Тогда
		...
		Для каждого Реквизит Из Стр.Реквизиты Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗначениеРеквизита) ИЛИ 
				ЗначениеРеквизита=ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.НеИзменять") Тогда
				Продолжить;
			КонецЕсли;
			Если ТипЗнч(ЗначениеРеквизита)=Тип("ПеречислениеСсылка.сфпБулевоТриСостояния") Тогда
				Значения.Добавить(?(
				ЗначениеРеквизита=ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.Истина"), Истина,?(
				ЗначениеРеквизита=ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.Ложь"), Ложь,
				Неопределено)));
			Иначе
...
Процедура ОбновитьРеквизитыФильтровВПанелиПоПредставлению(Представление, Значение)
	...
	Строки = СтрокиДерева(ПоляНастроек, СтруктураОтбора);
	Для каждого Стр Из Строки Цикл
		Если НЕ МодульОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Стр.ДополнительныеПараметры, "СвязьПоПредставлению", Ложь) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьРеквизитыФильтровВПанели(Поле, Значение, ВидСравнения = Неопределено, Удаление = Ложь)
	...
	ИначеЕсли Стр.ВидЭлемента="Тумблер" Тогда
		Если Стр.ТипЗначения.Типы().Количество()=1 И Стр.ТипЗначения.Типы().Получить(0)=Тип("Булево") Тогда
			Если Удаление Тогда
				НовоеЗначение = ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.НеИзменять");
			Иначе 
	...
	Для каждого Стр Из Строки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Параметр.Значение) Тогда
			...
			Для каждого Элемент Из Стр.Элементы Цикл
				сфпОбщегоНазначенияКлиентСервер.сфпСообщитьПользователю(
				сфпСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не заполнен обязательный параметр ""%1""",
...
Процедура ОпределитьЭлементыИРеквизитыПараметра(Стр, Параметр = Неопределено, ДоступныйПараметр)
	...
	Если Метаданные.Справочники.Найти("Организации") <> Неопределено Тогда
		Если ТипЗначения.СодержитТип(Тип("СправочникСсылка." + "Организации")) Тогда
			Если Метаданные.ФункциональныеОпции.Найти("ИспользоватьНесколькоОрганизаций") <> Неопределено Тогда
				ИспользоватьНесколькоОрганизаций = Ложь;
				Попытка
					ИспользоватьНесколькоОрганизаций = Вычислить("ПолучитьФункциональнуюОпцию(""ИспользоватьНесколькоОрганизаций"");");
	...
	ИначеЕсли ТипЗначения.Типы().Количество()>1 Тогда
		...
		Стр.ВидЭлемента = "Поле";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТипЗначения, Тип("Строка")) Тогда
		// Строка
		Стр.ВидЭлемента = "Поле";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТипЗначения, Тип("Дата")) Тогда
		// Дата
		Стр.ВидЭлемента = "Поле";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТипЗначения, Тип("СтандартныйПериод")) Тогда
		// Дата
		Стр.ВидЭлемента = "Период";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТипЗначения, Тип("Число")) Тогда
		// Число
		Стр.ВидЭлемента = "Поле";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТипЗначения, Тип("Булево")) Тогда
		// Булево
		Стр.ВидЭлемента = "Булево";
		Стр.Реквизиты.Вставить(Идентификатор, Значение);
	ИначеЕсли МодульОбщегоНазначения.ЭтоСсылка(ТипЗначения.Типы().Получить(0)) Тогда
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Структура") Тогда
			Если НЕ Стр.Значение.ЗначениеОт=Неопределено И НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru = 'от '; en = 'from'")+Строка(Стр.Значение.ЗначениеОт) + НСтр("ru = ' до '; en = 'before'") + Строка(Стр.Значение.ЗначениеДо);
			...
			ИначеЕсли НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru = 'до '; en = 'до '")+Строка(Стр.Значение.ЗначениеДо);
			КонецЕсли; 
			Префикс = ЗаголовокФильтра;
		...
		ИначеЕсли МодульОбщегоНазначения.ЭтоСсылка(ТипЗнч(Стр.Значение)) Тогда
			ПредставлениеЗначения = Строка(Стр.Значение);
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Булево") ИЛИ ТипЗнч(Стр.Значение)=Тип("Дата") Тогда
			ПолеСхемы = сфпОтчеты.ПолеСхемы(СхемаКД, Стр.Поле);
...
Функция ПредставлениеВидаСравнения(ВидСравнения)
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии Тогда
		Возврат НСтр("ru = ' в группе '; en = 'in a group'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке Тогда
		Возврат НСтр("ru = ' в списке '; en = 'on the list'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		Возврат НСтр("ru = ' в списке групп '; en = 'in the list of groups'");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВИерархии Тогда
		Возврат НСтр("ru = ' не в группе '; en = 'not in the group'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВСписке Тогда
		Возврат НСтр("ru = ' не в списке '; en = 'not on the list'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
		Возврат НСтр("ru = ' не в списке групп '; en = 'not in the group list'");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеСодержит Тогда
		Возврат НСтр("ru = ' не содержит '; en = 'does not contain'");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Подобно Тогда
		Возврат НСтр("ru = ' подобно '; en = 'similarly'");
...
Процедура ОбновитьОтображениеОтметокФильтров()
	...
	Если ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
		МаксимальнаяДлинаСтрокиОтметок = 100;
	Иначе
	...
	Для каждого Элемент Из Элементы.СтрокиОтметокФильтров.ПодчиненныеЭлементы Цикл
		Для каждого ПодЭлемент Из Элемент.ПодчиненныеЭлементы Цикл
			Если ПодЭлемент=Элементы.ПредставлениеПериода ИЛИ ПодЭлемент=Элементы.ПредставлениеПериодаКнопка ИЛИ ПодЭлемент=Элементы.ДекорацияНаполнитель_1 ИЛИ ПодЭлемент=Элементы.СтрокаОтбора1 Тогда
				Продолжить;
			КонецЕсли; 
	...
	Если Элементы.ПредставлениеПериодаКнопка.Видимость Тогда
		Если ФормаПараметры.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
			Элементы.ПредставлениеПериодаКнопка.Ширина = Окр(СтрДлина(ПредставлениеПериода)*0.75+4);
		Иначе
			Элементы.ПредставлениеПериодаКнопка.Ширина = Окр(СтрДлина(ПредставлениеПериода)*0.8+4);
		КонецЕсли; 
		ТекущаяДлинаСтрокиОтметок = Элементы.ПредставлениеПериодаКнопка.Ширина+СтрДлина(Элементы.ПредставлениеПериодаКнопка.Заголовок)+1;
	Иначе
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ЭлементыЗаголовка.Добавить(Новый ФорматированнаяСтрока(БиблиотекаКартинок.сфпУдалитьСиняя,,,, Стр.ИмяКнопки));
		...
		ЭлементОтметка.ЦветТекста = ЦветаСтиля.сфпОсновнойГолубой;
...
Функция СформироватьСписокЭлементовСервер(Текст, МассивТипов, УправляющиеСимволы)
	...
	Для Каждого Тип Из МассивТипов Цикл
		...
		Если НЕ МодульОбщегоНазначения.ЭтоПеречисление(ОбъектМетаданных) Тогда
			...
			ТекстЗапросаСПараметрами(
			"ВЫБРАТЬ ПЕРВЫЕ 5
			|	""&Параметр1"" КАК Тип,
			|	Объект.Ссылка КАК Ссылка,
			|	Объект.Представление КАК Представление,
			|	&Параметр3 КАК ПрочиеПоля
			|ИЗ
			|	&Параметр1 КАК Объект
			|ГДЕ
			|	&Параметр2
			|
			|СГРУППИРОВАТЬ ПО
	...
	Пока ВыборкаТип.Следующий() Цикл
		...
		Если Выборка.Количество()>1 ИЛИ (УправляющиеСимволы="""" И Выборка.Количество()>0) Тогда
			...
			Представление = ПредставлениеТипа+НСтр("ru = ' содержит '; en = 'contains'")+Текст;
...
&НаСервереБезКонтекста
Функция ТекстЗапросаСПараметрами(ТекстЗапроса, ЭтоПервыйЗапрос, Параметр1, Параметр2, Параметр3)
	
	Результат = ТекстЗапроса;
	
	Если ЭтоПервыйЗапрос Тогда
		Результат = СтрЗаменить(Результат, "ВЫБРАТЬ ПЕРВЫЕ", "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ");
	КонецЕсли;

	Результат = СтрЗаменить(Результат, "&Параметр1", Параметр1);
	Результат = СтрЗаменить(Результат, "&Параметр2", Параметр2);
	Результат = СтрЗаменить(Результат, "&Параметр3", Параметр3);
...
Процедура РедактироватьСКД()
	...
	Если Не НастройкиОтчета.РазрешитьРедактироватьСКД Тогда
		Состояние(НСтр("ru = 'Редактирование настроек данного отчета запрещено'; en = 'Editing the settings of this report is prohibited.'"));
		Возврат;
	КонецЕсли;
...
Функция СуммаЯчеек(ТабличныйДокумент, ВыделенныеОбласти)
	...
	Если ВыделенныеОбласти = Неопределено Тогда
		...
		#Иначе
			Возврат НСтр("ru = 'Не указано значение параметра ""ВыделенныеОбласти"".'; en = 'The value for the ""SelectedRegions"" parameter is not specified.'");
		#КонецЕсли
	КонецЕсли;
...
Процедура РассчитатьСуммуЯчеек()
	...
	Если КоличествоВыделенныхЯчеек()>1 Тогда
		...
		Если ПустаяСтрока(СуммаВыделенныхЯчеек) Тогда
			Элементы.СтрокаОтбора1.ПодсказкаВвода = "";
		Иначе
			Элементы.СтрокаОтбора1.ПодсказкаВвода = НСтр("ru = 'Сумма'; en = 'Sum'")+" "+СуммаВыделенныхЯчеек;
		КонецЕсли; 
	Иначе
		Элементы.СтрокаОтбора1.ПодсказкаВвода = "";
	КонецЕсли;
	
	Если ПустаяСтрока(Элементы.СтрокаОтбора1.ПодсказкаВвода) Тогда
		Элементы.СтрокаОтбора1.ПодсказкаВвода = НСтр("ru = 'Сумма'; en = 'Sum'");
	КонецЕсли; 
...
Функция ДобавитьОписаниеНастройки(ЭлементыДобавления, Тип, ИмяПоля, Представление = "", ЭлементСхемы = Неопределено)
	...
	Если НЕ СуществующаяСтрока=Неопределено Тогда
		...
		ОпределитьЭлементыИРеквизитыФильтра(СуществующаяСтрока, ЭлементСхемы, ДоступноеПоле, Ложь);
...
Процедура ОбновитьДополнительныйСвойстваСКД()
	...
	Свойства = сфпСтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИменаПолей, Символы.ПС);
...
Функция ВБулевоТриСостояния(Значение)
	...
	Если Значение=Истина Тогда 
		Возврат ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.Истина");
	ИначеЕсли Значение=Ложь Тогда 
		Возврат ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.Ложь");
	Иначе
		Возврат ПредопределенноеЗначение("Перечисление.сфпБулевоТриСостояния.НеИзменять");
	КонецЕсли;
...
Функция ОпределитьФормуВыбора(Значение, ВыборГруппИЭлементов)
	...
	Если НЕ МодульОбщегоНазначения.ЭтоСсылка(Тип) Тогда
		Возврат "";
	КонецЕсли; 
	ПолноеИмяТипа = МодульОбщегоНазначения.ИмяТаблицыПоСсылке(Новый(Тип));
...
#Область РаботаСРезультатом

&НаСервере
Функция CRM_СформироватьРезультатДляСохраненияСервер()
	
	Возврат сфпОтчеты.СформироватьРезультатДляСохраненияСервер(ЭтотОбъект);
...
Процедура CRM_СохранитьРезультат(Команда)
	...
	Если ТипЗнч(СтруктураПараметры) = Тип("Структура") Тогда
		ОткрытьФорму("ОбщаяФорма.сфпСохранениеЗагрузкаРезультатовОтчетов", СтруктураПараметры, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
...
Процедура CRM_ЗагрузитьРезультатВОтбор(Команда)
	...
	Форма = ПолучитьФорму("ОбщаяФорма.сфпЗагрузкаРезультатовОтчетовВОтбор", СтруктураПараметры, ЭтотОбъект,,,);
...
Процедура CRM_ЗагрузитьРезультатВОтборЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") Тогда
		сфпОбщегоНазначенияКлиент.УстановитьОтборПоСпискуРезультатаОтчета(
