...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если Не ЗначениеЗаполнено(ТранспортСообщенийОбменаВызовСервера.ТранспортПоУмолчанию(ПараметрКоманды)) Тогда
		...
		Текст = НСтр("ru = 'Для узла не назначен тип транспорта по умолчанию';
					|en = 'No default transport type is specified for this node'",
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ПоказатьПредупреждение(, Текст);
	...
	Если ИспользуетсяВидТранспортаSM(ПараметрКоманды) Тогда
		...
		Текст = НСтр("ru = 'Для этого вида транспорта не предполагается сохранение настроек в файл.';
					|en = 'This transport type does not support saving settings to a file.'",
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура СохранитьФайлыНастроекПодключенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Настройки подключения сохранены';
							|en = 'Connection settings saved'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
