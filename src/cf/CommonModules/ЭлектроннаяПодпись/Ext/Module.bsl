					...
					Исключение
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'Электронная подпись.Получение контейнера подписи';
								|en = 'Digital signature.Get signature container'",
...
Функция ШтампыВизуализацииЭлектронныхПодписей(Знач ПодписанныйФайл, Знач ЭлектронныеПодписи = Неопределено,
			...
			ТекстОшибки = ?(Подпись.ИмяФайлаПодписи = "", НСтр(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат подписанта отсутствует в подписи ""%1"" и не найден в локальном хранилище.';
						|en = 'The certificate is not found in the signature ""%1"" or the local certificate store.'"),
				Подпись.ИмяФайлаПодписи));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Получение штампов визуализации электронных подписей';
					|en = 'Digital signature.Get digital signature visualization stamps'",
	...
	Для Каждого ПараметрыШтампа Из ПараметрыШтампов Цикл
		...
		ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Сертификат';
													|en = 'Certificate'"), ПараметрыШтампа.НомерСертификата);
		ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Владелец';
													|en = 'Owner'"), ПараметрыШтампа.ВладелецСертификата);
		ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Действителен';
													|en = 'Valid till'"), ПараметрыШтампа.СрокДействия);
		...
		Если ЗначениеЗаполнено(ПараметрыШтампа.ДатаПодписиИзМетки) Тогда
			...
			ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Дата подписи';
														|en = 'Date signed'"), ПараметрыШтампа.ДатаПодписи);
			ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Метка времени';
														|en = 'Timestamp'"), ПараметрыШтампа.ДатаПодписиИзМетки);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыШтампа.Доверенность) Тогда
			ДобавитьПолеШтампа(ПоляТекстаШтампа, НСтр("ru = 'Доверенность';
														|en = 'Letter of authority'"), ПараметрыШтампа.Доверенность);
		КонецЕсли;
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если Не ОбщиеНастройки().ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
			...
			ВызватьИсключение НСтр("ru = 'Недоступно усовершенствование подписей на сервере. Регламентное задание отключено.';
									|en = 'Cannot enhance signatures on the server. The scheduled job is disabled.'");
		Иначе
