...
Процедура ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	уатОбщегоНазначения.ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений);
	// {/УАТ}
...
Процедура ЗаполнитьОбластиПерсональныхДанных(КатегорииПерсональныхДанных) Экспорт
	...
	Если КатегорииПерсональныхДанных.Найти("ЛичныеДанные", "Имя") = Неопределено Тогда
		...
		НоваяОбласть.Представление = НСтр("en='Personal data';ru='Личные данные физических лиц'");
	КонецЕсли;
	
	Если КатегорииПерсональныхДанных.Найти("ПаспортныеДанные", "Имя") = Неопределено Тогда
		НоваяОбласть = КатегорииПерсональныхДанных.Добавить();
		НоваяОбласть.Имя = "ПаспортныеДанные";
		НоваяОбласть.Представление = НСтр("en='Passport data';ru='Паспортные данные физических лиц'");
	...
	Если КатегорииПерсональныхДанных.Найти("КонтактнаяИнформация", "Имя") = Неопределено Тогда
		...
		НоваяОбласть.Представление = НСтр("en='Addresses and phone numbers';ru='Адреса и телефоны физических лиц'");
		НоваяОбласть.Родитель = "ЛичныеДанные";
	КонецЕсли;
	// Конец СтандартныеПодсистемы.ФизическиеЛица
	
	уатОбщегоНазначения.ЗаполнитьОбластиПерсональныхДанных(КатегорииПерсональныхДанных);
	// {/УАТ}
...
Процедура ДополнитьДанныеСубъектовПерсональныхДанных(СубъектыПерсональныхДанных, ДатаАктуальности) Экспорт
	...
	Для Каждого СтрокаСубъект Из СубъектыПерсональныхДанных Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаСубъект.Субъект) = Тип("СправочникСсылка.Сотрудники") Тогда
			СтрокаСубъект.ФИО              = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСубъект.Субъект.ФизическоеЛицо, "Наименование");
			СтрокаСубъект.ПаспортныеДанные = РегистрыСведений.ДокументыФизическихЛиц.ДокументУдостоверяющийЛичностьФизлица(СтрокаСубъект.Субъект.ФизическоеЛицо, ДатаАктуальности);
			СтрокаСубъект.Адрес            = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(СтрокаСубъект.Субъект.ФизическоеЛицо, Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица);
		КонецЕсли;
	КонецЦикла;
	// Конец СтандартныеПодсистемы.ФизическиеЛица
	// {/УАТ}
...
Процедура ДополнитьДанныеОрганизацииОператораПерсональныхДанных(Организация, ДанныеОрганизации, ДатаАктуальности) Экспорт
	...
	// {УАТ}
	ДанныеОрганизации.Вставить("АдресОрганизации", УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации, ДатаАктуальности));
	ДанныеОрганизации.Вставить("НаименованиеОрганизации", Справочники.Организации.НаименованияОрганизации(Организация).НаименованиеДляПечатныхФорм);
	// {/УАТ}
...
Процедура ПриЗаполненииСведенийОбУничтожаемыхПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначения.ПриЗаполненииСведенийОбУничтожаемыхПерсональныхДанных(ТаблицаСведений);
	// {/УАТ}
...
Процедура ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений, ОтказОтСкрытия) Экспорт
	...
	Если уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("ЛичныеДанные") Тогда
		Для Каждого Субъект Из Субъекты Цикл
			Если Субъект.ЭтоГруппа Тогда
				СтрокаИсключения = ТаблицаИсключений.Добавить();
				СтрокаИсключения.Субъект = Субъект;
				СтрокаИсключения.ПричинаОтменыСкрытия = НСтр("ru = 'Группы субъектов не подлежат скрытию'; en = 'Subject groups are not subject to concealment'");
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	НовыеСубъекты = Новый Массив;
	...
	Для Каждого Субъект Из Субъекты Цикл
		Если ТипЗнч(Субъект) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				НовыеСубъекты.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
			КонецЦикла;
			
			//Если ЗначениеЗаполнено(Субъект.Подпись) 
			//	И уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("ЛичныеДанные") Тогда
			//	НовыеСубъекты.Добавить(Субъект.Подпись);
			//КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ТекСубъект Из НовыеСубъекты Цикл
		Субъекты.Добавить(ТекСубъект);
	КонецЦикла;
	// {/УАТ}
...
Процедура ПередУничтожениемПерсональныхДанных(Объект, Субъекты, ВыполнитьЗаписьОбъекта) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначения.ПередУничтожениемПерсональныхДанных(Объект, Субъекты, ВыполнитьЗаписьОбъекта);
	// {/УАТ}
...
Процедура ПослеУничтоженияПерсональныхДанных(Объект, Субъекты) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначения.ПослеУничтоженияПерсональныхДанных(Объект, Субъекты);
	// {/УАТ}
