...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	Иначе
		...
		Сообщение = НСтр("ru = 'Не определен идентификатор транспорта';
						|en = 'Couldn''t determine transport ID'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';
											|en = 'The data processor file required for export  debugging does not exist. The data exchange is canceled.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';
											|en = 'The data processor file required for import debugging does not exist. The data exchange is canceled.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	Если ВыполняемоеДействие = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
		...
		Если ЕстьПоддержкаСопоставления Тогда
			...
			Если ПолученоСообщениеДляСопоставления Тогда 
				...
				|en = 'To continue, import the data mapping message.
...
Процедура НастроитьЭлементыФормыЗацикливания(Форма)
	...
	ШаблонТекста = НСтр("ru = '<br>Обнаружено зацикливание синхронизаций. Для получения подробной информации перейдите по 
			  ...
			  |en = '<br>Synchronization loop is found. For more information, follow the <a href=""%1"">link</a>.
			  |<br><br>
...
Процедура СохранитьСообщениеИзАрхиваВКэш(УзелИнформационнойБазы) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		ВызватьИсключение НСтр("ru = 'Архивное сообщение отсутствует.';
								|en = 'The archived message is missing.'");
	КонецЕсли;
	...
	Если РаспаковатьZipФайл(ИмяВременногоФайлаАрхива, ИмяВременногоКаталога) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() > 1 Тогда
			ВызватьИсключение НСтр("ru = 'Ожидается, что в архиве будет один файл.';
									|en = 'The archive is expected to contain a single file.'");
		КонецЕсли;
