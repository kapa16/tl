...
Процедура ОтразитьРезультатПроверкиМЧД(МЧД, Результат, ТекстОшибки, ОписаниеОшибкиРегистрации = "") Экспорт
	...
	Попытка
		...
		Если Не ПроверенаВручную(МЧДОбъект) Тогда
...
Процедура ЗаполнитьПодписанаВерна(ОбъектМЧД, ДанныеДляПроверки, ТребуетсяПроверкаМЧДНаКлиенте = Ложь,
	...
	ИначеЕсли ПроверенаВручную(ДанныеДляПроверки.ДанныеДоверенности) Тогда
		РезультатВерна = Истина;
		ТребуетсяПроверкаМЧДНаКлиенте = Ложь;
...
Функция ПроверитьПолномочияДоверенности(Доверенность, ЭлектронныйДокумент)
	...
	Иначе // Упрощенная настройка
		...
		ТекстОшибкиПроверкаПравилСуммаДокумента = "";
		ТекстОшибкиПроверкаПравилКонтрагент = "";
		ТекстОшибкиПроверкаПравилВидДокумента = "";
		
		Если Не Результат.Успех Тогда
			
			Для Каждого ТекущаяСтрокаНастроек Из ТаблицаНастроек Цикл 
				
				Если ТекущаяСтрокаНастроек.УсловиеВыполнено Тогда
					Продолжить;
				КонецЕсли;
				
				Если ТекущаяСтрокаНастроек.ИмяПоляДанных = "ВидДокумента" Тогда  	
					...
					ТекстОшибкиПроверкаПравилВидДокумента = СтрСоединить(Полномочия, Символы.ПС);
					
				ИначеЕсли ТекущаяСтрокаНастроек.ИмяПоляДанных = "СуммаДокумента" Тогда
					
					ТекстОшибкиПроверкаПравилСуммаДокумента = 
						НСтр("ru = 'Сумма документа не соответствует диапазону значений, указанному в правилах проверки полномочий.'");
					
				ИначеЕсли ТекущаяСтрокаНастроек.ИмяПоляДанных = "Контрагент" Тогда	
					
					ТекстОшибкиПроверкаПравилКонтрагент = 
						НСтр("ru = 'Контрагент отсутствует в списке значений правил проверки полномочий.'");
					
				КонецЕсли;
				
			КонецЦикла;
			
			Результат.ТекстОшибки = СтрШаблон("%1%2%3%4%5%6%7",
				ТекстПолномочияНеПодтверждены,
				?(ТекстОшибкиПроверкаПравилСуммаДокумента <> "", Символы.ПС, ""),
				ТекстОшибкиПроверкаПравилСуммаДокумента,
				?(ТекстОшибкиПроверкаПравилКонтрагент <> "", Символы.ПС, ""),
				ТекстОшибкиПроверкаПравилКонтрагент,
				?(ТекстОшибкиПроверкаПравилВидДокумента <> "", Символы.ПС, ""),
				ТекстОшибкиПроверкаПравилВидДокумента);
...
//						- см. ПолучитьСведенияДоверенностиНаСервереМЧД		
...
Функция ДанныеИзФайлаОбменаВПилотномФормате(ВходящиеДанные)
	...
	Для Каждого СтрокаМассива Из СведенияУполномоченных Цикл
		...
		Если СведенияФЛ.Количество() > 0 Тогда 
			...
			ДанныеУдостоверенияЛичности = НовыеДанныеУдостоверенияЛичности(); 
			СерияНомер = ПодготовитьПредставлениеСерияНомер(ПолучитьЗначениеДокумента(СведенияФЛ, "УдЛичн/@СерНомДок")); // АПК:1036 не проверять строку на орфографию.
			ДанныеУдостоверенияЛичности.СерДок = СерияНомер.Серия;
			ДанныеУдостоверенияЛичности.НомДок = СерияНомер.Номер;
			ДанныеУдостоверенияЛичности.ДатаДок = ПолучитьЗначениеДокумента(СведенияФЛ, "УдЛичн/@ДатаДок", '00010101'); // АПК:1036 не проверять строку на орфографию.
			ДанныеУдостоверенияЛичности.ВыдДок = ПолучитьЗначениеДокумента(СведенияФЛ, "УдЛичн/@ВыдДок"); // АПК:1036 не проверять строку на орфографию.
			ДанныеУдостоверенияЛичности.ВидДок = ПолучитьЗначениеДокумента(СведенияФЛ, "УдЛичн/@КодВидДок"); // АПК:1036 не проверять строку на орфографию.
			ДанныеУдостоверенияЛичности.КодВыдДок = ПолучитьЗначениеДокумента(СведенияФЛ, "УдЛичн/@КодВыдДок"); // АПК:1036 не проверять строку на орфографию.
...
Функция ДанныеДляПодписания(Доверенности, ОтпечаткиСертификатов, КонтекстДиагностики, УчитыватьСНИЛСДоверителя) Экспорт
...
// записывает в журнал регистрации информацию об ошибке обработки МЧД.
// 
// Параметры:
//  Объект - СправочникОбъект.МашиночитаемыеДоверенностиОрганизаций
//         - СправочникОбъект.МашиночитаемыеДоверенностиКонтрагентов
...
Функция ПолучитьИнформациюОбОшибкахПроверкиПравилПолномочий(РезультатПроверкиПодписи)
	...
	Если ЗначениеЗаполнено(ПротоколПроверки.ПроверкаМЧД.ПроверкаПолномочий)
		И ЗначениеЗаполнено(ПротоколПроверки.ПроверкаМЧД.ПроверкаПолномочий.Ошибка) Тогда 
		 ТекстСообщения = НСтр("ru = 'Ошибки при проверке правил:'") + Символы.ПС
		 					 + ПротоколПроверки.ПроверкаМЧД.ПроверкаПолномочий.Ошибка + ".";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПротоколПроверки.ПроверкаМЧД.ПроверкаОтзываМЧД)
		...
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "") 
			+ НСтр("ru = 'Проверка отзыва:'") + Символы.ПС 
	...
	Если ЗначениеЗаполнено(ПротоколПроверки.ПроверкаМЧД.ПроверкаСтатусаВРеестреФНС)  
		 ...
		 ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "") 
		 					+ НСтр("ru = 'Предупреждение:'") + Символы.ПС 
...
// Возвращает массив из выбранных элементов, отсутствующих в дереве отборов настройки правил справочника МЧД
//
// Параметры:
//  ПроверяемыеЭлементы - Массив из СправочникСсылка.ВидыДокументовЭДО, СправочникСсылка.Контрагенты
//  ДеревоОтборов - ДеревоЗначений
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.ВидыДокументовЭДО, СправочникСсылка.Контрагенты
//
Функция ПроверитьДублированиеЗаписейВДеревеОтбораПриДобавлении(ПроверяемыеЭлементы, ДеревоОтборов) Экспорт 
	
	Результат = Новый Массив;
	
	Для Каждого ТекущийЭлемент Из ПроверяемыеЭлементы Цикл
		
		ПараметрыОтбора = Новый Структура("Данные", ТекущийЭлемент);
		НайденныеСтроки = ДеревоОтборов.Строки.НайтиСтроки(ПараметрыОтбора, Истина); 
...
Процедура ОбновитьЗаголовкиЭлементовФормы(Доверенность, ЭлементыФормы) Экспорт 
	...
	ЭлементыФормы.ПоказатьСкрытьПровереныУспешно.Заголовок = 
		СтрШаблон(НСтр("ru = 'Прошли проверку подписи (%1)'"), Результат.КоличествоПровереныУспешно);
		
	ЭлементыФормы.ПоказатьСкрытьОшибки.Заголовок = 
		СтрШаблон(НСтр("ru = 'Не прошли проверку (%1)'"), Результат.КоличествоСОшибками); 
		
	ЭлементыФормы.ГруппаПодписанныеДокументы.Заголовок = 
		СтрШаблон(НСтр("ru = 'Подписанные документы (%1)'"), Результат.КоличествоДокументовВсего);
...
// Возвращаемое значение:
//  Дата
Функция ДатаНачалаИсключенияСведенийДУЛПредставителяИзМЧДПоУмолчанию()
...
Процедура ОбновитьДатуНачалаИсключенияСведенийДУЛПредставителяИзМЧД()
	...
	Попытка
		...
		ДанныеФайла = ПолучитьСтрокуИзДвоичныхДанных(РезультатЗагрузки.ДанныеФайла);
		ДанныеНастроекИзСервиса = ОбщегоНазначения.JSONВЗначение(ДанныеФайла, ИмяСвойстваДляЧтения, Ложь);
