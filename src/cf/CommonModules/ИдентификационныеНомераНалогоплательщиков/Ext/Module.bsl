...
// Процедуры и функции работы с ИНН.
...
Процедура УдалитьИзНаименованияИНН(Наименование, ИНН) Экспорт
	...
	Если ЭлементыНаименования[1] <> ИНН Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверитьСоответствиеТребованиямИНН(Знач ИНН, Знач ЭтоЮрЛицо) Экспорт
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если РезультатПроверки.ЭтоЮрЛицо И НЕ ЭтоИННЮридическогоЛица Тогда
			РезультатПроверки.ОписаниеОшибки = НСтр("en='TIN of the legal person should consist of 10 digits';ru='ИНН юридического лица должен состоять из 10 цифр'");
		...
		ИначеЕсли НЕ РезультатПроверки.ЭтоЮрЛицо И НЕ ЭтоИННФизическогоЛица Тогда
			РезультатПроверки.ОписаниеОшибки = НСтр("en='TIN of individual must consist of 12 digits';ru='ИНН физического лица должен состоять из 12 цифр'");
		...
		Если РезультатПроверки.СоответствуетТребованиям Тогда
			Если Лев(ИНН, 2) = "00" Тогда
				РезультатПроверки.ОписаниеОшибки = НСтр("en='First two digits of TIN cannot be ""00""';ru='Первые две цифры ИНН не могут быть ""00""'");
				РезультатПроверки.СоответствуетТребованиям = Ложь;
			Иначе
...
Функция ПроверитьСоответствиеТребованиямКПП(Знач КПП, Знач ЭтоЮрЛицо, Знач ОбособленноеПодразделение, Знач ЭтоКонтрагент = Истина) Экспорт
	...
	Если СтрДлина(КПП) <> 9 Тогда
		РезультатПроверки.СоответствуетТребованиям = Ложь;
		РезультатПроверки.ОписаниеОшибки = НСтр("en='TRRC shall consist of 9 symbols';ru='КПП должен состоять из 9 символов'");
		Возврат РезультатПроверки;
	КонецЕсли;
	
	Если СтрНачинаетсяС(КПП, "00") Тогда
		РезультатПроверки.СоответствуетТребованиям = Ложь;
		РезультатПроверки.ОписаниеОшибки = НСтр("en='RRC should not begin with 00';ru='КПП не должен начинаться на 00'");
		Возврат РезультатПроверки;
	КонецЕсли;
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Лев(КПП, 4))           // NNNN
		...
		РезультатПроверки.СоответствуетТребованиям = Ложь;
		РезультатПроверки.ОписаниеОшибки = НСтр("en='RRC must consist of digits and characters A-Z (at positions 5 and 6)';ru='КПП должен состоять из цифр и символов A-Z (в позициях 5 и 6)'");
		Возврат РезультатПроверки;
	КонецЕсли;
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодПричиныПостановкиНаУчет) Тогда
		...
		Если ОбособленноеПодразделение и ПризнакГоловногоПодразделения Тогда 
			...
			РезультатПроверки.ОписаниеОшибки = СтрШаблон(
				НСтр("en='Probably, RRC of a legal entity has been input. Change type % 1';ru='Возможно, введен КПП юридического лица. Измените вид %1'"), 
				?(ЭтоКонтрагент, НСтр("en='counterparty';ru='контрагента'"), НСтр("en='of company';ru='организации'")));
			Возврат РезультатПроверки;
		ИначеЕсли НЕ ОбособленноеПодразделение И ПризнакОбособленногоПодразделения Тогда  
			РезультатПроверки.СоответствуетТребованиям = Истина;
			РезультатПроверки.ОписаниеОшибки = СтрШаблон(
				НСтр("en='Probably, RRC of separate department was introduced. Change type %1';ru='Возможно, введен КПП обособленного подразделения. Измените вид %1'"), 
				?(ЭтоКонтрагент, НСтр("en='counterparty';ru='контрагента'"), НСтр("en='of company';ru='организации'")));
			Возврат РезультатПроверки;
		ИначеЕсли НЕ ПризнакГоловногоПодразделения и НЕ ПризнакОбособленногоПодразделения Тогда
			РезультатПроверки.СоответствуетТребованиям = Истина;
			РезультатПроверки.ОписаниеОшибки = НСтр("en='Perhaps RRC not accord to format';ru='Возможно, КПП не соответствует формату'");
			Возврат РезультатПроверки;
		КонецЕсли;
	КонецЕсли;
