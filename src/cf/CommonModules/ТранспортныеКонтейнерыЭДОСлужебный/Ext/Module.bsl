	...
	Попытка
		...
		Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
			...
			Для Каждого ДанныеМЧД Из ДанныеФайла.ДоверенностиПоСсылке Цикл
				...
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеДоверенности, "RepresentativeInn",
					СтруктураДанныхМЧД.СвойстваМЧД.ПредставительИНН, Истина, Ошибки);
...
//
Функция ИсправленныйКонтрагентПоОбъектуНеНайден(КонтрагентОбъектНеНайден)
	
	Контрагент = Неопределено;
	УстановитьПривилегированныйРежим(Истина);
	ПриглашенияСКонтрагентомОбъектНеНайден = ПриглашенияЭДО.ПриглашенияПоКонтрагенту(КонтрагентОбъектНеНайден);
	
	Если ПриглашенияСКонтрагентомОбъектНеНайден.Количество() = 0 Тогда
		Возврат Контрагент;
	Конецесли;
	
	НачатьТранзакцию();
	Попытка
		Для Каждого Приглашение Из ПриглашенияСКонтрагентомОбъектНеНайден Цикл
			
			Контрагент = Неопределено;
			ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП("Контрагенты", Приглашение.ИНН, Приглашение.КПП,
				Контрагент);
			Если Контрагент <> Неопределено Тогда
				ОтражениеВУчетеЭДО.ИзменитьНастройкиПолученияЭлектронныхДокументовПоКонтрагентуОбъектНеНайден(
					Приглашение.Контрагент, Контрагент, Приглашение.Организация);
				Параметры = Новый Структура;
				Параметры.Вставить("Организация", Приглашение.Организация);
				Параметры.Вставить("Контрагент", Приглашение.Контрагент);
				АдресРезультата = "";
				РегистрыСведений.НастройкиОтправкиЭлектронныхДокументовПоВидам.УдалитьНастройкиОтправкиЭДО(Параметры,
					АдресРезультата);
				ПолныйКлючПриглашения = Новый Структура("Ключ, ИдентификаторОрганизации");
				ЗаполнитьЗначенияСвойств(ПолныйКлючПриглашения, Приглашение);
				ПриглашенияЭДО.ИзменитьКонтрагентаВПриглашении(ПолныйКлючПриглашения, Контрагент);
			КонецЕсли;
...
//
Функция ДанныеКонтрагентаИзПриглашения(Выборка, ОписаниеОбъекта, МассивРеквизитов)
	
	ДанныеКонтрагентаИзПриглашения = Неопределено;
	Если Выборка.Следующий()
		И Выборка.Контрагент <> ОписаниеОбъекта.Контрагент
		И ЗначениеЗаполнено(Выборка.Контрагент) Тогда
				
		ДанныеКонтрагентаИзПриглашения = ИнтеграцияЭДО.ЗначенияРеквизитовОбъектаУчета(Выборка.Контрагент,
			СтрСоединить(МассивРеквизитов, ","));
		Если ОбщегоНазначенияБЭДКлиентСервер.ЭтоСтруктураСПустымиЗначениями(ДанныеКонтрагентаИзПриглашения) Тогда
			Контрагент = ИсправленныйКонтрагентПоОбъектуНеНайден(Выборка.Контрагент);
			Если Контрагент <> ОписаниеОбъекта.Контрагент
				И ЗначениеЗаполнено(Контрагент) Тогда
				ДанныеКонтрагентаИзПриглашения = ИнтеграцияЭДО.ЗначенияРеквизитовОбъектаУчета(Контрагент,
					СтрСоединить(МассивРеквизитов, ","));
			КонецЕсли;
		КонецЕсли

	КонецЕсли;
	Если ДанныеКонтрагентаИзПриглашения <> Неопределено
		И ОбщегоНазначенияБЭДКлиентСервер.ЭтоСтруктураСПустымиЗначениями(ДанныеКонтрагентаИзПриглашения) Тогда
		ДанныеКонтрагентаИзПриглашения = Неопределено;
	КонецЕсли;
	Возврат ДанныеКонтрагентаИзПриглашения;
КонецФункции
...
Процедура ПодготовитьДанныеПоКарточке(ОписаниеОбъекта, ДеревоКарточки)
	...
	Если Не ОписаниеОбъекта.СпособОбмена = Перечисления.СпособыОбменаЭД.БыстрыйОбмен Тогда
		...
		ТребуетсяПовторнаяПодпись = ОписаниеОбъекта.ТребуетсяПодтверждение;
		Если ОписаниеОбъекта.ТипРегламента = Перечисления.ТипыРегламентовЭДО.УПД
			И ОписаниеОбъекта.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА Тогда
			ТребуетсяПовторнаяПодпись = Истина;
		КонецЕсли;
		ЗначенияРеквизитов.Вставить("Определение.ТребуетсяПовторнаяПодпись", ТребуетсяПовторнаяПодпись);
		...
		Если РегламентыЭДО.ЭтоЭлементРегламентаОтправителя(ОписаниеОбъекта.ТипЭлементаРегламента)
			...
			Если ОписаниеОбъекта.ОписаниеДокумента <> Неопределено Тогда
				...
				НоваяСтрока.Значение = Формат(ОписаниеОбъекта.ОписаниеДокумента.Сумма, "ЧДЦ=6; ЧРД=,");
				
				НоваяСтрока = ДополнительныеДанные.Добавить();
				НоваяСтрока.Имя = "ДатаДокумента";
				НоваяСтрока.Значение = ОписаниеОбъекта.ОписаниеДокумента.Дата;
			КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоКарточки, "Получатель.Тип")) Тогда
		Если ОписаниеОбъекта.СпособОбмена <> Перечисления.СпособыОбменаЭД.БыстрыйОбмен Тогда
			...
			ДанныеКонтрагентаИзПриглашения = ДанныеКонтрагентаИзПриглашения(Выборка, ОписаниеОбъекта, МассивРеквизитов);
			
			Если ДанныеКонтрагентаИзПриглашения <> Неопределено Тогда
				...
				СтруктураДанных = ДанныеКонтрагентаИзПриглашения;
			
			КонецЕсли;
			
			ЗначенияРеквизитов.Вставить("Получатель.Наименование", СтруктураДанных[НаименованиеКонтрагента]);
			Если ЗначениеЗаполнено(СтруктураДанных[ИННКонтрагента]) Тогда
...
Процедура ПодготовитьДанныеПоТранспортнойИнформации(ОписаниеОбъектов, ДанныеФайлов, ДеревоТранспортнойИнформации)
	...
	Для Каждого ОписаниеОбъекта Из ОписаниеОбъектов Цикл
		...
		Иначе
			...
			КодТранзакции = ПолучитьКодТранзакции(ОписаниеОбъекта, КодРегламента);
		КонецЕсли;
...
Функция ТипДокументаСтрокой(ОписаниеОбъекта, ДляДополнительныхДанных = Ложь)
	...
	ТипДокумента = ТрансграничныйЭДО.ТипДокументаСтрокой(ОписаниеОбъекта);
...
//  Строка -
//
Функция ПолучитьКодТранзакции(ОписаниеОбъекта, КодРегламента)
	
	КодТранзакции = ТрансграничныйЭДО.КодТранзакции(ОписаниеОбъекта);
	Если ЗначениеЗаполнено(КодТранзакции) Тогда
		Возврат КодТранзакции;
	КонецЕсли;
	
	ТипРегламента = ОписаниеОбъекта.ТипРегламента;
	ТипЭлементаРегламента = ОписаниеОбъекта.ТипЭлементаРегламента;
	СпособОбмена = ОписаниеОбъекта.СпособОбмена;
	ОтветнаяПодпись = ОписаниеОбъекта.ЭтоОтветнаяПодпись;
	...
	ИначеЕсли ЭтоУведомлениеОбУточнении(ТипЭлементаРегламента) Тогда
		...
		Если КодТранзакции = "" Тогда
			...
			Если ТипРегламента = Перечисления.ТипыРегламентовЭДО.ФормализованныйАктСверки
				И СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				И КодРегламента = "Formalized" Тогда
				КодТранзакции = "MainDocumentReject";
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияПолучателя_ИОП
		И СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском И КодРегламента = "Formalized" Тогда
		КодТранзакции = "CustomerTitleReceiveNotice";
...
// Формирует таблицу типов элементов регламента.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
// * КодТранзакции - Строка
...
Функция ТаблицаТиповЭлементовРегламента()
	...
	ТрансграничныйЭДО.ПриФормированииТаблицыТиповЭлементовРегламента(Таблица);
...
Функция СформироватьКарточкуCML(ДеревоДанных, Ошибки, ЭлектронныйДокумент, СпособОбмена,
	...
	Попытка
		...
		ПараметрыЗаполнения.НеобязательныеСвойства = "Inn,Kpp";
		...
		Если ЗначениеЗаполнено(ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "Получатель.Тип")) Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойстваXDTOИзДерева(АбонентПолучатель,
				ЗначенияСвойствXDTO, ДеревоДанных, Ошибки, ПараметрыЗаполнения);
...
Процедура СохранитьФайлыДопДанныхЭД(ОписаниеОбъекта, ИмяКаталога, СтруктураФайловЭД)
	...
	Если ЗначениеЗаполнено(ОписаниеДополнительныхДанных.ИмяФайла) Тогда
		...
		ЗаписатьДвоичныеДанныеВФайл(ОписаниеДополнительныхДанных.ДвоичныеДанные, ИмяКаталога,
			ОписаниеДополнительныхДанных.ИмяФайла);
		СтруктураФайловЭД.Вставить("ДополнительныйФайл", ОписаниеДополнительныхДанных.ИмяФайла);
	КонецЕсли;
...
// 
// Параметры:
//  ОписаниеОбъекта - СтрокаТаблицыЗначений - Строка из см. СинхронизацияЭДО.НовыеДанныеОбъектов
//  АдресКаталога - Строка - Адрес каталога
//  Подписи - Массив из Структура - Массив из см. СинхронизацияЭДО.НовоеОписаниеДанныхОбъекта
//  СоответствиеФайлЭД - см. СоответствиеФайлЭД
//
Процедура СохранитьПодписи(ОписаниеОбъекта,
	...
	РасширенияПодписи = "p7s";
	СчетчикПодписи = 0;
	Для Каждого Подпись Из Подписи Цикл
		ИмяФайлаПодписи = Подпись.ИмяФайла;
		
		СчетчикПодписи = СчетчикПодписи + 1;
		
		Если ПустаяСтрока(ИмяФайлаПодписи) Тогда
			ИмяФайлаПодписи = СтрШаблон("%1-%2.%3", ИмяФайлаБезРасширения, СчетчикПодписи, РасширенияПодписи);
		КонецЕсли;
		...
		Пока ФайлСуществует Цикл
			СчетчикФайла = СчетчикФайла + 1;
			
			ИмяФайлаПодписи = СтрШаблон("%1 (%2).%3", ИмяФайлаПодписиБезПостфикса,
				Строка(СчетчикФайла), РасширенияПодписи);
		...
		ЗаписатьДвоичныеДанныеВФайл(Подпись.ДвоичныеДанные, АдресКаталога, ИмяФайлаПодписи);
		ОписаниеОбъекта.ОписаниеДанных.ИмяФайла = ИмяФайлаПодписи;
...
Процедура СохранитьПодписьМЧД(ОписаниеОбъекта, АдресКаталога, СоответствиеФайлЭД, Подпись, ИмяФайлаПодписиДокумента)
	...
	Если МЧД.ЭтоФайловаяДоверенность Тогда
		...
		ЗаписатьДвоичныеДанныеВФайл(МЧД.Доверенность.ДвоичныеДанные, АдресКаталога, МЧД.Доверенность.ИмяФайла);
		ДекодированныеДвоичныеДанные = 
			ОбщегоНазначенияБЭДКлиентСервер.ДекодированныеДвоичныеДанные(МЧД.Подпись.ДвоичныеДанные);
		ЗаписатьДвоичныеДанныеВФайл(ДекодированныеДвоичныеДанные, АдресКаталога, МЧД.Подпись.ИмяФайла);
...
// Создать текст описи ЭД.
// 
// Параметры:
//  Конверт - ДокументСсылка.ТранспортныйКонтейнерЭДО
//  СоответствиеВложений - см. ИнформацияОФайлахИПодписях
//  СоответствиеФайлЭД - Соответствие из КлючИЗначение:
//   * Ключ     - Строка - Имя файла
//   * Значение - Массив Из Строка
// 
// Возвращаемое значение:
//  Строка - Создать текст описи ЭД
//
Функция СоздатьТекстОписиЭД(Конверт, СоответствиеВложений, СоответствиеФайлЭД)
	
	РеквизитыКонверта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Конверт, "Отправитель, Получатель, ДанныеЗашифрованы, СертификатШифрования");
	...
	Для Каждого ВложениеДокумент Из ОбъектыКонтейнера Цикл
		...
		Если РеквизитыКонверта.ДанныеЗашифрованы Тогда
		...
		Если РеквизитыКонверта.ДанныеЗашифрованы Тогда
			...
			ЗаписьXML.ЗаписатьТекст(Строка(РеквизитыКонверта.СертификатШифрования));
			ЗаписьXML.ЗаписатьКонецЭлемента();
		КонецЕсли;
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЦикла;
...
Функция СохранитьФайл(ОписаниеОбъекта,
	...
	ЗаписатьДвоичныеДанныеВФайл(ОписаниеДанных.ДвоичныеДанные, ИмяКаталога, ОписаниеДанных.ИмяФайла);
	...
	Возврат ИмяКаталога + ОписаниеДанных.ИмяФайла;
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ЗаписатьДвоичныеДанныеВФайл(ДвоичныеДанные, АдресКаталога, ИмяФайла, КонтекстДиагностики = Неопределено)
	Если КонтекстДиагностики = Неопределено Тогда
		КонтекстДиагностики = ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики();
	КонецЕсли;
	ПолныйПуть = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(АдресКаталога, ИмяФайла);
	Попытка
		ДвоичныеДанные.Записать(ПолныйПуть);
	Исключение
		РаботаСФайламиБЭД.СократитьИмяФайла(ИмяФайла);
		ПолныйПутьПодписи = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(АдресКаталога, ИмяФайла);
		Если Не РаботаСФайламиБЭД.ЗаписатьДвоичныеДанные(ДвоичныеДанные, ПолныйПутьПодписи, КонтекстДиагностики) Тогда
			ВызватьИсключение ОбработкаНеисправностейБЭД.ТекстОшибокКонтекстаДиагностики(КонтекстДиагностики, Истина);
		КонецЕсли;
	КонецПопытки;
КонецПроцедуры
