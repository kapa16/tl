...
Процедура ФормаПечатьЭтикетокИЦенниковПриСозданииНаСервере(Форма) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначения_проф.ФормаПечатьЭтикетокИЦенниковПриСозданииНаСервере(Форма);
	уатОбщегоНазначения_проф.НастроитьПодключаемоеОборудование(Форма);
	// {/УАТ}
...
Процедура ФормаШаблоныЭтикетокИЦенниковБПОПриСозданииНаСервере(Форма) Экспорт
	...
	// {УАТ}
	Форма.Элементы.ТипЦенника.Видимость = Ложь;
	Если НЕ ПравоДоступа("Изменение", Метаданные.Справочники.ШаблоныЭтикетокИЦенниковБПО) Тогда
		Форма.Элементы.РедактироватьШаблон.Доступность = Ложь;
		Форма.Элементы.ЗагрузитьСКД.Доступность = Ложь;
		Форма.Элементы.ЗагрузитьСКДПоУмолчанию.Доступность = Ложь;
	КонецЕсли;
	// {/УАТ}
...
Функция ДанныеДляФормированияПечатныхФорм(ПараметрыПечати, ПоляДляЗаполнения, ТоварыДляПечати = Неопределено) Экспорт
	...
	// {УАТ}
	Возврат уатОбщегоНазначения_проф.ДанныеДляФормированияПечатныхФормЭтикеток(ПараметрыПечати, ПоляДляЗаполнения, ТоварыДляПечати);
	// {/УАТ}
...
Функция МакетСхемыКомпоновкиДанных(ТипЦенника = "") Экспорт
	...
	// {УАТ}
	Возврат уатОбщегоНазначения_проф.МакетСхемыКомпоновкиДанныхПечатиЭтикеток(ТипЦенника);
	// {/УАТ}
