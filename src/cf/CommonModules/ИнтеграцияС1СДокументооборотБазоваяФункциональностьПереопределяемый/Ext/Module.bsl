...
//	КомандаПечати.Представление = НСтр("ru = 'Документооборот: Лист согласования'; en = 'Document management: Approval sheet'");
...
//	КомандаПечати.Представление = НСтр("ru = 'Документооборот: Лист согласования с историей'; en = 'Document flow: Approval sheet with history'");
//	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
//	КомандаПечати.Представление = НСтр("ru = 'Документооборот: Протокол разногласий'; en = 'Document flow: Protocol of disagreements'");
//	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
//	КомандаПечати.Представление = НСтр("ru = 'Документооборот: Лист ознакомления'; en = 'Document management: Preview sheet'");
//	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Процедура ЗаполнитьРеквизитыИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель) Экспорт
	...
	уатИнтеграцияС1СДокументооборот.ЗаполнитьРеквизитыИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель);
...
//		НСтр("ru = 'Будут созданы правила интеграции и виды внутренних документов для всех видов договоров.'; en = 'Integration rules and types of internal documents will be created for all types of contracts.'");
//
...
//				ПредставлениеВидаОбъектаПотребителя = НСтр("ru = 'Товары (накладная)'; en = 'Goods (invoice)'");
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;
//
...
//			ВидОперации = КлючевыеРеквизиты[0].ЗначениеРеквизитаИС;
//			Если ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("ru = 'Поступление товаров'; en = 'Receipt of goods'");
//			ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Услуги Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("ru = 'Поступление услуг'; en = 'Receipt of services'");
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;
//
...
//
// Пример:
//	СокращенноеНаименование = НСтр("ru = '1С:ERP УП'; en = '1C:ERP EM'");
//
Процедура ПриОпределенииСокращенногоНаименованияКонфигурации(СокращенноеНаименование) Экспорт
	...
	Если уатОбщегоНазначенияПовтИсп.ВариантПоставкиПРОФ() Тогда
		СокращенноеНаименование = НСтр("en = 'Управление автотранспортом Проф, редакция 2.2'; ru = 'Управление автотранспортом Проф, редакция 2.2'");
	Иначе
		СокращенноеНаименование = НСтр("en = '1С:Транспортная логистика, экспедирование и управление автотранспортом КОРП, редакция 1.1'; ru = '1С:Транспортная логистика, экспедирование и управление автотранспортом КОРП, редакция 1.1'");
	КонецЕсли; 
...
Процедура ПриОпределенииТиповОбъектовПоддерживающихИнтеграцию(Типы) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(Типы, Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
...
//
// Пример:
//	Форма.Заголовок = НСтр("ru = 'Форма документа'; en = 'Document form'");
//	Форма.Автозаголовок = Ложь;
//	Форма.Пояснение = НСтр("ru = 'Выберите вид документа:'; en = 'Select document type:'");
...
Процедура ПриСозданииПоСсылке(ОбъектИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено) Экспорт
		...
		Если ИсточникXDTO <> Неопределено Тогда
			...
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
				...
				Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
					...
					Если ИсточникXDTO.legalPrivatePerson.objectID.ID = "ФизЛицо" Тогда //@NON-NLS-1
	...
	Если ТипЗнч(ОбъектИС) = Тип("СправочникОбъект.ПодразделенияОрганизаций") И ЗаполняемыйОбъектИС <> Неопределено Тогда
