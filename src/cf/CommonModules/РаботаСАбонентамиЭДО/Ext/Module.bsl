...
Функция ПолучитьИдентификаторыВСервисе1СЭДО(Организация) Экспорт
	...
	ИменаРеквизитовОрганизации = Новый Структура("ИНН,КПП", ИмяРеквизитаИННОрганизации, ИмяРеквизитаКППОрганизации);
	...
	Если ЗначениеЗаполнено(РеквизитыОрганизации.ИНН) Тогда
		...
		СписокИдентификаторов = ПолучитьИдентификаторыВСервисе1СЭДОПоРеквизитам(
			РеквизитыОрганизации, КонтекстДиагностики);
...
//
// Возвращаемое значение:
//  См. РаботаСАбонентамиЭДОСлужебный.НайтиУчетныеЗаписиАбонентов
Функция ПолучитьИдентификаторыВСервисе1СЭДОПоРеквизитам(РеквизитыОрганизации, КонтекстДиагностики) Экспорт
	
	Отбор = НовыйОтборДанныхАбонентаНастроек();
	Отбор.СписокИНН = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РеквизитыОрганизации.ИНН);
...
Функция ДанныеАбонентовИзСервисаНастроек(Отбор, КонтекстДиагностики, Отказ = Ложь) Экспорт
	...
	ПроизошлаОшибкаОбновленияАбонента = Ложь;
	...
	Если ЗначениеЗаполнено(Абоненты) Тогда
		...
		Для Каждого СтрокаАбонент Из Абоненты Цикл
			...
			Исключение
				...
				ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ОбработкаОшибок.ПодробноеПредставлениеОшибки(
					Ошибка));
				ПроизошлаОшибкаОбновленияАбонента = Истина;
			КонецПопытки;
		КонецЦикла;
			
		Если Не ПроизошлаОшибкаОбновленияАбонента Тогда
			НачатьТранзакцию();
			Попытка
			...
			Исключение
				...
				ВидОперации = НСтр("ru = 'Обновление абонентов из сервиса настроек 1С:ЭДО, удаление устаревших абонентов'");
				ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ОбработкаОшибок.ПодробноеПредставлениеОшибки(
					Ошибка));
			КонецПопытки;
		КонецЕсли;
...
//  * Успех - Булево 
//  * КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ОбновитьДанныеАбонентаИзСервисаНастроекПоИдентификатору(ИдентификаторЭДО) Экспорт
	
	КонтекстДиагностики = ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики();
	Результат = Новый Структура("Успех, КонтекстДиагностики", Ложь, КонтекстДиагностики);
	
	Отбор = НовыйОтборДанныхАбонентаНастроек();
	Отбор.ИдентификаторыЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторЭДО);
	
	ИдентификаторыАбонента = ДанныеАбонентовИзСервисаНастроек(Отбор, КонтекстДиагностики);
	
	Если ЗначениеЗаполнено(ИдентификаторыАбонента) Тогда
...
Процедура ПодготовитьДанныеКПроверке(ДанныеКонтрагентовДляПроверки)
	...
	ОставитьРазрешенныхКонтрагентов(ДанныеКонтрагентовДляПроверки);
	СвернутьДанныеКонтрагентов(ДанныеКонтрагентовДляПроверки);
...
//
// Параметры:
//  ДанныеКонтрагентов - cм. ТаблицаДанныхКонтрагентовДляПередачи - содержит данные проверяемых контрагентов. 
//
Функция РезультатПроверкиКонтрагентов(ДанныеКонтрагентов)
	...
	Запрос.Текст =
		...
		|	ВТДанныеКонтрагентов КАК ВТДанныеКонтрагентов
		|ГДЕ
		|	ВТДанныеКонтрагентов.Состояние <> &СостояниеПоПриглашениям
		|	И ВТДанныеКонтрагентов.СостояниеПоАбонентам = &СостояниеПоАбонентам
		|	И ВТДанныеКонтрагентов.СостояниеПоПриглашениям = &СостояниеПоПриглашениям";
	...
	Запрос.УстановитьПараметр("СостояниеПоАбонентам", Перечисления.СостоянияКонтрагентаБЭД.Подключен);
	Запрос.УстановитьПараметр("СостояниеПоПриглашениям", Перечисления.СостоянияКонтрагентаБЭД.НастроенЭДО);
	// {УАТ}
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"ВТДанныеКонтрагентов.ОбластьДанныхВспомогательныеДанные",
		"Истина");
...
Функция ПолучитьСостоянияКонтрагентовИзКэша(ДанныеКонтрагентов)
	...
	Запрос.Текст =	
		...
		|		И СОКРЛП(ПриглашенияРеквизитыКонтрагентов.ИНН) <> """"
