...
// Обновляет шапку динамического списка.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма, на которой находится динамический список.
//   СтароеПолеДляСортировки - Строка - имя реквизита динамического списка, по которому сортировка выполнялась ранее.
...
//
Процедура КомандаУпорядочить(Форма, СтароеПолеДляСортировки, ПолеДляСортировки, НаправлениеСортировкиПоля,
		КлючФормы) Экспорт
	
	Если СтароеПолеДляСортировки <> "" Тогда
		ПолеЭлемент = Форма.Элементы[СтароеПолеДляСортировки];
...
#КонецЕсли
	...
	ОбновитьСортировкуВЗаголовкеЯчейки(Форма, ПолеДляСортировки, НаправлениеСортировкиПоля);
...
//   НаправлениеСортировкиПоля - Булево - если Ложь, то сортировка по возрастанию, если Истина, то по убыванию.
//
Процедура ОбновитьСортировкуВЗаголовкеЯчейки(Форма, ПолеДляСортировки, НаправлениеСортировкиПоля) Экспорт
...
Функция ВызванаИнтерактивнаяОбработка(РезультатДействия, ТипОперации, ПовторноеДействие) Экспорт
	...
	ИначеЕсли РезультатДействия.ТребуемоеИнтерактивноеДействие.ЗадатьПроизвольныйВопрос Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			...
			ЭтотОбъект,
			ПараметрыОповещения);
		ПоказатьВопросПередВыполнениемЗадачи(
			РезультатДействия.ТребуемоеИнтерактивноеДействие.Контекст,
			ОписаниеОповещения);
		ВызванаИнтерактивнаяОбработка = Истина;
		
	ИначеЕсли РезультатДействия.ТребуемоеИнтерактивноеДействие.ЗадатьВопросОЛимитеЦиклов Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ОбработатьДанныеВыполненияПослеОтветаНаВопросОЛимитеЦиклов",
			ЭтотОбъект,
			ПараметрыОповещения);
		ПоказатьВопросПередВыполнениемЗадачи(
			РезультатДействия.ТребуемоеИнтерактивноеДействие.Контекст,
...
Функция КлючСтрокиДерева(ТипДерева, СтрокаДерева) Экспорт
	...
	ИначеЕсли ТипДерева.ЭтоДеревоУзлыКОД Тогда
		Возврат КлючСтрокиДереваУзлыКОД(СтрокаДерева);
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения = Неопределено,
	...
	Если ОбъектИС.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Связанный объект интегрированной системы не задан или не записан.'; en = 'A linked object of the integrated system is not set or not saved.'");
		ПоказатьПредупреждение(,ТекстОшибки,, НСтр("ru = 'Не удалось создать объект 1С:Документооборота'; en = 'Cannot create a 1C:Document Management object'"));
		Возврат
	КонецЕсли;
	...
	Иначе
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Для объекта %1 нет подходящих правил интеграции.'; en = 'There are no suitable integration rules for the %1 object.'"), ОбъектИС);
		ПоказатьПредупреждение(,ТекстОшибки,, НСтр("ru = 'Не удалось создать объект 1С:Документооборота'; en = 'Cannot create a 1C:Document Management object'"));
...
Процедура НачатьСозданиеСвязанногоОбъектаДОПослеВыбораПравила(Знач Результат, Параметры) Экспорт
	...
	Если ТипЗнч(ОбъектДОИлиСообщение) = Тип("Строка") Тогда // Сообщение об ошибке.
		...
		ПоказатьПредупреждениеИзДО(,ОбъектДОИлиСообщение,,
			НСтр("ru = 'Не удалось создать объект 1С:Документооборота'; en = 'Cannot create a 1C:Document Management object'"));
...
//   АдресБазыДО - Строка - адрес публикации базы ДО, которую требуется открыть.
//
Процедура ОткрытьОбъектДО(Знач СсылкаНаОбъектДО = Неопределено, Знач АдресБазыДО = "") Экспорт
	...
	Если АдресБазыДО = "" Тогда
		АдресБазыДО = ИнтеграцияС1СДокументооборот3ВызовСервера.НавигационнаяСсылкаБазыДО(Истина);
	КонецЕсли;
	
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СтрШаблон("%1#%2", АдресБазыДО, СсылкаНаОбъектДО));
	
КонецПроцедуры

// Выводит на экран окно предупреждения. В тексте предупреждения навигационные ссылки на объекты ДО и ИС
// преобразуются в гиперссылки.
//
// Параметры:
//   ОписаниеОповещенияОЗавершении - ОписаниеОповещения - содержит описание процедуры, которая будет
//     вызвана после закрытия окна предупреждения.
...
//   Заголовок - Строка - содержит заголовок окна предупреждения.
//     Если не установлен, то у диалога заголовок не отображается.
//
Процедура ПоказатьПредупреждениеИзДО(ОписаниеОповещенияОЗавершении = Неопределено, ТекстПредупреждения, Таймаут = 0,
		Заголовок = Неопределено) Экспорт
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.УстановитьТекст(ТекстПредупреждения);
	МассивСтрок = Новый Массив;
	УстановленаСсылкаНаОбъектДО = Ложь;
	ЗаголовокСсылкиНаОбъектИС = СтрШаблон(
		НСтр("ru = 'Ссылка на объект в %1'; en = 'Reference to an object in %1'"),
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СокращенноеНаименованиеКонфигурации());
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		НавигационнаяСсылка = "";
		НачалоСсылки = СтрНайти(ТекущаяСтрока, "e1cib/data/");
		ДлинаСсылки = СтрНайти(ТекущаяСтрока, "?ref=") + 37 - НачалоСсылки;
		Если НачалоСсылки > 0 И ДлинаСсылки > НачалоСсылки Тогда
			НавигационнаяСсылка = Сред(ТекущаяСтрока, НачалоСсылки, ДлинаСсылки);
		КонецЕсли;
		Если Не ПустаяСтрока(НавигационнаяСсылка) Тогда
			Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ИС:'; en = 'Reference to the object in IS:'")) = 1 Тогда
				МассивСтрок.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(
					СтрШаблон("<a href = ""%1"">%2</a>", НавигационнаяСсылка, ЗаголовокСсылкиНаОбъектИС)));
			ИначеЕсли Не УстановленаСсылкаНаОбъектДО Тогда
				ИнтеграцияС1СДокументооборот3ВызовСервера.УстановитьСсылкуНаОбъектДО(НавигационнаяСсылка);
				УстановленаСсылкаНаОбъектДО = Истина;
				ПредставлениеСсылки = СтрШаблон(
					"<a href = ""e1cib/command/Обработка.ИнтеграцияС1СДокументооборот3.Команда.ОткрытьОбъектДО"">%1</a>",
					НСтр("ru = 'Ссылка на объект в ДО'; en = 'Reference to an object in DI'"));
				Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ДО:'; en = 'Reference to the object in DI:'")) = 1 Тогда
					МассивСтрок.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(ПредставлениеСсылки));
				Иначе
					МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Лев(ТекущаяСтрока, НачалоСсылки - 1)));
					МассивСтрок.Добавить(СтроковыеФункцииКлиент.ФорматированнаяСтрока(ПредставлениеСсылки));
					МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Сред(ТекущаяСтрока, НачалоСсылки + ДлинаСсылки)));
				КонецЕсли;
			Иначе
...
Процедура РазвернутьДерево(ТипДерева, СтрокиДерева, ТаблицаФормы, ТекущееСостояниеДерева = Неопределено) Экспорт
	...
	Если ТекущееСостояниеДерева = Неопределено Тогда
		...
		ИначеЕсли ТипДерева.ЭтоДеревоУзлыКОД Тогда
			РазвернутьДеревоУзлыКОД(СтрокиДерева, ТаблицаФормы);
...
Функция ТипДерева() Экспорт
	...
	ТипДерева.Вставить("ЭтоДеревоУзлыКОД", Ложь);
...
""
"Перенести файлы данного %1 из %3 в 1С:Документооборот?'; en = 'The ""%2"" %1 has attachments stored in %3."
""
"Do you want to transfer the files of the %1 from %3 to 1C:Document Management?'"),
	...
	ПараметрыПредупреждения.Заголовок = НСтр("ru = 'Перенос файлов в 1С:Документооборот'; en = 'File transfer to 1C:Document Management'");
	...
	Если ПараметрыПредупреждения.ПредлагатьБольшеНеЗадаватьЭтотВопрос Тогда
		ПараметрыПредупреждения.ТекстФлажка =
			СтрШаблон(
				НСтр("ru = 'Больше не показывать (перенос можно будет выполнить вручную в форме списка файлов командой ""Добавить - Файлы %1"")'; en = 'Do not show again. You can click ""Add > %1 files"" in the file list form to transfer them manually'"),
				ПараметрыПереносаФайлов.ПредставлениеТипаВладельца);
	КонецЕсли;
	ПараметрыОповещения = Новый Структура;
...
Процедура ЗаменитьРольФайлов(СписокФайлов, ВыбраннаяРольID, ВладелецФайлов,
	...
	Если ТипЗнч(СписокФайлов) = Тип("ДанныеФормыКоллекция") Тогда
	...
	Для Каждого Файл Из ДанныеФайлов Цикл
		...
		ДействиеНадФайлом.ИзменениеРолиФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменена роль файла ""%1""'; en = 'The ""%1"" file role is changed'"), Файл.Наименование);
	...
	Если ТипЗнч(Файл) = Тип("Структура") Тогда
		// Отдельно единоразово проинформируем формы, содержащие список файлов.
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
		ДействиеНадФайлом.ИзменениеРолиФайла = Истина;
...
Процедура ПеренестиФайлыИСвДО(ПрисоединенныеФайлыВИС, ВладелецID, ВладелецТип, ВладелецИС,
	...
	Для Каждого Файл Из СозданныеФайлы Цикл
		...
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Создан файл ""%1""'; en = 'The ""%1"" file is created'"), Файл.Наименование);
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ИнформироватьОбИзмененииФайла(
			ДействиеНадФайлом,
			Файл.Наименование,
			Файл.ID,
...
Процедура ОткрытьПриложениеЗадачиНаПросмотр(СтрокаДереваПриложений, УникальныйИдентификаторФормы) Экспорт
	...
	ОбъектДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
		...
		СтрокаДереваПриложений.НавигационнаяСсылка);
	ОткрытьОбъектНаПросмотр(
		СтрокаДереваПриложений.Ссылка,
		ОбъектДО,
...
// Открывает форму "Документооборот".
//
// Параметры:
//   НачальнаяСтраница - Строка - имя страницы формы "Документооборот", которую требуется открыть.
//   ДеревоПриложений - ДанныеФормыДерево - дерево приложений.
//   СтрокаДереваПриложений - ДанныеФормыЭлементДерева - текущая строка дерева приложений.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьФормуДокументооборотИзДереваПриложений(НачальнаяСтраница, ДеревоПриложений, СтрокаДереваПриложений,
		УникальныйИдентификаторФормы) Экспорт
	
	СтрокаОсновнойПредмет = ОсновнойПредметВДеревеПриложений(СтрокаДереваПриложений, ДеревоПриложений);
	Если СтрокаОсновнойПредмет <> Неопределено Тогда
		Если ЗначениеЗаполнено(СтрокаОсновнойПредмет.Ссылка) Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ВнешнийОбъект", СтрокаОсновнойПредмет.Ссылка);
			ПараметрыФормы.Вставить("НачальнаяСтраница", НачальнаяСтраница);
			ОткрытьФорму(
				...
				РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ОткрытьПриложениеЗадачиНаПросмотр(СтрокаОсновнойПредмет, УникальныйИдентификаторФормы);
		КонецЕсли;
	КонецЕсли;
...
Асинх Процедура ПриложенияЗадачиФайлыВставитьКартинкуИзБуфера(Знач СтрокаДереваПриложений,
	...
	Если СтрокаДереваПриложений.РольПредмета = "filledin"
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Добавить приложение задачи можно только на стороне 1С:Документооборот.'; en = 'You can add task applications only in 1C:Document Management.'"));
		Возврат;
...
Процедура ПриложенияЗадачиФайлыЗакончитьРедактирование(СтрокаДереваПриложений, УникальныйИдентификаторФормы) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Выберите файл.'; en = 'Select a file.'"));
...
Процедура ПриложенияЗадачиФайлыУдалить(СтрокаДереваПриложений, УникальныйИдентификаторФормы) Экспорт
	...
	Если СтрокаДереваПриложений.РольПредмета = "filledin"
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Удалить приложение задачи можно только на стороне 1С:Документооборот.'; en = 'You can delete task applications only in 1C:Document Management.'"));
		Возврат;
...
Процедура НачатьОбработкуПослеПоискаСвязанногоОбъектаДО(Результат, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборот3ВызовСервера.НачатьОбработку(Параметры.Предмет, ТекстОшибки) Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Обработка'; en = 'Processing'"),,
			СтрШаблон(НСтр("ru = 'Обработка ""%1"" успешно запущена'; en = 'Processing ""%1"" is started'"), Параметры.Предмет.name),
		...
		КонецЕсли;
		// {УАТ} 
		ВремяОкончания = ТекущаяДата() + 2;
		Пока ТекущаяДата() <= ВремяОкончания Цикл
		КонецЦикла;
		Оповестить("Документооборот_ПриИзмененииСостоянияСогласования");
		// {/УАТ}
	Иначе
		
		ПоказатьПредупреждениеИзДО(,ТекстОшибки,,
			НСтр("ru = 'Не удалось начать обработку'; en = 'Cannot start processing'"));
...
Процедура ДобавитьЭтапДействия(Форма, ФункцияЭтапа) Экспорт
	...
	СтрокаЭтап.НаименованиеЭтапа = НСтр("ru = 'Новый этап'; en = 'New stage'");
...
Процедура ИзменитьПризнакУчастникаДействияЗащищенный(Форма) Экспорт
	...
	Если ПоказыватьПредупреждение Тогда
		ТекстПредупреждения =
			НСтр("ru = 'Защищенного участника действий нельзя удалить ни автоматически, ни вручную.'; en = 'Cannot delete a protected action participant automatically or manually.'");
		ПараметрыПредупреждения = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыПредупреждения.Заголовок = НСтр("ru = 'Защита участника от удаления'; en = 'Protect participants from deletion'");
		ПараметрыПредупреждения.Картинка = БиблиотекаКартинок.Информация32;
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентПереопределяемый.ПриОпределенииНеобходимостиПредлагатьБольшеНеЗадаватьЭтотВопрос(
			ПараметрыПредупреждения.ПредлагатьБольшеНеЗадаватьЭтотВопрос,
			ТекстПредупреждения,
		...
		Если ПараметрыПредупреждения.ПредлагатьБольшеНеЗадаватьЭтотВопрос Тогда
			ПараметрыПредупреждения.ТекстФлажка = НСтр("ru = 'Больше не показывать'; en = 'Do not show again'");
		КонецЕсли;
...
Процедура ОписаниеОтложенногоНачалаВыполненияДействияНажатие(Форма, СтандартнаяОбработка) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Отложенное начало выполнения'; en = 'Deferred execution start'"));
...
Процедура ПереместитьУчастникаДействияВверх(Форма, МожноМенятьПорядокЭтапов = Истина, ПоПунктам = Ложь) Экспорт
	...
	Если ТекущиеДанные.ЭтоЭтап Тогда
		...
		Если Не МожноМенятьПорядокЭтапов И Не ПоПунктам Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Нельзя менять порядок этапов.'; en = 'Cannot change the stage order.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьУчастниковИЗаписатьДействие(ДействиеXDTOСтрока, Форма, ОповещениеОЗавершении) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьЗапросАсинхронно(
		...
		ПроверитьУчастниковИЗаписатьДействиеЗавершение,
		Истина,
		НСтр("ru = 'Проверка участников действия.'; en = 'Check action participants.'"),
		Ложь);
...
Процедура СрокВыполненияДействияУчастникомПриИзменении(Форма, ЗаполнитьСоисполнителей = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыИзмененияСрока.ТекстСообщенияПредупреждения) Тогда
		ПоказатьПредупреждениеИзДО(,ПараметрыИзмененияСрока.ТекстСообщенияПредупреждения);
	КонецЕсли;
...
Процедура УчастникиДействияВыбор(Форма, ВыбраннаяСтрока, Поле, СтандартнаяОбработка,
	...
	Если ОткрыватьОсобоеНаименованиеИОписание И Поле = Форма.Элементы.ЕстьОсобоеНаименованиеОписание Тогда
		...
		ПараметрыФормы.Вставить("ОписаниеОсобое", "");
...
Процедура УчастникиДействияПередНачаломДобавления(Форма, Отказ, Копирование, ПараметрыДобавленияУчастников) Экспорт
	...
	Если СЭтапами Тогда
		...
		Если ИнтеграцияС1СДокументооборот3КлиентСервер.ЭтоФункцияОбработатьРезультат(ТекущиеДанные.Функция) Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Добавление в этап обработки результата невозможно.'; en = 'Cannot add to the result processing stage.'"));
			Возврат;
		КонецЕсли;
	...
	Если ИндексПредыдущегоЭлемента >= 0 Тогда
		...
		Если СтрокаУчастника.Функция = "ReviewingPerson" Тогда
	...
	Форма.ЗаполнитьПредставлениеСроков();
	Если СтрокаУчастника.Функция = "ReviewingPerson" Тогда
		Форма.ЗаполнитьПредставлениеСроковОбработкиРезолюции();
	КонецЕсли;
			...
			Если СтрокаДерева.ЭтоЭтап И МожноУдалятьЭтапы Тогда
				...
				Если СчЭтаповДанногоВида < 2 Тогда
					Если СтрокаДерева.Функция = "Signer" Тогда
						ТекстПредупреждения = НСтр("ru = 'Удаление единственного этапа подписания невозможно.'; en = 'Cannot delete the only signing stage.'");
					ИначеЕсли СтрокаДерева.Функция = "ApprovingPerson" Тогда
						ТекстПредупреждения = НСтр("ru = 'Удаление единственного этапа согласования невозможно.'; en = 'Cannot delete the only approval stage.'");
					ИначеЕсли ИнтеграцияС1СДокументооборот3КлиентСервер.ЭтоФункцияОбработатьРезультат(СтрокаДерева.Функция) Тогда
						ТекстПредупреждения = НСтр("ru = 'Удаление этапа обработки результата невозможно.'; en = 'Cannot delete the result processing stage.'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция ЭтапыДействияУникальны(Участники) Экспорт
	...
	Для Каждого ЭлементЭтап Из Участники.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(ЭлементЭтап.НаименованиеЭтапа)
			...
			ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'Повторяется этап ""%1""'; en = 'The ""%1"" stage is duplicated'"), ЭлементЭтап.НаименованиеЭтапа));
			Возврат Ложь;
		КонецЕсли;
...
// Открывает связанный объект на просмотр.
//
// Параметры:
//   СтрокаДереваСвязей - ДанныеФормыЭлементДерева - текущая строка дерева связей.
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьСвязанныйОбъектНаПросмотр(Знач СтрокаДереваСвязей, УникальныйИдентификаторФормы) Экспорт
	
	Если СтрокаДереваСвязей = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрокаДереваСвязей.ЭтоВеткаСвязи Тогда
		ОткрытьФормуДокументооборотИзДереваСвязей(
			"СтраницаСвязи",
			СтрокаДереваСвязей.ПолучитьРодителя(),
			УникальныйИдентификаторФормы);
		Возврат;
	ИначеЕсли СтрокаДереваСвязей.ЭтоРольФайла Тогда
		СтрокаДереваСвязей = СтрокаДереваСвязей.ПолучитьРодителя();
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрокаДереваСвязей.СвязаннаяСтрока) Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СтрокаДереваСвязей.СвязаннаяСтрока);
		Возврат;
	КонецЕсли;
	
	ОбъектДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО(
		СтрокаДереваСвязей.СвязанныйОбъектID,
		СтрокаДереваСвязей.СвязанныйОбъектТип,
		СтрокаДереваСвязей.СвязанныйОбъект,,
		СтрокаДереваСвязей.НавигационнаяСсылка);
	ОткрытьОбъектНаПросмотр(
		СтрокаДереваСвязей.Ссылка,
		ОбъектДО,
		СтрокаДереваСвязей.РасширениеФайла,
		УникальныйИдентификаторФормы);
...
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьФормуДокументооборотИзДереваСвязей(НачальнаяСтраница, Знач СтрокаДереваСвязей,
		УникальныйИдентификаторФормы) Экспорт
	
	Если СтрокаДереваСвязей = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если СтрокаДереваСвязей.ЭтоВеткаСвязи Или СтрокаДереваСвязей.ЭтоРольФайла Тогда
		СтрокаДереваСвязей = СтрокаДереваСвязей.ПолучитьРодителя();
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрокаДереваСвязей.Ссылка) Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВнешнийОбъект", СтрокаДереваСвязей.Ссылка);
		ПараметрыФормы.Вставить("НачальнаяСтраница", НачальнаяСтраница);
		ОткрытьФорму(
			"Обработка.ИнтеграцияС1СДокументооборот3.Форма.Документооборот",
			ПараметрыФормы,
			...
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОткрытьСвязанныйОбъектНаПросмотр(СтрокаДереваСвязей, УникальныйИдентификаторФормы);
	КонецЕсли;
...
Процедура ПоказатьНарушениеПравилКоммуникаций() Экспорт
	...
	ТекстПредупреждения = НСтр("ru = 'Выбранный элемент недоступен по правилам коммуникаций'; en = 'The selected item is unavailable according to the communication rules'");
	ПоказатьПредупреждение(,ТекстПредупреждения);
...
Функция ДоступноИзменениеЗащищенностиУчастника(Участник, МожноИзменятьЗащищенных)
	...
	Если Родитель <> Неопределено Тогда
		ЭтоРассмотрение = (Родитель.Функция = "ReviewingPerson") И Не Родитель.ЭтоЭтап;
	КонецЕсли;
...
Процедура ОбработатьРезультатПроверкиДействительностиУчастниковДействия(РезультатПроверки, ПараметрыДляЗаписи)
	...
	КнопкиВопроса.Добавить(1, НСтр("ru = 'Оставить как есть'; en = 'Keep as-is.'"));
...
Функция ТекстПредупрежденияПриУдаленииЭтапа()
	...
	Возврат НСтр("ru = 'Удаление этапа невозможно.'; en = 'Cannot delete the stage.'");
...
Функция ТекстПредупрежденияПриУдаленииЗащищенных()
	...
	Возврат НСтр("ru = 'Удаление защищенных участников невозможно.'; en = 'Cannot delete protected participants.'");
...
Функция ТекстПредупрежденияПриУдаленииЗавершившего()
	...
	Возврат НСтр("ru = 'Удаление участника завершившего задачу невозможно.'; en = 'Cannot delete a participant who completed a task.'");
...
Функция ТекстПредупрежденияПриУдаленииНедоступного()
	...
	Возврат НСтр("ru = 'Удаление участника невозможно.'; en = 'Cannot delete the participant.'");
...
Процедура УстановитьДоступностьКнопкиРазныеСроки(Форма)
	...
	Если Форма.Элементы.СрокПредставление.ТолькоПросмотр Тогда
		Если Не СтрокаДерева.ЭтоЭтап Тогда
			Возврат; // Если изменение срока недоступно, то и кнопку нажимать нельзя.
		КонецЕсли;
		
		// Если это этап, то нужно проверить доступно ли изменение участников этого этапа.
...
//
// Параметры:
//   НаименованиеИОписание - Структура:
//     * Наименование - Строка
//     * Описание - Строка
...
//     * ТекущиеДанные - ДанныеФормыЭлементДерева
//
Процедура ЗавершитьНастройкуОсобогоНаименованияИОписанияИсполнителя(НаименованиеИОписание, ПараметрыОповещения) Экспорт
	...
	ПараметрыОповещения.ТекущиеДанные.ЕстьОсобоеНаименованиеОписание =
		ЗначениеЗаполнено(НаименованиеИОписание.Наименование)
			Или ЗначениеЗаполнено(НаименованиеИОписание.Описание)
			Или ЗначениеЗаполнено(НаименованиеИОписание.ОписаниеОсобое);
	
	Если ПараметрыОповещения.ИзменятьНаименованиеЗадачи Тогда
		ПараметрыОповещения.ТекущиеДанные.НаименованиеЗадачи = НаименованиеИОписание.Наименование;
	КонецЕсли;
	НаименованиеИОписание.Удалить("Наименование");
	
	ЗаполнитьЗначенияСвойств(ПараметрыОповещения.ТекущиеДанные, НаименованиеИОписание);
...
// Формирует ключ строки дерева узлов КОД.
//
// Параметры:
//   СтрокаДереваУзлыКОД - Структура:
//     * АдресВебСервиса - Строка
//     * Активен - Число
//     * ИдентификаторУзла - Строка
//     * ИмяУзла - Строка
//     * ПредставлениеУзла - Строка
//     * ЦентральныйУзел - Булево
//
// Возвращаемое значение:
//   Строка
//
Функция КлючСтрокиДереваУзлыКОД(СтрокаДереваУзлыКОД)
	
	Возврат СтрШаблон("%1_%2", СтрокаДереваУзлыКОД.ИмяУзла, СтрокаДереваУзлыКОД.ИдентификаторУзла);
...
Процедура ОбработатьДанныеВыполненияПослеИзмененияПроцесса(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат = Неопределено
			Или (ТипРезультата = Тип("КодВозвратаДиалога") И Результат <> КодВозвратаДиалога.ОК) Тогда
		// Вернуться к задаче.
		Возврат;
	КонецЕсли;
...
// Обработчик оповещения после ответа на произвольный вопрос перед выполнением задачи.
//
// Параметры:
...
// Обработчик оповещения после ответа на вопрос о достижении лимита циклов перед выполнением задачи.
//
// Параметры:
//   Ответ - Строка - ответ от пользователя в диалоге вопроса.
//   ПараметрыОповещения - Структура:
...
//     * ПовторноеДействие - ОписаниеОповещения - метод, который будет вызван по завершении интерактивной обработки.
//
Процедура ОбработатьДанныеВыполненияПослеОтветаНаВопросОЛимитеЦиклов(Ответ, ПараметрыОповещения) Экспорт
	
	ТребуемоеИнтерактивноеДействие =
		ПараметрыОповещения.ПовторноеДействие.ДополнительныеПараметры.ТребуемоеИнтерактивноеДействие;
	ТребуемоеИнтерактивноеДействие.ЗадатьВопросОЛимитеЦиклов = Истина;
	ТребуемоеИнтерактивноеДействие.Контекст = Ответ;
...
Функция ОсновнойПредметВДеревеПриложений(СтрокаДереваПриложений, ДеревоПриложений)
	
	Если СтрокаДереваПриложений <> Неопределено И НРег(СтрокаДереваПриложений.ТипСтроки) = "application" Тогда
		Возврат СтрокаДереваПриложений;
	КонецЕсли;
	
	Для Каждого Элемент Из ДеревоПриложений.ПолучитьЭлементы() Цикл
		Если НРег(Элемент.ТипСтроки) = "application" Тогда
			Возврат Элемент;
		КонецЕсли;
	КонецЦикла;
...
//   УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор вызвавшей формы.
//
Процедура ОткрытьОбъектНаПросмотр(ОбъектИС, ОбъектДО, РасширениеФайла, УникальныйИдентификаторФормы)
	
	Если ЗначениеЗаполнено(ОбъектИС) Тогда
		ПоказатьЗначение(, ОбъектИС);
		
	ИначеЕсли ОбъектДО.Тип = "DMFile" Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьФайл(
			ОбъектДО.ID,
			ОбъектДО.Наименование,
			РасширениеФайла,
			УникальныйИдентификаторФормы);
		
	ИначеЕсли ЗначениеЗаполнено(ОбъектДО.НавигационнаяСсылка)
			И (ИнтеграцияС1СДокументооборот3КлиентПовтИсп.ЭтоДокументДО3(ОбъектДО.Тип)
				Или ОбъектДО.Тип = "DMCorrespondent"
				Или ОбъектДО.Тип = "DMMeeting"
				Или ОбъектДО.Тип = "DMIncomingElectronicDocument"
				Или ОбъектДО.Тип = "DMOutgoingElectronicDocument") Тогда
		ОткрытьОбъектДО(ОбъектДО.НавигационнаяСсылка);
...
// Выводит на экран окно вопроса пользователю.
//
// Параметры:
//   СтруктураВопроса - см. ИнтеграцияС1СДокументооборот3.СтруктураВопросаПользователю
//   ОписаниеОповещенияОЗавершении - ОписаниеОповещения - содержит описание процедуры, которая будет
//     вызвана после ответа на вопрос.
//
Процедура ПоказатьВопросПередВыполнениемЗадачи(СтруктураВопроса, ОписаниеОповещенияОЗавершении)
	
	ЗаданаКнопкаПоУмолчанию = ЗначениеЗаполнено(СтруктураВопроса.КнопкаПоУмолчанию)
		И (СтруктураВопроса.Кнопки.НайтиПоЗначению(СтруктураВопроса.КнопкаПоУмолчанию) <> Неопределено);
	
	Если ЗаданаКнопкаПоУмолчанию Тогда
		ПоказатьВопрос(
			ОписаниеОповещенияОЗавершении,
			СтруктураВопроса.Текст,
			СтруктураВопроса.Кнопки,,
			СтруктураВопроса.КнопкаПоУмолчанию,
			СтруктураВопроса.Заголовок);
	Иначе
		ПоказатьВопрос(
			ОписаниеОповещенияОЗавершении,
			СтруктураВопроса.Текст,
...
Процедура РазвернутьДеревоУзлыКОД(СтрокиДерева, ТаблицаФормы)
	
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		ТаблицаФормы.Развернуть(СтрокаДерева.ПолучитьИдентификатор());
		РазвернутьДеревоУзлыКОД(СтрокаДерева.ПолучитьЭлементы(), ТаблицаФормы);
	КонецЦикла;
