...
Функция ОтменитьПроведениеОбъектаВИБ(Объект, Отправитель, КомпонентыОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонТекста = НСтр("ru = 'При отмене проведения документа:
			...
			|For more information about the error, see the event log.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыгрузитьУдаление(КомпонентыОбмена, Ссылка, ПравилоКонвертации = Неопределено)
	...
	Если ПравилоКонвертации <> Неопределено Тогда
		...
		уатОбменДанными.ЗаписатьУдалениеОбъектаXDTO(КомпонентыОбмена, Ссылка, ПравилоКонвертации.ТипСсылкиXDTO);
		// {/УАТ}
	Иначе
		...
		Для Каждого ПравилоКонвертацииИмя Из МассивИменПКО Цикл
			...
			уатОбменДанными.ЗаписатьУдалениеОбъектаXDTO(КомпонентыОбмена, Ссылка, ТипСсылкиXDTO);
			// {/УАТ}
...
Процедура ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеПроходитФильтр(КомпонентыОбмена, ОписаниеСсылкиЗапроса, ПодобраннаяОбъектнаяСсылка)
	...
	ШаблонТекста = НСтр("ru = 'Объект %1 не будет повторно выгружен, т.к. не соответствует условиям фильтра.';
						|en = 'Object %1 will not be re-exported because it does not meet the filter conditions.'");
...
Процедура ФиксироватьПредупреждениеЗапросаНаВыгрузкуСсылкаНеНайдена(КомпонентыОбмена, ОписаниеСсылкиЗапроса)
	...
	ОписаниеОшибки = НСтр("ru = 'Ссылка с идентификатором %1 по объекту формата %2 не будет повторно выгружена,
							...
							|en = 'Reference with ID %1 for format object %2 will not be re-exported,
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено ПОД для типа объекта XDTO.
					|Тип объекта XDTO: %1
					|Описание ошибки: %2';
					|en = 'Cannot find a DPR for an XDTO object type.
					|XDTO object type: %1.
					|Error details: %2'"),
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	Если ПравилоОбработки = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Объект метаданных: %3.';
			|en = 'Cannot find a DPR for a metadata object.
			|Metadata object: %3.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1';
																						|en = 'Cannot find a DPR with name ""%1"".'"), Имя);
