...
// Возвращаемое значение:
//  Булево
Функция ЭтоКодОшибкиКонтроляНедействительныхСертификатов(ИдентификаторОператора, КодОшибки) Экспорт
	
	Результат = Ложь;
	РезультатЗапросаКодОшибкиНедействительныхСертификатов =
		РезультатЗапросаКодОшибкиНедействительныхСертификатов(ИдентификаторОператора);
	Выборка = РезультатЗапросаКодОшибкиНедействительныхСертификатов.Выбрать();
	Если Выборка.Следующий() Тогда
		КодОшибкиСтрокой = Строка(КодОшибки);
		КодОшибкиСтрокой = СтрЗаменить(КодОшибкиСтрокой, Символ(160), "");
		КодОшибкиСтрокой = ВРег(КодОшибкиСтрокой);
		КодОшибкиНедействительныхСертификатов = ВРег(Выборка.КодОшибкиНедействительныхСертификатов);
		Результат = (КодОшибкиСтрокой = КодОшибкиНедействительныхСертификатов);
	КонецЕсли;
	Возврат Результат;
...
Функция ОператорКонтролируетНедействительныеСертификаты(ИдентификаторОператора) Экспорт
	...
	Выборка = РезультатЗапросаКодОшибкиНедействительныхСертификатов.Выбрать();
	Если Выборка.Следующий() Тогда
		Результат = (СокрЛП(Выборка.КодОшибкиНедействительныхСертификатов) <> "");
	КонецЕсли;
	Возврат Результат;
...
Функция ДанныеАбонентовИзСервисаНастроек(Отбор, Отказ, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если Отбор.СписокИНН.Количество() = 0 И Отбор.ИдентификаторыЭДО.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ДанныеАбонентов = Неопределено;
		
	Пока КоличествоПолучено = РазмерСтраницы Цикл
		
		Отбор.Страница = Страница;
		ПараметрыКоманды = ПараметрыКомандыПолученияДанныхАбонентовВСервисНастроек(Отбор);
		
		ДанныеАбонентовПорции = ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, КонтекстДиагностики);
		
		Если Отказ Тогда
			Возврат Результат;
		КонецЕсли;
		
		Если ДанныеАбонентов = Неопределено Тогда
			ДанныеАбонентов = ДанныеАбонентовПорции;
		Иначе
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДанныеАбонентов.identifierInfos,
				ДанныеАбонентовПорции.identifierInfos);
		КонецЕсли;
		
		РазмерСтраницы = ДанныеАбонентовПорции.pageSize;
		КоличествоПолучено = ДанныеАбонентовПорции.identifierInfos.Количество();
...
// Возвращаемое значение:
//  См. ДанныеФайлаОблачногоХранилищаНастроек
Функция ДатаНачалаИсключенияСведенийДУЛПредставителяИзМЧД() Экспорт
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение даты исключения из МЧД сведений документа, удостоверяющего личность физического лица представителя.'"));
	Путь = "/mchd/DateOfExclusionOfInformationFromEMCHD.json";
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
...
Функция ТипДокументаПоЗначениюСервиса(Значение) Экспорт
	...
	ТрансграничныйЭДО.ДополнитьТипыДокументов(ТипыДокументов);
...
Функция ОбработкаРезультатаСвязьТиповИФорматовЭД(Знач ПолученныеДанные, Отказ = Ложь, КонтекстДиагностики = Неопределено) Экспорт
	...
	ТрансграничныйЭДО.ДополнитьСвязьТиповИФорматовЭД(ПолученныеДанные);
...
Функция ОбработкаРезультатаКаталогФорматовЭД(Знач ПолученныеДанные, Отказ = Ложь, КонтекстДиагностики = Неопределено) Экспорт
	...
	ТрансграничныйЭДО.ДополнитьДаннымиФорматовЭД(ПолученныеДанные);
...
Функция ОбработкаРезультатаКаталогОператоров(ПолученныеДанные, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого ОператорЭДО Из ПолученныеДанные.operators Цикл
		...
		НоваяСтрока.КодОшибкиНедействительныхСертификатов = ОператорЭДО.inactiveCertificatesErrorCode;
...
Функция ВидОшибкиНетПравНаИзменениеИнформацииПоУчетнойЗаписи()
	...
	ОписаниеРешения = НСтр("ru = 'Способ устранения описан <a href = ""%1"">здесь</a>'");
	ВидОшибки.ОписаниеРешения = СтрШаблон(ОписаниеРешения, "http://1c-edo.ru/handbook/24/4358/");
...
// Возвращаемое значение:
//  Строка
Функция ТекстЗапросаКодОшибкиНедействительныхСертификатов()
	
	Возврат
		"ВЫБРАТЬ
		|	ОператорыЭДО.КодОшибкиНедействительныхСертификатов КАК КодОшибкиНедействительныхСертификатов
		|ИЗ
		|	РегистрСведений.ОператорыЭДО КАК ОператорыЭДО
		|ГДЕ
		|	ОператорыЭДО.ИдентификаторОператора = &ИдентификаторОператора";
...
// Возвращаемое значение:
//  РезультатЗапроса
Функция РезультатЗапросаКодОшибкиНедействительныхСертификатов(ИдентификаторОператора)
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаКодОшибкиНедействительныхСертификатов();
	Запрос.УстановитьПараметр("ИдентификаторОператора", ИдентификаторОператора);
