	...
	Если Не ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки() Тогда
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Сервис временно недоступен. Отсутствует подключение к интернет-поддержке пользователей.';
												|en = 'The service is temporarily unavailable. Online support is disabled.'");
		...
		Иначе
			КраткоеПредставлениеОшибки = 
				НСтр("ru = 'Для автоподбора и проверки адресов необходимо подключиться к интернет-поддержке пользователей или загрузить адресные сведения.';
					|en = 'To enable the address auto-completion and validation service, enable online support or import address details.'");
		КонецЕсли;
...
Функция ИнформацияДляПоддержки(Знач ПредставлениеАдреса, Знач СведенияОбАдресе) Экспорт
	...
	Результат.Добавить("");
	Результат.Добавить(НСтр("ru = 'Отчет создан';
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описание.Наименование               = НСтр("ru = 'Часовые зоны субъектов РФ';
												|en = 'Time zones of federal subjects of Russia'");
...
Процедура СписокИзвлеченныхФайлов(КаталогФайлов)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание, , ,
		...
		|en = 'List of extracted files with address details for the region: %1'"), СтрСоединить(СписокФайлов, ",")));
КонецПроцедуры
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		ИначеЕсли Не АдресныеСведенияРегиона.Загружен Тогда
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(Результат.Ошибки, "СубъектРФ",
				...
				|en = 'No address details for state %1'"), НаименованиеРегиона));
	...
	Для Позиция = 0 По ОКТМОБюджетополучателей.ВГраница() Цикл
		...
		Иначе
			...
			ДанныеПоОКТМО.ТекстОшибки = НСтр("ru = 'Недопустимая длина кода ОКТМО бюджетополучателя. Требуется 8 цифр.';
											|en = 'Invalid Municipal territory classifier code for the budget recipient. Code must be exactly 8 digits.'");
		...
		Если ПустаяСтрока(ПолученныеДанные) Тогда
			...
			Результат.ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Веб-сервис адресов успешно выполнил запрос ""%1"", но вернул пустой результат.';
					|en = 'The address web service successfully executed the request ""%1"", but an empty result was returned.'"), СтрокаЗапроса);
...
Функция ТекстОшибкиПриРаботеССервисомДляПользователя()
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ТекстОшибки = НСтр("ru = 'Сервис для подбора и проверки адресов временно недоступен.';
							|en = 'The address auto-completion and validation service is temporarily unavailable.'");
	Иначе
		ТекстОшибки = НСтр("ru = 'Сервис для подбора и проверки адресов временно недоступен по причине:
			...
			|en = 'The address validation service is currently unavailable. Possible reasons:
			|- Internet service provider issues.
			|- Server connection is blocked by firewall, antivirus, or other software.
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Не удалось очистить регистр сведений Уровни сокращений адресных сведений.';
				|en = 'Cannot clear the Address details shortening levels information register.'",
