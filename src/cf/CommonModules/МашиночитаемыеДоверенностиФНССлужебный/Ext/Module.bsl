		...
		И (Пользователи.ЭтоПолноправныйПользователь() Или ПравоДоступа("Просмотр",
			...
			Заголовок.Добавить(СтрШаблон("<a href = ""%1"">%2</a>", "РегистрСведений.ЖурналМашиночитаемыхДоверенностей.ФормаСписка",
				НСтр("ru = 'Для электронного документооборота (ЭДО)';
					|en = 'For EDI (Electronic Data Interchange)'")));
	КонецЕсли;
		...
		Если (Пользователи.ЭтоПолноправныйПользователь() Или ПравоДоступа("Просмотр",
			...
			Заголовок.Добавить(СтрШаблон("<a href = ""%1"">%2</a>", "Справочник.МашиночитаемыеДоверенностиРаспределенныйРеестр.ФормаСписка",
				НСтр("ru = 'Для ФНС (распределенный реестр)';
					|en = 'For Federal Tax Service (distributed ledger)'")));
		...
		Если (Пользователи.ЭтоПолноправныйПользователь() Или ПравоДоступа("Просмотр",
			...
			Заголовок.Добавить(СтрШаблон("<a href = ""%1"">%2</a>", "Справочник.МашиночитаемыеДоверенностиФСС.ФормаСписка",
				НСтр("ru = 'Для СФР (бывший ФСС)';
					|en = 'For Social Fund of Russia (former Social Insurance Fund of Russia)'")));
		КонецЕсли;
	КонецЕсли;
		...
		Если Заголовок.Количество() > 0 Тогда
			...
			Заголовок.Вставить(0, СтрШаблон("<a href = ""%1"">%2</a>", "Справочник.МашиночитаемыеДоверенности.ФормаСписка",
				НСтр("ru = 'Единого формата - для отчетности, кадрового ЭДО';
					|en = 'Unified format for reports and HR EDI'")));
		Иначе
			...
			Если Форма.ИмяФормы = "ОбщаяФорма.НастройкиЭлектроннойПодписиИШифрования" Тогда
				ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Машиночитаемые доверенности: %1';
						|en = 'Machine-readable LOA: %1'"), ТекстЗаголовка);
			Иначе
				ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'См. также Машиночитаемые доверенности: %1';
						|en = 'See also Machine-readable LOA: %1'"), ТекстЗаголовка);
			КонецЕсли;
		КонецЕсли;
...
Функция ДанныеДоверенностиДляПечати(Доверенность, НаДату = Неопределено) Экспорт
	...
	Результат.ПредставлениеДоверенности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%5выдана от %4';
			|en = '%1
			|%5 is valid from %2 to %3,
			|%5 issued on %4'"), РеквизитыДоверенности.НомерДоверенности, Формат(
	...
	Если ТипЗнч(ДоверенностьНастройка) = Тип("Структура") И ЗначениеЗаполнено(ДоверенностьНастройка.Доверенность) Тогда
		Элементы.МашиночитаемаяДоверенность.СписокВыбора.Добавить(ДоверенностьНастройка.Доверенность,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (Персональная настройка)';
					|en = '%1 (personal setting)'"), ДоверенностьНастройка.Доверенность));
	...
	Если ЗначениеЗаполнено(ДанныеДоверенности.НомерДоверенности)
		...
		РезультатПроверкиВРеестреФНС = ЧастичныеДанныеДоверенностиМЧДРР(
			ДанныеДоверенности.НомерДоверенности,,НСтр("ru = 'Не удалось проверить доверенность';
														|en = 'Cannot verify the letter of authority'"));
	...
	Иначе
		...
		Если КоличествоДоверителей = 0 Тогда
			Возврат НСтр("ru = 'Не заполнены данные доверителя в доверенности.';
						|en = 'Principal details are required.'");
		КонецЕсли;
...
Функция ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибки) Экспорт
	...
	ДобавитьФайлДляРасследования(ДополнительныеФайлы, ДанныеФайла, НСтр("ru = 'Сообщение классификатора';
																		|en = 'Classifier message'"), "xml");
...
Процедура ЗагрузитьКвитанцииИзРеестра(Доверенность, ОшибкаПриЗагрузке, УникальныйИдентификатор) Экспорт
	...
	Если РеквизитыДоверенности.Количество() = 0 Тогда
		ОшибкаПриЗагрузке = НСтр("ru = 'Не удалось получить реквизиты доверенности для загрузки. Загрузите доверенность из реестра ФНС вручную';
								|en = 'Couldn''t retrieve letter of authority details. Download the letter of authority manually from the FTS distributed registry'");
		Возврат;
	КонецЕсли;
	...
	ПолныеДанныеДоверенностиМЧДРР = ПолныеДанныеДоверенностиМЧДРР(
		...
		|en = 'Couldn''t import the letter of authority'"));
...
Функция ПрочитатьСостояниеМассиваМЧД(Доверенности)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Успешно
			...
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">: Ожидает регистрации в информационном ресурсе ""Доверенность"" ФНС России</span>';
					|en = '<span style=""color: %1"">: Pending registration in the Federal Tax Service''s LOA service</span>'"), "ПоясняющийТекст");
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Успешно
			И Выборка.ТипТранзакции = Перечисления.ТипыТранзакцийСРеестромМЧД.Регистрация Тогда
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Ошибка
			...
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">: <a href = ""%2"">Не удалось зарегистрировать</a>';
					|en = '<span style=""color: %1"">: <a href = ""%2"">Registration failed</a>'"), "ПоясняющийТекст", "ПоказатьОшибку");
		ИначеЕсли Выборка.СтатусТранзакции = Перечисления.СтатусыТранзакцииСРеестромМЧД.Ошибка
			И Выборка.ТипТранзакции = Перечисления.ТипыТранзакцийСРеестромМЧД.Отмена Тогда
			ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">: <a href = ""%2"">Не удалось отменить</a>';
					|en = '<span style=""color: %1"">: <a href = ""%2"">Revocation failed</a>'"), "ПоясняющийТекст", "ПоказатьОшибку");
...
Функция АвторизоватьсяНаСервереМЧДРР(ЦельАвторизации = "") Экспорт
	...
	Если Не ЭлектроннаяПодпись.РазрешенДоступКИнтернетСервисам() Тогда
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Не удается авторизоваться на сервере Распределенного реестра ФНС России';
														|en = 'Cannot authorize on the FTS distributed ledger server'"), ЦельАвторизации);
	...
	Исключение
		...
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Отказ при получении токена доступа на сервере Распределенного реестра ФНС России';
														|en = 'Cannot receive an access token on the FTS distributed ledger server'"), ЦельАвторизации);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, ПредставлениеОшибки);
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
	...
	Если НЕ ЗначениеЗаполнено(Результат.ТокенДоступа) Тогда
		...
		ЗаголовокОшибки = ?(ЦельАвторизации = "", НСтр("ru = 'Распределенный реестр ФНС России не предоставил токен для авторизации';
														|en = 'The FTS distributed ledger did not provide the authorization token'"), ЦельАвторизации);
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил токен для авторизации.';
				|en = 'The FTS distributed ledger did not provide the authorization token.'"));
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
		ПараметрыОшибкиМЧДРР.СтруктураОтвета = СтруктураОтвета;
		ПараметрыОшибкиМЧДРР.ДополнительныеФайлы = ДополнительныеФайлы;
...
Функция НомерМЧДРР(ТокенДоступа = "") Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось получить номер доверенности';
							|en = 'Couldn''t get the LOA number'");
	...
	Если НЕ ЗначениеЗаполнено(Результат.НомерДоверенности) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не выдал номер доверенности.';
				|en = 'The FTS distributed ledger did not provide the LOA number.'"));
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
		ПараметрыОшибкиМЧДРР.СтруктураОтвета = СтруктураОтвета;
		ПараметрыОшибкиМЧДРР.ДополнительныеФайлы = ДополнительныеФайлы;
...
Функция ЗарегистрироватьМЧДРР(
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось зарегистрировать доверенность';
							|en = 'Cannot register the letter of authority'");
	...
	Если НЕ ЗначениеЗаполнено(Результат.ИдентификаторТранзакции) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки,
			 НСтр("ru = 'Распределенный реестр ФНС России не предоставил идентификационные данные.';
					|en = 'The FTS distributed ledger did not provide identification data.'"));
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
		ПараметрыОшибкиМЧДРР.СтруктураОтвета = СтруктураОтвета;
		ПараметрыОшибкиМЧДРР.ДополнительныеФайлы = ДополнительныеФайлы;
	...
	Если НЕ ЗначениеЗаполнено(Результат.ИдентификаторТранзакции) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, 
			НСтр("ru = 'Распределенный реестр ФНС России не вернул данные о статусе транзакции.';
				|en = 'The FTS distributed ledger did not return the transaction status data.'"));
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
		ПараметрыОшибкиМЧДРР.СтруктураОтвета = СтруктураОтвета;
		ПараметрыОшибкиМЧДРР.ДополнительныеФайлы = ДополнительныеФайлы;
...
Функция ХронологическиеДанныеДоверенностиМЧДРР(Доверенность, ТокенДоступа = "", ЦельПолучения = "") Экспорт
	...
	ЗаголовокОшибки = ?(ЦельПолучения = "", НСтр("ru = 'Не удалось получить хронологические данные доверенности';
												|en = 'Couldn''t retrieve historical data for the LOA'"), ЦельПолучения);
	...
	Если Не ЗначениеЗаполнено(Результат.ХронологическиеДанные) Тогда
		...
		ПараметрыОшибкиМЧДРР = ПараметрыОшибкиМЧДРР(ЗаголовокОшибки, НСтр(
			...
			|en = 'The FTS distributed ledger did not return historical data for the LOA.'"));
		ПараметрыОшибкиМЧДРР.ЗапросHTTP = ЗапросHTTP;
		ПараметрыОшибкиМЧДРР.ОтветHTTP = ОтветHTTP;
		ПараметрыОшибкиМЧДРР.СтруктураОтвета = СтруктураОтвета;
		ПараметрыОшибкиМЧДРР.ДополнительныеФайлы = ДополнительныеФайлы;
...
Функция ПолныеДанныеДоверенностиМЧДРР(НомерДоверенности, ИННДоверителя, ИННПредставителя, ТокенДоступа = "", ЦельПолучения = "")
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Распределенный реестр ФНС России не предоставил полных данных доверенности, статус запроса данных: %1 (%2).';
				|en = 'The FTS distributed registry did not provide complete LOA data. The request status is %1 (%2).'"),
	...
	Если ЗначениеЗаполнено(Результат.СтатусПолучения) Тогда
		Результат.Ошибка.Решение = СтроковыеФункции.ФорматированнаяСтрока(
			...
			|en = 'Please try again later or download the LOA archive from the <a href=%1>official website of the FTS distributed registry</a> and upload it from a file using the command in the <a href=%2>machine-readable LOA list</a>. 
			|If needed, contact <a href=%3>FTS technical support</a>.'"),
