...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияВерсииПрограммы() Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Версия приложения обновлена, требуется перезапустить сеанс.';
						|en = 'The application is updated. Restart the application.'");
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияРасширенийПрограммы() Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Расширения приложения обновлены, требуется перезапустить сеанс.';
							|en = 'Extensions are updated. Restart the application.'");
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ru = 'Для возможности работы приложения.';
			|en = 'Basic permissions required to run the application.'")));
...
Процедура ПроверитьВозможностьЗапуска()
	...
	Рекомендация = НСтр("ru = 'Для восстановления работоспособности приложения, очистите свойство ""Профиль безопасности"" у информационной базы через консоль кластера серверов 1С:Предприятия и заново настройте профили безопасности в панели Администрирование - Общие настройки - Параметры серверной информационной базы - Использование профилей безопасности.';
						|en = 'To fix the issue, clear the ""Security profile"" property using the 1C:Enterprise server cluster console. Then, reconfigure the security profiles in Administration > General settings > Server infobase parameters > Security profiles.'");
			...
			Если Не ДоступенПривилегированныйРежим Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В информационной базе указано имя профиля безопасности ""%1"" (Администрирование - Общие настройки - Параметры серверной информационной базы - Использование профилей безопасности - Профиль безопасности информационной базы).
					           ...
					           |en = 'The infobase specifies the security profile ""%1"" (Administration > General settings > Server infobase parameters > Security profiles > Infobase security profile).
					           |
			...
			Если МодульРаботаВБезопасномРежиме <> Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В информационной базе не указано имя профиля безопасности (Администрирование - Общие настройки - Параметры серверной информационной базы - Использование профилей безопасности - Профиль безопасности информационной базы).
					           ...
					           |en = 'The infobase does not specify a security profile (Administration > General settings > Server infobase parameters > Security profiles > Infobase security profile).
					           |
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно установить параметры сеанса по причине:
					           ...
					           |en = 'Cannot set session parameters due to:
					           |%1.
					           |
					           |Perhaps, the server cluster configuration specifies a security profile for the infobase, but this profile is not set in the infobase.
