...
// Возвращаемое значение:
//   Структура:
//     * АдресВебСервисаДокументооборота - Неопределено
//     * ИнтеграцияС1СДокументооборотWSОпределения - ХранилищеЗначения
//     * ИнтеграцияС1СДокументооборотВерсияСервиса - Строка
//     * ИнтеграцияС1СДокументооборотИмяПользователя - Строка
//     * ИнтеграцияС1СДокументооборотИспользуетсяАутентификацияJWT - Булево
...
//     * ИнтеграцияС1СДокументооборотПароль - Строка
//     * ИнтеграцияС1СДокументооборотПарольИзвестен - Булево
//     * ИнтеграцияС1СДокументооборотУзел - Строка
//
Функция ПараметрыДлительнойОперации(ИспользоватьКэш = Истина, АдресВебСервисаДокументооборота = Неопределено) Экспорт
	
	ПараметрыОперации = Новый Структура;
	
	ПараметрыОперации.Вставить("АдресВебСервисаДокументооборота", АдресВебСервисаДокументооборота);
	...
	ПараметрыОперации.Вставить("ИнтеграцияС1СДокументооборотУзел", "");
	...
	Если ИспользоватьКэш Тогда
		...
		ПараметрыОперации.ИнтеграцияС1СДокументооборотУзел = ПараметрыСеанса.ИнтеграцияС1СДокументооборотУзел;
	КонецЕсли;
...
Процедура ПодключитьсяИПолучитьВерсиюСервисаВФоне(Параметры, АдресХранилища = "") Экспорт
	...
	УстановитьВерсиюСервиса(Истина,, Параметры.АдресВебСервисаДокументооборота);
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ИнтеграцияС1СДокументооборотУзел",
		"ИнтеграцияС1СДокументооборотБазоваяФункциональность.УстановитьПараметрыСеанса");
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Для Каждого ИмяФО Из СписокФО Цикл
		...
		Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОбменДаннымиС1СДокументооборотом;
		...
		Настройка = Настройки.Добавить();
		Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.УдалитьИнтеграцияС1СДокументооборотВыполнитьОбменДанными;
		Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции[ИмяФО];
	...
	Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда // Прочтем настройки из пользовательского хранилища.
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ru = 'Для пользователя регламентного задания обмена не указано имя пользователя 1С:Документооборота'; en = '1C:Document Management username is not specified for the user of scheduled exchange job'",
		...
		Если ПарольСохранен <> Истина Тогда
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,,,
				НСтр("ru = 'Для пользователя регламентного задания обмена не сохранен пароль 1С:Документооборота'; en = '1C:Document Management password is not saved for the user of scheduled exchange job'",
					ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаОбъектаИС)
		...
		Если ЗначениеЗаполнено(ИмяСвойства) И ЗначениеЗаполнено(ТипXDTOОбъекта) Тогда
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Ошибка при заполнении реквизита %1 объекта ИС типа %2 из реквизита %3 объекта ДО типа %4.'; en = 'An error occurred when filling the %1 attribute of the IS object of the %2 type from the %3 attribute of the 1C:Document Management object of the %4 type.'"),
		...
		Иначе
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Ошибка при заполнении реквизита %1 объекта ИС типа %2.'; en = 'An error occurred when filling the %1 attribute of the IS object of the %2 type.'"),
				ИмяРеквизитаОбъектаИС,
				Метаданные.НайтиПоТипу(ТипЗнч(ЗаполняемыйОбъектИС)).ПолноеИмя()));
		КонецЕсли;
	КонецЕсли;
	ТекстОшибки.Добавить(НСтр("ru = 'Нет возможности подобрать значение реквизита по данным для заполнения.'; en = 'Cannot select an attribute value by the data for filling.'"));
	ТекстОшибки.Добавить(СтрШаблон(
		НСтр("ru = 'Тип реквизита: %1.'; en = 'Attribute type: %1.'"), ОбщегоНазначения.СтроковоеПредставлениеТипа(ТипРеквизитаОбъектаИС)));
	ТекстОшибки.Добавить(СтрШаблон(
		НСтр("ru = 'Тип данных для заполнения: %1.'; en = 'Data type for filling: %1.'"), ТипЗнч(ЗначениеСвойства)));
	ТекстОшибки.Добавить(НСтр("ru = 'Данные для заполнения:'; en = 'Data for filling:'"));
	ТекстОшибки.Добавить(ПредставлениеЗначения);
	
	ЗаписатьПредупреждение(
		СтрСоединить(ТекстОшибки, Символы.ПС),
		НСтр("ru = 'Получение данных'; en = 'Receiving data'"),
		ЗаполняемыйОбъектИС);
...
"Обратитесь к администратору.'; en = 'Insufficient rights to create new items (%1)."
...
Процедура ПередЗаписьюОбъектаИС(ОбъектИС, Отказ,
	...
	СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru = 'Получение данных из 1С:Документооборота'; en = 'Get data from 1C:Document Management'"));
...
//
// Параметры:
//   АдресЦУ - Булево - требуется получить адрес центрального узла.
//
// Возвращаемое значение:
//   Строка - адрес сервиса.
//
Функция АдресВебСервиса1СДокументооборот(АдресЦУ = Ложь) Экспорт
	
	АдресВебСервиса = Константы.АдресВебСервиса1СДокументооборот.Получить();
	
	Если АдресЦУ = Ложь Тогда
		Настройки = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ИспользоватьИнтеграцию();
		Если Настройки.ИспользоватьИнтеграциюДО3 Тогда
			...
			ДанныеУзла = РегистрДанныеУзлов1СДокументооборота.ДанныеУзла(ПараметрыСеанса.ИнтеграцияС1СДокументооборотУзел);
			Если ЗначениеЗаполнено(ДанныеУзла.АдресВебСервиса) Тогда
				АдресВебСервиса = ДанныеУзла.АдресВебСервиса;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ОбработатьАдресВебСервиса1СДокументооборот(АдресВебСервиса);
...
Процедура ВыполнитьВБезопасномРежиме(Алгоритм, Параметры) Экспорт
	...
	ПроверитьАлгоритмНаЗапрещенныеВыражения(Алгоритм);
...
Функция ВыполнитьЗапрос(Прокси, Запрос) Экспорт
	...
	Исключение
		...
		Если Найти(КраткоеПредставлениеОшибки, НСтр("ru = 'Аутентификация пользователя не выполнена'; en = 'User authentication failed'")) <> 0 Тогда
...
"и пароль в настройках авторизации в 1С:Документообороте.'; en = 'Incorrect username or password. Check the name"
"and the password in authorization settings in 1C:Document Management.'");
		...
		Иначе
			...
			Если ВерсияСервиса() = "0.0.0.0" Тогда
				КраткоеПредставлениеОшибки = НСтр("ru = 'Нет доступа к 1С:Документообороту.'; en = 'No access to 1C:Document Management.'");
			КонецЕсли;
...
Функция ДанныеЗаполненияИнтегрируемогоОбъекта(УИДПравила, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не настроена интеграция с 1С:Документооборот'; en = 'Integration with 1C:Document Management is not configured'");
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыИзПотребителя(Прокси, ОбъектXDTO, ОбъектИС) Экспорт
	...
	ИначеЕсли ПроверитьТип(Прокси, ОбъектXDTO, "DMCountry")
			И ТипЗнч(ОбъектИС) = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипСтраныМира() Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ОбъектИС,
			"Код, КодАльфа2, КодАльфа3, УчастникЕАЭС, МеждународноеНаименование, НаименованиеПолное");
		ОбъектXDTO.code = Реквизиты.Код;
		ОбъектXDTO.codeAlpha2 = Реквизиты.КодАльфа2;
		ОбъектXDTO.codeAlpha3 = Реквизиты.КодАльфа3;
		ОбъектXDTO.EAEUMember = Реквизиты.УчастникЕАЭС;
		ОбъектXDTO.longDescription = Реквизиты.НаименованиеПолное;
		ОбъектXDTO.internationalDescription = Реквизиты.МеждународноеНаименование;
...
Функция ИмяСобытияЖурналаРегистрации(СобытиеВторогоУровня = "") Экспорт
	...
	Если СобытиеВторогоУровня = "" Тогда
		Возврат НСтр("ru = 'Интеграция с 1С:Документооборотом'; en = 'Integration with 1C:Document Management'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		Возврат СтрШаблон("%1.%2",
			НСтр("ru = 'Интеграция с 1С:Документооборотом'; en = 'Integration with 1C:Document Management'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция НайтиПоНаименованию(Менеджер, Наименование, ТочноеСоответствие = Ложь, Родитель = Неопределено,
	...
	Если Лев(ИмяМенеджера, 10) <> "Справочник" Тогда
...
Функция НовоеИсходящееПисьмо(Прокси, Предмет = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Эта версия 1С:Документооборота не поддерживает создание исходящих писем'; en = 'This version of 1C:Document Management does not support creation of outgoing emails'");
	КонецЕсли;
...
Функция НовыйОбъектИС(ПолноеИмя) Экспорт
	...
	Иначе
		ВызватьИсключение
			СтрШаблон(НСтр("ru = '1С:Документооборот не поддерживает интеграцию с объектами типа: %1'; en = '1C:Document Management does not support integration with objects of the %1 type'"), ПолноеИмя);
...
// Возвращает UUID по ссылке на объект ИС.
//
// Параметры:
...
// Возвращает имя типа по ссылке на объект ИС.
//
// Параметры:
//   Ссылка - ЛюбаяСсылка - ссылка на объект ИС.
//
...
Функция ПолучитьИмяТипаСсылки(Ссылка) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипСсылки) Тогда
...
Функция ПолучитьНастройки() Экспорт
	...
	Если СвойствоУстановлено(НастройкиXDTO, "showPdfBy1C") Тогда
		Настройки.Вставить("ПоказыватьPDFСредствами1С",
			НастройкиXDTO.showPdfBy1C И ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ЭтоТонкийКлиент());
	Иначе
		Настройки.Вставить("ПоказыватьPDFСредствами1С", Ложь);
	КонецЕсли;
...
Функция ПолучитьОбъект(Прокси, Тип, ID, Колонки = Неопределено, ПравилоИнтеграции = Неопределено,
	...
	Если Ответ.Количество() > 0 Тогда
		Возврат Ответ[0];
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось получить объект ДО: %1 (%2)'; en = 'Cannot get the DI object: %1 (%2)'"), ID, Тип);
	КонецЕсли;
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, НастройкиАвторизации = Неопределено,
	...
	Если НастройкиАвторизации.ИспользоватьКеш И ПараметрыСеанса.ИнтеграцияС1СДокументооборотПарольИзвестен
	...
	Иначе
		ОбработатьАдресВебСервиса1СДокументооборот(АдресВебСервисаДокументооборота);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АдресВебСервисаДокументооборота) Тогда
		ТекстСообщенияОбОшибке = НСтр("ru = 'Не указан адрес веб-сервиса 1С:Документооборота.'; en = 'Address of 1C:Document Management web service is not specified.'");
	...
	Если Определения = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя)
			...
			ДополнитьТекстСообщенияОбОшибке(
				ТекстСообщенияОбОшибке,
				НСтр("ru = 'Не указано имя пользователя 1С:Документооборота.'; en = '1C:Document Management username is not specified.'"));
		КонецЕсли;
		...
		Если НастройкиАвторизации.Пароль = Неопределено
			...
			ДополнитьТекстСообщенияОбОшибке(
				ТекстСообщенияОбОшибке,
				НСтр("ru = 'Не указан пароль для авторизации в 1С:Документооборот.'; en = 'Password for authorization in 1C:Document Management is not specified.'"));
		КонецЕсли;
...
""
"Подробности в журнале регистрации. Обратитесь к администратору системы.'; en = '1C:Document Management connection error."
""
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено,
	...
	Если РеквизитФормыПоИмени(РеквизитыФормы, "ИспользоватьИнтеграциюДО2") = Неопределено Тогда
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			...
			НСтр("ru = 'Использовать интеграцию с 1С:Документооборотом'; en = 'Use integration with 1C:Document Management'")));
	КонецЕсли;
	Если РеквизитФормыПоИмени(РеквизитыФормы, "ИспользоватьИнтеграциюДО3") = Неопределено Тогда
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			"ИспользоватьИнтеграциюДО3",
			Новый ОписаниеТипов("Булево"),,
			НСтр("ru = 'Использовать интеграцию с 1С:Документооборотом редакции 3'; en = 'Use integration with 1C:Document Management (revision 3)'")));
	КонецЕсли;
	Если РеквизитФормыПоИмени(РеквизитыФормы, "ИспользоватьПрисоединенныеФайлы1СДокументооборота") = Неопределено Тогда
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			"ИспользоватьПрисоединенныеФайлы1СДокументооборота",
			Новый ОписаниеТипов("Булево"),,
			НСтр("ru = 'Использовать присоединенные файлы 1С:Документооборота'; en = 'Use attached files of 1C:Document Management'")));
	КонецЕсли;
	Если РеквизитФормыПоИмени(РеквизитыФормы, "ИмяРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом") = Неопределено Тогда
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			"ИмяРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом",
			Новый ОписаниеТипов("Строка"),,
			НСтр("ru = 'Имя реквизита, содержащего ссылку на интегрируемый с 1С:Документооборотом объект'; en = 'Name of the attribute which contains a link to the object to integrate with 1C:Document Management'")));
	КонецЕсли;
	Если Форма.Параметры.Свойство("ИнтеграцияС1СДокументооборотом_ТипОбъектаДО")
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			...
			Новый ОписаниеТипов("Строка"),,
			НСтр("ru = 'Тип объекта 1С:Документооборота'; en = '1C:Document Management object type'")));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
			"ИнтеграцияС1СДокументооборотом_ИдентификаторОбъектаДО",
			Новый ОписаниеТипов("Строка"),,
			НСтр("ru = 'Идентификатор объекта 1С:Документооборота'; en = '1C:Document Management object ID'")));
	КонецЕсли;
			...
			Если ТипЗнч(ЭлементКомПанели) = Тип("ГруппаФормы") Тогда
				Для Каждого ЭлементГруппы Из ЭлементКомПанели.ПодчиненныеЭлементы Цикл
					Если ТипЗнч(ЭлементГруппы) = Тип("ГруппаФормы")
							И (ЭлементГруппы.Заголовок = НСтр("ru = 'Создать на основании'; en = 'Create on the basis of'")
								Или ЭлементГруппы.Заголовок = НСтр("ru = 'Органайзер'; en = 'Organizer'")
								Или ЭлементГруппы.Заголовок = НСтр("ru = 'Печать'; en = 'Print'")) Тогда
	...
	ТипИнтегрируемогоОбъекта = Неопределено;
	Если Форма.Параметры.Свойство("Основание")
			И ТипЗнч(Форма.Параметры.Основание) = Тип("Структура")
			И Форма.Параметры.Основание.Свойство("ТипСоздаваемогоОбъекта") Тогда
		ИмяТипаСоздаваемогоОбъекта = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(
			Форма.Параметры.Основание.ТипСоздаваемогоОбъекта);
		ТипИнтегрируемогоОбъекта = Тип(ИмяТипаСоздаваемогоОбъекта);
	КонецЕсли;
	
	Форма.ИмяРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом =
	...
	Если Ссылка = Неопределено Тогда
		Ссылка = СсылкаНаИнтегрируемыйОбъектИзФормы(Форма, РеквизитыФормы, ТипИнтегрируемогоОбъекта);
	КонецЕсли;
...
// Проверяет произвольный алгоритм на встроенном языке 1С:Предприятия, который задается пользователем в режиме
// предприятие, на наличие в нем опасных конструкций, позволяющих получить неограниченный доступ к базе.
//
// Параметры:
//   АлгоритмДляПроверки - Строка - алгоритм на встроенном языке "1С:Предприятия", который требуется проверить.
//
Процедура ПроверитьАлгоритмНаЗапрещенныеВыражения(Знач АлгоритмДляПроверки) Экспорт
	
	АлгоритмДляПроверки = НРег(АлгоритмДляПроверки);
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.ВК, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.ВТаб, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.НПП, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.ПС, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.ПФ, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, Символы.Таб, "");
	АлгоритмДляПроверки = СтрЗаменить(АлгоритмДляПроверки, " ", "");
	...
	// Исключением является метод Выполнить() объекта Запрос.
	...
	ЗапрещенныеСтроки.Добавить("БезопасныйРежим"); //@NON-NLS-1
	ЗапрещенныеСтроки.Добавить("SafeMode"); //@NON-NLS-1
	
	ЗапрещенныеСтроки.Добавить("Транзакция"); //@NON-NLS-1
	ЗапрещенныеСтроки.Добавить("Транзакцию"); //@NON-NLS-1
	ЗапрещенныеСтроки.Добавить("Transaction"); //@NON-NLS-1
...
"%1'; en = 'Cannot use the following constructions in the expression:"
"%1'"),
...
""
"Список исключений:"
"%2'; en = 'Cannot use the following constructions in the expression:"
"%1"
""
"List of exceptions:"
"%2'"),
...
Функция ПроверитьТип(Прокси, ОбъектXDTO, Тип) Экспорт
	...
	Если ФабрикаDM = Неопределено Тогда
		...
		Возврат (НРег(СокрЛП(ТипОбъектаXDTO.Имя)) = НРег(СокрЛП(Тип)));
		
	Иначе
		Возврат ТипыXDTOСовпадают(ТипОбъектаXDTO, ФабрикаDM.Тип(URIПространстваИменВебСервисаДокументооборота(), Тип));
...
//
Функция РеквизитФормыПоИмени(РеквизитыФормы, ИмяРеквизита) Экспорт
...
Функция РеквизитыОбъектаИС(ТипОбъектаИС, ДляВыражения = Ложь) Экспорт
	...
	Для Каждого МетаданныеРеквизита Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		...
		Если МетаданныеРеквизита.Имя = "Код"
				...
				Или МетаданныеРеквизита.Имя = "Дата"
				Или МетаданныеРеквизита.Имя = "ПометкаУдаления" Тогда
			...
			Если МетаданныеРеквизита.Имя = "ПометкаУдаления" Тогда
				Реквизит.Представление = НСтр("ru = 'Пометка удаления'; en = 'Deletion mark'");
			Иначе
	...
	Для Каждого МетаданныеРеквизита Из МетаданныеОбъекта.Реквизиты Цикл
		...
		Если СтрНачинаетсяС(МетаданныеРеквизита.Имя, "Удалить")
				Или СтрНайти(МетаданныеРеквизита.Синоним, НСтр("ru = '(Удалить)'; en = '(Delete)'")) > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого МетаданныеТаблицы Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Если Не ДляВыражения
				И (МетаданныеТаблицы.Имя = "ДополнительныеРеквизиты"
					Или МетаданныеТаблицы.Имя = "КонтактнаяИнформация") Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрНачинаетсяС(МетаданныеТаблицы.Имя, "Удалить")
				Или СтрНайти(МетаданныеТаблицы.Синоним, НСтр("ru = '(Удалить)'; en = '(Delete)'")) > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Метаданные.ФункциональныеОпции.Найти("ИспользоватьДополнительныеРеквизитыИСведения") <> Неопределено
		...
		ИмяТипа = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(ТипОбъектаИС);
		ВладелецСвойств = Новый(Тип(ИмяТипа));
		...
		Если УправлениеСвойствами.ИспользоватьДопРеквизиты(ВладелецСвойств) Тогда
			...
			Для Каждого ДопРеквизит Из НаборДопРеквизитов Цикл
				...
				Реквизит.Представление = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1'; en = 'Additional attribute: %1'"),
					СвойстваДопРеквизита.Наименование);
	...
	Если Не ДляВыражения Тогда
		...
		Для ИндексСтроки = 0 По Реквизиты.Количество() - 1 Цикл
			Реквизиты[ИндексСтроки].Порядок = ИндексСтроки;
		КонецЦикла;
		Повторять = Истина;
		Пока Повторять Цикл
			Повторять = Ложь;
...
Функция СвойствоСуществует(ОбъектXDTO, ИмяСвойства, ВызыватьИсключение = Ложь) Экспорт
	...
	Если ВызыватьИсключение И Не СвойствоСуществует Тогда
		ТекстИсключения = СтрШаблон(НСтр("ru = 'В объекте XDTO типа ""%1"" не существует свойства с именем ""%2"".'; en = 'In the XDTO object of the %1 type, a property with the %2 name does not exist.'"),
			ОбъектXDTO.Тип().Имя,
			ИмяСвойства);
				...
				Если ПроверитьТип(Прокси, Результат, "DMError") = Ложь И Результат.documents.Количество() > 0 Тогда
					...
					Если СвойствоУстановлено(ОбъектДО, "statusApproval") Тогда
						...
						ИначеЕсли ТипЗнч(ОбъектДО.statusApproval) = Тип("ОбъектXDTO")
								...
								И ОбъектДО.statusApproval.name = "Согласован" Тогда //@NON-NLS-3
							Согласован = Истина;
...
"Выполните команду ""Синхронизировать связи"" в настройках интеграции с 1С: Документооборотом.'; en = 'An incorrect link between the objects of 1C:Document Management and %1 is found."
...
Функция СоздатьExternalObject(Прокси, ВнешнийОбъектИС = Неопределено, ID = "", Тип = "", Наименование = "") Экспорт
	...
	Если ВнешнийОбъектИС <> Неопределено Тогда
		...
		ExternalObject.type = ПолучитьИмяТипаСсылки(ВнешнийОбъектИС);
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВнешнийОбъектИС)) И Не ВнешнийОбъектИС.Пустая() Тогда
			...
			ExternalObject.ID = ПолучитьIDПоСсылке(ВнешнийОбъектИС);
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьОбъект(Прокси, ТипОбъекта, ВнешнийОбъектИС = Неопределено, ЗаполнитьИзВнешнегоОбъекта = Истина) Экспорт
	...
	Если ТипОбъектаXDTO = Неопределено Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Тип объекта ""%1"" не поддерживается веб-сервисом 1С:Документооборот версии %2'; en = 'The ""%1"" object type is not supported by 1C:Document Management web service of version %2'"),
			ТипОбъекта,
			ВерсияСервиса());
	КонецЕсли;
...
Функция СокращенноеНаименованиеКонфигурации() Экспорт
	...
	СокращенноеНаименование = НСтр("ru = 'ИС'; en = 'IS'");
...
//
Функция СсылкаНаИнтегрируемыйОбъектИзФормы(Форма, РеквизитыФормы = Неопределено,
		ТипИнтегрируемогоОбъекта = Неопределено) Экспорт
...
//                                   - Неопределено - признак необходимости использовать ранее сохраненный адрес.
//
Процедура УстановитьВерсиюСервиса(ИзФоновогоЗадания = Ложь, ТекстОшибки = "",
		АдресВебСервисаДокументооборота = Неопределено) Экспорт
...
Процедура УстановитьПараметрыСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ИмяПараметра = "ИнтеграцияС1СДокументооборотУзел" Тогда
		...
		КонецЕсли;
		Если УстановленныеПараметры <> Неопределено Тогда
			УстановленныеПараметры.Добавить("ИнтеграцияС1СДокументооборотУзел");
		КонецЕсли;
	КонецЕсли;
...
"1С:Документооборота недоступен. Обратитесь к администратору.'; en = 'Integration with 1C:Document Management is not configured, or"
...
"Обратитесь к администратору.'; en = 'Cannot receive 1C:Document Management data: %1."
"Contact the administrator.'"),
						Объект.name);
				КонецЕсли;
...
"%2'; en = 'Cannot generate the %1 print form due to:"
"%2'"),
...
Функция ИмяПечатнойФормы(Источник, ПредставлениеКоманды, СинонимМакета, ИмяМакета) Экспорт
	...
	Если ОбщегоНазначения.ЭтоДокумент(ИсточникМетаданные) И ИсточникМетаданные.ДлинаНомера > 0 Тогда
		...
		ИмяФайла = СтрШаблон(НСтр("ru = '%1 №%2 от %3'; en = '%1 No%2 dated %3'"),
			ПредставлениеПечатнойФормы,
			Строка(ДатаНомер.Номер),
			Формат(ДатаНомер.Дата, "ДЛФ=D"));
	Иначе
		ИмяФайла = СтрШаблон(НСтр("ru = '%1 %2'; en = '%1 %2'"),
			ПредставлениеПечатнойФормы,
			Строка(Источник));
	КонецЕсли;
...
//     * СостояниеРазрешаетДобавлениеФайла - Булево - указывает доступно ли пользователю добавление файлов в объект ДО,
//         в соответствии с настройками доступности по состоянию.
//     * СостояниеРазрешаетДобавлениеСканКопии - Булево - указывает доступно ли пользователю добавление скан-копий в
//         объект ДО, в соответствии с настройками доступности по состоянию.
//     * ПрисоединенныеФайлыВИС - СписокЗначений из ЛюбаяСсылка - список файлов в ИС, не перенесенных в ДО.
...
Функция ПараметрыПереносаФайловВДО(ДанныеСвязи, ПрисоединенныеФайлыВИС = Неопределено,
	...
	ПараметрыПереносаФайлов.Вставить("СостояниеРазрешаетДобавлениеФайла", Истина);
	ПараметрыПереносаФайлов.Вставить("СостояниеРазрешаетДобавлениеСканКопии", Истина);
	...
	Если ПараметрыПереносаФайлов.ПрисоединенныеФайлыВИС = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ДанныеСвязи.ID) Тогда
			...
			ОбъектXDTO = ПолучитьОбъект(
				...
				ДанныеСвязи.Тип,
				ДанныеСвязи.ID,
				"files,enabledProperties");
			ЗаполнитьНДПСизОбъектаXDTO(ПараметрыПереносаФайлов, ОбъектXDTO);
			Если СвойствоУстановлено(ОбъектXDTO, "files") Тогда
...
Функция РасширениеСохраняемойПечатнойФормы(ТипФайла) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ТипФайла) И ОбщегоНазначения.СсылкаСуществует(ТипФайла) Тогда
...
// Конструктор структуры, описывающей дополнительные данные пользователя ДО, связанного с пользователем ИС.
//
// Возвращаемое значение:
//   Структура:
//     * ПользовательИБ - Строка - имя пользователя ИБ.
//     * ПользовательОС - Строка - имя пользователя ОС.
//     * АдресЭлектроннойПочты - Строка - адрес электронной почты.
//     * Узел - Строка - идентификатор узла ДО, из которого была создана данная связь.
//     * ЭтоПодчиненныйУзел - Булево - признак того что связь относится к подчиненному узлу ДО.
//
Функция ДополнительныеДанныеСвязиПользователейДОиИС() Экспорт
	
	ДополнительныеДанные = Новый Структура;
	ДополнительныеДанные.Вставить("ПользовательИБ", "");
	ДополнительныеДанные.Вставить("ПользовательОС", "");
	ДополнительныеДанные.Вставить("АдресЭлектроннойПочты", "");
	ДополнительныеДанные.Вставить("Узел", "");
	ДополнительныеДанные.Вставить("ЭтоПодчиненныйУзел", Ложь);
	
	Возврат ДополнительныеДанные;
	
КонецФункции

// Получает идентификатор узла ДО, куда гарантированно может войти пользователь ИС.
// Данные берутся из регистра ОбъектыИнтегрированныеС1СДокументооборотом, из связи пользователя ДО с пользователем ИС.
//
// Параметры:
//   ПользовательИС - ОпределяемыйТип.Пользователь - пользователь интегрированной системы.
//
// Возвращаемое значение:
//   Строка - идентификатор узла ДО.
//
Функция УзелДляАвторизацииВДО(ПользовательИС) Экспорт
	
	УзелДляАвторизацииВДО = "";
	ПользовательДО = РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДанныеОбъектаДОПоВнешнемуОбъекту(
		ПользовательИС);
	Если ЗначениеЗаполнено(ПользовательДО) И ТипЗнч(ПользовательДО.ДополнительныеДанные) = Тип("Структура") Тогда
		ДополнительныеДанныеСвязиПользователейДОиИС = ПользовательДО.ДополнительныеДанные; // см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДополнительныеДанныеСвязиПользователейДОиИС
		Если ДополнительныеДанныеСвязиПользователейДОиИС.Свойство("Узел") Тогда
			УзелДляАвторизацииВДО = ДополнительныеДанныеСвязиПользователейДОиИС.Узел;
		КонецЕсли;
	КонецЕсли;
	
	Возврат УзелДляАвторизацииВДО;
	
КонецФункции

// Получает имя пользователя ИБ, заданное у пользователя ДО, связанного с пользователем ИС.
// Данные берутся из регистра ОбъектыИнтегрированныеС1СДокументооборотом, из связи пользователя ДО с пользователем ИС.
//
// Параметры:
//   ПользовательИС - ОпределяемыйТип.Пользователь - пользователь интегрированной системы.
...
//
Функция ИмяПользователяИБДляАвторизацииВДО(ПользовательИС) Экспорт
	
	ИмяПользователяИБДляАвторизацииВДО = "";
	ПользовательДО = РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДанныеОбъектаДОПоВнешнемуОбъекту(
		ПользовательИС);
	Если ЗначениеЗаполнено(ПользовательДО) И ТипЗнч(ПользовательДО.ДополнительныеДанные) = Тип("Структура") Тогда
		ДополнительныеДанныеСвязиПользователейДОиИС = ПользовательДО.ДополнительныеДанные; // см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДополнительныеДанныеСвязиПользователейДОиИС
		Если ДополнительныеДанныеСвязиПользователейДОиИС.Свойство("ПользовательИБ") Тогда
			ИмяПользователяИБДляАвторизацииВДО = ДополнительныеДанныеСвязиПользователейДОиИС.ПользовательИБ;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ИмяПользователяИБДляАвторизацииВДО;
...
Процедура УдалитьИнтеграцияС1СДокументооборотВыполнитьОбменДанными() Экспорт
	...
	ВызватьИсключение НСтр("ru = 'Регламентное задание не используется'; en = 'The scheduled task is not used'");
...
Процедура ОтправитьДанные(Прокси, Настройки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИмяСобытияЖурналаРегистрации(
				НСтр("ru = 'Отправка данных'; en = 'Export'", ОбщегоНазначения.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка,
...
Функция ПроверитьРезультатОтправки(Прокси, Запрос, Результат, Настройки)
	...
	Для Каждого ОбъектXDTO Из Результат.objects Цикл
		...
		МассивОшибокОбъекта.Добавить(
			СтрШаблон(НСтр("ru = 'Ошибка на стороне 1С:Документооборот при получении сообщения обмена из %1:'; en = 'An error occurred on 1C:Document Management side when receiving the exchange message from %1:'"),
				ИмяКонфигурации));
		...
		Если ОбъектИС <> Неопределено Тогда
			МассивОшибокОбъекта.Добавить(
				СтрШаблон(НСтр("ru = 'Ссылка на объект %1, изменения которого отправляются в 1С:Документооборот: %2'; en = 'The link to the %1 object whose changes are sent to 1C:Document Management: %2'"),
					ИмяКонфигурации,
...
Процедура ПриОпределенииЗначенияРеквизитаОбъектаИСПоЗначениюДляЗаполнения(Прокси, Результат, ТипРеквизитаОбъектаИС,
	...
	Если Результат = Неопределено И ЭтоОбъектноеЗначение(Прокси, ЗначениеДляЗаполнения.ЗначениеСвойства) Тогда
		...
		Если ТипРеквизитаОбъектаИС = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипПользователи()
			...
			Если Не ПустаяСтрока(ЗначениеДляЗаполнения.ИдентификаторОбъектаДО) Тогда
				Результат =
					РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.СсылкаНаОбъектИСПоДаннымДокументооборота(
						ЗначениеДляЗаполнения.ИдентификаторОбъектаДО,
						ЗначениеДляЗаполнения.ТипОбъектаДО);
			...
			Если Не ЗначениеЗаполнено(Результат) И СвойствоУстановлено(ОбъектXDTO, "infoBaseUser") Тогда
				УстановитьПривилегированныйРежим(Истина);
				
				Если СвойствоУстановлено(ОбъектXDTO.infoBaseUser, "objectID")
						И СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОбъектXDTO.infoBaseUser.objectID.ID) Тогда
					Результат = Пользователи.НайтиПоИдентификатору(
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					...
					Если ПодходящийПользовательИБ <> Неопределено Тогда
						Результат = Пользователи.НайтиПоИдентификатору(ПодходящийПользовательИБ.УникальныйИдентификатор);
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли ТипРеквизитаОбъектаИС = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипСтраныМира()
				И ПроверитьТип(Прокси, ОбъектXDTO, "DMCountry") Тогда
...
Процедура ПриСозданииПоСсылке(ОбъектИС, ОбъектXDTO, ЗаполняемыйОбъектИС = Неопределено)
	...
	ИначеЕсли ТипЗнч(ОбъектИС) = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ТипСтраныМира(Истина)
			И ОбъектXDTO.objectID.type = "DMCountry" Тогда
		
		Если СвойствоУстановлено(ОбъектXDTO, "code") Тогда
			ОбъектИС.Код = ОбъектXDTO.code;
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "codeAlpha2") Тогда
			ОбъектИС.КодАльфа2 = ОбъектXDTO.codeAlpha2;
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "codeAlpha3") Тогда
			ОбъектИС.КодАльфа3 = ОбъектXDTO.codeAlpha3;
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "EAEUMember") Тогда
			ОбъектИС.УчастникЕАЭС = ОбъектXDTO.EAEUMember;
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "longDescription") Тогда
			ОбъектИС.НаименованиеПолное = ОбъектXDTO.longDescription;
		КонецЕсли;
		Если СвойствоУстановлено(ОбъектXDTO, "internationalDescription") Тогда
			ОбъектИС.МеждународноеНаименование = ОбъектXDTO.internationalDescription;
		КонецЕсли;
...
Процедура СформироватьСообщение(Прокси, ПорцияОбъектов, Настройки, МодульИнтеграцияС1СДокументооборотОбмен,
	...
	Для Каждого ИнтегрированныйОбъект Из ПорцияОбъектов Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(
					НСтр("ru = 'Формирование сообщения обмена'; en = 'Generate exchange message'", ОбщегоНазначения.КодОсновногоЯзыка())),
				УровеньЖурналаРегистрации.Ошибка,
...
Функция ВерсияНеМеньше(ОписаниеВерсииСервиса, ТребуемаяВерсия)
	...
	ПриоритетРедакции = Новый Соответствие;
	ПриоритетРедакции.Вставить("PROF", 0);
	ПриоритетРедакции.Вставить("CORP", 1);
	ПриоритетРедакции.Вставить("HOLD", 2);
	ПриоритетРедакции.Вставить("HOLD_CENTER", 2);
	
	ПриоритетРедакцииВерсииСервиса = ПриоритетРедакции[ОписаниеВерсииСервиса[4]];
	ПриоритетРедакцииТребуемойВерсии = ПриоритетРедакции[ОписаниеТребуемойВерсии[4]];
	
	Если ПриоритетРедакцииВерсииСервиса < ПриоритетРедакцииТребуемойВерсии Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ДобавитьКомандуФайлы(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.9.64") < 0
			...
			КомандаФормы.Заголовок = НСтр("ru = 'Файлы ДО'; en = 'DI files'");
		Иначе
			КомандаФормы.Заголовок = НСтр("ru = 'Файлы'; en = 'Files'");
		КонецЕсли;
		КомандаФормы.Подсказка = НСтр("ru = 'Файлы, хранящиеся в 1С:Документооборот'; en = 'Files stored in 1C:Document Management'");
...
Процедура ДополнитьТекстСообщенияОбОшибке(ТекстСообщенияОбОшибке, Знач Дополнение)
	...
	Дополнение = СтрЗаменить(Дополнение, "\r\n", Символы.ПС);
	Дополнение = СтрЗаменить(Дополнение, "\n", Символы.ПС);
	Дополнение = СтрЗаменить(Дополнение, "&nbsp", Символы.НПП);
	Дополнение = СокрЛП(Дополнение);
	...
	Иначе
		ТекстСообщенияОбОшибке = ТекстСообщенияОбОшибке + РазделительТекстаОшибки + Дополнение;
	КонецЕсли;
...
Процедура ЗаполнитьXDTOПодпись(Прокси, ПодписьXDTO, ДанныеПодписи)
	...
	Если ДанныеПодписи.Свойство("УстановившийПодпись") Тогда
		...
		ПодписьXDTO.signer.objectID = СоздатьObjectID(Прокси, ДанныеПодписи.УстановившийПодписьID, "DMUser");
	КонецЕсли;
...
Функция НужноВыводитьКомандуПрисоединенныхФайловДО(Форма)
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.9.64") >= 0
			...
			МодульРаботаСФайлами = ОбщегоНазначения.ОбщийМодуль("РаботаСФайлами");
			ИмяТипа = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(Форма.ИмяФормы);
...
Процедура ОбработатьАдресВебСервиса1СДокументооборот(АдресВебСервиса)
...
Процедура ПараметрыДлительнойОперацииВПараметрыСеанса(ПараметрыДлительнойОперации)
	...
	ПараметрыСеанса.ИнтеграцияС1СДокументооборотУзел = ПараметрыДлительнойОперации.ИнтеграцияС1СДокументооборотУзел;
...
Функция ПолучитьИОбновитьКэшWSОпределения(ДанныеСервиса, МестоположениеWSDL, ИмяПользователя, Пароль,
	...
	Попытка
		...
		Если РезультатСкачивания.Статус Тогда
			...
			Определения = Новый WSОпределения(ИмяФайлаWSDL,,,, ДанныеСервиса.Таймаут, ЗащищенноеСоединение);
...
"Сообщение об ошибке:"
"%3'; en = 'Cannot get a WSDL file with the description of 1C:Document Management web service."
"WSDL file address: %1."
...
Функция ПолучитьВерсиюВызовомСервиса(ИзФоновогоЗадания = Ложь, ТекстОшибки = "",
	...
	Если Прокси = Неопределено Тогда
		...
		ЗаписатьПредупреждение(НСтр("ru = 'Сервис недоступен'; en = 'Service is not available'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат "0.0.0.0";
	...
	Иначе
		...
		Попытка
			...
			Иначе
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Подключен сервис версии %1'; en = 'Service of version %1 enabled'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Ответ.versionNumber);
				ЗаписьЖурналаРегистрации(
					ИмяСобытияЖурналаРегистрации(),
...
Функция ПолучитьПроксиИзПараметровСеанса(ВызыватьИсключение = Истина)
	...
	Попытка
		...
		ЗащищенноеСоединение = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение();
		Определения = Новый WSОпределения(ИмяФайлаWSDL,,,, ТаймаутСервиса(), ЗащищенноеСоединение);
...
Процедура ПроверитьОднуПодпись(ДанныеСтроки, МенеджерКриптографии, СоответствиеИдОбъектаИДвоичныхДанных,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Исключение
			Неверна = Истина;
			ОписаниеОшибки = НСтр("ru = 'не указан менеджер криптографии'; en = 'crypto manager is not specified'");
		КонецПопытки;
	Иначе
		Неверна = Истина;
		ОписаниеОшибки = НСтр("ru = 'отсутствует стандартная подсистема ""Электронная подпись""'; en = 'the standard ""Digital signature"" subsystem is missing'");
	КонецЕсли;
	
	ДанныеСтроки.ДатаПроверкиПодписи = ТекущаяДатаСеанса();
...
"Обратитесь к администратору.'; en = 'Insufficient rights to access this object (%1)."
