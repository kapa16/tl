...
Функция ЗаписатьОбъектАсинхронно(УникальныйИдентификаторФормы, ОбъектСтрока) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Запись объекта в 1С:Документообороте'; en = 'Save the object in 1C:Document Management'");
...
Функция ПараметрыДлительнойОперации(ИспользоватьКэш = Истина, АдресВебСервисаДокументооборота = Неопределено) Экспорт
	...
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПараметрыДлительнойОперации(
		ИспользоватьКэш,
...
Функция ПодключитьсяИПолучитьВерсиюСервисаАсинхронно(ИмяПользователя, Пароль,
		УникальныйИдентификаторФормы = Неопределено, ИспользоватьКэш = Истина,
...
Функция ПолучитьОбъектАсинхронно(УникальныйИдентификаторФормы, Тип, ID, Колонки = Неопределено,
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификаторФормы);
	ПараметрыВыполнения.ОжидатьЗавершение = 0; // Запускать сразу.
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение данных из 1С:Документооборота'; en = 'Get data from 1C:Document Management'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИмяСобытияЖурналаРегистрации(
				НСтр("ru = 'Не удалось добавить связь'; en = 'Cannot add link'", ОбщегоНазначения.КодОсновногоЯзыка())),
...
// Находит в форме клиентского приложения реквизит, содержащий ссылку на объект, интегрируемый с 1С:Документооборотом.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма, в которой выполняется поиск реквизита.
//         - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ФормаИнтегрируемогоОбъекта
...
//                            - Неопределено - если тип интегрируемого объекта заранее не известен.
//
// Возвращаемое значение:
//   Строка - имя реквизита, содержащего ссылку на интегрируемый с 1С:Документооборотом объект.
//
Функция ИмяРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом(Знач Форма, РеквизитыФормы = Неопределено,
		ТипИнтегрируемогоОбъекта = Неопределено) Экспорт
	
	Если РеквизитыФормы = Неопределено И ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		РеквизитыФормы = Форма.ПолучитьРеквизиты();
	КонецЕсли;
	
	ИмяРеквизита = "";
	
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервераПереопределяемый.ПриОпределенииИмениРеквизитаОбъектаИнтегрируемогоС1СДокументооборотом(
		Форма,
		РеквизитыФормы,
	...
	ТипОсновногоОбъектаПодключенКИнтеграции = Ложь;
	ИмяТипаОсновногоОбъекта = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(
		Форма.ИмяФормы,
		Истина,
		Ложь);
	Если ИмяТипаОсновногоОбъекта <> "" Тогда
		ТипОсновногоОбъекта = Тип(ИмяТипаОсновногоОбъекта);
		ТипОсновногоОбъектаПодключенКИнтеграции = ТипВсеСсылки.СодержитТип(ТипОсновногоОбъекта);
	КонецЕсли;
	
	РеквизитыХранящиеОбъект = Новый ТаблицаЗначений;
	РеквизитыХранящиеОбъект.Колонки.Добавить("ИмяРеквизита", Новый ОписаниеТипов("Строка"));
	РеквизитыХранящиеОбъект.Колонки.Добавить("Приоритет", Новый ОписаниеТипов("Число"));
	
	Если (ТипЗнч(Форма.Параметры) = Тип("ДанныеФормыСтруктура") Или ТипЗнч(Форма.Параметры) = Тип("Структура"))
			И Форма.Параметры.Свойство("Ключ") Тогда
		ТипКлюча = ТипЗнч(Форма.Параметры.Ключ);
		Если ОбщегоНазначения.ЭтоСсылка(ТипКлюча) Тогда
			
			ТипКлючаПодключенКИнтеграции = ТипВсеСсылки.СодержитТип(ТипКлюча);
			
			НоваяСтрока = РеквизитыХранящиеОбъект.Добавить();
			НоваяСтрока.ИмяРеквизита = "Параметры.Ключ";
			
			Если ТипИнтегрируемогоОбъекта <> Неопределено И ТипКлюча = ТипИнтегрируемогоОбъекта Тогда
				НоваяСтрока.Приоритет = 0;
				
			ИначеЕсли ТипОсновногоОбъекта <> Неопределено
					И ТипОсновногоОбъектаПодключенКИнтеграции
					И ТипКлюча = ТипОсновногоОбъекта Тогда
				НоваяСтрока.Приоритет = 3;
				
			ИначеЕсли Не ТипОсновногоОбъектаПодключенКИнтеграции И ТипКлючаПодключенКИнтеграции Тогда
				НоваяСтрока.Приоритет = 6;
				
			ИначеЕсли ТипОсновногоОбъекта <> Неопределено И ТипКлюча = ТипОсновногоОбъекта Тогда
	...
	Если ТипЗнч(РеквизитыФормы) = Тип("Массив") Тогда
		Для Каждого Реквизит Из РеквизитыФормы Цикл
			Если (ТипЗнч(Форма[Реквизит.Имя]) = Тип("ДанныеФормыСтруктура") Или ТипЗнч(Форма[Реквизит.Имя]) = Тип("Структура"))
					И Форма[Реквизит.Имя].Свойство("Ссылка") = Истина Тогда
				
				ОбъектИС = Форма[Реквизит.Имя]; // ДокументОбъект
				НоваяСтрока = РеквизитыХранящиеОбъект.Добавить();
				НоваяСтрока.ИмяРеквизита = СтрШаблон("%1.Ссылка", Реквизит.Имя);
				
				ТипСсылки = ТипЗнч(ОбъектИС.Ссылка);
				ТипСсылкиПодключенКИнтеграции = ТипВсеСсылки.СодержитТип(ТипСсылки);
				
				Если ТипИнтегрируемогоОбъекта <> Неопределено И ТипСсылки = ТипИнтегрируемогоОбъекта Тогда
				...
				ИначеЕсли ТипОсновногоОбъекта <> Неопределено
						И ТипОсновногоОбъектаПодключенКИнтеграции
						И ТипСсылки = ТипОсновногоОбъекта Тогда
					Если Реквизит.Имя = "Объект" Тогда
				...
				ИначеЕсли Не ТипОсновногоОбъектаПодключенКИнтеграции И ТипСсылкиПодключенКИнтеграции Тогда
					Если Реквизит.Имя = "Объект" Тогда
				...
				ИначеЕсли ТипОсновногоОбъекта <> Неопределено И ТипСсылки = ТипОсновногоОбъекта Тогда
					Если Реквизит.Имя = "Объект" Тогда
	...
	РеквизитыХранящиеОбъект.Сортировать("Приоритет");
	
	Если РеквизитыХранящиеОбъект.Количество() > 0 Тогда
		ПриоритетныеСтроки = РеквизитыХранящиеОбъект.НайтиСтроки(
			Новый Структура("Приоритет",
			РеквизитыХранящиеОбъект[0].Приоритет));
		Если ПриоритетныеСтроки.Количество() = 1 Тогда
			ИмяРеквизита = ПриоритетныеСтроки[0].ИмяРеквизита;
		КонецЕсли;
	КонецЕсли;
...
//
// Возвращаемое значение:
//   см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации
//
Функция СокращенноеНаименованиеКонфигурации() Экспорт
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации();
...
Процедура УдалитьСвязь(ID, Тип, ИнтегрированныйОбъект) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИмяСобытияЖурналаРегистрации(
				НСтр("ru = 'Не удалось удалить связь'; en = 'Cannot remove link'", ОбщегоНазначения.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДанныеДляАвтоПодбора(ТипыЗначений, ДанныеВыбора, Текст, СтандартнаяОбработка, Отбор = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Невозможно подобрать данные для типа: %1'; en = 'Cannot select data for the type: %1'"), Строка(ТипыЗначений));
	...
	Для Каждого ТипXDTO Из МассивТипов Цикл
		...
		УстановленОтборДействующихСотрудников = Ложь;
		...
		Если Отбор <> Неопределено Тогда
			Для Каждого СтрокаОтбора Из Отбор Цикл
				...
				Иначе
					...
					КонецЕсли;
					УсловияОтбора.Добавить(Условие);
					Если НРег(Условие.property) = "isactual" Тогда
						УстановленОтборДействующихСотрудников = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Если ТипXDTO = "DMEmployee" И Не УстановленОтборДействующихСотрудников Тогда
			Условие = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси,"DMObjectListCondition");
			Условие.property = "isActual";
...
Функция СоздатьОбъектДОПоПравилу(ОбъектИС, Правило) Экспорт
	...
	ИначеЕсли Настройки.ИспользоватьИнтеграциюДО3 Тогда
		...
		ОбъектДОИлиСообщение = МодульИнтеграцияС1СДокументооборот3.СоздатьОбъектДОПоПравилу(
			...
			Правило);
	Иначе
		Возврат НСтр("ru = 'Интеграция с 1С:Документооборот не используется'; en = 'Integration with 1C:Document Management is not used'");
	КонецЕсли;
...
//
Функция ДвоичныеДанныеФайла(ID, ИспользоватьВизуализациюШтампаЭП = Истина) Экспорт
	
	Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси();
	
	Колонки = Новый Массив;
	Колонки.Добавить("objectID");
	Колонки.Добавить("name");
	Колонки.Добавить("binaryData");
	Колонки.Добавить("extension");
	Если Не ИспользоватьВизуализациюШтампаЭП Тогда
		Колонки.Добавить("withoutStampVisualization");
	КонецЕсли;
	
	ДанныеФайла = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъект(Прокси, "DMFile", ID, Колонки);
...
//
Функция ДвоичныеДанныеФайлов(Файлы, ИспользоватьВизуализациюШтампаЭП = Истина) Экспорт
	...
	Колонки.Добавить("name");
	Колонки.Добавить("binaryData");
	Колонки.Добавить("encrypted");
	Если Не ИспользоватьВизуализациюШтампаЭП Тогда
		Колонки.Добавить("withoutStampVisualization");
	КонецЕсли;
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъекты(Прокси, СписокОбъектов, Колонки);
...
Процедура ЗаполнитьПоляФайлаДаннымиВладельца(ID) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("2.0.15.1") Тогда
		ТекстСообщения = НСтр("ru = 'Для заполнения файлов нужен 1С:Документооборот версии не ниже 2.0.15.1'; en = 'To fill in files, 1C:Document Management version should be not earlier than 2.0.15.1'");
...
Процедура ЗаполнитьФайлыКопированием(ВнешнийОбъект, ID, Тип, Представление, УникальныйИдентификаторФормы) Экспорт
	...
	Для Каждого ДанныеФайла Из МассивФайлов Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Не удалось определить адрес временного хранилища присоединенного файла'; en = 'Cannot determine a temporary storage address of the attachment '");
		КонецЕсли;
...
Процедура ОбновитьДеревоФайлов(ДеревоФайлов, ДоступныеРоли, ФайлыXDTO, ИспользоватьРолиФайлов = Ложь,
	...
	Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
		...
		Если ФайлыКоличество = 0 Тогда
			ФайлыЗаголовок = НСтр("ru = 'Файлы'; en = 'Files'");
		Иначе
...
Функция ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(Знач Параметры) Экспорт
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Параметры.ТекущийФайл.ДатаМодификации)
			Или Не ЗначениеЗаполнено(Параметры.ТекущийФайл.ДатаМодификацииУниверсальная) Тогда
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла(Параметры.ТекущийФайл.Расширение);
		ДвоичныеДанные.Записать(ИмяВременногоФайла);
		ВременныйФайл = Новый Файл(ИмяВременногоФайла);
		Параметры.ТекущийФайл.ДатаМодификации = ВременныйФайл.ПолучитьВремяИзменения();
		Параметры.ТекущийФайл.ДатаМодификацииУниверсальная = ВременныйФайл.ПолучитьУниверсальноеВремяИзменения();
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
		...
		Если Параметры.ОбновитьСведенияОРедактировании Тогда
			...
			Иначе
				...
				СведенияОРедактировании.Вставить("ДатаСохранения", Параметры.ТекущийФайл.ДатаМодификации);
				СведенияОРедактировании.Вставить("ПолныйПуть", Параметры.ТекущийФайл.ИмяФайла);
			КонецЕсли;
...
//
Функция ПолучитьВерсиюФайлаИПоместитьВХранилище(ID, ИспользоватьВизуализациюШтампаЭП, УникальныйИдентификаторФормы,
	...
	ПолучаемыеПоля.Добавить("modificationDateUniversal");
	Если Не ИспользоватьВизуализациюШтампаЭП Тогда
		ПолучаемыеПоля.Добавить("withoutStampVisualization");
	КонецЕсли;
...
// Получает ДокументPDF, заполняет его данными из файла на сервере.
//
// Параметры:
//   ПолноеИмяФайлаПредпросмотра - Строка - полное имя файла предпросмотра на сервере.
//
// Возвращаемое значение:
//   ДокументPDF
//
Функция ПолучитьДокументPDF(ПолноеИмяФайлаПредпросмотра) Экспорт
	
	ДокументPDF = Новый ДокументPDF;
	ДокументPDF.Прочитать(ПолноеИмяФайлаПредпросмотра);
...
//
Функция ПолучитьФайлИПоместитьВХранилище(ID, ИспользоватьВизуализациюШтампаЭП, УникальныйИдентификаторФормы,
		ДатаЗаема = Неопределено, Размер = Неопределено, ДатаМодификацииУниверсальная = Неопределено) Экспорт
	...
	Иначе // Получение с захватом.
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
			ВызватьИсключение НСтр("ru = 'Заем файлов не поддерживается используемой версией 1С:Документооборота'; en = 'Files loan is not supported by the used version of 1C:Document Management'");
		КонецЕсли;
		Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMLockFileRequest");
...
Функция СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, ТекстСообщения = "") Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("1.4.6.1") Тогда
		ТекстСообщения = НСтр("ru = 'Для работы с версиями нужен 1С:Документооборот версии не ниже 1.4.6.1'; en = 'To operate with versions, 1C:Document Management not earlier than 1.4.6.1 is required'");
		Возврат Ложь;
	КонецЕсли;
...
// Сохраняет двоичные данные во временный файл на сервере.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Строка - полное имя файла на сервере.
//
Функция СохранитьФайлНаСервере(ДвоичныеДанные, Расширение) Экспорт
...
//
// Параметры:
//   ПолноеИмяФайла - Строка - полное имя файла на сервере.
//
// Возвращаемое значение:
//   Булево - Истина, если файл существует, Ложь в противном случае.
//
Функция ФайлЕстьНаСервере(ПолноеИмяФайла) Экспорт
	
	ФайлЕстьНаСервере = Ложь;
	Если ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
		Файл = Новый Файл(ПолноеИмяФайла);
		ФайлЕстьНаСервере = Файл.Существует();
	КонецЕсли;
	
	Возврат ФайлЕстьНаСервере;
...
// Читает и возвращает табличный документ из временного хранилища.
//
// Параметры:
//   АдресТабличногоДокументаВХранилище - Строка - адрес табличного документа во временном хранилище.
//
// Возвращаемое значение:
//   ТабличныйДокумент
//
Функция ЧтениеТабличногоДокумента(АдресТабличногоДокументаВХранилище) Экспорт
	
	ДанныеТабличногоДокумента = ПолучитьИзВременногоХранилища(АдресТабличногоДокументаВХранилище);
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("mxl");
	ДанныеТабличногоДокумента.Записать(ИмяВременногоФайла);
...
Процедура ЗаполнитьСписокПодписейСервер(ПодписиXDTO, ФайлыXDTO, Форма,
	...
	Если ЭлементДляИзмененияЗаголовка <> Неопределено Тогда
		ЭлементДляИзмененияЗаголовка.Заголовок = СтрШаблон(НСтр("ru = 'ЭП (%1)'; en = 'Digital signature (%1)'"), ВсегоПодписей);
...
Процедура ЗаполнитьСтрокуДереваПодписей(НоваяСтрока, НомерСтроки, Подпись, ДанныеВладельцаПодписи,
	...
	ФорматнаяСтрока = НСтр("ru = 'ДФ=""дд.ММ.гггг ЧЧ:мм""'; en = 'DF=""MM/dd/yyyy HH:mm""'");
	...
	НоваяСтрока.ОбъектID = ДанныеВладельцаПодписи.ID;
...
//   ОбъектID - Строка - идентификатор документа Документооборота.
...
//
Процедура ПолучитьОбъектыДляПодписи(ОбъектНаименование, ОбъектID, ОбъектТип, МассивОбъектовДляПодписи, Файлы,
		ДвоичныеДанные) Экспорт
	
	// Добавим документ в массив объектов для подписи.
	ОбъектДляПодписи = ОбъектДляПодписи();
	ОбъектДляПодписи.ДвоичныеДанные = ДвоичныеДанные;
	ОбъектДляПодписи.ОбъектСсылкаДляПодписи = ОбъектID;
	...
	ДанныеФайловXDTO = ДвоичныеДанныеФайлов(Файлы, Ложь);
...
Функция УдалитьПодписи(УдаляемыеПодписи, ТаблицаПодписей) Экспорт
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Для Каждого СтрокаУдаленнойПодписи Из УдаляемыеПодписи Цикл
			Если СтрокаУдаленнойПодписи.ОбъектID = ДанныеСтроки.ОбъектID
...
Функция ДанныеПодписиИзXDTO(ПодписьXDTO)
	...
	ДанныеПодписи.УстановившийПодписьID = ПодписьXDTO.signer.objectID.ID;
...
Процедура ДополнитьНаборКолонокУсловийВыбора(НаборКолонок, ТипXDTO)
	...
	ИначеЕсли ТипXDTO = "DMProduct" Тогда
		...
		НаборКолонок.Добавить("VATRateUniversal");
...
Функция ОперацияПодключенияИПолученияВерсииСервиса(ИспользоватьКэш, ИмяПользователя, Пароль,
		УникальныйИдентификаторФормы = Неопределено, АдресВебСервисаДокументооборота = Неопределено)
	
	ПараметрыДлительнойОперации = ПараметрыДлительнойОперации(ИспользоватьКэш, АдресВебСервисаДокументооборота);
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подключение к 1С:Документооборот'; en = 'Connect to 1C:Document Management'");
