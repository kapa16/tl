...
Процедура ЗаполнитьНаименованиеВидаДокумента(ВидДокументаОбъект, ПредставлениеКомандыПечати = "") Экспорт
	...
	Иначе
		...
		ВидДокументаОбъект.КраткоеНаименование =
			Перечисления.ТипыДокументовЭДО.КраткоеНаименование(ВидДокументаОбъект.ТипДокумента);
	КонецЕсли;
...
Процедура ОбновитьЭлементыСхемыРегламента(Приемник, Источник)
	...
	Для Каждого ЭлементИсточника Из КоллекцияЭлементовИсточника Цикл
		...
		КонецЕсли;
		// {УАТ}
		Если НЕ ЗначениеЗаполнено(ЭлементПриемника.Направление) Тогда
			ЭлементПриемника.Направление = Перечисления.НаправленияЭДО.Исходящий;
		КонецЕсли;
		// {/УАТ}
		Индекс = Индекс + 1;
	КонецЦикла;
