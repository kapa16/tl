...
// * Метод - Строка -
// * Маркер - Неопределено,ДвоичныеДанные
// * ТребуетсяМаркер - Булево
...
// * РазрешенныеКодыОшибок - Массив из Число
...
Функция ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации = Неопределено) Экспорт
		
	РезультатВыполненияОперации = НовыйРезультатВыполненияОперации();
	ТекстОшибкиДоступа = НСтр("ru = 'Отсутствует доступ к сервису 1С-ЭДО'");
	Если Не ЭтоОперацияПоТокену(ПараметрыОперации)
		И Не ЭтоОперацияPing(ПараметрыОперации)
		...
		РезультатВыполненияОперации.ОшибкаОператора = НоваяОшибкаОператора();
		РезультатВыполненияОперации.ОшибкаОператора.Текст = ТекстОшибкиДоступа;
		РезультатВыполненияОперации.ОшибкаОператора.Детали = ТекстОшибкиДоступа;
	...
	Если ИспользоватьТокенПриВыполненииОперации(ПараметрыОперации)
		И Не АктуализироватьТокенПриВыполненииОперации(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации) Тогда
		Возврат РезультатВыполненияОперации;
	КонецЕсли;
	...
	Если Не РезультатВыполненияЗапроса.Успех Тогда
		ОшибкаОператора = ПрочитатьОписаниеОшибкиОператора(РезультатВыполненияЗапроса.Ответ, ТекстОшибкиДоступа);
	...
	КодСостояния = РезультатВыполненияЗапроса.Ответ.КодСостояния;
	Если КодСостояния = 200 Тогда
		РезультатВыполненияОперации.Успех = Истина;
		Возврат РезультатВыполненияОперации;
	КонецЕсли;
	
	ОшибкаОператора = ПрочитатьОписаниеОшибкиОператора(РезультатВыполненияЗапроса.Ответ);
	
	Если КодСостояния = 403
		И ИспользоватьТокенПриВыполненииОперации(ПараметрыОперации) Тогда
		РезультатВыполненияОперации = ОбработатьОшибкуТокенаПриВыполненииОперации(ПараметрыОперации,
			КонтекстДиагностики, ОшибкаОператора, КлючСинхронизации);
		Возврат РезультатВыполненияОперации;
	КонецЕсли;
...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
		...
		Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
			...
			АвтоматическоеПолучениеЭДО.ДобавитьЗаголовокАвторизацииПоКлючуДоступа(
				ПараметрыВыполненияОперации, КлючСинхронизации);
...
Процедура ЗаполнитьПараметрыАутентификации(КлючиСинхронизации, КонтекстДиагностики) Экспорт
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Если ТипЗнч(МаркерЗашифрованный) = Тип("ДвоичныеДанные") Тогда
			...
			Если СертификатДоступенНаСервере И ДанныеСертификата.ПарольПолучен И КриптооперацииДоступныНаСервере Тогда
...
// 	
// Возвращаемое значение:
// 	См. ПриглашенияЭДО.НоваяТаблицаПриглашений
//
Функция ПолучитьПриглашения(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики) Экспорт
	...
	ДобавитьТокенВПараметрыОперации(ПараметрыВыполненияОперации, КлючСинхронизации);
...
// * СтатусКонтактаНаСервереОператора - ПеречислениеСсылка.СтатусыПриглашений
Функция ПринятьПриглашение(КлючСинхронизации, ИдентификаторКонтрагента, КонтекстДиагностики) Экспорт
	
	Возврат ПринятьОтклонитьПриглашение(КлючСинхронизации, ИдентификаторКонтрагента, Истина, КонтекстДиагностики);
	
КонецФункции

// Отклоняет приглашение к обмену электронными документами.
// 
// Параметры:
// 	КлючСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
...
//                                          не совпадает со статусом в сервисе).
// * СтатусКонтактаНаСервереОператора - ПеречислениеСсылка.СтатусыПриглашений
Функция ОтклонитьПриглашение(КлючСинхронизации, ИдентификаторПриглашения, КонтекстДиагностики,
	ПричинаОтказаОтПриглашения = "") Экспорт
	
	Возврат ПринятьОтклонитьПриглашение(КлючСинхронизации, ИдентификаторПриглашения, Ложь, КонтекстДиагностики,
		ПричинаОтказаОтПриглашения);
...
Функция ОтправитьКонтейнер(КлючСинхронизации, ОписаниеКонтейнера, КонтекстДиагностики) Экспорт
	...
	Если РезультатВыполненияОперации.ОшибкаОператора <> Неопределено
		И РезультатВыполненияОперации.ОшибкаОператора.Код <> 0 Тогда
		...
		Результат.Ошибка.Описание = ПредставлениеОшибкиОператора(РезультатВыполненияОперации.ОшибкаОператора);
	КонецЕсли;
...
Функция ПолучитьДокументыЭПД(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД")
		...
		СписокДокументов = ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, 
			КонтекстДиагностики, Перечисления.ВидыСервисовЭДО.ЭПД);
			
		ИдентификаторЭДО = КлючСинхронизации.ИдентификаторУчетнойЗаписи;
		Если МодульОбменСГИСЭПД.МобильныеПриложенияПодключеныПоИдентификаторуЭДО(ИдентификаторЭДО) Тогда
			ПараметрыЗапросаИсходящих = МодульОбменСГИСЭПД.ПараметрыПолученияСпискаИсходящихСообщений();
			СписокИсходящихДокументов = ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, 
				КонтекстДиагностики, Перечисления.ВидыСервисовЭДО.ЭПД, ПараметрыЗапросаИсходящих);
				
			МодульОбменСГИСЭПД.ПриПолученииСпискаИсходящихДокументовСервисаЭДО(СписокДокументов,
				СписокИсходящихДокументов, ИдентификаторЭДО);
		КонецЕсли;
...
//
// Возвращаемое значение:
//  Число - приоритет, где 0 - самый высокий
Функция ПриоритетРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой = "") Экспорт
	...
	ИначеЕсли КодТранзакции = "VendorTitle" Или КодТранзакции = "CustomerTitle" Тогда
		...
		Приоритет = 1;
		Если ТипДокументаСтрокой = "FormalizedDocumentOnAdditionalInformation" Тогда
			Приоритет = 2;
		КонецЕсли;
		
	ИначеЕсли КодТранзакции = "Invoice" Тогда
		
		Если ТипДокументаСтрокой = "PrimaryAccountingDocumentVendor"
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentVendorConsignment"
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentVendorWorkResult"
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentCustomer"
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentCustomerConsignment"
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentCustomerWorkResult" Тогда
			
			Приоритет = 0;
			
		ИначеЕсли ТипДокументаСтрокой = "CorPrimaryAccountingDocumentVendor"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentVendorConsignment"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentVendorWorkResult"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentCustomer"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentCustomerConsignment"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentCustomerWorkResult" Тогда
...
//                             - см. СервисЭДОКлиентСервер.НовыеПараметрыУведомлений
//      * КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ПолучитьПараметрыУведомлений(КлючСинхронизации) Экспорт
	...
	РезультатЗапроса = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики);
	...
	Попытка
		ОтветСервиса = РаботаСФайламиБЭД.ПрочитатьXDTO(РезультатЗапроса.Ответ.ПолучитьТелоКакПоток());
	Исключение
	...
	Результат.ПараметрыУведомлений = ПараметрыУведомлений;
...
// * ИдФайл - Строка - Используется для загрузки исходящих титулов в подсистеме ОбменСГИСЭПД.
Функция НоваяТаблицаКонтейнеров() Экспорт
	...
	ТЗ.Колонки.Добавить("ИдФайл", Новый ОписаниеТипов("Строка"));
...
// Параметры:
//  ИдентификаторУчетнойЗаписи - Строка
//  ДанныеМаркера - ДвоичныеДанные,Строка
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  - Неопределено
//  - См. НовыйТокенАвторизации
Функция ПолучитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ДанныеМаркера, КонтекстДиагностики) Экспорт
	
	Если ТипЗнч(ДанныеМаркера) = Тип("ДвоичныеДанные") Тогда
		МаркерСтрокой = ПолучитьСтрокуИзДвоичныхДанных(ДанныеМаркера);
	ИначеЕсли ТипЗнч(ДанныеМаркера) = Тип("Строка") Тогда 
		МаркерСтрокой = ДанныеМаркера;
	КонецЕсли;
	
	ПараметрыВыполнения = НовыеПараметрыВыполненияОперацииСервиса();
	ПараметрыВыполнения.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
	ПараметрыВыполнения.ТребуетсяМаркер = Ложь;
	ПараметрыВыполнения.ИдентификаторУчетнойЗаписи = ИдентификаторУчетнойЗаписи;
	ПараметрыВыполнения.ПредставлениеОперации = НСтр("ru = 'Получение токена доступа к сервису ЭДО.'");
	ПараметрыВыполнения.АдресРесурса = "/auth/token/by-certificate-ticket";
	ПараметрыВыполнения.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыВыполнения.Таймаут = 10;
	ИнформацияОПриложении = ИнтеграцияОблачногоЭДОПовтИсп.ИнформацияОПриложении();
	ПараметрыВыполнения.Заголовки.Вставить("x-app-name", ИнформацияОПриложении.ИмяКонфигурации);
	ПараметрыВыполнения.ТелоЗапроса = СтрШаблон("{ ""ticket"":""%1"" }", МаркерСтрокой);
	
	Результат = ВыполнитьОперацию(ПараметрыВыполнения, КонтекстДиагностики);
	
	ТокенАвторизации = ТокенАвторизацииИзРезультатаОперации(Результат,
		ПараметрыВыполнения.ПредставлениеОперации, КонтекстДиагностики);
	
	Если ТокенАвторизации <> Неопределено Тогда
		ЗаписатьТокенАвторизацииВБезопасноеХранилище(ИдентификаторУчетнойЗаписи, ТокенАвторизации);
	КонецЕсли;
...
//  - Неопределено
//  - См. НовыйТокенАвторизации
Функция ОбновитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ТокенОбновления, КонтекстДиагностики) Экспорт
	
	ПараметрыВыполнения = НовыеПараметрыВыполненияОперацииСервиса();
	ПараметрыВыполнения.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
	ПараметрыВыполнения.ТребуетсяМаркер = Ложь;
	ПараметрыВыполнения.ИдентификаторУчетнойЗаписи = ИдентификаторУчетнойЗаписи;
	ПараметрыВыполнения.ПредставлениеОперации = НСтр("ru = 'Обновление токена доступа к сервису ЭДО.'");
	ПараметрыВыполнения.АдресРесурса = "/auth/token/refresh";
	ПараметрыВыполнения.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыВыполнения.Таймаут = 10;
	ПараметрыВыполнения.ТелоЗапроса = СтрШаблон("{ ""refresh_token"":""%1"" }", ТокенОбновления);
	...
	Если ТокенАвторизации = Неопределено Тогда
		УдалитьТокенАвторизацииИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи);
	Иначе
		ЗаписатьТокенАвторизацииВБезопасноеХранилище(ИдентификаторУчетнойЗаписи, ТокенАвторизации);
	КонецЕсли;
...
//             - См. НовыйТокенАвторизации
Функция ТокеныАвторизацииИзБезопасногоХранилища(ИдентификаторыУчетныхЗаписей) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Возврат ОбщегоНазначения.ПрочитатьДанныеВладельцевИзБезопасногоХранилища(
		ИдентификаторыУчетныхЗаписей, КлючТокенаАвторизацииВБезопасномХранилище());
	
КонецФункции

// Параметры:
//  ИдентификаторУчетнойЗаписи - Строка
Процедура УдалитьТокенАвторизацииИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.УдалитьДанныеИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи,
		КлючТокенаАвторизацииВБезопасномХранилище());
...
Функция ПредставлениеОшибкиОператора(ОшибкаОператора)
	Если ЗначениеЗаполнено(ОшибкаОператора.Детали) Тогда
		Представление = СтрШаблон("%1: %2", ОшибкаОператора.Описание, ОшибкаОператора.Детали);
	Иначе
		Представление = ОшибкаОператора.Описание;
	КонецЕсли;
...
// 
// Параметры:
//  ОшибкаОператора - см. НоваяОшибкаОператора
// 
// Возвращаемое значение:
//  Булево
...
// Параметры:
//  Ошибки - Массив из Структура см. ОбработкаНеисправностейБЭД.НоваяОшибка
Функция ОшибкаНедействительностиСертификатаЗафиксирована(Ошибки)
	
	Возврат ОбработкаНеисправностейБЭД.ОшибкаСИдентификаторомЗафиксирована(Ошибки,
		УчетныеЗаписиЭДО.ИдентификаторОшибкиНедействительныеСертификаты());
...
//  ВидОперации - Строка
//  ОшибкаОператора - см. ПрочитатьОписаниеОшибкиОператора
//  ЗаписыватьОшибкуВЖурналРегистрации - Булево
Процедура ОбработатьОшибкуНедействительныхСертификатов(КонтекстДиагностики, ИдентификаторУчетнойЗаписи, ВидОперации,
	ОшибкаОператора, ЗаписыватьОшибкуВЖурналРегистрации)
	
	Если ОшибкаНедействительностиСертификатаЗафиксирована(КонтекстДиагностики.Диагностика.Ошибки) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не УчетныеЗаписиЭДО.ЕстьНедействительныеСертификаты(ИдентификаторУчетнойЗаписи) Тогда
		Возврат;
	КонецЕсли;
		
	НаименованиеУчетнойЗаписи =
		УчетныеЗаписиЭДО.ДанныеУчетнойЗаписи(ИдентификаторУчетнойЗаписи).НаименованиеУчетнойЗаписи;
	
	ВидОшибки =
		УчетныеЗаписиЭДО.ВидОшибкиНедействительныеСертификаты(НаименованиеУчетнойЗаписи,
			ИдентификаторУчетнойЗаписи);
	
	КраткоеПредставление = ПредставлениеОшибкиОператора(ОшибкаОператора);
	
	Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, ВидОшибки, ОшибкаОператора.Текст, КраткоеПредставление);
	ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
...
Процедура ОбработатьОшибкуСервиса(ПараметрыОперации, КонтекстДиагностики, ОшибкаОператора,
	...
	ИдентификаторОператора = УчетныеЗаписиЭДОКлиентСервер.ИдентификаторОператора(ПараметрыОперации.ИдентификаторУчетнойЗаписи);
	Если СервисНастроекЭДО.ЭтоКодОшибкиКонтроляНедействительныхСертификатов(ИдентификаторОператора,ОшибкаОператора.Код) 
		И УчетныеЗаписиЭДО.ЕстьНедействительныеСертификаты(ПараметрыОперации.ИдентификаторУчетнойЗаписи) Тогда
		
		ОбработатьОшибкуНедействительныхСертификатов(КонтекстДиагностики, ПараметрыОперации.ИдентификаторУчетнойЗаписи,
			ВидОперации, ОшибкаОператора, ЗаписыватьОшибкуВЖурналРегистрации);
		Возврат;
...
// Возвращаемое значение:
//  HTTPЗапрос
Функция ЗапросКСервису(ПараметрыОперации) Экспорт
	
	Заголовки = Новый Соответствие;
	
	Если ПараметрыОперации.ТребуетсяМаркер Тогда
		Если ИспользоватьТокенПриВыполненииОперации(ПараметрыОперации) Тогда
			ДобавитьЗаголовокАвторизацииПоТокену(Заголовки, ПараметрыОперации.Токен);
		Иначе
			ДобавитьЗаголовокАвторизацииПоМаркеру(Заголовки, ПараметрыОперации.Маркер);
		КонецЕсли;
	КонецЕсли;
...
//  * Ключ - Строка
//  * Значение - Строка
//  Маркер - ДвоичныеДанные,Строка
Процедура ДобавитьЗаголовокАвторизацииПоМаркеру(Заголовки, Маркер)
	...
	Если ЗначениеЗаполнено(МаркерСтрокой) Тогда
...
// Параметры:
//  РезультатВыполненияОперации - см. РезультатОперации
//  ДатаПоследнегоЗапроса       - Дата
//  ВидСервисаЭДО - Неопределено, ПеречислениеСсылка.ВидыСервисовЭДО - если ВидСервисаЭДО = Неопределено, то
//                                                                     это документы ЭДО.
// 
// Возвращаемое значение:
//  см. НовыйРезультатПолученияСпискаДокументов
Функция СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатВыполненияОперации, ДатаПоследнегоЗапроса, ВидСервисаЭДО = Неопределено)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		ДополнитьДаннымиЭПД = МодульОбменСГИСЭПД.ПриФормированииСпискаДокументовИзОтветаОтСервиса(Результат, ВидСервисаЭДО);
	Иначе
		ДополнитьДаннымиЭПД = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СписокЭД_XML) Тогда
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.ЛокальноеИмя = "Identifiers" Тогда
				...
				СтрТЗ.Приоритет          = ПриоритетРаспаковкиКонтейнера(КодТранзакции);
			...
			Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.ЛокальноеИмя = "Type" Тогда
				...
				СтрТЗ.Приоритет = ПриоритетРаспаковкиКонтейнера(КодТранзакции, ТипДокумента);
				
				Продолжить;
				
			КонецЕсли;
			
			Если ДополнитьДаннымиЭПД Тогда
				МодульОбменСГИСЭПД.ЗаполнитьВСтрокуСпискаДокументовДополнительныеДанныеЭПД(ЧтениеXML, СтрТЗ);
			КонецЕсли;
...
// * ОшибкаОператора - Неопределено, Структура -
...
// Описание ошибки оператора.
// 
// Возвращаемое значение:
//  Структура - Новая ошибка оператора:
// * Код - Число - 
// * Текст - Строка - 
// * Данные - Соответствие Из КлючИЗначение - 
// * Детали - Строка - 
...
// * ИстекСрокДействияМаркера - Булево - 
// * ДополнительныеДанные - Неопределено - 
Функция НоваяОшибкаОператора()
	
	Ошибка = Новый Структура;
	Ошибка.Вставить("Код", 0);
	Ошибка.Вставить("Текст", НСтр("ru = 'Неизвестная ошибка.'"));
...
Функция ПроверитьДоступКВебСервису1СЭДО() Экспорт
	...
	Если РезультатВыполненияЗапроса.Успех И РезультатВыполненияЗапроса.Ответ.КодСостояния = 200 Тогда
...
//  Структура:
//  * Успех - Булево
//  * ТребуетсяОбновлениеСтатусов - Булево
//  * СтатусКонтактаНаСервереОператора - ПеречислениеСсылка.СтатусыПриглашений
Функция ПринятьОтклонитьПриглашение(КлючСинхронизации, ИдентификаторКонтрагента, Принять,
	...
	ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "id", ИдентификаторКонтрагента);
	...
	Если Не Результат.Успех И Результат.ОшибкаОператора <> Неопределено
...
//  Ошибки - Неопределено,Массив из См. ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки 
// 
// Возвращаемое значение:
//  Структура:
//   * ДвоичныеДанные - ДвоичныеДанные
...
//      ** Имя - Строка
//      ** Отчество - Строка
//      ** ИННДоверителяИДоверенныхЛицДоверенностей - Соответствие из КлючИЗначение:
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	ПараметрыСертификатаФамилия = ПараметрыСертификата.Фамилия;
	ПараметрыСертификатаИмя = ПараметрыСертификата.Имя;
	ПараметрыСертификатаОтчество = ПараметрыСертификата.Отчество;
	...
	Если ИнтеграцияЭДО.ЭтоФизЛицо(ПараметрыРегистрации.Организация) Тогда
		Если Не ПустаяСтрока(ПараметрыОрганизацииБЭД.Фамилия)
			...
			ПараметрыСертификатаФамилия = ПараметрыОрганизацииБЭД.Фамилия;
			ПараметрыСертификатаИмя = ПараметрыОрганизацииБЭД.Имя;
			ПараметрыСертификатаОтчество = Отчество;
		КонецЕсли;
	КонецЕсли;
	
	ИННСубъектаСертификатаПодписанта = "";
	Если ПараметрыРегистрации.СертификатПодписанта <> Неопределено Тогда
		СертификатПодписанта = Новый СертификатКриптографии(
				ПараметрыРегистрации.СертификатПодписанта.ДанныеСертификата.Получить());
		ИННСубъектаСертификатаПодписанта = КриптографияБЭД.ИННСубъектаСертификата(СертификатПодписанта);
	КонецЕсли;
	
	НаименованиеОрганизации = ПараметрыОрганизации[ИмяРеквизитаНаименованиеОрганизации];
	УдалитьНедопустимыеСимволыИзНаименования(НаименованиеОрганизации);
	...
	РеквизитыПакета.Вставить("Фамилия",                  ПараметрыСертификатаФамилия);
	РеквизитыПакета.Вставить("Имя",                      ПараметрыСертификатаИмя);
	РеквизитыПакета.Вставить("Отчество",                 ПараметрыСертификатаОтчество);
	РеквизитыПакета.Вставить("ИННДоверителяИДоверенныхЛицДоверенностей", Новый Соответствие);
	РеквизитыПакета.Вставить("ИННСубъектаСертификатаПодписанта", ИННСубъектаСертификатаПодписанта);
	...
	ФамилияБезПодчеркивания = СтрЗаменить(ПараметрыСертификатаФамилия, "_", " ");
	ИмяБезПодчеркивания = СтрЗаменить(ПараметрыСертификатаИмя, "_", " ");
	ОтчествоБезПодчеркивания = СтрЗаменить(ПараметрыСертификатаОтчество, "_", " ");
	...
	СертификатыПользователя.Колонки.Добавить("ИННДоверенногоЛица");
	...
	ДанныеСертификатовОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ПараметрыРегистрации.СертификатыПодписейОрганизации,"ДанныеСертификата"); 
	ИННДоверителяИДоверенныхЛицДоверенностей = Новый Соответствие;
	СертификатыНеПрошедшиеПроверку = Новый Массив;
	МассивСертификатов = Новый Массив;
	ЭтоПодписаниеПоМЧД = Ложь;
	...
	Для каждого Сертификат Из ПараметрыРегистрации.СертификатыПодписейОрганизации Цикл
		...
		ТребуетсяДоверенность = УчетныеЗаписиЭДОСлужебный.ТребуетсяДоверенностьДляРегистрацииУчетнойЗаписиПоИНН(
			ИННОрганизации, Сертификат, ДанныеОператора.СпособОбмена, ДанныеОператора.Код);
		...
		Если Сертификат = ПараметрыРегистрации.СертификатПодписанта И ТребуетсяДоверенность Тогда
			ЭтоПодписаниеПоМЧД = Истина;
		КонецЕсли;
		
		Если ПараметрыРегистрации.ВидОперацииЭДО = УчетныеЗаписиЭДОКлиентСервер.ДействиеОбновлениеСертификата()
				И ТребуетсяДоверенность
				И Не ЗначениеЗаполнено(НомерДоверенности) Тогда
			СертификатыНеПрошедшиеПроверку.Добавить(Сертификат);
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(НомерДоверенности)
				И ЗначениеЗаполнено(Доверенность) Тогда
			...
			ИННДоверенногоЛица = СведенияМЧД.ИННПредставителей[0];
			
			СертификатПодписанта = Новый СертификатКриптографии(ДанныеСертификатовОрганизации[Сертификат].Получить());
			ИННСубъектаСертификатаПодписанта = КриптографияБЭД.ИННСубъектаСертификата(СертификатПодписанта);
			
			ИндексНайденногоДоверенногоЛица = СведенияМЧД.ИННПредставителей.Найти(ИННСубъектаСертификатаПодписанта);
			Если ИндексНайденногоДоверенногоЛица <> Неопределено Тогда
				ИННДоверенногоЛица = СведенияМЧД.ИННПредставителей[ИндексНайденногоДоверенногоЛица];
			КонецЕсли;
			
			ИННДоверителяИПредставителейПоДоверенности = Новый Структура;
			ИННДоверителяИПредставителейПоДоверенности.Вставить("ИННДоверителя", ИННДоверителя);
			ИННДоверителяИПредставителейПоДоверенности.Вставить("ИННПредставителей", СведенияМЧД.ИННПредставителей);
			
			ИННДоверителяИДоверенныхЛицДоверенностей.Вставить(Доверенность, ИННДоверителяИПредставителейПоДоверенности);
			
			НоваяСтрока.НомерМЧД = НомерДоверенности;
			НоваяСтрока.Передоверие = ИННДоверителя <> РеквизитыПакета.ИННОрганизации;
			НоваяСтрока.ИННДоверителя = ИННДоверителя;
			НоваяСтрока.ИННДоверенногоЛица = ИННДоверенногоЛица;
	...
	Если ИННДоверителяИДоверенныхЛицДоверенностей.Количество() > 0 Тогда 
		РеквизитыПакета.ИННДоверителяИДоверенныхЛицДоверенностей = ИННДоверителяИДоверенныхЛицДоверенностей;
	КонецЕсли;
	
	ПараметрыРегистрации.СертификатыПодписейОрганизации = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		ПараметрыРегистрации.СертификатыПодписейОрганизации, СертификатыНеПрошедшиеПроверку);
	...
	Если ПараметрыРегистрации.СертификатПодписанта <> Неопределено Тогда
		...
		СвойстваСубъектаСертификата = КриптографияБЭД.СвойстваСубъектаСертификатаПоСсылке(
			ПараметрыРегистрации.СертификатПодписанта);
			
		ОГРНВСертификате = ?(ЗначениеЗаполнено(СвойстваСубъектаСертификата.ОГРН), СвойстваСубъектаСертификата.ОГРН,
			СвойстваСубъектаСертификата.ОГРНИП);
	...
	ЭтоПодписаниеСертификатомРуководителя = ЗначениеЗаполнено(ОГРНВСертификате);
	
	Если ЭтоПодписаниеСертификатомРуководителя И Не ЭтоПодписаниеПоМЧД
		И ОГРНВСертификате <> ОГРНОрганизации Тогда
		Если Ошибки = Неопределено Тогда
			Ошибки = Новый Массив;
		КонецЕсли;
		
		ТекстОшибки = НСтр("ru = 'ОГРН в выбранном сертификате ""%1"" не соответствует ОГРН в организации ""%2"".
			|Скорректируйте данные и повторите попытку.'");
		
		Ошибка = ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки();
		Ошибка.ТекстОшибки = СтрШаблон(ТекстОшибки, ОГРНВСертификате, ОГРНОрганизации);
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыСертификатов.Значение) Тогда
			Для Каждого СтрСертификат Из СтрокаТаблицыСертификатов.Строки Цикл
				...
				ЗначенияСвойствXDTO.Вставить("ИННДоверенногоЛица", "УчастЭДО.СертифДолжн.НомерСтроки.ИННДоверенногоЛица");
				ЗначенияСвойствXDTO.Вставить("НомерМЧД", "УчастЭДО.СертифДолжн.НомерСтроки.НомерМЧД");
				ЗначенияСвойствXDTO.Вставить("СведСистОтм", "УчастЭДО.СертифДолжн.НомерСтроки.СведСистОтм");
				ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
				ПараметрыЗаполнения.НеобязательныеСвойства = 
					"Передоверие,ИННДоверителя,ИННДоверенногоЛица,НомерМЧД,СведСистОтм";
...
// Параметры:
//  НаименованиеОрганизации Наименование организации
Процедура УдалитьНедопустимыеСимволыИзНаименования(Наименование)
	
	НедопустимыеСимволы = Новый Массив;
	НедопустимыеСимволы.Добавить(Символы.ВК);
	НедопустимыеСимволы.Добавить(Символы.Таб);
	НедопустимыеСимволы.Добавить(Символы.ВТаб);
	НедопустимыеСимволы.Добавить(Символы.НПП);
	НедопустимыеСимволы.Добавить(Символы.ПС);
	НедопустимыеСимволы.Добавить(Символы.ПФ);
	НедопустимыеСимволы.Добавить("	");
	НедопустимыеСимволы.Добавить("¶");
	
	Наименование = СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими(
		СтрСоединить(НедопустимыеСимволы, ""),
		Наименование,
		" ");
	Наименование = СокрЛП(Наименование);
	Пока СтрНайти(Наименование, "  ") Цикл
...
//                                                                     это документы ЭДО
//  ПараметрыЗапроса - Неопределено, Структура - Дополнительные параметры, которые будут использованы при выполнении запроса
// 
// Возвращаемое значение:
//  См. НовыйРезультатПолученияСпискаДокументов
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, КонтекстДиагностики, ВидСервисаЭДО = Неопределено,
	...
	Если ПараметрыЗапроса <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыВыполненияОперации.ПараметрыЗапроса, ПараметрыЗапроса);
	КонецЕсли;
...
//  * ТокенДоступа - Строка
//  * ТокенОбновления - Строка
//  * СрокДействияТокенаДоступа - Дата - дата окончания срока действия в UTC.
//  * СрокДействияТокенаОбновления - Дата - дата окончания срока действия в UTC.
Функция НовыйТокенАвторизации()
	...
	ТокенАвторизации.Вставить("ТокенОбновления", "");
	ТокенАвторизации.Вставить("СрокДействияТокенаДоступа", '00010101');
	ТокенАвторизации.Вставить("СрокДействияТокенаОбновления", '00010101');
	Возврат ТокенАвторизации;
КонецФункции

// Параметры:
//  РезультатОперации - см. ВыполнитьОперацию
//  ВидОперации - Строка
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  - Неопределено
//  - См. НовыйТокенАвторизации
Функция ТокенАвторизацииИзРезультатаОперации(РезультатОперации, ВидОперации, КонтекстДиагностики)
	...
	Попытка
		ТокенАвторизацииВФорматеСервиса = ОбщегоНазначенияБЭД.JSONЗначение(
			РезультатОперации.Ответ.ПолучитьТелоКакСтроку());
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	ДатаUnix = ОбщегоНазначенияБЭД.ДатаUnix();
	
	ТокенАвторизации = НовыйТокенАвторизации();
	ТокенАвторизации.ТипТокена = ТокенАвторизацииВФорматеСервиса.token_type;
	ТокенАвторизации.ТокенДоступа = ТокенАвторизацииВФорматеСервиса.access_token;
	ТокенАвторизации.ТокенОбновления = ТокенАвторизацииВФорматеСервиса.refresh_token;
	ТокенАвторизации.СрокДействияТокенаДоступа = ДатаUnix + ТокенАвторизацииВФорматеСервиса.access_token_expires;
	ТокенАвторизации.СрокДействияТокенаОбновления = ДатаUnix + ТокенАвторизацииВФорматеСервиса.refresh_token_expires;
...
Процедура ЗаписатьТокенАвторизацииВБезопасноеХранилище(ИдентификаторУчетнойЗаписи, ТокенАвторизации)
	
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
		ИдентификаторУчетнойЗаписи, ТокенАвторизации, КлючТокенаАвторизацииВБезопасномХранилище());
...
//  ПараметрыОперации - см. НовыеПараметрыВыполненияОперацииСервиса
//  КлючСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
Процедура ДобавитьТокенВПараметрыОперации(ПараметрыОперации, КлючСинхронизации)
	
	ПараметрыОперации.Токен = КлючСинхронизации.Токен;
	
КонецПроцедуры

// Параметры:
//  ПараметрыОперации - см. НовыеПараметрыВыполненияОперацииСервиса
Функция ЭтоОперацияПоТокену(ПараметрыОперации)
	
	Возврат ПараметрыОперации.Токен <> Неопределено;
...
// Возвращаемое значение:
//  Булево
Функция ИспользоватьТокенПриВыполненииОперации(ПараметрыОперации)
...
// Возвращаемое значение:
//  Булево
Функция АктуализироватьТокенПриВыполненииОперации(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации = Неопределено)
	
	Если КлючСинхронизации <> Неопределено
		И Не КлючСинхронизации.ТокенВалиден Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТокенАвторизации = ПараметрыОперации.Токен;
	
	ЗапасВремени = 30;
	УниверсальнаяДатаПроверки = ТекущаяУниверсальнаяДата() + ЗапасВремени;
	Если ТокенАвторизации.СрокДействияТокенаДоступа > УниверсальнаяДатаПроверки Тогда
		Возврат Истина;
	КонецЕсли;
	
	ИдентификаторУчетнойЗаписи = ПараметрыОперации.ИдентификаторУчетнойЗаписи;
	
	Если УниверсальнаяДатаПроверки > ТокенАвторизации.СрокДействияТокенаОбновления Тогда
		ВидОперации = НСтр("ru = 'Обновление токена доступа'");
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Истек срок действия токена доступа для учетной записи ЭДО %1'"),
			ИдентификаторУчетнойЗаписи);
		ВидОшибки = СинхронизацияЭДО.ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ИдентификаторУчетнойЗаписи);
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, ВидОшибки, ТекстСообщения, ТекстСообщения);
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
		Если КлючСинхронизации <> Неопределено Тогда
			КлючСинхронизации.ТокенВалиден = Ложь;
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
	
	ТокенАвторизации = ОбновитьТокенАвторизации(ИдентификаторУчетнойЗаписи,
		ТокенАвторизации.ТокенОбновления, КонтекстДиагностики);
	...
	ПараметрыОперации.Токен = ТокенАвторизации;
	
	Если КлючСинхронизации <> Неопределено Тогда
		КлючСинхронизации.Токен = ПараметрыОперации.Токен;
	КонецЕсли;
...
//  * Значение - Строка
//  ТокенАвторизации - см. НовыйТокенАвторизации
Процедура ДобавитьЗаголовокАвторизацииПоТокену(Заголовки, ТокенАвторизации)
	
	ЗначениеЗаголовкаАвторизации = СтрШаблон("%1 %2", ТокенАвторизации.ТипТокена, ТокенАвторизации.ТокенДоступа);
	Заголовки.Вставить("Authorization", ЗначениеЗаголовкаАвторизации);
...
// Возвращаемое значение:
//  См. НовыйРезультатВыполненияОперации
Функция ОбработатьОшибкуТокенаПриВыполненииОперации(ПараметрыОперации, КонтекстДиагностики, ОшибкаОператора, КлючСинхронизации = Неопределено)
	
	ТокенДоступа = ПараметрыОперации.Токен.ТокенДоступа;
	Если АктуализироватьТокенПриВыполненииОперации(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации)
		И ТокенДоступа <> ПараметрыОперации.Токен.ТокенДоступа Тогда
		Возврат ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации);
	КонецЕсли;
	
	ВидОперации = ПараметрыОперации.ПредставлениеОперации;
	ВидОшибки = СинхронизацияЭДО.ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ПараметрыОперации.ИдентификаторУчетнойЗаписи);
	Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации, ВидОшибки, ОшибкаОператора.Текст,
		ОшибкаОператора.Описание);
	ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
		ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
	
	Возврат НовыйРезультатВыполненияОперации();
