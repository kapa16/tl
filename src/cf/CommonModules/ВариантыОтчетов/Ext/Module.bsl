			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось прочитать схему отчета %1:
						|%2';
						|en = 'Cannot read the schema of report ""%1"":
				...
				Исключение
					Если ОбщегоНазначения.РазделениеВключено() Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета %1 в неразделенном сеансе,
							...
							|because its settings refer to separated predefined objects.
							|
							|%2'");
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из приложения';
				|en = 'Cannot attach report %1 from the application. Reason: The report was added as an external report.'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
		...
		Если ТипЗнч(Результат.Метаданные) <> Тип("ОбъектМетаданных") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отчет ""%1"" не существует в приложении.';
					|en = 'The report %1 doesn''t exist in the application.'"),
				Результат.Имя);
			Возврат Результат;
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Варианты отчетов.Обновление служебного пользователя';
				|en = 'Report options.Update utility user'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Варианты отчетов.Создание служебного пользователя';
				|en = 'Report options.Create utility user'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.Пользователи,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" не существует в приложении>';
						|en = '<Report %1 doesn''t exist in application>'"),
					ИмяОтчета);
...
#Область ЧтениеФайлаОписанияНастроек
	...
	ШапкаОписанияОшибки = НСтр("ru = 'Некорректный формат описания настроек:';
								|en = 'Incorrect settings description format:'") + Символы.ПС;
