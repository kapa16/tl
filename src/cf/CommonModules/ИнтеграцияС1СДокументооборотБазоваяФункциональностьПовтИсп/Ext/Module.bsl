...
//
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, АдресВебСервисаДокументооборота = Неопределено) Экспорт
	...
	НастройкиАвторизации.ИспользоватьКеш = (АдресВебСервисаДокументооборота = Неопределено);
...
Функция СоответствиеТипов() Экспорт
	...
	КонецЕсли;
	Если ТипСтраныМира() <> Неопределено И ДоступенФункционалВерсииСервиса("3.0.16.33") Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДобавитьСтрокуСоответствияТипов(
			Таблица,
			"DMCountry",
			ТипСтраныМира());
...
// Возвращает тип справочника Страны мира.
//
// Параметры:
//   ТипОбъекта - Булево - требуется вернуть тип ссылки или тип объекта.
//
...
//   Неопределено
//
Функция ТипСтраныМира(ТипОбъекта = Ложь) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		Если ТипОбъекта Тогда
			ИмяТипа = "СправочникОбъект.СтраныМира";
		Иначе
			ИмяТипа = "СправочникСсылка.СтраныМира";
		КонецЕсли;
		Возврат Тип(ИмяТипа);
	КонецЕсли;
...
Функция УзелДокументооборота() Экспорт
	...
	Иначе
		...
		Узел1СДокументооборота.Наименование = НСтр("ru = '1С:Документооборот'; en = '1C:Document Management'");
...
Функция ЭтоТонкийКлиент() Экспорт
	...
	Возврат ОбщегоНазначения.ИспользуемыйКлиент() = "ТонкийКлиент";
