...
// см.ИнтерфейсДокументовЭДО.ДлительнаяОперацияФайлСДаннымиОЗадолженности
Функция ДлительнаяОперацияФайлСДаннымиОЗадолженности(УникальныйИдентификатор, СсылкаНаPDF) Экспорт
	
	Возврат ИнтерфейсДокументовЭДО.ДлительнаяОперацияФайлСДаннымиОЗадолженности(УникальныйИдентификатор, СсылкаНаPDF);
	
КонецФункции

// см.ИнтерфейсДокументовЭДО.СведенияОЗадолженности
...
Функция ПодготовитьДанныеДляПросмотра(Знач ОбъектУчета, Знач НастройкиФормирования, Знач ПараметрыОткрытия) Экспорт
	...
	ОписанияДокументовОбъектаУчета = ИнтеграцияЭДО.НовоеОписаниеОбъектаУчета();
	НовоеОписаниеОбъектаУчета = ОписанияДокументовОбъектаУчета.Добавить();
	ЗаполнитьЗначенияСвойств(НовоеОписаниеОбъектаУчета, ОписаниеОбъектаУчета);
	
	Возврат ИнтерфейсДокументовЭДО.ПодготовитьДанныеПредварительногоПросмотра(ОписанияДокументовОбъектаУчета,
...
Функция ПереформироватьДокумент(Параметры, КонтекстДиагностики) Экспорт
	...
	Попытка
		...
		Если Параметры.ТребуетсяФормированиеДокумента = Истина Тогда
			...
			РезультатыПоИнтеграциямЭДО = ЭлектронныеДокументыЭДО.ВыполнитьДействияПоИнтеграциямЭДО(
			...
			Если ЗначениеЗаполнено(Параметры.ИдентификаторПакета) Тогда
				...
				Для каждого СтрокаАктуальныйЭлектронныйДокумент Из АктуальныеЭлектронныеДокументыПоВидуДокумента Цикл
					...
					ПакетыДокументовЭДО.ДобавитьДокументыВПакет(Параметры.ИдентификаторПакета,
						ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(АктуальныйЭлектронныйДокумент),
						КонтекстДиагностики);
				КонецЦикла;
		...
		Если ЗначениеЗаполнено(Параметры.ИдентификаторПакета) Тогда
			Для Каждого СформированныйДокумент Из Результат.СформированныеДокументы Цикл
				ПакетыДокументовЭДО.ДобавитьДокументыВПакет(Параметры.ИдентификаторПакета,
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СформированныйДокумент.Ключ), КонтекстДиагностики);
			КонецЦикла;
		КонецЕсли;
		...
		РезультатыПоИнтеграциямЭДО = ЭлектронныеДокументыЭДО.ВыполнитьДействияПоИнтеграциямЭДО(
			ПараметрыДействийПоЭДО, КонтекстДиагностики);
		Результат.РезультатДействийПоЭДО = ЭлектронныеДокументыЭДО.ОбработатьРезультатыВыполненияДействийПоИнтеграциямЭДО(
			РезультатыПоИнтеграциямЭДО);
...
// 	ПараметрыПечати - См. ВизуализацияЭДОСлужебный.НовыеПараметрыВизуализацииДокумента
//
// Возвращаемое значение:
...
Функция РеквизитыДокументаПоФайлу(Знач ОписаниеФайла) Экспорт
...
Функция ДанныеФайла(Знач ФайлСсылка, Знач ПараметрыДанныхФайла = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(ФайлСсылка) Тогда
		ВызватьИсключение ИнтерфейсДокументовЭДОКлиентСервер.ТекстСообщенияОбОтсутствииФайлаЭлектронногоДокумента();
	КонецЕсли;
...
Процедура ОбработкаПолученияФормыЭлектронногоДокумента(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация,
	...
	ЭтоСуществующийДокумент = Параметры.Свойство("Ключ", ЭлектронныйДокумент) И ЗначениеЗаполнено(ЭлектронныйДокумент)
		И ОбщегоНазначения.СсылкаСуществует(ЭлектронныйДокумент);
	ЭтоСуществующийЛокальныйДокумент = ЭтоСуществующийДокумент И Не ЭлектронныеДокументыЭДО.ЭтоДокументОблачногоЭДО(
		ЭлектронныйДокумент);
	ЭтоСозданиеНовогоЛокальногоДокумента = Не ЭтоСуществующийДокумент
		И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоОблачныйЭДО",
	...
	Если ИспользуютсяЛегкиеКарточки Или ЭтоМобильныйКлиент Или Не (ЭтоСуществующийЛокальныйДокумент
		Или ЭтоСозданиеНовогоЛокальногоДокумента) Тогда
	...
	Если ВидФормы <> "ФормаОбъекта"
		Или ВариантИнтерфейса = ВариантыИнтерфейса.Расширенный Тогда
		Возврат;
	КонецЕсли;
	
	СтандартнаяОбработка = Ложь;
	
	ВыбраннаяФорма = ИнтерфейсДокументовЭДОКлиентСервер.ФормаПросмотраЭлектронногоДокументаПоВариантуИнтерфейса(
...
// Возвращаемое значение:
//  Булево
Функция ВключенаТранслитерацияИменФайлов() Экспорт
	
	ЭтоМобильныйКлиент = ОбщегоНазначения.ЭтоМобильныйКлиент();
	ИспользуютсяЛегкиеКарточки = ИспользуютсяЛегкиеКарточкиЭлектронныхДокументов();
	
	ВариантыИнтерфейса = ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО();
	Если ИспользуютсяЛегкиеКарточки Или ЭтоМобильныйКлиент Тогда
		ВариантИнтерфейса = ВариантыИнтерфейса.Легкий;
	Иначе
		ВариантИнтерфейса = ВариантыИнтерфейса.Расширенный;
	КонецЕсли;
	
	ФормаХраненияНастроек = ИнтерфейсДокументовЭДОКлиентСервер.ФормаПросмотраЭлектронногоДокументаПоВариантуИнтерфейса(
		ВариантИнтерфейса, Ложь);
		
	ОтключитьТранслитерацию = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(ФормаХраненияНастроек,
		"ОтключитьТранслитерацию", Ложь); // Булево
	
	Возврат Не ОтключитьТранслитерацию;
...
// 
// Возвращаемое значение:
//  См. ИнтерфейсДокументовЭДОИнтеграцияОблака.РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации
Функция РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации(Знач ДлительнаяОперация) Экспорт
	Возврат ИнтерфейсДокументовЭДОИнтеграцияОблака.РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации(
		ДлительнаяОперация);
КонецФункции
...
// 
// Возвращаемое значение:
//  См. ИнтерфейсДокументовЭДОИнтеграцияОблака.РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации
Функция РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации(Знач ДлительнаяОперация) Экспорт
	Возврат ИнтерфейсДокументовЭДОИнтеграцияОблака.РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации(
		ДлительнаяОперация);
КонецФункции
...
// См. ИнтерфейсДокументовЭДО.ПутьКФормеТекущихДелЭДО
...
// 
// Параметры:
//  ИспользуетсяПоказТрафикаЭлектронныхДокументов - Булево
Процедура СохранитьИспользованиеПоказаТрафикаЭлектронныхДокументовПоУмолчанию(ИспользуетсяПоказТрафикаЭлектронныхДокументов) Экспорт
	
	КлючОбъектаНастроек = ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО();
	КлючНастройки = ИнтерфейсДокументовЭДО.КлючНастройкиПоТекущемуТрафику();
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаНастроек, КлючНастройки, ИспользуетсяПоказТрафикаЭлектронныхДокументов);
...
// Возвращаемое значение:
//  Булево
Функция ТребуетсяВыборФайлаПередСозданиемНовогоПроизвольногоДокумента() Экспорт
	
	ВариантИнтерфейса = ИнтерфейсДокументовЭДО.ВариантИнтерфейсаДокументовЭДО();
	ВариантыИнтерфейса = ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО();
	
	Если ИспользуютсяЛегкиеКарточкиЭлектронныхДокументов()
		Или ВариантИнтерфейса = ВариантыИнтерфейса.ЛегкийКлиентЭДО
...
// Параметры:
//  ПомещенныйФайл - ОписаниеПомещенногоФайла - Помещенный файл
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
//  ПринудительнаяЗагрузка - Булево - Принудительная загрузка
// 
// Возвращаемое значение:
//  - Структура:
//   * Отказ - Булево
//   * ТекстОшибкиПревышенияРазмера - Строка
//   * ДлительнаяОперация - см. ДлительныеОперации.ВыполнитьФункцию
//                        - Неопределено
//
Функция ЗагрузитьФормализованныйДокументПоДаннымФайлаXML(ПомещенныйФайл, ЭлектронныйДокумент, 
		ПринудительнаяЗагрузка = Ложь) Экспорт
	Результат = Новый Структура;
	Результат.Вставить("Отказ", Ложь);
	Результат.Вставить("ТекстОшибкиПревышенияРазмера", "");
	...
	РезультатПроверки = ЭлектронныеДокументыЭДО.ПроверкаПревышенияМаксимальногоРазмераФайла(
		РеквизитыДокумента.ИдентификаторКонтрагента,
		РеквизитыДокумента.ИдентификаторОрганизации,
		ДвоичныеДанныеФайла);
	Если РезультатПроверки.Превышено Тогда
		...
		Результат.ТекстОшибкиПревышенияРазмера = 
			ПакетыДокументовЭДО.ТекстОПревышенииМаксимальногоРазмераПакетаПриСозданииДокумента(
				РезультатПроверки.ПревышаемыеОграничения);
		Возврат Результат;
	КонецЕсли;
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ИнтерфейсДокументовЭДО.ЗагрузитьФормализованныйДокументПоФайлуXMLВФоне", РеквизитыДокумента, ОписаниеФайла,
...
//  - Структура:
//   *Имя - Строка
//   *Хранение - Строка - адрес во временном хранилище
Функция ИсправитьИдФайла(ПомещенныйФайл, НовыйИдФайл) Экспорт
	ОписаниеФайла = РаботаСФайламиБЭД.ОписаниеФайлаПоДаннымПомещенногоФайла(ПомещенныйФайл);
	ОписаниеФайла = ФорматыЭДО.ИсправитьИдФайла(ОписаниеФайла, НовыйИдФайл);
	Результат = Новый Структура;
	Результат.Вставить("Имя", ОписаниеФайла.ИмяФайла);
	Результат.Вставить("Хранение", 
		ПоместитьВоВременноеХранилище(ОписаниеФайла.ДвоичныеДанные, ПомещенныйФайл.Хранение));
