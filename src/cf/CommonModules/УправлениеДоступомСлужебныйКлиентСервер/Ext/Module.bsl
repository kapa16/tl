...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма, ОбработкаНаКлиенте = Истина) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		// {УАТ}
		Если ТипЗнч(ТекущиеДанные.ВидДоступа) = Тип("СправочникСсылка.Пользователи")
			И Форма.Элементы.Найти("ТекстИнформацииОбычногоВидаДоступа") <> Неопределено Тогда
			Форма.Элементы.ТекстИнформацииОбычногоВидаДоступа.Заголовок = Нстр("ru = 'Список значений назначается в группах доступа.'; en = 'The list of values is assigned in access groups.'") +
				Символы.ПС + НСтр("ru = 'Ограничение доступа по Пользователям применяется к документам: Заказ на ТС, Заказ перевозчику, Маршрутный лист, Потребность в перевозке, Страховой сертификат, Счет на оплату покупателю, Счет на оплату поставщика, Реализация услуг, Поступление материалов и услуг.'; en = 'Access restrictions by users apply to the following documents: Vehicle Order, Carrier Order, Route Sheet, Transportation Request, Insurance Certificate, Buyer Invoice, Supplier Invoice, Service Delivery, and Receipt of Materials and Services.'")
		КонецЕсли;
		// {/УАТ}
