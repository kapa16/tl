...
// Параметры:
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
//   ПараметрыЗапроса - Структура, Соответствие из Произвольный - параметры,
...
//   См. РаспознаваниеДокументовHTTP.НовыйОтвет
//
Функция Get(URL, Знач ПараметрыЗапроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ПараметрыЗапроса) И СтрНайти(URL, "?") Тогда
		ВызватьИсключение НСтр("ru = 'Не реализовано. Поддерживается только URL без ?'; en = 'Not implemented. Only URL without ""?"" is supported'");
	КонецЕсли;
...
// Параметры:
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
//   Данные - Произвольный, Структура, Соответствие из КлючИЗначение - см. описание
...
//   См. РаспознаваниеДокументовHTTP.НовыйОтвет
//
Функция Post(URL, Знач Данные = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ТипЗнч(Данные) = Тип("Структура") Или ТипЗнч(Данные) = Тип("Соответствие") Тогда
		ВызватьИсключение НСтр("ru = 'Не реализовано'; en = 'Not implemented'");
	КонецЕсли;
...
//   См. РаспознаваниеДокументовHTTP.НовыйОтвет
//
Функция Put(URL, Знач Данные = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
//   См. РаспознаваниеДокументовHTTP.НовыйОтвет
//
Функция Delete(URL, Знач Данные = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
"%1'; en = 'An error occurred when deserializing the server response:"
"%1'"),
...
"%3"
"Ответ сервера:"
"%4'; en = 'An error occurred while executing the request:"
"%1 %2"
"%3"
"Server response:"
"%4'"),
...
//   * Продолжай_100 - Число
//   * ПереключениеПротокола_101 - Число
//   * ИдетОбработка_102 - Число
//   * РанняяМетаинформация_103 - Число
//   * ОК_200 - Число
//   * Создано_201 - Число
//   * Принято_202 - Число
//   * ИнформацияНеАвторитетна_203 - Число
//   * НетСодержимого_204 - Число
//   * СброситьСодержимое_205 - Число
//   * ЧастичноеСодержимое_206 - Число
//   * Многостатусный_207 - Число
//   * УжеСообщалось_208 - Число
//   * ИспользованоIM_226 - Число
//   * МножествоВыборов_300 - Число
//   * ПеремещеноНавсегда_301 - Число
//   * ПеремещеноВременно_302 - Число
//   * СмотретьДругое_303 - Число
//   * НеИзменялось_304 - Число
//   * ИспользоватьПрокси_305 - Число
//   * ВременноеПеренаправление_307 - Число
//   * ПостоянноеПеренаправление_308 - Число
//   * НеверныйЗапрос_400 - Число
//   * НеАвторизован_401 - Число
//   * НеобходимаОплата_402 - Число
//   * Запрещено_403 - Число
//   * НеНайдено_404 - Число
//   * МетодНеПоддерживается_405 - Число
//   * Неприемлемо_406 - Число
//   * НеобходимаАутентификацияПрокси_407 - Число
//   * ИстеклоВремяОжидания_408 - Число
//   * Конфликт_409 - Число
//   * Удален_410 - Число
//   * НеобходимаДлина_411 - Число
//   * УсловиеЛожно_412 - Число
//   * ПолезнаяНагрузкаСлишкомВелика_413 - Число
//   * СлишкомДлинныйURI_414 - Число
//   * НеподдерживаемыйТипДанных_415 - Число
//   * ДиапазонНеДостижим_416 - Число
//   * ОжиданиеНеУдалось_417 - Число
//   * ОшибкаПроверкиCSRF_419 - Число
//   * НеправильноНаправленныйЗапрос_421 - Число
//   * НеобрабатываемыйЭкземпляр_422 - Число
//   * Заблокировано_423 - Число
//   * НевыполненнаяЗависимость_424 - Число
//   * СлишкомРано_425 - Число
//   * НеобходимоОбновление_426 - Число
//   * НеобходимоПредусловие_428 - Число
//   * СлишкомМногоЗапросов_429 - Число
//   * ПоляЗаголовкаЗапросаСлишкомБольшие_431 - Число
//   * ПовторитьС_449 - Число
//   * НедоступноПоЮридическимПричинам_451 - Число
//   * КлиентЗакрылСоединение_499 - Число
//   * ВнутренняяОшибкаСервера_500 - Число
//   * НеРеализовано_501 - Число
//   * ОшибочныйШлюз_502 - Число
//   * СервисНедоступен_503 - Число
//   * ШлюзНеОтвечает_504 - Число
//   * ВерсияHTTPНеПоддерживается_505 - Число
//   * ВариантТожеПроводитСогласование_506 - Число
//   * ПереполнениеХранилища_507 - Число
//   * ОбнаруженоБесконечноеПеренаправление_508 - Число
//   * ИсчерпанаПропускнаяШиринаКанала_509 - Число
//   * НеРасширено_510 - Число
//   * ТребуетсяСетеваяАутентификация_511 - Число
//   * НеизвестнаяОшибка_520 - Число
//   * ВебСерверНеРаботает_521 - Число
//   * СоединениеНеОтвечает_522 - Число
//   * ИсточникНедоступен_523 - Число
//   * ВремяОжиданияИстекло_524 - Число
//   * КвитированиеSSНеУдалось_525 - Число
//   * НедействительныйСертификатSSL_526 - Число
//
...
//
// Параметры:
//   Объект - Произвольный, Структура, Соответствие Из КлючИЗначение - данные, которые необходимо преобразовать в JSON.
//
// Возвращаемое значение:
...
Функция ОбъектВJson(Объект) Экспорт
	...
	// {УАТ}
	ЗаписатьJSON(ЗаписьJSON, Объект, НастройкиСериализации, "ПреобразованиеJson", РаспознаваниеДокументовСериализацияСлужебный);
	// {/УАТ}
...
"%1 %2"
"Описание ошибки:"
"%3'; en = 'A network error occurred while executing the request:"
"%1 %2"
"Error details: "
"%3'"),
	...
	Если Результат.КодСостояния >= КодыСостоянияHTTP().ВнутренняяОшибкаСервера_500 Тогда
		...
		ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Попытка: %1'; en = 'Attempt: %1'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'РаспознаваниеДокументов.HTTP'; en = 'РаспознаваниеДокументов.HTTP'", ОбщегоНазначения.КодОсновногоЯзыка());
