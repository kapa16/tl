...
Процедура СоздатьДокументыКомплекта(Форма, ПараметрыСоздания, ОповещениеНаКлиенте)
	...
	Иначе
		...
		ТекстСоздан = НСтр("ru = 'Создан документ'; en = 'Document is created'");
		ТекстПрикреплен = НСтр("ru = 'Скан прикреплен'; en = 'Scanned copy is attached'");
...
Функция ТипДокументаИВидОперацииПоУмолчанию(Направление, ТипКомплекта, НаличиеТоваровУслуг = Неопределено) Экспорт
	...
	Иначе
		...
		СтрокаПоступленияРеализации = "";
		Если НаличиеТоваровУслуг <> Неопределено Тогда
			
			Если НаличиеТоваровУслуг.ЕстьТовары И НаличиеТоваровУслуг.ЕстьУслуги Тогда
				ПодстрокаПоиска = "Комиссия";
			ИначеЕсли НаличиеТоваровУслуг.ЕстьТовары Тогда
				ПодстрокаПоиска = "Товары";
			Иначе
				ПодстрокаПоиска = "Услуги";
			КонецЕсли;
			
			Для Каждого СтрокаКоманды Из ДоступныеКоманды Цикл
				Если СтрНайти(СтрокаКоманды, ПодстрокаПоиска) > 0 Тогда
					СтрокаПоступленияРеализации = СтрокаКоманды;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Если ПустаяСтрока(СтрокаПоступленияРеализации) Тогда
			СтрокаПоступленияРеализации = ДоступныеКоманды[0];
		КонецЕсли;
