...
Функция СтрокаBase64БезBOM(СтрокаДанных) Экспорт
	...
	ДвоичныеДанные = ПолучитьДвоичныеДанныеИзСтроки(СтрокаДанных, КодировкаТекста.UTF8, Ложь);
	Возврат РаботаСФайламиБЭД.Base64СтрокаИзДвоичныхДанных(ДвоичныеДанные);
...
#Область ПереходныеПроцедурыИФункцииБСП

// См.ОбщегоНазначения.ЭтоДобавлениеНабораЗаписей
...
// См.ОбщегоНазначения.ЭтоЗамещениеНабораЗаписей
...
Функция ЭтоУдалениеНабораЗаписей(НаборЗаписей, Режим) Экспорт
	...
	Возврат НаборЗаписей.Количество() = 0 Или ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Режим);
...
Процедура ЗаписатьВЖурналРегистрации(Комментарий, Подсистема, Уровень = Неопределено,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(Подсистема) Тогда
		ИменаПодсистем = СтрРазделить(Подсистема, ".", Ложь);
		ПолноеИмяПодсистемы = "";
		Для Каждого ИмяПодсистемы Из ИменаПодсистем Цикл
			ПолноеИмяПодсистемы = ?(ПустаяСтрока(ПолноеИмяПодсистемы), ПолноеИмяПодсистемы,
				ПолноеИмяПодсистемы + ".") + "Подсистема." + ИмяПодсистемы;
			...
			ПолноеИмяСобытия = ?(ПустаяСтрока(ПолноеИмяСобытия), ПолноеИмяСобытия,
				ПолноеИмяСобытия + ".") + ПредставлениеПодсистемы;
		КонецЦикла;
	КонецЕсли;
	
	Если ПустаяСтрока(ПолноеИмяСобытия) Тогда
		ПолноеИмяСобытия = НСтр("ru = 'Электронное взаимодействие.Общая подсистема'",
...
Функция ДатаВФорматеISO(Дата) Экспорт
	Возврат ЗаписатьДатуJSON(Дата, ФорматДатыJSON.ISO); 
КонецФункции

// Параметры:
//  ДатаВФорматеISO - Строка
// 
// Возвращаемое значение:
//  Дата
Функция ДатаИзФорматаISO(ДатаВФорматеISO) Экспорт
	Возврат ?(ЗначениеЗаполнено(ДатаВФорматеISO), ПрочитатьДатуJSON(ДатаВФорматеISO, ФорматДатыJSON.ISO), '00010101');
КонецФункции
...
//
Функция ПотокЗаписиСОтредактированнымОбъявлениемXML(Поток, Кодировка = "windows-1251") Экспорт
...
Функция ОбъектXDTOИзДанныхXML(Знач ДанныеXML, Знач ПространствоИмен, Знач ИмяТипа, Знач Кодировка = "windows-1251") Экспорт
	...
	ПотокЧтения = РаботаСФайламиБЭД.НовыйПотокЧтенияНормализованногоXML(ДанныеXML);
...
Процедура ДобавитьЗаполненноеЗначение(Набор, Значение) Экспорт
...
//        - Массив из Строка
//  Значение - Произвольный,Строка
Процедура ДобавитьУникальноеЗначение(Набор, Значение) Экспорт
...
// Параметры:
//  Множество1 - Массив из Произвольный
//  Множество2 - Массив из Произвольный
// 
// Возвращаемое значение:
//  Массив из Произвольный
Функция ПересечениеМассивов(Множество1, Множество2) Экспорт
	
	Результат = Новый Массив; // См. ПересечениеМассивов
...
// Записывает табличный документ в двоичные данные
// 
// Параметры:
//  ТабличныйДокумент - ТабличныйДокумент - Табличный документ, данные которого требуется получить
// 
// Возвращаемое значение:
//  ДвоичныеДанные - Данные табличного документа
Функция ДвоичныеДанныеТабличногоДокумента(Знач ТабличныйДокумент) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
		"ОбщегоНазначенияБЭД.ДвоичныеДанныеТабличногоДокумента", 
		"ТабличныйДокумент",
		ТабличныйДокумент,
	...
	ТабличныйДокумент.Записать(Поток);
...
//
Процедура УдалитьИзТаблицыСтрокиНеПодходящиеПоЗначениямВКолонке(Таблица, ИмяКолонки, Значения) Экспорт
	
	Если Таблица.Количество() > 0 Тогда
		КоличествоЭлементовКоллекции = Таблица.Количество();
		Для ОбратныйИндекс = 1 По КоличествоЭлементовКоллекции Цикл
			Индекс = КоличествоЭлементовКоллекции - ОбратныйИндекс;
			Если Значения.Найти(Таблица[Индекс][ИмяКолонки]) = Неопределено Тогда
...
Функция ИдентификаторПриложения() Экспорт
	...
	ИдентификаторПриложения = СтандартныеПодсистемыСервер.ИдентификаторИнформационнойБазы();
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
		НомерОбластиДанных = МодульРаботаВМоделиСервиса.ЗначениеРазделителяСеанса();
		ИдентификаторПриложения = ИдентификаторПриложения + "_" + Формат(НомерОбластиДанных, "ЧГ=;");
	КонецЕсли;
...
// Преобразует локальное строковое представление булево в значение
// 
// Параметры:
//  СтроковоеПредставление - Строка.
//
// Возвращаемое значение:
//  - Булево
//  - Неопределено
//
Функция БулевоИзСтроки(СтроковоеПредставление) Экспорт
	
	Попытка
	
		ЗначениеСтрокиВБулево = Булево(СтроковоеПредставление);
		Возврат ЗначениеСтрокиВБулево;
	
	Исключение
	
		ВозможныеЗначения =  Новый Соответствие();
		ВозможныеЗначения.Вставить("yes", Истина);
		ВозможныеЗначения.Вставить("no", Ложь);
		ВозможныеЗначения.Вставить("да", Истина);
		ВозможныеЗначения.Вставить("нет", Ложь);
		ВозможныеЗначения.Вставить("0", Ложь);
		ВозможныеЗначения.Вставить("1", Истина);
	
		ПредставлениеВНижнемРегистре = НРег(СтроковоеПредставление);
		ЗначениеСвойстваВБулево = ВозможныеЗначения[ПредставлениеВНижнемРегистре];
	
		Возврат ЗначениеСвойстваВБулево;
...
Процедура Пауза(Секунд) Экспорт
	...
	Если ФоновоеЗадание = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(
			...
			, 
			, 
			НСтр("ru = 'ОбщегоНазначенияБЭД.Пауза() разрешается использовать только в фоновом задании.'"));
		Параметры = Новый Массив;
		Параметры.Добавить(Секунд);
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбщегоНазначенияБЭД.Пауза", Параметры);
	КонецЕсли;
...
// Проверяет, что расширение файла входит в список допустимых, в противном случае вызывается исключение.
// При модификации метода в будущем не допускается удалять или заменять в расширении любые символы, кроме пробелов, 
//  так как это может привести к уязвимости.
// 
// Параметры:
//  РасширениеФайла - Строка - проверяемое расширение файла. Допускается передача с точкой и без. Пример "xml", ".xml".
//  ДопустимыеРасширения - Строка - допустимые расширения через запятую без точек.
//                                  Пример, "xml, json", неверно: ".xml, .json".
//                       - Массив из Строка - пример, ["xml", "json"], неверно: [".xml", ".json"].
//  КонтекстПроверки - Строка - например, имя процедуры или функции, в которой выполняется проверка.
//
Процедура ПроверитьРасширениеФайла(Знач РасширениеФайла, Знач ДопустимыеРасширения, Знач КонтекстПроверки = "") Экспорт
	
	ИмяМетода = "ОбщегоНазначенияБЭД.ПроверитьРасширениеФайла";
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяМетода, "РасширениеФайла", РасширениеФайла, Тип("Строка"));
	...
	Если ТипЗнч(ДопустимыеРасширения) = Тип("Строка") Тогда
		ДопустимыеРасширения = СтрРазделить(ДопустимыеРасширения, ",");
	КонецЕсли;
	
	Расширения = Новый Соответствие;
	Для Каждого ДопустимоеРасширение Из ДопустимыеРасширения Цикл
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(ИмяМетода,
			"ДопустимыеРасширения",
			ДопустимоеРасширение,
			Тип("Строка"));
		НовоеДопустимоеРасширение = НормализоватьРасширениеФайла(ДопустимоеРасширение);
		
		ОбщегоНазначенияКлиентСервер.Проверить(
			Не СтрНачинаетсяС(НовоеДопустимоеРасширение, "."),
			СтрШаблон(
				НСтр("ru='Недопустимое значение параметра %1: %2. Ожидаются значения без точки.'"), 
					"ДопустимыеРасширения",
					НовоеДопустимоеРасширение),
			ИмяМетода);
		
		Расширения.Вставить(НовоеДопустимоеРасширение, Истина);
		Расширения.Вставить("." + НовоеДопустимоеРасширение, Истина);
	КонецЦикла;
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		Расширения[НормализоватьРасширениеФайла(РасширениеФайла)],
		СтрШаблон(
			НСтр("ru='Недопустимое расширение файла %1. Ожидается: %2'"), 
				РасширениеФайла, 
...
Функция КоличествоОрганизаций() Экспорт
	...
	ИмяСправочникаОрганизации = ИмяПрикладногоСправочника("Организации");
	Если Не ЗначениеЗаполнено(ИмяСправочникаОрганизации) Тогда
		Возврат 0;
	КонецЕсли;
	
	МодульОрганизации = ОбщегоНазначения.ОбщийМодуль("Справочники." + ИмяСправочникаОрганизации);
	Если ОбщегоНазначения.ПодсистемаСуществует("КлиентЭДО") Тогда
		КоличествоОрганизаций = МодульОрганизации.КоличествоОрганизаций();
	Иначе
		ОписаниеЗапроса = НовоеОписаниеЗапроса();
		ТекстШаблона =
		"ВЫБРАТЬ
		...
		ОписаниеЗапроса.Текст = СтрЗаменить(ТекстШаблона, "&ИмяСправочникаОрганизации", "Справочник."
			+ ИмяСправочникаОрганизации);
		Результат = ВыполнитьЗапрос(ОписаниеЗапроса, Истина);
...
// Возвращаемое значение:
//  Булево
Функция ЭтоКППФилиалаИностраннойОрганизации(КПП) Экспорт
	
	КодыПричин = Новый Массив;
	КодыПричин.Добавить("51"); // постановка на учет отделения иностранной организации
	КодыПричин.Добавить("52"); // постановка на учет отделения иностранной организации в Российской Федерации,
	                           // созданной филиалом этой иностранной организации в иностранном государстве
	КодыПричин.Добавить("53"); // постановка на учет отделения иностранной организации,
	                           // созданной филиалом этой иностранной организации в Российской Федерации
	
	КодПричиныПостановкиНаУчет = Сред(КПП, 5, 2);
	
	Возврат КодыПричин.Найти(КодПричиныПостановкиНаУчет) <> Неопределено;
...
Функция ЗначениеФункциональнойОпции(НаименованиеФО) Экспорт
	...
	СоответствиеФО.Вставить("ИспользоватьЧерновикиЭлектронныхДокументов", "ИспользоватьЧерновикиЭлектронныхДокументов");
...
// 	Отличается от ОбщегоНазначения.ОписаниеТипаВсеСсылки отсутствием типов внешних источников данных.
//
// Возвращаемое значение:
...
#Область ПроверкиПараметровПереопределяемыхМетодов

// Метод предназначен для проверки корректности возвращаемый переопределяемыми методами параметров типа ТаблицаЗначений.
// 
// Параметры:
//  ПроверяемаяТаблица - ТаблицаЗначений
//  ЭталоннаяТаблица - ТаблицаЗначений
//  КонтекстПроверки - Строка - см. одноименный параметр ОбщегоНазначенияКлиентСервер.Проверить
//  ПроверятьТаблицуБезСтрок - Булево - по умолчанию Ложь
//
Процедура СтруктураТаблицыСоответствуетЭталонной(ПроверяемаяТаблица, ЭталоннаяТаблица, КонтекстПроверки = "",
	ПроверятьТаблицуБезСтрок = Ложь) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ОбщегоНазначенияБЭД.СтруктураТаблицыСоответствуетЭталонной",
		"ПроверяемаяТаблица", ПроверяемаяТаблица, Тип("ТаблицаЗначений"));
	
	Если Не ПроверятьТаблицуБезСтрок И ПроверяемаяТаблица.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Колонка Из ЭталоннаяТаблица.Колонки Цикл
		ОбщегоНазначенияКлиентСервер.Проверить(ПроверяемаяТаблица.Колонки.Найти(Колонка.Имя) <> Неопределено,
			СтрШаблон(НСтр("ru = 'Не найдена колонка %1'"), Колонка.Имя),
			КонтекстПроверки);
	КонецЦикла;
...
//  так как это может привести к уязвимости.
//
Функция НормализоватьРасширениеФайла(СтрокаПараметр)
	
	Возврат НРег(СокрЛП(СтрокаПараметр));
