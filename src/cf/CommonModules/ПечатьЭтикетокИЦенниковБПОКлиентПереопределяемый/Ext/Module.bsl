...
Процедура ОткрытьФормуПодбора(ПараметрыОткрытия) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначенияКлиент_проф.ОткрытьФормуПодбораПечатиЭтикеток(ПараметрыОткрытия);
	// {/УАТ}
...
// Переопределяет обработку оповещения формы.
//
// Параметры:
//  ИмяСобытия - Строка - имя события оповещения,
//  Параметр - Строка - параметр оповещения,
//  Источник - Строка - источник вызова оповещения,
