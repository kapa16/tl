...
// Устарела. Следует использовать методы из См. РаспознаваниеДокументовSDK которые бросают исключение с текстом ошибки.
//
...
Функция ВыполнитьКомандуСервисаАвторизации(КомандаСервиса)
	...
	Если РезультатКоманды = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка при обращении к сервису распознавания.'; en = 'An error occurred while accessing the recognition service.'");
	...
	Если РезультатКоманды.КодСостояния <> КодСостояния_200_ОК()
		...
		Если РезультатКоманды.КодСостояния = КодСостояния_400_BadRequest() Тогда
			ТекстОшибки = НСтр("ru = 'Неверный логин или пароль.'; en = 'Incorrect login or password.'");
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Ошибка при авторизации в сервисе распознавания.'; en = 'An error occurred upon authorization in the recognition service.'");
		КонецЕсли;
...
Функция ПередатьОбратнуюСвязь(ИдентификаторРезультата, Данные) Экспорт
	...
	Если РаспознаваниеДокументовСлужебный.ТребуетсяЗаблокироватьСервисРаспознавания() Тогда
		ВызватьИсключение НСтр("ru = 'Отправка обратной связи не удалась по причине блокировки сервиса распознавания.'; en = 'Cannot send the feedback as the recognition service is blocked.'");
	КонецЕсли;
...
Функция ПередатьОбратнуюСвязьОПереподключении(СтараяАвторизация, СтароеРазделение, НоваяАвторизация, НовоеРазделение, Авто, ТекстПользователя, КонтактПользователя) Экспорт
	...
	КомандаСервиса.Заголовки.Вставить("Content-Type", "application/json");
	КомандаСервиса.Заголовки.Вставить("Charset", "utf-8");
	КомандаСервиса.Данные = РаспознаваниеДокументовHTTP.ОбъектВJson(РезультатОбратнойСвязи);
...
Функция ЗарегистрироватьМобильноеПриложение(Имя) Экспорт
	...
	Если РезультатКоманды = Неопределено Или РезультатКоманды.КодСостояния <> КодСостояния_200_ОК() Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при регистрации мобильного приложения.'; en = 'An error occurred while registering the mobile application.'");
	КонецЕсли;
...
Процедура ОтключитьМобильноеПриложения(Идентификатор) Экспорт
	...
	Если РезультатКоманды = Неопределено Или РезультатКоманды.КодСостояния <> КодСостояния_200_ОК() Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при отключении мобильного приложения.'; en = 'An error occurred while disabling the mobile application.'");
	КонецЕсли;
...
Процедура ПереименоватьМобильноеПриложение(Идентификатор, Имя) Экспорт
	...
	Если РезультатКоманды = Неопределено Или РезультатКоманды.КодСостояния <> КодСостояния_200_ОК() Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при переименовании мобильного приложения.'; en = 'An error occurred while renaming the mobile application.'");
	КонецЕсли;
...
"Описание ошибки:"
"%2'; en = 'By URL query: %1 network error occurred "
"Error details:"
"%2'"),
...
"%4"
"Описание ошибки:"
"%5'; en = 'By URL query: %1 status code is received %2."
"Method: %3"
"Query body:"
"%4"
"Error details:"
...
"%1"
"Описание ошибки:"
"%2""'; en = 'Cannot read the JSON query body:"
