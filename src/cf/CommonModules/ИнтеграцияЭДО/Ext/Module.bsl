...
Процедура УстановитьСвязьЭлектронногоДокументаСОбъектомУчета(ЭлектронныйДокумент, ОбъектУчета, СпособОбработки = "") Экспорт
	...
	Если ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО") Тогда
		...
		Если Не Запрос.Выполнить().Пустой()
			...
			ОбщийМодульОтражениеВУчетеЭДО().ВыполнитьКонтрольПроведенияУчетныхДокументов(ЭлектронныйДокумент);
...
Процедура ОчиститьАктуальностьЭлектронногоДокумента(Знач ЭлектронныйДокумент) Экспорт
	...
	Попытка
		...
		Отказ = Ложь;
		ЗаблокироватьДанныеОбъектовУчетаДокументовЭДО(Неопределено, ЭлектронныйДокумент, Ложь, Отказ);
		Если Отказ Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось заблокировать сведения об объектах учета ЭДО'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция ОписаниеДанныхОбъектаУчета(ДанныеУчета, Параметры) Экспорт
	...
	ИначеЕсли Формат = Форматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
		
		Результат = УПД_5_03_ИнформацияПродавца_ДанныеУчета(ОбъектУчета, Параметры);
		
	ИначеЕсли Формат = Форматы.ФНС.УПД_5_03.ИнформацияПокупателя Тогда
		
		Результат = УПД_5_03_ИнформацияПокупателя_ДанныеУчета(Основания, Параметры);
	...
	ИначеЕсли Формат = Форматы.ФНС.СчетНаОплату_5_01 Тогда
		
		Результат = СчетНаОплату_5_01_ДанныеУчета(ОбъектУчета, Параметры);
	...
	ИначеЕсли ТрансграничныйЭДО.ЭтоИностранныйФормат(Формат) Тогда
		...
		Результат = ТрансграничныйЭДО.ОписаниеДанныхОбъектаУчета(ОбъектУчета, Параметры);
...
// Доступная функциональность формы "НастройкаОтправкиДокументов" РС "НастройкиОтправкиЭлектронныхДокументовПоВидам".
// 
// Возвращаемое значение:
//  см. ОбменСКонтрагентамиПереопределяемый.ПриОпределенииФункциональностиФормыНастройкиОтправкиДокументов.ДоступнаяФункциональность
//
Функция ФункциональностьФормыНастройкаОтправкиДокументов() Экспорт
	
	Функциональность = Новый Структура;
	Функциональность.Вставить("РасширенныйРежим", Истина);
	Функциональность.Вставить("НастройкаРегламента", Истина);
	Функциональность.Вставить("ИспользоватьДоговор", Истина);
	
	ОбменСКонтрагентамиПереопределяемый.ПриОпределенииФункциональностиФормыНастройкиОтправкиДокументов(Функциональность);
...
Процедура ПриОтраженииВУчетеЭлектронногоДокументаСНоменклатурой(ПараметрыОтражения, Отказ = Ложь, ОписаниеОшибки = "") Экспорт
	...
	ЗаполнитьНоменклатуруИБВДеревеДокумента(ПараметрыОтражения.Контрагент, ПараметрыОтражения.ДеревоДокумента,
		ПараметрыОтражения.ЭлектронныйДокумент, Настройки.ОтражатьТолькоСопоставленные, Отказ);
...
//  ОтражатьТолькоСопоставленные - Булево
//  Отказ - Булево
Процедура ЗаполнитьНоменклатуруИБВДеревеДокумента(Контрагент, ДеревоДокумента, ЭлектронныйДокумент,
	ОтражатьТолькоСопоставленные, Отказ) Экспорт
	...
	Если ОтражатьТолькоСопоставленные Тогда
		КонтрольСопоставленияНоменклатуры(НаСопоставление, СоответствиеНоменклатуры, ЭлектронныйДокумент, Отказ);
	КонецЕсли;
	...
	НачатьТранзакцию();
		Попытка
			...
			Для Каждого СсылкаНаОбъект Из ОбъектыУчета Цикл
				...
				ЗаписьРегистра.ПредставлениеСостояния = Строка(ЗаписьРегистра.СостояниеЭДО);
				ЗаписьРегистра.Комментарий = Комментарий;
				ЗаписьРегистра.Записать();
			КонецЦикла;
			
			Если НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов() Тогда
				РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.Удалить(СсылкаНаОбъект);
			КонецЕсли;
...
//
// Параметры:
//   РеквизитыКонтрагента - см. РеквизитыКонтрагента
//
//  Возвращаемое значение:
...
Функция СоздатьКонтрагента(РеквизитыКонтрагента) Экспорт
	...
	Реквизиты = РеквизитыКонтрагента(РеквизитыКонтрагента);
	
	ОбменСКонтрагентамиПереопределяемый.СоздатьКонтрагентаПоРеквизитам(Реквизиты, Контрагент, Отказ);
...
// 
// Возвращаемое значение:
//  См. КлючевыеРеквизитыУчастникаЭДО
Функция КлючевыеРеквизитыКонтрагента(Контрагент) Экспорт
	...
	Возврат КлючевыеРеквизитыУчастникаЭДО(Контрагент, ИмяРеквизитаИНН, ИмяРеквизитаКПП);
КонецФункции
...
//  * ИНН - Строка
//  * КПП - Строка
Функция КлючевыеРеквизитыКонтрагентов(Контрагенты) Экспорт
	Возврат КлючевыеРеквизитыУчастниковЭДО(Контрагенты, "Контрагенты");
КонецФункции

// Параметры:
//  КонтрагентыПоКлючам - См. КлючевыеРеквизитыКонтрагентов
//  КлючКонтрагента - Структура:
//  * ИНН - Строка
//  * КПП - Строка
// 
// Возвращаемое значение:
...
Функция КонтрагентИзОтбораПоКлючевымРеквизитам(КонтрагентыПоКлючам, КлючКонтрагента) Экспорт
	Возврат УчастникИзОтбораПоКлючевымРеквизитам(КонтрагентыПоКлючам, КлючКонтрагента);
КонецФункции

// Параметры:
//  КлючевыеРеквизитыКонтрагентов - Массив из см. НовыеКлючевыеРеквизитыУчастникаЭДО
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//  * Ссылка - ОпределяемыйТип.КонтрагентБЭД
//  * ИНН - Строка
//  * КПП - Строка
Функция КонтрагентыПоКлючевымРеквизитам(КлючевыеРеквизитыКонтрагентов) Экспорт
	Возврат УчастникиЭДОПоКлючевымРеквизитам(КлючевыеРеквизитыКонтрагентов, "Контрагенты");
КонецФункции
...
Функция ИспользуемыеТипыДокументов() Экспорт
	...
	ТрансграничныйЭДО.ПриОпределенииИспользуемыхТиповЭлектронныхДокументов(АктуальныеТипыДокументов);
...
Функция СписокОперацийВсехТиповДокумента() Экспорт 
	...
	ОбменСКонтрагентамиПереопределяемый.СписокОперацийВсехТиповДокумента(СписокОпераций, СтандартнаяОбработка);
...
// Возвращает натуральный ключ организации.
// 
// Параметры:
//  Организация - ОпределяемыйТип.Организация
// 
// Возвращаемое значение:
//  См. КлючевыеРеквизитыУчастникаЭДО
Функция КлючевыеРеквизитыОрганизации(Организация) Экспорт
	...
	Возврат КлючевыеРеквизитыУчастникаЭДО(Организация, ИмяРеквизитаИНН, ИмяРеквизитаКПП);
КонецФункции
...
// Возвращаемое значение:
//  ТаблицаЗначений:
//  * Ссылка - ОпределяемыйТип.Организация
//  * ИНН - Строка
//  * КПП - Строка
Функция КлючевыеРеквизитыОрганизаций(Организации) Экспорт
	Возврат КлючевыеРеквизитыУчастниковЭДО(Организации, "Организации");
КонецФункции

// Параметры:
//  ОрганизацииПоКлючам - См. КлючевыеРеквизитыОрганизаций
//  КлючОрганизации - Структура:
//  * ИНН - Строка
//  * КПП - Строка
// 
// Возвращаемое значение:
//  - Неопределено
//  - ОпределяемыйТип.Организация
Функция ОрганизацияИзОтбораПоКлючевымРеквизитам(ОрганизацииПоКлючам, КлючОрганизации) Экспорт
	Возврат УчастникИзОтбораПоКлючевымРеквизитам(ОрганизацииПоКлючам, КлючОрганизации);
КонецФункции

// Параметры:
//  КлючевыеРеквизитыОрганизации - См. КлючевыеРеквизитыОрганизации
// 
// Возвращаемое значение:
//  Неопределено,ОпределяемыйТип.Организация
Функция ОрганизацияПоКлючевымРеквизитам(КлючевыеРеквизитыОрганизации) Экспорт // АПК:299 - Отключение проверки отсутствия вызовов
	Возврат СсылкаНаОбъектПоИННКПП("Организации", КлючевыеРеквизитыОрганизации.ИНН, КлючевыеРеквизитыОрганизации.КПП);
КонецФункции

// Параметры:
//  КлючевыеРеквизитыОрганизаций - Массив из см. НовыеКлючевыеРеквизитыУчастникаЭДО
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//  * Ссылка - ОпределяемыйТип.Организация
//  * ИНН - Строка
//  * КПП - Строка
Функция ОрганизацииПоКлючевымРеквизитам(КлючевыеРеквизитыОрганизаций) Экспорт
	Возврат УчастникиЭДОПоКлючевымРеквизитам(КлючевыеРеквизитыОрганизаций, "Организации");
КонецФункции
...
// * ИНН - Строка
// * КПП - Строка
Функция НовыеКлючевыеРеквизитыУчастникаЭДО() Экспорт
...
// Возвращает тип участника ЭДО
//
// Параметры:
//  ПроверяемоеЛицо - ОпределяемыйТип.УчастникЭДО
// 
// Возвращаемое значение:
//  Структура:
//   * ЭтоЮрЛицо - Булево
//   * ЭтоИП - Булево
//   * ЭтоФизЛицо - Булево
//   * ЭтоИностранноеЛицо - Булево
//
Функция ТипУчастникаЭДО(ПроверяемоеЛицо) Экспорт
	
	Признаки = Новый Структура("ЭтоЮрЛицо, ЭтоИП, ЭтоФизЛицо", Ложь, Ложь, Ложь);
	Признаки.Вставить("ЭтоИностранноеЛицо", Ложь);
	ОбменСКонтрагентамиПереопределяемый.ПриОпределенииТипаУчастникаЭДО(ПроверяемоеЛицо, Признаки);
...
Функция ЗначениеСтавкиНДСИзПеречисления(СтавкаНДС) Экспорт
	...
	ИначеЕсли ПредставлениеСтавки = "20" ИЛИ ПредставлениеСтавки = "20/120" Тогда
		Значение = 0.20;
	ИначеЕсли ПредставлениеСтавки = "22" ИЛИ ПредставлениеСтавки = "22/122" Тогда
		Значение = 0.22;
	ИначеЕсли ПредставлениеСтавки = "5" ИЛИ ПредставлениеСтавки = "5/105" Тогда
		Значение = 0.05;
	ИначеЕсли ПредставлениеСтавки = "7" ИЛИ ПредставлениеСтавки = "7/107" Тогда
		Значение = 0.07;
	ИначеЕсли ПредставлениеСтавки = "9,09" Тогда
		Значение = 0.0909;
	ИначеЕсли ПредставлениеСтавки = "16,67" Тогда
		Значение = 0.1667;
	ИначеЕсли ПредставлениеСтавки = "18,03" Тогда
		Значение = 0.1803;
	Иначе
		Значение = 0;
	КонецЕсли;
...
//  
// Возвращаемое значение:
//  см. ВизуализацияЭДОСлужебный.НовыйРезультатПредставленияДанныхСообщения
Функция ПредставлениеФайлаПрикладногоЭлектронногоДокумента(ДвоичныеДанные, ПараметрыЗаполнения) Экспорт
	...
	РезультатФормирования = ВизуализацияЭДОСлужебный.НовыйРезультатПредставленияДанныхСообщения();
...
// 
// Возвращаемое значение:
//  ОпределяемыйТип.Организация, ОпределяемыйТип.КонтрагентБЭД, Неопределено -
//
Функция НайтиКонтрагентаПоРеквизитам(ТипОбъекта, РеквизитыКонтрагента, ДатаСведений = Неопределено) Экспорт
	
	Реквизиты = РеквизитыКонтрагента(РеквизитыКонтрагента);
	
	Если ЗначениеЗаполнено(СтрЗаменить(Реквизиты.ИНН, "0", "")) Тогда
		Возврат СсылкаНаОбъектПоИННКПП(ТипОбъекта, Реквизиты.ИНН, Реквизиты.КПП, ДатаСведений);
	КонецЕсли;
	
	Контрагент = Неопределено;
	ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект(ТипОбъекта, Контрагент, , Реквизиты);
	Возврат Контрагент;
...
Процедура ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ОбщийМодульОтражениеВУчетеЭДО().ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, КонтекстДиагностики);
	КонецЕсли;
	
	ОбменСКонтрагентамиПереопределяемый.ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент.ЭлектронныйДокумент);
...
Процедура ПриУстановкеПометкиУдаленияДокумента(ЭлектронныйДокумент, ПометкаУдаления, Отказ) Экспорт
	...
	Если НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов()
		И ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(ЭлектронныйДокумент) Тогда
		Возврат;
	КонецЕсли;
...
Функция АдресСоответствуетСтруктурированномуФорматуФНС(Адрес) Экспорт
	...
	Если Адрес.ТипАдреса <> "Административно-территориальный"
		...
		Или Не ВозможноКорректноеЗаполнениеСтруктурыАдресаФНС(Адрес) Тогда
...
//
Функция АдресСоответствуетФорматуГАР(Адрес) Экспорт // АПК:581 - Избыточное ключевое слово "Экспорт" (581)

	Возврат Адрес.ТипАдреса = "Муниципальный"
...
Функция ПриВыводеНавигационнойСсылкиВФормеОбъектаИБ(ДополнительнаяИнформация, МассивСсылок) Экспорт
	...
	Если ВыводитьНавигационнуюСсылку Тогда
		...
		Если ЗначениеЗаполнено(ТекстНавигационнойСсылки) Тогда
			ШаблонНавигационнойСсылки = НСтр("ru = '<a href = ""%1"">%2</a>'");
			ДополнительнаяИнформация.Текст = СтрШаблон(ШаблонНавигационнойСсылки, "Реклама1СЭДО", ТекстНавигационнойСсылки);
...
// Возвращаемое значение:
//  Неопределено,СправочникСсылка.Пользователи
Функция ОтветственныйВходящегоДокументаЭДО(Организация, Контрагент, Договор) Экспорт
	...
	Если АвтоматическоеПолучениеЭДО.Использовать() Тогда
		Ответственный = Пользователи.СсылкаНеуказанногоПользователя();
	Иначе
		Ответственный = Пользователи.ТекущийПользователь();
	КонецЕсли;
...
// Возвращаемое значение:
//  Произвольный
Функция ПустаяСсылкаДоговора() Экспорт // АПК:299 - Отключение проверки отсутствия вызовов
...
// См. ОбменСКонтрагентами.ДоступноВнесениеИзмененийВДанныеЭлектронногоДокументаПоОбъектамУчета
//
Функция ДоступноВнесениеИзмененийВДанныеЭлектронногоДокументаПоВсемОбъектамУчета(ОбъектыУчета) Экспорт
	
	ДанныеЭлектронныхДокументов = АктуальныеЭлектронныеДокументы(ОбъектыУчета);
	Если Не ЗначениеЗаполнено(ДанныеЭлектронныхДокументов) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ЭлектронныеДокументы = ДанныеЭлектронныхДокументов.ВыгрузитьКолонку("ЭлектронныйДокумент");
	СостоянияЭлектронныхДокументов = ЭлектронныеДокументыЭДО.СостоянияДокументов(ЭлектронныеДокументы);
	Для Каждого ДанныеПоСостоянию Из СостоянияЭлектронныхДокументов Цикл
		Если Не ЭлектронныеДокументыЭДО.ДоступноПереформированиеПоСостояниюЭлектронногоДокумента(
			ДанныеПоСостоянию.Значение) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
// Рассчитывает признак не в совокупности на все множество объектов учета, а в разрезе каждого объекта учета индивидуально.
// 
// Параметры:
//  ОбъектыУчета - Массив Из ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
// 
...
//  * Значение - Булево
Функция ДоступноВнесениеИзмененийВДанныеЭлектронногоДокументаПоОбъектамУчета(ОбъектыУчета) Экспорт
	Результат = Новый Соответствие;
	Для Каждого Ссылка Из ОбъектыУчета Цикл
	...
	Для Каждого ДанныеПоСостоянию Из СостоянияЭлектронныхДокументов Цикл
		Если Не ЭлектронныеДокументыЭДО.ДоступноПереформированиеПоСостояниюЭлектронногоДокумента(
			...
			НайденныеСтроки = ДанныеЭлектронныхДокументов.НайтиСтроки(Новый Структура("ЭлектронныйДокумент", ДанныеПоСостоянию.Ключ));
			Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
				Результат.Вставить(СтрокаТаблицы.ОбъектУчета, Ложь);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
// См. ОбменСКонтрагентами.ТребуетсяФормированиеСчетаФактурыПоОбъектуУчета
//
Функция ТребуетсяФормированиеСчетаФактурыПоОбъектуУчета(ОбъектУчета) Экспорт
	
	Результат = Ложь;
	
	ОписанияОбъектаУчета = ОписаниеОбъектаУчета(ОбъектУчета);
	Если Не ЗначениеЗаполнено(ОписанияОбъектаУчета) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ОписаниеОбъектаУчета = ОписанияОбъектаУчета[0];
	
	ИспользуетсяУПД_УКД = НастройкиЭДО.ИспользуетсяУПД_УКД(ОписаниеОбъектаУчета.Организация,
		ОписаниеОбъектаУчета.Контрагент, ОписаниеОбъектаУчета.Договор);
		
	Если ИспользуетсяУПД_УКД.ИспользуетсяУПД
		И (ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.СведенияОЗакупкеКомиссионером
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.СведенияОРеализацииКомиссионером) Тогда
		Результат = Истина;
	КонецЕсли;
	
	Если ИспользуетсяУПД_УКД.ИспользуетсяУКД
		И (ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОЗакупкеКомиссионером
		Или ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОРеализацииКомиссионером) Тогда
		Результат = Истина;
	КонецЕсли;
...
//  ПометкаУдаления - Булево
//  ОбновлятьЧерновик - Булево
Процедура ИзменитьПометкуУдаленияЧерновикаЭлектронногоДокумента(СсылкаНаОбъект, ПометкаУдаления,
	ОбновлятьЧерновик = Истина) Экспорт
	
	НачатьТранзакцию();
	Попытка
		ОбщегоНазначенияБЭД.УстановитьУправляемуюБлокировкуПоСсылке(СсылкаНаОбъект);
	
		ЧерновикОбъект = СсылкаНаОбъект.ПолучитьОбъект();
		ЧерновикОбъект.ПометкаУдаления = ПометкаУдаления;
		Если Не ЭлектронныеДокументыЭДО.ЭтоЭлектронныйДокумент(СсылкаНаОбъект) Тогда
			ЧерновикОбъект.Проведен = Не ПометкаУдаления;
		КонецЕсли;
		ЧерновикОбъект.Записать();
		
		Если ОбновлятьЧерновик Тогда
			УдалитьОбновитьЧерновик(СсылкаНаОбъект, ПометкаУдаления);
		КонецЕсли;
		
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Изменение пометки удаления черновика электронного документа'");
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки); // АПК:157 -не задан основной язык события для ИмяСобытия
	КонецПопытки;
...
//  СсылкаНаОбъект - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  Удалить - Булево
Процедура УдалитьОбновитьЧерновик(СсылкаНаОбъект, Удалить) Экспорт
	
	Если Удалить Тогда
		РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.Удалить(СсылкаНаОбъект);
	Иначе
		ОбновитьЧерновикПоОбъектуУчета(СсылкаНаОбъект);
		ОбновитьСостояниеЭДООбъектаУчета(СсылкаНаОбъект);
	КонецЕсли;
...
Функция НовыеСвойстваАктуальногоТипаДокумента() Экспорт
...
Процедура ПередЗаписьюОбъектаУчетаДокументаЭДО(Источник, Отказ, ЗаписьЭлементаСправочника = Ложь)
	...
	Если ЕстьНаправлениеЭДОВнутренний
		...
		И ДокументооборотНастроен
		И НЕ ОписаниеИзменений.СостояниеЭлектронногоДокумента = Перечисления.СостоянияДокументовЭДО.НеСформирован Тогда
...
//   * ИспользуетсяПереформирование - Булево - если установлено в Истина и исходящий электронный документ 
//                                             ещё не подписан - внесение изменений в данные электронного, 
//                                             при изменении ключевых данных в объекте учета,
//                                             будет выполнено через переформирование. В противном случае
//                                             изменения будут внесены через принудительное закрытие старого 
//                                             электронного документа и создание нового.
//
Функция НовоеОписаниеИзмененийОснованияЭлектронногоДокумента()
	...
	ОписаниеИзменений.Вставить("ИспользуетсяПереформирование", Ложь);
...
Процедура ПриИзмененииОснованияЭлектронногоДокумента(Знач ОбъектУчета, Знач ОписаниеИзменений)
	...
	Если ОписаниеИзменений.УчаствуетВОбмене Тогда
		...
		ИначеЕсли ОписаниеИзменений.ИзмененыКлючевыеДанные Тогда
			...
			Если ОписаниеИзменений.ИспользуетсяПереформирование
				И ЭлектронныеДокументыЭДО.ДоступноПереформированиеПоСостояниюЭлектронногоДокумента(
				ОписаниеИзменений.СостояниеЭлектронногоДокумента) Тогда
				ПереформироватьЭлектронныйДокумент(ОбъектУчета);
				Если НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов() Тогда
					ОбновитьЧерновикПоОбъектуУчета(ОбъектУчета);
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
// Параметры:
//  ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовДокументОбъектЭДО
Процедура ОбновитьЧерновикПоОбъектуУчета(ОбъектУчета)
	
	Описание = ОписаниеОснованияЭлектронногоДокумента(ОбъектУчета.Ссылка);
	ОписаниеЧерновика = РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.НовоеОписаниеЧерновика();
	ЗаполнитьЗначенияСвойств(ОписаниеЧерновика, Описание);
	
	НачатьТранзакцию();
	Попытка
		РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.Записать(ОбъектУчета.Ссылка,, ОписаниеЧерновика);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Запись черновика электронного документа'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		ВызватьИсключение ТекстОшибки;
...
Процедура ЗаписатьАктуальныйЭлектронныйДокумент(Знач ОбъектУчета, Знач ЭлектронныйДокумент)
	...
	Попытка
		...
		Если НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов() Тогда
			РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.Удалить(ОбъектУчета);
		КонецЕсли;
...
Процедура ОчиститьАктуальныйЭлектронныйДокумент(Знач ОбъектУчета, Знач ЭлектронныйДокумент = Неопределено)
	...
	Попытка
		...
		Отказ = Ложь;
		ЗаблокироватьДанныеОбъектовУчетаДокументовЭДО(ОбъектУчета, ЭлектронныйДокумент, Ложь, Отказ);
		Если Отказ Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось заблокировать сведения об объектах учета ЭДО'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ВызватьИсключение ТекстОшибки;
...
Процедура ЗаписатьОснованиеНовогоЭлектронногоДокумента(ОбъектУчета, Знач Описание)
	...
	Попытка
		...
		Если НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов() Тогда
			ОписаниеЧерновика = РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.НовоеОписаниеЧерновика();
			ЗаполнитьЗначенияСвойств(ОписаниеЧерновика, Описание);
			РегистрыСведений.ЧерновикиЭлектронныхДокументовЛегкийКлиентЭДО.Записать(ОбъектУчета,, ОписаниеЧерновика);
		КонецЕсли;
...
Процедура ПереформироватьЭлектронныйДокумент(Источник)
	
	АктуальныеЭлектронныеДокументы = 
		АктуальныеЭлектронныеДокументы(Источник.Ссылка).ВыгрузитьКолонку("ЭлектронныйДокумент");
	...
	РезультатыПоИнтеграциямЭДО = ЭлектронныеДокументыЭДО.ВыполнитьДействияПоИнтеграциямЭДО(ПараметрыВыполнения);
	РезультатДействийПоЭДО = ЭлектронныеДокументыЭДО.ОбработатьРезультатыВыполненияДействийПоИнтеграциямЭДО(
		РезультатыПоИнтеграциямЭДО);
	
	Для Каждого ОбработанныйДокумент Из РезультатДействийПоЭДО.ИтогВыполненияДействий.ОбработанныеДокументы Цикл
		ЗаписатьАктуальныйЭлектронныйДокумент(Источник.Ссылка, ОбработанныйДокумент.Ключ);
	КонецЦикла;
	...
	Для каждого ЭлектронныйДокумент Из АктуальныеЭлектронныеДокументы Цикл
		...
		ИначеЕсли СостоянияМожноЗакрывать.Найти(ТекущееСостояние) <> Неопределено Тогда
			...
			Попытка
				...
				ОчиститьАктуальныйЭлектронныйДокумент(Источник.Ссылка, ЭлектронныйДокумент);
				ЭлектронныеДокументыЭДО.ВыполнитьДействияПоИнтеграциямЭДО(ПараметрыВыполненияДействийПоЭДО);
				ЗафиксироватьТранзакцию();
			Исключение
				ОтменитьТранзакцию();
				ВызватьИсключение;
...
Процедура ДобавитьСведенияОВыбытииМаркированныхТоваров(Данные, ТекстоваяИнформация, Формат = "")
	...
	Если Формат = ПоддерживаемыеФорматы.УПД_5_02.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца Тогда
...
// Параметры:
//  СтруктурированнаяИнформацияОбОшибке - ИнформацияОбОшибке
Процедура ЗаписатьОшибкуФормированияОтветногоТитулаВЖурналРегистрации(СтруктурированнаяИнформацияОбОшибке)
		
	Шаблон = НСтр(
		"ru = 'Не удалось сформировать ответный титул, по причине:
		|%1'");
	ТекстСообщения = СтрШаблон(Шаблон, ОбработкаОшибок.ПодробноеПредставлениеОшибки(СтруктурированнаяИнформацияОбОшибке));
...
Процедура ЗаполнитьАдресаУчастниковСделкиУПД2019(Участники, ДеревоДанных, ДатаАдреса = Неопределено)
	...
	Для Каждого Участник Из Участники Цикл
		...
		Если ЗначениеЗаполнено(КонтактнаяИнформация) Тогда
			...
			АдресЗначение = КонтактнаяИнформация[0].Значение;
			ОтборДляПолученияАдресаПоПриказуМинфина = Новый Структура("КодыАдреса, БезПредставлений", Ложь, Истина);
			...
			Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
				...
				Если АдресСоответствуетСтруктурированномуФорматуФНС Тогда
					...
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
						ДеревоДанных, ПутьКАдресу + ПолеКодРегиона, АдресПоПриказуМинфина.КодРегиона,
						ПараметрыОшибкиЗаполненияКодаРегиона(ПараметрыОбработкиОшибок, ОбъектКонтактнойИнформации));
				...
				Иначе
					...
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
							ДеревоДанных, ПутьКАдресу + ".КодСтраны", АдресСПредставлениями.КодСтраны, ПараметрыОбработкиОшибок);

					Если АдресСПредставлениями.ТипАдреса = "Муниципальный" Тогда

						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
								ДеревоДанных, ПутьКАдресу + ".АдресТекст", АдресСПредставлениями.МуниципальноеПредставление,
							ПараметрыОбработкиОшибок);
					Иначе
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
								ДеревоДанных, ПутьКАдресу + ".АдресТекст", АдресСПредставлениями.Представление,
							ПараметрыОбработкиОшибок);
			...
			Иначе
				...
				ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
					ДеревоДанных, ПутьКАдресу + ПолеКодСтраны, АдресСПредставлениями.КодСтраны, ПараметрыОбработкиОшибок);
				
				ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
					ДеревоДанных, ПутьКАдресу + ПолеАдресТекст, АдресСПредставлениями.Представление, ПараметрыОбработкиОшибок);
	...
	Для Каждого КлючЗначение Из Участники Цикл
		...
		Если СтрокаДерева.Признак = "Таблица" Тогда
			...
			Для Каждого СтрокаТаблицы Из СтрокаДерева.Строки Цикл
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
					МодульЭлектронноеАктированиеЕИС = ОбщегоНазначения.ОбщийМодуль("ЭлектронноеАктированиеЕИС");
					МодульЭлектронноеАктированиеЕИС.ДополнитьПризнакДанныеЭлектронногоАктированияЕИСДляУчастника(
						Данные, Путь);
				КонецЕсли;
	...
	Для Каждого Участник Из Участники Цикл
		...
		Если ЗначениеЗаполнено(КонтактнаяИнформация) Тогда
			...
			Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС") Тогда
					МодульЭлектронноеАктированиеЕИС = ОбщегоНазначения.ОбщийМодуль("ЭлектронноеАктированиеЕИС");
					МодульЭлектронноеАктированиеЕИС.АдресСоответствуетСтруктурированномуФорматуФНС(
						АдресСоответствуетСтруктурированномуФорматуФНС, ДеревоДанных);
				КонецЕсли;
				...
				Если АдресСоответствуетСтруктурированномуФорматуФНС Тогда
					...
					НаименованиеРегиона = ПредставлениеРегиона(
						АдресПоПриказуМинфина.Регион, АдресПоПриказуМинфина.РегионТипКраткий, АдресПоПриказуМинфина.КодРегиона);
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
						ДеревоДанных, ПутьКАдресу + ".НаименованиеРегиона", НаименованиеРегиона,
				...
				ИначеЕсли АдресСоответствуетФорматуГАР(АдресПоПриказуМинфина) Тогда
					...
					УникальныйНомерАдреса = Строка(АдресПоПриказуМинфина.ИдентификаторАдресногоОбъекта);
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.ИдентификаторДома) Тогда
						УникальныйНомерАдреса = Строка(АдресПоПриказуМинфина.ИдентификаторДома);
					КонецЕсли;
					
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
						ПутьКАдресу + ".УникальныйНомерАдреса",
						УникальныйНомерАдреса, ПараметрыОбработкиОшибок);
					...
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
						...
						НаименованиеРегиона, ПараметрыОбработкиОшибок);
					
					// Район, округ, территория.
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.МуниципальныйРайон) Тогда
						
						КодыБСП = ИнтеграцияБСПБЭД.КодыМуниципальныхРайонов();
						...
						Вид = "";
						Если АдресПоПриказуМинфина.КодМуниципальногоРайона = КодыБСП.МуниципальныйРайон Тогда
							Вид = Виды.МуниципальныйРайон;
						ИначеЕсли АдресПоПриказуМинфина.КодМуниципальногоРайона = КодыБСП.ГородскойОкруг Тогда
							Вид = Виды.ГородскойОкруг;
						ИначеЕсли АдресПоПриказуМинфина.КодМуниципальногоРайона = КодыБСП.ВнутригородскаяТерритория Тогда
							Вид = Виды.ВнутригородскаяТерритория;
						ИначеЕсли АдресПоПриказуМинфина.КодМуниципальногоРайона = КодыБСП.МуниципальныйОкруг Тогда
							Вид = Виды.МуниципальныйОкруг;
						ИначеЕсли АдресПоПриказуМинфина.КодМуниципальногоРайона = КодыБСП.ФедеральнаяТерритория Тогда
							Вид = Виды.ФедеральнаяТерритория;	
						КонецЕсли;
						
						ПараметрыОбработкиОшибокВида = ЭлектронноеВзаимодействиеКлиентСервер.НовыеПараметрыОбработкиОшибки();
						Если ПустаяСтрока(Вид) Тогда
							ПараметрыОбработкиОшибокВида.ТекстОшибки =
								СтрШаблон(НСтр("ru = 'Не удалось определить вид муниципального района ""%1"".
									|Проверьте правильность адреса или попробуйте ввести его в другом формате.'"),
								АдресПоПриказуМинфина.МуниципальныйРайон);
						КонецЕсли;
						
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".РайонОкругТерритория.Вид",
							Вид, ПараметрыОбработкиОшибокВида);
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
					...
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.Поселение) Тогда
						...
						КодыБСП = ИнтеграцияБСПБЭД.КодыПоселений();
						...
						Вид = "";
						Если АдресПоПриказуМинфина.КодПоселения = КодыБСП.ГородскоеПоселение Тогда
							Вид = Виды.ГородскоеПоселение;
						ИначеЕсли АдресПоПриказуМинфина.КодПоселения = КодыБСП.СельскоеПоселение Тогда	
							Вид = Виды.СельскоеПоселение;
						ИначеЕсли АдресПоПриказуМинфина.КодПоселения = КодыБСП.МежселеннаяТерритория Тогда	
							Вид = Виды.МежселеннаяТерритория;
						ИначеЕсли АдресПоПриказуМинфина.КодПоселения = КодыБСП.ВнутригородскойРайон Тогда	
						...
						Если ПустаяСтрока(Вид) Тогда
							ПараметрыОбработкиОшибокВида.ТекстОшибки =
								СтрШаблон(НСтр("ru = 'Не удалось определить вид поселения ""%1"".
									|Проверьте правильность адреса или попробуйте ввести его в другом формате.'"),
					...
					ИначеЕсли ЗначениеЗаполнено(АдресПоПриказуМинфина.Город) Тогда
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".НаселенныйПункт.Вид",
							НРег(АдресПоПриказуМинфина.ГородТипКраткий), ПараметрыОбработкиОшибок);
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".НаселенныйПункт.Наименование",
							АдресПоПриказуМинфина.Город, ПараметрыОбработкиОшибок);
					КонецЕсли;
...
// 	Данные - ДеревоЗначений
//
Процедура СчетНаОплату_5_01_АвтозаполнениеАдресов(Данные)
	
	ДатаСведений = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Данные, "ДатаДокумента", Ложь);
	
	Участники = Новый Соответствие;
	Участники.Вставить("ПолучательДенежныхСредств");
	Участники.Вставить("Плательщик");
	Участники.Вставить("Продавец");
	Участники.Вставить("Покупатель");
	Участники.Вставить("Грузоотправитель");
	...
	Для Каждого КлючЗначение Из Участники Цикл
		УчастникМассивом = Новый Массив;
		УчастникМассивом.Добавить(КлючЗначение.Ключ);
		ЗаполнитьАдресаУчастниковСделкиСчетНаОплату_5_01(УчастникМассивом, Данные, ДатаСведений);
	КонецЦикла;
...
//  Участники - Массив из Строка - участники сделки в дереве электронного документа.
//  ДеревоДанных - ДеревоЗначений - Дерево электронного документа.
//  								см. ИнтеграцияЭДО.СчетНаОплату_5_01_НовыеДанные.
//  ДатаАдреса - Дата - дата получения контактной информации. Если не задана, то используется текущая дата сеанса.
//
Процедура ЗаполнитьАдресаУчастниковСделкиСчетНаОплату_5_01(Участники, ДеревоДанных, ДатаАдреса = Неопределено)
		...
		Если ЗначениеЗаполнено(КонтактнаяИнформация) Тогда
			...
			Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
				...
				Если АдресСоответствуетСтруктурированномуФорматуФНС Тогда
					...
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
						ДеревоДанных, ПутьКАдресу + ".НаименованиеРегиона", АдресПоПриказуМинфина.Регион,
						ПараметрыОшибкиЗаполненияНаименованияРегиона(ПараметрыОбработкиОшибок, ОбъектКонтактнойИнформации));	
...
Функция КаталогТоваров_ДанныеУчета(ОбъектУчета, Параметры)
	...
	Результат.Формат = "КаталогТоваров";
...
Функция СчетНаОплату101_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	СчетНаОплату101_АвтозаполнениеАдресов(Данные);
	...
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Формат = "СчетНаОплату_1_01";
	Результат.Данные = Данные;
	Результат.Основания.Добавить(ОбъектУчета);
...
Процедура СчетНаОплату101_АвтозаполнениеАдресов(Данные)
	...
	Для Каждого КлючЗначение Из Участники Цикл
		...
		Если СтрокаДерева.Признак = "Таблица" Тогда
			...
			Для Каждого СтрокаТаблицы Из СтрокаДерева.Строки Цикл
				...
				УчастникМассивом = Новый Массив;
				УчастникМассивом.Добавить(Путь + ".НомерСтроки" + ?(ЗначениеЗаполнено(Колонка), "." + Колонка, ""));
				ЗаполнитьАдресаУчастниковСделкиСчетНаОплату101(УчастникМассивом, СтрокаТаблицы, ДатаСведений);
		...
		ИначеЕсли СтрокаДерева.Признак = "Группа" Тогда
			...
			УчастникМассивом = Новый Массив;
			УчастникМассивом.Добавить(Путь);
			ЗаполнитьАдресаУчастниковСделкиСчетНаОплату101(УчастникМассивом, Данные, ДатаСведений);
...
Процедура ЗаполнитьАдресаУчастниковСделкиСчетНаОплату101(Участники, ДеревоДанных, ДатаАдреса = Неопределено)
	
	ПутьККорнюАдреса = ".Адрес";
	ПутьКСтруктурированномуАдресу = ПутьККорнюАдреса + ".АдресРФ";
	ПутьКАдресуГАР = ПутьККорнюАдреса + ".АдресФИАС";
	ПутьКПроизвольномуАдресу = ПутьККорнюАдреса + ".АдресИнформация";
	ПутьКИностранномуАдресу = ПутьККорнюАдреса + ".АдресИнформация";	
		...
		Если ЗначениеЗаполнено(КонтактнаяИнформация) Тогда
			...
			Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
				...
				ИначеЕсли АдресСоответствуетФорматуГАР(АдресПоПриказуМинфина) Тогда
					...
					ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
						ПутьКАдресу + ".ИдентификаторОбъекта",
					...
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.МуниципальныйРайон) Тогда
						...
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".МуниципальныйРайон.Вид",
							Вид, ПараметрыОбработкиОшибок);
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".МуниципальныйРайон.Наименование",
							АдресПоПриказуМинфина.МуниципальныйРайон, ПараметрыОбработкиОшибок);
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.Поселение) Тогда
						...
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".ГородскоеПоселение.Вид",
							Вид, ПараметрыОбработкиОшибок);
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".ГородскоеПоселение.Наименование",
							АдресПоПриказуМинфина.Поселение, ПараметрыОбработкиОшибок);	
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(АдресПоПриказуМинфина.Здание.Номер) Тогда
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".ЗданиеСооружение.Тип",
							НРег(АдресПоПриказуМинфина.Здание.ТипЗдания), ПараметрыОбработкиОшибок);
						ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
							ПутьКАдресу + ".ЗданиеСооружение.Номер",
							АдресПоПриказуМинфина.Здание.Номер, ПараметрыОбработкиОшибок);
					КонецЕсли;
...
Функция СчетНаОплату_5_01_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	
	Настройки = СчетНаОплату_5_01_НастройкиЗаполнения(Параметры);
	Данные = СчетНаОплату_5_01_НовыеДанные();
	Описание = СчетНаОплату_5_01_Описание(Параметры);
	Отказ = Ложь;
	
	ОбменСКонтрагентамиПереопределяемый.ЗаполнитьДанныеПоСчетуНаОплату_5_01(
		ОбъектУчета, Настройки, Данные, Описание, Отказ);
	...
	СчетНаОплату_5_01_АвтозаполнениеАдресов(Данные);
	СчетНаОплату_5_01_ЗаполнитьНалоговуюСтавкуИзСоответствия(Данные);
	
	Если Параметры.ВыгружатьДополнительныеСведения Тогда
		СчетНаОплату_5_01_ДобавитьДополнительныеДанные(Данные);
	КонецЕсли;
	
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Формат = "СчетНаОплату_5_01";
	Результат.Данные = Данные;
	Результат.Основания.Добавить(ОбъектУчета);
	Для каждого Основание Из Описание.Основания Цикл
		Результат.Основания.Добавить(Основание);
...
Функция СчетНаОплату_5_01_НастройкиЗаполнения(Знач Параметры)
...
Функция СчетНаОплату_5_01_НовыеДанные()
	
	Данные = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента(
		"ОбменСКонтрагентами.СчетНаОплату_5_01");
	
	Возврат Данные;
	
КонецФункции

Функция СчетНаОплату_5_01_Описание(Знач Параметры)
...
Процедура СчетНаОплату_5_01_ДобавитьДополнительныеДанные(Данные)
...
Процедура СчетНаОплату_5_01_ЗаполнитьНалоговуюСтавкуИзСоответствия(Данные)
	
	СведенияОТоварах = ДеревоЭлектронногоДокументаБЭД.СтрокаДерева(Данные, "ТаблицаТоваров");
	
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		ЗаполнитьНалоговуюСтавкуИзСоответствия(СведенияОТоваре, "ТаблицаТоваров.НомерСтроки.НалоговаяСтавка.СтавкаНДС");
		ЗаполнитьНалоговуюСтавкуИзСоответствия(СведенияОТоваре, "ТаблицаТоваров.НомерСтроки.Сопоставление.СтавкаНДС");
	КонецЦикла;
...
Функция УПД2019_ИнформацияПокупателя_ДанныеУчета(Знач Основания, Знач Параметры)
	...
	Исключение
		...
		ЗаписатьОшибкуФормированияОтветногоТитулаВЖурналРегистрации(ИнформацияОбОшибке());
		Если Не ПолучитьФункциональнуюОпцию("ЗапретитьФормированиеОтветногоТитулаБезДокументовУчета") Тогда
			Результат.ОшибкаФормированияВПрикладнойЧасти = Истина;
			Возврат Результат;
		Иначе
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не удалось сформировать ответный титул. Подробности см. в журнале регистрации.'"));
		КонецЕсли;
...
Процедура УПД2019_ИнформацияПокупателя_ЗаполнитьДанныеРучногоФормирования(Данные, Знач СтруктураОтвета)
	...
	Если СтруктураОтвета.Свойство("СоставительДокументаДоверенность")
		И ЗначениеЗаполнено(СтруктураОтвета.СоставительДокументаДоверенность) Тогда
		ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
	...
	Если СтруктураОтвета.Свойство("ДокументОРасхождениях")
		И ЗначениеЗаполнено(СтруктураОтвета.ДокументОРасхождениях) Тогда
		ЭлектронноеВзаимодействие.ЗагрузитьСтруктуруВГруппуДерева(
			Данные, СтруктураОтвета.ДокументОРасхождениях, "СведенияОПринятииТоваров.ДокументОРасхождениях");			
	КонецЕсли;
		
	Если СтруктураОтвета.Свойство("ЗакупкаДляГосударственныхНужд")
		И ЗначениеЗаполнено(СтруктураОтвета.ЗакупкаДляГосударственныхНужд) Тогда
...
Процедура УПД_5_02_ИнформацияПродавца_ДобавитьДополнительныеДанные(Данные)
	...
	Если ЗначениеЗаполнено(ТолькоУслуги) Тогда
		НоваяСтрока = ТекстоваяИнформация.Добавить();
		НоваяСтрока.Идентификатор = "ТолькоУслуги";
		НоваяСтрока.Значение = XMLСтрока(ТолькоУслуги);
		ДеревоЭлектронногоДокументаБЭД.ДобавитьЗаписьВТаблицуДерева(Данные, НоваяСтрока, ИнформационноеПолеФХЖ1);
	КонецЕсли;
...
Процедура УПД_5_02_ИнформацияПокупателя_ЗаполнитьДанныеРучногоФормирования(Данные, Знач СтруктураОтвета)
	...
	Если СтруктураОтвета.Свойство("ОснованиеСоставителяФайла")
		И ЗначениеЗаполнено(СтруктураОтвета.ОснованиеСоставителяФайла) Тогда
		ЭлектронноеВзаимодействие.ЗагрузитьСтруктуруВГруппуДерева(Данные,
			СтруктураОтвета.ОснованиеСоставителяФайла, "ОснованиеСоставителяФайла");
	КонецЕсли;
...
#Область УПД_5_03_ИнформацияПродавца

Функция УПД_5_03_ИнформацияПродавца_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	
	Настройки = УПД_5_02_ИнформацияПродавца_НастройкиЗаполнения(Параметры);
	Данные = УПД_5_03_ИнформацияПродавца_НовыеДанные();
	Описание = УПД_5_02_ИнформацияПродавца_Описание(Параметры);
	Отказ = Ложь;
	
	ОбменСКонтрагентамиПереопределяемый.ЗаполнитьДанныеУПД_5_03_ИнформацияПродавца(
		ОбъектУчета, Настройки, Данные, Описание, Отказ);
	...
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Данные = Данные;
	Результат.Формат = "УПД_ИнформацияПродавца_5_03";
	Результат.Основания.Добавить(ОбъектУчета);
	Для Каждого Основание Из Описание.Основания Цикл
		Результат.Основания.Добавить(Основание);
	КонецЦикла;
...
Функция УПД_5_03_ИнформацияПродавца_НовыеДанные()
	
	Данные = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента(
		"ОбменСКонтрагентами.УПД_ИнформацияПродавца_5_03");
...
#Область УПД_5_03_ИнформацияПокупателя

Функция УПД_5_03_ИнформацияПокупателя_ДанныеУчета(Знач Основания, Знач Параметры)
	
	Данные = УПД_5_03_ИнформацияПокупателя_НовыеДанные();
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Формат = "УПД_ИнформацияПокупателя_5_03";
	...
	Отказ = Ложь;
	Попытка
		ОбменСКонтрагентамиПереопределяемый.ЗаполнитьДанныеУПД_5_03_ИнформацияПокупателя(
			Основания, Данные, Отказ);
	Исключение
		ЗаписатьОшибкуФормированияОтветногоТитулаВЖурналРегистрации(ИнформацияОбОшибке());
		Если Не ПолучитьФункциональнуюОпцию("ЗапретитьФормированиеОтветногоТитулаБезДокументовУчета") Тогда
...
Функция УПД_5_03_ИнформацияПокупателя_НовыеДанные()
	
	Данные = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента(
		"ОбменСКонтрагентами.УПД_ИнформацияПокупателя_5_03");
...
Функция АктОРасхождениях_ИнформацияПокупателя_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Формат = "АктОРасхождениях_ИнформацияОтправителя_5_01";
...
Функция АктОРасхождениях_ДополнительныеСведения_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Формат = "АктОРасхождениях_ДополнительныеСведения_5_01";
...
Функция КоммерческоеПредложение_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	КоммерческоеПредложение_ДобавитьДополнительныеДанныеПоСопоставлению(Данные);
...
Процедура КоммерческоеПредложение_ДобавитьДополнительныеДанныеПоСопоставлению(Данные)
...
Функция АктСверкиВзаиморасчетов_ИнформацияОтправителя_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	Результат = НовыйРезультатФормированияДанныхОбъектаУчета();
	Результат.Данные = Данные;
	Результат.Формат = "АктСверкиВзаиморасчетов_ИнформацияОтправителя_5_01";
	Результат.Основания.Добавить(ОбъектУчета);
	Для каждого Основание Из Описание.Основания Цикл
		Результат.Основания.Добавить(Основание);
	КонецЦикла;
...
//  ОбъектУчета - Неопределено
//              - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  Параметры - См. ОписаниеДанныхОбъектаУчета.Параметры
// 
// Возвращаемое значение:
//  См. НовыйРезультатФормированияДанныхОбъектаУчета
Функция АктСверкиВзаиморасчетов_ИнформацияПолучателя_ДанныеУчета(Знач ОбъектУчета, Знач Параметры)
	...
	Результат.Формат = "АктСверкиВзаиморасчетов_ИнформацияПолучателя_5_01";
	
	СтруктураОтвета = Параметры.ДанныеРучногоФормированияТитула;
	Если ЗначениеЗаполнено(СтруктураОтвета) Тогда
		АктСверкиВзаиморасчетов_ИнформацияПолучателя_ЗаполнитьДанныеРучногоФормирования(Данные, СтруктураОтвета);
		Результат.Данные = Данные;
		Возврат Результат;
	КонецЕсли;
...
// Параметры:
//  Данные - ДеревоЗначений
//  СтруктураОтвета - Структура - заполнение в ИнтерфейсДокументовЭДОКлиентСервер.ДанныеРучногоФормированияОтветногоТитулаУПД_2019:
//  * ЕстьРасхождения - Булево
Процедура АктСверкиВзаиморасчетов_ИнформацияПолучателя_ЗаполнитьДанныеРучногоФормирования(Данные, Знач СтруктураОтвета)

	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
		Данные, "ПризнакНаличияРазногласий", СтруктураОтвета.ЕстьРасхождения);
...
// Возвращает реквизиты контрагента, необходимые для его поиска и создания в прикладном решении
// 
// Параметры:
//  Реквизиты - Структура:
// * ИНН - Строка
// * КПП - Строка
// * Наименование - Строка
// * GLN - Строка - это поле может отсутствовать
// 
// Возвращаемое значение:
//  Структура:
// * ИНН - Строка
...
// * GLN - Строка
//
Функция РеквизитыКонтрагента(Реквизиты) Экспорт
...
// 
// Возвращаемое значение:
//  См. НовыеКлючевыеРеквизитыУчастникаЭДО
Функция КлючевыеРеквизитыУчастникаЭДО(УчастникЭДО, ИмяРеквизитаИНН, ИмяРеквизитаКПП)
	ИменаРеквизитов = НовыеКлючевыеРеквизитыУчастникаЭДО();
...
// Параметры:
//  УчастникиЭДО - ОпределяемыйТип.Организация
//               - ОпределяемыйТип.КонтрагентБЭД
//  ТипУчастниковЭДО - Строка - Значения: Организации, Контрагенты
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//  * Ссылка - ОпределяемыйТип.Организация
...
//  * ИНН - Строка
//  * КПП - Строка
Функция КлючевыеРеквизитыУчастниковЭДО(УчастникиЭДО, ТипУчастниковЭДО)
	
	ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	УчастникиЭДО.Ссылка КАК Ссылка,
		|	УчастникиЭДО.ИмяРеквизитаИНН КАК ИНН,
		|	УчастникиЭДО.ИмяРеквизитаКПП КАК КПП
		|ИЗ
		|	&ИмяТаблицыУчастникиЭДО КАК УчастникиЭДО
		|ГДЕ
		|	УчастникиЭДО.Ссылка В (&Ссылки)";
	
	Если ТипУчастниковЭДО = "Организации" Тогда
		Тип = Метаданные.ОпределяемыеТипы.Организация.Тип.Типы()[0];
		ИмяРеквизитаИНН = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННОрганизации");
		ИмяРеквизитаКПП = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППОрганизации");
	ИначеЕсли ТипУчастниковЭДО = "Контрагенты" Тогда
		Тип = Метаданные.ОпределяемыеТипы.КонтрагентБЭД.Тип.Типы()[0];
		ИмяРеквизитаИНН = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННКонтрагента");
		ИмяРеквизитаКПП = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППКонтрагента");
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный тип участников ЭДО'");
	КонецЕсли;
	
	ИмяТаблицыУчастникиЭДО = ОбщегоНазначения.ИмяТаблицыПоСсылке(Новый(Тип));
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяТаблицыУчастникиЭДО", ИмяТаблицыУчастникиЭДО);
	...
	Запрос.УстановитьПараметр("Ссылки", УчастникиЭДО);
	РезультатЗапроса = Запрос.Выполнить();
...
// Параметры:
//  УчастникиПоКлючам - См. КлючевыеРеквизитыУчастниковЭДО
//  КлючУчастника - Структура:
//  * ИНН - Строка
//  * КПП - Строка
// 
// Возвращаемое значение:
...
//  - ОпределяемыйТип.Организация
//  - ОпределяемыйТип.КонтрагентБЭД
Функция УчастникИзОтбораПоКлючевымРеквизитам(УчастникиПоКлючам, КлючУчастника)
	НайденныеУчастники = УчастникиПоКлючам.НайтиСтроки(КлючУчастника);
	Если ЗначениеЗаполнено(НайденныеУчастники) Тогда
		Возврат НайденныеУчастники[0].Ссылка;
	Иначе
		Возврат Неопределено
	КонецЕсли;
КонецФункции
...
// Параметры:
//  КлючиУчастниковЭДО - Массив из см. НовыеКлючевыеРеквизитыУчастникаЭДО
//  ТипУчастниковЭДО - Строка - Значения: Организации, Контрагенты
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
...
//  * ИНН - Строка
//  * КПП - Строка
Функция УчастникиЭДОПоКлючевымРеквизитам(КлючиУчастниковЭДО, ТипУчастниковЭДО) Экспорт // АПК:581 - Избыточное ключевое слово "Экспорт" (581)
	
	НаборРеквизитов = Новый ТаблицаЗначений;
	НаборРеквизитов.Колонки.Добавить("ИНН", ОбщегоНазначения.ОписаниеТипаСтрока(12));
	НаборРеквизитов.Колонки.Добавить("КПП", ОбщегоНазначения.ОписаниеТипаСтрока(9));
	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(КлючиУчастниковЭДО, НаборРеквизитов);
	
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	НаборРеквизитов.ИНН,
		|	НаборРеквизитов.КПП
		|ПОМЕСТИТЬ РеквизитыУчастниковЭДО
		|ИЗ
		|	&НаборРеквизитов КАК НаборРеквизитов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	&ИмяТаблицыУчастникиЭДО КАК УчастникиЭДО
		|ГДЕ
		|	(УчастникиЭДО.ИНН, УчастникиЭДО.КПП) В
		|		(ВЫБРАТЬ
		|			ИНН,
		|			КПП
		|		ИЗ
		|			РеквизитыУчастниковЭДО)";
...
Функция ПредставлениеРегиона(Наименование, ТипРегиона, КодРегиона)
	
	Если КодРегиона = "99" Тогда
		Возврат "иные территории, включая город и космодром Байконур";// АПК:1297 -Строка не локализована. Возможно, она видна пользователю (1297)
	ИначеЕсли СтрСравнить(ТипРегиона, "респ.") = 0 Тогда
		Возврат СоединитьНаименованиеИТипРегиона(Наименование, "Республика");
	ИначеЕсли СтрСравнить(ТипРегиона, "обл.") = 0 Тогда
		Возврат СоединитьНаименованиеИТипРегиона(Наименование, "область", Истина);
	ИначеЕсли СтрСравнить(ТипРегиона, "г.") = 0 Тогда
		Возврат СоединитьНаименованиеИТипРегиона(Наименование, "г.");
	КонецЕсли;

	Возврат СоединитьНаименованиеИТипРегиона(Наименование, ТипРегиона, Истина);

КонецФункции

Функция СоединитьНаименованиеИТипРегиона(Наименование, ТипРегиона, ЭтоРегион = Ложь)

	Если ПустаяСтрока(ТипРегиона) Тогда
		Возврат Наименование;
	КонецЕсли;
	
	НаименованиеРегиона = Наименование;
	Если Не ЭтоРегион Тогда
	    НаименованиеРегиона = ТипРегиона + " " + Наименование;
	КонецЕсли;
	
	РегионСоСпецифичнымНаименованием = РегионСоСпецифичнымНаименованием(НаименованиеРегиона);
	Если Не ПустаяСтрока(РегионСоСпецифичнымНаименованием) Тогда
		Возврат РегионСоСпецифичнымНаименованием;
	КонецЕсли;
	
	Если ЭтоРегион Тогда
		Возврат Наименование + " " + ТипРегиона;
	КонецЕсли;

	Возврат ТипРегиона + " " + Наименование;

КонецФункции

Функция РегионСоСпецифичнымНаименованием(Наименование)
	
	РегионыСоСпецифичнымНаименованием = 
	...
	|Ямало-Ненецкий автономный округ";// АПК:1297 -Строка не локализована. Возможно, она видна пользователю (1297)
	
	Результат = "";
	Если СтрЧислоВхождений(ВРег(РегионыСоСпецифичнымНаименованием), ВРег(Наименование)) = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	ЧислоСтрок = СтрЧислоСтрок(РегионыСоСпецифичнымНаименованием);
	Для НомерСтроки = 1 По ЧислоСтрок Цикл
		ТекущийРегион = СтрПолучитьСтроку(РегионыСоСпецифичнымНаименованием, НомерСтроки);
		
		Если СтрНачинаетсяС(ВРег(ТекущийРегион), ВРег(Наименование)) Тогда
			Результат = ТекущийРегион;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗапуститьПроверкуАктуальностиВнутреннегоЭД(ОбъектУчета, Отказ)
	...
	ЗаблокироватьДанныеОбъектовУчетаДокументовЭДО(ОбъектУчета, Неопределено, Истина, Отказ);
	
	Если Отказ Тогда
		
		ТекстСообщения = НСтр("ru = 'Не удалось закончить проверку актуальности электронного документа: %Ошибка%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ошибка%", "Не удалось заблокировать сведения об актуальности");
...
// Установить управляемую блокировку данных регистра сведений объектов учета документов ЭДО.
// Устанавливает блокировку по всем измерениям регистра, если не задан ОбъектУчета или ЭлектронныйДокумент получает их.
// 
// Параметры:
//  ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО, Неопределено - 
//    Объект учета по которому будет установлена блокировка в регистре сведений, не обязателен если указан ЭлектронныйДокумент
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО, ДокументСсылка.ЭлектронныйДокументИсходящийЭДО, Неопределено - 
//    Электронный документ по которому будет установлена блокировка в регистре сведений, не обязателен если указан ОбъектУчета
//  БлокироватьТолькоАктуальные - Булево. - Если признак = Ложь блокируются все зависимые электронные документы
//  Отказ - Булево - Возвращает Истина если установить блокировку не удалось
Процедура ЗаблокироватьДанныеОбъектовУчетаДокументовЭДО(ОбъектУчета, ЭлектронныйДокумент, БлокироватьТолькоАктуальные, Отказ)
		
	Отказ = Ложь;
	ПространствоИмен = "РегистрСведений.ОбъектыУчетаДокументовЭДО";

	Поля = Новый Структура;
	Поля.Вставить("ЭлектронныйДокумент", "ЭлектронныйДокумент");
	Поля.Вставить("ОбъектУчета", "ОбъектУчета");
	
	Если ОбъектУчета = Неопределено И ЭлектронныйДокумент = Неопределено Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;

	Если Не ЭлектронныйДокумент = Неопределено И ОбъектУчета = Неопределено Тогда
		...
		|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент В (&ЭлектронныйДокумент)
		|	И ВЫБОР
		|		КОГДА &ПроверкаАктуальности
		|			ТОГДА ОбъектыУчетаДокументовЭДО.Актуальный = ИСТИНА
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ";
		Запрос.УстановитьПараметр("ПроверкаАктуальности", БлокироватьТолькоАктуальные);
		Запрос.УстановитьПараметр("ЭлектронныйДокумент", ЭлектронныйДокумент);		
		
	ИначеЕсли ЭлектронныйДокумент = Неопределено И Не ОбъектУчета = Неопределено Тогда
	...
	Иначе
		Поля.ЭлектронныйДокумент = ЭлектронныйДокумент;
		Поля.ОбъектУчета = ОбъектУчета;
		Запрос = Неопределено;
	КонецЕсли;	
	
	Попытка
		Источник = ?(ТипЗнч(Запрос) = Тип("Запрос"), Запрос.Выполнить(), Неопределено);
		ОбщегоНазначенияБЭД.УстановитьУправляемуюБлокировку(ПространствоИмен, Поля, Источник);
	Исключение
		Отказ = Истина;
		
		ТекстСообщения = НСтр("ru = 'Не удалось заблокировать данные объектов учета ЭД: %ИнформацияОбОшибке%'");
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, "ЭлектронноеВзаимодействие.ОбменСКонтрагентами");	
	КонецПопытки;
	
КонецПроцедуры

// Параметры:
//  НаСопоставление - Массив Из См. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагента
//  СоответствиеНоменклатуры - См. СопоставлениеНоменклатурыКонтрагентов.НайтиСоответствиеНоменклатуры
//  ЭлектронныйДокумент - Неопределено
//                      - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
//  Отказ - Булево
Процедура КонтрольСопоставленияНоменклатуры(НаСопоставление, СоответствиеНоменклатуры, ЭлектронныйДокумент, Отказ)
	
	Если Не ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		Возврат; // Если документ загружается из файла
	КонецЕсли;
	
	ВсегоНаСопоставление = НаСопоставление.Количество();
	Для ОбратныйИндекс = 1 По ВсегоНаСопоставление Цикл
		Индекс = ВсегоНаСопоставление - ОбратныйИндекс;
		Идентификатор = НаСопоставление[Индекс].Идентификатор;
		ЕстьДанные = Ложь;
		Для Каждого ЭлементСоответствия Из СоответствиеНоменклатуры Цикл
			Если ЭлементСоответствия.НоменклатураКонтрагента.Идентификатор = Идентификатор Тогда
				ЕстьДанные = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ЕстьДанные Тогда
			НаСопоставление.Удалить(Индекс);
		КонецЕсли;
	КонецЦикла;
	
	Если НаСопоставление.Количество() > 0 Тогда
		ОтражениеВУчетеЭДО.ВключитьКонтрольСопоставленияНоменклатуры(ЭлектронныйДокумент, НаСопоставление);
		Отказ = Истина;
	КонецЕсли;
