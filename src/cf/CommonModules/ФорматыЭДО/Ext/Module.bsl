...
//     *** ИнформацияПродавца - Строка - идентификатор формата информации продавца.
//     *** ИнформацияПокупателя - Строка - идентификатор формата информации покупателя.
//    ** УПД_5_03 - Структура - форматы УПД 5.03 (ЕД-7-26/1032@):
//     *** ИнформацияПродавца - Строка - идентификатор формата информации продавца.
//     *** ИнформацияПокупателя - Строка - идентификатор формата информации покупателя.
...
//    ** СчетНаОплату_5_01 - Строка - форматы счета на оплату (ЕД-7-26/29@) (версия 5.01).
...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.ФНС.Вставить("УПД_5_03", Новый Структура);
	Форматы.ФНС.УПД_5_03.Вставить("ИнформацияПродавца", ФорматыФНС.УПД_5_03.ИнформацияПродавца);
	Форматы.ФНС.УПД_5_03.Вставить("ИнформацияПокупателя", ФорматыФНС.УПД_5_03.ИнформацияПокупателя);
	...
	Форматы.ФНС.Вставить("СчетНаОплату_5_01", ФорматыФНС.СчетНаОплату_5_01);
	...
	ТрансграничныйЭДО.ДополнитьФорматы(Форматы);
...
Процедура ОбновитьФорматыЭлектронныхДокументов(Форматы, ДатаИзменения) Экспорт
	...
	|	И НЕ ФорматыЭлектронныхДокументов.Формат В (&ИсключениеНеподдерживаемыеФорматы)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Запрос.УстановитьПараметр("ИсключениеНеподдерживаемыеФорматы", НеподдерживаемыеФорматыЭлектронныхДокументов);
	...
	Если Форматы.ДатаПоследнегоИзменения > ДатаИзменения Или ЕстьНеподдержанныеФорматы Или Не ЕстьЗаписи Тогда
		...
		ТрансграничныйЭДО.ОтметитьПоддерживаемыеФорматы(ФорматыЭлектронныхДокументов);
...
//  * Формализованный - Булево
//  * ПриказОВведении - Строка
//  * СсылкаНаПриказОВведении - Строка
//  * ВозможноИспользованиеУПД - Булево
//  * Приоритет - Число
...
Функция ВариантыЗаполненияПолейЭлектронныхДокументов(ТипДокумента, Формат) Экспорт
	...
	ЭтоУПД_5_03 = ПроверитьФорматЭлектронногоДокумента(ФорматЭлектронногоДокумента, "УПД", "5.03");
	...
	Если (ЭтоПередачаТоваров И Формат = "ON_SCHFDOPPR_1_995_01_05_01")
		ИЛИ (ЭтоПередачаТоваров И (ЭтоУПД_2019 Или ЭтоУПД_5_03))
...
//  ИмяВременнойТаблицы - Строка
//  Отбор - Неопределено
//        - См. НовыйОтборЗапросаФорматовПоУмолчанию
// 
// Возвращаемое значение:
//  См. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
...
Функция ЭтоСводныйУПД(ДвоичныеДанные) Экспорт
	...
	Если ПараметрыДокумента = Неопределено
		...
		ИЛИ ПараметрыДокумента.ИсходныйФормат <> ПоддерживаемыеФорматы().ФНС.УПД_5_02.ИнформацияПродавца
		ИЛИ ПараметрыДокумента.ИсходныйФормат <> ПоддерживаемыеФорматы().ФНС.УПД_5_03.ИнформацияПродавца Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ЧтениеXML = Новый ЧтениеXML;
	Поток = РаботаСФайламиБЭД.НовыйПотокЧтенияНормализованногоXML(ДвоичныеДанные);
...
Функция ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Экспорт
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца
		...
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
	...
	Попытка
		...
		Если Набор.Количество() > 0 Тогда
			...
			Для Каждого СтрокаНабора Из Набор Цикл
				Если СтрокаНабора.ПоддерживаетсяКонфигурацией Тогда
					Продолжить;
				КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ПодписаниеСИспользованиемБазовогоТипаПодписиДоступно(СодержаниеДокумента) Экспорт
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	Возврат СодержаниеДокумента.Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении
		Или СодержаниеДокумента.Формат = ПоддерживаемыеФорматы.ФНС.ИзвещениеОПолучении0201;
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	Если (Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Если ПустаяСтрока(ОписаниеОшибки) Тогда
			...
			Если (ЕстьСвойствоXDTO(Подписант, "ИП") И Подписант.ИП <> Неопределено)
				...
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
	...
	Если Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Иначе
			...
			Если ФорматыЭДО_ФНС.ДействуютПравилаЗаполненияПоПроектуПостановления1137()
				И ЗначениеЗаполнено(ДанныеЮрФизЛица.ОГРН)
				И ЗначениеЗаполнено(ДанныеЮрФизЛица.ДатаОГРН) Тогда
				
				ПредставлениеОГРНиДатыРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1;%2'"), ДанныеЮрФизЛица.ОГРН, Формат(ДанныеЮрФизЛица.ДатаОГРН, "ДФ=dd.MM.yyyy"));
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ИП, "СвГосРегИП", ПредставлениеОГРНиДатыРегистрации, , Ошибки);
			Иначе
		...
		Если (Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца
			...
			Если ЭтоФизЛицо И 
				Не ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи) Тогда
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		...
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПокупателя Тогда
		...
		ТребуетсяДоверенность = ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи);
		...
		Если ЭтоФизЛицо И Не ТребуетсяДоверенность Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "Должн", НСтр("ru = 'ИП'"), Истина, Ошибки);
		Иначе
		...
		Если (Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПокупателя
			Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПокупателя) 
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПокупателя
		...
		Иначе
			...
			Если Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПокупателя 
				И ТребуетсяДоверенность Тогда
					
					Если ДанныеФайлаЭД.ИнфПок.СодФХЖ4.ИнфПолФХЖ4 = Неопределено Тогда
						ИнфПолФХЖ4 = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML("Файл.ИнфПок.СодФХЖ4.ИнфПолФХЖ4", 
							ПространствоИменСхемы);
						РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.ИнфПок.СодФХЖ4, "ИнфПолФХЖ4", ИнфПолФХЖ4,
							Истина, Ошибки);
					КонецЕсли;
					
					ТекстИнф = ПолучитьОбъектТипаCML("Файл.ИнфПок.СодФХЖ4.ИнфПолФХЖ4.ТекстИнф", ПространствоИменСхемы);
					ИмяПоляДолжность = СтрШаблон("ДолжностьПодписанта_%1", ДанныеФайлаЭД.ИнфПок.Подписант.Количество());
					РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнф, "Идентиф", ИмяПоляДолжность,, Ошибки);
					РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнф, "Значен", ДолжностьПоСертификату,, Ошибки);
					
					ДанныеФайлаЭД.ИнфПок.СодФХЖ4.ИнфПолФХЖ4.ТекстИнф.Добавить(ТекстИнф);
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату
		И (Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101
		Или Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01) Тогда
		...
		ТребуетсяМашиночитаемаяДоверенность =
			ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи);
		Если ТребуетсяМашиночитаемаяДоверенность Тогда
			// В соответствии с электронной доверенностью в машиночитаемой форме хранение, которой осуществляется
			// в информационной системе, при этом сведения о ней указаны в электронном документе.
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "СпосПодтПолном", "3", Истина, Ошибки);
		Иначе
		...
		Если ТребуетсяМашиночитаемаяДоверенность И ЗначениеЗаполнено(ВыбраннаяМЧД) Тогда
			// Сведения о доверенности.
			...
			Если Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101 Тогда
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ДатаДовер",
					Формат(РеквизитыМЧД.ДатаВыдачи, "ДФ=dd.MM.yyyy"), Истина, Ошибки);
			ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ДатаВыдДовер",
					Формат(РеквизитыМЧД.ДатаВыдачи, "ДФ=dd.MM.yyyy"), Истина, Ошибки);
			КонецЕсли;
...
Функция ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	...
	Если ТипЗнч(Обработка) = Тип("Неопределено") Тогда
		Возврат Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
			...
			Или Формат = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01
			Или Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ИнформацияПокупателя
			Или Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения
...
Функция СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл")
		...
		Если ВерсияФорматаУПД = "5.02" Или ВерсияФорматаУПД = "5.03" Тогда
...
Процедура СброситьСведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Если ПространствоИмен = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		Или ПространствоИмен = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
		ВерсияСхемы = "4.01";
		ЛокальноеИмя = "Файл";
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		ОписаниеФайла.ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД, Ложь, , ЛокальноеИмя);
		ОписаниеФайла.ДвоичныеДанные = ОбщегоНазначенияБЭД.УдалитьПространствоИмен(ОписаниеФайла.ДвоичныеДанные,
			 ПространствоИмен);
		КонвертацияЭДО.ВыполнитьПреобразованиеСодержимого(ОписаниеФайла);
	Иначе
		ОписаниеФайла.ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД);
	КонецЕсли;
...
//
Функция СформироватьДокумент(Знач Формат, Знач Данные, ДанныеТЭДО = Неопределено) Экспорт

	ЭтоФорматCML = ЭтоФорматCML(Формат);
	ЭтоИностранныйФормат = ТрансграничныйЭДО.ЭтоИностранныйФормат(Формат);
	
	Результат = НовыйРезультатФормированияДокумента();
	Если ЭтоФорматCML Тогда 
		Результат = ФорматыЭДО_CML.СформироватьДокумент(Формат, Данные);
	ИначеЕсли ЭтоИностранныйФормат Тогда 
		Результат = ТрансграничныйЭДО.СформироватьДокумент(Формат, ДанныеТЭДО);
	Иначе
		Результат = ФорматыЭДО_ФНС.СформироватьДокумент(Формат, Данные);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Результат.Ошибки) И НЕ ЭтоИностранныйФормат Тогда
		КонвертацияЭДО.ВыполнитьПреобразованиеСодержимого(Результат.Документ);
	КонецЕсли;
...
Функция НовыеДанныеДляФормированияОтветногоТитула() Экспорт
	...
	Данные.Вставить("ДанныеТЭДО", Неопределено);
...
Функция НовыеДанныеДляФормированияСлужебногоСообщения() Экспорт
	...
	Данные.Основание.Вставить("Ссылка", Справочники.СообщениеЭДОПрисоединенныеФайлы.ПустаяСсылка());
...
Функция СформироватьДанныеОсновногоТитулаПоОбъектуУчета(ОбъектУчета, Настройки, Данные) Экспорт
	...
	ЗначенияДополнительныхПолей = СформироватьЗначенияДополнительныхПолейДокумента(ОбъектУчета, Настройки, Данные, Настройки.ОтключитьРассчетДополнительныхПолей);
	
	ДополнитьДанныеДокумента(ДанныеДокумента, Данные);
	
	Проверка = ПроверитьЗаполнениеДанных(Формат, ДанныеДокумента);
	
	Если ЕстьОшибкиЗаполненияДанных(Проверка) Тогда
		Результат = РезультатФормированияСОшибкойЗаполненияДанных(Проверка);
	Иначе
...
Функция СформироватьДанныеОтветногоТитула(Формат, Данные) Экспорт
	...
	Проверка = ПроверитьЗаполнениеДанных(Формат, ДанныеДокумента, Данные.ДанныеТЭДО);

	Если ЕстьОшибкиЗаполненияДанных(Проверка) Тогда
		Результат = РезультатФормированияСОшибкойЗаполненияДанных(Проверка);
	Иначе
 		Результат = СформироватьДокумент(Формат, ДанныеДокумента, Данные.ДанныеТЭДО);
	КонецЕсли;
...
Функция СформироватьДанныеСлужебногоСообщения(Знач ТипДокумента, Знач Данные) Экспорт
	...
	РезультатПоУчету = ТрансграничныйЭДО.ДанныеСлужебногоСообщения(ТипДокумента, Данные);
	Если РезультатПоУчету <> Неопределено Тогда
		Возврат РезультатПоУчету;
	КонецЕсли;
...
//  Настройки - см. НовыеНастройкиФормированияДокументаПоОбъектуУчета
//  Данные - Структура
//  ОтключитьРассчетДополнительныхПолей - Булево
// 
// Возвращаемое значение:
//  Структура - значения дополнительных полей и ошибки расчета:
Функция СформироватьЗначенияДополнительныхПолейДокумента(ОбъектУчета, Настройки, Данные, ОтключитьРассчетДополнительныхПолей) Экспорт
		
	Если ОтключитьРассчетДополнительныхПолей 
	...
	Возврат ЗначенияДополнительныхПолей;
...
//  * Предупреждения - Соответствие Из КлючИЗначение:
//   ** Ключ - Строка - текст ошибки, имя атрибута
//   ** Значение - Структура - Ложь, если принудительная загрузка невозможна, если загрузка возможна, тогда Структура:
//                 *** ВИсходномДокументе - Строка - значение в загруженном файле
//                 *** ВЗагружаемомДокументе - Строка - значение в загружаемом файле
//  * Ошибка - Строка - текст блокирующей ошибки
//  * СуществующийДокумент - Неопределено
//                         - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
//  * НовоеИмяИдФайла - Строка - заполняется, если требуется переименование ИдФайла
Функция НовыйРезультатЗагрузкиФормализованногоДокументаПоДаннымФайлаXML() Экспорт
	...
	Результат.Вставить("НовоеИмяИдФайла", "");
	Возврат Результат;
...
Функция ПроверитьДокумент(ОписаниеФайла) Экспорт
	...
	Если ТрансграничныйЭДО.ЭтоИностранныйФайл(ОписаниеФайла.ИмяФайла) Тогда
		Возврат ТрансграничныйЭДО.ПроверитьДокумент(ОписаниеФайла);
	КонецЕсли;
...
Функция ЗагружаемыйФайлСоответствуетЗагруженномуФайлу(ДанныеДокумента, ЗагружаемыйФайл, ЗагруженныйФайл) Экспорт

	РезультатЗагрузки = НовыйРезультатЗагрузкиФормализованногоДокументаПоДаннымФайлаXML();
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		РезультатЗагрузки.Ошибка = ОписаниеОшибки;
	...
	Если ПространствоИменЗагружаемыхДанных <> ПространствоИменЗагруженныхДанных Тогда
		Если ФорматыНеСовместимы(ПространствоИменЗагружаемыхДанных, ПространствоИменЗагруженныхДанных) Тогда
			...
			Возврат РезультатЗагрузки;
		Иначе
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ПространствоИменЗагруженныхДанных;
			ДанныеСравнения.ВЗагружаемомДокументе = ПространствоИменЗагружаемыхДанных;
			Предупреждения.Вставить(НСтр("ru = 'Формат файла'"), ДанныеСравнения);
			РезультатЗагрузки.Успех = Ложь;
			РезультатЗагрузки.Предупреждения = Предупреждения;
			Возврат РезультатЗагрузки;
		КонецЕсли;
	КонецЕсли;
	
	ПространстваИменФорматов = ФорматыЭДО_ФНС.ПространстваИменФорматов();
	ИдентификаторОрганизацииПодписанта = "";
	
	Если ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_02.ИнформацияПродавца
		Или ПространствоИменЗагружаемыхДанных = ПространстваИменФорматов.УПД_5_03.ИнформацияПродавца Тогда
		
		СодержаниеДокумента = ПрочитатьСодержаниеДокумента(ОписаниеЗагружаемогоФайла);
		ПроверкаИмениФайла = ПроверкаИмениФайла(ДанныеДокумента, ЗагружаемыеДанные, ЗагруженныеДанные, 
			СодержаниеДокумента);
		
		Если Не ПроверкаИмениФайла.КорректировкаВозможна Тогда
			ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Документ не загружен, так как не найдены 
				|настройки отправки ЭДО по отправителю %1 и получателю %2
				|Загрузка приведет к ошибкам со стороны оператора.
				|Выберите другой файл и повторите загрузку.'"), 
				СодержаниеДокумента.Отправитель.Наименование,
				СодержаниеДокумента.Получатель.Наименование);
			РезультатЗагрузки.Успех = Ложь;
			РезультатЗагрузки.Ошибка = ОписаниеОшибки;
			Возврат РезультатЗагрузки;
		КонецЕсли;
		
		Если Не ПроверкаИмениФайла.ИмяФайлаКорректное Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ЗагружаемыеДанные.ИдФайл;
			ДанныеСравнения.ВЗагружаемомДокументе = ПроверкаИмениФайла.НовоеИмяФайла;
			РезультатЗагрузки.Успех = Ложь;
			Предупреждения.Вставить(НСтр("ru = 'Исправлено имя файла'"), ДанныеСравнения);
			РезультатЗагрузки.НовоеИмяИдФайла = ПроверкаИмениФайла.НовоеИмяФайла;
		КонецЕсли;
		...
		Если ЧастиИмениФайлаЗагружаемыеДанные.Количество() >= 3 Тогда
			...
			ИдентификаторОрганизацииПодписанта = ИдОтправителяВЗагружаемомДокументе;
		Иначе
		...
		Если ИдОтправителяВЗагружаемомДокументе <> ИдОтправителяВИсходномДокументе Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ИдОтправителяВИсходномДокументе;
			ДанныеСравнения.ВЗагружаемомДокументе = ИдОтправителяВЗагружаемомДокументе;
			Предупреждения.Вставить("ИдФайл[ИдентификаторОтправителя]", ДанныеСравнения);
		КонецЕсли;
		
		Если ИдПолучателяВИсходномДокументе <> ИдПолучателяВЗагружаемомДокументе Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ИдПолучателяВИсходномДокументе;
			ДанныеСравнения.ВЗагружаемомДокументе = ИдПолучателяВЗагружаемомДокументе;
			Предупреждения.Вставить("ИдФайл[ИдентификаторПолучателя]", ДанныеСравнения);
		КонецЕсли;
		
	Иначе
		
		Если Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "ИдФайл") 
			Или Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "СвУчДокОбор") Тогда
		...
		Если ЗагружаемыеДанные.СвУчДокОбор.ИдОтпр <> ЗагруженныеДанные.СвУчДокОбор.ИдОтпр Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ЗагруженныеДанные.СвУчДокОбор.ИдОтпр;
			ДанныеСравнения.ВЗагружаемомДокументе = ЗагружаемыеДанные.СвУчДокОбор.ИдОтпр;
			Предупреждения.Вставить("ИдОтпр", ДанныеСравнения);
			ИмяФайла = СтрЗаменить(ИмяФайла, ДанныеСравнения.ВЗагружаемомДокументе, ДанныеСравнения.ВИсходномДокументе);
		КонецЕсли;
		...
		Если ИдПолЗагружаемый <> ИдПолИсходный Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ИдПолИсходный;
			ДанныеСравнения.ВЗагружаемомДокументе = ИдПолЗагружаемый;
			Предупреждения.Вставить("ИдПол", ДанныеСравнения);
			ИмяФайла = СтрЗаменить(ИмяФайла, ДанныеСравнения.ВЗагружаемомДокументе, ДанныеСравнения.ВИсходномДокументе);
		КонецЕсли;
		
		Если ИмяФайла <> ЗагружаемыеДанные.ИдФайл Тогда
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = ЗагружаемыеДанные.ИдФайл;
			ДанныеСравнения.ВЗагружаемомДокументе = ИмяФайла;
			РезультатЗагрузки.Успех = Ложь;
			Предупреждения.Вставить(НСтр("ru = 'Исправлено имя файла'"), ДанныеСравнения);
			РезультатЗагрузки.НовоеИмяИдФайла = ИмяФайла;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗагружаемыйДокумент = Неопределено И ТекущийДокумент = Неопределено Тогда
		РезультатЗагрузки.Успех = Предупреждения.Количество() = 0;
		РезультатЗагрузки.Предупреждения = Предупреждения;
		Возврат РезультатЗагрузки;
	ИначеЕсли ЗагружаемыйДокумент = Неопределено Тогда 
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе =
			СтрШаблон(НСтр("ru = 'данные формата %1'"), ЗагруженныеДанные.Тип().URIПространстваИмен);
		ДанныеСравнения.ВЗагружаемомДокументе = НСтр("ru = 'нет данных'");
		Предупреждения.Вставить("Документ", ДанныеСравнения);
		РезультатЗагрузки.Успех = Предупреждения.Количество() = 0;
		РезультатЗагрузки.Предупреждения = Предупреждения;
		Возврат РезультатЗагрузки;
	КонецЕсли;
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		...
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "Функция");
		ДанныеСравнения.ВЗагружаемомДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "Функция");
		Предупреждения.Вставить("Функция", ДанныеСравнения);
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		...
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "ПоФактХЖ");
		ДанныеСравнения.ВЗагружаемомДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "ПоФактХЖ");
		Предупреждения.Вставить("ПоФактХЖ", ДанныеСравнения);
			
	КонецЕсли;
	// АПК:1036-выкл не требуется проверки орфографии.
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимДокОпр") <>
		 РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимДокОпр") Тогда
				
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимДокОпр");
		ДанныеСравнения.ВЗагружаемомДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимДокОпр");
		Предупреждения.Вставить("НаимДокОпр", ДанныеСравнения);
			
	КонецЕсли;
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимЭконСубСост") <>
		 РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимЭконСубСост") Тогда
				
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ТекущийДокумент, "НаимЭконСубСост");
		ДанныеСравнения.ВЗагружаемомДокументе = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЗагружаемыйДокумент, "НаимЭконСубСост");
		Предупреждения.Вставить("НаимЭконСубСост", ДанныеСравнения);
	...
	Если РаспознаниеТекущего.НомерДокумента <> РаспознаниеЗагружаемого.НомерДокумента Тогда
		...
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = РаспознаниеТекущего.НомерДокумента;
		ДанныеСравнения.ВЗагружаемомДокументе = РаспознаниеЗагружаемого.НомерДокумента;
		Предупреждения.Вставить("Номер документа", ДанныеСравнения);
	...
	Если РаспознаниеТекущего.ДатаДокумента <> РаспознаниеЗагружаемого.ДатаДокумента Тогда
		...
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = Формат(РаспознаниеТекущего.ДатаДокумента, "ДЛФ=Д");
		ДанныеСравнения.ВЗагружаемомДокументе = Формат(РаспознаниеЗагружаемого.ДатаДокумента, "ДЛФ=Д");
		Предупреждения.Вставить("Дата документа", ДанныеСравнения);
	...
	Если РаспознаниеТекущего.СуммаДокумента <> РаспознаниеЗагружаемого.СуммаДокумента Тогда
		...
		ДанныеСравнения = НовыеДанныеСравнения();
		ДанныеСравнения.ВИсходномДокументе = Формат(РаспознаниеТекущего.СуммаДокумента, "ЧЦ=10; ЧДЦ=2");
		ДанныеСравнения.ВЗагружаемомДокументе = Формат(РаспознаниеЗагружаемого.СуммаДокумента, "ЧЦ=10; ЧДЦ=2");
		Предупреждения.Вставить("Сумма документа", ДанныеСравнения);
	...
	Если ФайлСформированВ1С(ОписаниеЗагружаемогоФайла) = Ложь Тогда
		...
		Если Не ЗначениеЗаполнено(Подписант.ИдентификаторОрганизации)
			И ЗначениеЗаполнено(ИдентификаторОрганизацииПодписанта) Тогда
			Подписант.ИдентификаторОрганизации = ИдентификаторОрганизацииПодписанта;
		КонецЕсли;
		
		Если Не ПодписантЗаполнен(Подписант, ДанныеДокумента.Организация, ОписаниеОшибки) 
			Или Не ВУчетнойЗаписиЕстьСертификатПодписанта(Подписант, ОписаниеОшибки) Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			...
			ДанныеСравнения = НовыеДанныеСравнения();
			ДанныеСравнения.ВИсходномДокументе = "";
			ДанныеСравнения.ВЗагружаемомДокументе = НСтр("ru = 'нет сертификата'");
			Предупреждения.Вставить("Подписант", ДанныеСравнения);
		КонецЕсли;
	КонецЕсли;
...
// Параметры:
//  ОписаниеФайла - См. РаботаСФайламиБЭД.НовоеОписаниеФайла
//  НовыйИдФайл - Строка - имя файла без расширения
//
// Возвращаемое значение:
//  см. РаботаСФайламиБЭД.НовоеОписаниеФайла
Функция ИсправитьИдФайла(ОписаниеФайла, НовыйИдФайл) Экспорт
	Результат = РаботаСФайламиБЭД.НовоеОписаниеФайла();
	ОписаниеОшибки = "";
	ДанныеФайлаЭД = ДанныеФайлаЭД(ОписаниеФайла, ОписаниеОшибки);
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Возврат ОписаниеФайла;;
	КонецЕсли;
	ДанныеФайлаЭД.ИдФайл = НовыйИдФайл;
	Если ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл") И ЕстьСвойствоXDTO(ДанныеФайлаЭД, "СвУчДокОбор") Тогда
		МассивИдФайла = СтрРазделить(НовыйИдФайл, "_");
		Если МассивИдФайла.Количество() = 6 Тогда
			Если ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдПол") Тогда
				ДанныеФайлаЭД.СвУчДокОбор.ИдПол = МассивИдФайла[2];
			КонецЕсли;
			Если ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдПок") Тогда
				ДанныеФайлаЭД.СвУчДокОбор.ИдПок = МассивИдФайла[2];
			КонецЕсли;
			Если ЕстьСвойствоXDTO(ДанныеФайлаЭД.СвУчДокОбор, "ИдОтпр") Тогда
				ДанныеФайлаЭД.СвУчДокОбор.ИдОтпр = МассивИдФайла[3];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД, Ложь, "windows-1251", ЛокальноеИмя);
	ДвоичныеДанные = ОбщегоНазначенияБЭД.УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИменСхемы);
	Результат.ИмяФайла = СтрШаблон("%1.xml", НовыйИдФайл);
	Результат.ДвоичныеДанные = ДвоичныеДанные;
	Возврат Результат;
КонецФункции

// Проверяет, относится ли переданная версия формата к формату CML.
// 
// Параметры:
//  Формат - Строка
// 
// Возвращаемое значение:
//  Булево
Функция ЭтоФорматCML(Формат) Экспорт
...
Функция НовоеОписаниеОшибокФормированияДокумента() Экспорт
...
Функция ПроверитьЗаполнениеДанных(Формат, Данные, ДанныеТЭДО = Неопределено) Экспорт

	ЭтоФорматCML = ЭтоФорматCML(Формат);
	
	Если ЭтоФорматCML Тогда
		Проверка = ФорматыЭДО_CML.ПроверитьЗаполнениеДанных(Формат, Данные);
	ИначеЕсли ТрансграничныйЭДО.ЭтоИностранныйФормат(Формат) Тогда
		Проверка = ТрансграничныйЭДО.ПроверитьЗаполнениеДанных(Формат, ДанныеТЭДО);
	Иначе
		Проверка = ФорматыЭДО_ФНС.ПроверитьЗаполнениеДанных(Формат, Данные); 

		Если ТипЗнч(Данные) = Тип("ДеревоЗначений") Тогда
			ДеревоЭлектронногоДокументаБЭД.ПроверитьЗаполнениеРеквизитовДереваДанныхРекурсивно(Данные, Проверка.Ошибки, Истина);
...
// Проверяет результат проверки на наличие ошибок заполнения данных.
// 
// Параметры:
//  Проверка - Структура - см. НовыйРезультатПроверкиЗаполненияДанных
// 
// Возвращаемое значение:
//  Булево - Есть ошибки заполнения данных
Функция ЕстьОшибкиЗаполненияДанных(Проверка) Экспорт
...
// 
// Возвращаемое значение:
//  Структура - см. НовыйРезультатФормированияДокумента
Функция РезультатФормированияСОшибкойЗаполненияДанных(Знач Проверка) Экспорт
...
Функция ДанныеФайлаЭД(ДанныеФайла, ОписаниеОшибки, ПространствоИменФНС = Неопределено, ИмяКорневогоУзла = "", МягкоеЧтение = Ложь) Экспорт
	...
	КонецЕсли;
	ДвоичныеДанныеФайла = РаботаСФайламиБЭД.ДвоичныеДанныеНормализованногоXML(ДвоичныеДанныеФайла);
	...
	Если ЗначениеЗаполнено(ИдФайл) И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не МягкоеЧтение Тогда
			...
			ИначеЕсли СтрНайти(ИдФайл, "ON_NSCHFDOPPR") Тогда
				...
				ИначеЕсли ВерсФорм = "5.03" Тогда
					URIПространстваИмен = "ON_NSCHFDOPPR_1_997_01_05_03_01";
				Иначе
			...
			ИначеЕсли СтрНайти(ИдФайл, "ON_NSCHFDOPPOK") Тогда
				...
				ИначеЕсли ВерсФорм = "5.03" Тогда
					URIПространстваИмен = "ON_NSCHFDOPPOK_1_997_02_05_03_01";
				Иначе
		...
		ИначеЕсли СтрНайти(ИдФайл, "ON_CHETOP") > 0 Тогда // Счет на оплату
			Если ВерсФорм = "5.01" Тогда
				URIПространстваИмен = "ON_CHETOP_1_998_01_05_01_01";
			Иначе
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			...
			Иначе
				...
				УчитыватьПробелыПриЧтении = Ложь;
				Если ЗначениеЗаполнено(URIПространстваИмен) Тогда
					УчитыватьПробелыПриЧтении = Не ПакетXDTOСодержитMixedContent(URIПространстваИмен);
				ИначеЕсли ЗначениеЗаполнено(ПространствоИменФайла) Тогда
					УчитыватьПробелыПриЧтении = Не ПакетXDTOСодержитMixedContent(ПространствоИменФайла);
				КонецЕсли;
				
				ИгнорироватьПробельныеСимволы = Не УчитыватьПробелыПриЧтении;
				
				Если Не ЗначениеЗаполнено(ПространствоИменФайла) Тогда
					УдаляемыеАтрибуты = Новый Массив;
					УдаляемыеАтрибуты.Добавить("xsi:type");
					ДвоичныеДанныеФайла = 
						УдалитьОбъявлениеПространстваИменВТелеДокумента(ДвоичныеДанныеФайла, Кодировка, URIПространстваИмен);
					ДвоичныеДанныеФайла = УдалитьАтрибутыУзлов(ДвоичныеДанныеФайла, Кодировка, УдаляемыеАтрибуты);
				КонецЕсли;
				ПараметрыЧтенияXML = 
					Новый ПараметрыЧтенияXML(,,,,,,,, ИгнорироватьПробельныеСимволы,, УчитыватьПробелыПриЧтении);
				ЧтениеXML.ОткрытьПоток(ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения(), ПараметрыЧтенияXML);
...
Функция ПрочитатьСодержаниеДокумента(ОписаниеФайла) Экспорт
	...
	Если ТрансграничныйЭДО.ЭтоИностранныйФайл(ОписаниеФайла.ИмяФайла) Тогда
		Возврат ТрансграничныйЭДО.ПрочитатьСодержаниеДокумента(ОписаниеФайла);
	КонецЕсли;
...
Функция ИдентификаторыДатаНомерДокументаПоДаннымФайла(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Если РаспознаниеТекущего.Свойство("ИсходныйФормат") Тогда
		...
		Если РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
			Или РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
...
// * СпециальныеОбстоятельстваФормированияСЧФ - Строка
...
Функция НовоеОписаниеФайлаДокумента() Экспорт
	...
	Содержание.Вставить("СпециальныеОбстоятельстваФормированияСЧФ", "");
...
//   * Корректировка - Структура:
//    ** СуммаКорректировки - Число
//    ** СуммаКорректировкиНДС - Число
Функция НовоеОписаниеСвойствДокументовПоФорматам() Экспорт
	...
	СвойстваКорректировки = Новый Структура;
	СвойстваКорректировки.Вставить("СуммаКорректировки", 0);
	СвойстваКорректировки.Вставить("СуммаКорректировкиНДС", 0);
	Результат.Вставить("Корректировка", СвойстваКорректировки);
...
// Параметры:
//  ДеревоДанных - См. ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента
...
Процедура ПроверитьРегистрационныеДанныеДляОператораЭДО(ДеревоДанных, Ошибки) Экспорт
	...
	ЭтоЮЛ = ТипУчастника = "ЮЛ";
	ЭтоИП = ТипУчастника = "ИП";
	ЭтоФилиалИностраннойОрганизации = Ложь;
	Если ЭтоЮЛ Тогда
		ОГРН = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "УчастЭДО.ТипУчастника.ЮЛ.ОГРН",,
			НайденнаяСтрока);
		КПП = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "УчастЭДО.ТипУчастника.ЮЛ.КПП");
		ЭтоФилиалИностраннойОрганизации = ОбщегоНазначенияБЭД.ЭтоКППФилиалаИностраннойОрганизации(КПП);
	Иначе
		ОГРН = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных, "УчастЭДО.ТипУчастника.ИП.ОГРНИП",,
			НайденнаяСтрока);
	КонецЕсли;
	...
	Если ЭтоИП Или ЭтоЮЛ И Не ЭтоФилиалИностраннойОрганизации Тогда
...
Функция ТекстУточнения(ОписаниеФайла) Экспорт
	...
	ЧтениеXML = Новый ЧтениеXML;
	Поток = РаботаСФайламиБЭД.НовыйПотокЧтенияНормализованногоXML(ОписаниеФайла.ДвоичныеДанные);
	ЧтениеXML.ОткрытьПоток(Поток);
...
// 
// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДО.НовыеДанныеНоменклатурыДокумента
Функция НоваяТаблицаТоваров() Экспорт
	Возврат ЭлектронныеДокументыЭДО.НовыеДанныеНоменклатурыДокумента();
КонецФункции
...
// 
// Возвращаемое значение:
// 	см. ВизуализацияЭДОСлужебный.НовыеПараметрыВизуализацииДокумента
//
Функция НовыеПараметрыВизуализацииДокумента() Экспорт
	
	Возврат ВизуализацияЭДОСлужебный.НовыеПараметрыВизуализацииДокумента();
...
Функция ИнформацияОТовареИзФайла(Параметры, ИменаДопКолонок = "") Экспорт
	...
	Если ТрансграничныйЭДО.ЭтоИностранныйФайл(Параметры.ОсновнойФайл.ИмяФайла) Тогда
		Возврат ТрансграничныйЭДО.ТоварыИзФайла(Параметры.ОсновнойФайл);
	КонецЕсли;
	...
	Попытка
		...
		Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
			...
			ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_CHETOP") > 0 Тогда	
				...
				НаборДанных = Неопределено;
				Если ДанныеФайлаЭД.ВерсФорм = "1.01" Тогда
					НаборДанных = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ.СведТовЦенПок",,, Истина);
				ИначеЕсли ДанныеФайлаЭД.ВерсФорм = "5.01" Тогда	
					НаборДанных = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ.СодСч.СведТовЦенПок",,, Истина);
				КонецЕсли;
...
// Возвращает таблицу с данными для расшифровки формата и вида ЭД в Формат, вариант заполнения и в обратном направлении.
//
// Возвращаемое значение:
//  ТаблицаЗначений - Расшифровка вариантов заполнения:
//  * Формат - Строка
//  * ВариантЗаполнения - Строка - Обязателен к заполнению, иначе будет ошибка при обновлении данных в сервисе настроек.
//  * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
Функция РасшифровкаВариантовЗаполненияФорматовЭДО() Экспорт
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = "ON_AKTREZRABP_1_971_01_01_00_01";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктПриемкиСтроительныхРаботУслуг;
	НоваяСтрока.ВариантЗаполнения = "АктПриемкиСтроительныхРаботУслуг";
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = "ON_NSCHFDOPPR_1_997_01_05_03_01";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.ВариантЗаполнения = "ДОП";
	///////////////////////////////////////////////////////////////////////////////////
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = "ON_CHETOP_1_998_01_05_01_01";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату;
	НоваяСтрока.ВариантЗаполнения = "СчетНаОплату_5_01";
	///////////////////////////////////////////////////////////////////////////////////
	...
	//Внутреняя проверка на заполнение обязательных полей, для выявления неявных ошибок ранних этапах
	ТелоОшибки = "";
	ЗаголовокОшибки = НСтр("ru = 'В расшифровке вариантов заполнения не указаны варианты заполнения для следующих форматов:'");
	Для Каждого ПроверяемаяСтрока Из ВариантыЗаполнения Цикл
		Если ЗначениеЗаполнено(ПроверяемаяСтрока.ВариантЗаполнения) Тогда
			Продолжить;
		КонецЕсли;
		ТелоОшибки = Символы.ПС + Строка(ПроверяемаяСтрока.Формат);
	КонецЦикла;
	Если ЗначениеЗаполнено(ТелоОшибки) Тогда
		ТекстОшибки = ЗаголовокОшибки + ТелоОшибки;
...
#Область ЗаполнениеУчастниковЭДО

// Конструктор данных участников ЭДО
// 
// Возвращаемое значение:
//  Структура:
//    * ИдентификаторОтправителя - Строка 
//    * ИдентификаторПолучателя - Строка
//    * ДанныеОператораЭДО - Структура:
//      ** Наименование - Строка
//      ** ИНН - Строка
//      ** Идентификатор - Строка
Функция НовыеДанныеУчастниковЭДО() Экспорт
	...
	ДанныеОператораЭДО = Новый Структура;
	ДанныеОператораЭДО.Вставить("Наименование", "");
	ДанныеОператораЭДО.Вставить("ИНН", "");
	ДанныеОператораЭДО.Вставить("Идентификатор", "");
	
	ДанныеУчастников.Вставить("ДанныеОператораЭДО", ДанныеОператораЭДО);
	
	Возврат ДанныеУчастников;
	
КонецФункции

// Возвращает описание файла с заполненными
// 
// Параметры:
//  ОписаниеФайла - см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
...
// Возвращаемое значение:
//  см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
Функция ЗаполнитьДанныеУчастниковЭДО(ОписаниеФайла, ДанныеУчастниковЭДО) Экспорт
	
	Результат = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
	ЗаполнитьЗначенияСвойств(Результат, ОписаниеФайла);
	...
	Если Не ЗаполнениеДанныхУчастниковЭДОДоступно(Формат) Тогда
		Возврат Результат;
	КонецЕсли;
	...
	// УПД 5.03 обрабатываем в особом порядке, поскольку тут данные участников есть только в ИД файла
	Если ЭтоУПД_5_03(Формат) Тогда
		Возврат ОписаниеФайлаУПД_5_03_СЗаполненнымИдентификаторомФайла(ДанныеФайла, Формат, ДанныеУчастниковЭДО);
	КонецЕсли;
	
	ИменаПолей = ИменаПолейСведенийУчастниковЭДО(Формат);
	
	СведенияУчастниковЗаписанные = 
		РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайла, ИменаПолей.СведенияУчастников);
	
	ИдентификаторОтправителяЗаписанный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		СведенияУчастниковЗаписанные, ИменаПолей.ИдентификаторОтправителя);
	ИдентификаторПолучателяЗаписанный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		СведенияУчастниковЗаписанные, ИменаПолей.ИдентификаторПолучателя);
	
	ИдентификаторФайлаОригинальный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайла, ИменаПолей.ИдентификаторФайла);
	ИдентификаторФайла = ИдентификаторФайлаОригинальный;
	Если ЗначениеЗаполнено(ИдентификаторОтправителяЗаписанный) Тогда
		ИдентификаторФайла = СтрЗаменить(ИдентификаторФайла,
			ИдентификаторОтправителяЗаписанный, ДанныеУчастниковЭДО.ИдентификаторОтправителя);
	КонецЕсли;
	Если ЗначениеЗаполнено(ИдентификаторПолучателяЗаписанный) Тогда
		ИдентификаторФайла = СтрЗаменить(ИдентификаторФайла,
			ИдентификаторПолучателяЗаписанный, ДанныеУчастниковЭДО.ИдентификаторПолучателя);
	КонецЕсли;
	
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайла,
		ИменаПолей.ИдентификаторФайла, ИдентификаторФайла);
	Результат.ИмяФайла = СтрЗаменить(Результат.ИмяФайла, ИдентификаторФайлаОригинальный, ИдентификаторФайла);
	
	АдресСведенийУчастников = СтрШаблон("Файл.%1", ИменаПолей.СведенияУчастников);
	СведенияУчастниковЭДО = ПолучитьОбъектТипаCML(АдресСведенийУчастников, ПространствоИмен);
	
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияУчастниковЭДО,
		ИменаПолей.ИдентификаторОтправителя, ДанныеУчастниковЭДО.ИдентификаторОтправителя);
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияУчастниковЭДО,
		ИменаПолей.ИдентификаторПолучателя, ДанныеУчастниковЭДО.ИдентификаторПолучателя);
	
	АдресСведенийОператора = СтрШаблон("Файл.%1.%2", ИменаПолей.СведенияУчастников, ИменаПолей.СведенияОператора);
	СведенияОператораЭДО = ПолучитьОбъектТипаCML(АдресСведенийОператора, ПространствоИмен);
	
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОператораЭДО,
		ИменаПолей.НаименованиеОператора, ДанныеУчастниковЭДО.ДанныеОператораЭДО.Наименование);
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОператораЭДО,
		ИменаПолей.ИННОператора, ДанныеУчастниковЭДО.ДанныеОператораЭДО.ИНН);
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СведенияОператораЭДО,
		ИменаПолей.ИдентификаторОператора, ДанныеУчастниковЭДО.ДанныеОператораЭДО.Идентификатор);
	...
	КонвертацияЭДО.ВыполнитьПреобразованиеСодержимого(Результат);
...
// 
// Возвращаемое значение:
//  см. НовыеДанныеУчастниковЭДО
Функция ПрочитатьДанныеУчастниковЭДО(ОписаниеФайла, Формат) Экспорт
	...
	СведенияУчастниковXDTO = 
		РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайла, ИменаПолей.СведенияУчастников);
	
	ИдентификаторОтправителяЗаписанный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		СведенияУчастниковXDTO, ИменаПолей.ИдентификаторОтправителя);
	ИдентификаторПолучателяЗаписанный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		СведенияУчастниковXDTO, ИменаПолей.ИдентификаторПолучателя);
...
Функция НастройкаЗаполненияДополнительныхПолей(ОбъектУчета, ПараметрыПолучения)
	...
	ОписаниеОбъектаУчета = ИнтеграцияЭДО.ОписаниеОбъектаУчетаПоОтбору(ОбъектУчета, ОтборОписанияОбъектаУчета, Ложь);
...
//  ДополнительныеДанные - см. НовыеДанныеДляФормированияОтветногоТитула
//  Замещать - Булево
//
Процедура ДополнитьДанныеДокумента(Данные, Знач ДополнительныеДанные, Знач Замещать = Ложь)
	
	ТипДанных = ТипЗнч(Данные);
	Если ТипДанных = Тип("ДеревоЗначений") И Данные.Строки.Количество() > 0 Тогда
...
Функция СодержаниеДокументаПоПравиламКонвертации(ОписаниеФайла)
	...
	Если ПараметрыДокумента.Свойство("СвойстваДокументовПоФорматам") Тогда 
		ЗаполнитьЗначенияСвойств(СодержаниеДокумента, ПараметрыДокумента, , "СвойстваДокументовПоФорматам");
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(СодержаниеДокумента.СвойстваДокументовПоФорматам, 
			ПараметрыДокумента.СвойстваДокументовПоФорматам, Истина);
	Иначе
	...
	ПоддерживаемыеФорматыФорматыЭДО = ПоддерживаемыеФорматы();
	Если СодержаниеДокумента.Формат = ПоддерживаемыеФорматыФорматыЭДО.ФНС.АктОРасхождениях.ИнформацияПокупателя Тогда
		СодержаниеДокумента.ТипРегламента = ТипРегламентаДляАктаОРасхождениях(СодержаниеДокумента);
	КонецЕсли;
...
Функция ТипРегламентаДляАктаОРасхождениях(СодержаниеДокумента)
	
	ТипРегламента = Перечисления.ТипыРегламентовЭДО.АктОРасхождениях;
	
	ПараметрыИзИдентификатораФайла = ФорматыЭДО_ФНС.СтруктураИдентификатораФайла_АктОРасхождениях_ИнформацияПокупателя(
		СодержаниеДокумента.ИдентификаторДокумента);
	ОператорОтправителя = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(
		ПараметрыИзИдентификатораФайла.ИдентификаторОтправителя);
	ОператорПолучателя = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(
		ПараметрыИзИдентификатораФайла.ИдентификаторПолучателя);
		
	РезультатПроверки = ФорматПоддержанОператорами(ОператорОтправителя, ОператорПолучателя,
		СодержаниеДокумента.Формат, Перечисления.ТипыРегламентовЭДО.АктОРасхождениях);
	Если Не ЗначениеЗаполнено(РезультатПроверки)
		Или Не РезультатПроверки.ФорматПоддержанОператоромОтправителя
...
// * ИНН - Строка
// * КПП - Строка
// * УчастникТЭДО - Неопределено
//                - см. ТрансграничныйЭДО.ОписаниеУчастника
// 
Функция НовоеОписаниеУчастникаЭДО() Экспорт
	...
	ОписаниеУчастника.Вставить("УчастникТЭДО", Неопределено);
...
//  * ДанныеТЭДО - см. ТрансграничныйЭДО.ДанныеЭД
//
Функция ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанных) Экспорт
	...
	Если ТрансграничныйЭДО.ЭтоИностранныйФайл(ПараметрыПолученияДанных.ОсновнойФайл.ИмяФайла) Тогда
		
		Результат = Новый Структура();
		Результат.Вставить("ДеревоРазбора", ДеревоРазбора);
		Результат.Вставить("НовыйЭД", НовыйЭД);
		Результат.Вставить("Содержание", Неопределено);
		Результат.Вставить("ДанныеТЭДО", ТрансграничныйЭДО.ДанныеЭД(ПараметрыПолученияДанных));
		Возврат Результат;
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда //чтение файлов без namespace по форматам ФНС
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPR") > 0 Тогда
			...
			ВерсияФорматаУПД = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
			Если ВерсияФорматаУПД = "5.01" Тогда
			...
			ИначеЕсли ВерсияФорматаУПД = "5.02" Тогда
			...
			ИначеЕсли ВерсияФорматаУПД = "5.03" Тогда
				ФорматыЭДО_ФНС.ПрочитатьИнформациюПродавцаXDTO_УПД_5_03(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				НовыйЭД.ВерсияФормата = "ON_NSCHFDOPPR_1_997_01_05_03_01";
			КонецЕсли;
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPOK") > 0 Тогда
			...
			ИначеЕсли ВерсияФорматаУПД = "5.02" Тогда
				...
				НовыйЭД.ВерсияФормата = "ON_NSCHFDOPPR_1_997_01_05_02_01";
			ИначеЕсли ВерсияФорматаУПД = "5.03" Тогда
				ФорматыЭДО_ФНС.ПрочитатьИнформациюПокупателяXDTO_УПД_5_03(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				НовыйЭД.ВерсияФормата = "ON_NSCHFDOPPR_1_997_01_05_03_01";
			КонецЕсли;
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_CHETOP") > 0 Тогда
			ВерсияФормата = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
			Если ВерсияФормата = "5.01" Тогда
				ФорматыЭДО_ФНС.ПрочитатьСчетНаОплату_5_01_XDTO(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				НовыйЭД.ВерсияФормата = "ON_CHETOP_1_998_01_05_01_01";
			ИначеЕсли ВерсияФормата = "1.01" Тогда
	...
	Результат.Вставить("НовыйЭД", НовыйЭД);
	Результат.Вставить("Содержание", Неопределено);
	Результат.Вставить("ДанныеТЭДО", Неопределено);
...
// Удаляет объявление пространства имен в теле документа
//
// Параметры:
...
//  ДвоичныеДанные
//
Функция УдалитьОбъявлениеПространстваИменВТелеДокумента(Знач ДвоичныеДанныеФайла, Знач Кодировка, Знач URIПространстваИмен)
	...
	ПараметрыЧтенияXML = Новый ПараметрыЧтенияXML(,,,,,,,, Ложь,, Истина);
	ЧтениеXML.ОткрытьПоток(ПотокДляЧтения, ПараметрыЧтенияXML, , Кодировка);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента(ИмяТекущегоЭлемента);
			Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
				Если Лев(ЧтениеXML.Имя, 5) = "xmlns" 
					И Пакет.КорневыеСвойства.Получить(ИмяТекущегоЭлемента) = Неопределено
					И ФабрикаXDTO.Пакеты.Получить(ЧтениеXML.Значение) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
...
Функция ТипКорневогоУзла(URIПространстваИмен)
	...
	Если URIПространстваИмен = "ON_AKTSVEROTP_1_972_01_05_01_01" Или URIПространстваИмен = "ON_AKTSVERPOL_1_972_02_05_01_01"
		...
		Или URIПространстваИмен = "ON_NSCHFDOPPR_1_997_01_05_03_01" Или URIПространстваИмен = "ON_NSCHFDOPPOK_1_997_02_05_03_01"
	...
	ИначеЕсли URIПространстваИмен = "ON_CHETOP_1_998_01_01_01"
		Или URIПространстваИмен = "ON_CHETOP_1_998_01_05_01_01" Тогда
		ВерсияСхемы = "4.01";
...
Процедура СформироватьТЗПоНаборуДанныхXDTO(ТЗ, НаборДанных, ДопПараметры = Неопределено) 
	...
	Для Каждого ТекЭлемент Из НаборДанных Цикл
		...
		Если ИнфПолФХЖ2 = Неопределено Тогда
			ИнфПолФХЖ2 = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Элемент, "ИнфПол", , , Истина);
		КонецЕсли;
		Если ИнфПолФХЖ2 <> Неопределено Тогда
			...
			Для Каждого ДопИнформация Из ИнфПолФХЖ2 Цикл
				// В пространствах имен "ON_CHETOP_1_998_01_01_01" и "ON_CHETOP_1_998_01_05_01_01" тип "ИнфПолТип"
				...
				Если ТекстИнф <> Неопределено Тогда 
					...
					Для Каждого ЭлементСписка Из ТекстИнф Цикл
						Если ИмяПоляЗначения = "" Тогда
							ЗначениеЭлемента = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭлементСписка, "Значение");
							ИмяПоляЗначения = ?(ЗначениеЭлемента = Неопределено, "Значен", "Значение");
						КонецЕсли;
						Если Лев(ЭлементСписка.Идентиф, ДлинаПрефикса) = Префикс Тогда
							ИмяСвойства = Сред(ЭлементСписка.Идентиф, ДлинаПрефикса + 1);
							Сопоставление.Вставить(ИмяСвойства, ЭлементСписка[ИмяПоляЗначения]);
						КонецЕсли;
					КонецЦикла;
				Иначе	
		...
		Если ТекстИнф <> Неопределено Тогда 
			...
			Для Каждого ЭлементСписка Из ТекстИнф Цикл
				...
				Если ВРег(ЭлементСписка.Идентиф) = ВРег(ИмяРеквизита) Тогда
					Результат = ЭлементСписка[ИмяПоляЗначения];
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
#Область ЗаполнениеДанныхУчастниковЭДО

// Возвращает, доступно ли заполнение данных участников ЭДО
// 
// Параметры:
//  Формат - Строка
// 
// Возвращаемое значение:
//  Булево
Функция ЗаполнениеДанныхУчастниковЭДОДоступно(Формат) Экспорт
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	Возврат Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПокупателя
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПокупателя
		Или Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Или Формат = ПоддерживаемыеФорматы.ФНС.ТорговаяОперация.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.ТорговаяОперация.ИнформацияПокупателя
		Или Формат = ПоддерживаемыеФорматы.ФНС.ПриемкаСдачаРабот.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.ПриемкаСдачаРабот.ИнформацияПокупателя
		Или Формат = ПоддерживаемыеФорматы.ФНС.СчетФактура
		Или Формат = ПоддерживаемыеФорматы.ФНС.КорректировочныйСчетФактура
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПокупателя;
	
КонецФункции

// Возвращает имена полей xml-файлов сведений участников ЭДО по формату
// 
// Параметры:
//  Формат - Строка
// 
// Возвращаемое значение:
//  Структура:
//    * СведенияУчастников - Строка
//    * ИдентификаторОтправителя - Строка
//    * ИдентификаторПолучателя - Строка
//    * СведенияОператора - Строка
//    * НаименованиеОператора - Строка
//    * ИННОператора - Строка
...
Функция ИменаПолейСведенийУчастниковЭДО(Формат)
	...
	ИменаПолей.Вставить("ИдентификаторФайла", "ИдФайл");
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	Если Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ИнформацияПокупателя Тогда
	...
	ИначеЕсли  Формат = ПоддерживаемыеФорматы.ФНС.ПриемкаСдачаРабот.ИнформацияПокупателя Тогда
		...
		ИменаПолей.СведенияОператора = "СвОЭДОтпрСФ";
		ИменаПолей.ИдентификаторОператора = "ИдЭДОСФ";
...
// Возвращает, является ли указанный формат УПД 5.03
// 
// Параметры:
//  Формат - Строка - Имя формата, см. ПоддерживаемыеФорматы
// 
// Возвращаемое значение:
//  Булево - Это УПД 5 03
Функция ЭтоУПД_5_03(Формат)
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	Возврат Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПокупателя;
	
КонецФункции

// Возвращает описание файла УПД 5.03 с заполненным согласно регламенту идентификатором файла
// 
// Параметры:
//  ДанныеФайла - ОбъектXDTO
//  Формат - Строка - Имя формата, см. ПоддерживаемыеФорматы
//  ДанныеУчастниковЭДО - см. НовыеДанныеУчастниковЭДО
// 
// Возвращаемое значение:
//  см. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
Функция ОписаниеФайлаУПД_5_03_СЗаполненнымИдентификаторомФайла(ДанныеФайла, Формат, ДанныеУчастниковЭДО)
	
	Результат = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
	
	ИдентификаторФайлаОригинальный = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайла,
		ИмяПоляИдентификатораФайлаУПД_5_03());
	
	ПостфиксИдентификатора = ПостфиксИдентификатораУПД_5_03(ИдентификаторФайлаОригинальный);
	ПрефиксИдентификатора = ПрефиксИдентификатораФайлаУПД_5_03(Формат);
	
	ИдентификаторФайла = СтрШаблон("%1_%2_%3_%4",
		ПрефиксИдентификатора,
		ДанныеУчастниковЭДО.ИдентификаторПолучателя,
		ДанныеУчастниковЭДО.ИдентификаторОтправителя,
		ПостфиксИдентификатора);
	
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайла,
		ИмяПоляИдентификатораФайлаУПД_5_03(), ИдентификаторФайла);
	Результат.ИмяФайла = СтрШаблон("%1.xml", ИдентификаторФайла);
	
	ПространствоИмен = ДанныеФайла.Тип().URIПространстваИмен;
	
	ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайла, Ложь, "windows-1251", "Файл");
	ДвоичныеДанные = ОбщегоНазначенияБЭД.УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИмен);
...
// Имя xml-поля идентификатора файла УПД 5.03.
// 
// Возвращаемое значение:
//  Строка
Функция ИмяПоляИдентификатораФайлаУПД_5_03()
	
	Возврат "ИдФайл";
	
КонецФункции

// Префикс идентификатора файла и имени файла, по формату УПД 5.03
// 
// Параметры:
//  Формат - Строка - см. ПоддерживаемыеФорматы
// 
// Возвращаемое значение:
//  Строка
Функция ПрефиксИдентификатораФайлаУПД_5_03(Формат)
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	Если Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
		Возврат "ON_NSCHFDOPPR";
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПокупателя Тогда
...
// Возвращает постфикс индентификатора УПД 5.03, не зависящий от настроек отправки
//  Из идентификатора УПД R_Т_A_О_GGGGMMDD_N1_N2_N3_N4_N5_N6_N7
//  возвращает GGGGMMDD_N1_N2_N3_N4_N5_N6_N7
// 
// Параметры:
//  Идентификатор - Строка - Идентификатор из описания ФНС в формате R_Т_A_О_GGGGMMDD_N1_N2_N3_N4_N5_N6_N7
// 
// Возвращаемое значение:
//  Строка - Постфикс, от входного идентификатора GGGGMMDD_N1_N2_N3_N4_N5_N6_N7
Функция ПостфиксИдентификатораУПД_5_03(Идентификатор)
	
	ЧастиИдентификатора = СтрРазделить(Идентификатор, "_");
	
	Если ЧастиИдентификатора.Количество() < 12 Тогда
		ВызватьИсключение НСтр("ru = 'Неверный формат идентификатора УПД 5.03.'")
	КонецЕсли;
	
	// Считаем с конца, потому что выгрузка в файл не для обмена ЭДО генериует идентификаторы участников как <ИНН>_<КПП>
	Н7 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 1];
	Н6 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 2];
	Н5 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 3];
	Н4 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 4];
	Н3 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 5];
	Н2 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 6];
	Н1 = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 7];
	ДатаФормирования = ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 8];
	
	Возврат СтрШаблон("%1_%2_%3_%4_%5_%6_%7_%8", ДатаФормирования, Н1, Н2, Н3, Н4, Н5, Н6, Н7);
...
// Новые данные сравнения содержимого формализованных файлов
// 
// Возвращаемое значение:
//  Структура - Новые данные сравнения:
// * ВИсходномДокументе - Строка
// * ВЗагружаемомДокументе - Строка
Функция НовыеДанныеСравнения()
	Результат = Новый Структура;
	Результат.Вставить("ВИсходномДокументе", "");
	Результат.Вставить("ВЗагружаемомДокументе", "");
	Возврат Результат;
КонецФункции

// @skip-check constructor-function-return-section
// Проверяет имя файла по содержимому файла и по данным документа ЭДО.
// 
// Параметры:
//  ДанныеДокумента - Структура:
//                    * Ссылка - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
...
//                    * ИдентификаторОрганизации - Строка
//                    * ИдентификаторКонтрагента - Строка
//  ЗагружаемыеДанные - см. ДанныеФайлаЭД
//  ЗагруженныеДанные - см. ДанныеФайлаЭД
//  СодержаниеДокумента - см. ПрочитатьСодержаниеДокумента
// 
// Возвращаемое значение:
//  Структура:
//   *ИмяФайлаКорректное - Булево
//   *КорректировкаВозможна - Булево
//   *НовоеИмяФайла - Строка
Функция ПроверкаИмениФайла(ДанныеДокумента, ЗагружаемыеДанные, ЗагруженныеДанные, СодержаниеДокумента)
	
	ИмяФайла = ЗагружаемыеДанные.ИдФайл;
	Результат = Новый Структура;
	Результат.Вставить("ИмяФайлаКорректное", Истина);
	Результат.Вставить("КорректировкаВозможна", Истина);
	Результат.Вставить("НовоеИмяФайла", ИмяФайла);

	Если СодержаниеДокумента.ТипРегламента <> Перечисления.ТипыРегламентовЭДО.УПД
		Или СодержаниеДокумента.Формат <> ФорматыЭДО_ФНС.ПоддерживаемыеФорматы().УПД_5_03.ИнформацияПродавца Тогда
			
			Возврат Результат;
	КонецЕсли;
	Если ЗагруженныеДанные.ИдФайл = ЗагружаемыеДанные.ИдФайл Тогда
		Возврат Результат;
	КонецЕсли;
	
	Организация = ИнтеграцияЭДО.ОрганизацияПоКлючевымРеквизитам(СодержаниеДокумента.Отправитель);
	Если Не ЗначениеЗаполнено(Организация) Или Организация <> ДанныеДокумента.Организация Тогда
		Результат.КорректировкаВозможна = Ложь;
		Возврат Результат;
	КонецЕсли;
	МассивКонтрагентов = ИнтеграцияЭДО.КонтрагентыПоКлючевымРеквизитам(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		СодержаниеДокумента.Получатель)).ВыгрузитьКолонку("Ссылка");
	
	Если МассивКонтрагентов.Найти(ДанныеДокумента.Контрагент) = Неопределено Тогда
		Результат.КорректировкаВозможна = Ложь;
		Возврат Результат;
	КонецЕсли;

	ЧастиИмениФайла = СтрРазделить(ИмяФайла, "_");
	Если ЧастиИмениФайла.Количество() = 14 Тогда
		ЧастиИмениФайла[2] = СтрШаблон("%1-%2", ЧастиИмениФайла[2], ЧастиИмениФайла[3]);
		ЧастиИмениФайла[3] = СтрШаблон("%1-%2", ЧастиИмениФайла[4], ЧастиИмениФайла[5]);
		ЧастиИмениФайла.Удалить(4);
		ЧастиИмениФайла.Удалить(4);
	КонецЕсли;
	
	Если ЧастиИмениФайла.Количество() <> 12 Тогда
		Результат.ИмяФайлаКорректное = Ложь;
		Результат.КорректировкаВозможна = Ложь;
		Результат.НовоеИмяФайла = "";
		Возврат Результат;
	КонецЕсли;
	
	Результат.ИмяФайлаКорректное = ЧастиИмениФайла[3] = ДанныеДокумента.ИдентификаторОрганизации 
		И ЧастиИмениФайла[2] = ДанныеДокумента.ИдентификаторКонтрагента;

	ЧастиИмениФайла[3] = ДанныеДокумента.ИдентификаторОрганизации;
	ЧастиИмениФайла[2] = ДанныеДокумента.ИдентификаторКонтрагента;
	
	Если СодержаниеДокумента.Свойство("ЕстьПрослеживаемость") И Не СодержаниеДокумента.ЕстьПрослеживаемость Тогда
		Результат.ИмяФайлаКорректное = Результат.ИмяФайлаКорректное И ЧастиИмениФайла[6] = "0";
		ЧастиИмениФайла[6] = "0";
	КонецЕсли;
	Если СодержаниеДокумента.Свойство("ЕстьМаркировка") И Не СодержаниеДокумента.ЕстьМаркировка Тогда
		Результат.ИмяФайлаКорректное = Результат.ИмяФайлаКорректное И ЧастиИмениФайла[7] = "0" 
			И ЧастиИмениФайла[8] = "0" И ЧастиИмениФайла[9] = "0";
		ЧастиИмениФайла[7] = "0";
		ЧастиИмениФайла[8] = "0";
		ЧастиИмениФайла[9] = "0";
	КонецЕсли;
	Если СодержаниеДокумента.Свойство("ЕстьНефтепродукты") И Не СодержаниеДокумента.ЕстьНефтепродукты Тогда
		Результат.ИмяФайлаКорректное = Результат.ИмяФайлаКорректное И ЧастиИмениФайла[10] = "0";
		ЧастиИмениФайла[10] = "0";
	КонецЕсли;
	Результат.НовоеИмяФайла = СтрСоединить(ЧастиИмениФайла, "_");
	Возврат Результат;
КонецФункции
...
Функция СоответствиеТитулов()
	...
	СоответствиеТитулов.Вставить(Форматы.ФНС.УПД_5_03.ИнформацияПродавца, Форматы.ФНС.УПД_5_03.ИнформацияПокупателя);
	...
	ТрансграничныйЭДО.ДополнитьСоответствиеТитулов(СоответствиеТитулов);
...
Функция РазобратьУниверсальныйДопФайл(ДанныеФайла)
	...
	ОбъектXML = Новый ЧтениеXML;
	Поток = РаботаСФайламиБЭД.НовыйПотокЧтенияНормализованногоXML(ДанныеФайла.ДвоичныеДанные);
...
//   * ЕдиницаИзмеренияКод - Строка
//   * Артикул - Строка
//   * СтавкаНДС - см. ИнтеграцияЭДО.СтавкаНДСИзСоответствия
//   * ШтрихкодКомбинации - Строка
//   * ШтрихкодыНоменклатуры - Строка, Массив Из Строка -
//   * НоменклатураИБ - Неопределено, ОпределяемыйТип.НоменклатураБЭД -
//   * ХарактеристикаИБ - Неопределено, ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД -
//   * УпаковкаИБ - Неопределено, ОпределяемыйТип.УпаковкаНоменклатурыБЭД -
//
Функция ПодготовитьСопоставлениеНоменклатурыДляДереваДокумента(Знач Сопоставление) Экспорт
	...
	Если ЗначениеЗаполнено(НоменклатураИБ.Номенклатура) Тогда
		...
		Возврат Результат;
	Иначе
		Результат.НоменклатураИБ = Неопределено;
		Результат.ХарактеристикаИБ = Неопределено;
		Результат.УпаковкаИБ = Неопределено;
	КонецЕсли;
...
// Функция проверяет наличие в пакете XDTO элементов с типом "Смешанный".
// 
// Параметры:
...
//  Булево
//
Функция ПакетXDTOСодержитMixedContent(ПространствоИменСхемы)
	...
	Коллекция = Пакет.КорневыеСвойства;
	
	ЕстьСмешанныйТип = ПроверитьСвойстваЭлементовНаНаличиеMixedContent(Коллекция);
	
	Возврат ЕстьСмешанныйТип;
	
КонецФункции

// Проверяет содержит ли коллекция элементов схемы, элементы со свойством "Смешанный".
// 
// Параметры:
//  КоллекцияСвойств - КоллекцияСвойствXDTO
//  ПроверенныеЭлементы - Неопределено, Соответствие из Строка - Коллекция имен уже проверенных элементов
// 
// Возвращаемое значение:
//  Булево
Функция ПроверитьСвойстваЭлементовНаНаличиеMixedContent(КоллекцияСвойств, ПроверенныеЭлементы = Неопределено)
	
	Если ПроверенныеЭлементы = Неопределено Тогда
		ПроверенныеЭлементы = Новый Соответствие();
	КонецЕсли;
	
	ЕстьСмешанныйТип = Ложь;
	
	Для Каждого Элемент Из КоллекцияСвойств Цикл
		
		Если Не ПроверенныеЭлементы.Получить(Элемент.Имя) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ТипЗнч(Элемент.Тип) = Тип("ТипЗначенияXDTO") Тогда
			Продолжить;
		КонецЕсли;
		
		ПроверяемыйОбъект = Элемент.Тип;
		
		Если ПроверяемыйОбъект = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ЕстьСмешанныйТип = ПроверяемыйОбъект.Смешанный;
		
		ПроверенныеЭлементы.Вставить(Элемент.Имя, Истина);
		
		Если ПроверяемыйОбъект.Свойства.Количество() И Не ЕстьСмешанныйТип Тогда
			ЕстьСмешанныйТип = 
				ПроверитьСвойстваЭлементовНаНаличиеMixedContent(ПроверяемыйОбъект.Свойства, ПроверенныеЭлементы);
		КонецЕсли;
		
		Если ЕстьСмешанныйТип Тогда
			Прервать;
		КонецЕсли;
