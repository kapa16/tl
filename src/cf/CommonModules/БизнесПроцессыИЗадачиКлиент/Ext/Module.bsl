...
Процедура ПринятьЗадачуКИсполнению(Форма, ТекущийПользователь) Экспорт
	...
	Иначе
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Новый исполнитель: %1 (вместо %2)';
			|en = 'Accept the task with a change of assignee?
			|New assignee is %1, replacing %2'"), ТекущийПользователь, Исполнитель);
