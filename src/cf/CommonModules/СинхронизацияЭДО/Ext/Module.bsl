...
// Возвращает признак использования доверенностей при регистрации представителя организации.
//
// Параметры:
//  ОператорЭДО - Строка
// 
...
Функция ПриРегистрацииПредставителяТребуетсяМЧД(ОператорЭДО) Экспорт
	...
	Возврат РегистрыСведений.ОператорыЭДО.ПриРегистрацииПредставителяТребуетсяМЧД(ОператорЭДО);
...
Функция ОтправитьОбъекты(ЗапросОбъектов, КонтекстДиагностики, ОтпечаткиСертификатов, ВыбранныеСертификаты = Неопределено,
	...
	Если ЗначениеЗаполнено(РасшифрованныеМаркеры) Тогда
		ДанныеДляСинхронизации = СинхронизацияЭДОСлужебный.ДанныеДляСинхронизацииИзРасшифрованныхМаркеров(
			РасшифрованныеМаркеры, КонтекстДиагностики);
	Иначе
...
// * ПриоритетРаспаковки - Число - (служебное поле) значение показывающее порядок для распаковки. 0 - самый высокий.
...
Функция НовыеДанныеОбъектов() Экспорт
	...
	ДанныеОбъектов.Колонки.Добавить("ПриоритетРаспаковки", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(2)));

	ДанныеОбъектов.Колонки.Добавить("ДанныеТЭДО", Неопределено);
	
	Возврат ДанныеОбъектов;
	
КонецФункции

// Возвращает описание данных отправляемого объекта.
// 
// Возвращаемое значение:
// 	Структура:
// * ИмяФайла - Строка
...
Функция ДанныеОбъектаПоИдентификатору(ДанныеОбъектов, ИдентификаторСтроки) Экспорт
	...
	Возврат ДанныеОбъектов.Найти(ИдентификаторСтроки, "ИдентификаторСтроки");
...
// Возвращаемое значение:
//  Булево
Функция СвязьНастроенаПоИдентификаторамЭДО(ИдентификаторОрганизации, ИдентификаторКонтрагента) Экспорт
	
	Результат = Ложь;
	
	ПараметрыПриглашения = Новый Структура;
	ПараметрыПриглашения.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
	ПараметрыПриглашения.Вставить("ИдентификаторКонтрагента", ИдентификаторКонтрагента);
	ДанныеПриглашения = ПриглашенияЭДО.ДанныеПриглашенияПоПараметрам(ПараметрыПриглашения);
	Если ДанныеПриглашения <> Неопределено
		И ДанныеПриглашения.Статус = Перечисления.СтатусыПриглашений.Принято Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция НачатьПроверкуНастроекРегламентныхЗаданий(ИдентификаторФормы, Операция = "ПоУмолчанию") Экспорт
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"Обработки.СинхронизацияЭДО.ПроверитьНастройкиРегламентныхЗаданийЭДО", Операция);
...
// Возвращает дату последней проверки данных в сервисе настроек по переданному типу синхронизации.
// 
// Параметры:
//  ТипСинхронизации - ПеречислениеСсылка.ТипыСинхронизацииСервисаНастроекЭДО
// 
// Возвращаемое значение:
//  Дата - дата последнего успешного обращения к сервису настроек
//
Функция ДатаПоследнейПроверкиДанныхСервисаНастроек(ТипСинхронизации) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ДатаПоследнейПроверки = ВыборкаДетальныеЗаписи.ДатаПоследнейПроверки;
		
	КонецЦикла;
	
	Возврат ДатаПоследнейПроверки;
...
Функция ОбновитьКешиОператоровЭДОИФорматов(КонтекстДиагностики = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СостоянияСинхронизацииСервисНастроекЭДО.ДатаПоследнегоИзменения, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПоследнегоИзменения,
		|	ЕСТЬNULL(СостоянияСинхронизацииСервисНастроекЭДО.ДатаПоследнейПроверки, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПоследнейПроверки
		|ИЗ
...
Процедура ОбновитьФорматыИСвязьВидовФорматовИзСервисаНастроек() Экспорт
	...
	ДатаПроверкаАктуальности = Дата(1, 1, 1);
	
	СинхронизироватьФорматыЭлектронныхДокументовИзСервисаНастроек(СинхронизацияФорматовЭД, ДатаПроверкаАктуальности,
		КонтекстДиагностики);
	
	СинхронизироватьСвязьВидовИФорматовЭлектронныхДокументовИзСервисаНастроек(СвязьТиповИФорматовЭД,
		ДатаПроверкаАктуальности, КонтекстДиагностики);
...
//  * Значение - Булево
Функция ЕстьУчетнаяЗаписьПоОрганизациям(Организации) Экспорт
	Запросы = Новый Массив;
	Отбор = УчетныеЗаписиЭДО.НовыйОтборУчетныхЗаписей();
	Отбор.Организация = "&Организации";
	ЗапросУчетныхЗаписей = УчетныеЗаписиЭДО.ЗапросУчетныхЗаписей("УчетныеЗаписи", Отбор);
	...
	|	УчетныеЗаписи.ИдентификаторЭДО ЕСТЬ НЕ NULL КАК ЕстьУчетнаяЗапись
	|ИЗ
	|	Организации КАК Организации
	|	ЛЕВОЕ СОЕДИНЕНИЕ УчетныеЗаписи КАК УчетныеЗаписи
	|	ПО Организации.Ссылка = УчетныеЗаписи.Организация";
	ИтоговыйЗапрос = ОбщегоНазначенияБЭД.СоединитьЗапросы(Запрос, Запросы);
	ИтоговыйЗапрос.УстановитьПараметр("Организации", Организации);
	
	ТаблицаОрганизации = Новый ТаблицаЗначений;
	ТаблицаОрганизации.Колонки.Добавить("Ссылка", Метаданные.ОпределяемыеТипы.Организация.Тип);
	Для Каждого Ссылка Из Организации Цикл
		ТаблицаОрганизации.Добавить().Ссылка = Ссылка;
	КонецЦикла;
	
	ИтоговыйЗапрос.УстановитьПараметр("ТаблицаОрганизации", ТаблицаОрганизации);
	
	Выборка = ИтоговыйЗапрос.Выполнить().Выбрать();
	Результат = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		Результат.Вставить(Выборка.Организация, Выборка.ЕстьУчетнаяЗапись);
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьОписаниеДополнительныхДанныхДокумента(ДанныеДокумента, Знач ДополнительныеФайлы, Отказ) Экспорт
	...
	ФайлыОбъекта = ДополнительныеФайлы.НайтиСтроки(Новый Структура("ОбъектУчета", ДанныеДокумента.Объект));
	
	Если ФайлыОбъекта.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если ФайлыОбъекта.Количество() = 1 Тогда
		
		ДополнительныйФайл  = ФайлыОбъекта[0];
	...
	Иначе
		...
		ОбъектыДобавляемыеВАрхив = Новый Массив;
		Для Каждого ДополнительныйФайл Из ФайлыОбъекта Цикл
			
			ПутьКФайлу = АдресКаталога + ДополнительныйФайл.ИмяФайла;
			ТребуетсяПодкаталог = ОбъектыДобавляемыеВАрхив.Найти(ПутьКФайлу) <> Неопределено;
			Если ТребуетсяПодкаталог Тогда
				// Создаем подкаталог только для уже добавленных в корень каталога наименований файлов.
				АдресПодкаталога = АдресКаталога + Формат(НомерПодкаталога, "ЧГ=");
				...
				НомерПодкаталога = НомерПодкаталога + 1;
				
				ПутьКФайлу = АдресПодкаталога + ДополнительныйФайл.ИмяФайла;
				ОбъектыДобавляемыеВАрхив.Добавить(АдресПодкаталога);
			Иначе
				ОбъектыДобавляемыеВАрхив.Добавить(ПутьКФайлу);
			КонецЕсли;
		...
		ДвоичныеДанныеФайла = РаботаСФайламиБЭД.СформироватьАрхивФайлов(ОбъектыДобавляемыеВАрхив, , ПараметрыАрхива);
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики.
//
Процедура ПрочитатьДополнительныеДанныеДокумента(ДополнительныеФайлы, Знач ОписаниеДополнительныхДанных,
	...
	Попытка
		РезультатРаспаковки = РаботаСФайламиБЭД.РаспаковатьАрхив(ДвоичныеДанныеФайла, КаталогРаспаковки,
			КонтекстДиагностики, ВидОперации, ПараметрыРаспаковки);
	Исключение
	...
	Для Каждого СтрокаФайл Из РезультатРаспаковки Цикл
		ЭтоТипФайл = ТипЗнч(СтрокаФайл) = Тип("Файл");
		Если ЭтоТипФайл И Не СтрокаФайл.ЭтоФайл() Тогда
			Продолжить;
		КонецЕсли;
		ДополнительныйФайл = ДополнительныеФайлы.Добавить();
		...
		Если ЭтоТипФайл Тогда
			ДополнительныйФайл.ДвоичныеДанныеФайла = Новый ДвоичныеДанные(СтрокаФайл.ПолноеИмя);
		Иначе
			ДополнительныйФайл.ДвоичныеДанныеФайла = СтрокаФайл.ДвоичныеДанные;
		КонецЕсли;
	КонецЦикла;
...
Функция ВидОшибкиЕстьНеполученныеЭлектронныеДокументы() Экспорт
	...
	ВидОшибки.ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Выберите</a> дальнейшее действие'"), "Выберите");
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаЕстьНеполученныеЭлектронныеДокументы(Ошибка) Экспорт
	
	Возврат Ошибка.ВидОшибки.Идентификатор = "ЕстьНеполученныеЭлектронныеДокументы";
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаЕстьНеполученныеПриглашения(Ошибка) Экспорт
	
	Возврат Ошибка.ВидОшибки.Идентификатор = "ЕстьНеполученныеПриглашения";
...
Процедура СинхронизироватьПереченьСпециальныхОбстоятельств(ТипСинхронизации,
	ДатаПоследнегоИзменения, КонтекстДиагностики) Экспорт //АПК:581 Избыточное ключевое слово "Экспорт".
...
Функция РасшифрованныеМаркерыИзКлючейСинхронизации(КлючиСинхронизации) Экспорт
...
Функция ЗашифрованныеМаркерыИзКлючейСинхронизации(КлючиСинхронизации) Экспорт
	...
	Для Каждого КлючСинхронизацииУчетнойЗаписи Из КлючиСинхронизации Цикл
		КлючСинхронизации = КлючСинхронизацииУчетнойЗаписи.Значение;
		...
		Для Каждого ДанныеСертификата Из Сертификаты Цикл
			Если ДанныеСертификата.МаркерЗашифрованный = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ЗашифрованныеДанные = КриптографияБЭДКлиентСервер.НовыеДанныеРасшифровки();
			ЗашифрованныеДанные.Сертификат = ДанныеСертификата.Ссылка;
			ЗашифрованныеДанные.Данные = ДанныеСертификата.МаркерЗашифрованный;
			НаборДанных.Добавить(ЗашифрованныеДанные);
		КонецЦикла;
		Если ЗначениеЗаполнено(НаборДанных) Тогда
			ЗашифрованныеМаркеры.Вставить(КлючСинхронизацииУчетнойЗаписи.Ключ, НаборДанных);
		КонецЕсли;
	КонецЦикла;
...
Функция КлючиСинхронизацииИзРасшифрованныхМаркеров(РасшифрованныеМаркеры) Экспорт
	...
	ИдентификаторыЭДО = ОбщегоНазначения.ВыгрузитьКолонку(РасшифрованныеМаркеры, "Ключ");
	...
	Для Каждого РасшифрованныйМаркер Из РасшифрованныеМаркеры Цикл
		...
		КлючСинхронизации.ВыбранныйСертификат = РасшифрованныеДанные.Сертификат;
		КлючСинхронизации.МаркерРасшифрованный = РасшифрованныеДанные.Данные;
		
		ДанныеСертификата = НовыеДанныеСертификата();
		ДанныеСертификата.Ссылка = РасшифрованныеДанные.Сертификат;
		ДанныеСертификата.МаркерРасшифрованный = РасшифрованныеДанные.Данные;
...
//  РасшифрованныйМаркер - см. КриптографияБЭДКлиентСервер.НовыеДанныеРасшифровки
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  Булево
Функция ПолучитьНовыйТокенДоступаКСервисуЭДО(ИдентификаторЭДО, РасшифрованныйМаркер, КонтекстДиагностики) Экспорт
	
	ТокенАвторизации = СервисЭДО.ПолучитьТокенАвторизации(ИдентификаторЭДО,
		РасшифрованныйМаркер.Данные, КонтекстДиагностики);
...
// Возвращаемое значение:
//  См. СинхронизацияЭДОКлиентСервер.НовыеКлючиСинхронизации
Функция КлючиСинхронизацииПоТокенамДоступаКСервисуЭДО(ИдентификаторыЭДО, КонтекстДиагностики) Экспорт
	...
	ТокеныПоИдентификаторамЭДО = СервисЭДО.ТокеныАвторизацииИзБезопасногоХранилища(ИдентификаторыЭДО);
	
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		
		Токен = ТокеныПоИдентификаторамЭДО[ИдентификаторЭДО];
		Если Токен = Неопределено Тогда
			ВидОперации = НСтр("ru = 'Подготовка запроса в сервис ЭДО'");
			ТекстОшибки = НСтр("ru = 'Отсутствует токен доступа для авторизации в сервисе ЭДО'");
			ВидОшибки = ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ИдентификаторЭДО);
		...
		КлючСинхронизации.ТокенВалиден = Истина;
		КлючСинхронизации.Токен = Токен;
...
// Параметры:
//  ИдентификаторыЭДО - Массив из Строка
Процедура УдалитьТокеныДоступаКСервисуЭДО(ИдентификаторыЭДО) Экспорт
	
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		СервисЭДО.УдалитьТокенАвторизацииИзБезопасногоХранилища(ИдентификаторЭДО);
	КонецЦикла;
...
// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ИдентификаторУчетнойЗаписи) Экспорт
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "НекорректныйТокенАвторизации";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Ошибка авторизации в сервисе ЭДО'");
	ВидОшибки.ОписаниеПроблемы =
		НСтр("ru = 'Сервис 1С-ЭДО не смог распознать предоставленный токен как допустимый для авторизации'");
	ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Авторизоваться по сертификату</a> и повторить операцию.'"), "Авторизоваться");
	ВидОшибки.ОписаниеРешения = ОписаниеРешения;
	ВидОшибки.ОбработчикиНажатия.Вставить("Авторизоваться",
		"СервисЭДОКлиент.ОбработатьОшибкуНекорректныйТокенДоступаКСервисуЭДО");
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ИдентификаторУчетнойЗаписи", ИдентификаторУчетнойЗаписи);
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаНекорректныйТокенДоступаКСервисуЭДО(Ошибка) Экспорт
	Возврат Ошибка.ВидОшибки.Идентификатор = "НекорректныйТокенАвторизации";
КонецФункции
...
Процедура СинхронизироватьСвязьВидовИФорматовЭлектронныхДокументовИзСервисаНастроек(ТипСинхронизации,
	...
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	ПараметрыВыполнения.КлючФоновогоЗадания = "ОбновитьФорматПоУмолчаниюВНастройках";
	ПараметрыПроцедуры = Новый Структура;
	// См. НастройкиЭДО.ОбновитьФорматПоУмолчаниюВНастройках
	ДлительныеОперации.ВыполнитьВФоне("НастройкиЭДО.ОбновитьФорматПоУмолчаниюВНастройках",
...
Функция ВидОшибкиОтсутствуетСертификатДляПодписания()
	...
	ШаблонОписанияРешения = НСтр("ru = '<a href = ""%1"">Проверьте</a> настройки электронных подписей.'");
	ОписаниеРешения = СтроковыеФункции.ФорматированнаяСтрока(СтрШаблон(ШаблонОписанияРешения, "Проверьте"));
	ВидОшибки.ОписаниеРешения = ОписаниеРешения;
