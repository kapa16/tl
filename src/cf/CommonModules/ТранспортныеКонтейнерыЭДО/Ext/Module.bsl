...
Функция СохранитьКонтейнер(ОписаниеКонтейнера, КлючСинхронизации, КонтекстДиагностики, ИмяБезРасширения = "") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПослеИнициализацииОсновныхПараметровВходящегоКонтейнера(ПараметрыКонтейнера, ОписаниеКонтейнера);
	КонецЕсли;
	...
	// Запись контейнера с минимальным набором реквизитов. Главная задача на данном этапе - сохранить контейнер в ИБ
	// до распаковки и заполнения данных.
	НачатьТранзакцию();
	Попытка
	...
	Попытка
		...
		ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ОписаниеКонтейнера.ДвоичныеДанные,
	...
	Исключение
		СтатусКонтейнераНеизвестный = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный;
		ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, СтатусКонтейнераНеизвестный,
			КонтекстДиагностики, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Исключает из массива контейнеры, с неизвестным статусом.
// 
// Параметры:
//  Контейнеры - Массив из ДокументСсылка.ТранспортныйКонтейнерЭДО
Процедура ИсключитьНеизвестныеКонтейнеры(Контейнеры) Экспорт
	...
	|	И НЕ ТранспортныйКонтейнерЭДО.Статус = &Статус";
	
	Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный);
	...
	Контейнеры = Результат.Выгрузить().ВыгрузитьКолонку("Ссылка");
...
Функция ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, Статус, КонтекстДиагностики = Неопределено, 
	...
	Попытка
		...
		Если Не ПустаяСтрока(Комментарий) Тогда
			Если ПустаяСтрока(КонтейнерОбъект.Комментарий) Тогда
				КонтейнерОбъект.Комментарий = Комментарий;
			ИначеЕсли Комментарий <> КонтейнерОбъект.Комментарий Тогда
...
Функция РаспаковатьКонтейнеры(Контейнеры, Отпечатки, КонтекстДиагностики) Экспорт
	...
	// Дополняем связанными контейнерами, для случаев неправильного порядка прихода пакетов.
	ДополнитьСвязаннымиКонтейнерамиИУстановитьСтатусКРаспаковке(Контейнеры, КонтекстДиагностики);
...
//  * ДанныеТЭДО - см. ТрансграничныйЭДО.ДанныеКонтейнера
// 
Функция РаспаковатьФайлКонтейнера(Файл, КонтекстДиагностики = Неопределено) Экспорт
	...
	ОписаниеКонтейнера.Вставить("ДанныеТЭДО", Неопределено);
	...
	Если РезультатРаспаковки = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при распаковке архива транспортного контейнера ЭДО'");
	КонецЕсли;
	
	РаспакованныеФайлы = РезультатРаспаковки;
	
	Для каждого РаспакованныйФайл Из РаспакованныеФайлы Цикл
		Если ТипЗнч(РаспакованныйФайл) = Тип("Структура") Тогда
			Если НРег(РаспакованныйФайл.Имя) = "card.xml" Тогда
				ОписаниеКонтейнера.Карточка = ПрочитатьКарточкуЭлектронногоДокумента(
					РаспакованныйФайл.ДвоичныеДанные.ОткрытьПотокДляЧтения());
			КонецЕсли;
			Если НРег(РаспакованныйФайл.Имя) = "meta.xml" Тогда
				ОписаниеКонтейнера.Метаданные = 
					ФайлыКонтейнераПоФайлуМетаданных(РаспакованныйФайл.ДвоичныеДанные, РаспакованныеФайлы);
			КонецЕсли;
			Если НРег(РаспакованныйФайл.Имя) = "errormessage.xml" Тогда
				ОписаниеКонтейнера.Ошибка = ПрочитатьФайлОшибки(РаспакованныйФайл.ДвоичныеДанные.ОткрытьПотокДляЧтения());
			КонецЕсли;
			Если НРег(РаспакованныйФайл.Расширение) = ".zip" Тогда
				...
				ОписаниеКонтейнера.Картинки.ДвоичныеДанные = РаспакованныйФайл.ДвоичныеДанные;
			КонецЕсли;
		Иначе
	...
	Для каждого РаспакованныйФайл Из РаспакованныеФайлы Цикл
		Если НРег(РаспакованныйФайл.Имя) = НРег(ИмяФайлаКарточкиМЧД)
			...
			Если ТипЗнч(РаспакованныйФайл) = Тип("Структура") Тогда
				ОписаниеКонтейнера.Доверенности = 
					ДанныеДоверенностиПоХэшамПодписиИзРаспакованныхФайлов(РаспакованныйФайл, РаспакованныеФайлы); //АПК:1036 - Орфографическая ошибка в тексте модуля
			Иначе
	...
	ОписаниеКонтейнера.ДанныеТЭДО = ТрансграничныйЭДО.ДанныеКонтейнера(РаспакованныеФайлы, ОписаниеКонтейнера);
	...
	Если ОписаниеКонтейнера.Карточка = Неопределено Или ОписаниеКонтейнера.Метаданные = Неопределено Тогда
		...
		|Отсутствует один из служебных файлов card.xml или meta.xml'");
	КонецЕсли;
	
	ТрансграничныйЭДО.ДополнитьОписаниеКонтейнера(ОписаниеКонтейнера);
...
Процедура ОбработатьОшибкуЗагрузкиОбъекта(ДанныеОбъекта, КонтекстДиагностики, ТекстОшибки, МожноОтказатьсяОтЗагрузки = Ложь) Экспорт
	...
	Если ДанныеОбъекта.Отказ Тогда
		...
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
		ИзменитьСтатусКонтейнера(ДанныеОбъекта.ТранспортныйКонтейнер,
			Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный, КонтекстДиагностики, ТекстОшибки);
	КонецЕсли;
...
Функция ЗаполнитьТранспортныйКонтейнер(ПараметрыКонтейнера, Знач ТранспортныйКонтейнер = Неопределено) Экспорт
	...
	Попытка
		...
		Если ТранспортныйКонтейнерОбъект.ЭтоНовый()
			...
			ДанныеЗаполнения.Вставить("ТребуетсяПодтвердитьПолучение", Ложь);
		КонецЕсли;
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
			МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
			МодульОбменСГИСЭПД.ПриЗаполненииВходящегоКонтейнера(ПараметрыКонтейнера, ДанныеЗаполнения);
		КонецЕсли;
...
// Возвращает данные входящего контейнера.
// 
// Параметры:
//  ДвоичныеДанные      - ДвоичныеДанные - данные присоединенного файла транспортного контейнера
//  РезультатРаспаковки - Неопределено
//                      - см. РаспаковатьФайлКонтейнера
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  Структура - Данные входящего объекта:
// * ТипДокумента - Строка
// * ИдентификаторСообщения - Строка
...
// * ДанныеТЭДО - см. ТрансграничныйЭДО.ДанныеКонтейнера
// 
Функция ДанныеВходящегоОбъекта(ДвоичныеДанные, РезультатРаспаковки, КонтекстДиагностики)
	...
	Если Не ЗначениеЗаполнено(РезультатРаспаковки) Тогда
	...
	ДанныеКонтейнера.Вставить("ДанныеТЭДО", РезультатРаспаковки.ДанныеТЭДО);
...
// 	РезультатРаспаковки - см. РаспаковатьФайлКонтейнера
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанные, РезультатРаспаковки, КонтекстДиагностики)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриЗаполненииПараметровВходящегоКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки);
	КонецЕсли;
...
Процедура ОтложитьРаспаковкуКонтейнеровПоТранзакциям(ПараметрыКонтейнера, РезультатРаспаковки)
	...
	Если ДокументКонтейнера.КодРегламента = "Invoice"
		...
		ИЛИ ДокументКонтейнера.КодТранзакции = "TracingCancellationRejected"
		ИЛИ ДокументКонтейнера.КодТранзакции = "TracingAcceptedMod"
		ИЛИ ДокументКонтейнера.КодТранзакции = "TracingRejectedMod") Тогда
...
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	...
	Карточка.Вставить("ДатаДокумента", Дата(1, 1, 1));
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Возникла ошибка при чтении данных файла card.xml: %2.'");
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИмяФайла,
	...
	Если ДополнительныеПараметры.Свойство("ДатаДокумента") Тогда
		Попытка
			Карточка.ДатаДокумента = XMLЗначение(Тип("Дата"), ДополнительныеПараметры.ДатаДокумента);
		Исключение
			Карточка.ДатаДокумента = Дата(1, 1, 1);
			ШаблонПредупреждения = НСтр("ru = 'Выполнение операции: %1
										|%2'");
			ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, ВидОперации, ПодробноеПредставлениеОшибки);
	...
	Если ДополнительныеПараметры.Свойство("Сумма") Тогда
		Если ТипЗнч(ДополнительныеПараметры.Сумма) = Тип("Массив")
			И ДополнительныеПараметры.Сумма.Количество() Тогда
			Карточка.Сумма = ДополнительныеПараметры.Сумма[0];
		Иначе
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			Если Документ.Files.MainImageSignature <> Неопределено Тогда
				...
				Для Каждого Подпись Из Документ.Files.MainImageSignature Цикл
					...
					Иначе
						ПутьПеречитанногоФайла = ПолноеИмяФайлаЭДВКонтейнереЧтениемАрхива(КаталогРаспаковки,
							Подпись.Path, ДвоичныеДанныеКонтейнера);
						Если ПутьПеречитанногоФайла = Неопределено Тогда
							// Проблема распаковки файла. Ищем по расширению, исключая служебные файлы.
							ПутьПеречитанногоФайла = ПолноеИмяФайлаЭДВКонтейнереПоРасширению(КаталогРаспаковки, Документ.Files,
								СтруктурированноеИмяФайла.Расширение);
						КонецЕсли;
						ТекФайл = Новый Файл(ПутьПеречитанногоФайла);
						Если ТекФайл.Существует() Тогда
							ФайлыВТИ.Добавить(ТекФайл);
						Иначе
							ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка чтения файла подписи %1 из архива'"), 
								Подпись.Path);
			...
			Если Документ.Files.DataImage <> Неопределено Тогда
				...
				Исключение
					ПутьПеречитанногоФайла = ПолноеИмяФайлаЭДВКонтейнереЧтениемАрхива(КаталогРаспаковки,
						Документ.Files.DataImage.Path, ДвоичныеДанныеКонтейнера);
					Если ПутьПеречитанногоФайла = Неопределено Тогда
						ВызватьИсключение;
					Иначе
						ОписаниеДокументаКонтейнера.ОписаниеДополнительныхДанных.ИмяФайла = 
							СтруктурированноеИмяФайла.Имя;
						ОписаниеДокументаКонтейнера.ОписаниеДополнительныхДанных.ДвоичныеДанные = 
							Новый ДвоичныеДанные(СтруктурированноеИмяФайла.ПолноеИмя);
					КонецЕсли;
				КонецПопытки;
				Если Документ.Files.DataImageSignature <> Неопределено Тогда
					Для Каждого Подпись Из Документ.Files.DataImageSignature Цикл
						...
						Если Не ТекФайл.Существует() Тогда
							...
							ТекФайл = Новый Файл(ПутьПеречитанногоФайла);
							Если ТекФайл.Существует() Тогда
								СтруктурированноеИмяФайла.ПолноеИмя = ПутьПеречитанногоФайла;
							Иначе
								ТекстОшибки = СтрШаблон( 
									НСтр("ru = 'Ошибка чтения файла подписи %1 дополнительных данных из архива'"),
									Подпись.Path);
								ВызватьИсключение ТекстОшибки;
							КонецЕсли;
						КонецЕсли;
...
// 
// Параметры:
// 	РаспакованныйФайл - ДвоичныеДанные
// 	РаспакованныеФайлы - Массив из Структура:
//  *Имя - Строка 
//  *Расширение - Строка
//  *ПолноеИмя - Строка
...
// 	Структура:
// * Документы - Массив из см. НовоеОписаниеДокументаКонтейнера
Функция ФайлыКонтейнераПоФайлуМетаданных(РаспакованныйФайл, РаспакованныеФайлы)
	МетаданныеКонтейнера = Новый Структура;
	МетаданныеКонтейнера.Вставить("Документы", Новый Массив);
	
	МетаданныеXDTO = РаботаСФайламиБЭД.ПрочитатьXDTO(РаспакованныйФайл.ОткрытьПотокДляЧтения(), 
		РаботаСФайламиБЭД.ПолучитьТипЗначенияCML("ContainerDescription",
			"http://api-invoice.taxcom.ru/meta"));
	...
	Для Каждого Документооборот Из МетаданныеXDTO.DocFlow Цикл
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			ИмяОсновногоФайла = Неопределено;
			РасширениеОсновногоФайла = Неопределено;
			Если Документ.Files.MainImage <> Неопределено Тогда
				Если СтрНайти(Документ.Files.MainImage.Path, "/") Тогда
					МассивПути = СтрРазделить(Документ.Files.MainImage.Path, "/");
				Иначе
					МассивПути = СтрРазделить(Документ.Files.MainImage.Path, "\");
				КонецЕсли;
				ИмяОсновногоФайла = МассивПути.Получить(МассивПути.Количество()-1);
				РасширениеОсновногоФайла = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ИмяОсновногоФайла);
			КонецЕсли;
			ИмяФайлаДополнительныДанных = Неопределено;
			Если Документ.Files.DataImage <> Неопределено Тогда
				Если СтрНайти(Документ.Files.DataImage.Path, "/") Тогда
					МассивПути = СтрРазделить(Документ.Files.DataImage.Path, "/");
				Иначе
					МассивПути = СтрРазделить(Документ.Files.DataImage.Path, "\");
				КонецЕсли;
				ИмяФайлаДополнительныДанных = МассивПути.Получить(МассивПути.Количество()-1);
			КонецЕсли;
			ИмяФайлаКарточки = Неопределено;
			Если Документ.Files.ExternalCard <> Неопределено Тогда
				Если СтрНайти(Документ.Files.ExternalCard.Path, "/") Тогда
					МассивПути = СтрРазделить(Документ.Files.ExternalCard.Path, "/");
				Иначе
					МассивПути = СтрРазделить(Документ.Files.ExternalCard.Path, "\");
				КонецЕсли;
				ИмяФайлаКарточки = МассивПути.Получить(МассивПути.Количество()-1);
			КонецЕсли;
			ПодписиОсновногоФайла = Новый Массив;
			Если Документ.Files.MainImageSignature <> Неопределено Тогда
				Для каждого Объект Из Документ.Files.MainImageSignature Цикл
					Если СтрНайти(Объект.Path, "/") Тогда
						МассивПути = СтрРазделить(Объект.Path, "/");
					Иначе
						МассивПути = СтрРазделить(Объект.Path, "\");
					КонецЕсли;
					ДанныеПодписи = Новый Структура;
					ДанныеПодписи.Вставить("ИмяФайла", МассивПути.Получить(МассивПути.Количество()-1));
					ДанныеПодписи.Вставить("Расширение", 
						ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ДанныеПодписи.ИмяФайла));
					
					ПодписиОсновногоФайла.Добавить(ДанныеПодписи);
				КонецЦикла;
			КонецЕсли;
			ПодписиДополнительныхДанныхФайла = Новый Массив;
			Если Документ.Files.DataImageSignature <> Неопределено Тогда
				Для каждого Объект Из Документ.Files.DataImageSignature Цикл
					Если СтрНайти(Объект.Path, "/") Тогда
						МассивПути = СтрРазделить(Объект.Path, "/");
					Иначе
						МассивПути = СтрРазделить(Объект.Path, "\");
					...
					ПодписиДополнительныхДанныхФайла.Добавить(ДанныеПодписи);
				КонецЦикла;
			КонецЕсли;
			Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
				Если ДанныеФайла.Имя = ИмяОсновногоФайла 
					Или ДанныеФайла.ИменаВКодировках.Найти(ИмяОсновногоФайла) <> Неопределено Тогда
					ДвоичныеДанныеФайлаЭД = ДанныеФайла.ДвоичныеДанные;
				КонецЕсли;
				Если ДанныеФайла.Имя = ИмяФайлаДополнительныДанных
					Или ДанныеФайла.ИменаВКодировках.Найти(ИмяФайлаДополнительныДанных) <> Неопределено Тогда
					ОписаниеДокументаКонтейнера.ОписаниеДополнительныхДанных.ИмяФайла = ИмяФайлаДополнительныДанных;
					ОписаниеДокументаКонтейнера.ОписаниеДополнительныхДанных.ДвоичныеДанные = ДанныеФайла.ДвоичныеДанные;
				КонецЕсли;
			КонецЦикла;
			Если Не ЗначениеЗаполнено(ДвоичныеДанныеФайлаЭД) И ЗначениеЗаполнено(ИмяОсновногоФайла) Тогда
				Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
					Если Не ДанныеФайла.Расширение = РасширениеОсновногоФайла Тогда
						Продолжить;
					КонецЕсли;
					Если ДанныеФайла.Имя = ИмяФайлаДополнительныДанных Или ДанныеФайла.Имя = ИмяФайлаКарточки Тогда
						Продолжить;
					КонецЕсли;
					Если ЗначениеЗаполнено(ПодписиОсновногоФайла.Найти(ДанныеФайла.Имя))
						Или ЗначениеЗаполнено(ПодписиДополнительныхДанныхФайла.Найти(ДанныеФайла.Имя)) Тогда
						Продолжить;
					КонецЕсли;
					Если СтрНайти(НРег(ДанныеФайла.Имя), "meta") > 0 Тогда
						Продолжить;
					КонецЕсли;
					ДвоичныеДанныеФайлаЭД = ДанныеФайла.ДвоичныеДанные;
				КонецЦикла;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ДвоичныеДанныеФайлаЭД) И ЗначениеЗаполнено(ИмяОсновногоФайла) Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка при разборе файла meta.xml транспортного контейнера ЭДО.
					|Файл %1 не найден в транспортном контейнере.'"), ИмяОсновногоФайла);
			КонецЕсли;
			ОписаниеДокументаКонтейнера.ОписаниеДанных.ИмяФайла = ИмяОсновногоФайла;
			ОписаниеДокументаКонтейнера.ОписаниеДанных.ДвоичныеДанные = ДвоичныеДанныеФайлаЭД;
			Для Каждого Подпись Из ПодписиОсновногоФайла Цикл
				Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
					Если ДанныеФайла.Имя = Подпись.ИмяФайла 
						Или ДанныеФайла.ИменаВКодировках.Найти(Подпись.ИмяФайла) <> Неопределено Тогда
						ДанныеПодписи = СинхронизацияЭДО.НовоеОписаниеДанныхОбъекта();
						ДанныеПодписи.ИмяФайла = ДанныеФайла.Имя;
						ДанныеПодписи.ДвоичныеДанные = ДанныеФайла.ДвоичныеДанные;
						ОписаниеДокументаКонтейнера.ПодписиОсновныхДанных.Добавить(ДанныеПодписи);
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			Если ОписаниеДокументаКонтейнера.ПодписиОсновныхДанных.Количество() = 0 
				И ПодписиДополнительныхДанныхФайла.Количество() = 0 Тогда
				Для Каждого Подпись Из ПодписиОсновногоФайла Цикл
					Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
						Если ДанныеФайла.Расширение = Подпись.Расширение Тогда
							ДанныеПодписи = СинхронизацияЭДО.НовоеОписаниеДанныхОбъекта();
							ДанныеПодписи.ИмяФайла = ДанныеФайла.Имя;
							ДанныеПодписи.ДвоичныеДанные = ДанныеФайла.ДвоичныеДанные;
							ОписаниеДокументаКонтейнера.ПодписиОсновныхДанных.Добавить(ДанныеПодписи);
			...
			Для Каждого Подпись Из ПодписиДополнительныхДанныхФайла Цикл
				Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
					Если ДанныеФайла.Имя = Подпись.ИмяФайла 
						Или ДанныеФайла.ИменаВКодировках.Найти(Подпись.ИмяФайла) <> Неопределено Тогда
						ДанныеПодписи = СинхронизацияЭДО.НовоеОписаниеДанныхОбъекта();
...
// Читает карточку МЧД из двоичных данных и возвращает результат.
// 
// Параметры:
// 	РаспакованныйФайл - Структура:
//  *Имя - Строка 
//  *Расширение - Строка
//  *ПолноеИмя - Строка
//  *ИмяБезРасширения - Строка
//  *Путь - Строка
//  *ДвоичныеДанные - ДвоичныеДанные
// 	РаспакованныеФайлы - Массив из см. РаспакованныйФайл
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//   * Ключ - Строка - Хеш удостоверяемой подписи
//   * Значение - см. НовоеОписаниеДанныхДоверенности
Функция ДанныеДоверенностиПоХэшамПодписиИзРаспакованныхФайлов(РаспакованныйФайл, РаспакованныеФайлы)
	...
	Попытка
		КарточкаМЧД = РаботаСФайламиБЭД.ПрочитатьXDTO(РаспакованныйФайл.ДвоичныеДанные.ОткрытьПотокДляЧтения(),
			РаботаСФайламиБЭД.ПолучитьТипЗначенияCML("Warrant", "http://api-invoice.taxcom.ru/warrant"));
	Исключение
		...
		КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РаспакованныйФайл.Имя,
			КраткоеПредставлениеОшибки);
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		Возврат Результат;
	...
	Для Каждого ДанныеМЧД Из МЧД Цикл
		...
		ДополнительныеПараметрыМЧД = 
			РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеМЧД, "AdditionalData.AdditionalParameter");
		
		Если ДополнительныеПараметрыМЧД <> Неопределено Тогда
			
			ЭтоМЧДОснование = Ложь;
			Если ТипЗнч(ДополнительныеПараметрыМЧД) = Тип("СписокXDTO") Тогда
				Для Каждого ДополнительныйПараметр Из ДополнительныеПараметрыМЧД Цикл
					ИмяПараметра = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДополнительныйПараметр, "Name");
					Если ИмяПараметра = "МЧДОснованиеКФайлу" 
						Или ИмяПараметра = "МЧДОснованиеКМета" Тогда
							ЭтоМЧДОснование = Истина;
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли ТипЗнч(ДополнительныеПараметрыМЧД) = Тип("ОбъектXDTO") Тогда
				ИмяПараметра = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДополнительныеПараметрыМЧД, "Name");
				Если ИмяПараметра = "МЧДОснованиеКФайлу" 
					Или ИмяПараметра = "МЧДОснованиеКМета" Тогда
						ЭтоМЧДОснование = Истина;
				КонецЕсли;
		...
		Если ФайлыМЧД <> Неопределено Тогда
			...
			Если СтрНайти(ФайлыМЧД.WarrantImage.Path, "/") Тогда
				МассивПути = СтрРазделить(ФайлыМЧД.WarrantImage.Path, "/");
			Иначе
				МассивПути = СтрРазделить(ФайлыМЧД.WarrantImage.Path, "\");
			КонецЕсли;
			ИмяФайлаМЧД = МассивПути.Получить(МассивПути.Количество()-1);
		
			ДвоичныеДанныеМЧД = Неопределено;
			Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
				Если ДанныеФайла.Имя = ИмяФайлаМЧД Тогда
					ДвоичныеДанныеМЧД = ДанныеФайла.ДвоичныеДанные;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			...
			КарточкаМЧД.Доверенность.ДвоичныеДанные = ДвоичныеДанныеМЧД;
			КарточкаМЧД.Доверенность.ИмяФайла = ИмяФайлаМЧД;

			Подписи = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ФайлыМЧД, "WarrantSignature", , , Истина);

			Для Каждого Подпись Из Подписи Цикл
				Если СтрНайти(Подпись.Path, "/") Тогда
					МассивПути = СтрРазделить(Подпись.Path, "/");
				Иначе
					МассивПути = СтрРазделить(Подпись.Path, "\");
				КонецЕсли;
				ИмяФайлаПодписи = МассивПути.Получить(МассивПути.Количество() - 1);
				ДвоичныеДанныеПодписиМЧД = Неопределено;
				Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
					Если ДанныеФайла.Имя = ИмяФайлаПодписи Тогда
						ДвоичныеДанныеПодписиМЧД = ДанныеФайла.ДвоичныеДанные;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если ДвоичныеДанныеПодписиМЧД = Неопределено Тогда
					ШаблонПредупреждения = НСтр("ru = 'В контейнере отсутствуют данные подписи доверенности: %1'");
					ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, ИмяФайлаПодписи);
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстПредупреждения, ПодсистемаСобытия,
						УровеньЖурналаРегистрации.Предупреждение);
				...
				КарточкаМЧД.Подпись.ДвоичныеДанные = ДвоичныеДанныеПодписиМЧД;
				КарточкаМЧД.Подпись.ИмяФайла = ИмяФайлаПодписи;
		...
		Для Каждого Подпись Из Подписи Цикл
			Если СтрНайти(Подпись.File, "/") Тогда
				МассивПути = СтрРазделить(Подпись.File, "/");
			Иначе
				МассивПути = СтрРазделить(Подпись.File, "\");
			КонецЕсли;
			ИмяФайлаПодписи = МассивПути.Получить(МассивПути.Количество() - 1);
			...
			Для Каждого ДанныеФайла Из РаспакованныеФайлы Цикл
				Если ДанныеФайла.Имя = ИмяФайлаПодписи Тогда
					ДвоичныеДанныеПодписи = ДанныеФайла.ДвоичныеДанные;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
// 
// Возвращаемое значение:
//  Булево - Истина, если распаковка возможна
...
Функция СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики = Неопределено)
	...
	КонецЦикла;
	ДвоичныеДанныеФайлов = РаботаСФайлами.ДвоичныеДанныеФайлов(МассивПрисоединенныхФайлов);
	...
	ПрисутствуютКонтейнерыСервиса1СЭДО = Ложь;
	СтатусКонтейнераНеизвестный = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный;
	...
	Для каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			Если ПрисоединенныеФайлыКонтейнера = Неопределено Или ПрисоединенныеФайлыКонтейнера.Количество() = 0 Тогда
				ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, СтатусКонтейнераНеизвестный, КонтекстДиагностики);
				Продолжить;
			КонецЕсли;
			
			ДвоичныеДанныеФайла = ДвоичныеДанныеФайлов[ПрисоединенныеФайлыКонтейнера[0]];
			Если ДвоичныеДанныеФайла = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Ошибка при чтении транспортного контейнера ЭДО.
				|Отсутствуют двоичные данные контейнера.'");
				ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, СтатусКонтейнераНеизвестный, КонтекстДиагностики,
					ТекстОшибки);
				Продолжить;
			КонецЕсли;
			
			Если Не ПрисутствуютКонтейнерыСервиса1СЭДО 
				И ЗначенияРеквизитовКонтейнера.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
				ПрисутствуютКонтейнерыСервиса1СЭДО = Истина;
			КонецЕсли;
			
			РезультатРаспаковкиКонтейнера = Неопределено;
			ТребуетсяПроверкаЗаполненияКонтейнера = Ложь;
			...
			Если ЗначенияРеквизитовКонтейнера.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСерверЕИС 
				...
				Если ДанныеВходящегоОбъекта.Метаданные = Неопределено Тогда
					ТекстОшибки = НСтр("ru = 'Ошибка при чтении транспортного контейнера ЭДО.
					|Отсутствует служебный файл meta.xml.'");
					ИзменитьСтатусКонтейнера(ТранспортныйКонтейнер, СтатусКонтейнераНеизвестный, КонтекстДиагностики);
					Продолжить;
				КонецЕсли;
			Иначе
				ТребуетсяПроверкаЗаполненияКонтейнера = Истина;
				ДанныеВходящегоОбъекта = ДанныеВходящегоОбъекта(ДвоичныеДанныеФайла, РезультатРаспаковкиКонтейнера,
					КонтекстДиагностики);
			КонецЕсли;
			
			Если ТребуетсяПроверкаЗаполненияКонтейнера
				И (ЗначенияРеквизитовКонтейнера.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный
				...
				ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанныеФайла, РезультатРаспаковкиКонтейнера,
					КонтекстДиагностики);
			...
			Если РаспаковкаКонтейнераОтложена(ЗначенияРеквизитовКонтейнера) Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого Документ Из МетаданныеКонтейнера.Документы Цикл
				...
				Если ПараметрыДокумента.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.Ошибка Тогда
					...
					ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка по документообороту с идентификатором %1'"),
						ДанныеОбъекта.ИдентификаторДокументооборота) + Символы.ПС
						+ ДанныеВходящегоОбъекта.Ошибка.Описание;
				...
				Если ЗначениеЗаполнено(ДанныеВходящегоОбъекта.Карточка.ДатаДокумента) Тогда
					ДанныеОбъекта.ОписаниеДокумента.Дата = ДанныеВходящегоОбъекта.Карточка.ДатаДокумента;
				Иначе
				...
				ДанныеОбъекта.ПриоритетРаспаковки = СервисЭДО.ПриоритетРаспаковкиКонтейнера(Документ.КодТранзакции,
					ДанныеВходящегоОбъекта.ТипДокумента);
				ДанныеОбъекта.ДанныеТЭДО = ДанныеВходящегоОбъекта.ДанныеТЭДО;
	...
	Если ПрисутствуютКонтейнерыСервиса1СЭДО Тогда
	...
	ДанныеОбъектов.Сортировать("ПриоритетРаспаковки");
	Возврат ДанныеОбъектов;
	...
	Для Каждого СтрокаСОшибкой Из СтрокиСОшибкамиПередачи Цикл
		...
		Если ОшибкаПоКлассификатору <> Неопределено Тогда
			Описание = СтрШаблон(НСтр("ru = '%1
				|
				|Подробно:
				|%2'"), ОшибкаПоКлассификатору["description"], ОшибкаПередачи.Описание);
			ОшибкаПередачи.Описание = Описание;
...
Процедура ДобавитьОшибкуНезаполненныхКлючевыхСвойств(КонтекстДиагностики, Пакет, ТекстСообщения = "")
	...
	ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
		ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами);
	ИзменитьСтатусКонтейнера(Пакет, Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный, КонтекстДиагностики, ТекстСообщения);
КонецПроцедуры


// Дополняет коллекцию контейнеров связанными контейнерами, относящимся к тем же пакетам.
// 
// Параметры:
//  Контейнеры - Массив из ДокументСсылка.ТранспортныйКонтейнерЭДО
//
...
// Дополняет коллекцию переданных контейнеров нераспакованными элементами из того же документооборота и переводит
//  дополняющие элементы в статус "К распаковке".
//  
// В случаях, когда порядок входящих контейнеров был нарушен, может возникнуть ситуация, что сначала придут зависимые
//  контейнеры с технической информацией, а затем только придут основные данные. Это приведет к тому, что если начать
//  распаковку данных до прихода основных пакетов, зависимые контейнеры не распакуются и получат статус "Неизвестный".
// 
// Перевод контейнеров в новый статус требуется на случай прерывания процесса распаковки. Это может привести к тому,
//  что часть контейнеров по документообороту распакуется, а другая часть не успеет этого сделать, и контейнеры больше
//  не попадут в распаковку, если они до этого были в статусе "Неизвестный".
//
// Параметры:
//  Контейнеры - Массив из ДокументСсылка.ТранспортныйКонтейнерЭДО
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//
Процедура ДополнитьСвязаннымиКонтейнерамиИУстановитьСтатусКРаспаковке(Контейнеры, КонтекстДиагностики)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТранспортныйКонтейнерЭДО.ИдентификаторДокументооборота КАК ИдентификаторДокументооборота
		|ПОМЕСТИТЬ ИдентификаторыДокументооборота
		|ИЗ
		|	Документ.ТранспортныйКонтейнерЭДО КАК ТранспортныйКонтейнерЭДО
		|ГДЕ
		|	ТранспортныйКонтейнерЭДО.Ссылка В (&Контейнеры)
		|	И ТранспортныйКонтейнерЭДО.ИдентификаторДокументооборота <> """"
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	Документ.ТранспортныйКонтейнерЭДО КАК ТранспортныйКонтейнерЭДО
		|ГДЕ
		|	ТранспортныйКонтейнерЭДО.Статус В (&СтатусыНеРаспакованныхКонтейнеров)
		|	И ТранспортныйКонтейнерЭДО.Ссылка НЕ В (&Контейнеры)
		...
		|			ИдентификаторДокументооборота
		|		ИЗ
		|			ИдентификаторыДокументооборота)";
	
	Запрос.УстановитьПараметр("Контейнеры", Контейнеры);
	Запрос.УстановитьПараметр("СтатусыНеРаспакованныхКонтейнеров", СтатусыНеРаспакованныхТранспортныхСообщенийБЭД());
	УстановитьПривилегированныйРежим(Истина);
	РезультатЗапроса = Запрос.Выполнить();
	УстановитьПривилегированныйРежим(Ложь);
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	СтатусКонтейнераКРаспаковке = Перечисления.СтатусыТранспортныхСообщенийБЭД.КРаспаковке;
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ИзменитьСтатусКонтейнера(ВыборкаДетальныеЗаписи.Ссылка, СтатусКонтейнераКРаспаковке, КонтекстДиагностики);
		Контейнеры.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
...
Функция ВидОшибкиНеУдалосьРаспаковатьКонтейнер()
	...
	ВидОшибки.ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Отмените</a> распаковку контейнера'"), "Отмените");
...
// Заполняет контрагента при распаковке транспортного контейнера электронного документа
// Параметры:
//  ПараметрыКонтейнера - см. ПараметрыКонтейнера
//  РезультатРаспаковки - см. РаспаковатьФайлКонтейнера
//  СоздаватьКонтрагентовАвтоматически - Булево
Процедура ЗаполнитьКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки,
			ПараметрыПриглашения, Контрагент);
	КонецЕсли;
	
	ОбменПоПриглашению = СинхронизацияЭДОКлиентСервер.ЭтоОбменЧерезОператора(ПараметрыКонтейнера.СпособОбмена);
	
	Если Не ЗначениеЗаполнено(Контрагент)
		И ЗначениеЗаполнено(Карточка.КППКонтрагентаУточненный) Тогда
		...
		Если ОбменПоПриглашению Тогда
			...
			КлючКонтрагента.КПП = Карточка.КППКонтрагентаУточненный;
		...
		Иначе
			ОбменМеждуСобственнымиОрганизациями = СинхронизацияЭДО.ЭтоИнтеркампани(Карточка.ИдентификаторОтправителя);
		...
		И Не ОбменМеждуСобственнымиОрганизациями = Истина
	...
	Если Не ЗначениеЗаполнено(Контрагент)
		...
		Если ОбменПоПриглашению Тогда
			КлючКонтрагента = ПриглашенияЭДО.НовыйКлючУчастникаЭДО();
			КлючКонтрагента.ИНН = Карточка.ИННКонтрагента;
			КлючКонтрагента.КПП = Карточка.КППКонтрагента;
			КлючКонтрагента.ДатаСведений = Карточка.Дата;
			СтруктураУчастникаЭДО = ПриглашенияЭДОПовтИсп.УчастникЭДОПоКлючуИПриглашению(ПараметрыПриглашения, КлючКонтрагента);
			Контрагент = СтруктураУчастникаЭДО.Ссылка;
			ОбменМеждуСобственнымиОрганизациями = СтруктураУчастникаЭДО.ЭтоОрганизация;
...
// Параметры:
//  ТранспортныйКонтейнер - ДокументСсылка.ТранспортныйКонтейнерЭДО
//  КоличествоКонтейнеровВПакетах - Соответствие Из КлючИЗначение:
//    * Ключ - Строка - Идентификатор пакета
//    * Значение - Число - Количество контейнеров в пакете
//
Процедура ЗаполнитьКоличествоКонтейнеровВПакете(ТранспортныйКонтейнер, КоличествоКонтейнеровВПакетах) 
	
	ИдентификаторПакета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТранспортныйКонтейнер, "ИдентификаторПакета");
	Если ЗначениеЗаполнено(ИдентификаторПакета) Тогда
		КоличествоКонтейнеровВПакете = КоличествоКонтейнеровВПакетах[ИдентификаторПакета];
		Если КоличествоКонтейнеровВПакете = Неопределено Тогда
			КоличествоКонтейнеровВПакетах.Вставить(ИдентификаторПакета, 1);
		Иначе 
			КоличествоКонтейнеровВПакетах.Вставить(ИдентификаторПакета, КоличествоКонтейнеровВПакете + 1);
		КонецЕсли;
	КонецЕсли;
