...
//   * ОГРН                          - Строка - ОГРН.
//   * ДатаОГРН                      - Строка - дата внесения записи об ОГРН.
//   * ИдентификаторЮридическогоЛица - Строка - идентификатор юридического лица.
...
//   * ИНННерезидента                - Строка - ИНН. Обязательно для заполнения, если является нерезидентом.
//   * КодСтраныНерезидента          - Строка - Код страны. Заполняется, если является нерезидентом.
//
...
Процедура ПриЗаполненииПараметровСервисов(ПараметрыСервисов) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		МодульИнтерфейсДокументовЭДО.ПриЗаполненииПараметровСервисов(ПараметрыСервисов);
	КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиПоставляемыхДанных(Знач Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.БизнесСеть") Тогда
		МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("БизнесСетьВМоделиСервиса");
		МодульПодсистемы.ПолучитьОбработчикиПоставляемыхДанных(Обработчики);
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.СервисДоставки") Тогда
		...
		МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("СервисДоставки");
		МодульПодсистемы.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.БизнесСеть") Тогда
		...
		МодульПодсистемы = ОбщегоНазначения.ОбщийМодуль("БизнесСетьСлужебный");
		МодульПодсистемы.ПриДобавленииОбработчиковОбновления(Обработчики);
...
Процедура ПриПопыткеЗанятьФайл(ДанныеФайла, ОписаниеОшибки) Экспорт
	...
	// ОбменСКонтрагентами
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		МодульВизуализацияЭДО = ОбщегоНазначения.ОбщийМодуль("ВизуализацияЭДО");
		МодульВизуализацияЭДО.ПриПопыткеЗанятьФайл(ДанныеФайла, ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПриОпределенииСправочниковХраненияФайлов(ТипВладелецФайла, ИменаСправочников) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		Если Метаданные.ОпределяемыеТипы["ВладелецФайлаВизуализацииЭДО"].Тип.СодержитТип(ТипВладелецФайла) Тогда
			ИменаСправочников.Вставить("КэшВизуализацииДокументовЭДОПрисоединенныеФайлы",
				Не ЗначениеЗаполнено(ИменаСправочников));
		КонецЕсли;
		
		Если Метаданные.ОпределяемыеТипы.ВладелецФайлаКвитанцииДТС.Тип.СодержитТип(ТипВладелецФайла) Тогда
			ИменаСправочников.Вставить("КвитанцииДТСПрисоединенныеФайлы", Ложь);
		КонецЕсли;
