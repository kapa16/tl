...
Процедура ВыполнитьАвторизациюПоЛогинуПаролю(Логин, Пароль) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьРаспознаваниеДокументов") Тогда
		ВызватьИсключение НСтр("ru = 'Распознавание документов отключено. Обратитесь к Администратору.'; en = 'Document recognition is disabled. Contact your system administrator.'");
	КонецЕсли;
...
"по причине:"
"%1'; en = 'Cannot receive an authorization ticket from 1C:ITS Portal"
"due to:"
"%1'"),
...
"%1"
""
"Перейдите в настройки интернет-поддержки и измените данные авторизации.'; en = 'Cannot receive an authorization ticket from the 1C:ITS portal"
"due to:"
"%1"
""
...
Процедура УдалитьДанныеАвторизации(УдалитьИдентификаторИБ = Ложь) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("ru = 'Удалять данные авторизации доступно только Администратору.'; en = 'Only the Administrator can delete the authorization data.'");
	КонецЕсли;
...
"по причине:"
"%1'; en = 'Cannot automatically authorize with a redefined username and password"
"due to:"
"%1'"),
...
"по причине:"
"%1'; en = 'Cannot automatically authorize with a ticket from 1C:ITS Portal"
"due to:"
"%1'"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"по причине:"
"%1'; en = 'Cannot check the 1C:ITS Portal account activation"
"due to:"
"%1'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗагрузитьРаспознанныйДокументПоИдентификаторуРезультата(ИдентификаторРезультата) Экспорт
	...
	Если ДанныеСервиса = Неопределено Тогда
		Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Не удалось получить данные с сервиса распознавания.'; en = 'Cannot receive the recognition service data.'");
		Возврат Результат;
	КонецЕсли;
...
"%1'; en = 'An error occurred while receiving the recognized document:"
"%1'"),
		ДанныеСервиса.Ответ);
		Возврат Результат;
	КонецЕсли;
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ДокументСсылка) Тогда 
			ВызватьИсключение
				НСтр("ru = 'При попытке создать распознанный документ по загруженным данным возникла непредвиденная ошибка.'; en = 'An unexpected error occurred while creating a recognized document by the imported data.'");
		КонецЕсли;
		...
		ЗаписьЖурналаРегистрации(
			...
			ДокументСсылка,
			НСтр("ru = 'Записан документ с идентификатором:'; en = 'A document with the identifier was recorded:'") + Символы.ПС 
...
"Подробности см. в журнале регистрации.'; en = 'An error occurred while importing the documents."
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ru = 'Корректировка нераспознанных документов'; en = 'Correction of unrecognized documents'",
...
"автоматического распознавания по картинке, скану или фотографии.'; en = 'Additionally, it is assigned to users who should have access to the ability to correct unrecognized documents when performing automatic recognition based on an image, scan, or photograph.'");
...
// Устанавливает значение параметра сеанса КорректировкаНераспознанныхДокументовРазрешена,
...
Процедура ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения) Экспорт
	...
	ОписательСервиса.Наименование    = НСтр("ru = '1С:Распознавание первичных документов'; en = '1C:Source document recognition'");
	ОписательСервиса.Описание        = НСтр("ru = 'Вы можете подключить бесплатно 250 страниц для тестирования сервиса.'; en = 'You can connect 250 pages for free to test the service.'");
...
// Устарела. Устарела. Следует использовать 
// См. РаспознаваниеДокументов.ВыполнитьАвторизациюПоЛогинуПаролю
// 
// Параметры:
//  Логин - Строка - Логин
...
// 
// Возвращаемое значение:
//  Структура - Выполнить авторизацию на сервисе:
// * ВыполненоУспешно - Булево 
// * ТекстОшибки - Строка 
// * ВозвращаемыеДанные - Строка 
//
...
// Устарела. Следует использовать См Обработки.РаспознаваниеДокументов.ОтправитьФайлыНаРаспознавание
//
Функция СоздатьЗаданиеДляРаспознавания(
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru = 'Отправлены файлы для распознавания:'; en = 'Files sent for recognition:'") + Символы.ПС 
		+ ФайлыСтрокой
	);
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru = 'Получен идентификатор нового задания:'; en = 'The new job ID is received:'") + Символы.ПС 
		+ ИдентификаторЗадания
	);
	...
	СтруктураПредставление.Вставить("ДатаЗагрузки", ТекДата);
	СтруктураПредставление.Вставить("ФайлыСтрокой", ФайлыСтрокой);
	...
	Для Каждого ТекАдрес Из мАдресов Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(, СтрШаблон(
			НСтр("ru = 'Отправка файла %1 (%2 из %3)...'; en = 'Sending file %1 (%2 out of %3)...'"),
			Файлы[Ид].ИмяФайла,
			Ид + 1,
			КоличествоФайлов
		));
...
Функция ОтправитьОбратнуюСвязьRecognitionResult(ДокументОбъект) Экспорт
	...
	ПометкаУдаленияУстановленаРегЗаданием = ДокументОбъект.ДополнительныеСвойства.Свойство("ПометкаУдаленияУстановленаРегЗаданием");
	
	// СлужебныеПоля
	СлужебныеПоля.Вставить("База", НРег(СтрокаСоединенияИнформационнойБазы()));
	// {УАТ}
	СлужебныеПоля.Вставить("Область", ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса());
	// {/УАТ}
	ЗаполнитьДанныеПользователя(СлужебныеПоля);
	
	СлужебныеПоля.Вставить("ПометкаУдаления", ДокументОбъект.ПометкаУдаления);
	СлужебныеПоля.Вставить("ПометкаУдаленияУстановленаРегЗаданием", ПометкаУдаленияУстановленаРегЗаданием);
	...
	СлужебныеПоля.Вставить("ОбщееЧислоСтрок", 0); // Заполняется при анализе таблиц
	...
	Если НЕ СлужебныеПоля.ОшибкаОтСервиса Тогда 
		УчетДоговоров = Ложь;
        РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюУчетДоговоров(УчетДоговоров);
...
"- Значение должно быть ссылкой или именем предопределенного элемента'; en = 'Incorrect value of the first Ссылка parameter: "
...
Процедура ДополнитьРезультатОбратнойСвязи(ЭлементОбратнойСвязи, ДокументОбъект, Реквизит, СопоставлениеРеквизитовОбщее, ПроблемныеРеквизиты)
	...
	Если ЭтоПримитивныйТип Тогда
		Если ОбластьДобавления.Свойство(Реквизит.ИмяРеквизита) Тогда
			...
			// ВызватьИсключение НСтр("ru = 'Ошибка обратной связи'; en = 'Feedback error'");
		Иначе
...
Функция СформироватьПредставлениеЗадания(Параметры) Экспорт
	...
	ДатаЗагрузкиСтрокой = Формат(Параметры.ДатаЗагрузки, НСтр("ru = ""ДФ='yyyy.MM.dd HH:mm:ss'""; en = ""DF='yyyy.MM.dd HH:mm:ss'"""));
	
	Текст = СтрШаблон(
		НСтр("ru = 'Дата загрузки: %1. В обработке файлов: %2, загружено: %3, с ошибкой: %4.'; en = 'Import date: %1. Files being processed: %2, imported: %3, with error: %4.'"),
		ДатаЗагрузкиСтрокой,
	...
	Если Не ПустаяСтрока(Параметры.ФайлыСтрокой)Тогда 
		Текст = Текст + СтрШаблон(НСтр("ru = 'Исходные файлы: %1'; en = 'Source files: %1'"), Параметры.ФайлыСтрокой);
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'РаспознаваниеДокументов'; en = 'РаспознаваниеДокументов'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ВыполнитьНечеткийПоиск(СтрокаПоиска, ТипСсылкой)
	...
	Если ИмяТаблицыЗапроса = "Справочник.Контрагенты"
		...
		Или ИмяТаблицыЗапроса = "Перечисление.СтавкиНДС"  
		Или ИмяТаблицыЗапроса = "Справочник.НомераГТД"
		Или ИмяТаблицыЗапроса = "Документ.ПоступлениеТоваровУслуг" Тогда
...
Процедура СохранитьИнформациюОбИсходномФайле(ИдентификаторЗадания, ИдентификаторФайла, Данные, Отправитель)
	...
	Если Отправитель <> Неопределено Тогда
		...
		Менеджер.МобильноеПриложение = Справочники.МобильныеПриложенияРаспознаванияДокументов.НайтиПоКоду(Менеджер.Отправитель);
...
Процедура ОбновитьАдресЭлектроннойПочты()
	...
	Если Не ПустаяСтрока(ТекущиеНастройки.АдресЭлПочты) Тогда
		Попытка
			РаспознаваниеДокументовSDK.УстановитьАдресЭлектроннойПочты(ТекущиеНастройки.АдресЭлПочты);
		Исключение
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, , , 
				НСтр("ru = 'Обновление адреса электронной почты не удалось, адрес электронной почты очищен.'; en = 'Cannot update the email address. The email address is cleared.'", ОбщегоНазначения.КодОсновногоЯзыка())
...
Процедура ЗагрузкаДокумента(
	...
	ЗаписьЖурналаРегистрации(
		...
		УровеньЖурналаРегистрации.Информация, ,
		ИдентификаторЗадания,
		НСтр("ru = 'Завершено распознавание документа:'; en = 'The document is recognized:'") + Символы.ПС + РаспознанныйДокумент.ИдентификаторДокумента
	);
...
"Попробуйте загрузить документ на распознавание еще раз.'; en = 'An error occurred while recognizing the document."
...
Процедура ЗагрузкаДокументаV1(ИдентификаторЗадания, РаспознанныйДокумент, СостояниеЗадания, ДокументыБезОшибок)
	...
	Если ДанныеСервиса = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при получении информации о распознавании. Подробности см. в журнале регистрации.'; en = 'An error occurred while receiving the recognition information. For more information, see the event log.'");
	КонецЕсли;
