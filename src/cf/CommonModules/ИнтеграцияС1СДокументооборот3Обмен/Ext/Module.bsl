...
"   * Тип объекта 1С:Документооборота: %3.'; en = 'An error occurred when importing the ""%1"" status from 1C:Document Management:"
...
"   * Навигационная ссылка на объект %3: %4.'; en = 'An error occurred when importing data from 1C:Document Management object ""%1"":"
...
"   * The %3 object URL: %4.'",
		...
		Иначе
			Возврат ИнтеграцияС1СДокументооборот3.ОписаниеОшибкиXDTO(
				НСтр("ru = 'Ошибка при загрузке сообщения обмена из 1С:Документооборота'; en = 'An error occurred when importing the exchange message from 1C:Document Management'"),
				СтрСоединить(
					МассивОшибок,
					"
					|-----
...
"но подходящие правила интеграции не найдены.'; en = 'The ""%1"" (%2) object is registered to exchange data with the related object in 1C:Document Management."
"However, no suitable integration rules are found.'"),
			...
			Иначе
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Объект ""%1"" (%2) зарегистрирован к обмену данными со связанным объектом в 1С:Документооборот,"
"но не найдено подходящее правило загрузки данных в 1С:Документооборот.'; en = 'The ""%1"" (%2) object is registered to exchange data with the related object in 1C:Document Management."
"However, no suitable data import rules are found.'"),
			...
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьОшибку(
				ТекстОшибки,
				НСтр("ru = 'Отправка данных'; en = 'Export'"),
				ОбъектИС);
			Продолжить;
		КонецЕсли;
...
Процедура ЗагрузитьСостояниеДокумента(ОбъектXDTO)
	...
	Попытка
		Если ОбъектXDTO.status = Неопределено Тогда // Запись удалена (например, прерывание).
			Действие = НСтр("ru = 'Удаление статуса'; en = 'Delete status'", ОбщегоНазначения.КодОсновногоЯзыка());
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПриИзмененииСостоянияСогласования(
				...
				Выборка.Объект);
		Иначе
			Действие = НСтр("ru = 'Установка статуса'; en = 'Set status'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗагрузитьСсылочныйОбъект(ОбъектСсылка, ОбъектXDTO, УзелДокументооборота, СоставПланаОбмена)
	...
	Если ПодходящиеПравила = Неопределено Или ПодходящиеПравила.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не найдены подходящие правила интеграции'; en = 'Suitable integration rules are not found'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Попытка
		...
		Действие = НСтр("ru = 'Сохранение изменений'; en = 'Save the changes'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если Объект.ПометкаУдаления И Не ИсходнаяПометкаУдаления Тогда
			...
			Если Метаданные.Документы.Содержит(МетаданныеОбъекта)
				...
				Действие = НСтр("ru = 'Удаление с отменой проведения'; en = 'Delete and cancel posting'",
					ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Иначе
				...
				Действие = НСтр("ru = 'Удаление'; en = 'Delete'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Иначе
			...
			Если Объект.ПроверитьЗаполнение() Тогда
				...
				Если ТребуетсяПерепроведение Тогда
					...
					Действие = НСтр("ru = 'Проведение'; en = 'Posting'", ОбщегоНазначения.КодОсновногоЯзыка());
				...
				Иначе
					...
					Действие = НСтр("ru = 'Запись'; en = 'Record'", ОбщегоНазначения.КодОсновногоЯзыка());
					Объект.ОбменДанными.Загрузка = Истина;
			...
			Иначе
				...
				ТекстСообщения.Добавить(
					НСтр("ru = 'Ошибка заполнения:'; en = 'Filling error:'", ОбщегоНазначения.КодОсновногоЯзыка()));
