...
Функция ПолучитьДанныеФизическогоЛица(СсылкаСправочника) Экспорт
	...
	// {УАТ}
	ФИОСтруктура = уатОбщегоНазначения.уатФИОФизЛица(СсылкаСправочника, ТекущаяДатаСеанса());
	...
	Если НЕ ЗначениеЗаполнено(Результат.Телефон) Тогда
		Результат.Телефон = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
		СсылкаСправочника, Справочники.ВидыКонтактнойИнформации.ТелефонРабочийФизическиеЛица); 
	КонецЕсли;
	// {/УАТ}
...
Функция ВыполнитьОперацию(ПараметрыОперации)
	...
	// {УАТ} 
	//Попытка
	//	ВнутреннийХешИнтеграции = уатЗащищенныеФункцииСервер.ЗаполнитьВнутреннийКлючИнтеграции(ПараметрыОперации.ИдентификаторУчетнойЗаписи, 
	//		ПараметрыОперации.Заголовки);
	//Исключение
	...
	Пока ТекущийШаг Цикл
		...
		// {УАТ} 
		//Попытка
		//	уатЗащищенныеФункцииСервер.ОбновитьВнутреннийКлючИнтеграции(ПараметрыОперации.ИдентификаторУчетнойЗаписи, 
		//		РезультатВыполненияЗапроса.Ответ.Заголовки, ВнутреннийХешИнтеграции);
		//Исключение
		//КонецПопытки;
		// {/УАТ}
