...
//	ПараметрыРаботыКлиента - Структура - параметры работы клиента, заполненные на момент выполнения процедуры.
//
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры, ПараметрыРаботыКлиента) Экспорт
	...
	ПараметрыОбработки = Новый Структура("ПараметрЗапуска");
	ЗаполнитьЗначенияСвойств(ПараметрыОбработки, ПараметрыРаботыКлиента);
	
	ЗапускФоновыхЗаданийЗапрещен = Ложь;
	Если ЗначениеЗаполнено(ПараметрыОбработки.ПараметрЗапуска) Тогда
		ЗапускФоновыхЗаданийЗапрещен = (СтрНайти(
			ПараметрыОбработки.ПараметрЗапуска,
			"ОтключитьЛогикуНачалаРаботыСистемы") > 0);
	КонецЕсли;
	
	ПроверитьНаличиеОбновленийПриЗапуске(Параметры, ЗапускФоновыхЗаданийЗапрещен);
...
Процедура ИнтернетПоддержкаИСервисы_ПриСозданииНаСервере(Форма) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ОбновлениеИнформационнойБазы.ДоступенПросмотрОписанияИзмененийПрограммы());
...
Процедура ПриОпределенииОбработчиковПоставляемыхДанных(Обработчики) Экспорт
	...
	СтрОбработчик.Обработчик     = МодульПолучениеОбновленийПрограммы;
...
//  ЗапускФоновыхЗаданийЗапрещен - Булево - Истина, если запуск проверки наличия обновлений при запуске программы
//    запрещено.
//
Процедура ПроверитьНаличиеОбновленийПриЗапуске(Параметры, ЗапускФоновыхЗаданийЗапрещен)
	...
	Если ЗапускФоновыхЗаданийЗапрещен Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Проверка наличия обновлений при запуске программы пропущена, так как программа запущена с параметром ОтключитьЛогикуНачалаРаботыСистемы.'"));
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Если КодВозврата = 0 Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Новая версия платформы 1С:Предприятие успешно установлена.'"));
		
	ИначеЕсли КодВозврата = 1602 Или КодВозврата = 1 Тогда
	...
	Иначе
		...
		КодВозвратаСтрока = Формат(КодВозврата, "ЧГ=0;");
		...
		Если Параметры.РежимУстановкиПлатформы = 0 Тогда
			// Часть ошибок установки платформы в тихом режиме можно решить в интерактивном режиме.
			Если ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОграниченияСистемныхПолитик(КодВозврата) Тогда
				...
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю
					...
					+ НСтр("ru = 'Данная ошибка связана с ограничениями системных политик безопасности.
						|Рекомендуется <a href=""action:retruupdateplatfom"">установить платформу с ручными настройками</a>, либо запустить программу от имени администратора.'");
			ИначеЕсли ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаНеустранимойОшибкиПриУстановке(КодВозврата) Тогда
				// В процессе установки не удалось установить все компоненты программы. Например, установка
				// vc_redist.x64 завершается с ошибкой.
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю
					+ Символы.ПС
					+ Символы.ПС
					+ НСтр("ru = 'Не удалось установить один или несколько связанных компонентов программы.
						|Рекомендуется <a href=""action:retruupdateplatfom"">установить платформу с ручными настройками</a>, либо запустить программу от имени администратора.'");
			ИначеЕсли ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОшибкиНарушенияДоступа(КодВозврата) Тогда
				// Ошибка доступа к ресурсам операционной системы. Может возникать при пакетном запуске
				// 1С:Предприятие и программы установки новой версии платформы.
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю
					+ Символы.ПС
					+ Символы.ПС
					+ НСтр("ru = 'Ошибка доступа к ресурсам операционной системы.
						|Рекомендуется <a href=""action:retruupdateplatfom"">установить платформу с ручными настройками</a>, либо запустить программу от имени администратора.'");
			КонецЕсли;
		КонецЕсли;
...
// Проверяет разрешение на работу с сервисом получения обновлений программ.
//
// Возвращаемое значение:
//  Булево - если работа сервиса получения обновлений программ разрешена, то возвращается Истина, иначе Ложь.
//
Функция РазрешенаРаботаСХостом() Экспорт
	
	НастройкиСоединения = ИнтернетПоддержкаПользователей.НастройкиСоединенияССерверами();
	
	Возврат ИнтернетПоддержкаПользователей.РазрешенаРаботаСХостом(
		URLОперацииPing(НастройкиСоединения.ДоменРасположенияСерверовИПП));
	
КонецФункции

// Выполняет снятие блокировки работы с сервисом получения обновлений программы.
//
Процедура РазблокироватьРаботуСХостом() Экспорт
	
	НастройкиСоединения = ИнтернетПоддержкаПользователей.НастройкиСоединенияССерверами();
	
	ИнтернетПоддержкаПользователей.РазблокироватьРаботуСХостом(
		URLОперацииPing(НастройкиСоединения.ДоменРасположенияСерверовИПП));
