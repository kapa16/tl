...
Процедура ПолучитьПоРасписанию() Экспорт
	...
	Результат = Получить(ВидОперации);
	
	Если Результат.Остановлено Тогда
		УстановитьАктивностьРегламентногоЗадания(Ложь);
	КонецЕсли;
...
Функция НовыйРезультатПолучения() Экспорт
	...
	Результат = Новый Структура;
	Результат.Вставить("Остановлено", Ложь);
...
Функция Получить(ВидОперации, РасшифрованныеМаркеры = Неопределено)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(РезультатВариантовПолучения.СостояниеПоИдентификаторамЭДО)
			И Не РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.ЕстьАктивные() Тогда
			Результат.Остановлено = Истина;
			Возврат Результат;
		КонецЕсли;
...
Функция ПроверитьНаличиеОшибокПолученияЭДО(КонтекстДиагностики) 
	...
	Для Каждого Ошибка Из Ошибки Цикл
		...
		Если Не СинхронизацияЭДО.ЭтоОшибкаЕстьНеполученныеЭлектронныеДокументы(Ошибка)
			...
			ПредставленияОшибок.Добавить(Ошибка.ПодробноеПредставлениеОшибки);
		КонецЕсли;
...
Процедура ЗаполнитьРезультатПолученияПоИдентификаторуЭДО(Результат, ИдентификаторЭДО, ДатаНачала, РезультатДействийЭДО)
	...
	Если Остановлено Тогда
		ТекстСообщения = НСтр("ru = 'Автоматическое получение остановлено с ошибками:'")
	Иначе
		ТекстСообщения = НСтр("ru = 'Автоматическое получение выполнено с ошибками:'")
	КонецЕсли;
	
	ТекстСообщения = ТекстСообщения + Символы.ПС + СтрСоединить(
		РезультатПроверки.ПредставленияОшибок, ОбщегоНазначения.РазделительПакетаЗапросов());
	
	Подсистема = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().АвтоматическоеПолучениеЭДО;
	УровеньВажностиСобытия = ?(РезультатПроверки.ЕстьКритичныеОшибки,
		УровеньЖурналаРегистрации.Ошибка, УровеньЖурналаРегистрации.Предупреждение);
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, Подсистема, УровеньВажностиСобытия);
