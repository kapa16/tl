...
Функция СоздатьДокументыКомплектаВызовСервера(ПараметрыСоздания) Экспорт
	...
	ОбщийТекстОшибки = НСтр("ru = 'Исправьте ошибки заполнения документов комплекта или обработайте их по отдельности.'; en = 'Correct the kit document filling errors or process them separately.'");
	НачатьТранзакцию();
	Попытка
		Для Каждого ЭтотТипДокумента Из ТипыДокументов Цикл
			...
			Иначе
				...
				Если СоздаваемыйДокумент = Неопределено Тогда
					...
					// {УАТ}
					Если ЭтотТипДокумента = Тип("ДокументСсылка.уатСчетНаОплатуПокупателю")
						ИЛИ ЭтотТипДокумента = Тип("ДокументСсылка.уатСчетНаОплатуПоставщика") Тогда
					...
					Если РаспознаваниеДокументовТипы.ЭтоПоступлениеТоваровУслуг(ЭтотТипДокумента) 
						...
						Если Не ЗначениеЗаполнено(СоздаваемыйДокументОбъект[ИмяРеквизитаСклад]) Тогда
							СоздаваемыйДокументОбъект[ИмяРеквизитаСклад] = РаспознаваниеДокументовСлужебный.ПолучитьСкладПоУмолчанию();
						КонецЕсли;
					КонецЕсли;
					
					СоздаваемыйДокументОбъект.Записать(РежимЗаписи);
				...
				Иначе
					...
					Если РаспознаваниеДокументовТипы.ЭтоПоступлениеТоваровУслуг(ЭтотТипДокумента) 
						...
						Если ПараметрыСоздания.ПараметрыОперации.ВидОперации = РаспознаваниеДокументовСлужебный.ВидыОперацийПоступлениеТоваровУслуг().ПриемНаКомиссию
							Или ПараметрыСоздания.ПараметрыОперации.ВидОперации = РаспознаваниеДокументовСлужебный.ВидыОперацийРеализацияТоваров().ПередачаНаКомиссию Тогда
...
Функция ПараметрыЗаполненияДляСозданияДокумента(РаспознанныйДокумент, ТипРаспознанного, ТипСоздаваемого, ПараметрыОперации) Экспорт
	...
	// {УАТ}
	Если ТипСоздаваемого = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	// {/УАТ}
