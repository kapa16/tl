...
// Отражать универсальные документы одним объектом учета.
// 
// Возвращаемое значение:
//  Булево 
//
Функция ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Экспорт
	
	Результат = Ложь;
	ОбменСКонтрагентамиПереопределяемый.ОтражатьУниверсальныеДокументыОднимОбъектомУчета(Результат);
...
Процедура ПослеЗагрузкиНовогоЭлектронногоДокумента(ЗагруженныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		ЭтоЭПД = МодульОбменСГИСЭПД.ЭтоДокументЭПД(ЗагруженныйДокумент.ТипДокумента);
	КонецЕсли;
	
	ОбъектыУчетаЭлектронногоДокумента = ИнтеграцияЭДО.ОбъектыУчетаЭлектронныхДокументов(
		ЗагруженныйДокумент.ЭлектронныйДокумент);
	Если Не ЭтоЭПД И ЗначениеЗаполнено(ОбъектыУчетаЭлектронногоДокумента) Тогда
		Возврат;
	КонецЕсли;
...
// 
// Параметры:
//  Источник - РегистрСведенийНаборЗаписей.НоменклатураКонтрагентовБЭД
//  Отказ - Булево
//  Замещение - Булево
Процедура ВыполнитьКонтрольОтраженияВУчетеПриЗаписиНоменклатурыКонтрагентов(Источник, Отказ, Замещение) Экспорт
	...
	// При замещении в режиме совместимости до 8.3.26 включительно запись выполняется дважды: 
	//  сначала удаляются существующие записи, затем записываются новые.
	// Первую запись (удаление) не обрабатываем.
	ЭтоУдалениеВРежимеЗамещения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства,
		"ЭтоУдалениеВРежимеЗамещения",
		Ложь);
	Если ЭтоУдалениеВРежимеЗамещения Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЭтоУдалениеНабораЗаписей = Ложь;
	...
	Если Источник.Количество() = 0 Тогда
		// если это "настоящее" удаление (единственное, намеренное, не удаление старых записей при замещении)
		Владелец      = Источник.Отбор.Владелец.Значение; // ОпределяемыйТип.ВладелецНоменклатурыБЭД
		Идентификатор = Источник.Отбор.Идентификатор.Значение; // Строка
		НоменклатураНаОтключение = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагента(
			Владелец, 
			Идентификатор);
		НаОтключение.Добавить(НоменклатураНаОтключение);
	Иначе
		ЭтоУдалениеНабораЗаписей = ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение);
	КонецЕсли;
	
	Для Каждого Элемент Из Источник Цикл
		// контроль сопоставления отключается в 2х случаях:
		// * удаление номенклатуры контрагента,
		// * номенклатура сопоставлена.
		Если Не ЭтоУдалениеНабораЗаписей И Не ЗначениеЗаполнено(Элемент.Номенклатура) Тогда
			Продолжить;
		КонецЕсли;
		
		Владелец      = Элемент.Владелец; // ОпределяемыйТип.ВладелецНоменклатурыБЭД
...
Процедура СоздатьНастройкиОтраженияВУчете(Организация, Контрагент, ИдентификаторОрганизации, ИдентификаторКонтрагента, Отказ = Ложь) Экспорт
	...
	ТребуетсяОтражениеВУчете = Истина;
	ПриОпределенииНеобходимостиОтраженияВУчетеВходящихДокументов(ТребуетсяОтражениеВУчете);
	Если Не ТребуетсяОтражениеВУчете Тогда
		ПрофильНастроек = ПрофилиНастроекОтраженияВходящихДокументов().НеОтражать;
	КонецЕсли;
	
	ТекущаяТаблица = ШаблонНастроекОтраженияВУчете(ПрофильНастроек);
...
Функция ТаблицаПредопределенногоПрофиля(Профиль) Экспорт 
	...
	Если Не ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
...
Функция СписокОперацийТипаДокумента(ТипДокумента, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт 
	...
	Если ТипЗнч(ТипДокумента) = Тип("ПеречислениеСсылка.ТипыДокументовЭДО") Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.КоммерческиеПредложения")
			И (ТипДокумента = Перечисления.ТипыДокументовЭДО.ЗапросКоммерческихПредложений
...
Функция СписокОперацийВидаДокумента(ВидДокумента, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт
	...
	Если ОписаниеВида.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
		И Не ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
		ТипДокументаСпискаОпераций = Перечисления.ТипыДокументовЭДО.СчетФактура;
	ИначеЕсли ОписаниеВида.ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД
		И Не ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
...
// Параметры:
//  КонтрагентОбъектНеНайден - ОпределяемыйТип.УчастникЭДО
//  Контрагент - ОпределяемыйТип.УчастникЭДО
//	Организация - ОпределяемыйТип.Организация
//
Процедура ИзменитьНастройкиПолученияЭлектронныхДокументовПоКонтрагентуОбъектНеНайден(КонтрагентОбъектНеНайден,
	Контрагент, Организация) Экспорт
	
	НаборЗаписейПоКонтрагентуОбъектНеНайден =
		РегистрыСведений.НастройкиПолученияЭлектронныхДокументов.СоздатьНаборЗаписей();
	НаборЗаписейПоКонтрагентуОбъектНеНайден.Отбор.Получатель.Установить(Организация);
	НаборЗаписейПоКонтрагентуОбъектНеНайден.Отбор.Отправитель.Установить(КонтрагентОбъектНеНайден);
	НаборЗаписейПоКонтрагентуОбъектНеНайден.Прочитать();
	ЗаписиПоКонтрагентуОбъектНеНайден = НаборЗаписейПоКонтрагентуОбъектНеНайден.Выгрузить();
	
	Если ЗаписиПоКонтрагентуОбъектНеНайден.Количество() > 0 Тогда
		РегистрыСведений.НастройкиПолученияЭлектронныхДокументов.Удалить(Организация, КонтрагентОбъектНеНайден, "", "");
		
		ОбновленныеЗаписи = РегистрыСведений.НастройкиПолученияЭлектронныхДокументов.СоздатьНаборЗаписей();
		ОбновленныеЗаписи.Отбор.Получатель.Установить(Организация);
		ОбновленныеЗаписи.Отбор.Отправитель.Установить(Контрагент);
		Для Каждого Запись Из ЗаписиПоКонтрагентуОбъектНеНайден Цикл
			НоваяЗапись = ОбновленныеЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяЗапись, Запись, ,"Отправитель,УдалитьВидДокумента,УдалитьПрикладнойВидЭД");
			НоваяЗапись.Отправитель = Контрагент;
		КонецЦикла;
		ОбновленныеЗаписи.Записать();
	КонецЕсли;
КонецПроцедуры
...
Функция СпособОбработкиДокумента(ДанныеДокумента) Экспорт
	...
	ИначеЕсли ДанныеДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД
		И Не ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
	...
	Если ЗначениеЗаполнено(ДополнительныеВиды) Тогда
		...
		СпособОбработкиСтруктура = НовыйСпособОбработкиУПДУКД();
...
//  * ПервичныйДокумент - Строка
//  * СчетФактура - Строка
Функция НовыйСпособОбработкиУПДУКД() Экспорт
	
	СпособОбработки = Новый Структура;
	СпособОбработки.Вставить("ПервичныйДокумент", "");
	СпособОбработки.Вставить("СчетФактура", "");
...
Процедура УстановитьКонтрольОтраженияВУчете(ЭлектронныйДокумент, ПараметрыКонтроля = Неопределено) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОтраженияВУчетеЭДО") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтразитьДанныеЭлектронногоДокумента(ДанныеЭлектронногоДокумента, СпособОбработки, ОбъектыУчета = Неопределено, ДанныеРазбора = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЭлектронногоДокумента.ДанныеРазбора) Тогда
		ДанныеРазбора = ДанныеЭлектронногоДокумента.ДанныеРазбора;
	КонецЕсли;
...
Процедура ПерезаполнитьОбъектУчетаПоЭлектронномуДокументу(ОбъектУчета, ЭлектронныйДокумент, СпособОбработки = "", КонтекстДиагностики = Неопределено,
	...
	ДанныеДокументов = ЭлектронныеДокументыЭДО.ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы);
	...
	Если ДанныеДокумента.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.УПД)
		Или ДанныеДокумента.ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.УКД)
		И Не ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
...
//  * Значение - Строка
//             - Массив из Строка - Способ обработки.
//  
// Возвращаемое значение:
//  Структура:
//  * НоменклатураДляСопоставления - Массив из см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагента
//  * ОбъектыУчетаДокументов - Соответствие из КлючИЗначение:
//    ** Ключ - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
//    ** Значение - Массив из ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  * КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ОтразитьЭлектронныеДокументы(СпособыОбработкиДокументов) Экспорт
	
	ОбъектыУчетаДокументов = Новый Соответствие;
	КонтекстДиагностики = ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики();
	
	Результат = Новый Структура;
	Результат.Вставить("НоменклатураДляСопоставления", Новый Массив);
	...
	Если Не ЗначениеЗаполнено(СпособыОбработкиДокументов) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ЭлектронныеДокументы = Новый Массив; // Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО
	ЭлектронныеДокументыБезКонтроля = Новый Массив; // Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО
	Для Каждого СпособОбработкиДокумента Из СпособыОбработкиДокументов Цикл
		ЭлектронныйДокумент = СпособОбработкиДокумента.Ключ;
		СпособОбработки = СпособОбработкиДокумента.Значение;
		ЭлектронныеДокументы.Добавить(ЭлектронныйДокумент);
		Если ОтражениеОтключено(СпособОбработки) Тогда
			ЭлектронныеДокументыБезКонтроля.Добавить(ЭлектронныйДокумент);
		КонецЕсли;
	КонецЦикла;
	
	ОтборПоДокументу = Новый Структура("ЭлектронныйДокумент", ЭлектронныеДокументы);
	Результат.НоменклатураДляСопоставления = НоменклатураКонтрагентовНаКонтроле(ОтборПоДокументу);
	
	Если ЗначениеЗаполнено(ЭлектронныеДокументыБезКонтроля) Тогда
		ДанныеНоменклатурыДокументов = ЭлектронныеДокументыЭДО.ДанныеНоменклатурыДокументов(
			ЭлектронныеДокументыБезКонтроля, КонтекстДиагностики);
		НоменклатураДокументовБезСопоставления = НоменклатураЭлектронныхДокументовБезСопоставления(ДанныеНоменклатурыДокументов);
		НоменклатураБезСопоставления = ВыгрузитьНоменклатураДокументовБезСопоставления(НоменклатураДокументовБезСопоставления);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат.НоменклатураДляСопоставления, НоменклатураБезСопоставления);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Результат.НоменклатураДляСопоставления) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ДанныеДляОтраженияВУчете = ЭлектронныеДокументыЭДО.ДанныеДокументовДляОтраженияВУчете(
		ЭлектронныеДокументы, КонтекстДиагностики);
	
	Для Каждого ДанныеДокумента Из ДанныеДляОтраженияВУчете Цикл
		
		СпособыОбработкиДокумента = СпособыОбработкиДокументов[ДанныеДокумента.ЭлектронныйДокумент];
		
		Если ТребуетсяОтражениеДвумяОбъектамиУчета(ДанныеДокумента.ТипДокумента, СпособОбработки) Тогда
			СпособОбработки = НовыйСпособОбработкиУПДУКД();
			СпособОбработки.ПервичныйДокумент = СпособыОбработкиДокумента[1];
			СпособОбработки.СчетФактура = СпособыОбработкиДокумента[0];
		Иначе
			СпособОбработки = СпособыОбработкиДокумента;
		КонецЕсли;
		...
		ОбъектыУчетаДокументов.Вставить(ДанныеДокумента.ЭлектронныйДокумент, ОбъектыУчета);
...
// Параметры:
//  ЭлектронныеДокументы - Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО
Процедура ОтразитьЭлектронныеДокументыНаКонтролеАвтоматически(ЭлектронныеДокументы) Экспорт
	
	КонтекстДиагностики = ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики();
	
	ДокументыНаКонтроле = ДокументыНаКонтроле(ЭлектронныеДокументы);
	Если Не ЗначениеЗаполнено(ДокументыНаКонтроле) Тогда
		Возврат;
	КонецЕсли;
	
	ВыполнитьКонтрольСопоставленияНоменклатурыДокументов(ДокументыНаКонтроле, КонтекстДиагностики);
	
	СпособыОбработкиДокументов = СпособыОбработкиДокументовАвтоматически(ДокументыНаКонтроле);
	Если Не ЗначениеЗаполнено(СпособыОбработкиДокументов) Тогда
		Возврат;
	КонецЕсли;
	
	ОтразитьЭлектронныеДокументы(СпособыОбработкиДокументов);
...
// Возвращаемое значение:
//  Булево
Функция ТребуетсяОтражениеДвумяОбъектамиУчета(ТипДокумента, СпособОбработки) Экспорт
	
	Если ТипЗнч(СпособОбработки) <> Тип("Массив")
		Или СпособОбработки.Количество() <> 2 Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВключитьКонтрольСопоставленияНоменклатуры(Знач ЭлектронныйДокумент, Знач НоменклатураБезСопоставления) Экспорт
	...
	Попытка
		...
		Для Каждого НоменклатураКонтрагента Из НоменклатураБезСопоставления Цикл
			...
			Если НЕ Метаданные.ОпределяемыеТипы.ВладелецНоменклатурыБЭД.Тип.СодержитТип(
				ТипЗнч(НоменклатураКонтрагента.Владелец)) Тогда
				Продолжить;
			КонецЕсли;	
...
//  ЭлектронныеДокументы - Массив из ДокументСсылка.ЭлектронныйДокументВходящийЭДО
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ВыполнитьКонтрольСопоставленияНоменклатурыДокументов(ЭлектронныеДокументы, КонтекстДиагностики) Экспорт
	
	ДанныеНоменклатурыДокументов = ЭлектронныеДокументыЭДО.ДанныеНоменклатурыДокументов(
		ЭлектронныеДокументы, КонтекстДиагностики);
	НоменклатураДокументовБезСопоставления = НоменклатураЭлектронныхДокументовБезСопоставления(ДанныеНоменклатурыДокументов);
	
	Для Каждого НоменклатураДокументаБезСопоставления Из НоменклатураДокументовБезСопоставления Цикл
		ЭлектронныйДокумент = НоменклатураДокументаБезСопоставления.Ключ;
		НоменклатураБезСопоставления = НоменклатураДокументаБезСопоставления.Значение;
		НаКонтроле = Ложь;
		Если ЗначениеЗаполнено(НоменклатураБезСопоставления) Тогда
			НаКонтроле = ВключитьКонтрольСопоставленияНоменклатуры(ЭлектронныйДокумент, НоменклатураБезСопоставления);
		КонецЕсли;
		ПараметрыКонтроляОтраженияВУчете = НовыеПараметрыКонтроляОтраженияВУчете();
		ПараметрыКонтроляОтраженияВУчете.СопоставитьНоменклатуру = НаКонтроле;
		ПараметрыКонтроляОтраженияВУчете.СоздатьУчетныйДокумент  = Истина;
		ПараметрыКонтроляОтраженияВУчете.ПровестиУчетныйДокумент = Истина;
		УстановитьКонтрольОтраженияВУчете(ЭлектронныйДокумент, ПараметрыКонтроляОтраженияВУчете);
	КонецЦикла;
...
// Используется для переопределения настроек отражения в учете входящих электронных документов по умолчанию.
// 
// Параметры:
//  ТребуетсяОтражениеВУчете - Булево
Процедура ПриОпределенииНеобходимостиОтраженияВУчетеВходящихДокументов(ТребуетсяОтражениеВУчете)
...
Процедура ОтразитьДанныеФормализованногоЭлектронногоДокумента(ДанныеЭлектронногоДокумента, ДанныеДляОтражения, СпособОбработки, ДокументыУчета = Неопределено)
	...
	Попытка
		...
		Если ДанныеДляОтражения.ДанныеТЭДО <> Неопределено Тогда
			
			ТрансграничныйЭДО.ПодготовитьКОтражениюВУчете(
				ДанныеДляОтражения.ДанныеТЭДО, ДанныеЭлектронногоДокумента.Отправитель);
			ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаТЭДО(
				ДокументУчета, СпособОбработки, ДанныеДляОтражения.ДанныеТЭДО);
			
		ИначеЕсли ЕстьЭлектронноеАктирование И МодульЭлектронноеАктированиеЕИС.ЭтоДокументЭлектронногоАктирования(
			ДанныеЭлектронногоДокумента, НовыйЭД) Тогда
		...
		ИначеЕсли НовыйЭД.ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда	
			ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйПередаточныйДокумент_5_03(ДеревоЭД, ДокументыУчета, СпособОбработки, ОписаниеОшибки);
		...
		ИначеЕсли НовыйЭД.ВерсияФормата = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
			ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьСчетНаОплату_5_01(ДеревоЭД, ДокументУчета, СпособОбработки, ОписаниеОшибки);
		ИначеЕсли НовыйЭД.ВерсияФормата = ПоддерживаемыеФорматы.ФНС.АктПриемкиСтроительныхРаботУслуг.ИнформацияПодрядчика Тогда
	...
	Если НЕ ЗначениеЗаполнено(ДокументыУчета) Тогда
		...
		ВидДокумента = ?(ЗначениеЗаполнено(НовыйЭД.ВидЭД),
			ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(НовыйЭД.ВидЭД), ДанныеЭлектронногоДокумента.ВидДокумента);
		...
		Иначе
			ИмяМетода = ?(ДанныеДляОтражения.ДанныеТЭДО = Неопределено,
				"ОбменСКонтрагентамиПереопределяемый.СохранитьДанныеОбъектаВБД",
				"ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаТЭДО");
			ТекстЖурналаРегистрации = СтрШаблон(НСтр("ru = 'Необходимо проверить работу процедуры %1'"), ИмяМетода);
		КонецЕсли;
...
Процедура ПередОтражениемВУчетеЭлектронногоДокумента(ПараметрыОтражения, Отказ = Ложь, ОписаниеОшибки = "")
	...
	ИнтеграцияЭДО.ЗаполнитьНоменклатуруИБВДеревеДокумента(ПараметрыОтражения.Контрагент,
		ПараметрыОтражения.ДеревоДокумента, ПараметрыОтражения.ЭлектронныйДокумент,
		Настройки.ОтражатьТолькоСопоставленные, Отказ);
...
Процедура ДополнитьДеревоДаннымиКомитента(ДеревоДанных, ДанныеЭлектронногоДокумента)
	...
	Если ЭтоФизЛицо Тогда
		...
		ПутьУчастникФИО = ВидУчастника + ".ТипУчастника.ИП.";
		ПоддерживаемыеФорматы = ФорматыЭДО.ПоддерживаемыеФорматы();
		Если ДанныеЭлектронногоДокумента.Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
			Или ДанныеЭлектронногоДокумента.Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
			ПутьУчастникФИО = ПутьУчастникФИО + "ФИО.";
		КонецЕсли;
...
Функция СпособыОбработкиДокументовАвтоматически(ЭлектронныеДокументы)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОтражатьАвтоматически(НастройкиОтраженияВУчете.СпособОбработки) Тогда
...
// Параметры:
//  ДанныеНоменклатурыДокументов - см. ЭлектронныеДокументыЭДО.ДанныеНоменклатурыДокументов
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
//  * Значение - См. НоменклатураКонтрагентовБезСопоставления
Функция НоменклатураЭлектронныхДокументовБезСопоставления(ДанныеНоменклатурыДокументов)
	
	НоменклатураДокументовБезСопоставления = Новый Соответствие; // См. НоменклатураЭлектронныхДокументовБезСопоставления
	
	ЭлектронныеДокументы = ОбщегоНазначения.ВыгрузитьКолонку(ДанныеНоменклатурыДокументов, "Ключ");
	КонтрагентыДокументов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ЭлектронныеДокументы, "Контрагент");
	
	Для Каждого ДанныеНоменклатурыДокумента Из ДанныеНоменклатурыДокументов Цикл
		ЭлектронныйДокумент = ДанныеНоменклатурыДокумента.Ключ;
		ИнформацияОТоваре = ДанныеНоменклатурыДокумента.Значение;
		Контрагент = КонтрагентыДокументов[ЭлектронныйДокумент];
		Владелец = СопоставлениеНоменклатурыКонтрагентовСлужебный.ВладелецНоменклатурыКонтрагента(Контрагент);
		НаборНоменклатурыКонтрагентов = НаборНоменклатурыКонтрагентовПоИнформацииОТоварах(ИнформацияОТоваре, Владелец);
		НоменклатураБезСопоставления = НоменклатураКонтрагентовБезСопоставления(НаборНоменклатурыКонтрагентов);
		НоменклатураДокументовБезСопоставления.Вставить(ЭлектронныйДокумент, НоменклатураБезСопоставления);
	КонецЦикла;
	
	Возврат НоменклатураДокументовБезСопоставления;
	
КонецФункции

// Параметры:
//  НоменклатураДокументовБезСопоставления - См. НоменклатураЭлектронныхДокументовБезСопоставления
// 
// Возвращаемое значение:
//  См. НоменклатураКонтрагентовБезСопоставления
Функция ВыгрузитьНоменклатураДокументовБезСопоставления(НоменклатураДокументовБезСопоставления)
	НоменклатураБезСопоставления = Новый Массив; // См. НоменклатураКонтрагентовБезСопоставления
	Для Каждого НоменклатураДокументаБезСопоставления Из НоменклатураДокументовБезСопоставления Цикл
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НоменклатураБезСопоставления,
			НоменклатураДокументаБезСопоставления.Значение);
	КонецЦикла;
	Возврат НоменклатураБезСопоставления;
КонецФункции
