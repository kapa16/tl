...
Процедура ВыбратьЗначениеДополнительногоРеквизита(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если СписокДоступныхТипов.Количество() > 1 Тогда
		...
		ЗаголовокФормы = НСтр("ru = 'Тип реквизита ""%Свойство%""'; en = 'The ""%Свойство%"" attribute type'");
...
Процедура ВыполнитьЗапросАсинхронно(Владелец, ДлительнаяОперация, ОповещениеОЗавершении,
	...
	Иначе
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Получение данных из 1С:Документооборота.'; en = 'Get data from 1C:Document Management.'");
	КонецЕсли;
...
"Выполнение действия ""%1"" возможно только после записи данных.'; en = 'You have unsaved data."
"You can perform the ""%1"" action only after you save the data.'"),
		...
		ТекстВопроса = СтрШаблон(
			НСтр("ru = '%1"
"Данные будут записаны.'; en = '%1"
"The data will be saved.'"),
			ТекстПредупреждения);
		ПараметрыВопросаДаНет = ПараметрыВопросаДаНет(КодВозвратаДиалога.Да);
		ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'ОК'; en = 'OK'");
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Отмена'; en = 'Cancel'");
...
Процедура ЗаписатьОбъектАсинхронно(УникальныйИдентификаторФормы, ОбъектСтрока, ОповещениеОЗавершении) Экспорт
	...
	ВыполнитьЗапросАсинхронно(
		...
		ОповещениеОЗавершении,
		Истина,
		НСтр("ru = 'Выполняется запись.'; en = 'Saving the object.'"),
		Ложь);
	...
	Иначе
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru = 'Просмотр объектов такого типа не поддерживается в этой версии 1С:Документооборота.'; en = 'View of this type objects is not supported in this version of 1C:Document Management.'"));
		КонецЕсли;
...
Функция ПоказатьВопросДаНетАсинх(ТекстВопроса, ПараметрыВопроса = Неопределено) Экспорт
	...
	Если ПараметрыВопроса.ТекстКнопкиДа = "" Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Да'; en = 'Yes'"));
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.Да, ПараметрыВопроса.ТекстКнопкиДа);
	КонецЕсли;
	Если ПараметрыВопроса.ТекстКнопкиНет = "" Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Нет'; en = 'No'"));
	Иначе
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Недопустимое значение кнопки по умолчанию: %1'; en = 'Invalid default button value: %1'"),
...
Асинх Процедура ПроверитьПодключение(Знач ОписаниеОповещения, Знач Владелец,
	...
	ВыполнитьЗапросАсинхронно(
		...
		НСтр("ru = 'Подключение к 1С:Документооборот.'; en = 'Connect to 1C:Document Management.'"),
		СкрыватьИнтерфейс);
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
		...
		Исключение
			...
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЗаписатьПредупреждение(
				НСтр("ru = 'Не найдены сохраненные учетные данные пользователя 1С:Документооборота'; en = 'Saved accounting data of 1C:Document Management user is not found'"));
		КонецПопытки;
...
"Проверьте <a href = ""%1"">правило интеграции</a>.'; en = 'Cannot fill the object by data from 1C:Document Management."
...
"%1'; en = 'Cannot save changes due to:"
"%1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Не СведенияОРедактировании.Сохранен Или СведенияОРедактировании.НаЧтение Тогда
		Ответ = Ждать ПоказатьВопросДаНетАсинх(НСтр("ru = 'Файл не найден в рабочем каталоге. Освободить его?'; en = 'File is not found in the working directory. Unlock the file?'"));
		Если Ответ = КодВозвратаДиалога.Да Тогда
	...
	ТекстПояснения = СтрШаблон(НСтр("ru = 'Идет сохранение файла ""%1"" (%2). Пожалуйста, подождите...'; en = 'Saving the ""%1"" file (%2). Please wait...'"),
...
Функция НеобходимоДобавитьСканКопиюАсинх(СостояниеРазрешаетДобавлениеСканКопии,
	...
	Если СостояниеРазрешаетДобавлениеСканКопии И Не СостояниеРазрешаетДобавлениеФайла Тогда
		ТекстВопроса = НСтр("ru = 'В текущем состоянии можно добавить только скан-копию оригинала документа. Продолжить?'; en = 'In the current state, you can add only a scanned copy of the original document. Proceed?'");
		ПараметрыВопросаДаНет = ПараметрыВопросаДаНет(КодВозвратаДиалога.Да);
		ПараметрыВопросаДаНет.Заголовок = НСтр("ru = 'Добавление файла'; en = 'Adding the file'");
...
"%1'; en = 'Cannot update file because:"
"%1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецПопытки;
		Возврат;
...
"расширения для работы с файлами.'; en = 'You can open the file directory only after"
"1C:Enterprise Extension is attached.'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОРедактировании.Сохранен Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Файл не найден в рабочем каталоге.'; en = 'File is not found in the working directory.'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Файл не найден в рабочем каталоге.'; en = 'File is not found in the working directory.'"));
		Возврат;
...
Процедура ОткрытьФормуПрисоединенныеФайлы(ВладелецФайла, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Иначе
		...
		ВыполнитьПроверкуЗаписиИПродолжить(
			...
			ОповещениеПродолжения,
			НСтр("ru = 'Присоединенные файлы'; en = 'Attachments'"));
...
Асинх Процедура ОтменитьРедактированиеФайла(Знач ID, Знач ОписаниеОповещенияОЗавершении = Неопределено,
	...
	Ответ = Ждать ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Отмена редактирования файла может привести к потере изменений. Продолжить?'; en = 'If you cancel file editing, the changes will be lost. Continue?'"));
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 освобожден.'; en = 'The %1 file is unlocked.'"), ТекущийФайл.Наименование);
...
Функция ПолучитьФильтрДляДиалогаВыбораФайла() Экспорт
	...
	Фильтр = НСтр("ru = 'Все файлы (*.*)|*.*'; en = 'All files (*.*)|*.*'")
		...
		+ НСтр("ru = '|Картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf'; en = '|Images (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf'");
...
Асинх Процедура ПометитьНаУдалениеСнятьПометкуФайлов(Знач ВыделенныеФайлы, Знач ТекущаяПометкаУдаления,
	...
	ИначеЕсли ВыделенныеФайлыКоличество = 1 Тогда
		Если ТекущаяПометкаУдаления = Истина Тогда
			ТекстШаблона = НСтр("ru = 'Снять с ""%1"" пометку на удаление?'; en = 'Unmark ""%1"" for deletion?'");
		Иначе
			ТекстШаблона = НСтр("ru = 'Пометить ""%1"" на удаление?'; en = 'Mark ""%1"" for deletion?'");
		КонецЕсли;
		ТекстВопроса = СтрШаблон(ТекстШаблона, ВыделенныеФайлы[0].Наименование);
		
	Иначе
		Если ТекущаяПометкаУдаления = Истина Тогда
			ТекстВопроса = НСтр("ru = 'Снять с выделенных файлов пометку на удаление?'; en = 'Unmark selected files for deletion?'");
		Иначе
	...
	Для Каждого Файл Из ВыделенныеФайлы Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 удален.'; en = 'The %1 file is deleted.'"), Файл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Процедура СделатьВерсиюАктивной(ИмяФайла, ИдентификаторФайла, ВладелецФайла, ИдентификаторВерсии,
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СделатьВерсиюАктивной(
		...
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("ru = 'Активная версия изменена.'; en = 'The active version is changed.'");
		КонецЕсли;
...
"так как он сейчас не захвачен на редактирование.'; en = 'Cannot save changes in the file"
"as it is not locked for editing now.'"));
		Возврат;
	КонецЕсли;
	Если СведенияОРедактировании.НаЧтение Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось сохранить изменения файла,"
"так как он сейчас сохранен только для просмотра.'; en = 'Cannot save changes"
...
Асинх Процедура СохранитьФайлыКакНачало(Знач Параметры) Экспорт
	...
	Если Не Ждать РасширениеРаботыСФайламиПодключеноАсинх() Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идет сохранение файла ""%1"". Пожалуйста, подождите...'; en = 'Saving the ""%1"" file. Please wait...'"),
	...
	Если Параметры.ОставшиесяФайлы.Количество() = 0 Тогда
		...
		Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Все файлы (*.%1)|*.%1'; en = 'All files (*.%1)|*.%1'"), Параметры.ТекущийФайл.Расширение, Параметры.ТекущийФайл.Расширение);
...
Процедура ИнформироватьОПодписании(Представление, ДобавленныеПодписиОбъектов, ДокументID,
	...
	Состояние(СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""'; en = 'Signature is set for ""%1""'"), Представление));
	...
	Для Каждого Элемент Из ДобавленныеПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип <> "DMFile" Тогда
			Продолжить;
		КонецЕсли;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""'; en = 'Signature is set for ""%1""'"), ДанныеВладельцаПодписи.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Процедура НачатьФормированиеПодписиОбъекта(Форма, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Отсутствует стандартная подсистема ""Электронная подпись"".'; en = 'The standard ""Digital signature"" subsystem is missing.'"));
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОбъектДляПодписи Из МассивОбъектовДляПодписи Цикл
		...
		ИначеЕсли ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			...
			Если Строки.Количество() = 1 Тогда
				ПредставлениеФайла = СтрШаблон(НСтр("ru = 'Файл: %1'; en = 'File: %1'"), Строки[0].Наименование);
				ДанныеДляПодписи.Вставить("Представление",
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ru = 'Подписание документа'; en = 'Document signing'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("ru = 'Документ'; en = 'Document'"));
	ОписаниеДанных.Вставить("НаборДанных", НаборДанных);
	ОписаниеДанных.Вставить("Представление", Форма.Наименование);
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Объекты (%1)'; en = 'Objects (%1)'"));
...
Процедура ПодписатьФайл(СвойстваФайла) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция", НСтр("ru = 'Подписание файла'; en = 'Sign file'"));
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Если Не ЕстьПодсистемаЭП Тогда
			ДанныеСтроки.Статус = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СтатусЭП(
				Истина,
				НСтр("ru = 'отсутствует стандартная подсистема ""Электронная подпись""'; en = 'the standard ""Digital signature"" subsystem is missing'"));
...
Асинх Процедура УдалитьПодписи(Знач УдаленныеСтроки, Знач ТаблицаПодписей, Знач ВладелецФайла,
	...
	ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Удалить'; en = 'Delete'");
	ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Не удалять'; en = 'Do not delete'");
	Ответ = Ждать ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Удалить выделенные подписи?'; en = 'Do you want to delete the selected signatures?'"),
		ПараметрыВопросаДаНет);
	...
	Для Каждого ЭлементСоответствия Из ПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип <> "DMFile" Тогда
			Продолжить;
		КонецЕсли;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Удалена подпись файла ""%1""'; en = 'The ""%1"" file signature is deleted.'"), ДанныеВладельцаПодписи.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
		ДействиеНадФайлом.ИзменениеЭПФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
...
Асинх Процедура УстановитьКомпоненту(Знач ОбработчикРезультата) Экспорт
	...
	Если ПараметрыПриложения["ИнтеграцияС1СДокументооборотКомпонентаПолученияКартинкиИзБуфера"] = Неопределено Тогда
		...
		Если КодВозврата Тогда
			Состояние(НСтр("ru = 'Компонента сканирования уже установлена.'; en = 'The scanner add-in is already installed.'"));
		Иначе
...
Процедура ВставитьКартинкуИзБуфераЗавершение(Результат, Параметры) Экспорт
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Буфер обмена не содержит картинки'; en = 'Clipboard contains no pictures'"));
	КонецЕсли;
...
Процедура ЗакончитьРедактированиеСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СохранитьСведенияОРедактированииФайла(
		СведенияОРедактировании);
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 обновлен и освобожден.'; en = 'The %1 file is updated and unlocked.'"), Параметры.ТекущийФайл.Наименование);
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.РедактированиеФайлаЗавершено = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
...
Процедура ОбновитьИзФайлаНаДискеПослеПомещенияФайлаБезРасширения(ОписаниеПомещенногоФайла, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(
			Параметры) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 обновлен.'; en = 'The %1 file is updated.'"), Параметры.ТекущийФайл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
""
"Операция прервана.'; en = 'The %1 file"
...
Процедура ОткрытьПрисоединенныеФайлыЗавершение(ВладелецФайла, ПараметрыОткрытия) Экспорт
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		...
		Если ВладелецФайла = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не указан владелец файла в интегрированной системе.'; en = 'A file owner is not specified in the integrated system.'"));
			Возврат;
		КонецЕсли;
...
Процедура ОткрытьФайлыПослеПолучения(ПолныеИменаФайлов, Параметры) Экспорт
	...
	Если Параметры.НаЧтение = Ложь Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Начато редактирование файла %1.'; en = 'Started editing the %1 file.'"), Параметры.ТекущийФайл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
"Возможно, он заблокирован другим приложением.'; en = 'Cannot replace a file on the disk with the file from 1C:Document Management."
...
"он заблокирован другим приложением.'; en = 'Cannot open the file to edit."
...
Асинх Процедура ПродолжитьСохранитьКакПослеВыбораИсточника(Знач ПолучаемыеФайлы, Знач Параметры) Экспорт
	...
	Если ТипЗнч(ПолучаемыеФайлы) = Тип("Массив") И ПолучаемыеФайлы.Количество() > 0 Тогда
		...
		Если Параметры.РежимПолучения = "ВзятьИзДокументооборота" Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Состояние(НСтр("ru = 'Файл сохранен.'; en = 'File is saved.'"),, ПолныйПуть);
	КонецЕсли;
...
Процедура СоздатьФайлСДискаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		...
		Если Параметры.ЯвляетсяСканКопией Тогда
			ВыборФайла.Заголовок = НСтр("ru = 'Выбор скан-копии оригинала документа'; en = 'Select the scanned copy of the original document'");
			...
			Если ЗначениеЗаполнено(СтрокаРасширений) Тогда
				...
				ВыборФайла.Фильтр = СтрШаблон(НСтр("ru = 'Скан-копии (%1)|%2|Все файлы (*.*)|*.*'; en = 'Scanned copies (%1)|%2|All files (*.*)|*.*'"),
...
"%1'; en = 'Cannot create file because:"
"%1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецПопытки;
...
Процедура СохранитьИзмененияСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменения файла %1 сохранены.'; en = 'The %1 file changes are saved.'"), Параметры.ТекущийФайл.Наименование);
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.РедактированиеФайлаЗавершено = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
	...
	ИначеЕсли Результат = "ИспользоватьСуществующий" Тогда
		...
		Исключение
			...
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось скопировать файл (""%1"").'; en = 'Cannot copy file (""%1"").'"),
...
Процедура НачатьДобавлениеЭПИзФайлаЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если МассивПодписей.Количество() = 1 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавлена подпись из файла для ""%1""'; en = 'The signature from file was added for ""%1""'"), СвойстваФайла.ИмяФайла);
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавлены подписи из файлов для ""%1""'; en = 'Signatures from files were added for ""%1""'"), СвойстваФайла.ИмяФайла);
	КонецЕсли;
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.ИзменениеЭПФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
...
Процедура НачатьДобавлениеЭПИзФайлаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Не Подключено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для выполнения операции нужно установить расширение работы с файлами.'; en = 'To perform the operation, install 1C:Enterprise Extension.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодписатьФайлЗавершение(ОписаниеДанных, СвойстваФайла) Экспорт
	...
	Если ТипЗнч(ОписаниеДанных) = Тип("Структура") И ОписаниеДанных.Свойство("СвойстваПодписи", СвойстваПодписи) Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""'; en = 'Signature is set for ""%1""'"), СвойстваФайла.ИмяФайла);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
		ДействиеНадФайлом.ИзменениеЭПФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
		ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
...
#Если МобильныйКлиент Тогда
	...
	ПоказатьПредупреждение(, НСтр("ru = 'В мобильном клиенте печать файлов не поддерживается.'; en = 'Printing files is not supported in the mobile client.'"));
	Возврат;
	
#ИначеЕсли ВебКлиент Тогда
	
	ПоказатьПредупреждение(, НСтр("ru = 'В веб-клиенте печать файлов не поддерживается.'; en = 'Printing files is not supported in the web client.'"));
	Возврат;
	...
	Иначе
		...
		Исключение
			...
			ПоказатьПредупреждение(, СтрШаблон(
				НСтр("ru = 'Описание=""%1""'; en = 'Details=""%1""'"),
				ОбработкаОшибок.КраткоеПредставлениеОшибки(Инфо)));
		КонецПопытки;
...
Функция ОткрытьФайлПриложением(ПолноеИмяФайла)
	...
	Если ЗначениеЗаполнено(СтруктураПолногоИмени.Путь) Тогда
		...
		Исключение
			ТекстПредупреждения = СтрШаблон(НСтр("ru = 'Описание=""%1""'; en = 'Details=""%1""'"),
				ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат Ложь;
		КонецПопытки;
...
Функция ОшибкаПриДобавленииСканКопии(ЯвляетсяСканКопией, Файл)
	...
	Если ЯвляетсяСканКопией Тогда
		...
		Если Не ПустаяСтрока(СписокРасширенийСканКопийОригиналов)
			...
			ТекстПредупреждения = СтрШаблон(
				НСтр("ru = 'Файл ""%1"" не является скан-копией. Выберите файл с расширением: %2.'; en = 'The %1 file is not a scanned copy. Select a file with the extension: %2.'"),
				Файл.Имя,
		...
		Если ЗначениеЗаполнено(СведенияОРедактировании.РекомендуемаяПапка) Тогда
			...
			Иначе // Попытаемся создать эту папку.
				...
				Исключение // Нет доступа или иные проблемы.
					...
					ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЗаписатьПредупреждение(
						СтрШаблон(
							НСтр("ru = 'Нет доступа к рабочей папке: %1'; en = 'No access to the working folder: %1'"),
...
"""%2"".'; en = 'Cannot create directory ""%1"". Reason:"
"%2'"),
					КаталогССуффиксом,
					ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) );
			КонецПопытки;
	...
	КонецЦикла;
	Если Не Успех Тогда
		ВызватьИсключение НСтр("ru = 'Не удается создать файл в каталоге временных файлов.'; en = 'Cannot create file in the temporary directory.'");
	КонецЕсли;
...
Асинх Функция НуженВыборДействийПриОбнаруженииОтличийФайлаАсинх(Знач СведенияОРедактировании, Знач Параметры,
	...
	Если РазницаДат > 0 Тогда
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'На диске новый файл'; en = 'A new file on the disk'"));
...
"возможно, с потерей изменений?'; en = 'Disk file is newer than the file in 1C:Document Management."
...
"and possibly lose the changes?'"));
	Иначе
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'В 1С:Документообороте новый файл'; en = 'There is a new file in 1C:Document Management.'"));
		ПараметрыФормы.Вставить("Сообщение",
...
"изменений, или использовать существующий файл?'; en = 'File in 1C:Document Management is newer than the one on the disk."
"It might have been modified."
""
...
Процедура СоздатьФайлСДискаЗавершение(Параметры)
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Создан файл ""%1""'; en = 'The ""%1"" file is created'"), Параметры.ТекущийФайл.Наименование);
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
