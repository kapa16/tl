...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено
		...
		ВызватьИсключение НСтр("ru = 'Невозможно загрузить файл без установленного расширения для работы с 1С:Предприятием.';
								|en = 'Cannot import the file as 1C:Enterprise Extension is not installed.'");
	КонецЕсли;
	...
	Исключение
		...
		Если СтрНайти(ОписаниеОшибки, "32(0x00000020)") Тогда 
			ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Завершите работу с файлом в другом приложении.';
															|en = 'Complete the operation with the file in another application.'"),, 
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Расширение для работы с 1С:Предприятием не установлено, переход по ссылке ""%1"" невозможен.';
				|en = 'Cannot follow the link ""%1"" as 1C:Enterprise Extension is not installed.'"),
			НавигационнаяСсылка);
...
Процедура ЗапуститьПрограммуПослеПроверкиРасширения1СПредприятия(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Не установлено расширение для работы с 1С:Предприятием, запуск приложения невозможен.';
								|en = 'Cannot start the application because 1C:Enterprise Extension is not installed.'");
	...
	Если Контекст.ДождатьсяЗавершения И КодВозврата = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Возникла непредвиденная ситуация при запуске приложения
				|%1';
				|en = 'Exception occurred during startup of
				|%1'"),
