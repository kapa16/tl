...
Процедура НачатьСозданиеЭлектронногоДокументаПоФайлу(ОповещениеОЗавершении, ПараметрыФормирования, ПараметрыФайла) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.ИдентификаторДокумента) Тогда
		ОписаниеДокумента.Идентификатор = ПараметрыФормирования.ИдентификаторДокумента;
	Иначе
	...
	ОписаниеДокумента.ОбъектыУчета = ПараметрыФормирования.ОбъектыУчета;
	...
	ПараметрыВыполненияДействийПоЭДО = ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО();
	ПараметрыВыполненияДействийПоЭДО.ОбъектыДействий.ОписанияДокументовПоФайлам.Добавить(ОписаниеДокумента);
	ЭлектронныеДокументыЭДОКлиентСервер.ДобавитьДействие(ПараметрыВыполненияДействийПоЭДО.НаборДействий,
	...
	ОбработчикЗавершения = Новый ОписаниеОповещения("ПослеФормированияЭлектронногоДокументаПоФайлу", ЭтотОбъект, Контекст);
...
Процедура ПроверитьЗаполнениеУПД_5_02ВФормеРучногоФормированияОтветногоТитула(Форма, СписокОшибок) Экспорт
	...
	Если Форма.ЗакупкаДляГосударственныхНужд Тогда
		...
		СтрокаОшибки = НСтр("ru = 'В строке %1 %2%3'");
		Для Каждого СтрокаДенежныхОбязательств Из Форма.ДенежныеОбязательства Цикл
			
			Если НЕ ЗначениеЗаполнено(СтрокаДенежныхОбязательств.ВидСредств) Тогда
				СписокОшибок.Добавить(СтрШаблон(СтрокаОшибки, СтрокаДенежныхОбязательств.НомерСтрокиИнформацииПродавца, 
					Форма.Элементы.ДенежныеОбязательстваВидСредств.Заголовок, ТекстОшибкиЗначениеНеЗаполнено),
					Форма.Элементы.ДенежныеОбязательстваВидСредств.Имя);
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(СтрокаДенежныхОбязательств.КодБюджетнойКлассификации) Тогда
				СписокОшибок.Добавить(СтрШаблон(СтрокаОшибки, СтрокаДенежныхОбязательств.НомерСтрокиИнформацииПродавца, 
					Форма.Элементы.ДенежныеОбязательстваКодБюджетнойКлассификации.Заголовок, ТекстОшибкиЗначениеНеЗаполнено),
					Форма.Элементы.ДенежныеОбязательстваКодБюджетнойКлассификации.Имя);
			КонецЕсли;
...
// Обработчик события, вызываемый после завершения формирования электронного документа по файлу.
//
// Параметры:
//   Итог - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыйИтогВыполненияДействийПоЭДО
//   Контекст - Структура:
//   * ОповещениеОЗавершении - см. НачатьСозданиеЭлектронногоДокументаПоФайлу.ОповещениеОЗавершении
Процедура ПослеФормированияЭлектронногоДокументаПоФайлу(Итог, Контекст) Экспорт
	
	ЭлектронныйДокумент = Неопределено;
	
	Если ЗначениеЗаполнено(Итог) Тогда
		Для Каждого ОбработанныйДокумент Из Итог.ОбработанныеДокументы Цикл
			ЭлектронныйДокумент = ОбработанныйДокумент.Ключ;
			Прервать;
		КонецЦикла;
	КонецЕсли;
	
	ВыполнитьОбработкуОповещения(Контекст.ОповещениеОЗавершении, ЭлектронныйДокумент);
