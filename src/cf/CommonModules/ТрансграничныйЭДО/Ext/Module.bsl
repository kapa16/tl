...
// Задает режим использования отдельных типов для трансграничных документов
// 
// Возвращаемое значение:
//  Булево
//
Функция ТипыДокументовИспользуются() Экспорт
	Возврат Истина;
КонецФункции

// Формирует краткое наименование типа документа.
// 
// Параметры:
...
//  Строка
//
Функция КраткоеНаименованиеТипаДокумента(ТипДокумента) Экспорт
	
	ТипыДокументов = СтандартныеТипыДокументов();
	
	Если ТипДокумента = ТипыДокументов.ТоварнаяНакладнаяРБ Тогда
		Возврат НСтр("ru = 'Товарная накладная РБ'");
	КонецЕсли;
...
// Дополняет типы документов.
// 
// Параметры:
//  ТипыДокументов - Соответствие из КлючИЗначение: 
//  * Ключ - Строка
//  * Значение - ПеречислениеСсылка.ТипыДокументовЭДО
//
Процедура ДополнитьТипыДокументов(ТипыДокументов) Экспорт
	
	Если НЕ ТипыДокументовИспользуются() Тогда
		Возврат;
	КонецЕсли;
	
	ТипыДокументов.Вставить("ТоварнаяНакладнаяРБ", Перечисления.ТипыДокументовЭДО.ТоварнаяНакладнаяРБ);
КонецПроцедуры

// Дополняет связь типов и форматов ЭД.
// 
// Параметры:
//  ДанныеСервиса - Структура
//
Процедура ДополнитьСвязьТиповИФорматовЭД(ДанныеСервиса) Экспорт
	
	Если НЕ ТипыДокументовИспользуются() Тогда
		Возврат;
	КонецЕсли;
	
	ТекстМакета = Обработки.ТрансграничныйЭДО.ПолучитьМакет("СвязьВидовИФорматовДокументовЭДО").ПолучитьТекст();
	ДанныеТЭДО = ОбщегоНазначения.JSONВЗначение(ТекстМакета, , Ложь); // Структура
	
	ИмяПоляТипов = "documentTypes";
	ТипыДокументов = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДанныеСервиса, ИмяПоляТипов, Новый Массив()); // Массив из Структура
	ТипыДокументовТЭДО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДанныеТЭДО, ИмяПоляТипов, Новый Массив()); // Массив из Структура
		
	Если ЗначениеЗаполнено(ТипыДокументов) И ЗначениеЗаполнено(ТипыДокументовТЭДО) Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТипыДокументов, ТипыДокументовТЭДО);
	КонецЕсли;
	
	ИмяПоляФорматов = "documentTypeFormats";
	ФорматыДокументов = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДанныеСервиса, ИмяПоляФорматов, Новый Массив()); // Массив из Структура
	ФорматыДокументовТЭДО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДанныеТЭДО, ИмяПоляФорматов, Новый Массив()); // Массив из Структура
	Если ЗначениеЗаполнено(ФорматыДокументов) И ЗначениеЗаполнено(ФорматыДокументовТЭДО) Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФорматыДокументов, ФорматыДокументовТЭДО);
	КонецЕсли;
	
КонецПроцедуры

// Отмечает поддерживаемые форматы.
// 
// Параметры:
//  ТаблицаФорматов - ТаблицаЗначений
//
Процедура ОтметитьПоддерживаемыеФорматы(ТаблицаФорматов) Экспорт
	
	Если НЕ ТипыДокументовИспользуются() Тогда
		Возврат;
	КонецЕсли;
	
	КэшФорматов = Новый Соответствие();
	
	Для Каждого СтрокаТаблицы Из ТаблицаФорматов Цикл
		
		Формат = СтрокаТаблицы["Формат"]; // Строка
		ЭтоИностранныйФормат = КэшФорматов[Формат];
		
		Если ЭтоИностранныйФормат = Истина ИЛИ ЭтоИностранныйФормат(Формат) Тогда
			
			Если ЭтоИностранныйФормат = Неопределено Тогда
				КэшФорматов.Вставить(Формат, Истина);
			КонецЕсли;
			
			СтрокаТаблицы["ПоддерживаетсяКонфигурацией"] = Истина;
			Продолжить;
			
		КонецЕсли;
			
		Если ЭтоИностранныйФормат = Неопределено Тогда
			КэшФорматов.Вставить(Формат, Ложь);
		КонецЕсли;
...
// Дополняет данными трансграничных форматов ЭД.
// 
// Параметры:
//  ФорматыСервиса - Структура
//  
Процедура ДополнитьДаннымиФорматовЭД(ФорматыСервиса) Экспорт
	
	Если НЕ ТипыДокументовИспользуются() Тогда
		Возврат;
	КонецЕсли;
	
	ИмяПоля = "documentFormats";
	Форматы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ФорматыСервиса, ИмяПоля, Новый Массив()); // Массив
	Если НЕ ЗначениеЗаполнено(Форматы) Тогда
		Возврат;
	КонецЕсли;
	
	ТекстМакета = Обработки.ТрансграничныйЭДО.ПолучитьМакет("ФорматыЭлектронныхДокументов").ПолучитьТекст();
	ДанныеФорматов = ОбщегоНазначения.JSONВЗначение(ТекстМакета, , Ложь); // Структура
	ФорматыТЭДО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДанныеФорматов, ИмяПоля, Новый Массив()); // Массив
	
	Если ЗначениеЗаполнено(ФорматыТЭДО) Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Форматы, ФорматыТЭДО);
	КонецЕсли;
	
КонецПроцедуры

// Перечисляет стандартные типы документов.
// 
// Возвращаемое значение:
//  Структура:
//  * ТоварнаяНакладнаяРБ - ПеречислениеСсылка.ТипыДокументовЭДО
//
Функция СтандартныеТипыДокументов() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("ТоварнаяНакладнаяРБ", Перечисления.ТипыДокументовЭДО.ТоварнаяНакладнаяРБ);
	Возврат Результат;
	
КонецФункции

// Вызывается при определении используемых типов электронных документов.
// 
// Параметры:
//  АктуальныеТипы - Соответствие Из КлючИЗначение:
//  * Ключ - ПеречислениеСсылка.ТипыДокументовЭДО
//  * Значение - см. ИнтеграцияЭДО.НовыеСвойстваАктуальногоТипаДокумента
//
Процедура ПриОпределенииИспользуемыхТиповЭлектронныхДокументов(АктуальныеТипы) Экспорт
	...
	Свойства = ИнтеграцияЭДО.НовыеСвойстваАктуальногоТипаДокумента();
	Свойства.ИспользоватьДляВходящих = Истина;
	Свойства.ИспользоватьДляИсходящих = Ложь;
	ТипыДокументов = СтандартныеТипыДокументов();
	АктуальныеТипы.Вставить(ТипыДокументов.ТоварнаяНакладнаяРБ, Свойства);
...
Процедура ЗаполнитьСопоставленнуюНоменклатуру(ДанныеЭД, Отправитель)
	...
	Для Каждого КлючИЗначение Из ДанныеЭД.Сопоставление Цикл
		...
		НоменклатураКонтрагента.Владелец =
			СопоставлениеНоменклатурыКонтрагентовСлужебный.ВладелецНоменклатурыКонтрагента(Отправитель);
