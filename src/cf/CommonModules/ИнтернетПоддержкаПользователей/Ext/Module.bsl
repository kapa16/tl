...
Функция ПроверитьЛогинИПароль(Логин, Пароль) Экспорт
	...
	РезультатЗапроса = ЗагрузитьСодержимоеИзИнтернет(
		URLСервиса,
		,
		,
		ДопПараметрыЗапроса);
	...
	ИначеЕсли РезультатЗапроса.КодСостояния < 0 Тогда
		Результат.КодОшибки         = РезультатЗапроса.КодОшибки;
		Результат.СообщениеОбОшибке = РезультатЗапроса.СообщениеОбОшибке;
	Иначе
...
Функция ПроверитьURLДоступен(
	...
	Если Не РазрешенаРаботаСХостом(URL) Тогда
		...
		РезультатПроверки.ИнформацияОбОшибке = НСтр("ru = 'Работа с внешними ресурсами заблокирована.
			|Обращение к сервисам Интернет-поддержки запрещено.
			|Для разрешения работы с сервисами откройте раздел Интернет-поддержка и сервисы и нажмите ""Настроить"" или разблокируйте работу с внешними ресурсами.'");
...
//  Строка - текст ошибки
//
Функция ЗапрещенДоступКИнтернетПоддержкеТекст() Экспорт
...
Функция ЗагрузитьСодержимоеИзИнтернет(
	...
	Если Не РазрешенаРаботаСХостом(URL) Тогда
		...
		ИнформацияОбОшибке = НСтр("ru = 'Работа с внешними ресурсами заблокирована.
			|Обращение к сервисам Интернет-поддержки запрещено.
			|Для разрешения работы с сервисами откройте раздел Интернет-поддержка и сервисы и нажмите ""Настроить"" или разблокируйте работу с внешними ресурсами.'");
	...
	ИзвлечьGZip =
		(НРег(ЗначениеЗаголовка(Ответ.Заголовки, "content-encoding")) = "gzip");
	...
	ИначеЕсли ПараметрыПолучения.ФорматОтвета = 1 Тогда
		Результат.Содержимое = ?(ИзвлечьGZip,
			ПолучитьСтрокуИзДвоичныхДанных(
				ЧтениеСодержимогоGZip(
					Результат,
...
// Возвращает значение заголовка вне зависимости от регистра.
// При наличии заголовков, отличающихся регистром, будет возвращен первый подходящий.
//
// Параметры:
//  Заголовки - Соответствие - HTTP-заголовки HTTP-ответа или HTTP-запроса в виде соответствия.
//  ИмяЗаголовка - Строка - имя заголовка, значение которого нужно получить.
// 
// Возвращаемое значение:
//  Строка, Неопределено - значение заголовка, если заголовок не найден - возвращается Неопределено.
//
Функция ЗначениеЗаголовка(Заголовки, ИмяЗаголовка) Экспорт
	
	Ключ = НРег(ИмяЗаголовка);
	
	Для Каждого Заголовок Из Заголовки Цикл
		Если НРег(Заголовок.Ключ) = Ключ Тогда
...
//
Функция РазрешенаРаботаСХостом(URL) Экспорт
...
// Выполняет снятие блокировки на работу с сервисом по переданному URL.
//
// Параметры:
//  URL - Строка - URL вызываемой операции.
//
Процедура РазблокироватьРаботуСХостом(URL) Экспорт
	...
	Если ПараметрыСервиса <> Неопределено Тогда
		...
		ПараметрыСервиса.РазрешенаРабота = Истина;
...
Функция СлужебнаяURLДляПереходаНаСтраницуИнтегрированногоСайта(URLСтраницыСайта) Экспорт
	...
	Если Не ОбщегоНазначения.РазрешенДоступКИнтернетСервисам(Ложь) Тогда
		Результат.КодОшибки = "ЗапрещенДоступКИнтернетСервисам";
		Возврат Результат;
	КонецЕсли;
...
Процедура ПриИзмененииДанныхАутентификации(Логин, Пароль)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		...
		МодульНастройкиПлатежныхСистемСлужебный.ПриИзмененииДанныхАутентификации(Логин);
	КонецЕсли;
	// Конец НастройкиПлатежныхСистем
...
Процедура УстановитьОписаниеОшибки(
	...
	ИначеЕсли КодОшибки = "LockError" Тогда
		Сообщение = "";
	КонецЕсли;
...
Функция ЧтениеСодержимогоGZip(РезультатЗагрузки, ДвоичныеДанные)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПриложения, "8.3.26.1498") < 0 Тогда
		// Был получен некорректный ответ от сервера.
		УстановитьОписаниеОшибки(
			РезультатЗагрузки,
			"ServerError",
			НСтр("ru = 'Некорректный ответ сервера.'"),
			НСтр("ru = 'Ошибка сервера при получении ответа. Некорректный (неподдерживаемый) ответ (gzip).'"),
			Новый Массив);
		Возврат ДвоичныеДанные;
	КонецЕсли;
	
	Каталог = ПолучитьИмяВременногоФайла("gzip");
	
	Попытка
		
		ДанныеВПотоке = ДвоичныеДанные.ОткрытьПотокДляЧтения();
		ЧтениеФайлаАрхива = ЧтениеФайлаАрхива(ДанныеВПотоке);
		ИмяФайла = ЧтениеФайлаАрхива.Элементы[0].Имя;
		ЧтениеФайлаАрхива.Извлечь(
			ЧтениеФайлаАрхива.Элементы[0],
			Каталог,
			РежимВосстановленияПутейФайлаАрхива().НеВосстанавливать);
		ЧтениеФайлаАрхива.Закрыть();
		ДанныеВПотоке.Закрыть();
		Результат = Новый ДвоичныеДанные(
				Каталог + ПолучитьРазделительПути() + ИмяФайла);
	...
	Исключение
		...
		ПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке());
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать данные gzip. %1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЧтениеФайлаАрхива(ДанныеВПотоке)
	
	// АПК:488-выкл Поддержка новых возможностей платформы (исполняемый код безопасен)
	Возврат Вычислить("Новый ЧтениеФайлаАрхива(ДанныеВПотоке, ,ТипФайлаАрхива.GZIP)")
	// АПК:488-вкл
	
КонецФункции

Функция РежимВосстановленияПутейФайлаАрхива()
	
	// АПК:488-выкл Поддержка новых возможностей платформы (исполняемый код безопасен)
	Возврат Вычислить("РежимВосстановленияПутейФайлаАрхива")
	// АПК:488-вкл
...
Функция ПараметрыБлокировкиССервисамиИнтернетПоддержки() Экспорт
	...
	Если ТипЗнч(Сохраненные) = Тип("Структура") Тогда
		ВосстановитьПараметрыБлокировки(Результат, Сохраненные);
	КонецЕсли;
...
Функция ХостыСервисовИнтернетПоддержки()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		МодульНастройкиПлатежныхСистемСлужебный = ОбщегоНазначения.ОбщийМодуль("НастройкиПлатежныхСистемСлужебный");
		МодульНастройкиПлатежныхСистемСлужебный.ПриЗаполненииХостовСервисовИнтернетПоддержки(ХостыСервисовИнтернетПоддержки);
	КонецЕсли;
...
// Загружает сохраненные значения настроек доступности сервисов Интернет-поддержки.
// 
// Параметры:
//  Текущие - см. НовыеПараметрыБлокировкиССервисамиИнтернетПоддержки.
//  Сохраненные - см. НовыеПараметрыБлокировкиССервисамиИнтернетПоддержки.
Процедура ВосстановитьПараметрыБлокировки(Текущие, Сохраненные)
	
	Эталон = Новый Структура;
	Эталон.Вставить("РаботаССервисамиИнтернетПоддержкиЗаблокирована", Ложь);
	Эталон.Вставить("СервисыИнтернетПоддержки",       Неопределено);
	
	ЗаполнитьЗначенияСвойств(Эталон, Сохраненные);
	
	ЗаполнитьЗначенияСвойств(
		Текущие,
		Эталон,
		"РаботаССервисамиИнтернетПоддержкиЗаблокирована");
		
	Если Эталон.СервисыИнтернетПоддержки <> Неопределено Тогда
		Для Каждого КлючЗначение Из Эталон.СервисыИнтернетПоддержки Цикл
			Если Текущие.СервисыИнтернетПоддержки.Получить(КлючЗначение.Ключ) <> Неопределено Тогда
				Текущие.СервисыИнтернетПоддержки.Вставить(
					КлючЗначение.Ключ,
					КлючЗначение.Значение);
			КонецЕсли;
		КонецЦикла;
...
#Область КодированиеСтроки

// Преобразует переданную строку в кодировку URL.
//
// Параметры:
//  СтрокаДляКодирования - Строка - строка для кодирования.
// 
// Возвращаемое значение:
//  Строка - перекодированная строка.
//
Функция СтрокаВURLКодировке(СтрокаДляКодирования) Экспорт
