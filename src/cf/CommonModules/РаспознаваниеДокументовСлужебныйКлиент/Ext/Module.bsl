...
Процедура ПоказатьВопросПользователю(ОписаниеОповещенияОЗавершении, ТекстВопроса, Кнопки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Да, НСтр("ru = 'Да'; en = 'Yes'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Нет, НСтр("ru = 'Нет'; en = 'No'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.ОК, НСтр("ru = 'ОК'; en = 'OK'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Отмена'; en = 'Cancel'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Повторить, НСтр("ru = 'Повторить'; en = 'Repeat'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Прервать, НСтр("ru = 'Прервать'; en = 'Interrupt'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Пропустить, НСтр("ru = 'Пропустить'; en = 'Skip'"));
	ПредставленияКнопок.Вставить(КодВозвратаДиалога.Таймаут, НСтр("ru = 'Таймаут'; en = 'Time-out'"));
...
"Обратитесь к администратору приложения или техническому специалисту'; en = 'Cannot save the files. They might have been deleted."
...
Процедура ПослеСохраненияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы <> Неопределено Тогда
		...
		Если ПолученныеФайлы.Количество() = 1 Тогда
			ТекстОповещения = НСтр("ru = 'Распознаваемый документ успешно сохранен в файл'; en = 'The document under recognition is successfully saved to the file'");
		Иначе
			ТекстОповещения = НСтр("ru = 'Файлы сохранены в выбранную папку'; en = 'The files are saved to the selected folder'");
		КонецЕсли;
		
		Заголовок = НСтр("ru = 'Сохранение файлов'; en = 'Save files'");
...
Процедура ПоказатьРаспознанныйДокумент(
	...
	Если Не ПустаяСтрока(ИмяФормыОбработчика) Тогда  
		...
		КоличествоСтрокПриКоторойВозможнаДолгаяЗагрузка = 2000;
		
		Если КоличествоСтрокТаблицы > КоличествоСтрокПриКоторойВозможнаДолгаяЗагрузка 
			И ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда  
			
			МодульОбщегоНазначенияБПКлиент 			= ОбщегоНазначенияКлиент.ОбщийМодуль("ОбщегоНазначенияБПКлиент");	
			МодульОбщегоНазначенияБПКлиентСервер	= ОбщегоНазначенияКлиент.ОбщийМодуль("ОбщегоНазначенияБПКлиентСервер");		
			
			ПараметрыОткрытия = МодульОбщегоНазначенияБПКлиентСервер.ПараметрыОткрытияФормыСОжиданием();
			ПараметрыОткрытия.Заголовок = НСтр("ru = 'Распознанный документ'; en = 'Recognized document'");
			ПараметрыОткрытия.ИмяФормы = ИмяФормыОбработчика;
			
			МодульОбщегоНазначенияБПКлиент.ОткрытьФормуСОжиданием(ПараметрыОткрытия, ПараметрыФормы, Истина);
...
Процедура ИзменитьТипДокумента(Форма, ОбработкаЗавершения, ПредлагатьЗаписать = Истина) Экспорт 
	...
	Если ПредлагатьЗаписать Тогда
		...
		Кнопки.Добавить("Записать", НСтр("ru = 'Записать'; en = 'Save'"));
		Кнопки.Добавить("Отмена", НСтр("ru = 'Отмена'; en = 'Cancel'"));
		
		Параметры = ПараметрыВопросаПользователю();
		Параметры.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Параметры.Заголовок = НСтр("ru = 'Записать документ'; en = 'Write a document'");
...
"Записать?'; en = 'To change the document type, save it."
"Save the document?'"),
			Кнопки,
			Параметры
		);
...
"Записать?'; en = 'To change the document processing option, save it."
"Save the document?'"),
			Кнопки,
			Параметры
		);
...
Процедура ОткрытьФормуСозданияЭлемента(
	...
	ИмяФормы = "Обработка.РаспознаваниеДокументов.Форма.СозданиеЭлементаСОтображениемКартинки";
	РаспознаваниеДокументовКлиентПереопределяемый.ИмяФормыСозданияЭлемента(ИмяЭлемента, ИмяФормы);
	
	ОткрытьФорму(ИмяФормы, ПараметрыОткрытия, Элемент);
...
Функция ЭтоФормаВыбораАналитикиУчета(ИсточникВыбора) Экспорт
	
	ФормыВыборкаАналитикиУчета = Новый Массив;
	РаспознаваниеДокументовКлиентПереопределяемый.ПриОпределенииФормВыборкаАналитикиУчета(
		ФормыВыборкаАналитикиУчета);
	
	Возврат (ФормыВыборкаАналитикиУчета.Найти(ИсточникВыбора.ИмяФормы) <> Неопределено);
...
"Для продолжения работы, пожалуйста, войдите с помощью учетной записи 1С:ИТС.'; en = 'The pilot use of the document recognition service is completed."
"To continue, please log in using your 1C:ITS account.'");
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Подключить", НСтр("ru = 'Войти'; en = 'Login'"));
	КнопкиВопроса.Добавить("НапомнитьПозже", НСтр("ru = 'Напомнить позже'; en = 'Remind me later'"));
	КнопкиВопроса.Добавить("НеСегодня", НСтр("ru = 'Не напоминать сегодня'; en = 'Do not remind today'"));
	
	ПараметрыВопроса = ПараметрыВопросаПользователю();
	ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыВопроса.Картинка = БиблиотекаКартинок.Предупреждение32;
	ПараметрыВопроса.Заголовок = НСтр("ru = 'Перейти на коммерческое использование сервиса'; en = 'Switch to commercial service use'");
...
"Необходимо пополнить баланс до [ДатаОкончанияПрописью], иначе работа в сервисе будет заблокирована.'; en = 'The paid pages in the document recognition service have run out."
"Top up your balance before [ДатаОкончанияПрописью], otherwise the service will be locked.'"),
		ПараметрыЗамены
	);
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Пополнить", НСтр("ru = 'Пополнить'; en = 'Top up the balance'"));
	КнопкиВопроса.Добавить("НапомнитьПозже", НСтр("ru = 'Напомнить позже'; en = 'Remind me later'"));
	КнопкиВопроса.Добавить("НеСегодня", НСтр("ru = 'Не напоминать сегодня'; en = 'Do not remind today'"));
	...
	ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыВопроса.Картинка = БиблиотекаКартинок.Предупреждение32;
	ПараметрыВопроса.Заголовок = НСтр("ru = 'Пополните баланс сервиса распознавания'; en = 'Top up the recognition service balance'");
...
Процедура ПоказатьПредупреждениеСервисОтключен(Обработчик) Экспорт
	...
	ТекстВопроса = 
		НСтр("ru = 'Закончились оплаченные страницы в сервисе распознавания документов."
"Работа в сервисе заблокирована. Пожалуйста, пополните баланс.'; en = 'The paid pages in the document recognition service have run out."
...
Процедура ПослеОтветаНаВопрос(Результат, Контекст) Экспорт
	...
	Если Результат.Значение = "Пополнить" Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
			РаспознаваниеДокументовСлужебныйКлиентСервер.АдресПополнитьБаланс()
		);
...
Функция ДоступенНеоплаченныйРаспознанныйДокумент(РаспознанныйДокумент) Экспорт
	...
	Если РаспознаваниеДокументовСлужебныйВызовСервера.ДоступенНеоплаченныйРаспознанныйДокумент(РаспознанныйДокумент) Тогда
		Возврат Истина;
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Данные может просматривать и скачивать только автор загруженного файла.'; en = 'Only the author of the imported file can view and download the data.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция КонтекстСозданияРаспознанногоДокумента(Форма, ПараметрыОперации)
	...
	////
	ЭтоСчетНаОплату = (ПараметрыОперации.ТипДокумента
		= РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("СчетНаОплатуПоставщикаБРД"))
		Или (ПараметрыОперации.ТипДокумента
		= РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("СчетНаОплатуПокупателюБРД"));
		
	ЭтоПлатежноеПоручение = (ПараметрыОперации.ТипДокумента
		= РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("ПлатежноеПоручениеБРД"));
		
	ЭтоКорректировкаПоступления = (ПараметрыОперации.ТипДокумента
		= РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("КорректировкаПоступленияБРД"));
	
	ЭтоВозвратПоставщику = (ПараметрыОперации.ТипДокумента
		= РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу("ВозвратТоваровПоставщикуБРД"));
	...
	СоздатьСчетФактуру = Не ЭтоПлатежноеПоручение
		И Не ЭтоСчетНаОплату
		И Не ЭтоКорректировкаПоступления
		И Не ЭтоВозвратПоставщику
		И Форма.СоздатьСчетФактуру;
	
	СозданныеДокументы = Новый СписокЗначений;
	Для Каждого Элемент Из Форма.ДокументыСозданные Цикл
		
		Если ЭтоПлатежноеПоручение Тогда
			
			Если РаспознаваниеДокументовСлужебныйКлиентПовтИсп.СодержитТипПоИмениОпределяемогоТипа(
				"ПлатежноеПоручениеБРД", Элемент.СозданныйДокумент) Тогда
				
				СозданныеДокументы.Добавить(Элемент.СозданныйДокумент);
			КонецЕсли;
			
		ИначеЕсли ЭтоСчетНаОплату Тогда
			
			Если РаспознаваниеДокументовСлужебныйКлиентПовтИсп.СодержитТипПоИмениОпределяемогоТипа(
				"СчетНаОплатуПокупателюБРД", Элемент.СозданныйДокумент)
				Или РаспознаваниеДокументовСлужебныйКлиентПовтИсп.СодержитТипПоИмениОпределяемогоТипа(
				"СчетНаОплатуПоставщикаБРД", Элемент.СозданныйДокумент) Тогда
...
"Изменить дату документа на текущую?'; en = 'The document date is in the closed period."
