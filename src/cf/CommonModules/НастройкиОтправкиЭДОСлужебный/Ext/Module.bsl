...
Функция НастройкиОтправки(КлючНастроекОтправки) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НастройкиЗаполненияДополнительныхПолей.ВидЭлектронногоДокумента = &ВидДокумента
		|	И НастройкиЗаполненияДополнительныхПолей.Формат В (&ФорматыДляПереносаНастроек)
		...
		|	И НастройкиЗаполненияДополнительныхПолей.Договор В (&Договор, &ПустойДоговор)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|
		...
		|ГДЕ
		|	НастройкиЗаполненияДополнительныхПолей.ВидЭлектронногоДокумента = &ВидДокумента
		|	И НастройкиЗаполненияДополнительныхПолей.Формат = &ФорматДляПроверкиНаличияНастроек
		|	И НастройкиЗаполненияДополнительныхПолей.Отправитель = &Отправитель
		|	И НастройкиЗаполненияДополнительныхПолей.Получатель = &Получатель
		|	И НастройкиЗаполненияДополнительныхПолей.Договор В (&Договор, &ПустойДоговор)";
	...
	ФорматыДляПереносаНастроек = Новый Массив;
	ФорматыДляПереносаНастроек.Добавить(ПоддерживаемыеФорматы.УПД2019.ИнформацияПродавца);
	ФорматыДляПереносаНастроек.Добавить(ПоддерживаемыеФорматы.УПД_5_02.ИнформацияПродавца);
	ФорматыДляПереносаНастроек.Добавить(ПоддерживаемыеФорматы.ПередачаРабот.ИнформацияПродавца);
	ФорматыДляПереносаНастроек.Добавить(ПоддерживаемыеФорматы.ПередачаТоваров.ИнформацияПродавца);
	Запрос.УстановитьПараметр("ФорматыДляПереносаНастроек", ФорматыДляПереносаНастроек);
	Запрос.УстановитьПараметр("ФорматДляПроверкиНаличияНастроек", ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца);
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	
	ЕстьНастройкаПоКлючу = Не МассивРезультатов[0].Пустой();
	ЕстьНастройкаПоВидуДокумента = Не МассивРезультатов[1].Пустой();
	ЕстьНастройкиЗаполненияДополнительныхПолей = Не МассивРезультатов[2].Пустой();
	ЕстьНастройкиЗаполненияДополнительныхПолейВНовомФормате = Не МассивРезультатов[3].Пустой();
	...
	ТребуетсяОбновлениеФормата = Ложь;
	УстановленныйФорматКорректен = Истина;
	ТребуетсяПереносНастроекЗаполненияДополнительныхПолей = Ложь;
	...
	Если ЕстьНастройкаПоКлючу И ЕстьНастройкаПоВидуДокумента Тогда
		...
		Если НастройкиОтправкиЭДО.ТребуетсяОбновлениеФорматовИСвязейВидовФорматовДляУПД_5_03(ПоддерживаемыеФорматы,
			ФорматыЭлектронныхДокументов) Или
			НастройкиОтправкиЭДО.ТребуетсяОбновлениеФорматовПоДатеСинхронизации() Тогда
		...
		ТребуетсяОбновлениеФормата = НастройкиОтправкиЭДО.ТребуетсяОбновленияФорматаВНастройкахОтправкиДляУПД_5_03(
			ПоддерживаемыеФорматы, ПолучаемаяНастройка.Формат, ПолучаемаяНастройка.ВерсияФорматаУстановленаВручную,
			ФорматыЭлектронныхДокументов);
			
		Если ЕстьНастройкиЗаполненияДополнительныхПолей
			И Не ЕстьНастройкиЗаполненияДополнительныхПолейВНовомФормате 
			И ТребуетсяОбновлениеФормата Тогда
			ТребуетсяПереносНастроекЗаполненияДополнительныхПолей = Истина;
		КонецЕсли;
		
		УстановленныйФорматКорректен = НастройкиОтправкиЭДО.УстановленныйФорматКорректенДляТекущегоВидаДокумента(
			ПолучаемаяНастройка.ВидДокумента, ПолучаемаяНастройка.Формат, ФорматыЭлектронныхДокументов);
			
	КонецЕсли;
	
	Если ТребуетсяОбновлениеФормата Или Не УстановленныйФорматКорректен Тогда
		НастройкиОтправкиЭДО.ОбновитьВерсиюФорматаВНастройкахОтправки(ПолучаемаяНастройка,
			ФорматыЭлектронныхДокументов);
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
	КонецЕсли;
	
	Если ТребуетсяПереносНастроекЗаполненияДополнительныхПолей Тогда
		НастройкиЗаполненияДополнительныхПолей = МассивРезультатов[2].Выбрать();
		Пока НастройкиЗаполненияДополнительныхПолей.Следующий() Цикл
			ДанныеНастройки = НастройкиЗаполненияДополнительныхПолей.Настройка.Получить();
			Если ДанныеНастройки = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			СкопироватьНастройкуЗаполненияДополнительныхПолейДляНовогоФормата(ПолучаемаяНастройка, ДанныеНастройки,
				ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца);
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьФорматыИСвязьВидовФорматовИзСервисаНастроек() Экспорт
	...
	ДатаПроверкаАктуальности = Дата('20250101');
...
// Временное решение. См. СинхронизацияЭДО.СинхронизироватьФорматыЭлектронныхДокументовИзСервисаНастроек
// 
// Параметры:
...
// Временное решение. См. СинхронизацияЭДО.СинхронизироватьСвязьВидовИФорматовЭлектронныхДокументовИзСервисаНастроек
// 
// Параметры:
//  ТипСинхронизации - ПеречислениеСсылка.ТипыСинхронизацииСервисаНастроекЭДО
//  ДатаПоследнегоИзменения - Дата
...
// Параметры:
//  ТекущаяНастройка - ВыборкаИзРезультатаЗапроса:
...
//  ДанныеНастройки - Строка
//  Формат - Строка
Процедура СкопироватьНастройкуЗаполненияДополнительныхПолейДляНовогоФормата(ТекущаяНастройка, ДанныеНастройки, Формат)

	УстановитьПривилегированныйРежим(Истина);
	МенеджерЗаписи = РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Отправитель = ТекущаяНастройка.Отправитель;
	МенеджерЗаписи.Получатель = ТекущаяНастройка.Получатель;
	МенеджерЗаписи.Договор = ТекущаяНастройка.Договор;
	МенеджерЗаписи.ВидЭлектронногоДокумента = ТекущаяНастройка.ВидДокумента;
	МенеджерЗаписи.Формат = Формат;
	МенеджерЗаписи.Настройка = Новый ХранилищеЗначения(ДанныеНастройки, Новый СжатиеДанных(9));
...
Процедура ИзменитьНастройку(Настройка) Экспорт
	...
	НаборЗаписей.Отбор.Отправитель.Установить(Настройка.Отправитель);
	НаборЗаписей.Отбор.Получатель.Установить(Настройка.Получатель);
	НаборЗаписей.Отбор.Договор.Установить(Настройка.Договор);
	НаборЗаписей.Отбор.ВидДокумента.Установить(Настройка.ВидДокумента);
	...
	Попытка
		...
		Для Каждого Запись Из НаборЗаписей Цикл
			...
			Если ЗначениеЗаполнено(Настройка.Формат) Тогда
				Запись.ВерсияФормата = Настройка.Формат;
			КонецЕсли;
		...
		Если Не НаборЗаписей.Количество() Тогда
			...
			НоваяЗапись.ВерсияФормата = Настройка.Формат;
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Изменение настройки отправки ЭДО'"),
...
Функция ИспользуетсяУПД_УКД(КлючНастроекОтправки) Экспорт
	...
	Результат.Вставить("ИспользуетсяУКД", Истина);
