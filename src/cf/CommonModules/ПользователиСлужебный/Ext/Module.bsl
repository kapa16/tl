		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".';
					|en = 'Service: Insufficient rights to change password for user ""%1.""'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	   ...
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь ""%1"" недействителен.';
				|en = 'User ""%1"" is inactive.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
					   ...
					   И ПользовательИБ.АдресЭлектроннойПочты <> АдресЭлектроннойПочты Тогда
						...
						Если ЗаписьВБезопасномРежиме Тогда
							ТекстОшибки = НСтр("ru = 'Изменение основного адреса электронной почты недоступно в безопасном режиме.';
												|en = 'Cannot change the main email address in save mode.'");
							ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
						КонецЕсли;
				...
				Если ПометкаУдаленияОбъектаАвторизации И ВнешнийПользовательОбъект.Недействителен = Ложь Тогда
					Если ЗаписьВБезопасномРежиме Тогда
						ТекстОшибки = НСтр("ru = 'Изменение действительности пользователя недоступно в безопасном режиме.';
											|en = 'Cannot change the user''s validity in save mode.'");
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить на удаление пользователя ""%1"", т.к. ему разрешен вход в приложение.';
				|en = 'Cannot mark authorized users for deletion: %1.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить недействительным пользователя ""%1"", т.к. ему разрешен вход в приложение.';
				|en = 'Cannot mark authorized users as inactive: %1.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно пометить подготовленным пользователя ""%1"", т.к. ему разрешен вход в приложение.';
				|en = 'Cannot mark authorized users as prepared: %1.'"),
			ПользовательОбъект.Ссылка);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
