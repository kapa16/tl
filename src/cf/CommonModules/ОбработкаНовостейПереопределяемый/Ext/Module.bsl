...
Процедура ЗначениеПредопределеннойКатегории(Категория, Значение) Экспорт
	...
	Если ВРег(КатегорияКод) = ВРег("ProductName") Тогда // В плане видов характеристик "КатегорииНовостей" такого элемента нет
		Значение = ИнтернетПоддержкаПользователей.ИмяПрограммы();
	КонецЕсли;
	// {/УАТ}
...
// 				"" + ИмяОбъектаМетаданных + ":Справочники._ДемоОрганизации." + ИмяМакета, // АПК:1297 Это идентификатор.
// 				СодержимоеМакета);
// 		КонецЕсли;
// 	КонецЕсли;
...
// 				Если НЕ ПустаяСтрока(СодержимоеМакета) Тогда
// 					СтандартныеЗначения.Вставить(
// 						"Справочники._ДемоОрганизации." + ИмяМакета, // АПК:1297 Это идентификатор.
// 						СодержимоеМакета);
// 				КонецЕсли;
// 			КонецЕсли;
// 		КонецЕсли;
...
// Вызывает проверку версии платформы в ОбработкаНовостейСлужебный.ПередЗапускомПрограммы.
// Отключение этой проверки при старте (установка СтандартнаяОбработка в Ложь) может несколько ускорить запуск,
//  но в этом случае новости, рассчитанные на определенную версию платформы будут показаны только
//  после срабатывания регламентного задания "ВсеОбновленияНовостей".
//
// Параметры:
...
//
//@skip-warning
Процедура ПередЗапускомПрограммыПроверитьИзменениеВерсииПлатформы(СтандартнаяОбработка) Экспорт
