...
Функция ПолучитьКарусельБаннеров(Размещение, ПоказатьПредыдущий) Экспорт
	...
	СохранитьРотациюБаннеров(БаннерыВРотации, Размещение);
	...
	Если БаннерыВРотации.Количество() > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Распознавание документов. Персонализированные предложения. Показан баннер'; en = 'Document recognition. Personalized recommendations. Banner is shown'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , , КарусельБаннеров.Баннер.Идентификатор);
	КонецЕсли;
	
	// Данные карусели следующего баннера, открываемого кнопкой "показать баннер".
	...
	Если СледующийБаннер <> Неопределено Тогда 
		...
		КарусельБаннеров.ИдентификаторБаннераОткрываемогоКнопкойПоказатьБаннер = СледующийБаннер.Идентификатор;
	КонецЕсли;
...
Процедура СохранитьОткрытиеБаннера(КарусельБаннеров) Экспорт
	...
	ЗакрытыеБаннеры.Удалить(КарусельБаннеров.ИдентификаторБаннераОткрываемогоКнопкойПоказатьБаннер);
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Распознавание документов. Персонализированные предложения. Открыт баннер'; en = 'Document recognition. Personalized recommendations. Banner is opened'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , , КарусельБаннеров.ИдентификаторБаннераОткрываемогоКнопкойПоказатьБаннер);
...
Процедура СохранитьЗакрытиеБаннера(КарусельБаннеров) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Распознавание документов. Персонализированные предложения. Закрыт баннер'; en = 'Document recognition. Personalized recommendations. Banner is closed'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , , КарусельБаннеров.Баннер.Идентификатор);
...
Функция ПолучитьСтраницуПриЗагрузкеФайла() Экспорт
	...
	Размещение = ИмяРазмещенияПриЗагрузкеФайла();
...
Процедура СохранитьРотациюБаннеров(БаннерыВРотации, Размещение)
...
Функция БаннерыПользователя(Размещение)
	...
	// Баннеры доступные текущему пользователю.
...
Функция НоваяКарусельБаннеров(Размещение)
	...
	Модель.Вставить("ИдентификаторБаннераОткрываемогоКнопкойПоказатьБаннер", Неопределено);
