	...
	Если ТипПодписи = Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES Тогда
		ПредставлениеПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Базовая подпись (%1)
						...
						|en = 'Basic Signature (%1)
						|A qualified basic signature is considered valid until the expiration of the signer''s certificate (typically within 1 year) in accordance with Article 11 of the Federal Law No. 63-FZ dated April 6, 2011, ""On Electronic Signatures.""'"),
				"CAdES-BES");
	КонецЕсли;
	// Конец Локализация
