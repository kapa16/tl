...
// Функция возвращает адрес веб-страницы для пополнения баланса сервиса "1С:Распознавание документов".
//
// Возвращаемое значение:
//  Строка - Адрес веб-страницы для пополнения баланса.
//
Функция АдресПополнитьБаланс() Экспорт
	
	Возврат "https://portal.1c.ru/applications/1C-Document-Recognition#prices";
...
#КонецЕсли
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не поддерживаемый тип параметра ШаблонСтроки'; en = 'Unsupported type of the ШаблонСтроки parameter'");
	КонецЕсли;
...
Функция МетаданныеЯчейки(СвойстваЯчеекТаблицы, НомерСтроки, ИмяРеквизита) Экспорт
...
Процедура ПриИзмененииНоменклатура(СтрокаТаблицы, ВыбранноеЗначение)
	...
	ЕстьРеквизитЭтоУслуга = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы , "ЭтоУслуга");
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		Если ЕстьРеквизитЭтоУслуга Тогда
			СтрокаТаблицы.ЭтоУслуга = РаспознаваниеДокументовСлужебныйВызовСервера.ЭтоНоменклатураУслуга(ВыбранноеЗначение);
		Иначе
...
Процедура ПересчитатьПроблемныеРеквизитыТаблицыДокумента(Форма, СтрокаТаблицы, ИмяТаблицы = "ТаблицаДокумента") Экспорт
	...
	ДанныеШапки.Вставить("ИтогоВсегоУвеличение", 0);
	ДанныеШапки.Вставить("ИтогоСуммаНДСУвеличение", 0);
	ДанныеШапки.Вставить("ИтогоСуммаУвеличение", 0);
	ДанныеШапки.Вставить("ИтогоВсегоУменьшение", 0);
	ДанныеШапки.Вставить("ИтогоСуммаНДСУменьшение", 0);
	ДанныеШапки.Вставить("ИтогоСуммаУменьшение", 0);
	...
	ДобавитьОшибкиКоличествоЦенаСумма(ПроблемныеРеквизиты, ДанныеШапки, ТаблицаДокумента, СтрокаТаблицы);
	ДобавитьОшибкиСуммаСуммаНДСВсего(ПроблемныеРеквизиты, ДанныеШапки, ТаблицаДокумента, СтрокаТаблицы);
	
	// Проверка итогов таблицы
	ТекстПодсказки = ПодсказкаЗначениеИтогаВПодвалеНеСовпадает();
	
	Если ЕстьОшибкаВЗначенииИтогаПоТаблице(ТаблицаДокумента, "СуммаСкидки", ДанныеШапки) Тогда
		ДобавитьПроблемныйЭлемент(ПроблемныеРеквизиты, "ИтогоСуммаСкидки", 0, "", ТекстПодсказки);
	КонецЕсли;
	
	Если ЕстьОшибкаВЗначенииИтогаПоТаблице(ТаблицаДокумента, "Сумма", ДанныеШапки) Тогда
		ДобавитьПроблемныйЭлемент(ПроблемныеРеквизиты, "ИтогоСумма", 0, "", ТекстПодсказки);
	КонецЕсли;
	
	Если ЕстьОшибкаВЗначенииИтогаПоТаблице(ТаблицаДокумента, "СуммаНДС", ДанныеШапки) Тогда
		ДобавитьПроблемныйЭлемент(ПроблемныеРеквизиты, "ИтогоСуммаНДС", 0, "", ТекстПодсказки);
	КонецЕсли;
	
	Если ЕстьОшибкаВЗначенииИтогаПоТаблице(ТаблицаДокумента, "Всего", ДанныеШапки) Тогда
		ДобавитьПроблемныйЭлемент(ПроблемныеРеквизиты, "ИтогоВсего", 0, "", ТекстПодсказки);
	КонецЕсли;
	
	Если Форма.Объект.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыДокументовРаспознаваниеДокументов.УКД") Тогда
		ДобавитьОшибкиРаспознаноПоДокументу(ПроблемныеРеквизиты, ДанныеШапки, ТаблицаДокумента, СтрокаТаблицы);
		...
		Для Каждого ИмяРеквизита Из ИменаРеквизитов Цикл
			Если ЕстьОшибкаВЗначенииУменьшенияУвеличенияИтогаПоТаблице(ТаблицаДокумента, ИмяРеквизита, ДанныеШапки) Тогда
				ДобавитьПроблемныйЭлемент(ПроблемныеРеквизиты, ИмяРеквизита, 0, "", ТекстПодсказки);
			КонецЕсли;
		КонецЦикла;
	...
	Пока ИндексПроблемы > 0 Цикл
		...
		Если СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаСкидки"
			...
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоВсего"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоВсегоУвеличение"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаНДСУвеличение"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаУвеличение"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоВсегоУменьшение"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаНДСУменьшение"
			Или СтрокаПроблемы.ИмяРеквизита = "ИтогоСуммаУменьшение" Тогда
		...
		Если УдалитьПроблему Тогда
			...
			Если ПроблемныеРеквизиты.Получить(Ключ) = Неопределено Тогда
				...
				Иначе
					ОтключитьУсловноеОформление(Форма, ИмяТаблицы, СтрокаПроблемы);
				КонецЕсли;
	...
	Для Каждого СтрокаПроблемы Из ПроблемныеРеквизиты Цикл
		...
		Если Форма.ПроблемныеЭлементы.НайтиСтроки(Отбор).Количество() = 0 Тогда
			...
			Иначе
				ДобавитьВУсловноеОформлениеИВПроблемные(Форма, ИмяТаблицы, СтрокаПроблемы.Значение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПодсказкаНеНайденДоговорСКонтрагентом() Экспорт
	...
	Возврат НСтр("ru = 'Не найден основной договор с выбранным контрагентом'; en = 'The main contract with the selected counterparty is not found'");
	
КонецФункции

Функция ПодсказкаТребуетсяВыбрать() Экспорт
	
	Возврат НСтр("ru = 'Требуется выбрать'; en = 'Select'");
	
КонецФункции

Функция ПодсказкаВозможнаОшибкаРаспознавания() Экспорт
	
	Возврат НСтр("ru = 'Возможна ошибка распознавания'; en = 'Possible recognition error'");
	
КонецФункции

Функция ПодсказкаВозможнаОшибкаСопоставленияРаспознаннойСтроки() Экспорт
	
	Возврат НСтр("ru = 'Возможна ошибка сопоставления распознанной строки'; en = 'An error may occur while mapping the recognized line'");
	
КонецФункции

Функция ПодсказкаНеНайденЭлементПоРаспознаннойСтроке() Экспорт
	
	Возврат НСтр("ru = 'В базе не найден элемент соответствующий распознанной строке'; en = 'The item corresponding to the recognized line is not found in the database'");
	
КонецФункции

Функция ПодсказкаЗначениеИтогаВПодвалеНеСовпадает() Экспорт
	
	Возврат НСтр("ru = 'Значение итога в подвале не совпадает с суммой по колонке'; en = 'The total value in the footer does not match the column amount'");
...
Процедура ДобавитьВУсловноеОформлениеИВПроблемные(Форма, ИмяТаблицы, СтрокаПроблемы) Экспорт
	...
	СтрокиТаблицы = ТаблицаДокумента.НайтиСтроки(Новый Структура("НомерСтроки", СтрокаПроблемы.НомерСтроки));
...
Процедура УстановитьЦветПоляПоУмолчанию(Форма, ИмяЭлемента) Экспорт
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ИзменяемыйЭлемент, "Родитель")
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ИзменяемыйЭлемент.Родитель, "ЦветФона") Тогда 
...
Процедура ДобавитьОшибкиРаспознаноПоДокументу(ПроблемныеРеквизиты, ДанныеШапки, ТаблицаДокумента, СтрокаТаблицы)
	...
	Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
		...
		Если СтрокаТаблицы[ИмяРеквизитаДоКорректировки] <> СтрокаТаблицы[ИмяРеквизитаПоДокументу] Тогда
...
Функция ПолучитьСписокДляВыбораПользователем(РаспознанныйТекст, Варианты = Неопределено, КартинкаСоздание = 1) Экспорт 
	...
	Если КартинкаСоздание = 1 Тогда
		ТекстРаспознано = НСтр("ru = 'Создать: '; en = 'Create:'");
		Картинка = БиблиотекаКартинок.ДобавитьЭлементСписка;
	Иначе
		ТекстРаспознано = НСтр("ru = 'Распознано: '; en = 'Recognized:'");
	...
	Для Каждого Вариант Из Варианты Цикл
		...
		Уверенность = Новый ФорматированнаяСтрока("(" + НСтр("ru = 'увер. '; en = 'conf.'") + Вариант.Уверенность + "%)", Новый Шрифт(,,,Истина));
