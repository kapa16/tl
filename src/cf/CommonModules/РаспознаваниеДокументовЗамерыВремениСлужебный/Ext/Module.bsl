...
Функция ОтправитьЗамерыВремени() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗамерВремени.Пользователь = MD5(Выборка.Пользователь);
		
		Данные = Неопределено;
		Попытка
			Данные = РаспознаваниеДокументовHTTP.JsonВОбъект(Выборка.Комментарий);
		Исключение
		...
		ДанныеПодходятДляПередачи = ТипЗнч(Данные) = Тип("Соответствие")
			И Данные["Платф"] <> Неопределено
			И Данные["КонфВер"] <> Неопределено
			И Данные["Конф"] <> Неопределено
			И Данные["Разд"] <> Неопределено
			И ТипЗнч(Данные["ДопИнф"]) = Тип("Соответствие")
			И Данные["ДопИнф"]["ИдентификаторРезультата"] <> Неопределено
		...
		Если ДанныеПодходятДляПередачи Тогда
			...
			// {УАТ}
			Если Данные["Разд"][0] <> ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса() Тогда
				Продолжить;
			КонецЕсли;
			// {/УАТ}
