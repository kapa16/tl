...
Процедура АвторизоватьсяВСервисеЭДОПослеПолученияОтпечатков(РезультатыПолученияОтпечатков, КонтекстАвторизации) Экспорт
	...
	ДлительнаяОперация = УчетныеЗаписиЭДОСлужебныйВызовСервера.АвторизоватьсяВСервисеЭДОВФоне(
		РезультатыПолученияОтпечатков, ПараметрыАвторизации.ИдентификаторыЭДО,
		ПараметрыАвторизации.ВыбранныеСертификаты, ИдентификаторФормы);
...
//  КонтекстАвторизации - см. НовыйКонтекстАвторизацииВСервисеЭДО
Процедура ОбработатьРезультатАвторизацииВСервисеЭДО(ДлительнаяОперация, КонтекстАвторизации) Экспорт
	
	РезультатАвторизацииВФоне = ОбщегоНазначенияБЭДКлиент.РезультатВыполненияФоновогоЗадания(
		УчетныеЗаписиЭДОКлиентСервер.ВидОперацииАвторизацияВСервисеЭДО(),
		ДлительнаяОперация, КонтекстАвторизации.КонтекстДиагностики, Истина);
	Если Не ЗначениеЗаполнено(РезультатАвторизацииВФоне) Тогда
		ИтогРасшифровки = КриптографияБЭДКлиент.НовыйИтогРасшифровкиНабораДанных();
		ЗавершитьАвторизациюВСервисеЭДОПослеРасшифровки(ИтогРасшифровки, КонтекстАвторизации);
		Возврат;
	КонецЕсли;
	
	ОбработкаНеисправностейБЭДКлиент.ДополнитьКонтекстДиагностики(КонтекстАвторизации.КонтекстДиагностики,
		РезультатАвторизацииВФоне.КонтекстДиагностики);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(КонтекстАвторизации.РасшифрованныеМаркеры,
		РезультатАвторизацииВФоне.РасшифрованныеМаркеры, Истина);
		
	Если Не ЗначениеЗаполнено(РезультатАвторизацииВФоне.ЗашифрованныеМаркеры) Тогда
		ИтогРасшифровки = КриптографияБЭДКлиент.НовыйИтогРасшифровкиНабораДанных();
		ЗавершитьАвторизациюВСервисеЭДОПослеРасшифровки(ИтогРасшифровки, КонтекстАвторизации);
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ЗавершитьАвторизациюВСервисеЭДОПослеРасшифровки",
		УчетныеЗаписиЭДОСлужебныйКлиент, КонтекстАвторизации);
	
	ПараметрыРасшифровки = КриптографияБЭДКлиент.НовыеПараметрыРасшифровкиНабораДанных();
	ПараметрыРасшифровки.ПаролиСертификатов = КонтекстАвторизации.ПараметрыАвторизации.ПаролиСертификатов;
	ПараметрыРасшифровки.КонтекстДиагностики = КонтекстАвторизации.КонтекстДиагностики;
	
	ПредставленияДанных = ПараметрыРасшифровки.ПредставленияДанных;
	Для Каждого ПредставлениеУчетнойЗаписиЭДО Из РезультатАвторизацииВФоне.ПредставленияУчетныхЗаписейЭДО Цикл
		ОповещениеПредставления = Новый ОписаниеОповещения("ОткрытьУчетнуюЗаписьПриАвторизацииВСервисеЭДО",
			УчетныеЗаписиЭДОСлужебныйКлиент, ПредставлениеУчетнойЗаписиЭДО.Ключ);
		ПредставлениеДанных = Новый Структура("Представление, Значение",
			ПредставлениеУчетнойЗаписиЭДО.Значение, ОповещениеПредставления);
		ПредставленияДанных.Вставить(ПредставлениеУчетнойЗаписиЭДО.Ключ, ПредставлениеДанных);
	КонецЦикла;
	
	КриптографияБЭДКлиент.НачатьРасшифровкуНабораДанных(Оповещение, РезультатАвторизацииВФоне.ЗашифрованныеМаркеры,
...
//  КонтекстАвторизации - см. НовыйКонтекстАвторизацииВСервисеЭДО
Процедура ЗавершитьАвторизациюВСервисеЭДОПослеРасшифровки(ИтогРасшифровки, КонтекстАвторизации) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(КонтекстАвторизации.РасшифрованныеМаркеры,
		ИтогРасшифровки.НаборРасшифрованныхДанных, Истина);
	
	РезультатАвторизации = УчетныеЗаписиЭДОКлиент.НовыйРезультатАвторизацииВСервисеЭДО();
	РезультатАвторизации.РасшифрованныеМаркеры = КонтекстАвторизации.РасшифрованныеМаркеры;
	РезультатАвторизации.ПаролиСертификатов = ИтогРасшифровки.ПаролиСертификатов;
	РезультатАвторизации.КонтекстДиагностики = КонтекстАвторизации.КонтекстДиагностики;
	
	ВыполнитьОбработкуОповещения(КонтекстАвторизации.ОповещениеОЗавершении, РезультатАвторизации);
...
Процедура ОткрытьУчетнуюЗаписьПриАвторизацииВСервисеЭДО(Результат, ИдентификаторЭДО) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Если РезультатВыполнения.Свойство("НаборДанных") Тогда
			...
			Если ЗначениеЗаполнено(Доверенность) Тогда
				...
				ПараметрыПроверкиМЧД = 
					ЭлектронныеДокументыЭДОКлиент.НовыеПараметрыПроверкиПодписиПоМЧДПриРегистрацииСертификата();
				...
				ИННСубъектаСертификатаПодписанта = 
					Контекст.ДанныеПакета.РеквизитыПакета.ИННСубъектаСертификатаПодписанта;
				ИННДоверителяИДоверенныхЛицДоверенностей = 
					Контекст.ДанныеПакета.РеквизитыПакета.ИННДоверителяИДоверенныхЛицДоверенностей;
				ИННДоверителяИДоверенныхЛицПоДоверенности = ИННДоверителяИДоверенныхЛицДоверенностей[Доверенность];
				
				Если ИННДоверителяИДоверенныхЛицПоДоверенности <> Неопределено Тогда
				
					ИННДоверителя = ИННДоверителяИДоверенныхЛицПоДоверенности.ИННДоверителя;
					ИННПредставителей = ИННДоверителяИДоверенныхЛицПоДоверенности.ИННПредставителей;
				
					ИндексНайденногоДоверенногоЛица = ИННПредставителей.Найти(ИННСубъектаСертификатаПодписанта);
					Если ИндексНайденногоДоверенногоЛица <> Неопределено Тогда
						ИННДоверенногоЛица = ИННПредставителей[ИндексНайденногоДоверенногоЛица];
					КонецЕсли;
				...
				ПараметрыПроверкиМЧД.ИННДоверенногоЛица = ИННДоверенногоЛица;
...
Процедура ОтправитьРегистрационныйПакет1СЭДОЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат = Неопределено Тогда
		Ошибка = ОбработкаНеисправностейБЭДКлиент.НоваяОшибка(НСтр("ru = 'Отправка регистрационного пакета 1С:ЭДО'"),
			...
			НСтр("ru ='При создании учетной записи произошла неизвестная ошибка'"), НСтр("ru ='Неизвестная ошибка'"));
...
Функция РезультатОперацииВСервисе1СЭДО(Контекст) Экспорт
	...
	РезультатОперации.Вставить("Доверенности", Новый Соответствие());
...
//  Контекст - Структура:
//  *Успех - Неопределено, Булево - 
//  *ИдентификаторУчетнойЗаписи - Строка -
//  *Оповещение - ОписаниеОповещения - 
//  *Форма - Форма - форма где было вызвано удаление
Процедура ПослеУдаленияУчетнойЗаписи(Результат, Контекст) Экспорт
	...
	Иначе
		Оповестить("УдаленаУчетнаяЗапись", Контекст.ИдентификаторУчетнойЗаписи);
	КонецЕсли;
...
// Параметры:
//  ПараметрыСоздания - см. УчетныеЗаписиЭДОКлиент.НовыеПараметрыСозданияУчетнойЗаписи
Процедура СоздатьУчетнуюЗаписьЛегкийИнтерфейс(ПараметрыСоздания) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЧерезОблачныйЭДО", ПараметрыСоздания.ЧерезОблачныйЭДО);
	ПараметрыФормы.Вставить("Организация", ПараметрыСоздания.Организация);

	ОткрытьФорму("РегистрСведений.УчетныеЗаписиЭДО.Форма.ПомощникПодключенияЭДОЛегкийИнтерфейс",
		ПараметрыФормы,
		ПараметрыСоздания.ВладелецФормы,,,,
		ПараметрыСоздания.ОповещениеОЗавершении,
		ПараметрыСоздания.РежимОткрытия);
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ИдентификаторУчетнойЗаписи - Строка
Процедура ОткрытьФормуУчетнойЗаписи(КонтекстДиагностики, ИдентификаторУчетнойЗаписи) Экспорт
	
	КлючЗаписи = УчетныеЗаписиЭДОКлиент.КлючУчетнойЗаписи(ИдентификаторУчетнойЗаписи);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", КлючЗаписи);
	ПараметрыФормы.Вставить("ЕстьНедействительныеСертификаты", Истина);
	ОткрытьФорму("РегистрСведений.УчетныеЗаписиЭДО.ФормаЗаписи", ПараметрыФормы);
	
КонецПроцедуры

// Параметры:
//  ОператорКонтролируетНедействительныеСертификаты - Булево
//
// Возвращаемое значение:
//  Строка
Функция ТекстВопросаУдаленияНедействительныхСертификатов(ОператорКонтролируетНедействительныеСертификаты) Экспорт
	
	Предложения = Новый Массив;
	Предложения.Добавить(НСтр("ru = 'В учетной записи есть недействительные сертификаты.'"));
	Если ОператорКонтролируетНедействительныеСертификаты Тогда
		Предложения.Добавить(НСтр("ru = 'Получение и отправка электронных документов по ней невозможны.'"));
	КонецЕсли;
	Предложения.Добавить(НСтр("ru = 'Удалить их?'"));
...
// Возвращаемое значение:
//  Строка
Функция ТекстПредупрежденияОНедействительныхСертификатах(ОператорКонтролируетНедействительныеСертификаты) Экспорт
	...
	Если ОператорКонтролируетНедействительныеСертификаты Тогда
		Предложения.Добавить(НСтр("ru = 'Получение и отправка электронных документов по ней невозможны.'"));
	КонецЕсли;
	Предложения.Добавить(НСтр("ru = 'Для их удаления обратитесь к администратору.'"));
	Возврат СтрСоединить(Предложения, Символы.ПС);
...
// Возвращаемое значение:
//  Строка
Функция ТекстВопросаОДобавленииДействующегоСертификата() Экспорт
	
	Предложения = Новый Массив;
	Предложения.Добавить(НСтр("ru = 'В учетной записи нет ни одного действующего сертификата.'"));
	Предложения.Добавить(НСтр("ru = 'Получение и отправка электронных документов по ней невозможны.'"));
	Предложения.Добавить(НСтр("ru = 'Добавить действующий сертификат?'"));
	Возврат СтрСоединить(Предложения, Символы.ПС);
