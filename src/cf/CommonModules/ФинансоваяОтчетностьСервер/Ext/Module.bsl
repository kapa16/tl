////////////////////////////////////////////////////////////////////////////////
// Модуль содержит процедуры и функции финансовой отчетности
...
//  Реквизит		 - Строка, ПланВидовХарактеристикСсылка.РеквизитыЭлементовФинансовыхОтчетов	 - - Строка - Имя реквизита
...
Функция ПредставлениеИнтервала(ДатаПериода, Периодичность) Экспорт
	...
	Если Периодичность = Перечисления.Периодичность.Полугодие И Месяц(ДатаНачала) = 7 Тогда
		Возврат "2 " + НСтр("en='half a year';ru='полугодие'") + " " + Формат(Год(ДатаНачала), "ЧГ=0") + " г.";
	КонецЕсли;
...
//  НаборСхемы	 - НаборДанныхОбъектСхемыКомпоновкиДанных	 - переменная в которую будет помещена ссылка
//  	на новый набор данных (необязательное).
// 
// Возвращаемое значение:
...
//  				НаборДанныхОбъектСхемыКомпоновкиДанных - набор данных в котором необходмио созадть новое поле.
...
//  		ГруппировкаТаблицыКомпоновкиДанных - структура схемы компоновки
//  		в которую необходимо добавить новое поле выбора.
...
//  							КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных - структура схемы компоновки
//  							в которую необходимо добавить новую группировку.
...
//
// Параметры:
//  Группировка			 - ГруппировкаКомпоновкиДанных	 - структура схемы компоновки
//  	в которую необходимо добавить новую группировку.
//  ИмяПоляГруппировки	 - Строка						 - имя поля группировки.
//  ТипГруппировки		 - ТипГруппировкиКомпоновкиДанных	 - тип группировки (необязательное по умолчанию Неопределено).
// 
...
//
Функция НовыйОтбор(Отбор, ЛевоеЗначение = Неопределено, ПравоеЗначение = Неопределено, ТипОтбора = Неопределено) Экспорт
...
//  	в которые необходимо добавить новый порядок.
//  ИмяПоля					 - 	 - Строка,
...
//
// Параметры:
//  СтруктураКД	 - КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных	 - отбор схемы компоновки
//  	в который необходимо добавить новый элемент.
...
//  ПоляИсключения		 - Структура - в ключах структуры перечсилены поля которые не должны попасть в отбор приемник
	...
	Для Каждого Элемент Из ОтборИсточник.Элементы Цикл
		...
		Иначе
			...
			Если ПоляИсключения <> Неопределено Тогда
				...
				Если ПоляИсключения.Свойство(ИмяПоля) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если Постфикс <> "" Тогда
				НовыйЭлемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(Строка(НовыйЭлемент.ЛевоеЗначение) + Постфикс);
			КонецЕсли;
...
//  НастройкиКомпоновщика	 - 						 - КомпоновщикНастроекКомпоновкиДанных,
//  						НастройкиКомпоновкиДанных - настройки исполняемой схемы компоновки данных.
// 
// Возвращаемое значение:
...
//  ВнешниеНаборыДанных	 - Структура, ключ		 - - Структура - внешние наборы компоновки данных
//  		- ключ - наименование внешнего набора,
...
//
// Параметры:
//  СхемаКомпоновкиДанных	 - СхемаКомпоновкиДанных			 - исполняемая схема компоновки данны.х
//  НастройкиКомпоновщика	 - КомпоновщикНастроекКомпоновкиДанных	 - компоновщик настроек
//  	исполняемой схемы компоновки данных.
//  ВнешниеНаборыДанных		 - Структура, ключ						 - - Структура - внешние наборы компоновки данных
//  						- ключ - наименование внешнего набора, значение - таблица значений внешнего набора.
//  ВыводВДерево			 - Булево								 - если Истина выводить в дерево значений.
// 
// Возвращаемое значение:
//  ТаблицаЗначений, ДеревоЗначений - результат выполнения схемы компоновки данных.
...
Процедура ДобавитьДоступныеПоляРекурсивно(СтрокиДерева, ДоступныеПоляВыбора, ТекущийУровень, Форма)
	...
	Для каждого ДоступноеПолеВыбора Из ДоступныеПоляВыбора Цикл
		...
		Если ТекущийУровень <> 0 И Не Искомый Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе
			ЧастиЗаголовка = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ДоступноеПолеВыбора.Заголовок, ".");
			Представление = ЧастиЗаголовка[ТекущийУровень];
		КонецЕсли;
		...
		ДобавитьДоступныеПоляРекурсивно(НоваяСтрокаДерева.ПолучитьЭлементы(), ДоступноеПолеВыбора.Элементы, НовыйУровень, Форма);
