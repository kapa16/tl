...
Процедура ПолучитьHTMLПредпросмотрОбъектаВФоне(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		Результат = HTMLПредпросмотрОбъекта(
			...
			Параметры.ВернутьДвоичныеДанные);
	Исключение
...
Функция ОбработатьУниверсальноеСообщение(Сообщение) Экспорт
	...
	Попытка
		...
		ПараметрыСеанса.ИнтеграцияС1СДокументооборотСхемаXML = СхемаDMServiceПоСообщению(Сообщение);
		...
		Если Результат = Неопределено Тогда
			...
			Если ПроверитьТипБИД(Сообщение, "DMILCheckExpressionOnBuiltInLanguageRequest") Тогда
			...
			ИначеЕсли ПроверитьТипБИД(Сообщение, "DMILGetMetadataRequest") Тогда
			...
			ИначеЕсли ПроверитьТипБИД(Сообщение, "DMILSynchronizeUsersRequest") Тогда
				Результат = СинхронизироватьПользователейИнтегрированнойСистемыИПользователейДО(Сообщение);
		...
		Возврат ОписаниеОшибкиXDTO(
			НСтр("ru = 'Неизвестное сообщение'; en = 'Unknown message'"),
			Строка(Сообщение.Тип()));
		
	Исключение
		
		Возврат ОписаниеОшибкиXDTO(
			НСтр("ru = 'Ошибка при обработке сообщения'; en = 'Message processing error'"),
...
Функция ПолучитьСсылкуПоObjectID(ObjectID, ВызыватьИсключение = Истина, ТекстСообщения = "") Экспорт
	...
	Если ObjectID <> Неопределено И ObjectID.ID <> "" И ObjectID.type <> "" Тогда
		...
		Иначе
			...
			Исключение
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Нет значения ""%1"" среди значений типа ""%2""'; en = 'There is no ""%1"" value among values of the ""%2"" type'"),
					ObjectID.ID,
					ObjectID.type);
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Ссылка)) И Не ОбщегоНазначения.СсылкаСуществует(Ссылка)Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Объект типа ""%1"" не найден по идентификатору ""%2""'; en = 'The object of the ""%1"" type is not found by ID %2'"),
				ObjectID.type,
				ObjectID.ID);
			Если ВызыватьИсключение Тогда
				ВызватьИсключение ТекстСообщения;
...
Функция ПроверитьТипБИД(ОбъектXDTO, Тип) Экспорт
	...
	Возврат (ТипЗнч(ОбъектXDTO) = Тип("ОбъектXDTO"))
		И ИнтеграцияС1СДокументооборотБазоваяФункциональность.ТипыXDTOСовпадают(
			ОбъектXDTO.Тип(),
			ФабрикаXDTO.Тип(URIПространстваИменВебСервисаБИД(), Тип));
...
// Читает объекты XDTO из сообщения типа DMILRequest.
//
// Параметры:
//   ДвоичныеДанныеСообщения - ДвоичныеДанные - свойство сообщения messageData.
//
// Возвращаемое значение:
//   Массив из ОбъектXDTO
//
Функция СписокОбъектовXDTOизСообщения(ДвоичныеДанныеСообщения) Экспорт
	
	СписокОбъектовXDTO = Новый Массив;
	
	ИмяФайлаСообщенияОбмена = ПолучитьИмяВременногоФайла("xml");
	ДвоичныеДанныеСообщения.Записать(ИмяФайлаСообщенияОбмена);
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		СписокОбъектовXDTO.Добавить(ФабрикаDM.ПрочитатьXML(ЧтениеXML, ТипXDTO));
	КонецЦикла;
...
//     * ДвоичныеДанные - ДвоичныеДанные, Неопределено - двоичные данные файла для визуализации средствами платформы.
//
Функция HTMLПредпросмотрОбъекта(Прокси, ID, Тип, ИгнорироватьМаксРазмер = Ложь,
	...
	Результат = Новый Структура("ПредставлениеHTML, ПредпросмотрУрезан, ДвоичныеДанные", "", Ложь, Неопределено);
	
	ЗапросыПакета = Новый Массив;
	
	Если ВернутьДвоичныеДанные Тогда
		ЗапросыПакета.Добавить(ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьОбъектыЗапрос(
			Прокси,
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("ID, Тип", ID, Тип)),
			"binaryData"));
	Иначе
	...
	ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОтветПредпросмотр, "DMRetrieveResponse") Тогда
		Результат.ДвоичныеДанные = ОтветПредпросмотр.objects[0].binaryData;
	Иначе
...
//   СписокВыгружаемыхТЧ - Массив из Строка - список имен табличных частей, которые требуется выгрузить в ДО.
...
//
Функция ВходящиеДанныеОбъектаИС(Прокси, СсылкаНаОбъектИС, СписокВыгружаемыхТЧ, СписокВыражений) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональность.ОбъектМетаданныхПоддерживается(ОбъектМетаданных) Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Объект %1 (%2) не поддерживается бесшовной интеграцией. Его нельзя отправить в 1С:Документооборот.'; en = 'Seamless integration does not support the %1 (%2) object. Cannot send the object to 1C:Document Management.'"),
			СсылкаНаОбъектИС,
	...
	Для Каждого ТабличнаяЧасть Из ОбъектМетаданных.ТабличныеЧасти Цикл
		...
		Если ТабличнаяЧасть.Имя = "ДополнительныеРеквизиты"
				Или (ТипЗнч(СписокВыгружаемыхТЧ) = Тип("Массив")
					И СписокВыгружаемыхТЧ.Найти(ТабличнаяЧасть.Имя) = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ДоступенФункционалФайлов() Экспорт
	...
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.9.18");
...
// Возвращаемое значение:
//   Структура:
//     * СоответствиеСпискаВыгружаемыхТЧОбъектамИС - Соответствие из КлючИЗначение:
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - Массив из Строка - список имен табличных частей, которые требуется выгрузить в ДО.
...
Функция ПредварительныеДанныеДляВыгрузкиВДО(Прокси, СписокОбъектовИС, СоответствиеПравилЗагрузкиВДООбъектамИС,
	...
	ТребуетсяСписокВыгружаемыхТЧ = ТребуетсяСписокВыражений
		И ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.16.30");
	
	Данные = Новый Структура;
	
	Данные.Вставить("СоответствиеСпискаВыгружаемыхТЧОбъектамИС", Новый Соответствие);
	...
	Если СписокОбъектовИС.Количество() = 0
			Или (Не ТребуетсяСписокВыгружаемыхТЧ И Не ТребуетсяСписокВыражений И Не ТребуютсяПечатныеФормы) Тогда
		Возврат Данные;
	КонецЕсли;
	
	Если ТребуетсяСписокВыгружаемыхТЧ Тогда
		ЗапросСпискаВыгружаемыхТЧ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMGetRequiredTabularSectionListRequest");
		ПравилаДляСпискаСпискаВыгружаемыхТЧ = ЗапросСпискаВыгружаемыхТЧ.allObjectsTabularSections; // СписокXDTO
	КонецЕсли;
	Если ТребуетсяСписокВыражений Тогда
	...
	ДобавленныеПравила = Новый Массив;
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		ПравилоЗагрузкиВДО = СоответствиеПравилЗагрузкиВДООбъектамИС[ОбъектИС];
		Если ПравилоЗагрузкиВДО = Неопределено
				Или ДобавленныеПравила.Найти(ПравилоЗагрузкиВДО.id) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ТребуетсяСписокВыгружаемыхТЧ Тогда
			ПравилоИСписокВсехТЧ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
				Прокси,
				"DMObjectTabularSections");
			СписокВсехТЧ = ПравилоИСписокВсехТЧ.tabularSections; // СписокXDTO
			ПравилоИСписокВсехТЧ.object = КопияОбъектаXDTO(ПравилоЗагрузкиВДО, Прокси);
			Для Каждого ТабличнаяЧасть Из ОбъектИС.Метаданные().ТабличныеЧасти Цикл
				СписокВсехТЧ.Добавить(ТабличнаяЧасть.Имя);
			КонецЦикла;
			ПравилаДляСпискаСпискаВыгружаемыхТЧ.Добавить(ПравилоИСписокВсехТЧ);
		КонецЕсли;
		Если ТребуетсяСписокВыражений Тогда
		...
		КонецЕсли;
		ДобавленныеПравила.Добавить(ПравилоЗагрузкиВДО.id);
	КонецЦикла;
	
	МассивВсехВыгружаемыхТЧ = Новый Массив;
	МассивВсехВыражений = Новый Массив;
	МассивВсехПечатныхФорм = Новый Массив;
	
	ЗапросыПакета = Новый Массив;
	
	Если ТребуетсяСписокВыгружаемыхТЧ И ПравилаДляСпискаСпискаВыгружаемыхТЧ.Количество() > 0 Тогда
		ЗапросыПакета.Добавить(ЗапросСпискаВыгружаемыхТЧ);
	КонецЕсли;
	...
	Если ЗапросыПакета.Количество() > 0 Тогда
		...
		Для Каждого Ответ Из ОтветНаЗапросыПакета.responses Цикл
			...
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(
					Прокси, Ответ, "DMGetRequiredTabularSectionListResponse") Тогда
				МассивВсехВыгружаемыхТЧ = Ответ.requiredObjectsTabularSections; // СписокXDTO
				
			ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(
					Прокси, Ответ, "DMGetExpressionListResponse") Тогда
				МассивВсехВыражений = Ответ.objectsExpressionRows; // СписокXDTO
				
			ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(
					Прокси, Ответ, "DMGetPrintedFormListResponse") Тогда
				МассивВсехПечатныхФорм = Ответ.objectsPrintedForms; // СписокXDTO
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		ПравилоЗагрузкиВДО = СоответствиеПравилЗагрузкиВДООбъектамИС[ОбъектИС];
		СписокВыгружаемыхТЧ = Новый Массив;
		...
		Если ПравилоЗагрузкиВДО <> Неопределено Тогда
			Если ТребуетсяСписокВыгружаемыхТЧ Тогда
				Для Каждого Элемент Из МассивВсехВыгружаемыхТЧ Цикл
					Если Элемент.object.id = ПравилоЗагрузкиВДО.id Тогда
						СписокВыгружаемыхТЧ = Элемент.tabularSections;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Если ТребуетсяСписокВыгружаемыхТЧ Тогда
			Данные.СоответствиеСпискаВыгружаемыхТЧОбъектамИС[ОбъектИС] = Новый Массив;
			Для Каждого ИмяТаблицыИС Из СписокВыгружаемыхТЧ Цикл
				Данные.СоответствиеСпискаВыгружаемыхТЧОбъектамИС[ОбъектИС].Добавить(ИмяТаблицыИС);
			КонецЦикла;
		КонецЕсли;
		Если ТребуетсяСписокВыражений Тогда
...
"%1'; en = 'An error is found in the expression in 1C:Enterprise language:"
"%1'"),
...
Функция СоздатьОбъектыДОПоПравилам(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции) Экспорт
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		Если ПравилоЗагрузкиВДО = Неопределено Тогда
			...
			РезультатПоОбъекту.ТекстОшибки = НСтр("ru = 'Не найдено подходящее правило загрузки данных в 1С:Документооборот'; en = 'Suitable 1C:Document Management data import rule is not found'");
		...
		Попытка
			СтруктураСозданияОбъекта.incomingData = ВходящиеДанныеОбъектаИС(
				Прокси,
				ОбъектИС,
				ПредварительныеДанные.СоответствиеСпискаВыгружаемыхТЧОбъектамИС[ОбъектИС],
				ПредварительныеДанные.СоответствиеСпискаВыраженийОбъектамИС[ОбъектИС]);
		Исключение
			РезультатПоОбъекту.Успешно = Ложь;
		...
		Если ТребуютсяПечатныеФормы Тогда
			ПечатныеФормыКСозданию = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПечатныеФормыКСозданию(
				ОбъектИС,
				ПрисоединяемыеПечатныеФормы(ПредварительныеДанные.СоответствиеПечатныхФормОбъектамИС[ОбъектИС]),
				Перечисления.ТипыФайловСохраненияПечатныхФормОбъектов[ПредварительныеДанные.ТипыФайловПечатныхФорм[ОбъектИС]],
...
Функция СоответствиеПравилЗагрузкиВДООбъектамИС(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции) Экспорт
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		Если ПодходящиеПравилаИнтеграции[ОбъектИС] <> Неопределено
				И ПодходящиеПравилаИнтеграции[ОбъектИС].Количество() = 1 Тогда
...
Функция ВыполнитьДействиеЗадачиЗапрос(Прокси, ДанныеПоЗадаче) Экспорт
	...
	ИначеЕсли ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.ЗадатьВопросОЛимитеЦиклов Тогда
		
		РезультатXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
			Прокси,
			"DMInteractiveActionContext");
		РезультатXDTO.interactiveAction = "CycleLimitExhaustQuestion";
		РезультатXDTO.context = ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.Контекст;
...
Процедура ДополнитьФильтрыОтбораЗадач(Прокси, Фильтры, БыстрыйПоиск) Экспорт
	...
	Для Каждого Строка Из БыстрыйПоиск Цикл
		...
		ИначеЕсли Строка.Параметр = "ВидПриложения" Тогда
			
			ВидПриложенияXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
				Прокси,
				Строка.ЗначениеID,
				Строка.ЗначениеТип);
			Фильтры.Добавить(ВидПриложенияXDTO, "applicationType");
...
Функция НовоеДеревоПриложений() Экспорт
	...
	НовоеДеревоПриложений.Колонки.Добавить("ПолноеИмяФайлаПредпросмотра", Новый ОписаниеТипов("Строка"));
...
// Конструктор структуры вопроса пользователю.
//
// Параметры:
//   ВопросПользователюXDTO - ОбъектXDTO - объект XDTO типа DMQuesion:
//     * text - Строка
//     * title - Строка
//     * buttons - Массив из Строка
//     * defaultButton - Строка
//
// Возвращаемое значение:
//   Структура:
//     * Текст - Строка
//     * Заголовок - Строка
//     * Кнопки - СписокЗначений из Строка
//     * КнопкаПоУмолчанию - Строка
//
Функция СтруктураВопросаПользователю(ВопросПользователюXDTO) Экспорт
	
	СтруктураВопросаПользователю = Новый Структура;
	СтруктураВопросаПользователю.Вставить("Текст", ВопросПользователюXDTO.text);
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ВопросПользователюXDTO, "title") Тогда
		СтруктураВопросаПользователю.Вставить("Заголовок", ВопросПользователюXDTO.title);
	Иначе
		СтруктураВопросаПользователю.Вставить("Заголовок", "");
	КонецЕсли;
	Кнопки = Новый СписокЗначений;
	Для Каждого КнопкаXDTO Из ВопросПользователюXDTO.buttons Цикл
		Кнопки.Добавить(КнопкаXDTO);
	КонецЦикла;
	СтруктураВопросаПользователю.Вставить("Кнопки", Кнопки);
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ВопросПользователюXDTO, "defaultButton") Тогда
		СтруктураВопросаПользователю.Вставить("КнопкаПоУмолчанию", ВопросПользователюXDTO.defaultButton);
	Иначе
		СтруктураВопросаПользователю.Вставить("КнопкаПоУмолчанию", "");
	КонецЕсли;
	
	Возврат СтруктураВопросаПользователю;
...
Функция УчастникЗадачXDTO(Прокси, Исполнитель, ОсновнойОбъектАдресации = Неопределено,
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Некорректно задан тип участника задачи: %1'; en = 'The task participant type is incorrect: %1'"), Исполнитель.Тип);
...
Процедура ЗаполнитьОбработкуВФорме(Форма, ДанныеОбОбработкеXDTO) Экспорт
	...
	Форма.Элементы.ДекорацияОписание.Заголовок = НСтр("ru = 'Не запущена'; en = 'Not started'");
	...
	Если Не ЗначениеЗаполнено(Форма.СостояниеОбработки) Или Форма.СостояниеОбработки = "Terminated" Тогда
		...
		Если КоличествоДействий > 0 Тогда
			Форма.Элементы.ДействияОтправить.Заголовок = НСтр("ru = 'Начать обработку'; en = 'Start processing'");
			Если ТипПредмета = "DMDocument" Тогда
				Подсказка = НСтр("ru = 'Начать обработку документа'; en = 'Start document processing'");
			Иначе
				Подсказка = НСтр("ru = 'Начать обработку мероприятия'; en = 'Start event processing'");
			КонецЕсли;
	...
	ИначеЕсли Форма.СостояниеОбработки = "Executing" Или Форма.СостояниеОбработки = "Stopped" Тогда
		...
		Если ЕстьПравоОстановкиОбработки Тогда
			Форма.Элементы.ДействияОтправить.Заголовок = НСтр("ru = 'Прервать'; en = 'Interrupt'");
			Если ТипПредмета = "DMDocument" Тогда
				Подсказка = НСтр("ru = 'Прервать обработку документа'; en = 'Terminate document processing'");
			Иначе
				Подсказка = НСтр("ru = 'Прервать обработку мероприятия'; en = 'Terminate event processing'");
			КонецЕсли;
			Если ТипЗнч(Форма.Элементы.ДействияОтправить) = Тип("КнопкаФормы") Тогда
				Форма.Команды[Форма.Элементы.ДействияОтправить.ИмяКоманды].Подсказка = Подсказка;
			Иначе
			...
			Если Форма.СостояниеОбработки = "Executing" Тогда
				Форма.Элементы.ДействияОстановить.Заголовок = НСтр("ru = 'На паузу'; en = 'Pause'");
				Если ТипПредмета = "DMDocument" Тогда
					Подсказка = НСтр("ru = 'Поставить обработку документа на паузу'; en = 'Pause document processing'");
				Иначе
					Подсказка = НСтр("ru = 'Поставить обработку мероприятия на паузу'; en = 'Pause event processing'");
				КонецЕсли;
				Форма.Элементы.ДействияОстановить.Картинка = БиблиотекаКартинок.ЗнакПауза;
			ИначеЕсли Форма.СостояниеОбработки = "Stopped" Тогда
				Форма.Элементы.ДействияОстановить.Заголовок = НСтр("ru = 'Продолжить'; en = 'Continue'");
				Форма.Элементы.ДекорацияОтступСостояния.Видимость = Истина;
				Если ТипПредмета = "DMDocument" Тогда
					Подсказка = НСтр("ru = 'Продолжить обработку документа'; en = 'Continue document processing'");
				Иначе
					Подсказка = НСтр("ru = 'Продолжить обработку мероприятия'; en = 'Continue event processing'");
				КонецЕсли;
		...
		Если Форма.СостояниеОбработки = "Executing" Тогда
			...
			Форма.Элементы.ДекорацияОписание.Заголовок = СтрШаблон(НСтр("ru = 'Обработка начата %1'; en = 'Processing is started on %1'"),
				Формат(ДатаНачалаОбработки, НСтр("ru = ""ДФ='dd.MM.yyyy HH:mm'""; en = ""DF='MM/dd/yyyy hh:mm tt'""")));
		ИначеЕсли Форма.СостояниеОбработки = "Stopped" Тогда
			Форма.Элементы.ДекорацияСостояния.Картинка = БиблиотекаКартинок.ЗнакПауза;
			Форма.Элементы.ДекорацияОписание.Заголовок = СтрШаблон(НСтр("ru = 'Обработка на паузе с %1'; en = 'Processing is paused from %1'"),
				Формат(ДатаИзмененияСостоянияОбработки, НСтр("ru = ""ДФ='HH:mm dd.MM.yyyy'""; en = ""DF='HH:mm MM/dd/yyyy'""")));
		КонецЕсли;
		Форма.Элементы.ДекорацияОписание.Подсказка = Форма.Элементы.ДекорацияОписание.Заголовок;
	...
	ИначеЕсли Форма.СостояниеОбработки = "Completed" Тогда
		...
		Форма.Элементы.ДекорацияОписание.Заголовок = СтрШаблон(НСтр("ru = 'Обработка завершена  %1'; en = 'Processing is completed on %1'"),
			Формат(ДатаИзмененияСостоянияОбработки, НСтр("ru = ""ДФ='dd.MM.yyyy HH:mm'""; en = ""DF='MM/dd/yyyy hh:mm tt'""")));
		Форма.Элементы.ДекорацияОписание.Подсказка = Форма.Элементы.ДекорацияОписание.Заголовок;
		Форма.Элементы.ДекорацияСостояния.Видимость = Истина;
	...
	Если ЗначениеЗаполнено(ДатаОкончания) Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Обработка прервана:'; en = 'Processing is terminated:'"), ЖирныйШрифт));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Формат(ДатаОкончания, НСтр("ru = ""ДФ='dd.MM.yyyy HH:mm'""; en = ""DF='MM/dd/yyyy hh:mm tt'"""))));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.ПС));
		Если ЗначениеЗаполнено(ПричинаПрерывания) Тогда
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Причина:'; en = 'Reason:'"), ЖирныйШрифт));
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
		...
		Если ЗначениеЗаполнено(Прервал) Тогда
			...
			МассивСтрок = Новый Массив;
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Прервал:'; en = 'Terminated by:'"), ЖирныйШрифт));
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Символы.НПП));
	...
	Если КоличествоДействий = 0 И Не ОбработкаЗаданаПравилами И Не Форма.Элементы.ДекорацияОписание.Видимость Тогда
		Если ТипПредмета = "DMDocument" Тогда
			Шаблон = НСтр("ru = 'Для документов вида ""%1"" обработка не настроена'; en = 'The data processor is not configured for documents of the %1 kind'");
		Иначе
			Шаблон = НСтр("ru = 'Для мероприятий вида ""%1"" обработка не настроена'; en = 'The data processor is not configured for events of the %1 kind'");
		КонецЕсли;
		Форма.Элементы.ДекорацияСостояния.Видимость = Истина;
...
Процедура ЗаполнитьСтрокуXDTOИзУчастникаДействия(Прокси, СтрокаXDTO, СтрокаУчастника) Экспорт
	...
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "detailsSpecial")
			И СтруктураСтрокиУчастника.ОписаниеОсобое <> Неопределено Тогда
		СтрокаXDTO.detailsSpecial = СтруктураСтрокиУчастника.ОписаниеОсобое;
	КонецЕсли;
	...
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDate")
			И СтруктураСтрокиУчастника.СрокОбработкиРезолюции <> Неопределено Тогда
		СтрокаXDTO.resolutionProcessingDate = СтруктураСтрокиУчастника.СрокОбработкиРезолюции;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDateDays")
			И СтруктураСтрокиУчастника.СрокОбработкиРезолюцииДни <> Неопределено Тогда
		СтрокаXDTO.resolutionProcessingDateDays = СтруктураСтрокиУчастника.СрокОбработкиРезолюцииДни;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDateHours")
			И СтруктураСтрокиУчастника.СрокОбработкиРезолюцииЧасы <> Неопределено Тогда
		СтрокаXDTO.resolutionProcessingDateHours = СтруктураСтрокиУчастника.СрокОбработкиРезолюцииЧасы;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDateMinutes")
			И СтруктураСтрокиУчастника.СрокОбработкиРезолюцииМинуты <> Неопределено Тогда
		СтрокаXDTO.resolutionProcessingDateMinutes = СтруктураСтрокиУчастника.СрокОбработкиРезолюцииМинуты;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDatePresentation") 
			И СтруктураСтрокиУчастника.СрокОбработкиРезолюцииПредставление <> Неопределено Тогда
		СтрокаXDTO.resolutionProcessingDatePresentation = СтруктураСтрокиУчастника.СрокОбработкиРезолюцииПредставление;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоСуществует(СтрокаXDTO, "resolutionProcessingDateSpecificationVariant") 
			И СтруктураСтрокиУчастника.ВариантУстановкиСрокаОбработкиРезолюции <> Неопределено
			И ЗначениеЗаполнено(СтруктураСтрокиУчастника.ВариантУстановкиСрокаОбработкиРезолюции) Тогда
		// Перечисление.
		СтрокаXDTO.resolutionProcessingDateSpecificationVariant = СтруктураСтрокиУчастника.ВариантУстановкиСрокаОбработкиРезолюции;
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуУчастникаДействияИзXDTO(СтрокаУчастника, СтрокаXDTO) Экспорт
	...
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "detailsSpecial") Тогда
		СтруктураСтрокиУчастника.ОписаниеОсобое = СтрокаXDTO.detailsSpecial;
	КонецЕсли;
	...
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDate") Тогда
		СтруктураСтрокиУчастника.СрокОбработкиРезолюции = СтрокаXDTO.resolutionProcessingDate;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDateDays") Тогда
		СтруктураСтрокиУчастника.СрокОбработкиРезолюцииДни = СтрокаXDTO.resolutionProcessingDateDays;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDateHours") Тогда
		СтруктураСтрокиУчастника.СрокОбработкиРезолюцииЧасы = СтрокаXDTO.resolutionProcessingDateHours;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDateMinutes") Тогда
		СтруктураСтрокиУчастника.СрокОбработкиРезолюцииМинуты = СтрокаXDTO.resolutionProcessingDateMinutes;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDatePresentation") Тогда
		СтруктураСтрокиУчастника.СрокПредставление = СтрокаXDTO.resolutionProcessingDatePresentation;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "resolutionProcessingDateSpecificationVariant") Тогда
		СтруктураСтрокиУчастника.ВариантУстановкиСрокаОбработкиРезолюции = СтрокаXDTO.resolutionProcessingDateSpecificationVariant;
	КонецЕсли;
...
Процедура ОбработатьОтветВебСервисаНаПолучениеДействия(Форма, ДействиеXDTO) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
		...
		"НастройкаДействия");
	Если Форма.ОсновнойПредметID = ""
			И ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ДействиеXDTO, "target") Тогда
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
			Форма,
			ДействиеXDTO.target,
			"ОсновнойПредмет");
...
Функция ПодсказкаКнопкиРазныеСрокиДействия(ПоПунктам, ПоЭтапам) Экспорт
	...
	Если ПоПунктам Тогда
		Уточнение = НСтр("ru = ', в пределах пункта,'; en = ', within the point,'")
	ИначеЕсли ПоЭтапам Тогда
		Уточнение = НСтр("ru = ', в пределах этапа,'; en = ', within the stage,'")
	КонецЕсли;
	
	Возврат СтрШаблон(
		НСтр("ru = 'Если включен, то у каждого участника%1 можно указать свой срок'; en = 'If enabled, you can specify a separate due date for every %1 participant'"), Уточнение);
...
//     * ID - Строка
//     * type - Строка
//     * ОсновнойПредмет - Строка
//     * ОсновнойПредметID - Строка
//     * ОсновнойПредметТип - Строка
//
Процедура ПриСозданииНаСервереФормыДействия(Форма, Параметры) Экспорт
	...
	Форма.ОсновнойПредмет = Параметры.ОсновнойПредмет;
	...
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
		...
		НСтр("ru = 'Участники при открытии'; en = 'Participants on opening'")));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(
		"УчастникиПередЗаписью",
		Новый ОписаниеТипов("ТаблицаЗначений"),,
		НСтр("ru = 'Участники перед записью'; en = 'Participants before writing'")));
...
Функция УчастникДействияXDTO(Прокси, Исполнитель, ОсновнойОбъектАдресации = Неопределено,
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Некорректно задан тип участника действия: %1'; en = 'The action participant type is incorrect: %1'"), Исполнитель.Тип);
...
// Выполняет перерисовку дерева связей в форме.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма с деревом связей.
//   СписокСвязейXDTO - ОбъектXDTO - объект XDTO типа DMGetRelationsResponse.
//
Процедура ЗаполнитьДеревоСвязейВФорме(Форма, СписокСвязейXDTO) Экспорт
	
	Если Форма.ДеревоСвязейРазворачиваемаяСтрока = 0 Тогда
		ЭлементыДерева = Форма.ДеревоСвязей.ПолучитьЭлементы();
	Иначе
		РазворачиваемаяВетка = Форма.ДеревоСвязей.НайтиПоИдентификатору(Форма.ДеревоСвязейРазворачиваемаяСтрока);
		ЭлементыДерева = РазворачиваемаяВетка.ПолучитьЭлементы();
	КонецЕсли;
	...
	Для Каждого Связь Из СписокСвязейXDTO.relations Цикл
		
		Если Форма.ДеревоСвязейРазворачиваемаяСтрока <> 0 И ЗначениеЗаполнено(Связь.relatedObject.objectID.ID) Тогда
			СуществующаяСвязь = ИнтеграцияС1СДокументооборот3КлиентСервер.СвязьПоID(
				Форма.ДеревоСвязей.ПолучитьЭлементы(),
				Связь.relatedObject.objectID.ID,
				Связь.relatedObject.objectID.type);
			Если СуществующаяСвязь <> Неопределено
					Или (Форма.ОбъектДОID = Связь.relatedObject.objectID.ID
						И Форма.ОбъектДОТип = Связь.relatedObject.objectID.type) Тогда
				// Для предотвращения зацикливания пропускаем связь с объектом, который уже присутствует в дереве.
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		Форма.ВсегоСвязей = Форма.ВсегоСвязей + 1;
		
		Если Форма.ТолькоВажныеСвязи И Не Связь.relationType.important Тогда
			// Пропускаем связь, которая не является важной.
			Продолжить;
		КонецЕсли;
		
		Форма.ПрочитаноСвязей = Форма.ПрочитаноСвязей + 1;
		
		СтрокаТипСвязи = Неопределено;
		Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
			Если ЭлементДерева.ТипСвязиID = Связь.relationType.objectID.ID
					И ЭлементДерева.ТипСвязиТип = Связь.relationType.objectID.type Тогда
				СтрокаТипСвязи = ЭлементДерева;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если СтрокаТипСвязи = Неопределено Тогда
			СтрокаТипСвязи = ЭлементыДерева.Добавить();
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
				СтрокаТипСвязи,
				Связь.relationType,
				"ТипСвязи");
			СтрокаТипСвязи.ИндексКартинки = -1;
			СтрокаТипСвязи.ЭтоРодитель = Истина;
		КонецЕсли;
		
		СвязанныеОбъекты = СтрокаТипСвязи.ПолучитьЭлементы();
		СтрокаСвязанныйОбъект = СвязанныеОбъекты.Добавить();
		СтрокаТипСвязи.СвязанныйОбъект = СтрШаблон("%1 (%2)", Связь.relationType.abbreviatedName, СвязанныеОбъекты.Количество());
		
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
			СтрокаСвязанныйОбъект,
			Связь.relatedObject,
			"СвязанныйОбъект");
		СтрокаСвязанныйОбъект.Ссылка =
			РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.СсылкаНаОбъектИСПоДаннымДокументооборота(
				СтрокаСвязанныйОбъект.СвязанныйОбъектID,
				СтрокаСвязанныйОбъект.СвязанныйОбъектТип);
		СтрокаСвязанныйОбъект.НавигационнаяСсылка = Связь.relatedObject.objectID.navigationRef;
		
		Если ЗначениеЗаполнено(СтрокаСвязанныйОбъект.Ссылка) Тогда
			// Ситуация, когда права на объект в ДО есть, а в ИС нет, не должна приводить к исключению.
			// При возникновении исключения получим представление из ДО, а пользователь
			// увидит "Нарушение прав доступа" лишь при попытке открыть объект.
			Попытка
				СтрокаСвязанныйОбъект.СвязанныйОбъект = СтрШаблон(НСтр("ru = '%1, объект %2'; en = '%1, object %2'"),
					Строка(СтрокаСвязанныйОбъект.Ссылка),
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИмяСобытияЖурналаРегистрации(
						НСтр("ru = 'Ошибка при получении представления связанного объекта'; en = 'An error occurred when receiving presentation of the linked object'",
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
			...
			Связь.relationType,
			"ТипСвязи");
		СтрокаСвязанныйОбъект.НомерДокумента = Связь.relatedObject.number;
		СтрокаСвязанныйОбъект.ДатаДокумента = Связь.relatedObject.date;
		СтрокаСвязанныйОбъект.Сумма = Связь.relatedObject.sumPresentation;
		СтрокаСвязанныйОбъект.ИндексКартинки = -1;
		СтрокаСвязанныйОбъект.ПометкаУдаления = Связь.relatedObject.deletionMark;
		СтрокаСвязанныйОбъект.ВажнаяСвязь = Связь.relationType.important;
		СтрокаСвязанныйОбъект.ИмяПредмета = Связь.relatedObject.subjectName;
		Если Не ЗначениеЗаполнено(СтрокаСвязанныйОбъект.СвязанныйОбъектID)
				И Не ЗначениеЗаполнено(СтрокаСвязанныйОбъект.СвязанныйОбъектТип) Тогда
			СтрокаСвязанныйОбъект.СвязаннаяСтрока = Связь.relatedObject.subjectName;
		КонецЕсли;
		
		Для Каждого Файл Из Связь.relatedObject.files Цикл
			СтрокаРоль = Неопределено;
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Файл, "role") Тогда
				Для Каждого ЭлементДерева Из СтрокаСвязанныйОбъект.ПолучитьЭлементы() Цикл
					Если ЭлементДерева.СвязанныйОбъектID = Файл.role.objectID.ID
							И ЭлементДерева.СвязанныйОбъектТип = Файл.role.objectID.type Тогда
						СтрокаРоль = ЭлементДерева;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если СтрокаРоль = Неопределено Тогда
					СтрокаРоль = СтрокаСвязанныйОбъект.ПолучитьЭлементы().Добавить();
					ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
						СтрокаРоль,
						Файл.role,
						"СвязанныйОбъект");
					ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
						...
						Связь.relationType,
						"ТипСвязи");
					СтрокаРоль.ИндексКартинки = 2;
					СтрокаРоль.ЭтоРольФайла = Истина;
				КонецЕсли;
			КонецЕсли;
			
			Если Форма.ПоказыватьТолькоОригиналыВСвязях = Истина И Файл.scannedOriginal = Ложь Тогда
				Продолжить
			КонецЕсли;
			
			Если СтрокаРоль = Неопределено Тогда
				СтрокаФайл = СтрокаСвязанныйОбъект.ПолучитьЭлементы().Добавить();
			Иначе
				СтрокаФайл = СтрокаРоль.ПолучитьЭлементы().Добавить();
			КонецЕсли;
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
				СтрокаФайл,
				Файл,
			...
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
				...
				Связь.relationType,
				"ТипСвязи");
			СтрокаФайл.ЯвляетсяОригиналом = Файл.scannedOriginal;
			СтрокаФайл.ИндексКартинки =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИндексПиктограммыФайла(Файл.extension);
			СтрокаФайл.ПредставлениеHTML = Файл.htmlView;
			СтрокаФайл.ПредпросмотрУрезан = Файл.htmlViewTruncated;
			СтрокаФайл.РасширениеФайла = Файл.extension;
			СтрокаФайл.ПометкаУдаления = Файл.deletionMark;
			СтрокаФайл.НавигационнаяСсылка = Файл.objectID.navigationRef;
			
			СтрокаФайл.Редактируется = Файл.editing;
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Файл, "editingUser") Тогда
				СтрокаФайл.РедактируетсяТекущимПользователем =
					ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ФайлРедактируетсяТекущимПользователем(
						Файл.editingUser.objectID.ID,
						Файл.editingUser.objectID.type,
						ТекущийПользовательИСотрудники);
			Иначе
				СтрокаФайл.РедактируетсяТекущимПользователем = Ложь;
			КонецЕсли;
		КонецЦикла;
		
		Если Связь.relatedObject.hasAnotherRelations Тогда
			СтрокаСвязь = СтрокаСвязанныйОбъект.ПолучитьЭлементы().Добавить();
			СтрокаСвязь.СвязанныйОбъект = НСтр("ru = 'Связи'; en = 'Links'");
			СтрокаСвязь.ИндексКартинки = 0;
			СтрокаСвязь.ЭтоВеткаСвязи = Истина;
			СлужебнаяСтрокаСвязи = СтрокаСвязь.ПолучитьЭлементы().Добавить();
			СлужебнаяСтрокаСвязи.СлужебнаяСтрока = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	Если Форма.ТолькоВажныеСвязи Тогда
		Форма.Элементы.ГруппаИнфоСвязи.Видимость = (Форма.ПрочитаноСвязей <> Форма.ВсегоСвязей);
		Форма.Элементы.ДекорацияТолькоВажныеТекст.Заголовок = СтрШаблон(
			НСтр("ru = 'Показаны только важные связи (%1). Чтобы увидеть все (%2), нажмите ""Показать все связи"".'; en = 'Only important links are shown (%1). To see all (%2), click ""Show all links"".'"),
			Форма.ПрочитаноСвязей, Форма.ВсегоСвязей);
	КонецЕсли;
	
КонецПроцедуры

// Получает связи объекта 1С:Документооборота.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   РеквизитыОбъекта - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetRelationsResponse.
//
Функция СвязиОбъекта(Прокси, РеквизитыОбъекта) Экспорт
	
	Запрос = СвязиОбъектаЗапрос(Прокси, РеквизитыОбъекта);
	Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(Прокси, Запрос);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, Ответ);
...
// Возвращает запрос для получения связей объекта 1С:Документооборота.
//
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   РеквизитыОбъекта - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДанныеСсылочногоОбъектаДО.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetRelationsRequest.
//
Функция СвязиОбъектаЗапрос(Прокси, РеквизитыОбъекта) Экспорт
	
	Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMGetRelationsRequest");
	
	Запрос.object = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMRelatedObject");
	Запрос.object.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
		Прокси,
		РеквизитыОбъекта.ID,
		РеквизитыОбъекта.Тип);
	Запрос.object.name = РеквизитыОбъекта.Наименование;
...
Процедура ДополнитьОтборКонтекстомПравилКоммуникаций(Прокси, Контекст, УсловияОтбора) Экспорт
	...
	Для Каждого ОписаниеЭлементаКонтекста Из Контекст Цикл
		Если ОписаниеЭлементаКонтекста.Значение = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
// Проверяет и добавляет программно реквизит "ДокументPDFРеквизит" и поле "ДокументPDFПоле" на форму.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - необходимая форма.
//   ИмяРодителя - Строка - имя родителя элемента "ДокументPDFПоле".
//
Процедура ВывестиPDFДокументНаФорму(Форма, ИмяРодителя) Экспорт
	
	Элементы = Форма.Элементы;
	Если Элементы.Найти("ДокументPDFПоле") <> Неопределено Тогда
		// ДокументPDFПоле уже есть на форме.
		Возврат;
	КонецЕсли;
	
	РеквизитДокументPDF = ИнтеграцияС1СДокументооборотБазоваяФункциональность.РеквизитФормыПоИмени(
		Форма.ПолучитьРеквизиты(),
		"ДокументPDFРеквизит");
	Если РеквизитДокументPDF = Неопределено Тогда
		ДобавляемыеРеквизиты = Новый Массив;
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДокументPDFРеквизит", Новый ОписаниеТипов("ДокументPDF")));
		Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	КонецЕсли;
	
	ГруппаКоманд = Элементы.Найти("ГруппаОбзорФайлаКоманды");
	Если ГруппаКоманд <> Неопределено
			И ГруппаКоманд.Группировка <> ГруппировкаПодчиненныхЭлементовФормы.Вертикальная Тогда
		// Только вертикальная группа команд должна располагаться правее поля PDF.
		ГруппаКоманд = Неопределено;
	КонецЕсли;
	
	ПолеPDF = Элементы.Вставить("ДокументPDFПоле", Тип("ПолеФормы"), Элементы.Найти(ИмяРодителя), ГруппаКоманд);
	ПолеPDF.Вид = ВидПоляФормы.ПолеPDFДокумента;
	ПолеPDF.ПутьКДанным = "ДокументPDFРеквизит";
	ПолеPDF.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	ПолеPDF.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
...
// Возвращает идентификатор приоритетного узла пользователя.
//
// Параметры:
//   Пользователь - ОпределяемыйТип.Пользователь - ссылка на пользователя.
//   ВсеУзлыXDTO - СписокXDTO - список объектов XDTO типа DMDataExchangeNode.
//
// Возвращаемое значение:
//   Строка - идентификатор узла.
//
Функция ПриоритетныйУзел(Пользователь, ВсеУзлыXDTO) Экспорт
	
	ДанныеУзлов = РегистрыСведений.ДанныеУзлов1СДокументооборота.ДанныеУзлов();
	ДоступныйУзел = ИнтеграцияС1СДокументооборотБазоваяФункциональность.УзелДляАвторизацииВДО(Пользователь);
	
	УзлыПользователя = Новый ТаблицаЗначений;
	УзлыПользователя.Колонки.Добавить("Узел", Новый ОписаниеТипов("Строка"));
	УзлыПользователя.Колонки.Добавить("ИдентификаторУзла", Новый ОписаниеТипов("Строка"));
	УзлыПользователя.Колонки.Добавить("ЦентральныйУзел", Новый ОписаниеТипов("Булево"));
	УзлыПользователя.Колонки.Добавить("ЕстьДоступ", Новый ОписаниеТипов("Булево"));
	
	Для Каждого УзелXDTO Из ВсеУзлыXDTO Цикл
		ИдентификаторУзла = УзелXDTO.objectID.ID;
		ЭтоЦентральныйУзел = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(УзелXDTO, "isCentral")
			И УзелXDTO.isCentral;
		Если Не ЭтоЦентральныйУзел
				И (ДанныеУзлов[ИдентификаторУзла] = Неопределено
					Или Не ЗначениеЗаполнено(ДанныеУзлов[ИдентификаторУзла].АдресВебСервиса)) Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = УзлыПользователя.Добавить();
		НоваяСтрока.Узел = УзелXDTO.name;
		НоваяСтрока.ИдентификаторУзла = ИдентификаторУзла;
		НоваяСтрока.ЦентральныйУзел = ЭтоЦентральныйУзел;
		НоваяСтрока.ЕстьДоступ = (ИдентификаторУзла = ДоступныйУзел);
	КонецЦикла;
	
	Если УзлыПользователя.Количество() > 0 Тогда
		УзлыПользователя.Сортировать("ЕстьДоступ УБЫВ, ЦентральныйУзел УБЫВ, Узел");
		УзелПользователя = УзлыПользователя[0];
		Если УзелПользователя.ЦентральныйУзел Тогда
			// Если пользователь есть в центральном узле - оставим параметр сеанса ИнтеграцияС1СДокументооборотУзел
			// пустым. Все обращения к сервису ДО будем выполнять по данным константы АдресВебСервиса1СДокументооборот.
			Возврат "";
		Иначе
			Возврат УзелПользователя.ИдентификаторУзла;
		КонецЕсли;
	Иначе
		// Данные узлов пользователя не заданы. Оставим параметр сеанса ИнтеграцияС1СДокументооборотУзел пустым.
		// Все обращения к сервису ДО будем выполнять по данным константы АдресВебСервиса1СДокументооборот.
		Возврат "";
	КонецЕсли;
	
КонецФункции

// Возвращает узел 1С:Документооборот Холдинг, которому соответствует текущий пользователь.
//
// Возвращаемое значение:
//   Строка - идентификатор узла.
//
Функция УзелТекущегоПользователя() Экспорт
	
	Если Не РегистрыСведений.ДанныеУзлов1СДокументооборота.ИспользуютсяУзлыХолдинга() Тогда
		Возврат "";
	КонецЕсли;
	
	Пользователь = Пользователи.ТекущийПользователь();
	
	НазначенныйУзел = РегистрыСведений.СоответствиеПользователейУзлам1СДокументооборота.УзелПользователя(Пользователь);
	Если ЗначениеЗаполнено(НазначенныйУзел) Тогда
		Возврат НазначенныйУзел;
	КонецЕсли;
	
	РеквизитыПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Пользователь, "Служебный, Недействителен, ПометкаУдаления");
	Если РеквизитыПользователя.Служебный = Истина
			Или РеквизитыПользователя.Недействителен = Истина
			Или РеквизитыПользователя.ПометкаУдаления = Истина Тогда
		// Служебный или недействительный пользователь должен обращаться в ЦУ.
		Возврат "";
	КонецЕсли;
	
	// Если по данным ИБ узел определить не удалось - требуется сделать запрос в ЦУ для определения списка узлов,
	// где есть пользователь, и для выбора приоритетного узла.
	...
	Попытка
		...
		НастройкиАвторизации.ИмяПользователя = Константы.ИнтеграцияС1СДокументооборотИмяПользователяДляОбмена.Получить();
		Если Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя) Тогда
			ВызватьИсключение
				НСтр("ru = 'Не установлено значение константы ""Имя пользователя для обмена с 1С:Документооборотом""'; en = 'The ""Username for exchange with 1C:Document Management"" constant value is not set.'");
		КонецЕсли;
		НастройкиАвторизации.Пароль = Константы.ИнтеграцияС1СДокументооборотПарольДляОбмена.Получить();
		УстановитьПривилегированныйРежим(Ложь);
		
		ВызыватьИсключение = Истина;
		ЭтоПользовательЗаданияОбмена = Истина;
		Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьПрокси(
			ВызыватьИсключение,
			НастройкиАвторизации,
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.АдресВебСервиса1СДокументооборот(Истина),,
			ЭтоПользовательЗаданияОбмена);
		Запрос = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMGetUserNodesRequest");
		
		Запрос.user = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(Прокси, "DMUser", Пользователь);
		
		Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(Прокси, Запрос);
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, Ответ);
		
		Возврат ПриоритетныйУзел(Пользователь, Ответ.nodes);
...
// Устанавливает условное оформление дерева узлов 1С:Документооборота Холдинга.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма, содержащая дерево узлов.
//
Процедура УстановитьУсловноеОформлениеУзловХолдинга(Форма) Экспорт
	
	// Центральный узел - жирным шрифтом.
	ЭлементОформления = Форма.УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Форма.Элементы.УзлыКОД.Имя);
	
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("УзлыКОД.ЦентральныйУзел");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = Истина;
	
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ВажнаяНадписьШрифт);
	
	// Не активный узел - серым цветом.
	ЭлементОформления = Форма.УсловноеОформление.Элементы.Добавить();
	...
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("УзлыКОД.Активен");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMILRequest. Сообщение из ДО.
//
// Возвращаемое значение:
//   Строка - версия ДО.
//
...
Процедура ДобавитьОписаниеДопРеквизитов(СписокРеквизитовXDTO, СписокРеквизитов, Синонимы)
	...
	Для Каждого Реквизит Из СписокРеквизитов Цикл
		...
		РеквизитXDTO.synonym = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1'; en = 'Additional attribute: %1'"), Реквизит.Наименование);
	...
	Для Каждого Реквизит Из СписокРеквизитов Цикл
		...
		Если ЭтоСтандартныеРеквизиты Тогда
			...
			Если Реквизит.Имя = "ПометкаУдаления" Тогда
				Синоним = НСтр("ru = 'Пометка удаления'; en = 'Deletion mark'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьОписаниеТиповРеквизита(СписокТиповXDTO, ТипРеквизита, Синонимы)
	...
	Если ТипЗнч(ТипРеквизита) <> Тип("ОписаниеТипов") Тогда
		СписокТиповXDTO.Добавить(НСтр("ru = 'Неопределено'; en = 'Undefined'"));
		Возврат;
	КонецЕсли;
	...
	Если МассивТипов.Количество() > 3 Тогда
		СписокТиповXDTO.Добавить(НСтр("ru = 'Составной тип'; en = 'Composite type'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗафиксироватьОшибкуПревышенияВремениВыполнения(ВремяВыполнения, Сообщение, Результат)
	...
	ЗаписьЖурналаРегистрации(
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИмяСобытияЖурналаРегистрации(
			НСтр("ru = 'Обработка запросов XDTO'; en = 'XDTO request processing'", ОбщегоНазначения.КодОсновногоЯзыка())),
		УровеньЖурналаРегистрации.Ошибка,,,
...
// Возвращает ObjectID по ссылке на объект ИС.
//
// Параметры:
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMILObjectID.
//
Функция ПолучитьDMILObjectIDПоСсылке(Ссылка, ТолькоОбязательныеПоля = Ложь, Представление = Неопределено)
	
	DMILObjectID = СоздатьОбъектБИД("DMILObjectID");
	DMILObjectID.ID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьIDПоСсылке(Ссылка);
	DMILObjectID.type = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьИмяТипаСсылки(Ссылка);
	
	Если ТолькоОбязательныеПоля Тогда
		Возврат DMILObjectID;
	КонецЕсли;
	
	Если Не Перечисления.ТипВсеСсылки().СодержитТип(ТипЗнч(Ссылка)) Тогда
		DMILObjectID.navigationRef = ПолучитьНавигационнуюСсылку(Ссылка);
	Иначе
		DMILObjectID.navigationRef = "";
	КонецЕсли;
	
	Если Представление <> Неопределено Тогда
		DMILObjectID.presentation = Представление;
	ИначеЕсли Ссылка.Пустая() Тогда
		DMILObjectID.presentation = Ссылка.Метаданные().Синоним;
	Иначе
		DMILObjectID.presentation = Строка(Ссылка);
	КонецЕсли;
...
"Обратитесь к администратору.'; en = 'Insufficient rights to execute an expression in 1C:Enterprise language."
"Contact the administrator.'");
	...
	Исключение
		...
		Ответ = ОписаниеОшибкиXDTO(
			НСтр("ru = 'Найдена ошибка в выражении на встроенном языке'; en = 'There is an error in expression in 1C:Enterprise language'"),
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Выполняет синхронизацию пользователей ДО и интегрированной системы.
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMILSynchronizeUsersRequest.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMILSynchronizeUsersResponse или DMILError.
//
Функция СинхронизироватьПользователейИнтегрированнойСистемыИПользователейДО(Сообщение)
	
	НачатьТранзакцию();
	
	Попытка
		
		Ответ = СоздатьОбъектБИД("DMILSynchronizeUsersResponse");
		
		ЭтоПодчиненныйУзел = (Прав(Сообщение.versionNumber, 4) = "HOLD");
		
		СвязиПользователей = Новый Соответствие;
		
		Для Каждого ПользовательДО Из СписокОбъектовXDTOизСообщения(Сообщение.messageData) Цикл
			
			СтарыйПользовательИС =
				РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.СсылкаНаОбъектИСПоДаннымДокументооборота(
					ПользовательДО.objectID.ID,
					ПользовательДО.objectID.type);
			Если ЗначениеЗаполнено(СтарыйПользовательИС) Тогда
				СтарыеДанные =
					РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДанныеОбъектаДОПоВнешнемуОбъекту(
						СтарыйПользовательИС);
				Если ТипЗнч(СтарыеДанные.ДополнительныеДанные) <> Тип("Структура") Тогда
					СтарыеДанные.ДополнительныеДанные = Новый Структура("ЭтоПодчиненныйУзел, Узел", Ложь, Неопределено);
				КонецЕсли;
				Если ЭтоПодчиненныйУзел И Не СтарыеДанные.ДополнительныеДанные.ЭтоПодчиненныйУзел Тогда
					// Связь пользователя ИС и пользователем ДО из ЦУ более приоритетна, чем связь с ПУ.
					ownerObject = СоздатьОбъектБИД("ExternalObjectID");
					ownerObject.ID = СтарыеДанные.ID;
					ownerObject.type = СтарыеДанные.type;
					СвязиПользователей[СтарыйПользовательИС] = ownerObject;
					Продолжить;
				КонецЕсли;
			...
			Если ЗначениеЗаполнено(ПользовательИС) Тогда
				...
				Если ТекущиеДанные <> Неопределено Тогда
					Если ТипЗнч(ТекущиеДанные.ДополнительныеДанные) <> Тип("Структура") Тогда
						ТекущиеДанные.ДополнительныеДанные = Новый Структура("ЭтоПодчиненныйУзел, Узел", Ложь, Неопределено);
					КонецЕсли;
					Если ЭтоПодчиненныйУзел И Не ТекущиеДанные.ДополнительныеДанные.ЭтоПодчиненныйУзел Тогда
						// Связь пользователя ИС и пользователем ДО из ЦУ более приоритетна, чем связь с ПУ.
						ownerObject = СоздатьОбъектБИД("ExternalObjectID");
						ownerObject.ID = ТекущиеДанные.ID;
						ownerObject.type = ТекущиеДанные.type;
						СвязиПользователей[ПользовательИС] = ownerObject;
						Продолжить;
					КонецЕсли;
				КонецЕсли;
				
				Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
						ПользовательДО, "infoBaseUser") Тогда
					ДополнительныеДанные =
						ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДополнительныеДанныеСвязиПользователейДОиИС();
					ДополнительныеДанные.ПользовательИБ = ПользовательДО.infoBaseUser.name;
					ДополнительныеДанные.ПользовательОС = ПользовательДО.infoBaseUser.OSUserName;
					ДополнительныеДанные.АдресЭлектроннойПочты = ПользовательДО.infoBaseUser.email;
					ДополнительныеДанные.Узел = Сообщение.nodeID;
					ДополнительныеДанные.ЭтоПодчиненныйУзел = ЭтоПодчиненныйУзел;
				КонецЕсли;
				РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДобавитьСвязь(
					...
					ДополнительныеДанные);
				ownerObject = СоздатьОбъектБИД("ExternalObjectID");
				ownerObject.ID = ПользовательДО.objectID.ID;
				ownerObject.type = ПользовательДО.objectID.type;
				СвязиПользователей[ПользовательИС] = ownerObject;
				
			ИначеЕсли ЗначениеЗаполнено(СтарыйПользовательИС)
					И СтарыеДанные.ДополнительныеДанные.Узел <> Сообщение.nodeID Тогда
				
				// Восстановим удаленную ранее связь, т.к. она может быть актуальна для другого узла.
				РегистрыСведений.ОбъектыИнтегрированныеС1СДокументооборотом.ДобавитьСвязь(
					СтарыеДанные.ID,
					СтарыеДанные.type,
					СтарыеДанные.ВнешнийОбъект,
					Ложь,
					СтарыеДанные.ДополнительныеДанные);
				
			КонецЕсли;
			
		КонецЦикла;
		
		СвязиПользователейXDTO = Ответ.links; // СписокXDTO
		
		Для Каждого Связь Из СвязиПользователей Цикл
			СвязьXDTO = СоздатьОбъектБИД("DMILLink");
			СвязьXDTO.linkedObject = ПолучитьDMILObjectIDПоСсылке(Связь.Ключ);
			СвязьXDTO.ownerObject = Связь.Значение;
			СвязиПользователейXDTO.Добавить(СвязьXDTO);
		КонецЦикла;
	...
	Исключение
		...
		КонецЕсли;
		Возврат ОписаниеОшибкиXDTO(
			СтрШаблон(НСтр("ru = 'Ошибка при синхронизации пользователей ДО с пользователями %1'; en = 'An error occurred when syncing DI users with %1 users'"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации()),
			ПолучитьОписаниеОшибки(ИнформацияОбОшибке()));
...
Функция СписокВыраженийНаВстроенномЯзыкеДляВыполненияНаСторонеДО(Сообщение)
	...
	Попытка
		...
		Для Каждого СоответствиеObjectID Из СоответствиеObjectIDОбъектамИС Цикл
			СоответствиеПравилXDTO = СоздатьОбъектБИД("DMILObjectExpressionRow");
			СкопироватьОбъектXDTO(СоответствиеПравилXDTO.object, СоответствиеObjectID.Значение);
			СписокВыражений = СоответствиеПравилXDTO.expressionRows; // СписокXDTO
			
			СоответствиеПравил = ПодходящиеПравила[СоответствиеObjectID.Ключ];
			Если ТипЗнч(СоответствиеПравил) <> Тип("Массив") Тогда
				КолВоПравил = 0;
			Иначе
				КолВоПравил = СоответствиеПравил.Количество();
			КонецЕсли;
			Если КолВоПравил = 1 Тогда
				ВыраженияПравила = ТаблицаВыраженияПравил.НайтиСтроки(Новый Структура("Ссылка", СоответствиеПравил[0]));
			...
			Иначе
				СоответствиеПравилXDTO.error = ОписаниеОшибкиXDTO(
					НСтр("ru = 'Ошибка при получении списка выражений на встроенном языке'; en = 'An error occurred when receiving the 1C:Enterprise language expression list'"),
					СтрШаблон(
						НСтр("ru = 'Для объекта ""%1"" не удалось подобрать единственное подходящее правило интеграции."
"Всего найдено правил: %2'; en = 'Cannot select the only integration rule suitable for the ""%1"" object."
"Found rules in total: %2'"),
							СоответствиеObjectID.Ключ,
							КолВоПравил));
			КонецЕсли;
...
Функция СписокПечатныхФормОбъектовИС(Сообщение)
	...
	Исключение
		...
		Ответ = ОписаниеОшибкиXDTO(
			НСтр("ru = 'Ошибка при получении списка печатных форм'; en = 'An error occurred when getting the print form list'"),
			ПолучитьОписаниеОшибки(ИнформацияОбОшибке()));
...
Функция СтруктураМетаданныхИнтегрированнойСистемы(Сообщение)
	...
	Попытка
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Сообщение, "columnSet")
			...
			Для Каждого РеквизитКПолучению Из Сообщение.columnSet Цикл
				Если НРег(РеквизитКПолучению) = "objectsonly" Тогда
					ПолучатьРеквизиты = Ложь;
					ПолучатьТабличныеЧасти = Ложь;
					ПолучатьПечатныеФормы = Ложь;
					Прервать;
				ИначеЕсли НРег(РеквизитКПолучению) = "attributes" Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Сообщение, "objects")
			...
			Для Каждого ИмяТипа Из Сообщение.objects Цикл
				Если СтрНайти(ИмяТипа, "Ссылка.") = 0 Тогда
					ИмяТипа = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(ИмяТипа);
				КонецЕсли;
		...
		Для Каждого ОбъектМетаданных Из ОбъектыПоддерживающиеИнтеграцию Цикл
			...
			Если ПолучатьВсе Или ПолучатьРеквизиты Тогда
				...
				РеквизитXDTO.synonym = НСтр("ru = 'Представление'; en = 'Presentation'");
	...
	Исключение
		...
		Возврат ОписаниеОшибкиXDTO(
			НСтр("ru = 'Ошибка при получении структуры метаданных'; en = 'An error occurred when receiving the metadata structure'"),
			ПолучитьОписаниеОшибки(ИнформацияОбОшибке()));
...
// Возвращает XML схему веб-сервиса ДО по свойству сообщения DMXMLSchema.
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMILRequest. Сообщение из ДО.
//
...
//
Функция СхемаDMServiceПоСообщению(Сообщение)
	
	СхемаXML = "";
	
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Сообщение, "DMXMLSchema")
			И ЗначениеЗаполнено(Сообщение.DMXMLSchema) Тогда
		СхемаXML = Сообщение.DMXMLSchema;
	КонецЕсли;
...
Процедура НастроитьУсловноеОформлениеЭтапа(УсловноеОформление, Дерево = "Участники",
	...
	ЭлементУсловногоОформления = ЭлементУсловногоОформленияПоПредставлению(
		УсловноеОформление,
		НСтр("ru = 'Выделение этапов'; en = 'Select stages'"));
...
Процедура НастроитьУсловноеОформлениеПустогоУчастника(УсловноеОформление, Дерево = "Участники",
	...
	ЭлементУсловногоОформления = ЭлементУсловногоОформленияПоПредставлению(
		УсловноеОформление,
		НСтр("ru = 'Выделение пустого участника'; en = 'Select an empty participant'"));
	...
	ЭлементЦветаОформления.Значение = НСтр("ru = '<Не указан>'; en = '<Not specified>'");
	ЭлементЦветаОформления.Использование = Истина;
...
Процедура НастроитьУсловноеОформлениеПустогоУсловия(УсловноеОформление, Дерево = "Участники",
	...
	ЭлементУсловногоОформления = ЭлементУсловногоОформленияПоПредставлению(
		УсловноеОформление,
		НСтр("ru = 'Выделение пустого условия'; en = 'Select an empty condition'"));
	...
	ЭлементЦветаОформления = ЭлементУсловногоОформления.Оформление.Элементы.Найти("Текст");
	ЭлементЦветаОформления.Значение = НСтр("ru = '<Не установлено>'; en = '<Not set>'");
	ЭлементЦветаОформления.Использование = Истина;
...
Процедура НастроитьУсловноеОформлениеКомментарияЗаполненияУчастника(УсловноеОформление, ПолеДляОтбора, ПолеДляОформления)
	...
	ПредставлениеЭлемента = СтрШаблон(
		НСтр("ru = 'Комментарий заполнения участника в поле: %1'; en = 'Participant population comment in the field: %1'"),
		ПолеДляОтбора);
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СтрокаXDTO, "application") Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Ссылка) Тогда
					...
					Попытка
						НоваяСтрока.ПредставлениеПриложения = СтрШаблон(НСтр("ru = '%1, объект %2'; en = '%1, object %2'"),
							Строка(НоваяСтрока.Ссылка),
							ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации());
					Исключение
						ЗаписьЖурналаРегистрации(
							ИнтеграцияС1СДокументооборотБазоваяФункциональность.ИмяСобытияЖурналаРегистрации(
...
"Идентификатор выражения: %1.'; en = 'Expression text in 1C:Enterprise language is not specified."
"Expression ID: %1'"),
				Выражение.id);
		КонецЕсли;
...
"Идентификатор выражения: %1.'; en = 'Option of expression execution in 1C:Enterprise language is not specified."
"Expression ID: %1.'"),
				Выражение.id);
...
Процедура ЗаполнитьИнтерактивноеДействие(Прокси, ИнтерактивноеДействие, ИнтерактивноеДействиеXDTO)
	...
	ИначеЕсли НРег(ИнтерактивноеДействиеXDTO.interactiveAction) = "askaquestion" Тогда
		...
		ИнтерактивноеДействие.Контекст = СтруктураВопросаПользователю(ИнтерактивноеДействиеXDTO.context);
		
	ИначеЕсли НРег(ИнтерактивноеДействиеXDTO.interactiveAction) = "cyclelimitexhaustquestion" Тогда
		ИнтерактивноеДействие.ЗадатьВопросОЛимитеЦиклов = Истина;
		ИнтерактивноеДействие.Контекст = СтруктураВопросаПользователю(ИнтерактивноеДействиеXDTO.context);
...
Функция ПреобразоватьРеквизитОбъектаИСВОбъектXDTO(Прокси, ИмяРеквизита, ЗначениеРеквизита)
	...
	Иначе
		Результат.error = СтрШаблон(
			НСтр("ru = 'Невозможно выгрузить данные из реквизита ""%1"" с типом ""%2"".'; en = 'Cannot export data from the ""%1"" attribute with the ""%2"" type.'"),
			ИмяРеквизита,
			ТипРеквизита);
...
Функция СтруктураСтрокиУчастника()
	...
	Возврат Новый Структура(
		...
		|Условие, УсловиеID, УсловиеТип, УсловиеПредставление, Описание, ОписаниеОсобое,
		...
		|СрокОбработкиРезолюции, СрокОбработкиРезолюцииДни, СрокОбработкиРезолюцииЧасы, СрокОбработкиРезолюцииМинуты,
		|СрокОбработкиРезолюцииПредставление, ВариантУстановкиСрокаОбработкиРезолюции");
