...
Процедура ЗаполнитьСтатусПодписи(СвойстваПодписи, ДатаСеанса) Экспорт
	...
	Если Не ЗначениеЗаполнено(СвойстваПодписи.АдресПодписи) Тогда
		...
		Иначе
			СвойстваПодписи.РезультатПроверкиКраткий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ожидается подписание документа в 1С: Мобильная подпись с %1';
					|en = 'A document is awaiting signing in 1C:Mobile Signature from %1'"), СвойстваПодписи.ДатаПодписи);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов) Тогда
		...
		Если РезультатПроверки.ПодписьМатематическиВерна = Ложь Тогда
			СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. %2';
						|en = '%1. %2'"), Статус, РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов);
		...
		Иначе
			СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. Документ не изменен, но один из сертификатов или дополнительные атрибуты подписи не прошли проверку. %2';
						|en = '%1. The document was not modified, but one of the certificates or additional attributes failed validation. %2'"), Статус,
				РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов);
		КонецЕсли;
	Иначе
		СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
