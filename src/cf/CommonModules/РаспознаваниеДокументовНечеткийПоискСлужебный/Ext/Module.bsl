		...
		Если ТипЗнч(Отбор) = Тип("Массив") Тогда
			Для Каждого Условие Из Отбор Цикл
				...
				ИначеЕсли ТипЗнч(Условие) = Тип("КлючИЗначение") Тогда
					...
					ТребуемоеЗначение = Условие.Значение;
				Иначе
					ВызватьИсключение НСтр("ru = 'Неверный тип значения параметра Отбор, ожидалось <Структура> или <КлючИЗначение>'; en = 'Incorrect type of the Отбор parameter value. The expected type is <Структура> or <КлючИЗначение>'");
				КонецЕсли;
...
// Получает коэффициент схожести Джаро-Винклера двух текстов
// 
// Параметры:
//  Строка1 		- Строка
//  Строка2 		- Строка
//  Переворачивать 	- Булево 
// Возвращаемое значение:
//  Число
//
Функция СравнитьСтрокиПоДжароВинклеру(Знач Строка1, Знач Строка2, Переворачивать = Истина) Экспорт
	
	Коэффициент = 0;
	МассивСлов1 = СтрРазделить(Строка1, " "); 
	МассивСлов2 = СтрРазделить(Строка2, " ");
	
	Если Переворачивать И МассивСлов2.Количество() > МассивСлов1.Количество() Тогда
		МассивСлов1 = СтрРазделить(Строка2, " ");
		МассивСлов2 = СтрРазделить(Строка1, " ");
	КонецЕсли;
	
	КоличествоСлов = 0;
	Для Инд1 = 0 По МассивСлов1.Количество()-1 Цикл
		Слово1 =  МассивСлов1.Получить(Инд1);
		Если СтрДлина(Слово1) = 1 Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Инд2 = 0 По МассивСлов2.Количество()-1 Цикл
			Слово2 =  МассивСлов2.Получить(Инд2); 
				Если СтрДлина(Слово2) = 1 Тогда
					Продолжить;
				КонецЕсли;
				КоэффициентВременный = СходствоДжароВинклера(Слово1,Слово2);
	...
	Результат = Коэффициент / КоличествоСлов;

	Возврат Результат;
		
КонецФункции

// Удаляет из строки лишние символы
// 
// Параметры:
//  Строка	- Строка
// Возвращаемое значение:
//  Строка
//
Функция ОчиститьСтрокуОтМусора(Знач Строка) Экспорт
	
	Строка = СтрЗаменить(Строка, Символы.ПС, "");
	
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(Строка);
	МассивСтрок = ОчиститьСтрокиОтМусора(МассивСтрок, 3);
	НоваяСтрока = МассивСтрок[0];
	Пока СтрНайти(НоваяСтрока, "  ")>0 Цикл
...
// Получает коэффициент схожести Джаро-Винклера двух слов
// 
// Параметры:
//  Строка1 		- Строка
//  Строка2 		- Строка
...
//  Число
//
Функция СходствоДжароВинклера(Знач Строка1, Знач Строка2) Экспорт

	Строка1 = ОчиститьСтрокуОтМусора(Строка1);
	Строка2 = ОчиститьСтрокуОтМусора(Строка2); 
	
	с1 = нрег(СокрЛП(Строка1));
	с2 = нрег(СокрЛП(Строка2));
	...
	Если СтрДлина(с2) > СтрДлина(с1) Тогда
		Буфер = с1;
		с1    = с2;
		с2    = Буфер;
	КонецЕсли;
	...
	Длина1  = СтрДлина(с1);
	Длина2  = СтрДлина(с2);
	Если Длина1=0 Или Длина2=0 Тогда
		Возврат 0;
	КонецЕсли;
	
	ДопРасстояние  = Цел(Макс(Длина1,Длина2)/2)-1; // Допустимое расстояние

	КолПодходСимв  = 0; //- количество подходящих символов
	Транспозиции = 0; //- количество транспозиций 
	ПоловинаТранспозиций  = 0; //- половина числа транспозиций	
	ОбщийПрефикс  = 0; //- длина общего префикса от начала строки до максимума 4-х символов
	
	Масштаб  = 0.1; // - постоянный коэффициент масштабирования
	ПорогУсиления = 0.7; // - "порог усиления" применения префиксного бонуса в реализации Винклера	
	
	дж = 0; //- расстояние Джаро
	дв = 0; //- результат Джаро - Винклера
	
	бф = Новый Соответствие; // буфер для хранения позиций найденных символов
	
	СимволыПервойСтроки = Новый Массив(Длина1);
	Для сч = 0 По СимволыПервойСтроки.ВГраница() Цикл
		СимволыПервойСтроки[сч] = 0;	
	КонецЦикла;
	СимволыВторойСтроки = Новый Массив(Длина2);
	Для сч = 0 По СимволыВторойСтроки.ВГраница() Цикл
		СимволыВторойСтроки[сч] = 0;	
	КонецЦикла;
	
	// поиск соответствия
	Для Инд1 = 1 По Длина1 Цикл
		Для Инд2 = Макс(1, Инд1 - ДопРасстояние) По Мин(Длина2, Инд1 + ДопРасстояние) Цикл
			Если Сред(с1,Инд1,1) = Сред(с2,Инд2,1) И СимволыВторойСтроки[Инд2-1] = 0 Тогда
				СимволыПервойСтроки[Инд1-1] = 1;
				СимволыВторойСтроки[Инд2-1] = 1;
				КолПодходСимв = КолПодходСимв + 1;
				Прервать;
	...
	// расчет коэффициента 
	Если КолПодходСимв > 0 Тогда
		Поз = 0;
		Для Индекс = 0 По СимволыПервойСтроки.ВГраница() Цикл  
			Если СимволыПервойСтроки[Индекс] = 1 Тогда
				Пока  СимволыВторойСтроки[Поз] = 0 Цикл
					Поз = Поз + 1;
				КонецЦикла;
				
				Если Сред(с1,Индекс+1,1) <> Сред(с2,Поз+1,1) Тогда
					Транспозиции = Транспозиции + 1;
				КонецЕсли;
				Поз = поз + 1;
			КонецЕсли;
		КонецЦикла;
		
		ПоловинаТранспозиций = Цел(Транспозиции/2); // половина количества транспозиций
		
		дж = (КолПодходСимв/Длина1 + КолПодходСимв/Длина2 + (КолПодходСимв-ПоловинаТранспозиций)/КолПодходСимв)/3; // Расстояние Джаро
		
		Если дж >= ПорогУсиления Тогда
			Для Индекс = 1 По Мин(Длина1,Длина2) Цикл
				Если Сред(с1,Индекс,1) = Сред(с2,Индекс,1) Тогда
					ОбщийПрефикс = ОбщийПрефикс+1;
				Иначе
					Прервать;
				КонецЕсли;
			КонецЦикла; 
			ОбщийПрефикс = мин(4, ОбщийПрефикс);
			
			дв = дж + (ОбщийПрефикс * Масштаб * (1-дж)); // Расстояние Джаро-Винклера
		Иначе
			дв = дж; // дж < ПорогУсиления - меньше "порога усиления" применения префиксного бонуса
		КонецЕсли;
...
Функция ОчиститьСтрокиОтМусора(МассивСтрок, МинимальнаяДлина = 2)
	
	МассивШаблонов  = МассивШаблоновМусора();
	
	Для Инд1 = 0 По МассивСтрок.Количество() - 1 Цикл
		
		ТекущаяСтрока = нрег(МассивСтрок.Получить(Инд1));
		...
		МассивПодстрок = СтрРазделить(ТекущаяСтрока, " ");
		НовыйМассивПодстрок = Новый Массив;
		Для Каждого Элемент Из МассивПодстрок Цикл
			Если СтрДлина(Элемент) >= МинимальнаяДлина Тогда
				НовыйМассивПодстрок.Добавить(Элемент);
			КонецЕсли;
		КонецЦикла;
		ТекущаяСтрока = СтрСоединить(НовыйМассивПодстрок, " ");
...
Функция МассивШаблоновМусора()
	
	МассивШаблонов = Новый Массив;
	МассивШаблонов.Добавить("1234567890%*()_=\/@$#$^&*""'{}[]:;-+");
