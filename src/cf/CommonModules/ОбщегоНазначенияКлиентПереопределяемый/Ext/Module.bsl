...
Процедура ПередНачаломРаботыСистемы(Параметры) Экспорт
	...
	Если НЕ уатОбщегоНазначения.КонтрольМинимальногоНабораРолейДляЗапускаСистемы() Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Недостаточно прав для запуска системы!"
"В составе ролей пользователя должна входить"
"роль ""Базовые права"" и одна из ролей подсистемы УАТ."
"Обратитесь к администратору базы данных.'; en = 'Insufficient rights to start the system!"
"The user''s roles must include the ""Basic Rights"" role and one of the UAT subsystem roles."
"Contact your database administrator.'"));
		Параметры.Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	уатОбщегоНазначенияКлиент.УстановитьСоставНачальнойСтраницы();
	уатОбщегоНазначенияКлиент.УстановитьРежимОсновногоОкна();
	ItobЦентрСпутниковогоМониторингаКлиент.ПередНачаломРаботыСистемы(Параметры);
...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	// Необходимо для изменения заголовка при запуске конфигурации ТЛЭ КОРП в варианте поставки ПРОФ
	...
	//	уатОбщегоНазначенияКлиент.УстановкаРасширенияРаботыСФайламиДляВебКлиента(); 
	//#КонецЕсли
	
	уатОбщегоНазначенияКлиент.ОткрытьФормуПервоначальныхНастроекПользователя(ДопПараметры);
	уатОбщегоНазначенияКлиент.ОткрытьТерминалПЛПриНачалеРаботыСистемы(ДопПараметры);
	
	ItobЦентрСпутниковогоМониторингаКлиент.ПриНачалеРаботыСистемы(Параметры);
...
Процедура ПередПериодическойОтправкойДанныхКлиентаНаСервер(Параметры) Экспорт
	...
	пр_ПоддержкаОтраслевыхРешенийКлиент.ПередПериодическойОтправкойДанныхКлиентаНаСервер(Параметры);
...
Процедура ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты) Экспорт
	...
	// {УАТ}
	// ПоддержкаОтраслевыхРешений
	пр_ПоддержкаОтраслевыхРешенийКлиент.ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты);
	// Конец ПоддержкаОтраслевыхРешений
	// {/УАТ}
