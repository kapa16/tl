...
//  * Успех - Булево
Функция ПроверитьКвалифицированныйСертификат(Сертификат, КонтекстПроверки) Экспорт
...
// Параметры:
//  СвойствоСертификата - СертификатКриптографии
//  ДвоичныеДанныеСертификата - ДвоичныеДанные
//
// Возвращаемое значение:
//  Дата
Функция ДатаОкончанияЗакрытогоКлюча(СвойствоСертификата, ДвоичныеДанныеСертификата) Экспорт
	
	ДополнительныеСвойстваСертификата = ДополнительныеСвойстваСертификата(
		СвойствоСертификата, ДвоичныеДанныеСертификата, Новый Соответствие());
	
	ДатаОкончанияЗакрытогоКлюча = ДополнительныеСвойстваСертификата.ДатаОкончанияЗакрытогоКлюча;
	
	Возврат ДатаОкончанияЗакрытогоКлюча;
...
Функция КлючЭлектроннойПодписиДействуетНаДату(Сертификат, ДатаПроверки, Контекст)
	...
	Если ЗначениеЗаполнено(ПериодКлюча.ДатаНачала) И ЗначениеЗаполнено(ПериодКлюча.ДатаОкончания)
		Или СертификатВыданПосле(Сертификат, Дата(2024, 09, 01), Контекст) Тогда // Приказ ФСБ устанавливающий обязательность свойств сертификата
...
Функция ДанныеУдостоверяющегоЦентра(Сертификат, Кеш)
	...
	Если ДанныеУдостоверяющегоЦентра = Неопределено Тогда
		// АПК:277-выкл Не проверять вызов служебной процедуры или функции другой подсистемы.
...
Функция ИздательГосударственныйУЦ(Сертификат, Контекст)
	ДанныеУЦ = ДанныеУдостоверяющегоЦентра(Сертификат, Контекст.Кеш);
	Возврат ДанныеУЦ <> Неопределено И ДанныеУЦ.Государственный;
КонецФункции
...
// Параметры:
//  Сертификат - См. ПроверитьКвалифицированныйСертификат.Сертификат
//  Контекст - Структура:
//  * ДостовернаяДатаПодписания - Дата
//  * ДатаПроверки - Дата
//  * РезультатПроверкиСредствамиКриптографии - Неопределено,Булево
//  * Подпись - ДвоичныеДанные
//  * ДвоичныеДанныеСертификата - ДвоичныеДанные
//  * Кеш - Соответствие из КлючИЗначение:
//    ** Ключ - Строка
...
Функция ЭтоСертификатСотрудника(Сертификат, Контекст)
	СвойстваСубъекта = СвойстваСубъектаСертификата(Сертификат, Контекст.Кеш);
	Возврат (ЗначениеЗаполнено(СвойстваСубъекта.ОГРН) Или ЗначениеЗаполнено(СвойстваСубъекта.ОГРНИП)) И ЗначениеЗаполнено(СвойстваСубъекта.Фамилия)
		И Не ИздательГосударственныйУЦ(Сертификат, Контекст)
		И Не РазрешеноПодписаниеТестовымСертификатом(Сертификат, Контекст.Кеш);
КонецФункции
...
Функция СертификатВыданПосле(Сертификат, Дата, Контекст)
	ДополнительныеСвойстваСертификата = ДополнительныеСвойстваСертификата(Сертификат, Контекст.ДвоичныеДанныеСертификата, Контекст.Кеш);
	Возврат ЗначениеЛежитНаПромежутке(ДополнительныеСвойстваСертификата.ДатаНачала, Отрезок(Дата, Неопределено));
КонецФункции
...
// Параметры:
//  Сертификат - См. ПроверитьКвалифицированныйСертификат.Сертификат
//  КонтекстПроверки - См. ПроверитьКвалифицированныйСертификат.КонтекстПроверки
// 
// Возвращаемое значение:
//  Структура:
// * Контекст - Структура:
// ** ДостовернаяДатаПодписания - Дата - Если проверка в контексте подписи и в подписи есть метка времени
//                              - Неопределено - Если проверка вне контекста подписи или в подписи нет метки времени
// ** ДатаПроверки - Дата - ДостовернаяДатаПодписания если в подписи есть метка времени или текущая дата сеанса 
...
// ** ФизическогоЛица - Структура - пустая структура, если сертификат не является сертификатом ФЛ
//                    - Структура:
...
//  * ИдентификаторПодписи - Неопределено,УникальныйИдентификатор
//  * СвойстваПодписиТЭДО - Неопределено,
//                        - см. ТрансграничныйЭДО.СвойстваПодписи
//               
Функция НовыеСвойстваПодписи() Экспорт
	...
	СвойстваПодписи.Вставить("ИдентификаторПодписи", Неопределено);
	СвойстваПодписи.Вставить("СвойстваПодписиТЭДО", Неопределено);
...
Функция ВидОшибкиКриптография() Экспорт
	...
	ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Выполните</a> диагностику криптографии'"), "Выполните");
...
Процедура ЗаполнитьОписаниеОшибкиПроваленнымиКритериями(ЧастиТекстаОшибки, Сертификат, Протокол,
	...
	// АПК:1299-выкл отключение проверки локализуемой строки на непечатаемые символы
	...
	Если Не Критерии.Общие.СертификатДействуетНаДатуПроверки Тогда
		ЧастиТекстаОшибки.Добавить(НСтр("ru=' • Требуемый сертификат не прошел проверки по сроку действия при сверке с системными часами или временем подписи в файле'"));
	КонецЕсли;

	Если Не Критерии.Общие.КлючЭлектроннойПодписиДействуетНаДатуПроверки Тогда
		ПериодКлюча = ПериодДействияКлючаЭлектроннойПодписи(Сертификат, Протокол.Контекст);
		Если ЗначениеЗаполнено(ПериодКлюча.ДатаНачала) И ЗначениеЗаполнено(ПериодКлюча.ДатаОкончания) Тогда
			ЧастиТекстаОшибки.Добавить(СтрШаблон(НСтр("ru=' • ключ электронной подписи не действует на дату %1'"),
			ФразаДатыПроверкиВВинительномПадеже));
		Иначе
			ЧастиТекстаОшибки.Добавить(НСтр("ru=' • в сертификате отсутствуют обязательные сведения о сроке действия ключа'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Критерии.СертификатаСотрудника) Тогда
		Если Не Критерии.СертификатаСотрудника.ВыданДо31082023 Тогда
			ЧастиТекстаОшибки.Добавить(НСтр("ru=' • Сертификат не действует, так как выдан коммерческим аккредитованным удостоверяющим центром после 31.08.2023'"));
		КонецЕсли;
...
Функция ДополнительныеСвойстваСертификата(Сертификат, ДвоичныеДанныеСертификата, Кеш) Экспорт
...
// Возвращаемое значение:
//  Булево
Функция РазрешеноПодписаниеТестовымСертификатом(Сертификат, Кеш)
	
	Если Не ЭтоТестовыйСертификат(Сертификат, Кеш) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПользовательскиеНастройкиСертификата = ИнтеграцияБСПБЭДВызовСервера.ПользовательскиеНастройкиСертификата(Сертификат.Отпечаток);
	Возврат ПользовательскиеНастройкиСертификата.ПодписаниеРазрешено = Истина;
	
КонецФункции

// Параметры:
//  Сертификат - см. РазрешеноПодписаниеТестовымСертификатом.Сертификат
//  Кеш - см. РазрешеноПодписаниеТестовымСертификатом.Кеш
// 
// Возвращаемое значение:
//  Булево
Функция ЭтоТестовыйСертификат(Сертификат, Кеш)
	
	СвойстваСубъектаСертификата = СвойстваСубъектаСертификата(Сертификат, Кеш);
	
	Если ЗначениеЗаполнено(СвойстваСубъектаСертификата.ИННЮЛ) Тогда
		ИНН = СвойстваСубъектаСертификата.ИННЮЛ;
	ИначеЕсли ЗначениеЗаполнено(СвойстваСубъектаСертификата.ИНН) Тогда 
		ИНН = СвойстваСубъектаСертификата.ИНН;
	Иначе
		ИНН = "";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СвойстваСубъектаСертификата.ОбщееИмя) Тогда
		ОбщееИмя = НРег(СвойстваСубъектаСертификата.ОбщееИмя);
	Иначе
		ОбщееИмя = "";
	КонецЕсли;
	
	Возврат СтрНачинаетсяС(ИНН, "96")
		Или СтрНайти(ОбщееИмя, "_тест_")
		Или СтрНайти(ОбщееИмя, "_test_");
