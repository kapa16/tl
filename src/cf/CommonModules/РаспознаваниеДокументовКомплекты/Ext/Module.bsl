...
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура - основной реквизит формы документа с типом ДанныеФормыСтруктура или подходящая Структура
// 
// Возвращаемое значение:
//  Число - Статус документа числом
//
...
Процедура ОбновитьИЗаполнитьСвязанныеИСозданныеДокументы(ПараметрыСоздания, КоличествоСвязанных = 0) Экспорт
	...
	Для Каждого ДанныеТипа Из ПараметрыСоздания.РаспознанныеДокументыПоТипам Цикл
		...
		Для Каждого Связанный Из Связанные Цикл
			...
			Если Не НовыйСозданный.СканУжеПрикреплен И Не НужноПрикрепитьДополнительныйСкан(
...
Процедура ДополнитьДанныеДокументовПоСтроке(СтрокаТаблицы) Экспорт
	...
	СтрокаТаблицы.Наименование = СтрШаблон(НСтр("ru = '%1 № %2'; en = '%1 No. %2'"), СтрокаТаблицы.ТипДокумента, СтрокаТаблицы.НомерДокумента);
...
Функция НайтиВсеВозможныеКомплекты(ВсеДанныеДокументов, ДанныеОбязательногоДокумента = Неопределено)
	...
	Для Каждого СтрокаТаблицыДокументовИТипов Из ВсеДанныеДокументов.ТаблицыДокументовИТипов Цикл
		...
		// этот документ обязательно должен входить в группу, а значит в него не могут входить документы того же типа.
			...
			Если МассивИд.Количество() <> 0 Тогда
				...
				Пока ИдРасшГруппы >= 0 Цикл
					...
					Если ИдРасшГруппы = -1 Тогда
						// Когда прошли все расширенные группы 1 нужно сделать то же самое с исходной группой 1
...
Процедура ДобавитьКомплект(ТаблицаДокументов, ИдентификаторыГрупп, ИдентификаторыСтрок, ТаблицаРезультата)
	...
	КоличествоСтрок = ИдентификаторыСтрок.Количество();
	...
	СтрокаРезультата.ДокументовВКомплекте = КоличествоСтрок;
...
Функция НайтиГруппыДляКомплектов(РазмерГруппы, ТаблицаДокументов, ТаблицаПоТипам, ИдТекущегоДокумента)
	...
	ИдТаблицыТипов = Новый Массив(К); // Текущий элемент обхода для ЭлементыТипа. Содержит индекс во внутреннем массиве из ЭлементыТипа
...
Функция СледующаяКомбинацияТаблицыТипов(ИдТаблицыТипов, ТаблицыТипа, К)
	...
	Если Ид < 0 Тогда
		// Перебрали все элементы, т.е. достигли того, что все ИдТаблицыТипов[i] = 0
		Возврат Ложь;
	КонецЕсли;
...
Функция СледующаяКомбинация(Комбинация, Н, К);
	...
	Если Ид < 0 Тогда
		// Перебрали все элементы, т.е. достигли Комбинация[0] = Н - К
		Возврат Ложь;
	КонецЕсли;
...
Функция НужноПрикрепитьДополнительныйСкан(ТипКомплекта, ТипРаспознанного, ТипСозданного) Экспорт
	...
	Если ТипКомплекта = "АктОбОказанииУслугСчетНаОплатуТОРГ12"
		...
		Если ТипРаспознанного = Перечисления.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг
			И (РаспознаваниеДокументовТипы.ЭтоПоступлениеТоваровУслуг(ТипСозданного)
				Или РаспознаваниеДокументовТипы.ЭтоРеализацияТоваровУслуг(ТипСозданного)) Тогда
