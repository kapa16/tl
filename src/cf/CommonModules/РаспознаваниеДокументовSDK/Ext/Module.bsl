		...
		ВызватьИсключение(
			РаспознаваниеДокументовHTTP.КакИсключение(Ответ,
				НСтр("ru = 'Ошибка при чтении свойств распознанного документа';
				     |en = 'Error reading properties of the recognized document'")
			), , , ,
			ИнформацияОбОшибке()
		);
	КонецПопытки;
...
Процедура ЗаполнитьЗначенияПоСоответствию(Приемник, Источник, СписокСвойств = "", ИсключаяСвойства = "")
	...
	Если ЗначениеЗаполнено(СписокСвойств) Тогда
		ВызватьИсключение НСтр("ru = 'СписокСвойств пока не поддерживается'; en = 'List of properties is not yet supported'");
	КонецЕсли;
