...
Процедура ДеревоОтбораПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если ТекущиеДанные <>  Неопределено Тогда
		...
		РазрешеноУдаление = Не Элемент.ТекущиеДанные.ДоступенСписок
			И Не ТекущиеДанные.Тип.СодержитТип(Тип("Число"));
...
Процедура ПослеОтраженияПерезаполненияОбъектовУчета(ОповещениеОЗавершении, ТипДокумента, ОбъектыУчета, КонтекстДиагностики)
	...
	Если ЭлектроннаяПодписьКлиент.ПроверятьЭлектронныеПодписиНаСервере() Тогда
		ТребованияПроверкиМЧДНаКлиенте = МашиночитаемыеДоверенностиВызовСервера.ТребованияПроверкиМЧДНаКлиенте(ОбъектыУчета);
	Иначе
		ТребованияПроверкиМЧДНаКлиенте = Новый Соответствие;
		Для Каждого Доверенность Из ОбъектыУчета Цикл
			ТребованияПроверкиМЧДНаКлиенте.Вставить(Доверенность, Истина);
		КонецЦикла;
	КонецЕсли;
	Для Каждого ОбъектУчета Из ОбъектыУчета Цикл
		...
		ПараметрыПроверкиМЧД.Вставить("ТребуетсяПроверкаМЧДНаКлиенте", ТребованияПроверкиМЧДНаКлиенте.Получить(ОбъектУчета));
...
Процедура ОтменитьМЧД(
	...
	ДополнительныеПараметры.Вставить("ФормаДоверенности", 		 ФормаДоверенности);
	ДополнительныеПараметры.Вставить("Результат", 				 Результат);
	ДополнительныеПараметры.Вставить("УчитыватьСНИЛСДоверителя", Ложь);
...
Процедура ВыполнитьПроверкуПодписейМЧД(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОжидания.ОповещениеОПрогрессеВыполнения = КонтекстОжиданияОперации.ОбработчикОбновленияПрогресса;
...
Процедура ПодписатьПослеПолученияОтпечатков(ОтпечаткиСертификатов, КонтекстПодписания) Экспорт
	...
	УчитыватьСНИЛСДоверителя =
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КонтекстПодписания.ОповещениеОЗавершении.ДополнительныеПараметры,
		"УчитыватьСНИЛСДоверителя", Истина);
	
	ДанныеДляПодписания = МашиночитаемыеДоверенностиВызовСервера.ДанныеДляПодписания(КонтекстПодписания.Доверенности,
		ОтпечаткиСертификатов, КонтекстПодписания.КонтекстДиагностики, УчитыватьСНИЛСДоверителя);
...
// Проверить заполнение правил проверки полномочий.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Булево - Проверить заполнение правил проверки полномочий
//
Функция ПроверитьЗаполнениеПравилПроверкиПолномочий(ДеревоОтбора, Скрипт) Экспорт
	...
	Для Каждого ТекущаяСтрокаДерева Из ДеревоОтбора.ПолучитьЭлементы() Цикл 
		
		Если Не ТекущаяСтрокаДерева.ДоступенСписок Тогда
			
			Если Макс(ТекущаяСтрокаДерева.НачальноеЗначение,
				ТекущаяСтрокаДерева.КонечноеЗначение, 0) > 0 Тогда 
				Результат = Истина;
				Прервать;
			КонецЕсли;
...
// Параметры:
//  Элемент - ПолеФормы
//  Поля - ДанныеФормыЭлементКоллекции
Процедура СброситьОформлениеИОчиститьПоляДокументаУдостоверяющегоЛичность(Элемент, Поля) Экспорт
	
	МашиночитаемыеДоверенностиКлиентСервер.СброситьОформлениеЭлементовФормы(Элемент.Родитель, Истина);
	
	Если Не ЗначениеЗаполнено(Поля.КодВидДок) Тогда
		ОписаниеПолейПоУмолчанию = НовыйОписаниеПолейДокументаУдостоверяющегоЛичностьФизическогоЛица();
		ЗаполнитьЗначенияСвойств(Поля, ОписаниеПолейПоУмолчанию);
	КонецЕсли;
