...
Процедура УстановитьТекущийШаг(ДокументОбъект) Экспорт
	...
	СоответствиеТитулов = ОбменСГИСЭПДКлиентСервер.СоответствиеТитуловИШагов(ДокументОбъект.Ссылка);
...
Функция ДвоичныеДанныеДокументаЭПД(Объект, Данные = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Для НомерСтроки = 1 По КоличествоСтрокМакета Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеРеквизита) Или МассивВозможныхПустыхЗначений.Найти(Узел) <> Неопределено Тогда	
			...
			Исключение
				Результат.Ошибка = "Значение """ + ЗначениеРеквизита 
					+ """ не соответствует ограничению типа реквизита """ + СтрСоединить(МассивИменУзлов, "/") + """";	
				Возврат Результат;
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
	...
	Исключение
		...
		Если ЗначениеЗаполнено(ИмяРеквизитаПоТекстуОшибки) Тогда
			Если СтрЗаканчиваетсяНа(ИмяРеквизитаПоТекстуОшибки, "ИдентификаторФайла") Тогда
				Результат.Ошибка = "Необходимо записать документ перед отправкой.
					|Отправить документ можно из формы документа по кнопке ""Отправить""";	
			Иначе
...
// Вызывается перед записью сообщения ЭДО. См. ЭлектронныеДокументыЭДО.СоздатьСообщение.
//
// Параметры:
...
Процедура ПередЗаписьюВходящегоСообщенияЭДО(СообщениеОбъект, ОписаниеСообщения, ЭлектронныйДокумент, НастройкиДокумента) Экспорт
	...
	Если СообщениеОбъект.Направление <> Перечисления.НаправленияЭДО.Входящий
		Или Не (ЭтоТитулЭПД(ОписаниеСообщения.Данные.Документ.ИмяФайла) Или ЭтоУОУ(ОписаниеСообщения.ТипЭлементаРегламента)) Тогда
		Возврат;	
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПередЗаписьюВходящегоСообщенияЭДО(СообщениеОбъект, ОписаниеСообщения,
			ЭлектронныйДокумент, НастройкиДокумента);
	КонецЕсли;
	
КонецПроцедуры

// Исключает из списка исходящих документов, полученных из сервиса ЭДО, записи с документами.
// которые не нужно загружать.
//
// Параметры:
//  СтруктураСоСпискомВходящих 	- Структура - Входящие документы, см. СервисЭДО.НовыйРезультатПолученияСпискаДокументов.
//  СтруктураСоСпискомИсходящих - Структура - Исходящие документы, см. СервисЭДО.НовыйРезультатПолученияСпискаДокументов.
...
Процедура ПриПолученииСпискаИсходящихДокументовСервисаЭДО(СтруктураСоСпискомВходящих, СтруктураСоСпискомИсходящих, ИдентификаторУчетнойЗаписи) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПриПолученииСпискаИсходящихДокументовСервисаЭДО(
			СтруктураСоСпискомВходящих, СтруктураСоСпискомИсходящих, ИдентификаторУчетнойЗаписи);
	КонецЕсли;
	
КонецПроцедуры

// Вызывается при формировании списка документов из ответа от сервиса ЭДО.
//
// Параметры:
//  РезультатПолученияСпискаДокументов	 - Струкутра - см. СервисЭДО.НовыйРезультатПолученияСпискаДокументов.
//  ВидСервисаЭДО						 - Неопределено, ПеречислениеСсылка.ВидыСервисовЭДО
// 
// Возвращаемое значение:
//  Булево - Признак того, что необходимо дополнять таблицу данными ЭПД.
//
Функция ПриФормированииСпискаДокументовИзОтветаОтСервиса(РезультатПолученияСпискаДокументов, ВидСервисаЭДО) Экспорт
	
	Если Не ЭтоВидСервисаЭПД(ВидСервисаЭДО) Тогда
		Возврат Ложь;
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
    КонецЕсли;
...
// Параметры:
//  ЧтениеXML	 - ЧтениеXML
//  СтрТЗ		 - СтрокаТаблицыЗначений - см. СервисЭДО.НовыйРезультатПолученияСпискаДокументов
//
Процедура ЗаполнитьВСтрокуСпискаДокументовДополнительныеДанныеЭПД(ЧтениеXML, СтрТЗ) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ЗаполнитьВСтрокуСпискаДокументовДополнительныеДанныеЭПД(ЧтениеXML, СтрТЗ);
	КонецЕсли;
	
КонецПроцедуры

// Вызывается при исключении загруженных контейнеров. Используется в СинхронизацияЭДОСлужебный.ПолучитьОписаниеДокументов.
//
// Параметры:
...
//
Процедура ПослеИсключенияКонтейнеровИзСпискаДокументовСервисаЭДО(ДокументыДляЗагрузки, ДокументыВСервисе,
		СпискиДокументовЭПД, КлючСинхронизации) Экспорт

	Если СпискиДокументовЭПД = Неопределено
		Или КлючСинхронизации.СпособОбмена <> Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПослеИсключенияКонтейнеровИзСпискаДокументовСервисаЭДО(
			ДокументыДляЗагрузки, ДокументыВСервисе, СпискиДокументовЭПД);
	КонецЕсли;
	
КонецПроцедуры

// Возвращает признак того, что переданный вид сервиса, является сервисом ЭПД.
//
// Параметры:
//  ВидСервисаЭДО - ПеречислениеСсылка.ВидыСервисовЭДО
// 
// Возвращаемое значение:
//  Булево - признак того, что переданный вид сервиса, является сервисом ЭПД.
//
Функция ЭтоВидСервисаЭПД(ВидСервисаЭДО) Экспорт
	
	Возврат ВидСервисаЭДО = Перечисления.ВидыСервисовЭДО.ЭПД;
...
Процедура ПослеИнициализацииОсновныхПараметровВходящегоКонтейнера(ПараметрыКонтейнера, ОписаниеКонтейнера) Экспорт
	...
	Если Не ЭтоВидСервисаЭПД(ПараметрыКонтейнера.ВидСервисаЭДО) Тогда
		Возврат;
	КонецЕсли;	
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПослеИнициализацииОсновныхПараметровВходящегоКонтейнера(ПараметрыКонтейнера,
			ОписаниеКонтейнера);
	КонецЕсли;

КонецПроцедуры

// Вызывается при заполнении параметров входящего транспортного контейнера по данным файла.
// 
// Параметры:
// 	ПараметрыКонтейнера - см. ТранспортныеКонтейнерыЭДО.ПараметрыКонтейнера
...
Процедура ПриЗаполненииПараметровВходящегоКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПриЗаполненииПараметровВходящегоКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки);
	КонецЕсли;
...
Процедура ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки, ПараметрыПриглашения, Контрагент) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПриЗаполненииКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера, РезультатРаспаковки,
			ПараметрыПриглашения, Контрагент);
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Контрагент) И 
		ПараметрыКонтейнера.Отправитель = ПараметрыКонтейнера.Получатель Тогда
		Карточка = РезультатРаспаковки.Карточка;
		
		Если ЗначениеЗаполнено(Карточка.КППКонтрагентаУточненный) Тогда
			КПП = Карточка.КППКонтрагентаУточненный;
		Иначе
			КПП = Карточка.КППКонтрагента;
		КонецЕсли;	
		
		// Поиск контрагента по приглашению
		КлючКонтрагента = ПриглашенияЭДО.НовыйКлючУчастникаЭДО();
		КлючКонтрагента.ИНН = Карточка.ИННКонтрагента;
		КлючКонтрагента.КПП = КПП;
		КлючКонтрагента.ДатаСведений = Карточка.Дата;
		СтруктураУчастникаЭДО = ПриглашенияЭДОПовтИсп.УчастникЭДОПоКлючуИПриглашению(ПараметрыПриглашения, КлючКонтрагента);
		Контрагент = СтруктураУчастникаЭДО.Ссылка;
		
		// Если контрагент не найден по приглашению
...
Процедура ПриЗаполненииВходящегоКонтейнера(ПараметрыКонтейнера, ДанныеЗаполнения) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭПД.ПриЗаполненииВходящегоКонтейнера(ПараметрыКонтейнера, ДанныеЗаполнения);
	КонецЕсли;

КонецПроцедуры

// Вызывается при получении подписей в легком интерфейсе.
// см. ЛегкийИнтерфейсДокументовЭДОСлужебный.ОписанияУстановленныхПодписейДокумента.
//
// Параметры:
//  СвойстваСообщений - см. ЭлектронныеДокументыЭДО.СвойстваСообщенийПросмотраКарточкиДокумента
// 
// Возвращаемое значение:
//  Неопределено,  Массив из Структура - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеДанныеПодписиСУчетомДоверенности 
//
//@skip-check doc-comment-description-ends-on-dot
//@skip-check function-return-types
Функция ПриПолученииПодписейВЛегкомИнтерфейсе(СвойстваСообщений) Экспорт
	
	УстановленныеПодписи = Неопределено;
	
	ТипыЭлементовРегламентаЭПД = ТипыЭлементовРегламентаЭПД();
	ЭтоПодписиЭПД = СвойстваСообщений.Количество() <> 0
		И ТипыЭлементовРегламентаЭПД.Найти(СвойстваСообщений[0].ТипЭлементаРегламента) <> Неопределено;
	
	Если ЭтоПодписиЭПД Тогда
		УстановленныеПодписи = Новый Массив;
		
		Для Каждого СвойстваСообщения Из СвойстваСообщений Цикл
			МассивПодписей = ЭлектронныеДокументыЭДО.УстановленныеПодписиСУчетомДоверенностей(СвойстваСообщения.Ссылка);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(УстановленныеПодписи, МассивПодписей);
		КонецЦикла;
	КонецЕсли;
...
// Вызывается при получении свойств сообщений просмотра карточки документа.
// 
// Параметры:
//  Запрос - Запрос
Процедура ПриПолученииСвойствСообщенийПросмотраКарточкиДокумента(Запрос) Экспорт
	
	Если Запрос.Параметры.Свойство("ТипыЭлементовРегламента")
		И ТипЗнч(Запрос.Параметры.ТипыЭлементовРегламента) = Тип("Массив") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Запрос.Параметры.ТипыЭлементовРегламента, ТипыЭлементовРегламентаЭПД());
	КонецЕсли;
	
КонецПроцедуры

// Вызывается при визуализации документа. см. ВизуализацияЭДОСлужебный.ВизуализацияДокумента.
// 
// Параметры:
//  ПараметрыПолученияПредставления - Структура
//  СвойстваСообщений - ТаблицаЗначений
// 
// Возвращаемое значение:
//  Неопределено, Структура - При визуализации документа:
// * Успех - Булево - 
...
// * НастройкиПредставления - Структура - :
...
// ** ЕстьQRКод - Булево - 
// * ПользовательскиеНастройкиПредставления - Структура:
// ** ЕстьНастройкаВыводаДополнительныхДанных - Булево - 
// ** ЕстьНастройкаВыводаБанковскихРеквизитов - Булево - 
// ** ЕстьНастройкаВыводаКопияВерна - Булево - 
// * ПолученИзКэша - Булево - 
// * ТекстОшибки - Строка - 
Функция ПриВизуализацииДокумента(ПараметрыПолученияПредставления, СвойстваСообщений) Экспорт
	
	ТипыЭлементовРегламентаЭПД = ТипыЭлементовРегламентаЭПД();
	ЭтоДокументЭПД = СвойстваСообщений.Количество() <> 0
		И ТипыЭлементовРегламентаЭПД.Найти(СвойстваСообщений[0].ТипЭлементаРегламента) <> Неопределено;
	
	Если Не ЭтоДокументЭПД Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПредставлениеДанных = ВизуализацияЭДОСлужебный.НовыйРезультатПредставленияДанныхСообщения();
	
	ТабличныйДокументПредставления = Новый ТабличныйДокумент();

	Для Каждого СвойстваСообщения Из СвойстваСообщений Цикл
		
		ПараметрыПолученияПредставления.Сообщение = СвойстваСообщения.Ссылка;
		ПредставлениеДанных = ВизуализацияЭДОСлужебный.ПредставлениеДанныхСообщенияПоСсылке(ПараметрыПолученияПредставления);
		
		Если ПредставлениеДанных.Успех Тогда
			ТабличныйДокументПредставления.Вывести(ПредставлениеДанных.ПредставлениеДокумента)
		КонецЕсли;
		
	КонецЦикла;
	
	ПредставлениеДанных.ПредставлениеДокумента = ТабличныйДокументПредставления;
...
// Вызывается при распозновании документа.
// См. Обработка.ИнтерфейсДокументовЭДО.ТекущиеДелаПоЭДО.РаспознатьДокумент.
// 
// Параметры:
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО 
//  УспешностьРаспознования- Булево - Успешность распознования.
// 
// Возвращаемое значение:
//  Неопределено, Массив из Структура - Параметры файла.
// @skip-check doc-comment-description-ends-on-dot
// @skip-check function-return-types
// @skip-check doc-comment-ref-link
Функция ПриРаспознованииДокумента(ТипДокумента, УспешностьРаспознования) Экспорт 
	
	Если Не ЭтоДокументЭПД(ТипДокумента) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПараметрыФайла = Новый Структура();
	ПараметрыФайла.Вставить("ТипДокумента", ОбщегоНазначения.ИмяЗначенияПеречисления(ТипДокумента));
	ПараметрыФайла.Вставить("ОтражениеВУчете", Неопределено);
	
	УспешностьРаспознования = Истина;
...
// Адрес ресурса получения списка сообщений.
// 
// Возвращаемое значение:
//  Строка - Адрес ресурса получения списка сообщений
...
// Параметры получения списка исходящих сообщений.
// 
// Возвращаемое значение:
//  Структура - Параметры необходимые для получения списка исходящих:
//	* direction - Строка
Функция ПараметрыПолученияСпискаИсходящихСообщений() Экспорт
	
	Пераметры = Новый Структура;
	Пераметры.Вставить("direction", "outgoing");
	
	Возврат Пераметры;
	
КонецФункции

// Адрес ресурса получения сообщения.
// 
// Возвращаемое значение:
//  Строка - Адрес ресурса получения сообщения
...
// Адрес ресурса отправки сообщения.
// 
// Возвращаемое значение:
//  Строка - Адрес ресурса отправки сообщения
...
Процедура ОтметитьИсправлениеДокумента(ОбъектУчета, Отметка = Истина) Экспорт
	...
	Исправлен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "Исправлен");
	Если Исправлен = Отметка Тогда
		Возврат;
	КонецЕсли;
	...
	Для НомерСимвола = 1 По ДлинаСтроки - 2 Цикл
		...
		Если НРег(ТекущийСимвол) = ТекущийСимвол 
			И ВРег(СледующийСимвол) = СледующийСимвол Тогда
		 	Результат = Результат + " ";
		// После нескольких заглавных букв подряд идет строчная буква
...
Функция ПолучитьОписаниеОшибки(ОбъектСсылка) Экспорт
	...
	Результат = Новый Структура("КодОшибки, ОписаниеОшибки, Блокирующая", 0, "", Истина);
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОшибкиПередачиЭДО КАК ОшибкиПередачиЭДО
	...
	|ВЫБРАТЬ
	|	0,
	|	СостоянияДокументовЭДО.Комментарий
	|ИЗ
	|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументИсходящийЭДО КАК ЭлектронныйДокументИсходящийЭДО
	|		ПО ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = ЭлектронныйДокументИсходящийЭДО.Ссылка
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Если СтрНачинаетсяС(Выборка.ОписаниеОшибки, "{") Тогда
			ОписаниеОшибкиJson = ОбщегоНазначения.JSONВЗначение(Выборка.ОписаниеОшибки, , Ложь);
			Если ОписаниеОшибкиJson <> Неопределено Тогда
				Если ОписаниеОшибкиJson.Свойство("code") Тогда		
					Результат.КодОшибки = ОписаниеОшибкиJson.code;
				КонецЕсли;
				Если ОписаниеОшибкиJson.Свойство("blocking") Тогда		
					Результат.Блокирующая = ОписаниеОшибкиJson.blocking;
				КонецЕсли;
				Если ОписаниеОшибкиJson.Свойство("text") Тогда		
					Результат.ОписаниеОшибки = ОписаниеОшибкиJson.text;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи, ВидДокумента, ОтборПодписанта = Неопределено) Экспорт
	...
	ПользователиДоступныхСертификатов.Добавить(АвторизованныйПользователь);
	...
	|	ВЫБОР
	|		КОГДА СертификатыКлючейЭлектроннойПодписиИШифрования.Пользователь = &АвторизованныйПользователь
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК ПриоритетПользователя
	|ИЗ
	...
	|		ПО СертификатыУчетныхЗаписейЭДО.Сертификат = ПодписываемыеВидыЭД.СертификатЭП
	|		И ПодписываемыеВидыЭД.ВидЭД = &ВидДокумента
...
Функция ПроверитьЗаполнениеДокументаЭПД(ДанныеДляПроверки, ТекущийТитул, ВидДокумента) Экспорт
	...
	Если ТипЗнч(ВидДокумента) = Тип("Строка") Тогда
		Если ВидДокумента = "Документ.ЭлектроннаяСопроводительнаяВедомость" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ;
		ИначеЕсли ВидДокумента = "Документ.ЭлектроннаяТранспортнаяНакладная" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН;
		ИначеЕсли ВидДокумента = "Документ.ЭлектронныйДоговорФрахтования" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ;
		ИначеЕсли ВидДокумента = "Документ.ЭлектронныйЗаказЗаявка" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ;
		ИначеЕсли ВидДокумента = "Документ.ЭлектронныйЗаказНаряд" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН;
		ИначеЕсли ВидДокумента = "Документ.ЭлектронныйПутевойЛист" Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЛ;	
		КонецЕсли;	
	Иначе
		ТипДокумента = ВидДокумента.ТипДокумента;
	КонецЕсли;
...
Процедура ЗаписатьВРеестрЭПД(Данные, ЗаписьИзФормы = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		Массив = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МенеджерЗаписи);
		МодульСервисВзаимодействияМПЭПД.ДобавитьДанныеМобильногоПриложенияДляЗаписиВРеестр(Массив, ЗаписьИзФормы);
	КонецЕсли;
...
// Добавляет баннер новости о мобильном приложении.
//
// Параметры:
//  Форма	 - ФормаКлиентскогоПриложения
//
...
// Возвращает признак того, что мобильные приложения подключены по идентификатору ЭДО.
//
// Параметры:
//  ИдентификаторЭДО - Строка
// 
// Возвращаемое значение:
//  Булево - Признак того, что мобильные приложения подключены по идентификатору ЭДО.
//
Функция МобильныеПриложенияПодключеныПоИдентификаторуЭДО(ИдентификаторЭДО) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
		МодульСервисВзаимодействияМПЭПД = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		Возврат МодульСервисВзаимодействияМПЭПД.ПоУчетнойЗаписиЕстьПодключенныеМП(ИдентификаторЭДО);
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
// Заполняет данные мобильных приложений для отображаемой версии титула.
//
// Параметры:
//  Форма		 - ФормаКлиентскогоПриложения
...
Процедура ЗаполнитьВерсиюТитулаМобильногоПриложения(Форма, Знач ТекущийТитул) Экспорт
	...
	ИнформацияПоТитулуМП = МодульСервисВзаимодействияМПЭПДКлиентСервер.НастройкиОтображенияИнформацииПоМП(Форма,
		Форма.Объект.Ссылка, Форма.Объект.РольУчастника, ТекущийТитул);
	...
	МассивТиповВзаимодействий = МодульСервисВзаимодействияМПЭПДКлиентСервер.МассивТиповВзаимодействийПривязокОтвязокИСообщений();
	...
	// Скрываются сообщения по всем титулам, если они есть на форме
	...
	ДанныеДокументовМП = Новый Соответствие;
	ДанныеМП = Новый Соответствие;
	ДанныеМП.Вставить("ТекущийТитул", ТекущийТитул);
	ПодписиСообщений = Новый Соответствие;
	
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.ЭтоСобытиеПривязкиОтвязкиПользователя Тогда
			ЭтоПривязка = МодульСервисВзаимодействияМПЭПДКлиентСервер.ЭтоПривязкаДокумента(Выборка.ВидСообщения);
			Если (Не ЭтоПривязка) Или (Не ЗначениеЗаполнено(Выборка.Идентификатор)) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		// Подготовливаются данные для разных событий (привязка МП, отвязка МП, собщение от МП и т.п.)
		ЭтоТипСообщенияОбОтправкеТитула = Ложь;
		Если Выборка.ЭтоСобытиеПривязкиОтвязкиПользователя Тогда
			ИндексРаботы = МодульСервисВзаимодействияМПЭПДКлиентСервер.ИндексРаботыПоТипуВзаимодействия(Выборка.ВидСообщения);
			Если ДанныеМП.Получить(ИндексРаботы) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
		Иначе
			// Сообщения, не содержащие данные, пропускаются
			ДанныеСообщения = ПолучитьДанныеJSON(Выборка.ДанныеСообщения);
			Если Не ЗначениеЗаполнено(Выборка.ДанныеСообщения) Тогда
				Продолжить;
			КонецЕсли;
			
			ТипСообщенияМП = ДанныеСообщения.Type;
			ЭтоТипСообщенияОбОтправкеТитула = 
				МодульСервисВзаимодействияМПЭПДКлиентСервер.ЭтоТипСообщенияОбОтправкеТитулаИзМП(ТипСообщенияМП);
			Если ЭтоТипСообщенияОбОтправкеТитула Тогда
				ТипСообщенияМП = 
					МодульСервисВзаимодействияМПЭПДКлиентСервер.ПривестиТипСообщенияКВидуПолученияТитулаБЭД(ТипСообщенияМП);
			КонецЕсли;
			
			// Отображается последнее сообщение по каждому типу, остальные пропускаются
			Если МассивПолученныхТипов.Найти(ДанныеСообщения.Type) <> Неопределено Тогда
				Продолжить;
			Иначе
				Если Не ЭтоТипСообщенияОбОтправкеТитула Тогда
					// Перед добавлением проекта титула проверяется наличие реального титула
					Если МассивПолученныхТипов.Найти(ДанныеСообщения.Type + "_titul") <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			...
			ТитулСообщения = СоответствиеТитуловТипамМП.Получить(ТипСообщенияМП);
			// Неизвестные титулы пропускаются
			Если ТитулСообщения = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ИндексРаботы = МодульСервисВзаимодействияМПЭПДКлиентСервер.ИндексРаботыПоТитулу(ТитулСообщения);
		КонецЕсли;
		
		// Запись информации о МП, информация необходима для открытия или изменения МП
		Если ЗначениеЗаполнено(Выборка.Идентификатор) Тогда
			СтруктураРаботы = МодульСервисВзаимодействияМПЭПДКлиентСервер.НоваяСтруктураДанныхПоРаботеМП();
			СтруктураРаботы.ИдентификаторМП = Выборка.ИдентификаторМП;
			СтруктураРаботы.ИдентификаторЗаписиМП = Выборка.Идентификатор;
			СтруктураРаботы.ИндексРабот = ИндексРаботы;
			СтруктураРаботы.ПолученТитулНевозможноИзменитьМП = Выборка.ЭтоСообщение 
				И ИндексРаботы <> МодульСервисВзаимодействияМПЭПДКлиентСервер.ИндексРаботыВодитель();
			ДанныеМП.Вставить(ИндексРаботы, СтруктураРаботы);
			
			// Заполнение значений реквизитов МП
			ИмяРеквизита = МодульСервисВзаимодействияМПЭПДКлиентСервер.ИмяСИндексом(ИндексРаботы, "МПНаименование");
			Форма[ИмяРеквизита] = Выборка.МобильноеПриложение;
			...
			Если ТипЗнч(Выборка.Фото) = Тип("ХранилищеЗначения") Тогда
				...
				Если ФотоЗначениеХранилища <> Неопределено Тогда
					МПФото = ПоместитьВоВременноеХранилище(ФотоЗначениеХранилища, Форма.УникальныйИдентификатор);
				КонецЕсли;
			КонецЕсли;
			
			Если МПФото = Неопределено Тогда
				МПВыбрано = Истина;
				КартинкаДвоичныеДанные = МодульСервисВзаимодействияМПЭПДКлиентСервер.КартинкаРаботПоИндексу(
					ИндексРаботы, МПВыбрано).ПолучитьДвоичныеДанные();
				МПФото = ПоместитьВоВременноеХранилище(КартинкаДвоичныеДанные, Форма.УникальныйИдентификатор);	
			КонецЕсли;
			
			ИмяРеквизита = МодульСервисВзаимодействияМПЭПДКлиентСервер.ИмяСИндексом(ИндексРаботы, "МПФото");
			Форма[ИмяРеквизита] = МПФото;
		КонецЕсли;
		
		// Далее обрабатывается отображение на форме документа сообщений, содержащих проекты титулов
		Если Не Выборка.ЭтоСообщение Или ЭтоТипСообщенияОбОтправкеТитула Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СоздатьВерсиюПользователяМП Тогда	
			...
			Если Принято = Ложь Тогда
				...
				Если ПредыдущийПарныйТитул <> Неопределено Тогда
					...
					МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Титул """ 
												+ ОбменСГИСЭПДКлиентСервер.СоответствиеТитуловИШагов().Получить(ПредыдущийПарныйТитул) 
			...
			Иначе			
				...
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Данные по титулу """ 
											+ ОбменСГИСЭПДКлиентСервер.СоответствиеТитуловИШагов().Получить(ТитулСообщения) 
	...
	Для Каждого КиЗ Из ПодписиСообщений Цикл
		...
		СтруктураПодписей.Вставить("ДвоичныеДанныеДокумента", КиЗ.Значение.ДвоичныеДанныеДокумента);
		СтруктураПодписей.Вставить("ДвоичныеДанныеПодписи", КиЗ.Значение.ДвоичныеДанныеПодписи);	
		ДанныеДокументовМП.Вставить(КиЗ.Ключ, СтруктураПодписей);
	КонецЦикла;
	
	Форма.ДанныеДокументовМП = Новый ФиксированноеСоответствие(ДанныеДокументовМП);
	Форма.ДанныеМП = Новый ФиксированноеСоответствие(ДанныеМП);
...
Функция ПолучитьРеквизитыУчастника(Участник, ДобавитьПлатежныеРеквизиты = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Участник) Тогда
		...
		СведенияОбУчастнике.Вставить("ЗначениеJSONЮридическийАдрес");
	...
	Для Каждого КиЗ Из СообщенияДокумента Цикл
		...
		Для Каждого Подпись Из ПодписиТитула Цикл
			...
			Если Подпись.Свойство("СвойстваПодписи") Тогда
				Если Не Подпись.СвойстваПодписи.Свойство("Сертификат")
					Или ТипЗнч(Подпись.СвойстваПодписи.Сертификат) <> Тип("ХранилищеЗначения") Тогда
					Подпись.СвойстваПодписи.Вставить("Сертификат", Новый ХранилищеЗначения(Неопределено));
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого ТаблицаКиЗ Из ОписаниеДанных.ДанныеЗаполненияТаблицФормы Цикл
		...
		Иначе
			...
			Для Каждого СтрокаСтруктура Из ТаблицаКиЗ.Значение Цикл
				...
				КонецЦикла;
			КонецЦикла;
			КэшТабЧасти.Удалить(0);
			Для НомерТекущейСтроки = НомерСтроки + 1 По КэшТабЧасти.Количество() Цикл
				КэшТабЧасти.Удалить(НомерТекущейСтроки);	
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьОписаниеХранимыхДанныхЭПДПоСсылке(ХранимыеДанныеЭПДСсылка, ГруппаДанных, ИдентификаторСтроки = Неопределено, ДляФормы = Истина) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		ИмяПоля = Выборка.Имя;
		Если ДляФормы = Ложь И ГруппыДанныхСПрефиксами.Найти(ГруппаДанных) <> Неопределено Тогда
	...
	Пока ВыборкаТЧ.Следующий() Цикл
		...
		Пока ВыборкаСтрока.Следующий() Цикл
			...
			Если СтруктураСтроки.Количество() > 0 Тогда
				Если ЗначениеЗаполнено(ИдентификаторСтроки) 
				И Не СтруктураСтроки.Свойство("ИдентификаторСтрокиРодителя") Тогда
					// Если выбор из таблица, но в хранимых данных на верхнем уровне нет идентификатора строки родителя
					СтруктураСтроки.Вставить("ИдентификаторСтрокиРодителя", ИдентификаторСтроки);	
				КонецЕсли;
				МассивСтрок.Добавить(СтруктураСтроки);
			КонецЕсли;
		КонецЦикла;
	...
	КонецЦикла;
	// Добавим совместимость хранимых данных между разными видами ЭПД
	Если ДанныеЗаполненияТаблицФормы.Свойство("ВодительНомераТелефонов")
		И Не ДанныеЗаполненияТаблицФормы.Свойство("ТитулФрахтовщикаКонтактыВодителяНомераТелефонов") Тогда
		МассивКонтактыВодителя = Новый Массив;
		СтруктураКонтактыВодителя = Новый Структура;
		СтруктураКонтактыВодителя.Вставить("ИдентификаторСтрокиРодителя", ИдентификаторСтроки);
		ИдентификаторСтрокиКонтактов = Строка(Новый УникальныйИдентификатор);
		СтруктураКонтактыВодителя.Вставить("ИдентификаторСтроки", ИдентификаторСтрокиКонтактов);
		МассивКонтактыВодителя.Добавить(СтруктураКонтактыВодителя);
		ДанныеЗаполненияТаблицФормы.Вставить("ТитулФрахтовщикаКонтактыВодителя", МассивКонтактыВодителя);
		
		МассивКонтактыВодителяНомераТелефонов = Новый Массив;
		Для Каждого СтрокаТелефон Из ДанныеЗаполненияТаблицФормы.ВодительНомераТелефонов Цикл
			СтруктураКонтактыВодителяНомераТелефонов = Новый Структура;
			СтруктураКонтактыВодителяНомераТелефонов.Вставить("ИдентификаторСтрокиРодителя", ИдентификаторСтрокиКонтактов);
			СтруктураКонтактыВодителяНомераТелефонов.Вставить("Телефон", СтрокаТелефон.Телефон);		
			МассивКонтактыВодителяНомераТелефонов.Добавить(СтруктураКонтактыВодителяНомераТелефонов);
			ДанныеЗаполненияТаблицФормы.Вставить("ТитулФрахтовщикаКонтактыВодителяНомераТелефонов", МассивКонтактыВодителяНомераТелефонов);
		КонецЦикла;
	ИначеЕсли Не ДанныеЗаполненияТаблицФормы.Свойство("ВодительНомераТелефонов")
		И ДанныеЗаполненияТаблицФормы.Свойство("ТитулФрахтовщикаКонтактыВодителяНомераТелефонов") Тогда
		МассивВодительНомераТелефонов = Новый Массив;
		Для Каждого СтрокаТелефон Из ДанныеЗаполненияТаблицФормы.ТитулФрахтовщикаКонтактыВодителяНомераТелефонов Цикл
			СтруктураВодительНомераТелефонов = Новый Структура;
			СтруктураВодительНомераТелефонов.Вставить("ИдентификаторСтрокиРодителя", ИдентификаторСтроки);
			СтруктураВодительНомераТелефонов.Вставить("Телефон", СтрокаТелефон.Телефон);
			МассивВодительНомераТелефонов.Добавить(СтруктураВодительНомераТелефонов);	
		КонецЦикла;
		ДанныеЗаполненияТаблицФормы.Вставить("ВодительНомераТелефонов", МассивВодительНомераТелефонов);
	КонецЕсли;
...
Функция ПолучитьТитулыПоДокументу(Документ, ЗавершенныеОтправки = Ложь, Подготовленные = Ложь) Экспорт
	...
	НуженОтборПоСтатусам = ЗавершенныеОтправки Или Подготовленные;
	МассивОтбораСтатусов = Новый Массив;
	Если НуженОтборПоСтатусам Тогда
		Если ЗавершенныеОтправки Тогда
			МассивОтбораСтатусов.Добавить(Перечисления.СтатусыСообщенийЭДО.Получен);
			МассивОтбораСтатусов.Добавить(Перечисления.СтатусыСообщенийЭДО.Отправлен);
		КонецЕсли;
		
		Если Подготовленные Тогда
			МассивОтбораСтатусов.Добавить(Перечисления.СтатусыСообщенийЭДО.ПодготовленКОтправке);
		КонецЕсли;
	КонецЕсли;
	...
	Пока ВыборкаТитул.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если МассивДобавленныхИмен.Найти(Выборка.ПолноеИмяФайла) = Неопределено Тогда
				ДобавитьСообщение = Не НуженОтборПоСтатусам
					Или МассивОтбораСтатусов.Найти(Выборка.Статус) <> Неопределено;
...
Функция ПолучитьАдресаДоставки(ЗначениеОтбора) Экспорт
	...
	КонецЕсли;
	// {УАТ} 
	Если НЕ ТипЗнч(ЗначениеОтбора) = Тип("СправочникСсылка.СтроковыеЗначенияЭПД") Тогда
...
Процедура ПриСозданииНаСервереПодчиненнойФормы(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ОписаниеРеквизитовФормы.ПараметрыФормы) Тогда
		ПараметрыФормыСтруктура = Новый Структура(ОписаниеРеквизитовФормы.ПараметрыФормы);
	Иначе
	...
	Если Форма.Параметры.Свойство("ФормаБезОбработки") = Ложь Тогда
		...
		МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("ПоказатьОшибкиЗаполнения", Новый ОписаниеТипов("Булево")));
		...
		Если ПараметрыФормыСтруктура.Свойство("Организация") = Ложь Тогда
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("Организация", ОрганизацияОписаниеТипов ));
		КонецЕсли;
		Если ПараметрыФормыСтруктура.Свойство("Контрагент") = Ложь Тогда
		...
		Если ЭтоОсновнаяФорма(Форма) И ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
			МодульСервисВзаимодействияМПЭПДКлиентСервер = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПДКлиентСервер");
			ДокументПоддерживается = МодульСервисВзаимодействияМПЭПДКлиентСервер.ДокументИРольУчастникаПоддерживаетсяМП(Форма.Объект.Ссылка);
			Если ДокументПоддерживается Тогда
				МодульСервисВзаимодействияМПЭП = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
				МодульСервисВзаимодействияМПЭП.ДобавитьРеквизитыМП(МассивРеквизитовДобавления, Форма);
				НужноДобавитьРеквизитыМП = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоОсновнаяФорма(Форма) Тогда
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("ОшибкиЗаполнения", Новый ОписаниеТипов("ТаблицаЗначений")));	
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("Описание", Новый ОписаниеТипов(), "ОшибкиЗаполнения"));
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("Текст", Новый ОписаниеТипов("Строка"), "ОшибкиЗаполнения"));
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("Узел", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(250)), "ОшибкиЗаполнения"));
			МассивРеквизитовДобавления.Добавить(Новый РеквизитФормы("Отметка", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)), "ОшибкиЗаполнения"));	
		КонецЕсли;		
	...
	Если ЭтоОсновнаяФорма(Форма) Тогда
		ДобавитьРеквизитыОшибокЗаполнения(Форма);
	КонецЕсли;
	
	Если Форма.Параметры.Свойство("ПоказатьОшибкиЗаполнения") Тогда
		Форма.ПоказатьОшибкиЗаполнения = Форма.Параметры.ПоказатьОшибкиЗаполнения;
	КонецЕсли;
	...
	Если НужноДобавитьРеквизитыМП = Истина Тогда
		МодульСервисВзаимодействияМПЭП = ОбщегоНазначения.ОбщийМодуль("СервисВзаимодействияМПЭПД");
		МодульСервисВзаимодействияМПЭП.ДобавитьЭлементыМобильныхПриложений(Форма);	
	КонецЕсли;
...
Функция ОписаниеРеквизитовФормы(Форма, ГруппаДанных = Неопределено) Экспорт
	...
	Результат.Вставить("ЕстьСтруктураРеквизитов", Ложь);
	...
	Для Каждого РеквизитФормы Из Форма.ПолучитьРеквизиты() Цикл 
		Если РеквизитФормы.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда
			...
			КонецЕсли;
		Иначе
			Если РеквизитФормы.Имя = "Объект" И РеквизитФормы.ТипЗначения.Типы().Количество() = 1 Тогда
			...
			ИначеЕсли РеквизитФормы.Имя = "СтруктураРеквизитов" Тогда
				Результат.ЕстьСтруктураРеквизитов = Истина;
...
Процедура ЗаполнитьДополнительныеРеквизитыФормы(Форма, ПрефиксТитула, КонтейнерОтображаемойВерсии = Неопределено) Экспорт
	...
	ЕстьЗначенияДопРеквизитов = Ложь;
	Для Каждого ОписаниеГруппыДопРеквизитов Из Форма.ОписаниеРеквизитовФормы.ОписаниеДопРеквизитов Цикл
		Если СтрНачинаетсяС(ОписаниеГруппыДопРеквизитов.Ключ, ПрефиксТитула) Тогда
			Если ОбменСГИСЭПДКлиентСервер.ЗагрузитьТаблицуВСтруктуруФормы(Форма, ОписаниеГруппыДопРеквизитов.Ключ + "ДополнительнаяИнформация", Истина, КонтейнерОтображаемойВерсии) Тогда
				ЕстьЗначенияДопРеквизитов = ОбменСГИСЭПДКлиентСервер.ЗагрузитьТаблицуВСтруктуруФормы(Форма, ОписаниеГруппыДопРеквизитов.Ключ + "ИнформационныеПоля", Истина, КонтейнерОтображаемойВерсии)	
			Иначе
	...
	Если ЕстьЗначенияДопРеквизитов Тогда
...
Процедура ДобавитьРеквизитыОшибокЗаполнения(Форма)
	...
	#Если НЕ ВнешнееСоединение Тогда
		...
		ГруппаОшибки = Элементы.Добавить("ГруппаОшибки", Тип("ГруппаФормы"), ЭлементРодитель);
		ГруппаОшибки.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаОшибки.Отображение = ОтображениеОбычнойГруппы.Нет;
		ГруппаОшибки.ОтображатьЗаголовок = Ложь;
		ГруппаОшибки.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаОшибки.ЦветФона = ЦветаСтиля.ЦветРазделителяГруппЭлементовЭПД;
		ГруппаОшибки.Видимость = Ложь;
		
		ГруппаШапкаОшибок = Элементы.Добавить("ГруппаШапкаОшибок", Тип("ГруппаФормы"), ГруппаОшибки);
		ГруппаШапкаОшибок.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаШапкаОшибок.Отображение = ОтображениеОбычнойГруппы.Нет;
		ГруппаШапкаОшибок.ОтображатьЗаголовок = Ложь;
		ГруппаШапкаОшибок.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		ГруппаШапкаОшибок.РастягиватьПоГоризонтали = Истина;
		
		ГруппаШапкаОшибкиЛево = Элементы.Добавить("ГруппаШапкаОшибкиЛево", Тип("ГруппаФормы"), ГруппаШапкаОшибок);
		ГруппаШапкаОшибкиЛево.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаШапкаОшибкиЛево.Отображение = ОтображениеОбычнойГруппы.Нет;
		ГруппаШапкаОшибкиЛево.ОтображатьЗаголовок = Ложь;
		ГруппаШапкаОшибкиЛево.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		ГруппаШапкаОшибкиЛево.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Лево;
		
		НадписьОшибки = Элементы.Добавить("НадписьОшибки", Тип("ДекорацияФормы"), ГруппаШапкаОшибкиЛево);
		НадписьОшибки.Вид = ВидДекорацииФормы.Надпись;
		НадписьОшибки.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Центр;
		НадписьОшибки.Шрифт = ШрифтыСтиля.ШрифтМикроЗаголовкаЭПД;
		НадписьОшибки.Заголовок = НСтр("ru='Ошибки заполнения'");
		НадписьОшибки.Высота = 2;
		НадписьОшибки.АвтоМаксимальнаяВысота = Ложь;
		НадписьОшибки.МаксимальнаяВысота = 2;
		НадписьОшибки.РастягиватьПоВертикали = Истина;
		
		ГруппаШапкаОшибкиПраво = Элементы.Добавить("ГруппаШапкаОшибкиПраво", Тип("ГруппаФормы"), ГруппаШапкаОшибок);
		ГруппаШапкаОшибкиПраво.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаШапкаОшибкиПраво.Отображение = ОтображениеОбычнойГруппы.Нет;
		ГруппаШапкаОшибкиПраво.ОтображатьЗаголовок = Ложь;
		ГруппаШапкаОшибкиПраво.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		ГруппаШапкаОшибкиПраво.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Право;
		
		НадписьОшибкиЗакрыть = Элементы.Добавить("НадписьОшибкиЗакрыть", Тип("ДекорацияФормы"), ГруппаШапкаОшибкиПраво);
		НадписьОшибкиЗакрыть.Вид = ВидДекорацииФормы.Надпись;
		НадписьОшибкиЗакрыть.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Центр;
		НадписьОшибкиЗакрыть.Шрифт = ШрифтыСтиля.ШрифтМикроЗаголовкаЭПД;
		НадписьОшибкиЗакрыть.Заголовок = НСтр("ru='Закрыть'");
		НадписьОшибкиЗакрыть.Высота = 2;
		НадписьОшибкиЗакрыть.АвтоМаксимальнаяВысота = Ложь;
		НадписьОшибкиЗакрыть.МаксимальнаяВысота = 2;
		НадписьОшибкиЗакрыть.Гиперссылка = Истина;
		НадписьОшибкиЗакрыть.РастягиватьПоВертикали = Истина;
		НадписьОшибкиЗакрыть.УстановитьДействие("Нажатие", "Подключаемый_НадписьОшибкиЗакрытьНажатие");
		
		ОшибкиЗаполнения = Элементы.Добавить("ОшибкиЗаполнения", Тип("ТаблицаФормы"), ГруппаОшибки);
		ОшибкиЗаполнения.ПутьКДанным = "ОшибкиЗаполнения";
		ОшибкиЗаполнения.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		ОшибкиЗаполнения.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
		ОшибкиЗаполнения.УстановитьДействие("Выбор", "Подключаемый_ОшибкиЗаполненияВыбор");
		
		ОшибкиЗаполненияГруппаКолонок = Элементы.Добавить("ОшибкиЗаполненияГруппаКолонок", Тип("ГруппаФормы"), ОшибкиЗаполнения);
		ОшибкиЗаполненияГруппаКолонок.Вид = ВидГруппыФормы.ГруппаКолонок;
		ОшибкиЗаполненияГруппаКолонок.Группировка = ГруппировкаКолонок.ВЯчейке;
		
		ОшибкиЗаполненияТекст = Элементы.Добавить("ОшибкиЗаполненияТекст", Тип("ПолеФормы"), ОшибкиЗаполненияГруппаКолонок);
		ОшибкиЗаполненияТекст.Вид = ВидПоляФормы.ПолеВвода;
		ОшибкиЗаполненияТекст.ПутьКДанным = "ОшибкиЗаполнения.Текст";
		...
		Если Параметры.Свойство("ДанныеЗаполненияТаблицФормы") Тогда
			Для Каждого ТаблицКиЗ Из Параметры.ДанныеЗаполненияТаблицФормы Цикл
				...
				Если ЗначениеЗаполнено(Форма.ИдентификаторСтрокиРодителя) Тогда
					...
					Если Параметры.ЕстьХранимыеДанные = Ложь И ЭлементТаблицыФормы <> Неопределено
					И Форма[ЭлементТаблицыФормы.ПутьКДанным].Выгрузить().Колонки.Найти("ИдентификаторСтрокиРодителя") <> Неопределено Тогда
...
Функция ОписаниеТипаДопРеквизита(Знач ТипСтрокой = Неопределено, Знач Квалификаторы = Неопределено)
...
// {УАТ}
// Получить QRКод от оператора.
// 
// Параметры:
//  Отправитель - Строка - Идентификатор ЭДО отправителя
//  УИДМинтранс - Строка - УИДМинтранс
//  Токен - Строка - токен оператора для работы с QR-кодами
//  СоединениеHTTP - Неопределено - Соединение HTTP
// 
// Возвращаемое значение:
//  Неопределено, ДвоичныеДанные - Получить QRКод от оператора
Функция ПолучитьQRКодОтОператора(Знач Отправитель, Знач УИДМинтранс, Знач Токен, СоединениеHTTP = Неопределено) Экспорт
// {/УАТ}
...
Функция НайтиСоздатьЭПД(ДеревоДляСовместимостиСБЭД, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект)
	...
	ПараметрыОбработки.ИдентификаторПолучателя = ВРег(ПараметрыОбработки.ИдентификаторПолучателя);
	КлючеваяИнформацияПоТитулу = КлючеваяИнформацияПоТитулу(ОбъектXDTO);
	...
	ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул3 Тогда
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Свойство(
			"ТитулВыпускРезультатПроведенияПредсменногоКонтроля", 
			ДокументОбъект.ТитулВыпускРезультатПроведенияПредсменногоКонтроля);
	ИначеЕсли КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул5 Тогда
		ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Свойство(
			"ТитулЗаездПризнакОкончанияРейса", 
			ДокументОбъект.ТитулЗаездПризнакОкончанияРейса);
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
		...
		ЗапросВерсии.Текст = "ВЫБРАТЬ
		                     |	МАКСИМУМ(ВЫБОР
		                     |			КОГДА ВерсииТитуловЭПД.ИдентификаторФайла <> """"
		                     |				ТОГДА ВерсииТитуловЭПД.НомерВерсии
		                     |			ИНАЧЕ NULL
		                     |		КОНЕЦ) КАК НомерПоследнейВерсии,
		                     |	МАКСИМУМ(ВЫБОР
...
Функция ЗаполнитьПредставлениеСсылкиУчастника(СтруктураРеквизитов, ПрефиксРеквизитов, Перезаписывать = Ложь)
	...
	Если ЗначениеЗаполнено(ЮЛ_ИНН) Тогда
		...
		Если ЗначениеЗаполнено(ЮЛНаименование) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ИП_ИНН) Тогда
		...
		Если ЗначениеЗаполнено(ИПФамилия) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ФЛ_ИНН) Тогда
		...
		Если ЗначениеЗаполнено(ФЛФамилия) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ИнострЛицоБезУчетаСтатусФЛ_ЮЛ) Тогда
		...
		Если ЗначениеЗаполнено(ИнострЛицоБезУчетаНаименование) Тогда
			МассивЧастей.Добавить(ИнострЛицоБезУчетаНаименование);
		КонецЕсли;

		Если ЗначениеЗаполнено(ИнострЛицоБезУчетаИдентификатор) Тогда
			МассивЧастей.Добавить(ИнострЛицоБезУчетаИдентификатор);
		КонецЕсли;

		Если ЗначениеЗаполнено(ИнострЛицоБезУчетаСтрана) Тогда
		...
		Иначе        	
			...
			Если ОбластьНайдено <> Неопределено Тогда
				...
				Если СтрНайти(ИмяРеквизита, ".") Тогда
					...
					ПрефиксСтрокиТЧ = ИмяТЧИКолонки[0] + "__" + НомерСтрокиТЧ;
					...
					Если УровниИерархий.Количество() > 0 Тогда
						ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
							ПрефиксСтрокиТЧ + "__ИдентификаторСтроки", УровниИерархий[УровниИерархий.ВГраница()]);
					КонецЕсли;
					Если УровниИерархий.Количество() > 1 Тогда
						ДокументОбъект.ДополнительныеСвойства.СтруктураРеквизитов.Вставить(
							ПрефиксСтрокиТЧ + "__ИдентификаторСтрокиРодителя", УровниИерархий[УровниИерархий.ВГраница() - 1]);
					КонецЕсли;
				Иначе      
					Если ТипРеквизита = "Дата" Тогда
						ЗначениеРеквизита = ОбменСГИСЭПДКлиентСервер.ПреобразоватьВДату(ТекущийОбъектXDTO.Объект);
...
Функция СформироватьДанныеПрикладногоДокумента(ОбъектУчета, Данные)
	...
	Если ДвоичныеДанныеДокументаЭПД.ДвоичныеДанные = Неопределено Тогда
		...
		СтруктураОшибки = ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки(ОбъектУчета);
...
Функция ПолучитьИмяРеквизитаПоТекстуОшибкиXDTO(ТекстОшибки, МакетСоответствиеИмен)
	...
	ГруппаВСхеме = НайтиСтрокуМежду(ТекстОшибки, "Структура объекта ", "не соответствует типу");
	ГруппаВСхеме = СтрЗаменить(ГруппаВСхеме, "'", "");
	РеквизитВСхеме = НайтиСтрокуМежду(ТекстОшибки, "Проверка свойства '", "'");
	
	Если ГруппаВСхеме = "" И РеквизитВСхеме = "" Тогда
		Возврат "";
	КонецЕсли;
...
Процедура СобратьОписаниеОшибки(ДочернийУзел, ОписаниеОшибкиПроверки, СтопУзел = "//Файл/Документ")
	...
	Если СтрЗаканчиваетсяНа(УзелРодителя, "]") Тогда
		...
		ТекущийВерхнийРодитель = ТекущийРодитель.Родитель;
		Пока Не ЗначениеЗаполнено(ТекущийВерхнийРодитель.Имя) Цикл
			ТекущийВерхнийРодитель = ТекущийВерхнийРодитель.Родитель;	
		КонецЦикла;
		ОписаниеОшибкиПроверки.НомераСтрокТаблиц.Вставить(ТекущийВерхнийРодитель.Имя, ИндексВСписке);
		
		СледующийРодитель = ТекущийВерхнийРодитель;	 
	КонецЕсли;
...
Функция ЭтоСтрокаТабЧастиВВидеФормыТранспортнойНакладной(ИмяФормы)
	...
	Если ИмяФормы = "ТитулПеревозчикаЗаменыПрицеп" Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция УзлыСхемыФормыТранспортнойНакладной(ИмяФормы)
	...
	ИначеЕсли ИмяФормы = "ОснованиеВладенияТСРеквизитыСторон" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфГО/СвТС/ТС/ОснАрЛиз/ИдРекСост");
	ИначеЕсли ИмяФормы = "СторонаДокументаОснованийВладенияТС" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфГО/СвТС/ТС/ОснАрЛиз/ИдРекСост");
	ИначеЕсли ИмяФормы = "Прицеп" Тогда
	...
	ИначеЕсли ИмяФормы = "СторонаДокументаОснованийВладенияПрицепом" Тогда
		МассивУзлов.Добавить("//Файл/Документ/СодИнфГО/СвТС/Прицеп/ОснАрЛиз/ИдРекСост");
...
Процедура СоздатьДеревоСоответствийИОформитьКнопки(ИмяФормы,
	...
	Если ЭтоОсновнаяФорма Тогда
		Префикс = ПолучитьПрефиксТитулаПоСтраницеОсновнойФормы(ТекущаяСтраница, ТипДокумента);
	Иначе
		Если ОбъектСДанными.Свойство("ГруппаДанных") Тогда
			Префикс = ОбменСГИСЭПДКлиентСервер.ПрефиксТитулаРеквизита(ОбъектСДанными.ГруппаДанных, "Документ." + ТипДокумента);	
		КонецЕсли;
		Если Префикс = "" Тогда
	...
	Если ПустаяСтрока(Префикс) Тогда
		...
		ИначеЕсли ИмяФормы = "Документ.ЭлектроннаяТранспортнаяНакладная.Форма.Прицеп" Тогда
			Префикс = "ТитулГрузоотправителя";
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьЭлементыОтбораДляУзла(Форма, ГруппаОтбора, СтрокаДереваСУзлом, СтрокаДереваДляИсключения, ДобавитьГруппуИ)
	
	ЭтоОсновнаяФорма = ЭтоОсновнаяФорма(Форма);
	
	Если СтрокаДереваСУзлом = Неопределено
		Или СтрокаДереваСУзлом.Строки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ПодчиненнаяСтрокаДерева Из СтрокаДереваСУзлом.Строки Цикл
				...
				ИначеЕсли Результат.ЕстьЗаполненныеЭлементы Тогда
					...
					Иначе
						...
						Если СтрокаТаблицы.ЭлементыСхемы.Количество() = 1
							...
							Если Не ОбъектСДаннымиДокумента.Свойство(ПодчиненнаяТабЧасть) Тогда
								ПодчиненнаяТабЧасть = ОбменСГИСЭПДКлиентСервер.ОчиститьИмяРеквизитаОтПрефиксаДокумента(ПодчиненнаяТабЧасть);
							КонецЕсли;
							Если Не ОбъектСДаннымиДокумента.Свойство(ПодчиненнаяТабЧасть) Тогда
								Продолжить;
							КонецЕсли;
...
Функция ПроверитьПодчиненныеОбязательныеРеквизиты(ДеревоСоответствий,
	...
	ПодчиненныеЭлементыОбязательны = ЭтоОбязательныйЭлемент(СтрокаДерева, ДеревоСоответствий, ОбъектСДаннымиДокумента,
		СтруктураРеквизитов, ТекущаяТабличнаяЧасть, Индекс);
...
Функция ОбязательныеЭлементыТабЧастиЗаполнены(Знач ТабличнаяЧасть,
	...
	Если СтруктураРеквизитов = Неопределено Тогда
		Если Не ОбъектСДаннымиДокумента.Свойство(ТабличнаяЧасть) Тогда
			ТабличнаяЧасть = ОбменСГИСЭПДКлиентСервер.ОчиститьИмяРеквизитаОтПрефиксаДокумента(ТабличнаяЧасть);
		КонецЕсли;
		Если Не ОбъектСДаннымиДокумента.Свойство(ТабличнаяЧасть) Тогда
...
Функция ДополнитьДеревоСоответствий(СтруктураПараметров)
	...
	Пока Истина Цикл
		...
		Если ПутьКЭлементуФормы = Неопределено Тогда
			ПутьКЭлементуФормы = СоответствиеНаличияПолей[
				ОбменСГИСЭПДКлиентСервер.ОчиститьИмяРеквизитаОтПрефиксаДокумента(ПутьКДаннымЭлементаФормы)];
		КонецЕсли;	 
