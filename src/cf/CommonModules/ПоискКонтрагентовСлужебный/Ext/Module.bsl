...
Процедура ЗаполнитьОписаниеОшибкиФормированияОтчета(
	...
	ИначеЕсли ВидОшибки = "СервисНеПодключен"
		Или ВидОшибки = "ОтсутствуетДоступКСервису" Тогда
		
		Заголовок = НСтр("ru = 'Сервис 1С:Контрагент не подключен.'");
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодключениеСервисовСопровождения") Тогда
			
			Идентификатор = РаботаСКонтрагентами.ИдентификаторСервиса();
			...
			Если РезультатПроверки.Ошибка Тогда
				...
				ТекстОшибки = НСтр("ru = 'Ошибка при проверке возможности подключения тестового периода.'");
				Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			...
			ИначеЕсли ДоступностьСервиса = "Подключение" Тогда
				ТекстОшибки = НСтр("ru = 'Запрос на подключение сервиса отправлен.'");
				ТекстСообщения = НСтр("ru = 'Ожидайте подтверждение подключения.'");
	...
	ИначеЕсли ИмяОбласти = "ПодключитьТестовыйПериод" Тогда
		Область.Параметры.ПереходКПокупкеСервиса   = "ПереходКПокупкеСервиса";
		Область.Параметры.ПодключитьТестовыйПериод = "ПодключитьТестовыйПериод";
...
// Формирует настройки поиска, используемые по умолчанию.
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение - настройки по умолчанию:
...
//           выведены в область быстрого доступа при первом открытии отчета. Значение по умолчанию - 4.
//
Функция НастройкиПоУмолчанию() Экспорт
	...
	Результат.Вставить(
		ПоискКонтрагентовСлужебныйКлиентСервер.ВидКонтрагентаЮрЛицо(),
		Новый Структура(
			"КоличествоКритериевВБыстромДоступе",
			4));
	...
	ИнтеграцияПодсистемБИП.ПриНастройкеПоискаКонтрагентов(Результат);
	ПоискКонтрагентовПереопределяемый.ПриНастройкеПоискаКонтрагентов(Результат);
...
//
// Возвращаемое значение:
//  Структура - Результат загрузки информации из веб-сервиса:
//    * Ошибка - Булево - Истина, если в процессе выполнения метода возникли ошибки.
//    * ВидОшибки - Строка - вид возникшей ошибки, либо пустая строка, если ошибок нет.
//    * ИнформацияОбОшибке - Строка - описание ошибки, либо пустая строка, если ошибок нет.
//    * ОписаниеОшибкиСервиса - Структура, Неопределено - подробное описание ошибки веб-сервиса, либо Неопределено,
...
//    * ИННПоСтрокам - Соответствие Из КлючИЗначение - хранит информацию о связи номера строки отчета с ИНН
//      контрагентов, выводимого в этой строке.
//
...
Функция РазборВнешнихДанных(ОтветСервиса, ПараметрыЗапроса)
	...
	Результат.Вставить("ИНН",                 Новый Массив);
	Результат.Вставить("ИННПоСтрокам",        Новый Соответствие);
	...
	ДополнительныеПараметры.Вставить("ИНН",                    Новый Массив);
	ДополнительныеПараметры.Вставить("ИННПоСтрокам",           Новый Соответствие);
	...
	Результат.ИНН              = ДополнительныеПараметры.ИНН;
	Результат.ИННПоСтрокам     = ДополнительныеПараметры.ИННПоСтрокам;
...
Процедура ДобавитьСтрокуПоискаВТаблицу(
	...
	Если ЗначениеЗаполнено(РасшифровкаКонтрагента) Тогда
		ДополнительныеПараметры.ИНН.Добавить(РасшифровкаКонтрагента);
		ДополнительныеПараметры.ИННПоСтрокам.Вставить(
			НоваяСтрока.НомерПП,
			РасшифровкаКонтрагента);
	КонецЕсли;
...
//     Если в поле хранится больше 3 значений, то дополнительно в него выводится общее количество не выведенных значений
...
//    "(еще ХХ значений)".
//  - Представление каждого отдельного кода сокращается до количества символов, указанных в методе
