...
// Формирует сообщение для отправки сообщения в службу технической поддержки. 
// В параметрах передаются данные заполнения, вложения и параметры выгрузки журнала регистрации.
// Для отправки сообщения рекомендуется использовать СообщенияВСлужбуТехническойПоддержкиКлиент.Отправить().
//
// Параметры:
...
//           текстовые вложения (*.txt, *.json, *.xml), файлы картинок (*.bmp, *.jpg, *.jpeg,
//           *.tif, *.tiff, *.gif, *.png) и видеофайлы (*.mp4, *.m4v, *.mpeg, *.mpg, *.mov, *.wmv, *.avi, *.mkv).
...
//   * URLСтраницы - Строка - URL страницы отправки сообщения, если не используется Информационный центр.
//
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Вид.Имя         = "СообщениеВСлужбуТехническойПоддержки";
	Вид.ИмяПодменю  = "ПодменюСообщениеВСлужбуТехническойПоддержки";
	Вид.Заголовок   = НСтр("ru = 'Вопрос в поддержку'");
	Вид.Порядок     = 99;
	Вид.Картинка    = БиблиотекаКартинок.Сообщение;
...
Процедура ПриОпределенииСоставаНастроекПодключаемыхОбъектов(НастройкиПрограммногоИнтерфейса) Экспорт
	...
	Настройка.Ключ = "ДобавитьКомандыСообщенияВСлужбуТехническойПоддержки";
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(
	...
	ИнтеграцияПодсистемБИП.ПриОпределенииОбъектовСКомандамиСообщенияВСлужбуТехническойПоддержки(
		ИменаОбъектов);
	СообщенияВСлужбуТехническойПоддержкиПереопределяемый.ПриОпределенииОбъектовСКомандамиСообщенияВСлужбуТехническойПоддержки(
		ИменаОбъектов);
	...
	Команда.Вид = "СообщениеВСлужбуТехническойПоддержки";
	Команда.Идентификатор = "СообщениеВСлужбуТехническойПоддержкиДляОтчетовИОбработок";
	Команда.Представление = НСтр("ru = 'Вопрос в поддержку'");
	Команда.Важность = "СмТакже";
	Команда.ТолькоВоВсехДействиях = Истина;
	Команда.Порядок = 99;
	Команда.ОтображениеКнопки = ОтображениеКнопки.КартинкаИТекст;
	Команда.Картинка = БиблиотекаКартинок.Сообщение;
	Команда.ТипПараметра = ТипПараметра;
	Команда.ИзменяетВыбранныеОбъекты = Ложь;
	Команда.МножественныйВыбор = Ложь;
	Команда.РежимЗаписи = "НеЗаписывать";
	Команда.Обработчик = "СообщенияВСлужбуТехническойПоддержкиКлиент.Подключаемый_СообщениеВСлужбуТехническойПоддержки";
...
// Формирует описание результата отправки сообщения.
//
// Возвращаемое значение:
//  Структура - результат отправки сообщения:
//   * КодОшибки - Строка - идентификатор ошибки при отправки;
...
Функция ОтборЖурналаРегистрации(ПараметрыОтбора) Экспорт
...
Функция ПодготовитьВложения(Вложения, ДанныеВложений)
	...
	РасширенияBase64   = РасширенияBase64();
	РасширенияУвеличен = РасширенияУвеличенМаксимальныРазмер();
	
	МаксимальныйРазмерФайла            = МаксимальныйРазмерФайла();
	МаксимальныйРазмерФайлаУвеличенный = МаксимальныйРазмерФайлаУвеличенный();
	
	Для Каждого Вложение Из Вложения Цикл
		
		РасширениеИмениФайла = ВРег(
			ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(
				Вложение.Представление));
		
		Если РасширенияУвеличен.Найти(РасширениеИмениФайла) <> Неопределено Тогда
			МаксимальныйРазмерТекущий = МаксимальныйРазмерФайлаУвеличенный;
		Иначе
			МаксимальныйРазмерТекущий = МаксимальныйРазмерФайла;
		КонецЕсли;
		
		Если Вложение.ВидДанных = "Адрес" Тогда
			
			ДанныеФайла = ПолучитьИзВременногоХранилища(Вложение.Данные);
			Если ДанныеФайла.Размер() > МаксимальныйРазмерТекущий Тогда
			...
			Если РасширенияBase64.Найти(РасширениеИмениФайла) <> Неопределено Тогда
				ТекстВложения = Base64Строка(ДанныеФайла);
			Иначе
		...
		ИначеЕсли Вложение.ВидДанных = "ИмяФайла" Тогда
			...
			Если Не ФайлНаДиске.Существует() Тогда
				...
				Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					Вложение.Представление);
				Возврат Результат;
			ИначеЕсли ФайлНаДиске.Размер() > МаксимальныйРазмерТекущий Тогда
				Результат.КодОшибки = КодОшибкиПревышенМаксимальныйРазмерВложения();
				Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Превышен максимально допустимый размер файла %1.'"),
					Вложение.Представление);
...
// Формирует текст описания версии подсистем.
//
// Параметры:
//  ДанныеПодсистем - Массив Из Структура, Неопределено - описание всех библиотек конфигурации,
//    См. ОбщегоНазначения.ОписанияПодсистем;
// 
// Возвращаемое значение:
//  Строка - описание подсистем.
//
Функция ТекстОписаниеВерсийПодсистем(ДанныеПодсистем) Экспорт
...
// Формирует текст описания установленных расширений.
//
// Параметры:
//  ДанныеРасширений - Массив из РасширениеКонфигурации, Массив из Структура, Неопределено - данные
//    расширений конфигурации.
// 
// Возвращаемое значение:
//  Строка - описание расширений.
//
Функция ТекстОписаниеУстановленныхРасширений(ДанныеРасширений) Экспорт
...
// Заполняет техническую информацию перед отправкой сообщения в службу поддержки.
//
// Параметры:
//  ДанныеТехническойИнформации - Структура - данные технической информации.
//    См. СообщенияВСлужбуТехническойПоддержкиКлиентСервер.ОписаниеТехническойИнформации;
//  НастройкиТехническойИнформации - Структура - данные технической информации.
//    См. СообщенияВСлужбуТехническойПоддержкиКлиентСервер.НастройкиТехническойИнформации.
Процедура ЗаполнитьДанныеТехническойИнформации(
	ДанныеТехническойИнформации,
	НастройкиТехническойИнформации) Экспорт
...
Функция ИспользуетсяИнформационныйЦентр()
	...
	МодульИнформационныйЦентрСервер = ОбщегоНазначения.ОбщийМодуль("ИнформационныйЦентрСервер"); // АПК:271 Ранее проверка существования подсистемы
...
Функция ПодготовитьВложенияДляИнформационногоЦентра(Вложения, ДанныеВложений)
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		ОчищатьПоЗавершению = Истина;
		
		Если Вложение.ВидДанных = "Адрес" Тогда
			
			АдресФайла = Вложение.Данные;
			ОчищатьПоЗавершению = Ложь;
		...
		ПараметрыВложения.Вставить("Очищать",                   ОчищатьПоЗавершению);
...
Функция ПодготовитьТекстЖурналаРегистрацииДляИнформационногоЦентра(ПараметрыОтбора, ДанныеВложений)
	...
	Попытка
		...
		ПараметрыВложения.Вставить("АдресВоВременномХранилище", АдресФайла);
		ПараметрыВложения.Вставить("Очищать",                   Истина);
...
Функция ОперацияОтправкиДанных(
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.КодОшибки = ПереопределитьКодОшибкиСервиса(РезультатОтправки.КодСостояния);
		Результат.СообщениеОбОшибке = ПереопределитьСообщениеПользователю(
			Результат.КодОшибки,
...
Функция ПараметрыV2RequestJSON(
	...
	ТелоСообщения = Лев(ДанныеСообщения.Сообщение, 5000);
	
	ДанныеОбращения = Новый Структура;
	ДанныеОбращения.Вставить("theme", ДанныеСообщения.Тема);
	ДанныеОбращения.Вставить("text", ТелоСообщения);
...
// Определяет максимальный размер файла вложения для форматов, определяемых методом
//   РасширенияУвеличенМаксимальныРазмер.
//
// Возвращаемое значение:
//  Число - максимальный размер файла в байтах.
//
Функция МаксимальныйРазмерФайлаУвеличенный()
	
	Возврат 20971520; // 20 МБ.
...
Функция ПереопределитьКодОшибкиСервиса(КодСостояния)
	...
	ИначеЕсли КодСостояния = 400 Тогда
		Возврат СообщенияВСлужбуТехническойПоддержкиКлиентСервер.КодОшибкиНеверныйФорматЗапроса();
	ИначеЕсли КодСостояния = 413 Тогда
		Возврат КодОшибкиПревышенМаксимальныйРазмерВложения();
	ИначеЕсли КодСостояния = 429 Тогда
		Возврат КодОшибкиПовторОтправкиЗапроса();
	ИначеЕсли КодСостояния = 500
		Или КодСостояния = 503 Тогда
		
		Возврат КодОшибкиНеизвестнаяОшибкаСервиса();
...
//
// Параметры:
//  КодОшибки - Строка - ошибка сервиса см. ПереопределитьКодОшибкиСервиса;
//  Получатель - Строка - условное имя получателя сообщения;
//  ИсходноеСообщение - Строка - исходное сообщение об ошибке;
//  ДополнительнаяИнформация - Строка - дополнительная информация к сообщению об ошибке.
//
// Возвращаемое значение:
//  Строка, ФорматированнаяСтрока - сообщение пользователю.
//
Функция ПереопределитьСообщениеПользователю(
	...
	Если КодОшибки = "" Тогда
		Возврат "";
	ИначеЕсли КодОшибки = СообщенияВСлужбуТехническойПоддержкиКлиентСервер.КодОшибкиНеверныйФорматЗапроса() Тогда
		Результат = НСтр("ru = 'Неверный набор параметров или формат запроса. Обратитесь к администратору.'");
	ИначеЕсли КодОшибки = КодОшибкиПовторОтправкиЗапроса() Тогда
		Результат = НСтр("ru = 'Превышено количество попыток ввода отправки обращений. Повторите попытку позже.'");
	ИначеЕсли КодОшибки = КодОшибкиНеизвестнаяОшибкаСервиса() Тогда
		Результат = НСтр("ru = 'Не удалось подключиться к сервису. Сервис временно недоступен. Повторите попытку подключения позже.'");
	ИначеЕсли КодОшибки = КодОшибкиПревышенМаксимальныйРазмерВложения() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Превышен максимально допустимый размер файла %1.'"),
					ДополнительнаяИнформация);
	ИначеЕсли КодОшибки = КодОшибкиОшибкаПодключения() Тогда
		Результат = ИсходноеСообщение;
	Иначе
		Результат = НСтр("ru = 'Не удалось отправить обращение, повторите попытку позже.'");
	КонецЕсли;
	
	ДополнительныйТекст = "";
	
	Если Получатель = "webIts"
		Или Получатель = "backup"
		Или Получатель = "taxcom" Тогда
		ДополнительныйТекст = НСтр("ru = 'Альтернативный способ отправки обращения: на электронную почту <a href=""mailto:webits-info@1c.ru"">webits-info@1c.ru</a>'");
	КонецЕсли;
	
	ИнтеграцияПодсистемБИП.ПриФормированииТекстаОшибкиОтправкиСообщенияВСлужбуПоддержки(
		Получатель,
		ДополнительныйТекст);
	СообщенияВСлужбуТехническойПоддержкиПереопределяемый.ПриФормированииТекстаОшибкиОтправкиСообщенияВСлужбуПоддержки(
		Получатель,
		ДополнительныйТекст);
...
// Определяет расширения файлов, которые должны передаваться как Base64
//
// Возвращаемое значение:
//  Массив Из Строка - расширения файлов.
//
Функция РасширенияBase64()
	
	Результат = СтрРазделить("BMP;JPG;JPEG;TIF;TIFF;GIF;PNG;MP4;M4V;MPEG;MPG;MOV;WMV;AVI;MKV", ";");
	Возврат Результат;
	
КонецФункции

// Определяет расширения файлов, для которых используется повышенный максимальный размер.
//
// Возвращаемое значение:
//  Массив Из Строка - расширения файлов.
//
Функция РасширенияУвеличенМаксимальныРазмер()
	
	Результат = СтрРазделить("MP4;M4V;MPEG;MPG;MOV;WMV;AVI;MKV", ";");
	Возврат Результат;
