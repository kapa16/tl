...
Функция сфпСоздатьТелефонныйЗвонок(СтруктураЗвонка, ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураЗвонка.Контакт) Тогда
		...
		Если Найти(Строка(СтруктураЗвонка.Контакт), НСтр("ru = '<Объект не найден>'; en = '<Object is not found>'")) > 0 Тогда
			...
			+ НСтр("ru = ' Для разрешения работы с абонентом обратитесь к руководителю или администратору.'; en = 'To obtain permission to work with a subscriber, please contact your manager or administrator.'");
...
Функция сфпПолучитьМассивДоступныхДействий() Экспорт
	...
	сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Нет действий'; en = 'No action'"), "");
	сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Открыть телефонный звонок'; en = 'Open a phone call'"), "сфпОткрытьТелефонныйЗвонок");
	сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Карточка клиента'; en = 'Customer card'"), "сфпОткрытьКарточкуКонтакта");
	Если НЕ (Метаданные.Документы.Найти("Событие") = Неопределено) Тогда        
		сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Событие'; en = 'Event'"), "сфпОткрытьСобытие");
	КонецЕсли;	
	Если НЕ (Метаданные.Обработки.Найти("КонтактЦентр") = Неопределено) Тогда
		сфпДобавитьДействие(МассивДействий, Нстр("ru = 'Открыть ""Контакт-центр""'; en = 'Open the Contact Center'"), "ОткрытьКонтактЦентр");
	КонецЕсли;
	
	Если Найти(ВРЕГ(Метаданные.Имя), "АВТОСАЛОН") = 0 Тогда
		Если НЕ (Метаданные.Документы.Найти("ЗаказПокупателя") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Заказ покупателя'; en = 'Customer order'"), "сфпОткрытьЗаказПокупателя");
		КонецЕсли;	
		Если НЕ (Метаданные.Документы.Найти("ЗаказКлиента") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Заказ клиента'; en = 'Customer order'"), "сфпОткрытьЗаказКлиента");
		КонецЕсли;	
		Если НЕ (Метаданные.Документы.Найти("ЗаказПоставщику") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Заказ поставщику'; en = 'Order to supplier'"), "сфпОткрытьЗаказПоставщику");
		КонецЕсли;	
		Если НЕ (Метаданные.Обработки.Найти("ПомощникПродаж") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Помощник продаж'; en = 'Sales wizard'"), "сфпОткрытьПомощникПродаж");
		КонецЕсли;	
		Если НЕ (Метаданные.Отчеты.Найти("ДебиторскаяЗадолженность") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Дебиторская задолженность'; en = 'Receivable'"),	"сфпОткрытьДебиторскаяЗадолженность");
		КонецЕсли;	
		Если НЕ (Метаданные.Отчеты.Найти("Взаиморасчеты") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Взаиморасчеты'; en = 'Mutual settlements'"), "сфпОткрытьВзаиморасчеты");
		КонецЕсли;	
		Если НЕ (Метаданные.Отчеты.Найти("РасчетыСПартнерами") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Взаиморасчеты'; en = 'Mutual settlements'"), "сфпОткрытьРасчетыСПартнерами");
		КонецЕсли;	
		Если НЕ (Метаданные.Отчеты.Найти("КарточкаРасчетовСКлиентами") = Неопределено) Тогда
			сфпДобавитьДействие(МассивДействий,	Нстр("ru = 'Карточка расчетов'; en = 'Payment card'"), "сфпОткрытьКарточкаРасчетовСКлиентами");
		КонецЕсли;	
	КонецЕсли;
...
Процедура сфпОбработкаПолученияФормыОбработкаПолученияФормы(Источник, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументМенеджер.ТелефонныйЗвонок") Тогда
		...
		// {УАТ} СофтФон ++
		ИспользуетсяСофтФон = ИспользуетсяСофтФон И НЕ (Параметры.Свойство("Основание")
			И Метаданные.ОпределяемыеТипы.ПредметВзаимодействия.Тип.СодержитТип(ТипЗнч(Параметры.Основание))); 
		// {/УАТ} СофтФон --
