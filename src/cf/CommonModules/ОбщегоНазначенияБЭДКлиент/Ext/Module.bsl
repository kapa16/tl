...
// Автоподбор произвольной строки по списку выбора
//
// Параметры:
//  Сведения - СписокЗначений Из Произвольный - Содержит список значений для осуществления выбора по нажатию кнопки списка выбора
//  Текст - Строка - Текст для поиска
//  ДанныеВыбора - СписокЗначений Из Произвольный - Подобранный список значений для осуществления выбора по нажатию кнопки списка выбора
//  СтандартнаяОбработка - Булево - Принимает значение Ложь при наличии текста для поиска
Процедура АвтоПодборНайтиИВыделитьОформлением(Сведения, Текст, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	Для Каждого СтрокаСписка Из Сведения Цикл
		...
		Если ВыделениеСФорматированием <> Неопределено Тогда
			ДанныеВыбора.Добавить(СтрокаСписка.Значение, ВыделениеСФорматированием);
		КонецЕсли;
	КонецЦикла;
...
//  ЗаголовокКнопкиЗакрыть - Строка - заполняется, если необходимо изменить заголовок у кнопки закрытия формы
//  ОбработчикОповещения - Неопределено, ОписаниеОповещения - оповещение, которой необходимо вызвать 
//   при закрытии формы
//
Процедура ПоказатьФорматированнуюСтроку(Знач ФорматированнаяСтрока, Знач ЗаголовокФормы, ЗаголовокКнопкиЗакрыть = "",
	ОбработчикОповещения = Неопределено) Экспорт
	
	ПараметрыФормы = Новый Структура("ФорматированнаяСтрока, Заголовок, ЗаголовокКнопкиЗакрыть",
		ФорматированнаяСтрока, ЗаголовокФормы, ЗаголовокКнопкиЗакрыть);
...
// Возвращаемое значение:
//  ОписаниеОповещения
Функция ОписаниеОповещенияПоПутиКОбработчику(Обработчик, ДополнительныеПараметры = Неопределено) Экспорт
	
	СтруктураИмениОбработчика = СтрРазделить(Обработчик, ".");
	МодульОбработчика = ОбщегоНазначенияКлиент.ОбщийМодуль(СтруктураИмениОбработчика[0]);
	ИмяОбработчика = СтруктураИмениОбработчика[1];

	Возврат Новый ОписаниеОповещения(ИмяОбработчика, МодульОбработчика, ДополнительныеПараметры);
...
//             - Произвольный
//  Ключ - Неопределено,Произвольный - Идентификатор обработчика оповещения. 
Процедура ВыполнитьОбработкуОповещенияЧерезИнтервал(Интервал, Оповещение, Результат = Неопределено, Ключ = Неопределено) Экспорт
	
	ОбработкаОповещений = ПараметрыПриложения[ИмяПараметраПриложенияОбработкаОповещенийЧерезИнтервал()];
	Если ОбработкаОповещений = Неопределено Тогда
		ОбработкаОповещений = Новый Соответствие;
		ПараметрыПриложения.Вставить(ИмяПараметраПриложенияОбработкаОповещенийЧерезИнтервал(), ОбработкаОповещений);
	ИначеЕсли Ключ <> Неопределено И ОбработкаОповещений[Ключ] <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ОбработкаОповещений.Вставить(Ключ, ПараметрыОбработки);
	
	ПодключитьОбработчикОжидания("ОбработчикОжиданияВыполненияОповещенийЧерезИнтервал", Интервал, Истина);
...
//  УдалитьИзВременногоХранилища - Булево
//  
// Возвращаемое значение:
//  Произвольный
//
Функция РезультатВыполненияФоновогоЗадания(ВидОперации, Результат, КонтекстДиагностики, УдалитьИзВременногоХранилища = Ложь) Экспорт
	...
	Если УдалитьИзВременногоХранилища Тогда
		РезультатВыполнения = ОбщегоНазначенияБЭДВызовСервера.ПолучитьУдалитьИзВременногоХранилища(
...
// Возвращаемое значение:
//  Строка
Функция ИмяПараметраПриложенияОбработкаОповещенийЧерезИнтервал()
	Возврат "ЭлектронноеВзаимодействие.БазоваяФункциональность.ОбработкаОповещенийЧерезИнтервал";
КонецФункции

// Обрабатывает активные оповещения, добавленные методом см. ВыполнитьОбработкуОповещенияЧерезИнтервал
Процедура ВыполнитьОбработкуОповещенийПослеОжиданияИнтервала() Экспорт
	
	ОбработкаОповещений = ПараметрыПриложения[ИмяПараметраПриложенияОбработкаОповещенийЧерезИнтервал()]; // Массив из см. НовыеПараметрыОбработкиОповещенияЧерезИнтервал
	Если Не ЗначениеЗаполнено(ОбработкаОповещений) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ОбработкаОповещения Из ОбработкаОповещений Цикл
		ПараметрыОбработки = ОбработкаОповещения.Значение;
		Если ПараметрыОбработки.Начало <= ТекущаяУниверсальнаяДатаВМиллисекундах() Тогда
			НачатьВыполнениеОбработкиОповещения(ПараметрыОбработки.Оповещение, ПараметрыОбработки.Результат);
			ОбработкаОповещений.Удалить(ОбработкаОповещения.Ключ);
		КонецЕсли;
	КонецЦикла;
