...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Комментарий = НСтр("ru = 'Проверка правил интеграции с 1С:Документооборотом'; en = 'Check integration rules with 1C:Document Management'");
	...
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Комментарий = НСтр("ru = 'Проверка соответствия определяемых типов и плана обмена'; en = 'Verification of correspondence of defined types and exchange plan'");
	...
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Комментарий = НСтр("ru = 'Обновление правил интеграции с 1С:Документооборотом'; en = 'Integration rules updating with 1C:Document Management'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.1.8.4";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБИД.ПерейтиНаВерсию_1_1_8_4";
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Комментарий = НСтр("ru = 'Обновление правил интеграции с 1С:Документооборотом""'; en = 'Update integration rules with 1C:Document Management'");
	...
	Обработчик.Комментарий = СтрШаблон(
		НСтр("en = 'Transfer settings to the new """"%1"""" scheduled job adapted for SaaS mode using 1C:Fresh technology'; ru = 'Перенос настроек в новое регламентное задание ""%1"", адаптированное для работы в режиме сервиса по технологии 1С:Фреш'"),
...
Процедура ПроверитьСоответствиеОпределяемыхТиповМеждуСобой(Ошибки) Экспорт
	...
	Для Каждого ТипЗначения Из ТипыОбъектовПоддерживающихИнтеграцию Цикл
		...
		Если ОбъектМетаданных = Неопределено
			...
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но интеграция данного объекта с ДО не поддерживается.'; en = 'Type ""%1"" is selected in the defined type ""%2"", but the integration of this object with DI is not supported.'"),
					ТипЗначения,
					ИмяТипаВсеСсылки));
			Продолжить;
		КонецЕсли;
		...
		Если Не ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но в него должны быть включены только ссылочные типы.'; en = 'The type ""%1"" is selected in the defined type ""%2"", but only reference types should be enabled in it.'"),
					ТипЗначения,
					ИмяТипаВсеСсылки));
			Продолжить;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
			...
			Если Не Метаданные.ОпределяемыеТипы.ИнтеграцияС1СДокументооборотДокументыОбъектыПереопределяемый.Тип.СодержитТип(Тип(ИмяТипа)) Тогда
				Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Документ ""%1"" выбран в определяемом типе ""%2"", но отсутствует в типе ""%3"".'; en = 'The ""%1"" document is selected in the ""%2"" flexible type collection but is not present in the ""%3"" type.'"),
		...
		Иначе
			...
			Если Не Метаданные.ОпределяемыеТипы.ИнтеграцияС1СДокументооборотСправочникиОбъектыПереопределяемый.Тип.СодержитТип(Тип(ИмяТипа)) Тогда
				Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Справочник ""%1"" выбран в определяемом типе ""%2"", но отсутствует в типе ""%3"".'; en = 'The ""%1"" catalog is selected in the ""%2"" flexible type collection but is missing in the ""%3"" type.'"),
					ОбъектМетаданных.Имя,
					ИмяТипаВсеСсылки,
					ИмяТипаСправочникиОбъекты));
			КонецЕсли;
	...
	Для Каждого ТипЗначения Из Метаданные.ОпределяемыеТипы.ИнтеграцияС1СДокументооборотДокументыОбъектыПереопределяемый.Тип.Типы() Цикл
		...
		Если ОбъектМетаданных = Неопределено Или Не ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но в него должны быть включены только документы.'; en = 'Type ""%1"" is selected in the defined type ""%2"", but it should only enable documents.'"),
					ТипЗначения,
					ИмяТипаДокументыОбъекты));
			Продолжить;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но в него должны быть включены только типы %3.'; en = 'The ""%1"" type is selected in the ""%2"" flexible type collection but it should include only %3 types.'"),
					ТипЗначения,
					ИмяТипаДокументыОбъекты,
					"ДокументОбъект"));
			Продолжить;
	...
	Для Каждого ТипЗначения Из Метаданные.ОпределяемыеТипы.ИнтеграцияС1СДокументооборотСправочникиОбъектыПереопределяемый.Тип.Типы() Цикл
		...
		Если ОбъектМетаданных = Неопределено Или ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но в него должны быть включены только справочники.'; en = 'Type ""%1"" is selected in the defined type ""%2"", but it should only enable directories.'"),
					ТипЗначения,
					ИмяТипаСправочникиОбъекты));
			Продолжить;
		КонецЕсли;
...
Процедура ПроверитьСоответствиеОпределяемыхТиповИПланаОбмена(Ошибки) Экспорт
	...
	Для Каждого ЭлементСостава Из Метаданные.ПланыОбмена.ИнтеграцияС1СДокументооборотомПереопределяемый.Состав Цикл
		...
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональность.ОбъектМетаданныхПоддерживается(ОбъектМетаданных) Тогда
			Ошибки.Добавить(СтрШаблон(
					НСтр("ru = 'Объект ""%1"" есть в плане обмена ""%2"", но интеграция данного объекта с ДО не поддерживается.'; en = 'The ""%1"" object is available in the ""%2"" exchange plan but its integration with EDI is not supported.'"),
					ПолноеИмя,
					ИмяТипаПланОбмена));
			Продолжить;
		КонецЕсли;
		...
		Если Не Метаданные.ОпределяемыеТипы.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый.Тип.СодержитТип(Тип(ИмяТипа)) Тогда
			Ошибки.Добавить(СтрШаблон(
				НСтр("ru = 'Объект ""%1"" есть в плане обмена ""%2"", но отсутствует в типе ""%3"".'; en = 'The ""%1"" object is in the ""%2"" exchange plan but is missing in the ""%3"" type.'"),
				ПолноеИмя,
				ИмяТипаПланОбмена,
				ИмяТипаВсеСсылки));
		КонецЕсли;
	...
	Для Каждого ТипЗначения Из ТипыОбъектовПоддерживающихИнтеграцию Цикл
		...
		Если Не Метаданные.ПланыОбмена.ИнтеграцияС1СДокументооборотомПереопределяемый.Состав.Содержит(ОбъектМетаданных) Тогда
			Ошибки.Добавить(СтрШаблон(
				НСтр("ru = 'Тип ""%1"" выбран в определяемом типе ""%2"", но отсутствует в составе плана обмена ""%3"".'; en = 'The ""%1"" type is selected in the ""%2"" flexible type collection but is missing in the ""%3"" exchange plan.'"),
				ТипЗначения,
				ИмяТипаВсеСсылки,
				ИмяТипаПланОбмена));
		КонецЕсли;
...
Процедура ПерейтиНаВерсию_3_0_2_1() Экспорт
	...
	ИмяЗаданияОбмена = "УдалитьИнтеграцияС1СДокументооборотВыполнитьОбменДанными";
	Задание = РегламентныеЗаданияСервер.Задание(
		Метаданные.РегламентныеЗадания[ИмяЗаданияОбмена]);
