...
"по причине:"
"%1'; en = 'An attempt to log in using a ticket from the 1C:ITS Portal failed for the following reason:"
"%1'"),
...
Функция КорректныДанныеАутентификацииПользователяИнтернетПоддержки() Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(
			НСтр("ru = 'Нет прав подключения сервиса 1С:Распознавание первичных документов'; en = 'Insufficient rights to enable the 1C:Source document recognition service'"),
...
Функция НачатьПроверкуДопустимыхДействийПриАвторизации() Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка допустимых действий при авторизации'; en = 'Check allowed actions during authorization'");
...
"Необходимо указать сетевой каталог для обмена.'; en = 'You cannot use the specified %1 directory for exchange as it is unavailable on the server."
...
Процедура ОтправкаВложенийИзПочты(УчетнаяЗапись) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда 
		// {УАТ}
		КлючЗадания = "ОтправкаВложенийИзПочты" + XMLСтрока(ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса());
		// {/УАТ}
	Иначе
	...
	Попытка
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("РаспознаваниеДокументов.ОтправитьВложенияИзПочтыНаРаспознавание", ПараметрыЗадания,
			КлючЗадания, НСтр("ru = 'Отправка вложений из почты на распознавание'; en = 'Send attachments from email for recognition'"));
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Невозможно выполнить фоновое задание с ключом: %1"
"Описание ошибки:"
"%2'; en = 'Cannot complete the background job with the key: %1"
"Error details:"
"%2'"),
			КлючЗадания,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОтправкаФайловИзКаталога(ПараметрыПоиска) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда 
		// {УАТ}
		КлючЗадания = "ОтправкаФайловИзКаталога" + XMLСтрока(ИнтернетПоддержкаПользователей.ЗначениеРазделителяСеанса());  
		// {/УАТ}
	Иначе
	...
	Попытка
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("РаспознаваниеДокументов.ОтправитьФайлыИзКаталогаНаРаспознавание", ПараметрыЗадания,
			КлючЗадания, НСтр("ru = 'Отправка файлов из каталога на распознавание'; en = 'Send attachments from the directory for recognition'"));
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Невозможно выполнить фоновое задание с ключом: %1"
"Описание ошибки:"
...
Функция НаличиеТоваровУслуг(ТаблицаДокумента) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьТовары", Ложь);
	Результат.Вставить("ЕстьУслуги", Ложь);
	Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы , "ЭтоУслуга") Тогда
			ИмяКолонки = "ЭтоУслуга";
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы , "Услуга") Тогда
			ИмяКолонки = "Услуга";
		Иначе
			Прервать;
		КонецЕсли;
		
		Если СтрокаТаблицы[ИмяКолонки] Тогда
...
Функция ПараметрыЗаполненияТиповойНаОсновеФормыТОРГ12(Знач НашДокумент, Знач ВидОперации, Знач ТипДокументаСтрокой) Экспорт
	...
	ПодсистемаБухгалтерияПредприятияПодсистемыСуществует = ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы");
	
	Если ТипДокументаСтрокой = "ПоступлениеТоваровУслуг" Или ТипДокументаСтрокой = "ПриходнаяНакладная" Тогда
		...
		Если ПодсистемаБухгалтерияПредприятияПодсистемыСуществует Тогда 
			МодульПоступлениеТоваровУслугФормыКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ПоступлениеТоваровУслугФормыКлиентСервер");
			ОтражениеВУСН = МодульПоступлениеТоваровУслугФормыКлиентСервер.ОтражениеВУСН(ВидОперации, Ложь);
		КонецЕсли;
	...
	СуммаВключаетНДС = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(ДокументОбъект, "СуммаВключаетНДС", Ложь);	
	...
	Если ТипДокументаСтрокой = "ПоступлениеТоваровУслуг"
		И ПараметрыЗаполнения.Свойство("Склад") И ПодсистемаБухгалтерияПредприятияПодсистемыСуществует Тогда
		...
		СпособОценкиТоваровВРознице = МодульУчетнаяПолитика.СпособОценкиТоваровВРознице(ПараметрыЗаполнения.Организация, ПараметрыЗаполнения.Дата);
		Если ТипСклада = ОбщегоНазначения.ПредопределенныйЭлемент("Перечисление.ТипыСкладов.НеавтоматизированнаяТорговаяТочка")
			И СпособОценкиТоваровВРознице =  ОбщегоНазначения.ПредопределенныйЭлемент("Перечисление.СпособыОценкиТоваровВРознице.ПоПродажнойСтоимости") Тогда
	...
	Для Каждого СтрокаТаблицыДокумента Из ТаблицаДокумента Цикл
		...
		Если СтрокаТаблицыДокумента.ЭтоУслуга Тогда
...
Функция ПараметрыЗаполненияТиповойНаОсновеСчетНаОплату(Знач НашДокумент, Знач ТипДокументаСтрокой) Экспорт
	...
	Если ТипДокументаСтрокой = "ПлатежноеПоручение" Тогда
		ПараметрыЗаполнения.Вставить("СуммаДокумента", ДокументОбъект.РеквизитыДокумента[12].Значение);
		...
		Для Каждого СтрокаТаблицыДокумента Из ТаблицаТовары Цикл
			Если СтавкиНДС.Найти(СтрокаТаблицыДокумента.СтавкаНДС) = Неопределено Тогда
				СтавкиНДС.Добавить(СтрокаТаблицыДокумента.СтавкаНДС);
			КонецЕсли;
		КонецЦикла;
		
		Если СтавкиНДС.Количество() = 1 Тогда
			ПараметрыЗаполнения.Вставить("СтавкаНДС", СтавкиНДС[0]);
		КонецЕсли;
		
		ПараметрыЗаполнения.Вставить("СчетКонтрагента", ПараметрыЗаполнения.БанковскийСчетКонтрагента);
		
	КонецЕсли;
	
	РаспознаваниеДокументовПереопределяемый.ЗаполнитьСкидкиВТаблицеТовары(ДокументОбъект, ТаблицаТовары, ПараметрыЗаполнения, ТипДокументаСтрокой);	
