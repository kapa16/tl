...
#Если МобильныйКлиент Тогда
	...
	СтрокаСообщенияОбОшибке = НСтр("ru = 'Подключение к приложению недоступно в мобильном клиенте.';
									|en = 'Mobile client does not support connecting to other applications.'");
...
#Иначе
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru = 'Не удалось подключиться к приложению: %1';
										|en = 'Failed to connect to the application: %1'");
	...
	Если СодержитНебезопасныеДействия(КомандаЗапуска) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось запустить приложение. Недопустимая строка команды:
			           ...
			           |en = 'Cannot start the application. Invalid command line:
			           |%1
			           |
