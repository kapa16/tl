...
Процедура Сортировка(Команда)
	ОткрытьФорму("Отчет.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Форма.ФормаСортировки",, ЭтотОбъект,,,,
...
Процедура СформироватьОтчет()
	...
	СхемаКомпоновкиДанных = Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.ПолучитьМакет("ИсточникНастроекОтбораКомпоновщика");
	...
	Макет = Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.ПолучитьМакет("Макет");
	...
	Если ПечататьТитульныйЛист Тогда
		...
		Если ВидКонтроля = Перечисления.уатВидыКонтроляТранспортныхДокументов.Предрейсовый Тогда
			ОбластьЗаголовок.Параметры.ВидКонтроля = "ПРЕДРЕЙСОВОГО";
		ИначеЕсли ВидКонтроля = Перечисления.уатВидыКонтроляТранспортныхДокументов.Послерейсовый Тогда
			ОбластьЗаголовок.Параметры.ВидКонтроля = "ПОСЛЕРЕЙСОВОГО";
		Иначе
			ОбластьЗаголовок.Параметры.ВидКонтроля = "ПРЕДРЕЙСОВОГО И ПОСЛЕРЕЙСОВОГО";
		КонецЕсли;
...
Процедура ИнициализироватьКомпоновщик()
	МакетКомпоновки = Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.ПолучитьМакет("ИсточникНастроекОтбораКомпоновщика");
	...
	// Сортировка
	СхемаКомпоновкиДанныхСортировка = Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.ПолучитьМакет("ИсточникНастроекОтбораКомпоновщика");
	...
	НастройкиПорядка = ХранилищеНастроекДанныхФорм.Загрузить(
		"Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Форма.ФормаОтчета",
		"уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС_НастройкиКомпоновщика_Порядок");
	...
	НастройкиОтчета = ХранилищеНастроекДанныхФорм.Загрузить(
		"Отчеты.уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Форма.ФормаОтчета",
		"уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС_Настройки");
...
Функция ПолучитьДокументыЖурнала(Журнал)
	...
	|ГДЕ
	|	уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Журнал = &Журнал
	|	И уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Проведен
	|
	|УПОРЯДОЧИТЬ ПО
	|	уатЖурналПредрейсовогоИПослерейсовогоКонтроляТС.Дата,
