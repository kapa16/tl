...
&НаКлиенте
Процедура УчетнаяЗаписьОблачногоЭДОПриИзменении(Элемент)
...
Процедура ДоверенностьНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПараметрыОткрытия = НовыйПараметрыОткрытияФормыВыбораДоверенности();
	ОткрытьФорму("Справочник.МЧД003.ФормаВыбора", ПараметрыОткрытия, Элемент);
...
Процедура УправлениеФормой(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Организация)
		...
		И (ЗначениеЗаполнено(Форма.Доверенность) Или Не Форма.ТребуетсяДоверенность)
		И (Не Форма.ЭтоОблачныйЭДО Или ЗначениеЗаполнено(Форма.УчетнаяЗаписьОблачногоЭДО)) Тогда
	...
	Элементы.УчетнаяЗаписьОблачногоЭДО.Видимость = Форма.ЭтоОблачныйЭДО;
...
Процедура УстановитьЗначенияПоУмолчанию()
	...
	Если Операция = Неопределено И ЗначениеЗаполнено(Организация) Тогда
		...
		ПараметрыДанных.УчетнаяЗаписьОблачногоЭДО = УчетнаяЗаписьОблачногоЭДО;
		
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение идентификаторов в сервисе ЭДО'");
		// См. УчетныеЗаписиЭДО.ДанныеПоИдентификаторамДляРегистрацииВЭДО
...
Процедура ЗапуститьОжиданиеПолученияИдентификаторовЭДО()
	...
	Если Не ПолучатьИдентификаторыВСервисе1СЭДО
		Или ДлительнаяОперация = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверкаЗаполнения()
	...
	Если ЭтоОблачныйЭДО И Не ЗначениеЗаполнено(УчетнаяЗаписьОблачногоЭДО) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
					НСтр("ru = 'Учетная запись облачного ЭДО'")),,"УчетнаяЗаписьОблачногоЭДО",,Отказ);
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатЧерезОблачныйЭДО()
	...
	ПараметрыРегистрации.КодНалоговогоОргана = КодНалоговогоОргана;
	ПараметрыРегистрации.ЭлектроннаяПочта = ЭлектроннаяПочта;
	ПараметрыРегистрации.СертификатыДляПодписания.Добавить(СертификатКриптографии);
...
// Параметры:
//  Результат - см. УчетныеЗаписиЭДОИнтеграцияОблакаКлиент.НовыйРезультатОтправкиСертификатовДляРегистрацииВЭДО
...
// Параметры:
//  РезультатРегистрации - см. УчетныеЗаписиЭДОИнтеграцияОблакаКлиент.НовыйРезультатРегистрацииСертификатовВЭДО
//  Контекст - Неопределено
&НаКлиенте
...
Процедура ОбновитьПараметрыУведомлений()
	...
	Если ЭтоОблачныйЭДО Тогда
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияОблакаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"УчетныеЗаписиЭДОИнтеграцияОблакаКлиент");
		ПараметрыОбновления = ОбщийМодульУчетныеЗаписиЭДОИнтеграцияОблакаКлиент.НовыеПараметрыОбновленияНастройкиУведомленийУчетнойЗаписиЭДО();
		ПараметрыОбновления.ОповещениеОЗавершении = Оповещение;
		ПараметрыОбновления.УчетнаяЗаписьОблачногоЭДО = УчетнаяЗаписьОблачногоЭДО;
		ПараметрыОбновления.ИдентификаторЭДО = ИдентификаторОрганизацииСуществующий;
		...
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияОблакаКлиент.ОбновитьНастройкиУведомленийУчетнойЗаписиЭДО(ПараметрыОбновления);
	Иначе
...
Процедура ВыполнитьОтправкуДанныхВСервисНастроек()
	...
	Если ЭтоОблачныйЭДО Тогда
		...
		ПараметрыОбновления.Вставить("ИдентификаторЭДО", ИдентификаторОрганизации);
		ПараметрыОбновления.Вставить("Наименование", НаименованиеУчетнойЗаписи);
		ПараметрыОбновления.Вставить("Назначение", НазначениеУчетнойЗаписи);
		ПараметрыОбновления.Вставить("ПодробноеОписание", ПодробноеОписаниеУчетнойЗаписи);
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияОблакаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"УчетныеЗаписиЭДОИнтеграцияОблакаКлиент");
...
Процедура ОтправкаДанныхВСервисНастроекЧерезОблачныйЭДОЗавершение(Результат, ИдентификаторЭДО) Экспорт
	...
	ВключитьАвтоматическоеПолучениеЭДО(ИдентификаторЭДО);
...
Функция СохранитьУчетнуюЗапись()
	...
	Если ЗначениеЗаполнено(СертификатКриптографии) Тогда
		ОписаниеУчетнойЗаписи.Сертификаты.Добавить(СертификатКриптографии);
		Если ЗначениеЗаполнено(Доверенность) Тогда
			ОписаниеУчетнойЗаписи.Доверенности.Вставить(СертификатКриптографии, Доверенность);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ВключитьАвтоматическоеПолучениеЭДО(ИдентификаторЭДО)
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОткрытьУчетнуюЗаписьПослеВключенияАвтоматическогоПолученияЭДО",
		ЭтотОбъект, ИдентификаторЭДО);
	
	Если Не ЭтоОблачныйЭДО 
...
//  ИдентификаторЭДО - Строка
&НаКлиенте
Процедура ОткрытьУчетнуюЗаписьПослеВключенияАвтоматическогоПолученияЭДО(Результат, ИдентификаторЭДО) Экспорт
...
&НаКлиенте
Функция НовыйПараметрыОткрытияФормыВыбораДоверенности()
	...
	Результат.Вставить("ДополнительныеПараметры", 
		Новый Структура("СертификатКриптографии", СертификатКриптографии));
