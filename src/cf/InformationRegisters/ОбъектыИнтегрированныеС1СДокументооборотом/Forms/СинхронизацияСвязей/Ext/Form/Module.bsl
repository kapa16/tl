...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		ШаблонПояснения = НСтр("ru = 'В базу 1С:Документооборот будут добавлены недостающие ссылки на объекты %1."
"В базу %1 будут добавлены недостающие ссылки на объекты 1С:Документооборота."
"Внимание, операция может занять продолжительное время. Во время выполнения операции бесшовная интеграция с 1С:Документооборотом будет ограничена.'; en = 'Missing links to %1 objects will be added to 1C:Document Management."
...
Процедура НачатьСинхронизацию()
	...
	ТекущееСостояние = НСтр("ru = 'Подготовка данных для выполнения операции'; en = 'Prepare data to perform the operation'");
	Прогресс = 0;
...
Функция НачатьСинхронизациюНаСервере()
	...
	Попытка
		...
		Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапросАсинхронно(
			...
			НСтр("ru = 'Подготовка данных для синхронизации связей'; en = 'Prepare data to synchronize links'"));
...
Процедура СинхронизироватьПорциюОбъектов(ИнтегрированныеОбъекты, КоличествоОбъектов, Индекс, РазмерПорции)
	...
	Элементы.Синхронизировать.Доступность = Ложь;
	ТекущееСостояние = НСтр("ru = 'Выполнение операции'; en = 'Performing operation'");
...
Функция СинхронизироватьПорциюОбъектовНаСервере(ИнтегрированныеОбъекты, КоличествоОбъектов, Индекс, РазмерПорции)
	...
	Попытка
		...
		Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапросАсинхронно(
			...
			Запрос,
			УникальныйИдентификатор,
			НСтр("ru = 'Синхронизация связей'; en = 'Link synchronization'"));
...
Процедура ЗавершитьСинхронизацию()
	...
	Элементы.Синхронизировать.Доступность = Ложь;
	ТекущееСостояние = НСтр("ru = 'Завершение выполнения операции'; en = 'Complete operation execution'");
	Прогресс = 100;
...
Функция ЗавершитьСинхронизациюНаСервере()
	...
	Попытка
		...
		Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапросАсинхронно(
			...
			Запрос,
			УникальныйИдентификатор,
			НСтр("ru = 'Завершение синхронизации связей'; en = 'Complete link synchronization'"));
...
Процедура ЗавершитьСинхронизациюЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если РезультатЗавершенияСинхронизации.Успешно Тогда
			...
			ТекущееСостояние = НСтр("ru = 'Синхронизация успешно выполнена'; en = 'Synchronization is completed successfully'");
			Элементы.Синхронизировать.Заголовок = НСтр("ru = 'Завершить'; en = 'Finish'");
		Иначе
