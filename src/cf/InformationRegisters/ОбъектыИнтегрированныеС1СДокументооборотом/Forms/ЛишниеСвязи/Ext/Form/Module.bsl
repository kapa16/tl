	...
	Иначе
		Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
			Элементы.ПояснениеЛишниеСвязиДО.Заголовок = СтрШаблон(
				НСтр("ru = 'В базе 1С:Документооборот есть ссылки на несуществующие объекты %1.'; en = '1C:Document Management contains references to non-existent %1 objects.'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиДОТипОбъектаИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Тип объекта %1'; en = 'Object type %1'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиДОИдентификаторОбъектаИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Идентификатор объекта %1'; en = '%1 object ID'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиДОУстановитьСвязьСНовымОбъектомИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Установить новую связь с объектом %1'; en = 'Add new link to %1 object'"),
				СокращенноеНаименованиеКонфигурации);
	...
	Иначе
		Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
			Элементы.ПояснениеЛишниеСвязиИС.Заголовок = СтрШаблон(
				НСтр("ru = 'В базе %1 есть ссылки на несуществующие объекты 1С:Документооборот.'; en = '%1 contains references to non-existent 1C:Document Management objects.'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиИСУдалитьСвязиИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Удалить лишние связи в базе %1'; en = 'Delete excessive links in %1'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЗаголовокЛишниеСвязиИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Лишние связи в базе %1'; en = 'Excessive links in %1'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиИСОбъектИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Объект %1'; en = 'Object %1'"),
				СокращенноеНаименованиеКонфигурации);
			Элементы.ЛишниеСвязиИСКонтекстноеМенюОткрытьОбъектИС.Заголовок = СтрШаблон(
				НСтр("ru = 'Открыть объект %1'; en = 'Open the %1 object'"),
				СокращенноеНаименованиеКонфигурации);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСвязьСНовымОбъектомДО(Команда)
	...
	Иначе
		...
		СписокТипов.Добавить("DMDocument", НСтр("ru = 'Документ'; en = 'Document'"));
		СписокТипов.Добавить("DMCorrespondent", НСтр("ru = 'Контрагент'; en = 'Counterparty'"));
		СписокТипов.Добавить("DMMeeting", НСтр("ru = 'Мероприятие'; en = 'Activity'"));
		СписокТипов.ПоказатьВыборЭлемента(Оповещение, НСтр("ru = 'Выбор типа данных'; en = 'Select data type'"));
	КонецЕсли;
...
Процедура УстановитьСвязьСНовымОбъектомИС(Команда)
	...
	Иначе
		...
		ИнтеграцияС1СДокументооборотВсеСсылки.ПоказатьВыборЭлемента(Оповещение, НСтр("ru = 'Выбор типа данных'; en = 'Select data type'"));
	КонецЕсли;
...
Процедура УстановитьСвязьСНовымОбъектомИСПослеВыбораТипа(Знач ТипОбъектаИС, ТекущиеДанные) Экспорт
	...
	ИначеЕсли ТипЗнч(ТипОбъектаИС) = Тип("Строка") Тогда
		...
		ИмяТипаОбъектаИС = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ИмяТипаИзПолногоИмени(ТипОбъектаИС);
		ТипОбъектаИС = Тип(ИмяТипаОбъектаИС);
	...
	ПоказатьВводЗначения(
		Оповещение,,
		СтрШаблон(НСтр("ru = 'Выберите объект, связанный с ""%1""'; en = 'Select an object connected with ""%1""'"), ТекущиеДанные.ПредставлениеОбъектаДО),
		ТипОбъектаИС);
