...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	ТекстСерия				= НСтр("en=', series: %1';ru=', серия: %1'");
	ТекстНомер				= НСтр("en=', No.%1';ru=', № %1'");
	ТекстДатаВыдачи			= НСтр("en=', issued: %1 year';ru=', выдан: %1 года'");
	ТекстСрокДействия		= НСтр("en=', valid till: %1 year';ru=', действует до: %1 года'");
	ТекстКодПодразделения	= НСтр("en=', %1 div. No.';ru=', № подр. %1'");
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	ТекстСообщения = НСтр("en='ID document of individual %2 has been already entered as of 1%.';ru='На %1 у физлица %2 уже введен документ, удостоверяющий личность.'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если Не ПустаяСтрока(Запись.Серия) Тогда
			ТекстОшибки = "";
			Отказ = Не СотрудникиКлиентСервер.СерияДокументаУказанаПравильно(Запись.ВидДокумента, Запись.Серия, ТекстОшибки) Или Отказ;
		...
		Если Не ПустаяСтрока(Запись.Номер) Тогда
			ТекстОшибки = "";
			Отказ = Не СотрудникиКлиентСервер.НомерДокументаУказанПравильно(Запись.ВидДокумента, Запись.Номер, ТекстОшибки) Или Отказ;
			Если Не ПустаяСтрока(ТекстОшибки) Тогда
