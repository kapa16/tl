...
Процедура ПриОткрытии(Отказ)
	...
	Если Не АдресныйКлассификаторСодержитЗагруженныеСведения Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru = 'Адресные сведения для очистки отсутствуют'; en = 'No address information to clear'"));
	Иначе
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Прервать очистку адресного классификатора?'; en = 'Stop clearing address classifier?'");
...
Процедура ОбновитьИнтерфейсПоКоличествуОчищаемых()
	...
	// Страница загрузки
	ТекстОписанияОчистки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Очищаются данные выбранных регионов (%1)'; en = 'Data of the selected regions is being cleared (%1)'"), ВыбраноРегионовДляОчистки);
...
Процедура ОчиститьКлассификатор()
	...
	ТекстСостоянияОчистки = НСтр("ru = 'Очистка адресного классификатора ...'; en = 'Clearing the address classifier ...'");
...
Процедура ПослеФоновойОчистки(Задание, ДополнительныеПараметры) Экспорт
	...
	Элементы.ШагиОчистки.ТекущаяСтраница = Элементы.УспешноеЗавершение;
		ТекстОписанияОчистки = НСтр("ru = 'Адресный классификатор успешно очищен.'; en = 'Address classifier is successfully cleared.'");
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не удается очистить адресный классификатор.'; en = 'Cannot clear address classifier.'");
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Техническая информация:'; en = 'Technical information:'") + Задание.ПодробноеПредставлениеОшибки;
...
Функция ЗапуститьФоновуюОчисткуНаСервере(Знач КодыРегионов, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Очистка адресного классификатора'; en = 'Address classifier cleanup'");
