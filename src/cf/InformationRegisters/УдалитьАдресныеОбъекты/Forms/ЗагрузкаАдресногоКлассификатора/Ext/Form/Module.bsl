	...
	Иначе
		...
		Если ДоступнаЗагрузкаАдресныхСведенийИзИнтернет Тогда
			...
			Если КоличествоОбновлений > 0 Тогда
				...
				ИнформацияОбОбновление = НСтр("ru = 'Доступно обновление от'; en = 'Update is available from'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Прервать загрузку адресного классификатора?'; en = 'Stop importing address classifier?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ru = 'Папка с файлами адресного классификатора'; en = 'Folder with address classifier files'"),
...
Процедура Загрузить(Команда)
	...
	Если КодИсточникаЗагрузки = "КАТАЛОГ" Тогда
		Текст = НСтр("ru = 'Для загрузки адресного классификатора из папки установите расширение для работы с 1С:Предприятием.'; en = 'To import the address classifier from the folder, install 1C:Enterprise extension.'");
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр("ru = 'Не указан вариант загрузки классификатора.'; en = 'Classifier import option is not specified.'") );
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Доступно обновление от %1'; en = 'Update from %1 is available'"), ДатаОбновленияСтрокой);
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка обновления для адресного классификатора'; en = 'Check the update to the address classifier'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружаются данные по выбранным регионам (%1)'; en = 'Importing data for the selected regions (%1)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач КодыРегионов)
	...
	Элементы.ШагиЗагрузки.ТекущаяСтраница = Элементы.ОжиданиеЗагрузки;
	ТекстСостоянияЗагрузки = НСтр("ru = 'Загрузка файлов с Портала 1С:ИТС...'; en = 'Downloading files from 1C:ITS Portal…'");
...
""
"Техническая информация:'; en = ""Unable to load address information. Possible causes:"
"• Incorrect or missing login and password;"
"• No internet connection;"
"• The site is undergoing maintenance. Please try loading again later;"
"• A firewall or other intermediary software (antivirus software, etc.) is blocking the program's attempts to connect to the internet;"
"• The internet connection is established through a proxy server, but its settings are not configured in the program."
""
...
"Техническая информация:'; en = 'There might be some routine maintenance on the website. Try again later."
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСостоянияЗагрузки = НСтр("ru = 'Передача файла на сервер приложения...'; en = 'Sending file to application server…'");
...
""
"Техническая информация:'; en = 'Unable to load address information from files."
"Save the files from the 1C website http://its.1c.ru/download/fias2 to disk, and then load them into the program."
""
"Technical information:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
		ВывестиСообщениеОбОшибке(ТекстОшибки);
		Возврат;
...
Процедура ЗавершениеЗагрузки()
	...
	ТекстОписанияЗагрузки = НСтр("ru = 'Адресный классификатор успешно загружен.'; en = 'Address classifier was successfully imported.'");
...
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, ЗагружатьИсторию, УникальныйИдентификатор, ВременныйКаталог)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Загрузка адресного классификатора с сайта'; en = 'Import address classifier from site'");
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Адресный классификатор'; en = 'Address classifier'", ОбщегоНазначения.КодОсновногоЯзыка());
