	...
	Пока ЗаполненаДата Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСтроки) Тогда
			...
			Если ВсеПоляЗаполненыКорректно Тогда
				Запись = РегистрыСведений.уатОтносительныеКурсыВалют.СоздатьМенеджерЗаписи();
...
Процедура ИнициализироватьПараметрыЗагрузки()
	...
	МакетШаблона = РегистрыСведений.уатОтносительныеКурсыВалют.ПолучитьМакет("МакетЗагрузкиКурсов");
