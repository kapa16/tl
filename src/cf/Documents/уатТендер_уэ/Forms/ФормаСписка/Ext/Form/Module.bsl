...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Редактирование", Метаданные.Документы.уатТендер_уэ) Тогда 
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.СтатусТендера Тогда 
...
Процедура СписокВыборПослеВыбораСтатуса(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры) И ТипЗнч(ДополнительныеПараметры) = Тип("ДокументСсылка.уатТендер_уэ") Тогда 
...
Процедура УстановитьСтатусДокумента(ДокументСсылка, НовыйСтатус)
	...
	ДокОб.СтатусТендера = НовыйСтатус;
	
	Если ДокОб.СтатусТендера = Справочники.уатСтатусы_уэ.Закрыт И Не ЗначениеЗаполнено(ДокОб.ДатаОкончания) Тогда 
		ДокОб.ДатаОкончания = ТекущаяДата();
	КонецЕсли;
