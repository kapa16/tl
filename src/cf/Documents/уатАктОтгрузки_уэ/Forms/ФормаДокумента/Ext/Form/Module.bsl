...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.уатЗаказГрузоотправителя") ИЛИ ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.уатМаршрутныйЛист") Тогда
		Если Параметры.Свойство("Склад") И ЗначениеЗаполнено(Параметры.Склад) Тогда
			Объект.Склад = Параметры.Склад;
		КонецЕсли;
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		Если Параметры.Свойство("ТаблицаЗаказы") Тогда // 2 этап ввода на основании - заполнение документа таблицей заказов
			...
			Если Параметры.Свойство("ТаблицаВсеЗаказы") Тогда
				ДокОбъект.ДополнительныеСвойства.Вставить("ТаблицаВсеЗаказы", Параметры.ТаблицаВсеЗаказы);
			...
			ТаблицаЗаказы.ЗагрузитьЗначения(Параметры.ТаблицаЗаказы);
			Если ДокОбъект.ДополнительныеСвойства.Свойство("ВсеГрузыПереданыСоСклада") Тогда
				ВсеГрузыПереданыСоСклада = ДокОбъект.ДополнительныеСвойства.ВсеГрузыПереданыСоСклада;
			КонецЕсли;
			Если Параметры.Свойство("ТаблицаНомераЭтапов") Тогда
				ТаблицаНомераЭтапов = Параметры.ТаблицаНомераЭтапов;
			КонецЕсли;
			
		Иначе // 1 этап ввода на основании - формирование таблицы заказов
			...
			Если ДокОбъект.ДополнительныеСвойства.Свойство("ТаблицаВсеЗаказы") Тогда
				мсвТаблицаВсеЗаказы = ДокОбъект.ДополнительныеСвойства.ТаблицаВсеЗаказы;
				ТаблицаВсеЗаказы.ЗагрузитьЗначения(уатОбщегоНазначенияСервер.ПреобразоватьТаблицувМассивСтрок(мсвТаблицаВсеЗаказы));
			КонецЕсли;
			
			Если ДокОбъект.ДополнительныеСвойства.Свойство("ТаблицаЗаказы") Тогда
				...
				Если ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.уатМаршрутныйЛист") Тогда
					// Теперь подготовим таблицу заменяемых этапов, которая будет показана пользователю.
					// Замена этапов возможна только для создания по Маршрутному листу
					ДокОбъект.ДополнительныеСвойства.ТаблицаЗаказы.Свернуть("НомерИсходногоЭтапа, НомерНовогоЭтапа");
					Для Каждого СтрокаТаблицы Из ДокОбъект.ДополнительныеСвойства.ТаблицаЗаказы Цикл
						Если СтрокаТаблицы.НомерИсходногоЭтапа = СтрокаТаблицы.НомерНовогоЭтапа Тогда
							ДокОбъект.ДополнительныеСвойства.ТаблицаЗаказы.Удалить(СтрокаТаблицы);
						КонецЕсли;
					КонецЦикла;
					ТаблицаНомераЭтапов.ЗагрузитьЗначения(уатОбщегоНазначенияСервер.ПреобразоватьТаблицувМассивСтрок(ДокОбъект.ДополнительныеСвойства.ТаблицаЗаказы));
				КонецЕсли;
				...
				Если мсвСклады.Количество() > 1 Тогда
					...
					Возврат;
				ИначеЕсли ДокОбъект.ДополнительныеСвойства.Свойство("ВсеГрузыПереданыСоСклада") Тогда
					ВсеГрузыПереданыСоСклада = ДокОбъект.ДополнительныеСвойства.ВсеГрузыПереданыСоСклада;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.уатАктПриемки_уэ") Тогда
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		ДокОбъект.ДополнительныеСвойства.Вставить("ИнтерактивныйВвод", Истина);
		ДокОбъект.Заполнить(Параметры.Основание);
		ЗначениеВРеквизитФормы(ДокОбъект, "Объект");
		Если ДокОбъект.ДополнительныеСвойства.Свойство("ВсеГрузыПереданыСоСклада") Тогда
			ВсеГрузыПереданыСоСклада = ДокОбъект.ДополнительныеСвойства.ВсеГрузыПереданыСоСклада;
	...
	Если Объект.ПринадлежностьПеревозки = ПредопределенноеЗначение("Перечисление.уатПринадлежностьТС.Привлеченное") Тогда
		Элементы.Перевозчик.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
	Иначе
		Элементы.Перевозчик.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Организации");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если мсвСклады.Количество() > 1 Тогда
		ПараметрыФормы = Новый Структура("мсвСклады, ТаблицаЗаказы, ДокументОснование, СоздаватьДокумент", мсвСклады, ТаблицаЗаказы, ДокументОснование, 1);
		Если ТаблицаВсеЗаказы.Количество() > 0 Тогда
			ПараметрыФормы.Вставить("ТаблицаВсеЗаказы", ТаблицаВсеЗаказы);
		КонецЕсли;
		Если ТаблицаНомераЭтапов.Количество() > 0 Тогда
			ПараметрыФормы.Вставить("ВозможнаЗаменаЭтапов", Истина);
		КонецЕсли;
		ОткрытьФорму("Документ.уатАктПриемки_уэ.Форма.ФормаВыборСклада", ПараметрыФормы, ЭтаФорма.ВладелецФормы);
		Отказ = Истина;
		Возврат;
	ИначеЕсли ВсеГрузыПереданыСоСклада И Объект.Грузы.Количество() = 0 Тогда
		ТекстНСТР = НСтр("en='Tabular section is not filled! There are no pending transfer cargo.';ru='Табличная часть не заполнена! Отсутствуют ожидаемые к передаче грузы.'");
	...
	Если ТаблицаНомераЭтапов.Количество() > 0 Тогда
		ЭтаФорма.ПодключитьОбработчикОжидания("ЗадатьВопросЗаменаЭтапов", 0.5, Истина);
	КонецЕсли;	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Изменен акт отгрузки" И Параметр = Объект.Ссылка Тогда 
		Прочитать();
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.Грузополучателю")
		ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ПрочееСписание"))
...
Процедура ПринадлежностьПеревозкиПриИзменении(Элемент)
	...
	Иначе
		Элементы.Перевозчик.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Организации");
		Объект.Перевозчик                   = ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка");
	КонецЕсли;
...
Процедура МаршрутныйЛистПриИзменении(Элемент)
	...
	РеквизитыМаршрутногоЛиста = ПолучитьДанныеМаршрутногоЛиста(Объект.МаршрутныйЛист);
	ЗаполнитьЗначенияСвойств(Объект, РеквизитыМаршрутногоЛиста);
	Если Объект.ПринадлежностьПеревозки = ПредопределенноеЗначение("Перечисление.уатПринадлежностьТС.Привлеченное") Тогда
		Элементы.Перевозчик.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
	Иначе
		Элементы.Перевозчик.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Организации");
...
Процедура Водитель1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Объект.ПринадлежностьПеревозки = ПредопределенноеЗначение("Перечисление.уатПринадлежностьТС.Привлеченное") Тогда
			ПараметрФормы = Новый Структура("РежимВыбора, Отбор", Истина, Новый Структура("Контрагент", Объект.Перевозчик));
...
Процедура СтатусДокументаНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура("Отбор", Новый Структура("ДействуетНаАктыОтгрузки", Истина));
...
Процедура ОперацияВыполненаПриИзменении(Элемент)
	Если Объект.ОперацияВыполнена = Ложь 
		...
		ТекстНСТР = НСтр("en='Operation start and finish times will be cleared. Continue?';ru='Даты начала и окончания отгрузки будут очищены. Продолжить?'");
...
Процедура ГрузыПриАктивизацииСтроки(Элемент)
	...
	Если ТекСтрока = Неопределено ИЛИ НЕ ЗначениеЗаполнено(ТекСтрока.Заказ) Тогда
		...
		Элементы.ГрузыГрузовоеМесто.РежимВыбораИзСписка = Ложь;
		Возврат;
	Иначе
		Элементы.ГрузыГрузовоеМесто.КнопкаВыбора = Ложь;
		Элементы.ГрузыГрузовоеМесто.РежимВыбораИзСписка = Истина;
	КонецЕсли;
...
Процедура ВыполнитьОперацию(Команда)
	...
	Если РазделятьПланФакт И ЗначениеЗаполнено(Объект.НачалоОперации) И Объект.Грузы.Итог("Количество") = 0 Тогда
		ТекстНСТР = НСтр("en='There is no cargo to be transferred. Fill factual data from planned data?';ru='Количество к отгрузке по факту: 0. Заполнить факт из плана?'");
...
&НаКлиенте
Процедура ОперацияВРейсВПункте(Команда)
	НовыйВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте");
	ВидОперацииПриИзменении(НовыйВидОперации);
КонецПроцедуры

&НаКлиенте
Процедура ОперацияВРейсНаСкладе(Команда)
	НовыйВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейс");
	ВидОперацииПриИзменении(НовыйВидОперации);
КонецПроцедуры

&НаКлиенте
Процедура ОперацияГрузополучателю(Команда)
	НовыйВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.Грузополучателю");
	ВидОперацииПриИзменении(НовыйВидОперации);
КонецПроцедуры

&НаКлиенте
Процедура ОперацияПрочееСписание(Команда)
	НовыйВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ПрочееСписание");
	ВидОперацииПриИзменении(НовыйВидОперации);
КонецПроцедуры
...
Процедура ПоказатьПолучателейУведомлений(Команда)
	...
	ПараметрыФормы.Вставить("ТипДокумента", ПредопределенноеЗначение("Перечисление.уатТипыОбъектовДляУведомлений_уэ.АктОтгрузки"));
...
Процедура УстановитьВидимость()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейс")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте") Тогда 
		Элементы.МаршрутныйЛист.Видимость          = Истина;
		Элементы.ПринадлежностьПеревозки.Видимость = Истина;
		Элементы.Перевозчик.Видимость              = Истина;
		Элементы.Водитель1.Видимость               = Истина;
	Иначе 
		Элементы.МаршрутныйЛист.Видимость          = Ложь;
		Элементы.ПринадлежностьПеревозки.Видимость = Ложь;
		Элементы.Перевозчик.Видимость              = Ложь;
		Элементы.Водитель1.Видимость               = Ложь;
	КонецЕсли;
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте") Тогда
	...
	Элементы.ФормаОперацияВРейсВПункте.Пометка    = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте");
	Элементы.ФормаОперацияВРейсНаСкладе.Пометка   = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейс");
	Элементы.ФормаОперацияГрузополучателю.Пометка = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.Грузополучателю");
	Элементы.ФормаОперацияПрочееСписание.Пометка  = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ПрочееСписание");
...
&НаСервере
Процедура ОбновитьФактическиеПоказателиСервер()
	
	ТаблицаГрузовПлан = Объект.Грузы.Выгрузить(,"ГрузовоеМесто, Заказ, Количество, ЕдиницаИзмерения, НомерСтроки");
	ТаблицаГрузовПлан.Колонки.Найти("Количество").Имя = "КоличествоПлан";
	НоваяТаблицаГрузовПлан = Документы.уатАктПриемки_уэ.ПолучитьВесоОбъемныеПоказателиПлан(ТаблицаГрузовПлан);
...
Процедура ПодборЗаказовОткрытьФормуПодбора()
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ДокументПодбора",    "уатАктОтгрузки_уэ");
	ПараметрыФормы.Вставить("ТекущийАктОтгрузки", Объект.Ссылка);
...
Процедура УстановитьУсловноеОформлениеПолейСтатусов()
	...
	|			ИЛИ уатСтатусы_уэ.ДействуетНаАктыОтгрузки)";
...
&НаКлиенте
Процедура ЗадатьВопросЗаменаЭтапов()
	ТекстРусский = СтрШаблон("На складе %1 будет выполена замена этапов:", Объект.Склад) + Символы.ПС;
	ТекстАнглийский = СтрШаблон("At warehouse %1 some stages will be changed:", Объект.Склад) + Символы.ПС;
	Для Каждого СтрокаТаблицы Из ТаблицаНомераЭтапов Цикл
		НомерИсходногоЭтапа = СтрокаТаблицы.Значение.НомерИсходногоЭтапа;
		НомерНовогоЭтапа = СтрокаТаблицы.Значение.НомерНовогоЭтапа;
		ТекстРусский    = ТекстРусский    + "Этап "  + НомерИсходногоЭтапа + " -> Этап "  + НомерНовогоЭтапа + Символы.ПС;
		ТекстАнглийский = ТекстАнглийский + "Stage " + НомерИсходногоЭтапа + " -> Stage " + НомерНовогоЭтапа + Символы.ПС;
	КонецЦикла;
	ТекстРусский = ТекстРусский + "Продолжить?";
	ТекстАнглийский = ТекстАнглийский + "Continue?";
	ТекстНСТР = НСтр("en='" + ТекстАнглийский + "';ru='" + ТекстРусский + "'");
	Оповещение = Новый ОписаниеОповещения("ЗаменаЭтапаПослеЗакрытияВопроса", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, ТекстНСТР, РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Да);
КонецПроцедуры

&НаКлиенте
Процедура ЗаменаЭтапаПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		СоздатьПереносОстатков();
	ИначеЕсли Результат = КодВозвратаДиалога.Нет Тогда
		
		// Удаляем все заказы, которые числятся на складе по этапам не из текущего маршрутного листа
		мсвЗаказы = Новый Массив;
		Для Каждого ТекСтрока Из ТаблицаЗаказы Цикл
			Если Не ТекСтрока.Значение.ЗаказВМаршрутномЛисте Тогда
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("Заказ", ТекСтрока.Значение.ЗаказГрузоотправителя);
				ПараметрыОтбора.Вставить("ГрузовоеМесто", ТекСтрока.Значение.ГрузовоеМесто);
				СтрокиДляУдаления = Объект.Грузы.НайтиСтроки(ПараметрыОтбора);
				Для Каждого СтрокаТЧГрузы Из СтрокиДляУдаления Цикл
					СтрокаТЧГрузы.Количество = 0;
					СтрокаТЧГрузы.КоличествоПлан = 0;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
&НаКлиенте
Процедура СоздатьПереносОстатков()
	ПараметрыОперации = Новый Структура;
	ПараметрыОперации.Вставить("Склад", Объект.Склад);
	ПараметрыОперации.Вставить("Организация", Объект.Организация);
	ПараметрыОперации.Вставить("ВидОперации", ПредопределенноеЗначение("Перечисление.уатВидыОперацийОперацияСГрузом_уэ.ПереносОстатков"));
	ПараметрыОперации.Вставить("ГрузыДляПереноса", ТаблицаЗаказы);
	ПараметрыФормы = Новый Структура("Основание", ПараметрыОперации);
	ОткрытьФорму("Документ.уатОперацияСГрузом_уэ.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма, Объект.Склад,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ВидОперацииПриИзменении(НовыйВидОперации)
	...
	// Изменение статуса грузов с "ВыданСоСклада" на "ВыданВРейс" или наоборот, при необходимости
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейс")
		И Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте") Тогда
		Объект.МаршрутныйЛист = Неопределено;
		Объект.ПринадлежностьПеревозки = Неопределено;
		Объект.Перевозчик = Неопределено;
		Объект.Водитель1 = Неопределено;
	КонецЕсли;
...
Функция СтатусНовогоГруза(ВидОперации)
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.уатВидыОперацийАктОтгрузки_уэ.ВРейсВПункте") Тогда
		Статус = ПредопределенноеЗначение("Справочник.уатСтатусы_уэ.ВыданСоСкладаКлиента");
	Иначе
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеМаршрутногоЛиста(МаршрутныйЛист);
	
	Результат = Новый Структура();
	Результат.Вставить("Водитель1", МаршрутныйЛист.Водитель1);
	Результат.Вставить("Перевозчик", МаршрутныйЛист.Контрагент);
	Результат.Вставить("ПринадлежностьПеревозки", МаршрутныйЛист.ПринадлежностьПеревозки);
	Если Не ЗначениеЗаполнено(Результат.ПринадлежностьПеревозки) Тогда
		Результат.ПринадлежностьПеревозки = Перечисления.уатПринадлежностьТС.Собственное;
	КонецЕсли;
