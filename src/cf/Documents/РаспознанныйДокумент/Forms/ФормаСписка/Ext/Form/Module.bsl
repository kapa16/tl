...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьРаспознаваниеДокументов") Тогда
		ВызватьИсключение НСтр("ru = 'Функциональная опция ИспользоватьРаспознаваниеДокументов отключена.'; en = 'The ИспользоватьРаспознаваниеДокументов functional option is disabled.'");
	КонецЕсли;
	...
	БалансПредставление = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = 'Пожалуйста подождите...'; en = 'Please wait...'"));
	...
	Если Поле.Имя = "СтатусРаспознавания" Тогда
		...
		Если СтатусРаспознавания = 0 Тогда
			Если Элемент.ТекущиеДанные.ЭтоЗаданиеРаспознавания Тогда 
				Статус = НСтр("ru = 'Задание распознавания отменено.'; en = 'The recognition job is canceled.'");
			Иначе 
				Статус = НСтр("ru = 'Установлена пометка на удаление.'; en = 'The deletion mark is set.'");
			КонецЕсли;
		ИначеЕсли СтатусРаспознавания = 1 Тогда
			Статус = НСтр("ru = 'Ошибка распознавания.'; en = 'Recognition error.'");
		ИначеЕсли СтатусРаспознавания = 4 Тогда
			Статус = НСтр("ru = 'Документ обработан.'; en = 'The document is processed.'");
		ИначеЕсли СтатусРаспознавания = 2 Тогда
			Статус = НСтр("ru = 'Документ распознан и ожидает проверки.'; en = 'The document is recognized and ready for verification.'");
		ИначеЕсли СтатусРаспознавания = 3 Тогда
			Статус = НСтр("ru = 'Документ в процессе распознавания. Пожалуйста, подождите...'; en = 'Recognizing the document. Please wait...'");
		КонецЕсли;
	...
	Если ТекущиеДанные.ЭтоЗаданиеРаспознавания Тогда
		Если ТекущиеДанные.ПометкаУдаления Тогда
			ПоказатьПредупреждение(,
				НСтр("ru = 'Нельзя сохранить файл отмененного задания распознавания.'; en = 'Cannot save the canceled recognition job file.'"));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru = 'Нельзя сохранить в процессе обработки."
"Дождитесь окончания распознавания и повторите.'; en = 'Cannot save when processing."
...
Процедура ПоказатьИдентификаторДокумента(Команда)
	...
	Если ТипЗнч(Элементы.Список.ТекущиеДанные) = Тип("ДанныеФормыСтруктура") Тогда
		...
		ПараметрыСообщения.Вставить("Заголовок", НСтр("ru = 'Идентификатор документа'; en = 'Document ID'"));
...
Процедура ВводКомплектовДокументов(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда  
		...
		ПараметрыОткрытия.Заголовок = НСтр("ru = 'Список комплектов'; en = 'List of sets'");
		ПараметрыОткрытия.ИмяФормы = "Обработка.РаспознаваниеДокументов.Форма.СписокКомплектов";
		
		ПараметрыФормы = Новый Структура("ОтборИзСписка", ОтборИзСписка);
		МодульОбщегоНазначенияБПКлиент.ОткрытьФормуСОжиданием(ПараметрыОткрытия, ПараметрыФормы);
...
Процедура СохранитьАдресЭлПочты(Команда)
	...
	Попытка
		...
		ПараметрыВопроса.Заголовок = НСтр("ru = 'Подписка оформлена'; en = 'Subscription is created'");
		
		РаспознаваниеДокументовСлужебныйКлиент.ПоказатьВопросПользователю(
			Неопределено,
			НСтр("ru = 'Адрес электронной почты подписан на уведомления'; en = 'Email address is subscribed to notifications'"),
...
Процедура ПриВыполненииПомещенияФайлов(ПомещаемыйФайл, Помещено, ОтказОтПомещенияФайла, ПомещеноВсего, ОтказОтПомещенияВсехФайлов, Контекст) Экспорт
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = '%1 (%2 Мб)'; en = '%1 (%2 MB)'"), ПомещаемыйФайл.Имя, РазмерДанных);
	Состояние(НСтр("ru = 'Загрузка'; en = 'Loading'"), ПомещеноВсего, ТекстСообщения);
...
Процедура ИзменитьТекстПодсказки(ВключитьФильтр, ИмяКоманды)
	...
	Если ВключитьФильтр Тогда
		ТекстПодсказки = СтрЗаменить(ТекстПодсказки, НСтр("ru = 'Показать'; en = 'Show'"), НСтр("ru = 'Скрыть'; en = 'Hide'"));
	Иначе
		ТекстПодсказки = СтрЗаменить(ТекстПодсказки, НСтр("ru = 'Скрыть'; en = 'Hide'"), НСтр("ru = 'Показать'; en = 'Show'"));
	КонецЕсли;
...
Процедура Подключаемый_ОбновитьСписок()
	...
	Элементы.СписокФильтрСтатусВОбработке.Заголовок = СтрШаблон(НСтр("ru = 'В обработке: %1'; en = 'Processing: %1'"), СостоянияПоСтатусам.ФайловКОбработке);
	Элементы.СписокФильтрСтатусНовые.Заголовок = СтрШаблон(НСтр("ru = 'Распознанные: %1'; en = 'Recognized: %1'"), СостоянияПоСтатусам.ФайловНовых);
	Элементы.СписокФильтрСтатусСОшибкой.Заголовок = СтрШаблон(НСтр("ru = 'С ошибкой: %1'; en = 'With error: %1'"), СостоянияПоСтатусам.ФайловСОшибкой);
	Элементы.СписокФильтрСтатусОбработан.Заголовок = СтрШаблон(НСтр("ru = 'Обработанные: %1'; en = 'Processed: %1'"), СостоянияПоСтатусам.ДокументовОбработано);
...
Функция НачатьПолучениеСостояниеБаланса(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение состояния по балансу в фоне'; en = 'Receive states by balance in the background'");
...
Процедура ОтрисоватьПредставлениеБалансаНаКлиенте(СостоянияПоБалансу)
	...
	Если СостоянияПоБалансу = Неопределено Тогда
		...
		БалансЗаголовок = НСтр("ru = 'Проверка статуса подключения'; en = 'Connection status check'");
	...
	Если СостоянияПоБалансу.СостояниеБаланса = "Ошибка" Тогда
		...
		БалансЗаголовок = НСтр("ru = 'Проверка не удалась'; en = 'Check failed'");
	...
	Если СостоянияПоБалансу.СостояниеБаланса = "НеАвторизован" Тогда
		...
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
		БалансЗаголовок = НСтр("ru = 'Подключить сервис'; en = 'Enable service'");
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "НеАктивирован" Тогда
		...
		БалансЗаголовок = НСтр("ru = 'Заявка отправлена'; en = 'Request is sent'");
...
""
"На адрес [АдресЭлПочты] мы отправим уведомление по факту подключения.'; en = 'The request to activate the document recognition service has been sent"
...
"We will send you an activation notification to [АдресЭлПочты].'"),
			...
			// {УАТ}
			Элементы.АдресЭлПочты.Заголовок = НСтр("ru = 'Возможна замена e-mail адреса для уведомления'; en = 'You can change the e-mail for notification'");
			// {/УАТ}
			Элементы.СохранитьАдресЭлПочты.Заголовок = НСтр("ru = 'Заменить'; en = 'Replace'");
		КонецЕсли;
		
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "ПилотныйРежим" Тогда
		
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Информация;
		БалансЗаголовок = НСтр("ru = 'Пилотный режим'; en = 'Pilot mode'");
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "ПилотныйРежимДатаОкончания" Тогда
		...
		БалансЗаголовок = РаспознаваниеДокументовСлужебныйКлиентСервер.ВставитьПараметрыВСтроку(
			НСтр("ru = 'Пилотный режим до [ДатаОкончания]'; en = 'Pilot mode until [ДатаОкончания]'"),
			ПараметрыЗамены
		);
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "БалансНулевой" Тогда
		...
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
		БалансЗаголовок = РаспознаваниеДокументовСлужебныйКлиентСервер.ВставитьПараметрыВСтроку(
			НСтр("ru = 'Баланс: [БалансПрописью]'; en = 'Balance: [БалансПрописью]'"),
			ПараметрыЗамены
		);
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "БалансОтрицательный" Тогда
		...
		Элементы.ГруппаБиллингПодробно.ЦветТекстаЗаголовка = WebЦвета.Красный;
		БалансЗаголовок = РаспознаваниеДокументовСлужебныйКлиентСервер.ВставитьПараметрыВСтроку(
			НСтр("ru = 'Кредит: использовано [Использовано] из [КредитПрописьюРодительный]'; en = 'Credit: used [Использовано] out of [КредитПрописьюРодительный]'"),
			ПараметрыЗамены
		);
	...
	ИначеЕсли СостоянияПоБалансу.СостояниеБаланса = "СервисОтключен" Тогда
		...
		Элементы.Биллинг.Картинка = БиблиотекаКартинок.Предупреждение;
		Элементы.ГруппаБиллингПодробно.ЦветТекстаЗаголовка = WebЦвета.Красный;
		БалансЗаголовок = НСтр("ru = 'Сервис отключен'; en = 'The service is disabled'");
...
Функция ПредставлениеБаланса(Знач Баланс, Знач Падеж = "Именительный")
	...
	Если Баланс = 0 Тогда
		Возврат НСтр("ru = '0 страниц'; en = '0 pages'");
	КонецЕсли;
...
Процедура ОтметитьОбработаноНаСервере(ДокументСсылка)
	...
	Попытка
		...
		Если ДокументОбъект.ПометкаУдаления Тогда 
			ВызватьИсключение НСтр("ru = 'Невозможно отправить в обработанные помеченный на удаление документ.'; en = 'Cannot send a document marked for deletion to the processed ones.'");
		КонецЕсли;
...
Функция НачатьПолучениеБаннера(ПоказатьПредыдущий)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение баннера в фоне'; en = 'Receive banner in the background'");
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'За все время'; en = 'For all time'"));
	ЭлементОформления.Использование = Истина;
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Не определено'; en = 'Undefined'"));
	ЭлементОформления.Использование = Истина;
