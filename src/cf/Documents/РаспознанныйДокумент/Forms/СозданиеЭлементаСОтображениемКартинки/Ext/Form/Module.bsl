...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИмяСправочника = "НомераГТД" Тогда
		Заголовок = "Номер ГТД " + НСтр("ru = '(Создание)'; en = '(Create)'");
	Иначе
		Заголовок = Строка(ТипЗнч(СоздаваемыйОбъект)) + " " + НСтр("ru = '(Создание)'; en = '(Create)'");
	КонецЕсли;
...
Процедура Создать(Команда)
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
		Если РеквизитСоздаваемогоОбъекта.Обязательный И Не ЗначениеЗаполнено(РеквизитСоздаваемогоОбъекта.Значение) Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""%1"" не заполнено'; en = 'Field ""%1"" is not filled in'");
...
Процедура ПроверитьНомерСчетаБИК()
	...
	Если ЗначениеЗаполнено(Банк) Тогда
		...
		БИКБанкРаспознанный = РаспознаваниеДокументовСлужебный.ИзвлечьЦифрыИзСтроки(БанкРаспознанныйТекст);
	Иначе 
		БИКБанк = РаспознаваниеДокументовСлужебный.ИзвлечьЦифрыИзСтроки(БанкРаспознанныйТекст);
		БИКБанкРаспознанный = БИКБанк;
	КонецЕсли;
	
	Если БИКБанк <> БИКБанкРаспознанный Тогда
...
Процедура СоздатьБанкПоБИК(БИК)
	...
	Если ЗначениеЗаполнено(БИК) Тогда
		...
		Если Не ЗначениеЗаполнено(Банк) Тогда
			...
			// {УАТ}
			БанкИзКлассификатора = уатРаспознаваниеДокументов.СсылкаПоКлассификатору(БИК);
			// {/УАТ} 
			Если ЗначениеЗаполнено(БанкИзКлассификатора) Тогда
				// {УАТ}
				уатРаспознаваниеДокументов.ПодобратьБанкИзКлассификатора(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(БанкИзКлассификатора));
				// {/УАТ}
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ЕдиницаИзмерения.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.КлассификаторЕдиницИзмерения");
...
Процедура ОтобразитьРезультатПроверкиКонтрагентовВФорме()
	...
	// {УАТ}
	// ИНН
	ЭтотОбъект.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(ЭтотОбъект.РезультатПроверкиИНН);
	
	// КПП
	ЭтотОбъект.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(ЭтотОбъект.РезультатПроверкиКПП);
	
	// Дубли и ФНС
	МассивПодстрокДублиФНС = Новый Массив;
	Если НЕ ПустаяСтрока(ЭтотОбъект.РезультатПроверкиНаДубли) Тогда
		МассивПодстрокДублиФНС.Добавить(ЭтотОбъект.РезультатПроверкиНаДубли);
	КонецЕсли;
	ЭтотОбъект.НадписьПояснениеПроверкиДублейФНС = Новый ФорматированнаяСтрока(МассивПодстрокДублиФНС);
	// {/УАТ}
	УстановитьВидимостьЭлементов();
