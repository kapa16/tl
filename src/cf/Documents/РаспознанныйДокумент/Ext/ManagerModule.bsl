...
// ОтборИзСписка: "РаспознанныеДокументы, ТекущаяДата, ТекущаяДатаПериод, ТекущаяОрганизация, ТекущийКонтрагент"
//
Функция ПолучитьДокументыПоОтбору(ОтборИзСписка) Экспорт
	...
	|	И РаспознанныйДокумент.ТипДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.НеопределенныйДокумент)
	|	И РаспознанныйДокумент.ТипДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.ПустаяСсылка)
	|	И РаспознанныйДокумент.ТипДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.КассовыйЧек)
	|	И РаспознанныйДокумент.ТипДокумента <> ЗНАЧЕНИЕ(Перечисление.ТипыДокументовРаспознаваниеДокументов.УКД)
	...
	|	&НеСопоставлен + (ВЫРАЗИТЬ(СтрокаНомерДокумента.РаспознанныйТекст КАК СТРОКА(1024))) КАК НомерДокументаРаспознано,
	|	ОсновнаяТаблица.ДатаДокумента КАК ДатаДокумента,
	|	&НеСопоставлен + (ВЫРАЗИТЬ(СтрокаДатаДокумента.РаспознанныйТекст КАК СТРОКА(1024))) КАК ДатаДокументаРаспознано,
	|	ОсновнаяТаблица.Контрагент КАК Контрагент,
	|	&НеСопоставлен + (ВЫРАЗИТЬ(СтрокаКонтрагент.РаспознанныйТекст КАК СТРОКА(1024))) КАК КонтрагентРаспознано,
	|	ОсновнаяТаблица.Организация КАК Организация,
	|	&НеСопоставлен + (ВЫРАЗИТЬ(СтрокаОрганизация.РаспознанныйТекст КАК СТРОКА(1024))) КАК ОрганизацияРаспознано,
	|	ОсновнаяТаблица.ИтогоВсего КАК ИтогоВсего,
	|	&НеСопоставлен + (ВЫРАЗИТЬ(СтрокаИтогоВсего.РаспознанныйТекст КАК СТРОКА(1024))) КАК ИтогоВсегоРаспознано,
	|	ОсновнаяТаблица.ДанныеЗаполнены КАК ДанныеЗаполнены,
	|	ОсновнаяТаблица.Наименование КАК Наименование,
	|	ЕСТЬNULL(СтрокаВидСкидки.Значение, 0) <> 0 КАК ЕстьСкидка
	|ИЗ
	...
	Запрос.УстановитьПараметр("НеСопоставлен", НСтр("ru = 'Не сопоставлен: '; en = 'Not mapped:'"));
...
Функция НаименованиеРаспознанногоДокумента(ДокументОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ТипДокумента) Тогда
		...
		ИначеЕсли ДокументОбъект.ВариантОбработки = Перечисления.ВариантыОбработкиЗаданияРаспознавания.СчетНаОплатуПоставщика Тогда
			ПредставлениеОперации = НСтр("ru = 'Счет от поставщика'; en = 'Vendor invoice'");
		Иначе
			ПредставлениеОперации = НСтр("ru = 'Документ'; en = 'Document'");
		КонецЕсли;
		...
		ПредставлениеДаты = Формат(ДокументОбъект.ДатаДокумента, "ДЛФ=D");
		...
		Если Не ЕстьНомер И Не ЕстьДата И Не ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 из %2'; en = '%1 of %2'"),
		...
		Если Не ЕстьНомер И Не ЕстьДата И ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 на сумму %2'; en = '%1 for the amount of %2'"),
				ПредставлениеОперации, ДокументОбъект.СуммаДокумента);
		КонецЕсли;
		
		// 0 1 0
		// Счет от поставщика от 22.12.2016
		Если Не ЕстьНомер И ЕстьДата И Не ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 от %2'; en = '%1 of %2'"),
		...
		Если Не ЕстьНомер И ЕстьДата И ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 от %2 на сумму %3'; en = '%1 dated %2 in the amount of %3'"),
				ПредставлениеОперации, ПредставлениеДаты, ДокументОбъект.СуммаДокумента);
		КонецЕсли;
		
		// 1 0 0
		// Документ № 587
		Если ЕстьНомер И Не ЕстьДата И Не ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 № %2'; en = '%1 No. %2'"),
		...
		Если ЕстьНомер И Не ЕстьДата И ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 № %2 на сумму %3'; en = '%1 No. %2 in the amount of %3'"),
				ПредставлениеОперации, ДокументОбъект.НомерДокумента, ДокументОбъект.СуммаДокумента);
		КонецЕсли;
		
		// 1 1 0
		// Поступление № 587 от 22.12.2016
		Если ЕстьНомер И ЕстьДата И Не ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 № %2 от %3'; en = '%1 No. %2 of %3'"),
		...
		Если ЕстьНомер И ЕстьДата И ЕстьСумма Тогда
			Наименование = СтрШаблон(НСтр("ru = '%1 № %2 от %3 на сумму %4'; en = '%1 No. %2 dated %3 in the amount of %4'"),
				ПредставлениеОперации, ДокументОбъект.НомерДокумента, ПредставлениеДаты, ДокументОбъект.СуммаДокумента);
		КонецЕсли;
	...
	ИначеЕсли ДокументОбъект.ТребуетсяОплата Тогда
		...
		Наименование = СтрШаблон(НСтр("ru = 'Требуется оплата для обработки %1'; en = 'Payment is required to process %1'"), ДокументОбъект.ИмяФайла);
	Иначе
		...
		Наименование = СтрШаблон(НСтр("ru = 'Ошибка при обработке %1'; en = '%1 processing error'"), ДокументОбъект.ИмяФайла);
	КонецЕсли;
	...
	Пока СледующаяПорция Цикл 
		...
		Попытка
			...
			Пока Выборка.Следующий() Цикл
				...
				Исключение
					ДокументыСПроблемами.Добавить(Выборка.Ссылка);
...
"%1'; en = 'Cannot mark the document for deletion as outdated:"
"%1'"),
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				Документы.РаспознанныйДокумент, ,
				СтрШаблон(
					НСтр("ru = 'Не удалось пометить на удаление документы в порции:"
"%1'; en = 'Cannot mark documents in the batch for deletion:"
"%1'"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
				)
			);
...
"%1'; en = 'Cannot delete the recognized documents:"
"%1'"),
...
Функция СоздатьРаспознанныйДокументСОшибкойЗагрузки(ИдентификаторЗадания, ИдентификаторДокумента, ОписаниеОшибки) Экспорт
	...
	НовыйДокумент.Источник = РегистрыСведений.ПредставлениеЗаданийРаспознаваниеДокументов
		.ИсточникПоИдентификатору(ИдентификаторЗадания);
...
Функция СоздатьРаспознанныйДокумент(ДанныеСервиса, ДанныеВыборки, РезультатыОбработки, ДатаСоздания = Неопределено, ДанныеСтраниц = Неопределено) Экспорт
	...
	Если ТипЗнч(ДесериализованноеЗначение) <> Тип("Соответствие") Тогда
		ВызватьИсключение НСтр("ru = 'Ожидалось, что ДесериализованноеЗначение будет типа Соответствие.'; en = 'ДесериализованноеЗначение must have the Соответствие type.'");
	КонецЕсли;
...
"Неизвестный тип документа.'; en = 'Document ID: %1"
"Unknown document type.'"),
	...
	Если ТипЗнч(Печати) = Тип("Массив") Тогда
		Для Каждого Печать Из Печати Цикл
			...
			СтрокаПометки.ОбластьИзображения = РаспознаваниеДокументовКоннекторСлужебный.ИзвлечьДанныеКартинки(
				Печать["chunk_img"]
			);
	...
	НовыйДокумент.ИсходноеИзображение = РаспознаваниеДокументовКоннекторСлужебный.ИзвлечьДанныеКартинки(
		СвойстваШапки["chunk_img"]
	);
	НовыйДокумент.Миниатюра = РаспознаваниеДокументовКоннекторСлужебный.ИзвлечьДанныеКартинки(
	...
	Если ВсеНомераСтрок.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Ожидалось, что будет хотя бы одна строка.'; en = 'Expected at least one line.'");
	КонецЕсли;
...
Функция СоздатьРаспознанныйДокументСОшибкой(ДанныеСервиса, ДанныеВыборки, РезультатыОбработки, ОписаниеОшибки = Неопределено, ДатаСоздания = Неопределено, ДанныеСтраниц = Неопределено) Экспорт
	...
	Если ДесериализованноеЗначение <> Неопределено И ТипЗнч(ДесериализованноеЗначение) = Тип("Соответствие") Тогда
		...
		Если ДесериализованноеЗначение.Получить("thumbnail") <> Неопределено Тогда
			НовыйДокумент.Миниатюра = РаспознаваниеДокументовКоннекторСлужебный
				.ИзвлечьДанныеКартинки(ДесериализованноеЗначение["thumbnail"]);
		КонецЕсли;
		
		Если ДесериализованноеЗначение.Получить("full_img") <> Неопределено Тогда
			НовыйДокумент.ИсходноеИзображение = РаспознаваниеДокументовКоннекторСлужебный
				.ИзвлечьДанныеКартинки(ДесериализованноеЗначение["full_img"]);
		
		ИначеЕсли ДесериализованноеЗначение.Получить("chunk_img") <> Неопределено Тогда
			НовыйДокумент.ИсходноеИзображение = РаспознаваниеДокументовКоннекторСлужебный
				.ИзвлечьДанныеКартинки(ДесериализованноеЗначение["chunk_img"]);
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		НовыйДокумент.Записать();
		РегистрыСведений.РезультатыОбработкиЗаданийРаспознаваниеДокументов
			.ЗаписатьРезультатОбработкиДокумента(ДанныеВыборки.ИдентификаторЗадания, РезультатыОбработки);
		РегистрыСведений.ИсходныеДанныеЗаданийРаспознаваниеДокументов
			.ИзменитьВОбработке(НовыйДокумент.ИдентификаторРезультата, Ложь);
		ЗаписьЖурналаРегистрации(
			РаспознаваниеДокументов.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.РаспознанныйДокумент,
			НовыйДокумент.Ссылка,
			НСтр("ru = 'Записан документ с идентификатором:'; en = 'A document with the identifier was recorded:'") + Символы.ПС + РезультатыОбработки.ИдентификаторРезультата);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			РаспознаваниеДокументов.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru = 'Не удалось записать документ по причине:'; en = 'Cannot save the document due to:'") + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);
		Возврат Неопределено;
	КонецПопытки;
...
Функция СоздатьРаспознанныйДокументV2(СвойстваРаспознанногоДокумента, РаспознанныйДокумент, ИдентификаторЗадания, СостояниеЗадания) Экспорт
	...
	РезультатыОбработки.Вставить("ИдентификаторРезультата", ИдентификаторДокумента);
	...
	Если СвойстваРаспознанногоДокумента.Класс = "КассовыйЧек" Тогда
		...
		ТипОперацииЧек = РаспознаваниеДокументовSDK.СущностьПоКлассу(СвойстваРаспознанногоДокумента, "ТипОперацииЧек").Значение;
		Если ТипОперацииЧек <> 1 Тогда
			НовыйДокумент.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.НеопределенныйДокумент;
		КонецЕсли;
	...
	ЗаполнитьРеквизитыДокументаПоСущностям(НовыйДокумент, СвойстваРаспознанногоДокумента);
	...
	Если НовыйДокумент.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.КассовыйЧек Тогда
		...
		Если НастройкиРаспознавания.АвтоматическиРегистрироватьЧеки Тогда
			РаспознаваниеДокументовПереопределяемый.ЗарегистрироватьЧек(НовыйДокумент, Истина, "Автоматически");
	...
	НовыйДокумент.ДополнительныеСвойства.Вставить("Автозаполнение", Истина);
	НовыйДокумент.ДанныеСервиса = РаспознаваниеДокументовSDK
		.СвойстваРаспознанногоДокументаВJson(СвойстваРаспознанногоДокумента); 
		
	ЗаполнитьДокументыОснованияКорректировки(НовыйДокумент, СвойстваРаспознанногоДокумента);
...
Процедура ЗаполнитьРеквизитыДокументаПоСущностям(НовыйДокумент, СвойстваРаспознанногоДокумента)
	
	ВсеПравила = РаспознаваниеДокументовСлужебный.ПолучитьСопоставлениеРеквизитовИСвойствJSON(НовыйДокумент.ТипДокумента);
	
	Для Каждого СущностьКлючЗначение Из РаспознаваниеДокументовSDK.ВсеСущности(СвойстваРаспознанногоДокумента) Цикл
		...
		Если Сущность.ЭтоТипСоставной Тогда 
			Для Каждого Правило Из ПравилаОбработкиСущностиПоКлассу(Сущность.Класс, ВсеПравила) Цикл
				...
				СущностьИсточникаДанных = РаспознаваниеДокументовSDK
					.СущностьПоСвойствуСоставнойСущности(СвойстваРаспознанногоДокумента, Сущность, Правило.Путь);
		...
		ИначеЕсли Сущность.ЭтоТипТаблица Тогда
			...
			Для Каждого СтрокаТаблицы Из Сущность.Таблица.Строки Цикл
				...
				Для Каждого ЯчейкаТаблицыКлючЗначение Из СтрокаТаблицы Цикл
					...
					Для Каждого Правило Из ПравилаОбработкиЯчейкиТаблицыПоКлассу(ЯчейкаТаблицы.Класс, ВсеПравила) Цикл
		...
		Иначе
			КлассСущности = Сущность.Класс;
			Для Каждого Правило Из ПравилаОбработкиСущностиПоКлассу(КлассСущности, ВсеПравила) Цикл
				ЗаполнитьЗначенияРеквизитаV2(Сущность, НовыйДокумент, Правило);
			КонецЦикла;
		КонецЕсли;
...
Функция ПравилаОбработкиЯчейкиТаблицыПоКлассу(КлассЯчейкиТаблицы, ВсеПравила)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Ошибка вызова.'; en = 'Call error.'");
	КонецЕсли;
...
Процедура ПослеУспешнойЗаписиРаспознанногоДокумента(НовыйДокумент, ИдентификаторЗадания, РезультатыОбработки)
	...
	КандидатыВСвязанныеДокументы = РаспознаваниеДокументовСлужебный.НайтиПотенциальныеСвязанныеДокументы(НовыйДокумент);
...
Процедура ЗаполнитьДокументыОснованияКорректировки(НовыйДокумент, СвойстваРаспознанногоДокумента)
	
	НомераСчетовФактурКорректировки = РаспознаваниеДокументовSDK.СущностьПоКлассу(
		СвойстваРаспознанногоДокумента, "НомераСчетовФактурКорректировки");
	
	Если НомераСчетовФактурКорректировки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДокументыОснованияКорректировок = НомераСчетовФактурКорректировки.Свойства["ДокументыОснованияКорректировок"];
		
	Если ДокументыОснованияКорректировок = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	Для Каждого ДокументОснование Из ДокументыОснованияКорректировок Цикл
		
		НоваяСтрока = НовыйДокумент.ДокументыОснованияКорректировки.Добавить();
		
		НоваяСтрока.НомерДокумента = ДокументОснование["number"];
		НоваяСтрока.ДатаДокумента = ПрочитатьДатуJSON(ДокументОснование["date"], ФорматДатыJSON.ISO);
		НоваяСтрока.РаспознанныйТекст = ДокументОснование["value"];
