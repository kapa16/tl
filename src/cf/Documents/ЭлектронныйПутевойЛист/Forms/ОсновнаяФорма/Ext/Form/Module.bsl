...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// {УАТ}
	Если Параметры.Свойство("ТитулОткрытия") Тогда
		ТитулОткрытия = Параметры.ТитулОткрытия;
	КонецЕсли; 
	// {/УАТ}
	...
	Если НЕ Параметры.Свойство("ФормаБезОбработки") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(
				...
				"ХранимыеДанныеТитулМедосмотрСтороннийМедработник")) Тогда
			ВариантМедработник = 1;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	// {УАТ}
	Если ЗначениеЗаполнено(ТитулОткрытия) Тогда
		Если ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул1") Тогда
			Шаг1ОбработкаНавигационнойСсылки(Элементы.Шаг1, Элементы.Шаг1.Заголовок, Истина);
		ИначеЕсли ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2") Тогда 
			Шаг2ОбработкаНавигационнойСсылки(Элементы.Шаг2, Элементы.Шаг2.Заголовок, Истина);
		ИначеЕсли ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул3") Тогда 
			Шаг3ОбработкаНавигационнойСсылки(Элементы.Шаг3, Элементы.Шаг3.Заголовок, Истина);
		ИначеЕсли ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул4") Тогда 
			Шаг4ОбработкаНавигационнойСсылки(Элементы.Шаг4, Элементы.Шаг4.Заголовок, Истина);
		ИначеЕсли ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул5") Тогда 
			Шаг5ОбработкаНавигационнойСсылки(Элементы.Шаг5, Элементы.Шаг5.Заголовок, Истина);
		ИначеЕсли ТитулОткрытия = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул6") Тогда 
			Шаг6ОбработкаНавигационнойСсылки(Элементы.Шаг6, Элементы.Шаг6.Заголовок, Истина);
        КонецЕсли;
	КонецЕсли;
	// {/УАТ}
...
Процедура ЗаполнитьЗначенияПоУмолчаниюДляТекущегоТитула(ТитулЗаполнения = Неопределено)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка)
		...
		Если Не ЗначениеЗаполнено(ТитулВыездПризнакНачалаРейса) Тогда
			ТитулВыездПризнакНачалаРейсаВКонтейнере = Неопределено;
			...
			СтруктураВерсииТитула.Свойство("ТитулВыездПризнакНачалаРейса", ТитулВыездПризнакНачалаРейсаВКонтейнере);
			ТитулВыездПризнакНачалаРейса = ТитулВыездПризнакНачалаРейсаВКонтейнере;
		КонецЕсли;
...
Процедура ИсправитьТитул(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторФайлаТитула) Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
			ИнформацияПоПрефиксамТитула.ВПрограмме + "НомерВерсии", НовыйНомерВерсии, , , Истина);
...
Процедура НавигацияКартинкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "Шаг3") Тогда
		СформироватьТ3ПоУмолчанию();
	КонецЕсли;
...
Процедура ТитулОформлениеПредыдущийПутевойЛистПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СсылкаПредыдущийПутевойЛист)
		...
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
													"ТитулОформлениеУИДМинтрансПредыдущегоЭПЛ", , , , Истина);
...
Процедура ПриИзмененииПредыдущегоПутевогоЛиста()
	...
	Если ЗначениеЗаполнено(РеквизитыПЛ.УИДМинтранс) Тогда
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
													"ТитулОформлениеУИДМинтрансПредыдущегоЭПЛ", РеквизитыПЛ.УИДМинтранс, , , Истина);	
	Иначе
		Объект.СсылкаПредыдущийПутевойЛист = Неопределено;
		ОбменСГИСЭПДКлиентСервер.УстановитьЗначениеРеквизитаВСтруктуреФормы(ЭтотОбъект, 
													"ТитулОформлениеУИДМинтрансПредыдущегоЭПЛ", , , , Истина);
...
&НаКлиенте
Процедура СформироватьТ3ПоУмолчанию()
