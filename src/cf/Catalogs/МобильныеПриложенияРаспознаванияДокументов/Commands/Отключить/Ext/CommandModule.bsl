...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если Контекст.МобильныеПриложенияКУдалению.Количество() = 1 Тогда
		ТекстВопроса = НСтр("ru = 'Отключить мобильное приложение?'; en = 'Disable the mobile application?'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Отключить выбранные мобильные приложения?'; en = 'Disable the selected mobile applications?'");
	КонецЕсли;
