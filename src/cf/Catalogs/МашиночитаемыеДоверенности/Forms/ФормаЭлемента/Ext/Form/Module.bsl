		...
		Если ТипЗнч(ХронологическиеДанные) = Тип("Структура") Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ХронологическиеДанные.ХронологическиеДанные.ПричиныОтказаВПриемеМЧД) Тогда
				...
				ПараметрыФормы.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Отказ при регистрации в налоговом органе';
																		|en = 'Registration with the tax authority was denied.'"));
	...
	ИначеЕсли ЭтоАдресВременногоХранилища(АдресХронологическихДанных) Тогда
		...
		Если ТипЗнч(ХронологическиеДанные) = Тип("Структура") Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ru = 'Хронологические данные регистрации доверенности в налоговом органе';
											|en = 'Historical data of the LOA registration with the tax authority'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнениеПроверено()
	...
	Если Объект.ПолномочияВТекстовомВиде И СтрДлина(Объект.ТекстПолномочий) > 10000 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Текстовые полномочия не должны превышать 10000 символов.';
													|en = 'The description of one''s powers cannot exceed 10,000 characters.'"),,,"Объект.ТекстПолномочий", Отказ);
	КонецЕсли;
		...
		Иначе
			Если СтрДлина(Объект.КодНалоговогоОрганаПредставления) <> 4
				...
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код налогового органа представления должен содержать 4 цифры.';
															|en = 'A tax authority''s code must be exactly 4 digits.'"),,,"Объект.КодНалоговогоОрганаПредставления", Отказ);
			КонецЕсли;
		КонецЕсли;
		Для Каждого НалоговыйОрганДействия Из Объект.НалоговыеОрганыДействия Цикл
			Если СтрДлина(НалоговыйОрганДействия.КодНалоговогоОргана) <> 4
				...
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Код налогового органа, в котором действуют полномочия, должен содержать 4 цифры. Неверный код: %1';
						|en = 'A tax authority''s code must be exactly 4 digits. The code is invalid: %1'"), 
			...
			Если СтрНачинаетсяС(СтрокаПолномочий.Код, "1_FNS") Тогда
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Доверенность содержит устаревший код налоговых полномочий %1. Коды налоговых полномочий должны начинаться с 2_FNS.';
						|en = 'The letter of authority contains an outdated tax authority code %1. Tax authority codes must start with 2_FNS.'"),
...
Процедура ДоступностьПризнакаРегистрацииВРеестре(Форма)
	...
	Если Форма.Элементы.РегистрироватьВРеестре.ТолькоПросмотр И Форма.Объект.РегистрироватьВРеестре
		...
		ЗаголовокРегистрироватьВРеестре = НСтр(
			...
			|en = 'Registered in the <a href = ""https://m4d.nalog.gov.ru/EMCHD/check-status?guid=%1"">FTS distributed ledger</a>'");
	Иначе
		ЗаголовокРегистрироватьВРеестре = НСтр(
			"ru = 'Регистрировать в <a href = ""https://m4d.nalog.gov.ru/"">распределенном реестре ФНС России</a>';
			|en = 'Register in the <a href = ""https://m4d.nalog.gov.ru/"">FTS distributed ledger</a>'");
	КонецЕсли;
		...
		Форма.Элементы.ОписаниеСостояния.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">Отменена в реестре ФНС</span>';
					|en = '<span style=""color: %1"">Canceled in the FTS registry</span>'"), "ПоясняющийТекст"));
		Возврат;
	КонецЕсли;
	...
	Если ДанныеСостояния.ОтображатьСтатусВДокументе И Объект.РегистрироватьВРеестре И Объект.ДляНалоговыхОрганов Тогда
		Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные) Тогда
			...
			Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.Ошибка) Тогда
				...
				|en = '<a href = ""ХронологическиеДанныеОшибка"">Failed to retrieve registration information from the tax authority</a>'"));
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.ХронологическиеДанные) Тогда
				...
				Иначе
					ОписаниеСобытия = НСтр("ru = 'Не удалось получить информацию о регистрации в налоговом органе';
											|en = 'Failed to retrieve registration information from the tax authority'");
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ДанныеСостояния.ХронологическиеДанные.ХронологическиеДанные.ПричиныОтказаВПриемеМЧД) Тогда
					
					Элементы.ДекорацияРегистрацияВФНС.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(НСтр(
					"ru = '<a href = ""ОтказПриРегистрацииВНалоговомОргане"">%1</a>';
					|en = '<a href = ""ОтказПриРегистрацииВНалоговомОргане"">%1</a>'"), ОписаниеСобытия);
					Элементы.ДекорацияКартинкаРегистрацияВФНС.Картинка = БиблиотекаКартинок.ДиалогСтоп;
					
				Иначе
					
					Элементы.ДекорацияРегистрацияВФНС.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(НСтр(
					"ru = '<a href = ""РегистрацияВНалоговомОргане"">%1</a>';
					|en = '<a href = ""РегистрацияВНалоговомОргане"">%1</a>'"), ОписаниеСобытия);
	...
	Иначе
		...
		ДоверительПервоначальнойДоверенностиНаименование = ?(ОшибкаПриПолученииДанныхДоверенности = "",
			НСтр("ru = 'Отсутствует файл доверенности, нет данных о первоначальном доверителе';
				|en = 'The LOA file is missing. The original principal is unknown.'"),
			ОшибкаПриПолученииДанныхДоверенности);
