...
Процедура РедактироватьНастройки(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.уатФормаОформленияДокументов_уэ", Новый Структура("НастройкаКомплектовДокументов", Объект.Ссылка), ЭтотОбъект, Параметры.Ключ,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура РедактироватьНастройкиЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ПриЧтенииНаСервере(Объект.Ссылка);	
	ЭтаФорма.Прочитать();
