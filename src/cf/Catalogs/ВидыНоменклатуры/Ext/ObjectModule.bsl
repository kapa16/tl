...
Процедура ВыполнитьПроверкуКлючевыхРеквизитов(Отказ)
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("ru = 'Нельзя изменять признак ""Услуга"" если есть номенклатура данного вида.'; en = 'You cannot change the ""Service"" attribute if there is a nomenclature of this type.'"); 
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", 
			НСтр("ru = 'Услуга'; en = 'Service'"), , , ТекстСообщения);
