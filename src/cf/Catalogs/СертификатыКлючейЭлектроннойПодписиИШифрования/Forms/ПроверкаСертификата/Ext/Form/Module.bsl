...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	ТекстОшибок = ТекстОшибок + НСтр("ru = 'Результат проверки на клиенте';
									|en = 'Result of validation on client'") + ":" + Символы.ПС;
	...
	ТекстОшибок = ТекстОшибок + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Результат проверки на сервере ""%1"":';
			|en = 'Result of verification on server ""%1"":'"), ИмяСервера) + Символы.ПС;
		...
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			ТекстОшибок, Новый Структура("Тема, Сообщение",
				?(ПустаяСтрока(ТемаСообщения), НСтр("ru = 'Проблема при проверке сертификата электронной подписи';
													|en = 'Issue occurred verifying the signature certificate'"), ТемаСообщения),
...
Асинх Процедура ПроверитьНаСторонеКлиентаПослеСозданияЛюбогоМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	УстановитьЭлемент(ЭтотОбъект, "НаличиеПрограммы", Ложь, НСтр("ru = 'Проверка сертификата выполняется средствами токена.';
																|en = 'Certificate is validated using the token''s built-in mechanisms.'" + Символы.ПС + ОписаниеОшибкиНаКлиенте),
	...
	Иначе
		...
		Если ТипЗнч(ЗашифрованныеДанные) <> Тип("ДвоичныеДанные") Тогда
			УстановитьЭлемент(ЭтотОбъект, "Шифрование", Ложь, НСтр(
				"ru = 'Не удалось зашифровать данные средствами токена.';
				|en = 'Couldn''t encrypt data using the token.'"), Истина, ОбъединятьРезультаты);
		Иначе
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'В сертификате не указано приложение для использования закрытого ключа.';
								|en = 'The certificate does not specify an application for using the private key.'");
		ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(ОписаниеОшибки, Контекст);
	КонецЕсли;
