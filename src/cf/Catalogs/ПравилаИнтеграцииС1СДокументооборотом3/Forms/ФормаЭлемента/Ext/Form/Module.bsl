...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		Элементы.ТипОбъектаИС.Заголовок = СтрШаблон(НСтр("ru = 'Объект %1'; en = 'Object %1'"), СокращенноеНаименованиеКонфигурации);
		Элементы.ГруппаПолучениеИзДО.Заголовок = СтрШаблон(НСтр("ru = 'Заполнение реквизитов объекта %1'; en = 'Fill %1 object attributes'"),
			СокращенноеНаименованиеКонфигурации);
		Элементы.КлючевыеРеквизитыИСПредставление.Заголовок = СтрШаблон(НСтр("ru = 'Реквизит объекта %1'; en = '%1 object attribute'"),
			СокращенноеНаименованиеКонфигурации);
	КонецЕсли;
...
"поэтому правила для отдельных реквизитов не нужны.'; en = 'The table is populated with the result of the expression in 1C:Enterprise language,"
" so the rules for the individual details are not required.'"));
			Возврат;
		КонецЕсли;
		Если Родитель.Вариант = ВариантПустой() Тогда
			ПоказатьПредупреждение(,
				НСтр("ru = 'Вначале нужно выбрать вариант заполнения таблицы.'; en = 'First select a table filling option.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Асинх Процедура ПометитьНаУдаление(Знач Команда)
	...
	Если Модифицированность Тогда
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru = 'Для снятия пометки на удаление необходимо записать внесенные изменения'; en = 'To clear a deletion mark, save the changes'");
			ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Записать и снять пометку'; en = 'Save and clear checkbox'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Для пометки на удаление необходимо записать внесенные изменения'; en = 'To set a deletion mark, save the changes'");
			ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Записать и пометить'; en = 'Save and mark'");
		КонецЕсли;
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Отмена'; en = 'Cancel'");
	Иначе
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'Снять с ""%1"" пометку на удаление?'; en = 'Unmark ""%1"" for deletion?'"),
		...
		Иначе
			ТекстВопроса = СтрШаблон(
				...
				Строка(Объект.Ссылка));
		КонецЕсли;
		ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Да'; en = 'Yes'");
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Нет'; en = 'No'");
	КонецЕсли;
...
Асинх Процедура ЗаполнитьПоУмолчанию(Знач Команда)
	...
	Если НужноЗадатьВопрос Тогда
		Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
			НСтр("ru = 'Правила будут заполнены по умолчанию. Заполнить?'; en = 'Rules will be populated by default. Populate?'"),
...
Асинх Процедура ПравилаЗаполненияРеквизитовИСПередУдалениемПродолжение(Знач ТекущиеДанные)
	...
	Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Очистить правило заполнения реквизита?'; en = 'Clear the rule for filling in the requisite?'"),
...
Асинх Процедура ПриИзмененииОбъектаДО(Знач СтрокаПодходящиеПравилаЗагрузкиВДО)
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаДО) И ЗначениеЗаполнено(Объект.ПредставлениеОбъектаДО) Тогда
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Установить заполнение реквизитов из объекта ""%1""?'; en = 'Do you want to set attribute filling from the %1 object?'"),
			Объект.ПредставлениеОбъектаДО);
		Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
			ТекстВопроса,
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Да));
...
Асинх Процедура ПриПодключении()
	...
	Если ОбработатьФормуСогласноВерсииСервиса() Тогда
		...
		Если Не ВопросЗадан
			...
			Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
				НСтр("ru = 'Заполнить правила интеграции по умолчанию?'; en = 'Do you want to populate integration rules by default?'"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Нет));
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ЗаполнитьПоУмолчаниюНаСервере();
			КонецЕсли;
...
Процедура ЗаполнитьРеквизитыДокументооборота(ДопЗапрос = Неопределено, ОтветНаДопЗапрос = Неопределено)
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаДО) Тогда
		...
		НоваяСтрока.Представление = НСтр("ru = 'Тип объекта'; en = 'Object type'");
...
Процедура ЗаполнитьРеквизитыПотребителя()
	...
	Реквизиты.Сортировать("ТаблицаДляСортировки, ЭтоТаблица УБЫВ, ЭтоДополнительныйРеквизитИС, Представление");
	...
	Для Каждого Строка Из Реквизиты Цикл
		...
		Если НоваяСтрока.Вариант = ВариантРеквизитТаблицы() Тогда
			НоваяСтрока.Пояснение = НСтр("ru = 'Заполняется по правилам для отдельных реквизитов таблицы'; en = 'Filled in according to the rules of separate table attributes'");
		КонецЕсли;
...
Процедура ЗаполнитьПоУмолчаниюНаСервере(НачальноеЗаполнение = Ложь, ЗаполнятьИзОбъектаДО = Истина)
	...
	ЭтоСправочник = (СтрНайти(Объект.ТипОбъектаИС, "Справочник.") > 0);
	ЭтоДокумент = (СтрНайти(Объект.ТипОбъектаИС, "Документ.") > 0);
...
"%1'; en = 'Cannot get attributes of the 1C:Document Management object. Reason:"
"%1'"),
