	...
	Если Не ЗначениеЗаполнено(Подписант) Тогда
		Если ЗначениеЗаполнено(ПараметрыПодписанта) Тогда
			Если Не ЗначениеЗаполнено(ПараметрыПодписанта.ИННФЛ) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен ИНН физического лица подписанта';
																|en = 'Signer''s TIN required'"), ,"Подписант", , Отказ);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ПараметрыПодписанта.НомерТелефона) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен телефон подписанта';
																|en = 'Signer''s phone number required'"), ,"Подписант", , Отказ);
			КонецЕсли;
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен подписант';
															|en = 'Signer required'"), ,"Подписант", , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьНаПодписаниеПослеОбработкиПередВыполнением(РезультатПослеОбработки, Контекст) Экспорт
	...
	Если РезультатПослеОбработки.Свойство("ОписаниеОшибки") Тогда
		ОбработатьОшибку(Контекст.Оповещение, НоваяОшибка(НСтр("ru = 'Не удалось отправить данные в сервис мобильной подписи';
																|en = 'Couldn''t send data to the mobile signing service'"), РезультатПослеОбработки.ОписаниеОшибки), Новый Структура);
		Возврат;
	КонецЕсли;
		...
		Если ТипЗнч(Элемент.Данные) <> Тип("Строка") И ТипЗнч(Элемент.Данные) <> Тип("ДвоичныеДанные") Тогда
			...
			ОбработатьОшибку(Контекст.Оповещение,
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не поддерживается отправка в сервис данных с типом %1';
						|en = 'Service doesn''t support data of the %1 type'"), ТипЗнч(Элемент.Данные))));
			Возврат;
		КонецЕсли;
