...
Функция ВидБюджетаИспользуетГраницуФактическихДанных(ВидБюджета) Экспорт
	...
	СтруктураКолонок = БюджетнаяОтчетностьСервер.ПолучитьСтруктуруКолонокТаблицыДанныхВидаБюджета(ВидБюджета);
	ИсточникиДанных = БюджетнаяОтчетностьСервер.ПолучитьОписаниеИсточниковДанных(ВидБюджета, СтруктураКолонок, 
																Перечисления.РежимыФормированияБюджетныхОтчетов.Отчет);
	ИсточникиПериодовГруппировок = БюджетнаяОтчетностьСервер.ИсточникиПериодовГруппировок(ВидБюджета, СтруктураКолонок, 
																Перечисления.РежимыФормированияБюджетныхОтчетов.Отчет);
	
	Для Каждого Источник из ИсточникиДанных Цикл
		Если Найти(ВРег(Источник.ВыражениеНачалоПериода), ВРег("&ГраницаФактДанных")) Тогда
			ИспользуетГраницуФактическихДанных = Истина;
		КонецЕсли;
		Если Найти(ВРЕГ(Источник.ВыражениеКонецПериода), ВРег("&ГраницаФактДанных")) Тогда
			ИспользуетГраницуФактическихДанных = Истина;
		КонецЕсли;
		Если Найти(ВРЕГ(Источник.ВыражениеПериодаГруппировки), ВРег("&ГраницаФактДанных")) Тогда
			ИспользуетГраницуФактическихДанных = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Источник из ИсточникиПериодовГруппировок Цикл
		Если Найти(ВРЕГ(Источник.ВыражениеНачалоПериодаГруппировки), ВРег("&ГраницаФактДанных")) Тогда
			ИспользуетГраницуФактическихДанных = Истина;
		КонецЕсли;
		Если Найти(ВРЕГ(Источник.ВыражениеКонецПериодаГруппировки), ВРег("&ГраницаФактДанных")) Тогда
			ИспользуетГраницуФактическихДанных = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура НастроитьГруппуЭлементовПериода(ВидБюджета, Элементы, ГраницаФактДанных) Экспорт
	...
	Если ЗначениеЗаполнено(ВидБюджета) Тогда
		...
		ЗаголовокПериода = "";
		Если Реквизиты.СпособПланирования = Перечисления.СпособыСоставленияБюджетов.НаФиксированныйПериод Тогда
			ТекстНСТР = НСтр("en='Фиксированный бюджет с периодом планирования %1';ru='Фиксированный бюджет с периодом планирования %1'");
			ЗаголовокПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, НРег(Реквизиты.Периодичность));
		...
		ИначеЕсли ВидБюджета.СпособПланирования = Перечисления.СпособыСоставленияБюджетов.Скользящий Тогда
			...
			ТекстНСТР = НСтр("en='Скользящий бюджет на %1';ru='Скользящий бюджет на %1'");
			ЗаголовокПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, Периоды);
	...
	Иначе
		...
		Элементы.НадписьВидБюджета.Заголовок = НСтр("en='Фиксированный бюджет';ru='Фиксированный бюджет'");
	...
	Если ВидБюджетаИспользуетГраницуФактическихДанных(ВидБюджета) Тогда
		...
		ТекстНСТР = НСтр("en='Граница факт.данных: %1';ru='Граница факт.данных: %1'");
		Элементы.ГраницаФактДанных.Заголовок = 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, Формат(ГраницаФактДанных, "ДЛФ=D"));
...
Процедура ПроверитьДублиСтрокСтатейБюджетовПоАналитике(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	ДеревоЭлементов = БюджетнаяОтчетностьСервер.ПолучитьДеревоЭлементовВидаБюджета(ВидБюджета);
	...
	Для Каждого Периодичность из СтруктураКолонокТаблицыДанных.Периодичности Цикл
		Измерение = "ВыражениеПериода" + Периодичность;
		СтрокаСвертки = СтрокаСвертки + Измерение + ", ";
	КонецЦикла;
	...
	Для Каждого КлючиИЗначение Из СоответствиеСтатей Цикл
		ТекстНСТР = НСтр("en='Статья ""%1"" встречается внутри вида бюджета несколько раз с одинаковой аналитикой';ru='Статья ""%1"" встречается внутри вида бюджета несколько раз с одинаковой аналитикой'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, КлючиИЗначение.Ключ);
...
Процедура ПроверитьОтсутствиеИзмеренийАналитикВРежимеВвода(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	ДеревоЭлементов = БюджетнаяОтчетностьСервер.ПолучитьДеревоЭлементовВидаБюджета(ВидБюджета, Истина);
	...
	Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
		...
		ТекстНСТР = НСтр("en='Обнаружен недоступный для ввода элемент ""%1""';ru='Обнаружен недоступный для ввода элемент ""%1""'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, НайденнаяСтрока.НаименованиеДляПечати);
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
...
Процедура ПроверитьДублиВидовАналитик(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого СтрокаЭлементаРодителя из ТаблицаРодителей Цикл
		...
		Пока ТаблицаПроверки.Количество() И ТаблицаПроверки[0].Флаг > 1 Цикл
			ТекстНСТР = НСтр("en='Фиксированное значение ""%1"" не должно повторятся более одного раза';ru='Фиксированное значение ""%1"" не должно повторятся более одного раза'");
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, ТаблицаПроверки[0].НаименованиеДляПечати);
...
Процедура ПроверитьНепересечениеПериодов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого СтрокаСтруктуры из СтруктураЯчеек Цикл
		...
		Если МассивВышестоящихПериодовКолонки.Количество()
			...
			ТекстНСТР = НСтр("en='Период не может быть и в строках и в колонках!';ru='Период не может быть и в строках и в колонках!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
			Прервать;
		...
		Если НеверныйПорядокПериодов(МассивВышестоящихПериодовКолонки)
			...
			ТекстНСТР = НСтр("en='Больший период не может входить в меньший!';ru='Больший период не может входить в меньший!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
			Прервать;
	...
	Для Каждого СтрокаСтруктуры из СтруктураЯчеек Цикл
		...
		Если ЕстьНеделяВДекадах(МассивВышестоящихПериодовСтроки)
			...
			ТекстНСТР = НСтр("en='Неделя не может входить в декаду!';ru='Неделя не может входить в декаду!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,,ТекстНСТР, "");
			Возврат;
...
Процедура ПроверитьОтсутствиеЭлементовВидаВсе(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
		...
		Если Не ВходитВПроизводныйПоказатель Тогда
			...
			ТекстНСТР = НСтр("en='Обнаружен недоступный для ввода элемент ""Все показатели""';ru='Обнаружен недоступный для ввода элемент ""Все показатели""'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
			Прервать;
	...
	Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
		...
		Если Не ВходитВПроизводныйПоказатель Тогда
			...
			ТекстНСТР = НСтр("en='Обнаружен недоступный для ввода элемент ""Все статьи""';ru='Обнаружен недоступный для ввода элемент ""Все статьи""'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
...
Процедура ПроверитьНаличиеЦиклов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Пока НайденныеСтроки.Количество() Цикл
		...
		Если СталоСтрок = ВсегоСтрок Тогда
			ТекстНСТР = НСтр("en='Обнаружены циклические ссылки в формулах!';ru='Обнаружены циклические ссылки в формулах!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,,ТекстНСТР, "");
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьНаличиеИсточниковДанных(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Если Не ИсточникиДанных.Количество() Тогда
		ТекстНСТР = НСтр("en='Не выбраны статьи бюджетов или показатели бюджетов в структуре вида бюджета!';ru='Не выбраны статьи бюджетов или показатели бюджетов в структуре вида бюджета!'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
	КонецЕсли;
...
Процедура ПроверитьКорректностьУказанияАналитик(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого Ячейка из СтруктураЯчеек Цикл
		...
		Для Каждого ВидАналитики из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
			Имя = БюджетнаяОтчетностьСервер.ИмяПоляБюджетногоОтчета(ВидАналитики);
	...
	Для Каждого КлючиИЗначение из Структура Цикл
		Для Каждого СтрокаОшибки из КлючиИЗначение.Значение Цикл
			ТекстНСТР = НСтр("en='Для %1"
"""%2"" указана недоступная на %3 аналитика ""%4""';ru='Для %1"
"""%2"" указана недоступная на %3 аналитика ""%4""'");
					
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, 
										НСтр("en='статьи';ru='статьи'"), СтрокаОшибки.СтатьяПоказатель, НСтр("en='статье';ru='статье'"), СтрокаОшибки.ВидАналитики);
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, 
										НСтр("en='показателя';ru='показателя'"), СтрокаОшибки.СтатьяПоказатель, НСтр("en='показателе';ru='показателе'"), СтрокаОшибки.ВидАналитики);
			КонецЕсли;
...
Процедура ПроверитьСоответствиеВалютыСтатьям(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого КлючиИЗначение из Структура Цикл
		Для Каждого СтрокаОшибки из КлючиИЗначение.Значение Цикл
			ТекстНСТР = НСтр("en='Для %1 ""%2"" недоступен учет по валюте';ru='Для %1 ""%2"" недоступен учет по валюте'");
					
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, НСтр("en='статьи';ru='статьи'"), 
																					СтрокаОшибки.СтатьяПоказатель);
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, НСтр("en='показателя';ru='показателя'"), 
																					СтрокаОшибки.СтатьяПоказатель);
			КонецЕсли;
...
Процедура ПроверитьКорректностьУказанияИзмерений(ВидБюджета, СообщенияОбОшибках)
	...
	Иначе
		...
		НастройкиМодели = уатОбщегоНазначенияТиповые.ПолучитьЗначенияРеквизитов(ВидБюджета.Владелец, 
	...
	Для Каждого Измерение из СтруктураКолонокТаблицыДанных.Измерения Цикл
		...
		Для Каждого ЭлементМассива из МассивНедоступных Цикл
			ТекстНСТР = НСтр("en='Измерение отчета ""%1"" недоступно для текущего вида бюджета';ru='Измерение отчета ""%1"" недоступно для текущего вида бюджета'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, ЭлементМассива);
...
Процедура ПроверитьНаличиеСтрокКолонок(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			ТекстНСТР = НСтр("en='Строки таблицы не могут быть пустыми!';ru='Строки таблицы не могут быть пустыми!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			ТекстНСТР = НСтр("en='Колонки таблицы не могут быть пустыми!';ru='Колонки таблицы не могут быть пустыми!'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстНСТР, "");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьПересечениеАналитическихРазрезов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Таблицы = БюджетнаяОтчетностьСервер.ПолучитьТаблицыВидаБюджета(ВидБюджета);
	ДеревоЭлементов = БюджетнаяОтчетностьСервер.ПолучитьДеревоЭлементовВидаБюджета(ВидБюджета, Ложь);
	
	Для Каждого Таблица из Таблицы Цикл
		
		КореньТаблицы = ДеревоЭлементов.Строки.Найти(Таблица.Ссылка);
		
		ЭлементыСтрок = ЭлементыВидаОтчетаПоНаправлению(КореньТаблицы.Строки.Найти("Строки"));
		ЭлементыСтрок = ПреобразоватьЭлементыВТаблицуСравнения(ЭлементыСтрок, КэшДопРеквизитов);
		
		ЭлементыКолонок = ЭлементыВидаОтчетаПоНаправлению(КореньТаблицы.Строки.Найти("Колонки"));
		...
		Пока ЭлементыСтрок.Количество() И
			...
			ТекстНСТР = НСтр("en='Недопустим повтор элементов вида ""%1"" одновременно и в строках и в колонках';ru='Недопустим повтор элементов вида ""%1"" одновременно и в строках и в колонках'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, ЭлементыСтрок[0].ОбъектСравнения);
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
	...
	Для Каждого ЯчейкаОтчета из ВводимыеЯчейки Цикл
		...
		Для Каждого СтатьяБюджета из НайденныеСтроки Цикл
			Если СтатьяБюджета.Строки.Количество() Тогда 
				...
				ТекстНСТР = НСтр("en='Ошибка в ячейке [%1; %2]: запрещен ввод на уровне группировок';ru='Ошибка в ячейке [%1; %2]: запрещен ввод на уровне группировок'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, 
...
Процедура ПроверитьПериодичностьЭлементовВидаБюджета(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого Периодичность из СтруктураКолонок.Периодичности Цикл
		...
		Если ИндексОтчета > ИндексТекущей Тогда
			ТекстНСТР = НСтр("en='Периодичность, выбираемая в группировках, не может быть больше чем %1';ru='Периодичность, выбираемая в группировках, не может быть больше чем %1'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, НРег(ПериодичностьБюджета));
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьПересечениеАналитикВШапкеИСтруктуре(ВидБюджета, СообщенияОбОшибках)
	...
	Если ОбщиеАналитики.Количество() Тогда
		...
		ТекстНСТР = НСтр("en='Виды аналитик ""%1"" не могут быть выбраны и в шапке и в структуре бюджета';ru='Виды аналитик ""%1"" не могут быть выбраны и в шапке и в структуре бюджета'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, ПредставлениеАналитик);
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
	КонецЕсли;
...
Процедура ПроверитьОдинаковыеИзмеренияВРамкахОднойВетки(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого Измерение из Измерения Цикл
		...
		Если ЕстьОшибка Тогда
			ТекстНСТР = НСтр("en='Элемент отчета ""%1"" не может быть подчинен элементу ""%2""';ru='Элемент отчета ""%1"" не может быть подчинен элементу ""%2""'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНСТР, Измерение.НаименованиеДляПечати, ТекущаяСтрока.НаименованиеДляПечати);
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
		КонецЕсли;
