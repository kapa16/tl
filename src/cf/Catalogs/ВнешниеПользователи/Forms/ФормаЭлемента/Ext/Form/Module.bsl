	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Ограничение должно быть до завтра или более.';
				|en = 'The expiration date must be tomorrow or later.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
	...
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Чтобы разрешить вход в приложение, снимите
			           ...
			           |en = 'To allow login to the application, remove the deletion mark from the external user.
			           |'"));
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБПоказыватьВСпискеВыбора Тогда
			...
			УточнениеСвойств.Добавить(НСтр("ru = 'Показывать в списке выбора (включено)';
											|en = 'Show in choice list (selected)'"));
		КонецЕсли;
