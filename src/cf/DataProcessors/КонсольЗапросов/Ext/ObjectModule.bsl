...
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ТДРезультатаЗапроса, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса) Экспорт
	...
	Если Не Успешно Тогда 
		Если ВозможноОшибкаИзЗаТочкиСЗапятой(ТекстЗапроса) Тогда 
			ОтчетПоВыполнениюЗапроса.ТекстСообщения = НСтр("en='The query result was not displayed. A semicolon may not be escaped. A backslash is used to escape the semicolon - ""\;"" (see help)';ru='Результат запроса не был выведен. Возможно не экранирована точка с запятой. Для экранирования точки с запятой используется обратный слеш -""\;""(см. справку)'");
		Иначе
			ОтчетПоВыполнениюЗапроса.ТекстСообщения = НСтр("en='Request was not executed, because request text is incorrect';ru='Запрос не был выполнен, т.к. текст запроса некорректный'");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьИмяЗапроса(знач ТекстЗапроса)
	РезультатЗначение = НСтр("en='Query:';ru='Запрос:'") + " ";
...
Функция СформироватьСписокТипов(СписокДобавленныхТипов = Неопределено) Экспорт
	...
	СписокТипов.Добавить("Строка", НСтр("en='String';ru='Строка'"));
	СписокТипов.Добавить("Число", НСтр("en='Number';ru='Число'"));
	СписокТипов.Добавить("Дата", НСтр("en='Date';ru='Дата'"));
	СписокТипов.Добавить("Булево", НСтр("en='Boolean';ru='Булево'"));
	СписокТипов.Добавить("Граница", НСтр("en='Border';ru='Граница'"));
	СписокТипов.Добавить("МоментВремени", НСтр("en='Point in time';ru='Момент времени'"));
	СписокТипов.Добавить("СписокЗначений", НСтр("en='Value list';ru='Список значений'"));
	СписокТипов.Добавить("ТаблицаЗначений", НСтр("en='Values table';ru='Таблица значений'"));
...
Функция ВыполнитьЗапросСВременнойТаблицей(знач ЗапросДляВыполнения, ИмяВременнойТаблицы, ИмяЗапроса)
	...
	Попытка
		...
		ИмяЗапроса = НСтр("en='Temporary table:';ru='Временная таблица:'") + " " + ИмяВременнойТаблицы;
	Исключение
...
Процедура ВключениеТехнологическогоЖурнала(ПараметрыТехнологическогоЖурнала, РезультатВключения) Экспорт
	...
	Если КаталогКонфигурацииПриложения <> Неопределено Тогда 
		...
		ТекущийПользователь = ПользователиКлиентСервер.АвторизованныйПользователь().Наименование;
		...
		Исключение
			  ...
			  РезультатВключения.Причина = НСтр("en='Error creating configuration file in the directory';ru='Ошибка создания конфигурационного файла в каталоге'") + " " + КаталогКонфигурацииПриложения + Символы.ПС + НСтр("en='Check access rights.';ru='Проверьте права доступа.'");
		КонецПопытки;
	...
	Для каждого Поле Из СоответствиеБДИндекс Цикл
		Если Найти(ПланЗапросаВМетаданных, Поле.Ключ) Тогда
			...
			Иначе
				Ключ = Поле.Ключ;
			КонецЕсли;
			ПланЗапросаВМетаданных = СтрЗаменить(ПланЗапросаВМетаданных, Ключ, НСтр("en='Index';ru='Индекс по'") + " " + Поле.Значение + "");
		КонецЕсли;
