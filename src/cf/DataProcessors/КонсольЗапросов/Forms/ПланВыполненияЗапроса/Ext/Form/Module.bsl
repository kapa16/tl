...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок =  НСтр("en='Query execution plan';ru='План выполнения запроса'") + " (" + Параметры.ИмяЗапроса + ")";
...
Процедура ПрочитатьДанныеИзТехнологическогоЖурналаОбработчик()
	...
	Иначе
		...
		Иначе
			...
			ТребуетсяПрочитатьЖурналИмя = Неопределено;
			Элементы.ГруппаПолучениеПланаВыполненияЗапроса.Видимость = Ложь;
			ТекстНСТР = НСтр("en='Error of getting query execution plan';ru='Ошибка получения плана выполнения запроса'");
