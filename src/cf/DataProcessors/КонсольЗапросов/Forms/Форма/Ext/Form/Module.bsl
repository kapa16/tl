...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяЗапросаПоУмолчанию 			= НСтр("en='A new request';ru='Новый запрос'");
	...
	ЗаголовокФормы 		= НСтр("en='Query console (%ИмяЗапросаПоУмолчанию%)';ru='Консоль запросов (%ИмяЗапросаПоУмолчанию%)'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("en='Data was changed. Save changes?';ru='Данные изменены. Сохранить изменения?'");
...
Процедура ПоказатьПланВыполненияЗапросаНажатие(Элемент)
	Если НЕ ЗначениеЗаполнено(МеткаЗапроса) Тогда 
		ТекстНСТР = НСтр("en='To get query execution plan, you first need to complete query.';ru='Для получения плана выполнения запроса сначала требуется выполнить запрос.'");
	...
	Иначе
		...
		Элементы.ПоказатьПланВыполненияЗапроса.Заголовок = НСтр("en='Preparing...';ru='Подготовка...'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ТипВФормеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокЭлемента = НСтр("en='Select type';ru='Выбрать тип'");
...
Процедура ПараметрыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не Копирование Тогда 
		...
		Если ИндексТекущегоЗапроса = Неопределено Тогда
			ТекстСообщения = НСтр("en='Choose a request.';ru='Выберите запрос.'");
...
Процедура ЗначениеВФормеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипПараметра = НСтр("en='Values table';ru='Таблица значений'") Тогда 
		Путь = Объект.ПутьКФормам + "." + "ТаблицаЗначений";
	ИначеЕсли ТипПараметра = НСтр("en='Point in time';ru='Момент времени'") Тогда 
		Путь = Объект.ПутьКФормам + "." + "МоментВремени";
	ИначеЕсли ТипПараметра = НСтр("en='Border';ru='Граница'") Тогда
...
Процедура ПолучатьПланВыполненияЗапросаПриИзменении(Элемент)
	...
	Если ПоказыватьПланВыполненияЗапроса Тогда
			...
			Если РезультатВключения.Результат Тогда
				ТекстНСТР      = НСтр("en='Show the query execution plan';ru='Показывать план выполнения запроса'");
				ТекстПояснение = НСтр("en='Inclusion of an analysis of the query execution plan takes up to one minute';ru='Включение анализа плана выполнения запроса занимает до одной минуты'");
				ПоказатьОповещениеПользователя(ТекстНСТР,, ТекстПояснение);
...
Процедура ЗадатьУникальныйИдентификатор(Команда)
	...
	ПоказатьВводСтроки(Оповещение, УникальныйИдентификаторСтрокой, НСтр("en='Enter a unique ID';ru='Введите уникальный идентификатор'"));
КонецПроцедуры
...
Процедура ВыполнитьВыборРезультатаЗапроса(Команда)
	...
	ТекстПредупреждения = НСтр("en='It is necessary to enter query text and execute it.';ru='Необходимо ввести текст запроса и выполнить его.'");
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	Если ВерсияКонфигурации835ИБольше() Тогда
		...
		#Иначе
			ТекстСообщения = НСтр("en='Query builder is only available in modes: Thin client or Thick client.';ru='Конструктор запроса доступен только в режимах: Тонкий клиент или Толстый клиент.'");
	...
	Иначе
		#Если ТолстыйКлиентУправляемоеПриложение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
			...
			КонецЕсли;
		#Иначе
			ТекстСообщения = НСтр("en='Query constructor is only available in Thick Client mode.';ru='Конструктор запроса доступен только в режиме Толстого клиента.'");
			ПоказатьПредупреждение(, ТекстСообщения);
		#КонецЕсли
	КонецЕсли;
...
Процедура ВыбратьЗапросыИзФайла(Команда)
	...
	ОбработкаЧтенияФайла(Новый ОписаниеОповещения("ВыбратьЗапросыИзФайлаЗавершение", ЭтотОбъект), Истина);
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьЗапросыИзФайлаЗавершение(Результат1, ДополнительныеПараметры) Экспорт
...
Процедура ВывестиРезультатЗапроса()
	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл 
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда 
			...
			РезультирующаяСтрока	= НСтр("en='Result of query (number of lines = %КолСтрок%, lead time = %ВремяВыполнения%)';ru='Результат запроса (количество строк = %КолСтрок%, время выполнения = %ВремяВыполнения% с)'");
...
Процедура ОбновитьФормуКлиент() 
	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда
			...
			ЗаголовокФормы = НСтр("en='Query console (%ИмяЗапроса%)';ru='Консоль запросов (%ИмяЗапроса%)'");
	...
	КоличествоЗапросов = Объект.Запросы.Количество();
	ЗаголовокКнопкиВыбораЗапроса = НСтр("en='Queries';ru='Запросы'");
...
Процедура СохранитьЗапросы()
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда 
		...
		ТекстНСТР = НСтр("en='AutoSave was successful.';ru='Автосохранение прошло успешно.'");
		ПоказатьОповещениеПользователя(ТекстНСТР, ИмяФайла);
		Модифицированность = Ложь;
	КонецЕсли;
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Если ПустаяСтрока(ОтформатированныйТекст) Тогда 
		ТекстПредупреждения = НСтр("en='Enter text of query.';ru='Введите текст запроса.'");
		ПоказатьПредупреждение(, ТекстПредупреждения );
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	РезультирующаяСтрока = НСтр("en='Result of query (number of lines = %КоличествоСтрок%, lead time = %ВремяВыполнения% s)';ru='Результат запроса (количество строк = %КоличествоСтрок%, время выполнения = %ВремяВыполнения% с)'");
...
Процедура СохранитьФайлЗапроса(ОписаниеОповещения)
	...
	НачатьПодключениеРасширенияРаботыСФайлами(Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершениеРасширениеФайлов", ЭтотОбъект, Новый Структура("ОписаниеОповещения, Оповещение", ОписаниеОповещения, Оповещение)));
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеРасширениеФайлов(Подключено, ДополнительныеПараметры) Экспорт
...
Процедура СохранитьФайлЗапросаЗавершение(Оповещение) Экспорт
	...
	НачатьПодключениеРасширенияРаботыСФайлами(Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершениеЗавершение", ЭтотОбъект, Новый Структура("ИмяФайла, Оповещение", ИмяФайла, Оповещение)));
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеЗавершение(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Если Подключено Тогда
        Если ПустаяСтрока(ИмяФайла) Тогда 
            ...
            Диалог.Показать(Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершениеДиалогВыбора", ЭтотОбъект, Новый Структура("Диалог, Оповещение", Диалог, Оповещение)));
            Возврат;
        КонецЕсли;
        
        // Записываются текст и параметры запроса в файл.
        СохранитьФайлЗапросаЗавершениеЗавершениеФрагмент(ИмяФайла, Оповещение);
...
&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеДиалогВыбора(ВыбранныеФайлы, ДополнительныеПараметры1) Экспорт
...
&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеЗавершениеФрагмент(ИмяФайла, Знач Оповещение)
...
Процедура ЗаполнитьПараметрыКлиент()
	...
	Если Не ПустаяСтрока(ТекущийЗапрос.Текст) Тогда
		...
		Если МассивПараметров.Количество() > 0 Тогда
			Текст = НСтр("en='Parameters table is not empty. Clear table?';ru='Таблица параметров не пуста. Очистить таблицу?'");
	...
	Иначе
		ТекстНСТР = НСтр("en='Request body is empty.';ru='Текст запроса пустой.'");
	...
	Для каждого СчитанныйПараметр Из РезультатСтруктура Цикл
		...
		Если Не НайденПараметр Тогда
			Если Не ФлагВыводаСообщения Тогда
				ТекстНСТР = НСтр("en='Found parameters were added automatically.';ru='Найденные параметры были добавлены автоматически.'");
				ПоказатьСообщениеПользователю(ТекстНСТР, "Объект");
...
Функция ВключениеПланаВыполненияЗапроса()
	...
	Иначе
		...
		РезультатВключения.Причина = НСтр("en='Display query execution plan is only available when working on Windows OS.';ru='Отображение плана выполнения запроса доступно только при работе на ОС Windows.'");
	КонецЕсли;
