...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеВодителя", ДанныеВодителя)
		...
		КонецЕсли; 
		// {УАТ}
		Если Параметры.ДанныеВодителя.Свойство("ХранимыеДанныеВодитель") Тогда
			Запрос = Новый Запрос();
			Запрос.УстановитьПараметр("ХранимыеДанныеЭПД", Параметры.ДанныеВодителя.ХранимыеДанныеВодитель);
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			|	уатСоответствиеХранимыхДанныхЭПД.ОбъектУАТ КАК ОбъектУАТ
			|ИЗ
			|	РегистрСведений.уатСоответствиеХранимыхДанныхЭПД КАК уатСоответствиеХранимыхДанныхЭПД
			|ГДЕ
			|	уатСоответствиеХранимыхДанныхЭПД.ХранимыеДанныеЭПД = &ХранимыеДанныеЭПД";
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				Если ТипЗнч(Выборка.ОбъектУАТ) = Тип("СправочникСсылка.Сотрудники") Тогда
					ФизическоеЛицо = Выборка.ОбъектУАТ.ФизическоеЛицо;
				Иначе
					ФизическоеЛицо = Выборка.ОбъектУАТ;
				Конецесли; 
