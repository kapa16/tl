...
Процедура ОснованияПередУдалением(Элемент, Отказ)
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		ПоказатьПредупреждение(
			, 
			НСтр("ru = 'Нельзя удалить документ, так на него есть ссылки в списке товаров'; en = 'The document cannot be deleted because it is referenced in the product list.'"),
			30,
			НСтр("ru = 'Удаление документа-основания'; en = 'Removing the underlying document'"));
	КонецЕсли;
...
Процедура СоздатьКорректировкаПоступления(Команда)
	...
	Если Не РаспознаваниеДокументовСлужебныйВызовСервера.ИспользоватьКорректировочныеДокументы() Тогда
		ВызватьИсключение НСтр("ru = 'Для создания корректировок необходимо включить настройку ""Исправительные и корректировочные документы""'; en = 'To create adjustments, you must enable the ""Corrective and Adjustment Documents"" setting.'");
	КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(Форма, СтрокаТаблицы, СуммаВключаетНДС, СведенияОНоменклатуре)
	...
	СтрокаТаблицы.НадписьПоДокументу     = НСтр("ru = 'до (в основании)'; en = 'to (at the base)'");
	СтрокаТаблицы.НадписьДоКорректировки = НСтр("ru = 'до (распознано):'; en = 'to (recognized):'");
	СтрокаТаблицы.НадписьПослеИзменения  = НСтр("ru = 'после (распознано):'; en = 'after (recognized):'");
...
#КонецОбласти
	...
	Если КоличествоСвязанных = 0 Тогда
		...
		Элементы.ГруппаВсплывающаяСвязанныеДокументы.Заголовок = НСтр("ru = 'Нет связанных документов'; en = 'No linked documents'");
	Иначе
...
Процедура Подключаемый_СозданиеТипового(ПараметрыОперации) Экспорт
	...
	Если НовыеДокументы.Количество() Тогда
		...
		Для Каждого НовыйДокумент Из НовыеДокументы Цикл
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Создан документ'; en = 'Document is created'"), НавигационнаяСсылка, Строка(НовыйДокумент));
...
Процедура ОткрытьФормуВыбораОснования(Элемент)
	...
	ОткрытьФорму("Документ.уатПоступлениеТоваровУслуг.ФормаВыбора",
