...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтборСостояние = (НСтр("ru = '<Состояние известно>'; en = '<Known state>'"));
...
Процедура УстановитьИнтервалЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		ТекстЗаголовка = НСтр("ru = 'Журнал учета оригиналов первичных документов'; en = 'Source document tracking log'")+ " ";
		...
		ИначеЕсли Не ЗначениеЗаполнено(ВыбранноеЗначение.ДатаНачала) Тогда
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(по [ДатаОкончания])'; en = '(to [ДатаОкончания])'"), Значения);
		ИначеЕсли Не ЗначениеЗаполнено(ВыбранноеЗначение.ДатаОкончания)Тогда
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(с [ДатаНачала])'; en = '(from [ДатаНачала])'"), Значения);
		ИначеЕсли ВыбранноеЗначение.Вариант = ВариантСтандартногоПериода.Сегодня Или ВыбранноеЗначение.Вариант = ВариантСтандартногоПериода.Вчера 
			...
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '([ДатаНачала])'; en = '([ДатаНачала])'"), Значения);
		Иначе
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(с [ДатаНачала] по [ДатаОкончания])'; en = '(from [ДатаНачала] to [ДатаОкончания])'"), Значения);
		КонецЕсли;
...
Процедура ОткрытьДокумент()
	...
	Если Элементы.СписокДокументов.ВыделенныеСтроки = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выделено ни одного документа.'; en = 'No document is selected.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораОтветственного()
	...
	Элементы.ОтборАвторИзменения.СписокВыбора.Добавить(Пользователи.ТекущийПользователь(),НСтр("ru = '<Мои записи>'; en = '<My records>'"));
...
Процедура ЗаполнитьСписокВыбораСостоянияОригиналаОтбор()
	...
	КонецЦикла;
	Элементы.ОтборСостояние.СписокВыбора.Добавить("Состояниеизвестно",НСтр("ru = '<Состояние известно>'; en = '<Known state>'"));
	Элементы.ОтборСостояние.СписокВыбора.Добавить("Состояниенеизвестно",НСтр("ru = '<Состояние неизвестно>'; en = '<Unknown state>'"));
...
Процедура УстановитьОтборПоВидуДокумента(Результат)
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если ОтборВидыДокументов.Количество() = КоличествоДокументов Или ОтборВидыДокументов.Количество()= 0 Тогда
			Элементы.ИнформационнаяНадписьОтбор.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = 'Показаны все документы журнала <a href=""НастроитьОтбор"">Настроить</a>'; en = 'All documents in the journal are shown <a href=""ConfigureSelection"">Configure</a>'"));
		Иначе	
			ТекстНадписи = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru = ';Показан %1 документ журнала;;"
"Показано %1 документа журнала;Показано %1 документов журнала;'; en = ';%1 log document is displayed;;"
"%1 log documents are displayed;%1 log documents are displayed;'"),ОтборВидыДокументов.Количество());
			Элементы.ИнформационнаяНадписьОтбор.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ТекстНадписи + " " +НСтр("ru = '<a href=""НастроитьОтбор"">Настроить</a>'; en = '<a href=""ConfigureSelection"">Configure</a>'"));	
		КонецЕсли;
	...
	Иначе 
		...
		Если Не ВыбранноеЗначение=Неопределено Тогда
			Если ВыбранноеЗначение = "Состояниенеизвестно" Тогда
				ОтборСостояние = НСтр("ru = '<Состояние неизвестно>'; en = '<Unknown state>'");
