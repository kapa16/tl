...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивСтрокПояснениеСрокХранения.Добавить(НСтр("ru = 'Количество времени, в течении которого будет храниться'; en = 'Period of time during which'"));
	МассивСтрокПояснениеСрокХранения.Добавить(" ");
	МассивСтрокПояснениеСрокХранения.Добавить(
		Новый ФорматированнаяСтрока(НСтр("ru = 'история отправки'; en = 'the sending history'"),,
			Элементы.ПояснениеСрокХраненияСообщений.ЦветТекста,,
			"e1cib/list/РегистрСведений.ИсторияОтправкиСообщенийВ1СДокументооборот"));
	МассивСтрокПояснениеСрокХранения.Добавить(" ");
	МассивСтрокПояснениеСрокХранения.Добавить(НСтр("ru = 'и'; en = 'and'"));
	МассивСтрокПояснениеСрокХранения.Добавить(" ");
	МассивСтрокПояснениеСрокХранения.Добавить(
		Новый ФорматированнаяСтрока(НСтр("ru = 'очередь отправки'; en = 'a message sending queue'"),,
			Элементы.ПояснениеСрокХраненияСообщений.ЦветТекста,,
			"e1cib/list/РегистрСведений.ОчередьСообщенийВ1СДокументооборот"));
	МассивСтрокПояснениеСрокХранения.Добавить(" ");
	МассивСтрокПояснениеСрокХранения.Добавить(НСтр("ru = 'сообщений'; en = 'messages'"));
	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		Элементы.ПояснениеСинхронизироватьСвязи.Заголовок = СтрШаблон(
			НСтр("ru = 'Синхронизировать связи объектов 1С:Документооборота и %1'; en = 'Synchronize connections of 1C:Document Management and %1 objects'"),
			СокращенноеНаименованиеКонфигурации);
		Элементы.ПояснениеНастройкиХолдинга.Заголовок = СтрШаблон(
			НСтр("ru = 'Позволяет указать адреса публикации подчиненных узлов, а также задать соответствие пользователей %1 узлам 1С:Документооборота холдинга'; en = 'Allows you to specify the publication URLs of subordinate nodes and map %1 users to 1C:Document Management nodes of the holding company'"),
			СокращенноеНаименованиеКонфигурации);
		// {УАТ}
		ПояснениеПравилаИнтеграции = СтрШаблон(
			НСтр("ru = 'Задают соответствия между объектами 1С:Документооборота и %1, а также правила заполнения реквизитов'; en = 'Sets the correspondence between 1C:Document Management objects and %1, as well as the rules for filling in the details'"),
			СокращенноеНаименованиеКонфигурации);
		// {/УАТ}
...
Процедура ДекорацияНастройкиАвторизацииНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(НаборКонстант.АдресВебСервиса1СДокументооборот) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не указан адрес веб-сервиса.'; en = 'A web service address is not specified.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьКорневуюПапкуФайловДокументооборота(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Выбор папки файлов'; en = 'Select file folder'"));
...
Процедура УстановитьВидимость()
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СервисДоступен(ВерсияСервиса) Тогда
		...
		Если НомерРедакции = "1" Тогда
			...
			Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр("ru = 'Интеграция с 1С:Документооборот редакции 1 не поддерживается.'; en = 'Integration with 1C:Document Management (revision 1) is not supported.'");
		...
		ИначеЕсли НомерРедакции = "2" Тогда
			...
			Иначе
				Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаВерсияНеПоддерживается;
				Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр("ru = 'Интеграция с 1С:Документооборот редакции 2 не поддерживается.'; en = 'Integration with 1C:Document Management (revision 2) is not supported.'");
			КонецЕсли;
		...
		ИначеЕсли НомерРедакции = "3" Тогда
			Если ЕстьПодсистемаДО3 Тогда
				...
				Иначе
					Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаВерсияНеПоддерживается;
					Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр("ru = 'Функционал не поддерживается в данной версии 1С:Документооборота."
"Требуется 1С:Документооборот версии 3.0.7 или выше.'; en = 'Functionality is not supported in this version of 1C:Document Management."
"Version 3.0.7 or later is required.'");
			...
			Иначе
				Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаВерсияНеПоддерживается;
				Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр("ru = 'Интеграция с 1С:Документооборот редакции 3 не поддерживается.'; en = 'Integration with 1C:Document Management (revision 3) is not supported.'");
			КонецЕсли;
			
		Иначе
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаВерсияНеПоддерживается;
			Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = СтрШаблон(
				НСтр("ru = 'Интеграция с 1С:Документооборот редакции %1 не поддерживается.'; en = 'Integration with 1C:Document Management (revision %1) is not supported.'"),
				НомерРедакции);
...
Процедура ОбновитьИспользованиеРегламентногоЗадания(Использование, Расписание = Неопределено)
	...
	Если Использование И ПустаяСтрока(Константы.ИнтеграцияС1СДокументооборотИмяПользователяДляОбмена.Получить()) Тогда
		...
		ВызватьИсключение НСтр("ru = 'Не задан служебный пользователь для обмена'; en = 'A utility user for exchange is not set'");
	КонецЕсли;
...
Процедура ОбновитьНастройкиОбновленияСвязанныхРеквизитов()
	...
	ОбновлятьСвязанныеОбъектыАвтоматически = Ложь;
	ОписаниеНастройкиОбновления = Новый ФорматированнаяСтрока(НСтр("ru = 'Расписание не доступно'; en = 'Schedule is unavailable'"));
	...
	СтрокаСсылки = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить расписание'; en = 'Change schedule'"),,,, "ИзменитьРасписание");
	...
	Если ПустаяСтрока(ИмяПользователяДляОбмена) Тогда
		ИмяКоманды = НСтр("ru = 'Задать служебного пользователя для обмена'; en = 'Set a utility user for exchange'");
	Иначе
		...
		КонецЕсли;
		ИмяКоманды = СтрШаблон(НСтр("ru = 'Изменить пользователя для обмена (%1)'; en = 'Change the user for exchange (%1)'"), ИмяПользователяДляОбмена);
	КонецЕсли;
