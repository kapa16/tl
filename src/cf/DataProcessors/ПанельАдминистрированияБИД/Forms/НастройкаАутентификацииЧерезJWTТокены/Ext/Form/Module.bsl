...
Процедура СгенерироватьОписание(Команда)
	...
	Если Не ЗначениеЗаполнено(КлючПодписиТокенаДоступа1СДокументооборот)
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не указан ключ подписи JWT-токенов.'; en = 'A signature key of JWT tokens is not specified.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьВнесенныеНастройки(Команда)
	...
	Если Не ЗначениеЗаполнено(ОписаниеПубликации) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не сформировано описание веб-сервиса 1С:Документооборот.'; en = 'The details of 1C:Document Management web service are not generated.'"));
		Возврат;
	КонецЕсли;
