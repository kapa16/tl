...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ИмяДобавляемогоФайла", ИсправляемыйФайл);
	...
	ИначеЕсли ЗначениеЗаполнено(ИсправляемыйФайл) Тогда
		Элементы.ГруппаДокумент.Видимость = Истина;
		Элементы.ДекорацияПредставление.Заголовок = СтрШаблон(НСтр("ru = 'Документ: %1'"), ИсправляемыйФайл);
	Иначе
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОшибкиПриФормированииДокументовПоФайлам

&НаКлиенте
Процедура ОшибкиПриФормированииДокументовПоФайламВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.ОшибкиПриФормированииДокументовПоФайлам.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Поле = Элементы.ОшибкиПриФормированииДокументовПоФайламРезультат Тогда
		СтандартнаяОбработка = Ложь;
		
		Если ТекущиеДанные.ОшибокБольшеНет Тогда
			Возврат;
		КонецЕсли;

		ОткрытьФормуИсправленияОшибокДляФайла(ТекущиеДанные.ИмяФайла, ТекущиеДанные.ОписаниеОшибок.ВыгрузитьЗначения());
	КонецЕсли;
...
Процедура ПодписатьИОтправить(Команда)
	...
	Если МассовыйРежимИсправленияОшибокПоФайлам Тогда
		ЗавершитьРаботуФормыПриМассовомРежимеИсправленияОшибокПоФайлам();
		Возврат;
	КонецЕсли;
	...
	Если ЕстьОшибкиПоФайлам И Не ЗначениеЗаполнено(СписокДокументов) Тогда
		ПараметрыДокументов = ПараметрыФормированияПоФайлам();
	Иначе
	...
	Если Не ЗначениеЗаполнено(МассивОбъектовУчета) И Не ЕстьОшибкиПоФайлам Тогда
		Закрыть();
		Возврат;
	КонецЕсли;	
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ОшибкиПриФормированииДокументовПоФайламРезультат");
	
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("ОшибкиПриФормированииДокументовПоФайлам.ОшибокБольшеНет");
	ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = Ложь;
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Процедура УправлениеФормой(ИндексНовойСтраницы)
	...
	ИначеЕсли ОшибкиПриФормированииДокументовПоФайлам.Количество() > 1 Тогда
		
		МассовыйРежимИсправленияОшибокПоФайлам = Истина;
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из ДокументыКОтправке Цикл
			...
			Если СтрокаТЧ.ОшибкиСозданияПоФайлу.Количество() > 0 Тогда
				СтрокаТЧ.ЕстьОшибки = Истина;
				ФормированиеЗапрещено = Истина;
				
				Для Каждого Ошибка Из СтрокаТЧ.ОшибкиСозданияПоФайлу Цикл
					...
					Если ТекущаяСтрока.СписокОшибок.НайтиПоЗначению(Ошибка.ТекстОшибки) = Неопределено Тогда
						ТекущаяСтрока.СписокОшибок.Добавить(Ошибка.ТекстОшибки, Ошибка.КраткийТекстОшибки);
					КонецЕсли;
...
Функция ПоказатьВыбранныйЛистПомощника(Форма, ПереходВперед, НовыйИндекс = 0)
	...
	Если Форма.МассовыйРежимИсправленияОшибокПоФайлам Тогда
		СписокСтраниц.Добавить(Элементы.СтраницаПоФайлам);
		ТолькоПредупреждения = Ложь;
	КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере(ПараметрыФормы)
	...
	Если ЗначениеЗаполнено(АдресСведенийОбОшибках) Тогда
		...
		ЗаполнитьТаблицуОшибокПриФормированииПоФайлам(ОшибкиПриФормированииДокументов);
		
		ЕстьОшибкиПоФайлам = ОшибкиПриФормированииДокументовПоФайлам.Количество();
...
Процедура ЗаполнитьТаблицуОтправляемыхДокументов(ОшибкиПриФормированииДокументов, СписокДокументовКОтправке)
	...
	Если ЗначениеЗаполнено(ОшибкиПриФормированииДокументов) Тогда
		...
		Для Каждого ОшибкиПриФормированииДокумента Из ОшибкиПриФормированииДокументов Цикл
			...
			Если Не ЗначениеЗаполнено(ДанныеДокумента.Документ) Тогда
				Основание = ОснованиеОтправляемогоДокумента(ОбъектУчета, СписокДокументов);
				ЗаполнитьДанныеОтправляемогоДокумента(ДанныеДокумента, Основание, ОписаниеОбъектаУчета, ВидДокумента);
			КонецЕсли;
	...
	Для Каждого Документ Из СписокДокументовКОтправке Цикл
		...
		Для каждого СтрокаОписания Из ОписаниеОбъектаУчета Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаОписания.ТипДокумента)
				И Не ЗначениеЗаполнено(СтрокаОписания.Направление) Тогда
				Продолжить;
			КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТаблицуОшибокПриФормированииПоФайлам(ОшибкиПриФормированииДокументов)
	
	Для Каждого ОшибкиПриФормированииДокумента Из ОшибкиПриФормированииДокументов Цикл
	
		Если ОшибкиПриФормированииДокумента.Свойство("ЭтоОшибкаФормированияДокументаПоФайлу")
			И Не ОшибкиПриФормированииДокумента.ЭтоОшибкаФормированияДокументаПоФайлу Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрокаОшибкиПриФормированииПоФайлам = ОшибкиПриФормированииДокументовПоФайлам.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаОшибкиПриФормированииПоФайлам, ОшибкиПриФормированииДокумента);
		Если НоваяСтрокаОшибкиПриФормированииПоФайлам.ОтсутствуютПодписанты Тогда
			НоваяСтрокаОшибкиПриФормированииПоФайлам.Результат = НСтр("ru = 'Не указаны подписанты'");
		ИначеЕсли НоваяСтрокаОшибкиПриФормированииПоФайлам.ФормированиеЗапрещено Тогда
			НоваяСтрокаОшибкиПриФормированииПоФайлам.Результат = НСтр("ru = 'Формирование запрещено'");
		ИначеЕсли НоваяСтрокаОшибкиПриФормированииПоФайлам.ОтсутствуютНастройки Тогда
			НоваяСтрокаОшибкиПриФормированииПоФайлам.Результат = НСтр("ru = 'Отсутствуют настройки ЭДО'");
		КонецЕсли;
		
		НоваяСтрокаОшибкиПриФормированииПоФайлам.ОписаниеОшибок.Добавить(ОшибкиПриФормированииДокумента);
...
&НаСервереБезКонтекста
Функция ОснованиеОтправляемогоДокумента(ОбъектУчетаИзОшибокФормирования, СписокОтправляемыхДокументов)
	
	Если ЗначениеЗаполнено(ОбъектУчетаИзОшибокФормирования) Тогда
		Возврат ОбъектУчетаИзОшибокФормирования;
	ИначеЕсли ЗначениеЗаполнено(СписокОтправляемыхДокументов) Тогда
		Возврат СписокОтправляемыхДокументов[0];
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюОбОшибкахДокумента(СтрокаТаблицы, ОшибкиПриФормированииДокумента, КоличествоЗаписейОбОшибках)
	...
	Иначе
		...
		Если ОшибкиПриФормированииДокумента.Свойство("ОшибкиСозданияПоФайлу") Тогда
	
			Для Каждого СтрокаОшибки Из ОшибкиПриФормированииДокумента.ОшибкиСозданияПоФайлу Цикл
				НоваяСтрока = СтрокаТаблицы.ОшибкиСозданияПоФайлу.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОшибки);
				НоваяСтрока.КраткийТекстОшибки = СтрокаОшибки.КраткийТекст;
			КонецЦикла;
	...
	КоличествоОшибок = СтрокаТаблицы.ОшибкиЗаполнения.Количество()
		...
		+ СтрокаТаблицы.ОшибкиСозданияПоФайлу.Количество();
	...
	ИначеЕсли СтрокаТаблицы.ОшибкиСозданияПоФайлу.Количество() > 0 Тогда
		ПредставлениеОшибок = СтрокаТаблицы.ОшибкиСозданияПоФайлу[0].ТекстОшибки;
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуИсправленияОшибокДляФайла(ИмяФайла, ОписаниеОшибок)

	Оповещение = Новый ОписаниеОповещения("ПроверкаПослеИсправленияОшибокДляФайла", ЭтотОбъект);
	
	ПараметрыПроблемПриОбработкеДокументов = ИнтерфейсДокументовЭДОКлиентСервер.НовыеПараметрыПроблемПриОбработкеДокументов();
	ПараметрыПроблемПриОбработкеДокументов.ИмяДобавляемогоФайла = ИмяФайла;
	ПараметрыПроблемПриОбработкеДокументов.АдресСведенийОбОшибках = ПоместитьВоВременноеХранилище(ОписаниеОшибок,
		Новый УникальныйИдентификатор());
...
&НаКлиенте
Процедура ПроверкаПослеИсправленияОшибокДляФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыВыполненияДействийПоЭДО = Неопределено Тогда
		ПараметрыВыполненияДействийПоЭДО = Результат;
	Иначе
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ПараметрыВыполненияДействийПоЭДО.ПодписантыОбъектов,
			Результат.ПодписантыОбъектов, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(ПараметрыВыполненияДействийПоЭДО.МаршрутыПодписанияОбъектов,
			Результат.МаршрутыПодписанияОбъектов, Истина);
	КонецЕсли;
	...
	Для Каждого ПодписантыОбъекта Из Результат.ПодписантыОбъектов Цикл
		Если ТекущиеДанные.ИмяФайла = ПодписантыОбъекта.Ключ Тогда
			ТекущиеДанные.ОтсутствуютПодписанты = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;

	Для Каждого МаршрутыПодписанияОбъекта Из Результат.МаршрутыПодписанияОбъектов Цикл
		Если ТекущиеДанные.ИмяФайла = МаршрутыПодписанияОбъекта.Ключ Тогда
			ТекущиеДанные.ОтсутствуютПодписанты = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаТаблицы Из ОшибкиПриФормированииДокументовПоФайлам Цикл
		СтрокаТаблицы.ОшибокБольшеНет = Не СтрокаТаблицы.ОтсутствуютНастройки И Не СтрокаТаблицы.ОтсутствуютПодписанты
			И Не СтрокаТаблицы.ФормированиеЗапрещено;
		Если СтрокаТаблицы.ОшибокБольшеНет Тогда
			СтрокаТаблицы.Результат = НСтр("ru = 'Ошибки отсутствуют'");
		КонецЕсли;
	КонецЦикла;
...
Процедура ПомощникУстраненияОшибокПослеФормированияЭД(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ОбработанныйДокумент Из Результат.Итог.ОбработанныеДокументы Цикл
		СформированныеДокументы.Добавить(ОбработанныйДокумент.Ключ);
	КонецЦикла;
	
	АдресРезультата = ПоместитьВоВременноеХранилище(Результат.ОшибкиФормирования);
	ОбъектыКОтправке = ДополнительныеПараметры.Документ;
		
	ПомощникУстраненияОшибокПослеФормированияЭДНаСервере(АдресРезультата, ОбъектыКОтправке, СформированныеДокументы,
		ДополнительныеПараметры.ИндексТекущейСтраницы);
	
КонецПроцедуры

&НаСервере
Процедура ПомощникУстраненияОшибокПослеФормированияЭДНаСервере(Знач АдресРезультата, Знач ОбъектыКОтправке,
	Знач СформированныеДокументы, Знач ИндексТекущейСтраницы)
	
	ОбъектыУчета = ИнтеграцияЭДО.ОбъектыУчетаЭлектронныхДокументов(СформированныеДокументы);
	
	Для Каждого ДанныеОбъектаУчета Из ОбъектыУчета Цикл
		Если ДанныеОбъектаУчета.Актуальный И ОбъектыКОтправке.Найти(ДанныеОбъектаУчета.ОбъектУчета) = Неопределено Тогда
			ОбъектыКОтправке.Добавить(ДанныеОбъектаУчета.ОбъектУчета);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыОткрытия.Вставить("ТекущаяСтраница", ИндексТекущейСтраницы);
...
&НаКлиенте
Процедура ЗавершитьРаботуФормыПриМассовомРежимеИсправленияОшибокПоФайлам()
	
	ОчиститьСообщения();
	
	ЕстьОшибки = Ложь;

	Для Каждого Ошибка Из ОшибкиПриФормированииДокументовПоФайлам Цикл
		Если Ошибка.ОшибокБольшеНет Тогда
			Продолжить;
		КонецЕсли;
		
		ЕстьОшибки = Истина;
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(Ошибка.Результат);
	КонецЦикла;
...
Функция ПараметрыФормированияДокументов(Форма, МассивДокументов)
	...
	ПараметрыФормирования = НовыеПараметрыФормирования();
...
&НаСервере
Функция ПараметрыФормированияПоФайлам()
	
	ПараметрыФормирования = НовыеПараметрыФормирования();
	
	Если ЗначениеЗаполнено(СписокПодписантов) Тогда
		ПараметрыФормирования.ПодписантыОбъектов.Вставить(ИсправляемыйФайл,
			СписокПодписантов.Выгрузить().ВыгрузитьКолонку("Подписант"));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Маршрут) Тогда
		ПараметрыФормирования.МаршрутыПодписанияОбъектов.Вставить(ИсправляемыйФайл, Маршрут);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция НовыеПараметрыФормирования()
