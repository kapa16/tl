...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийВариантИнтерфейса <> ВариантыИнтерфейса.Расширенный Тогда
		ВызватьИсключение ИнтерфейсДокументовЭДО.ТекстСообщенияОНедоступностиОткрытияФормыТекущихДелЭДО( 
			Метаданные.Обработки.ИнтерфейсДокументовЭДО.Формы.ТекущиеДелаПоЭДО.Синоним,
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ИнтерфейсДокументовЭДОКлиент.ИмяСобытияОбновленияСостоянияЭДО()
		...
		ОбновитьТекущиеДелаПоЭДО(Параметр);
	...
	ИначеЕсли ИмяСобытия = "Запись_НаборКонстант" 
		И Источник = "ИспользоватьРазделениеДокументовПоОтветственнымЭДО" Тогда
...
&НаКлиенте
Процедура НадписьПредупреждениеАвтоматическогоПолученияЭДООбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
Процедура СоздатьПроизвольныйДокумент(Команда)
	...
	Если ИнтерфейсДокументовЭДОВызовСервера.ТребуетсяВыборФайлаПередСозданиемНовогоПроизвольногоДокумента() Тогда
		ИнтерфейсДокументовЭДОКлиент.ОткрытьНовыйПроизвольныйЭлектронныйДокументСВыборомФайла();
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПолезныеСсылки(Команда)
	
	ИнтерфейсДокументовЭДОКлиент.ОткрытьФормуПолезныхСсылок();
...
Функция ЗаменитьПоляРаздела(Знач СтрокаПолей, Знач ПараметрыЗамены)
	...
	СтрокаПолейБезПробелов = СтрЗаменить(СтрокаПолей, " ", "");
	СтрокаПолейБезПереносовИПробелов = СтрЗаменить(СтрокаПолейБезПробелов, Символы.ПС, "");
	
	Поля = СтрРазделить(СтрокаПолейБезПереносовИПробелов, ",");
...
Функция ПоляРаздела(Знач Раздел, Знач ПакетноеОтображение)
	...
	ПоляТаблицыВходящие = "ВидДокумента, ДатаПакета, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
		|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете,
		|ЭлектронныйДокумент";
	ПоляТаблицыИсходящие = "ВидДокумента, ДатаПакета, Дата, Номер, СуммаДокумента, Контрагент, Организация, Подписи,
		|СостояниеЭДО, НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание,
		|ОтражениеВУчете, ЭлектронныйДокумент";
	...
	ПоляТаблицыПриглашения = "Контрагент, ИдентификаторКонтрагента,СпособОбменаЭД, НовыйКонтрагент,
		|ИдентификаторОрганизации, Организация, СтатусПодключения, ЭлектроннаяПочта, ТекстПриглашения, ОписаниеОшибки,
		|ДатаОтправки, ДатаПолучения";
	ПоляТаблицыОзнакомиться = "ВидДокумента, ДатаПакета, Дата, Номер, СостояниеЭДО, Организация, Контрагент,
		|СуммаДокумента, ЭлектронныйДокумент";
	ПоляТаблицыОтправить = "ВидДокумента, ДатаПакета, Дата, Номер, Организация, Контрагент, СуммаДокумента,
		|ЭлектронныйДокумент";
	ПоляТаблицыНоменклатураКонтрагентов = "Контрагент, Сопоставить";
	ПоляТаблицыОшибки = "ВидДокумента, ДатаПакета, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
		|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете, Ссылка";
	...
	Если Раздел = "Входящие" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
				|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "Обработать" Тогда
		...
		ПоляТаблицы = ПоляТаблицыВходящие;
		ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО, НаправлениеЭД,
			|Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, ОтражениеВУчете, Описание";
	...
	ИначеЕсли Раздел = "Утвердить" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, НаправлениеЭД,
				|Ответственный, ДополнительнаяИнформация, ОтражениеВУчете, Описание";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "Аннулировать" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, НаправлениеЭД,
				|Ответственный, ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "НаКонтроле" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
				|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, ОтражениеВУчете, Описание";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "ПодписатьИсх" Тогда
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			ПоляРаздела = "СуммаДокумента, Контрагент, СостояниеЭДО, ВидДокумента, ЭлектронныйДокумент";
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, Подписи, НаправлениеЭД,
				|Ответственный, ДополнительнаяИнформация, Описание, ОтражениеВУчете";
		КонецЕсли;

	ИначеЕсли Раздел = "ИсправитьИсх" Тогда

		ПоляТаблицы = ПоляТаблицыИсходящие;
		ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, НаправлениеЭД, Ответственный,
			|ДополнительнаяИнформация, ПричинаОтклонения, Описание, ОтражениеВУчете";
	...
	ИначеЕсли Раздел = "НаКонтролеИсх" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, СостояниеЭДО,
				|НаправлениеЭД, Ответственный, ДополнительнаяИнформация, Описание, ОтражениеВУчете";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "Ознакомиться" Тогда
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			ПоляРаздела = "СуммаДокумента, Контрагент, СостояниеЭДО, ВидДокумента, ЭлектронныйДокумент";
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СостояниеЭДО, Организация, Контрагент, СуммаДокумента, Описание";
		КонецЕсли;
	...
	ИначеЕсли Раздел = "Отправить" Тогда
		...
		Иначе
			ПоляРаздела = "ВидДокумента, Дата, Номер, СуммаДокумента, Контрагент, Организация, Описание";
		КонецЕсли;
	...
	ПоляТаблицыБезПереносовСтрок = СтрЗаменить(ПоляТаблицы, Символы.ПС, "");
	ПоляТаблицы = СтрЗаменить(ПоляТаблицыБезПереносовСтрок, " ", "");
	ПоляРазделаБезПереносовСтрок = СтрЗаменить(ПоляРаздела, Символы.ПС, "");
	ПоляРаздела = СтрЗаменить(ПоляРазделаБезПереносовСтрок, " ", "");
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, Знач НастройкиОтображения,
	...
	ИначеЕсли Раздел = "Аннулировать" Тогда
		...
		Значение = Новый Массив;
		Значение.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяПодтверждениеАннулирования"));
		Значение.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяПодписаниеАннулирования"));
		Значение.Добавить(ПредопределенноеЗначение("Перечисление.СостоянияДокументовЭДО.ТребуетсяОтправкаАннулирования"));
...
Процедура ПослеВыполненияДействийПоЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ИначеЕсли Результат.Свойство("ОшибкиФормирования") И Результат.ОшибкиФормирования.Количество() Тогда
			...
			КонецЕсли;
			Если Результат.ОшибкиФормирования.Количество() = 1
		...
		Если Результат.Свойство("ОбработанныеДокументы") Тогда
			ОбработанныеДокументы = Результат.ОбработанныеДокументы;
		ИначеЕсли Результат.Свойство("Итог") Тогда
			ОбработанныеДокументы = Результат.Итог.ОбработанныеДокументы;
		Иначе
			ОбработанныеДокументы = Новый Массив;
		КонецЕсли;
		
		Для Каждого ОбработанныйДокумент Из ОбработанныеДокументы Цикл
...
&НаКлиенте
Процедура ОбновитьТекущиеДелаПоЭДО(Параметр)
...
Функция РаспознатьДокумент(ЭлектронныйДокумент)
	...
	СтруктураРеквизитов.Вставить("СпособОбмена", "СпособОбмена");
	СтруктураРеквизитов.Вставить("ТипДокумента", "ВидДокумента.ТипДокумента");
	СвойстваДокумента = ЭлектронныеДокументыЭДО.СвойстваДокумента(ЭлектронныйДокумент, СтруктураРеквизитов);
	СпособОбмена = СвойстваДокумента.СпособОбмена;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС")
		...
		ДокументРаспознан = ЗначениеЗаполнено(РезультатРаспознавания);
	КонецЕсли;
	
	Если Не ДокументРаспознан 
		И ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначения.ОбщийМодуль("ОбменСГИСЭПД");
		РезультатРаспознавания = МодульОбменСГИСЭПД.ПриРаспознованииДокумента(ТипДокумента, ДокументРаспознан); 
	КонецЕсли;
	
	Если Не ДокументРаспознан Тогда
		ДокументРаспознан = ЗначениеЗаполнено(ЭлектронныеДокументыЭДО.РаспознатьСообщение(СообщениеОтправителя));
	КонецЕсли;
	
	Возврат ДокументРаспознан;
...
Функция ТекстЗапросаОзнакомитьсяКартинкаМК()
	...
	Возврат "
		|ВЫБОР
		|	КОГДА СостоянияЭДОПереопределяемый.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияДокументовЭДО.Аннулирован)
		|		ТОГДА 0
		|	КОГДА СостоянияЭДОПереопределяемый.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияДокументовЭДО.ОбменЗавершен)
		|		ТОГДА 3
		|	ИНАЧЕ 4
		|КОНЕЦ"
КонецФункции
...
Процедура ОчиститьОбластьПросмотра(Форма)
	...
	Если Не Форма.ЭтоМобильныйКлиент Тогда
		Если Форма.СведенияОPdfДокументе <> Неопределено Тогда
			Форма[Форма.СведенияОPdfДокументе.ИмяЭлемента] = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеОбластиПросмотраПоФайлу(СтруктураВозврата, ПараметрыФормирования)
	...
	Если Не ЗначениеЗаполнено(ЭлементСхемыРегламента.ПрисоединенныйФайл) Тогда
		СтруктураВозврата.ФайлПредставление = ИнтерфейсДокументовЭДОКлиентСервер.ПредставлениеОтсутствующегоФайла();
		...
		Если ТипЗнч(РезультатФормирования.ПредставлениеДокумента) = Тип("ДвоичныеДанные") Тогда
			...
			Попытка
				...
				Если СведенияОPdfДокументе = Неопределено Тогда
					...
					СведенияОPdfДокументе.ИмяРодителяЭлемента = "СтраницаТекущийДокументPDF";
...
Процедура ПоказатьПредставлениеДанныхДокументаПослеФормирования(Результат, Контекст) Экспорт
	...
	КонецЕсли;
	Если Статус = "ВыполненоТекущийДокументPDF" Тогда
		ДанныеРезультата = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		Если ЗначениеЗаполнено(ДанныеРезультата) Тогда
			ИнтерфейсДокументовЭДОКлиент.ВывестиPDFДокументНаФорму(ЭтотОбъект, 
				СведенияОPdfДокументе, ДанныеРезультата.ПредставлениеДокумента);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьОбластьПросмотра()
	...
	ПоказатьФайл = ЗначениеЗаполнено(ДанныеПросмотра.ФайлПредставление);
