		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Настройка стандартного интерфейса OData.Запись пользователя';
					|en = 'Set up standard OData interface.Save user'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОшибкиСоставаРолиOData(ОшибкиПоОбъектам = Неопределено) Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ru = 'Следующие права избыточно включены в состав роли:';
										|en = 'The role contains the following redundant rights:'") + Символы.ПС + Символы.ВК
		...
		Для Каждого КлючИЗначение Из ИзбыточныеПрава Цикл
			...
			ОшибкиПоОбъектам.Вставить(ПолноеИмя, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Следующие права на объект %1 избыточно включены в состав роли %2: %3';
					|en = 'The %2 role contains the following redundant rights for object %1: %3'"),
...
Процедура ПроверитьВозможностьСозданияПользователяДляВызововСтандартногоИнтерфейсаOData()
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в приложении отсутствуют другие пользователи.';
								|en = 'Cannot create a separate username and password for the automatic REST service, as there are no application users.'");
	КонецЕсли;
