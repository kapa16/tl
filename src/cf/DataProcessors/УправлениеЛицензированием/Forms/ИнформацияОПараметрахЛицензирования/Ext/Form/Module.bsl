...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Отказ Тогда
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'Не удалось получить параметры ключа защиты'; en = 'Failed to get security key parameters'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура КомандаСохранитьВФайл(Команда)
	...
	Текст = Текст + НСтр("ru = 'Дата:'; en = 'Date:'") + Символы.НПП + ТекущаяДата() + Символы.ВК + Символы.ПС;
...
Процедура КомандаСохранитьВФайлЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ОписаниеТекстаПредупреждения = Новый Массив;
	ОписаниеТекстаПредупреждения.Добавить(
		НСтр("ru = 'Описание текущих параметров лицензирования сохранено в файл:'; en = 'Description of the current licensing options stored in the file'"));
	ОписаниеТекстаПредупреждения.Добавить(ИмяФайла);
	ТекстПредупреждения = СтрСоединить(ОписаниеТекстаПредупреждения, " ");
	ПоказатьПредупреждение(
		Неопределено,
		ТекстПредупреждения, ,
		НСтр("ru = 'Сохранен файл текущих параметров лицензирования'; en = 'The file is saved'"));
...
Функция ПолучитьПараметрыКлюча(ИмяОбработки)
	...
	Результат = ЛицензированиеСервер.ПолучитьПараметрыКлючаЗащиты(
		ИмяОбработки,
...
//
// Параметры:
//  ТипКлюча - Число - Идентификатор типа ключа.
//
// Возвращаемое значение:
//  Строка - Представление типа ключа.
//
&НаСервере
...
//
// Параметры:
//  Всего - Число - Общее количество подключений.
//  Свободно - Число - Количество доступных подключений.
//
// Возвращаемое значение:
//  Строка - Представление количества подключений.
//
&НаСервере
Функция СтрокаКоличествоПодключений(Всего, Свободно)
	Если Всего >= 255 Тогда
		Строка = НСтр("ru = 'Без ограничений'; en = 'Without restriction'")
			+ Символы.НПП
			+ НСтр("ru = 'занято -'; en = 'used -'")
			+ Символы.НПП
			+ Строка(Всего - Свободно);
	Иначе
		Строка = НСтр("ru = 'всего -'; en = 'total -'")
			+ Строка(Всего)
			+ ", "
			+ НСтр("ru = 'занято -'; en = 'used -'")
			+ Строка(Всего - Свободно)
			+ ", "
			+ НСтр("ru = 'свободно -'; en = 'available -'")
			+ Строка(Свободно);
	КонецЕсли;
	Возврат Строка;
КонецФункции
...
//
// Параметры:
//  ДатаОкончания - Дата - Дата окончания действия ключа.
//
// Возвращаемое значение:
//  Строка - Представление даты действия ключа.
//
&НаСервере
...
Процедура ЗаполнитьТаблицуПараметров()
	...
	Если СтрНайти(МестоХраненияКомпоненты, "ОбщийМакет") Тогда
