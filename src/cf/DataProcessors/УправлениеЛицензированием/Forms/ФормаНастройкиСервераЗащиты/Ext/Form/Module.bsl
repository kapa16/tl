...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоWindows Тогда
		...
		ПолучитьСведениеПоРежимуСтарта();
	Иначе
...
//  СписокЗначений Из Строка - Список серверов лицензирования.
//
...
&НаКлиенте
Процедура ПолучитьСведениеПоРежимуСтарта()
...
"а при использовании нескольких серверов в кластере - на каждом сервере кластера.)'; en = 'This option should be selected when working in local mode, i.e., on a single workstation, without using a network."
""
"If a hardware security key is used, it should be connected to this computer."
""
"If a software security key is used, it should be activated on this computer."
"(When using a server database, the licensing server must be installed on the same computer as the 1C server,"
"and when using multiple servers in a cluster, on each server in the cluster.)'");
...
"По умолчанию сервер лицензирования устанавливается на сетевой порт 15200.'; en = 'This option is used in network mode."
"If selected, you must specify the network address of the computer on which the licensing server is installed."
"By default, the licensing server is installed on network port 15200.'");
	Иначе
...
"В этом режиме невозможна активация программного ключа.'; en = 'This option is only used if licensing servers are configured on the network."
""
"When using this option, licensing servers are automatically searched for on the network."
"In this mode, software key activation is not possible.'");
	КонецЕсли;
