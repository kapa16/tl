...
Функция ДанныеКарточкиКонтрагента(ПараметрыОтчета) Экспорт
	...
	ДанныеОтчета.Вставить("СостояниеФакторыРиска", "");
	...
	Если ПараметрыОтчета.СостояниеОтчета = ""
		...
		Если ЗначениеЗаполнено(ИнформацияОСвязях.ОписаниеОшибки) Тогда
			...
			ДанныеОтчета.Вставить("СостояниеФакторыРиска",       "Завершено");
	...
	Если ДанныеОтчета.СостояниеОтчета <> "Ожидание"
			И (ПараметрыОтчета.СостояниеПроверки = ""
				Или ПараметрыОтчета.СостояниеПроверки = "Ожидание") Тогда
		...
		МодульРаботаСКонтрагентами = ОбщегоНазначения.ОбщийМодуль("РаботаСКонтрагентами");
		ИнформацияОПроверках = МодульРаботаСКонтрагентами.ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИНН(
		...
		Иначе
			Если ПараметрыОтчета.СостояниеСобытияМониторинга = "" Тогда
				...
				ИначеЕсли Не ИнформацияОСобытияхМониторинга.ВидОшибки.Пустая() Тогда
					...
					ОписаниеОшибки.Добавить(ДанныеОтчета.ОписаниеОшибки);
					ОписаниеОшибки.Добавить(ИнформацияОСобытияхМониторинга.ТекстОшибки);
					ДанныеОтчета.ОписаниеОшибки = СтрСоединить(ОписаниеОшибки, Символы.ПС);
					ДанныеОтчета.Вставить("СостояниеСобытияМониторинга", "Завершено");
			...
			Если ПараметрыОтчета.СостояниеФакторыРиска = ""
				Или ПараметрыОтчета.СостояниеФакторыРиска = "Ожидание" Тогда
				
				МодульСПАРКФакторыРискаКлиентСервер = ОбщегоНазначения.ОбщийМодуль("СПАРКФакторыРискаКлиентСервер");
				ПараметрыЗапроса = МодульСПАРКФакторыРискаКлиентСервер.ПараметрыПолученияФакторов();
				ПараметрыЗапроса.ИдентификаторФормы = ПараметрыОтчета.ИдентификаторФормы;
				
				МодульСПАРКФакторыРиска = ОбщегоНазначения.ОбщийМодуль("СПАРКФакторыРиска");
				ИнформацияОФакторахРиска = МодульСПАРКФакторыРиска.ФакторыРискаКонтрагента(
					ПараметрыОтчета.ИНН,
					ПараметрыЗапроса);
				
				Если Не ИнформацияОФакторахРиска.ИспользованиеРазрешено Тогда
					ДанныеОтчета.Вставить("СостояниеФакторыРиска", "Завершено");
				ИначеЕсли ИнформацияОФакторахРиска.ОжиданиеОтвета Тогда
					ДанныеОтчета.Вставить("СостояниеФакторыРиска", "Ожидание");
				ИначеЕсли Не ИнформацияОФакторахРиска.ВидОшибки.Пустая() Тогда
					
					ДанныеОтчета.ОписаниеОшибки = ДанныеОтчета.ОписаниеОшибки
						+ Символы.ПС
						+ ИнформацияОФакторахРиска.ТекстОшибки;
					ДанныеОтчета.Вставить("СостояниеФакторыРиска", "Завершено");
					
				КонецЕсли;
				
				ДанныеОтчета.Вставить("СостояниеФакторыРиска", "ПолученыДанные");
				
			Иначе
				ДанныеОтчета.Вставить("СостояниеФакторыРиска", ПараметрыОтчета.СостояниеФакторыРиска);
			КонецЕсли;
		КонецЕсли;
		
		КарточкаСправкиСПАРКРиски = Новый Структура;
		Если ДанныеОтчета.СостояниеОтчета <> "Ожидание" Тогда
			КарточкаСправкиСПАРКРиски.Вставить(
				"ДанныеАктуальнойСправки",
				МодульСПАРКРиски.АктуальнаяСправка(ПараметрыОтчета));
		КонецЕсли;
	...
	Если ДанныеОтчета.СостояниеФакторыРиска = "ПолученыДанные"
		И СПАРКРискиИспользованиеРазрешено Тогда
		
		Если ИнформацияОФакторахРиска.ОжиданиеОтвета Тогда
			ДанныеОтчета.СостояниеФакторыРиска = "Ожидание";
		Иначе
			
			ДанныеФакторыРиска = Новый Структура;
			ДанныеФакторыРиска.Вставить("РезультатФакторыТребующиеВнимания", Неопределено);
			ДанныеФакторыРиска.Вставить("РезультатДанныеОбАрбитражныхДелах", Неопределено);
			ДанныеФакторыРиска.Вставить("РезультатДанныеГосударственныеКонтракты", Неопределено);
			
			СформироватьРезультатФакторыРиска(
				ИнформацияОФакторахРиска,
				ДанныеФакторыРиска);
			
			ДанныеОтчета.Вставить("ДанныеФакторыРиска", ДанныеФакторыРиска);
			ДанныеОтчета.СостояниеФакторыРиска = "СформированОтчет";
...
Функция РасширеннаяИнформацияСПАРКРиски(ПараметрыОтчета) Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СПАРКРиски") Тогда
		ТекстОшибки = НСтр("ru = 'Не внедрена подсистема ""1СПАРК Риски""'");
		ПроверкаКонтрагента.ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ПолучитьСостояниеКонтрагентаПоДаннымЕГРН(Форма, Сведения) Экспорт
	
	Если Форма.ОперацияЗагрузкиСостоянияФНС <> Неопределено Тогда
		ДлительныеОперации.ОтменитьВыполнениеЗадания(Форма.ОперацияЗагрузкиСостоянияФНС.ИдентификаторЗадания);
	КонецЕсли;
	...
	Форма.АдресХранилищаСостоянияФНС = ПоместитьВоВременноеХранилище(Неопределено, Форма.УникальныйИдентификатор);
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияПроцедуры();
	Форма.ОперацияЗагрузкиСостоянияФНС = ДлительныеОперации.ВыполнитьПроцедуру(
		ПараметрыВыполнения,
		"ПроверкаКонтрагентов.ПроверитьКонтрагента",
		ДополнительныеПараметры,
		Форма.АдресХранилищаСостоянияФНС);
...
Процедура ПодготовитьДанныеФинансовогоАнализа(
	...
	ДанныеПоГодам.Колонки.Добавить("КлючГода");
	...
	Индекс = ДанныеПоГодам.Количество() - 1;
	КлючГода = 0;
	Пока Индекс >= 0 Цикл
		ДанныеПоГодам[Индекс].КлючГода = КлючГода;
		КлючГода = КлючГода + 1;
		Индекс = Индекс - 1;
	КонецЦикла;
...
Процедура ВывестиРазделРискБанкротства(
	...
	ОбластьПоказатели = ?(Показатели.ПятиФакторнаяМодель,
		"РискБанкротстваПоказателиПроизводство",
		"РискБанкротстваПоказатели");
	
	МакетФинансовыйАнализ = ПолучитьМакет("ФинансовыйАнализ");
	ТабличныйДокумент.Вывести(
		МакетФинансовыйАнализ.ПолучитьОбласть(
			ОбластьПоказатели + " | НаименованиеРасчета"));
	Для Год = 0 По Показатели.КоличествоЛет - 1 Цикл
		Область = МакетФинансовыйАнализ.ПолучитьОбласть(ОбластьПоказатели + " | Год_" + Год);
		Область.Параметры.Заполнить(СтрокаДанных);
		Область.Параметры.Заполнить(Показатели);
...
Процедура ВывестиРазделКредитоспособность(
	...
	// Показатели для расчета
	МакетФинансовыйАнализ = ПолучитьМакет("ФинансовыйАнализ");
	ОбластьПоказатели = МакетФинансовыйАнализ.ПолучитьОбласть(
		"КредитоспособностьПоказатели | КредитоспособностьНаименование");
	ОбластьПоказатели.Параметры.Заполнить(Показатели);
	ТабличныйДокумент.Вывести(ОбластьПоказатели);
	Для Год = 0 По Показатели.КоличествоЛет - 1 Цикл
		Область = МакетФинансовыйАнализ.ПолучитьОбласть("КредитоспособностьПоказатели | КредитоспособностьГод_" + Год);
		Область.Параметры.Заполнить(СтрокаДанных);
		Область.Параметры.Заполнить(Показатели);
		ТабличныйДокумент.Присоединить(Область);
	КонецЦикла;
...
Процедура ВывестиРазделРентабельностьАктивов(
	...
	МакетФинансовыйАнализ = ПолучитьМакет("ФинансовыйАнализ");
	ОбластьПоказатели = МакетФинансовыйАнализ.ПолучитьОбласть(
		"РентабельностьАктивовПоказатели | НаименованиеРасчета");
	ОбластьПоказатели.Параметры.Заполнить(СтрокаДанных);
	ТабличныйДокумент.Вывести(ОбластьПоказатели);
	Для Год = 0 По Показатели.КоличествоЛет - 1 Цикл
		Область = МакетФинансовыйАнализ.ПолучитьОбласть("РентабельностьАктивовПоказатели | Год_" + Год);
		Область.Параметры.Заполнить(СтрокаДанных);
		Область.Параметры.Заполнить(Показатели);
		ТабличныйДокумент.Присоединить(Область);
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(Показатели.ВидДеятельности) Тогда
		...
		ОбластиМакета.РентабельностьАктивовПоказателиПояснение.Параметры.ПояснениеРасчета = ПояснениеРасчета;
		ТабличныйДокумент.Вывести(ОбластиМакета.РентабельностьАктивовПоказателиПояснение);
	КонецЕсли;
	ТабличныйДокумент.Вывести(ОбластиМакета.РентабельностьАктивовПоказателиПодвал);
...
Процедура ВывестиРазделРентабельностьПродаж(
	...
	МакетФинансовыйАнализ = ПолучитьМакет("ФинансовыйАнализ");
	ОбластьПоказатели = МакетФинансовыйАнализ.ПолучитьОбласть(
		"РентабельностьПродажПоказатели | НаименованиеРасчета");
	ОбластьПоказатели.Параметры.Заполнить(СтрокаДанных);
	ТабличныйДокумент.Вывести(ОбластьПоказатели);
	Для Год = 0 По Показатели.КоличествоЛет - 1 Цикл
		Область = МакетФинансовыйАнализ.ПолучитьОбласть("РентабельностьПродажПоказатели | Год_" + Год);
		Область.Параметры.Заполнить(СтрокаДанных);
		Область.Параметры.Заполнить(Показатели);
		ТабличныйДокумент.Присоединить(Область);
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(Показатели.ВидДеятельности) Тогда
		ПояснениеРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Средняя рентабельность приведена по данным Росстата для вида деятельности ""%1""'"),
			Показатели.ВидДеятельности);
		ОбластиМакета.РентабельностьПродажПоказателиПояснение.Параметры.ПояснениеРасчета = ПояснениеРасчета;
		ТабличныйДокумент.Вывести(ОбластиМакета.РентабельностьПродажПоказателиПояснение);
	КонецЕсли;
	ТабличныйДокумент.Вывести(ОбластиМакета.РентабельностьПродажПоказателиПодвал);
...
Процедура ВывестиРезультатСобытияМониторинга(
	...
	Иначе
		...
		Область = ПолучитьМакет("ОшибкиПроверкиКонтрагента").ПолучитьОбласть(ИмяОбласти);
...
Процедура СформироватьРезультатФакторыРиска(
	ИнформацияОФакторахРиска,
	ДанныеФакторыРиска)
	
	КарточкаФакторыРиска = НовыйКарточкаФакторыРиска();
	Если Не ИнформацияОФакторахРиска.ВидОшибки.Пустая() Тогда
		КарточкаФакторыРиска.ДанныеОбАрбитражныхДелах = СкрытыеСводныеДанныеОбАрбитражныхДелах();
		КарточкаФакторыРиска.ДанныеГосударственныеКонтракты = СкрытыеДанныеГосударственныеКонтракты();
		Если Не ИнформацияОФакторахРиска.ИспользованиеРазрешено Тогда
			КарточкаФакторыРиска.ИспользованиеРазрешено = Ложь;
		КонецЕсли;
		ДанныеФакторыРиска.Вставить("КарточкаФакторыРиска", КарточкаФакторыРиска);
		Возврат;
	КонецЕсли;
	
	МодульСПАРКФакторыРискаКлиентСервер = ОбщегоНазначения.ОбщийМодуль("СПАРКФакторыРискаКлиентСервер");
	КарточкаФакторыРиска.ДанныеОбАрбитражныхДелах =
		МодульСПАРКФакторыРискаКлиентСервер.ДанныеОбАрбитражныхДелахПредставление(
			ИнформацияОФакторахРиска.СводныеДанныеОбАрбитражныхДелах,
			КарточкаФакторыРиска.ЕстьДанныеОбАрбитражныхДелах);
	КарточкаФакторыРиска.ДанныеГосударственныеКонтракты =
		МодульСПАРКФакторыРискаКлиентСервер.ДанныеГосударственныеКонтрактыПредставление(
			ИнформацияОФакторахРиска.ДанныеГосударственныеКонтракты,
			КарточкаФакторыРиска.ЕстьДанныеГосударственныеКонтракты);
	Если ИнформацияОФакторахРиска.ВидКонтрагента = Перечисления.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо Тогда
		КарточкаФакторыРиска.ФакторыРиска = ИнформацияОФакторахРиска.ФакторыРискаЮридическогоЛица;
		КарточкаФакторыРиска.ФакторыРискаПо115ФЗ = ИнформацияОФакторахРиска.ФакторыРискаЮридическогоЛицаПо115ФЗ;
	Иначе
		КарточкаФакторыРиска.ФакторыРиска = ИнформацияОФакторахРиска.ФакторыРискаФизическогоЛица;
		КарточкаФакторыРиска.ФакторыРискаПо115ФЗ = ИнформацияОФакторахРиска.ФакторыРискаФизическогоЛицаПо115ФЗ;
	КонецЕсли;
	КарточкаФакторыРиска.НегативныеСписки = ИнформацияОФакторахРиска.НегативныеСписки;
	
	ДанныеФакторыРиска.Вставить("КарточкаФакторыРиска", КарточкаФакторыРиска);
	
	Если Не КарточкаФакторыРиска.ИспользованиеРазрешено Тогда
		Возврат;
	КонецЕсли;
	
	ВывестиРезультатФакторыРиска(
		ДанныеФакторыРиска,
		ИнформацияОФакторахРиска);
	
	ДанныеФакторыРиска.Вставить("ИмяФайлаФакторыТребующиеВнимания",
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='ИНН %1 - факторы риска СПАРК'"),
				ИнформацияОФакторахРиска.ИНН)));
	
	ВывестиРезультатДанныеОбАрбитражныхДелах(
		ДанныеФакторыРиска,
		ИнформацияОФакторахРиска);
	
	ДанныеФакторыРиска.Вставить("ИмяФайлаДанныеОбАрбитражныхДелах",
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='ИНН %1 - данные об арбитражных делах СПАРК'"),
				ИнформацияОФакторахРиска.ИНН)));
	
	ВывестиРезультатДанныеГосударственныеКонтракты(
		ДанныеФакторыРиска,
		ИнформацияОФакторахРиска);
	
	ДанныеФакторыРиска.Вставить("ИмяФайлаДанныеГосударственныеКонтракты",
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='ИНН %1 - данные о государственных контрактах СПАРК'"),
				ИнформацияОФакторахРиска.ИНН)));
	
КонецПроцедуры

Функция НовыйКарточкаФакторыРиска()
	
	Результат = Новый Структура;
	Результат.Вставить("ИспользованиеРазрешено", Истина);
	Результат.Вставить("ФакторыРиска", Неопределено);
	Результат.Вставить("ФакторыРискаПо115ФЗ", Неопределено);
	Результат.Вставить("ДанныеГосударственныеКонтракты", Неопределено);
	Результат.Вставить("НегативныеСписки", Неопределено);
	Результат.Вставить("ДанныеОбАрбитражныхДелах", "");
	Результат.Вставить("ЕстьДанныеОбАрбитражныхДелах", Ложь);
	Результат.Вставить("ЕстьДанныеГосударственныеКонтракты", Ложь);
	
	Возврат Результат;
	
КонецФункции

Функция СкрытыеСводныеДанныеОбАрбитражныхДелах()
	
	Возврат НСтр("ru='░░ рассматривается - ░░ решений и постановлений -·░░ завершено'");
	
КонецФункции

Функция СкрытыеДанныеГосударственныеКонтракты()
	
	Возврат НСтр("ru='Приняли участие в ░░ торговых процедурах.
				|Заключено ░░ контрактов на·░ ░░░ ░░░ ₽.'");
	
КонецФункции

Процедура ВывестиРезультатФакторыРиска(
	ДанныеФакторыРиска,
	ИнформацияОФакторахРиска)
	
	ДанныеФакторыРиска.РезультатФакторыТребующиеВнимания = Новый ТабличныйДокумент;
	Результат = ДанныеФакторыРиска.РезультатФакторыТребующиеВнимания;
	
	Если ПустаяСтрока(ИнформацияОФакторахРиска.ТекстОшибки) Тогда
		
		ОбластиМакета = ВсеОбластиМакета("ФакторыРиска");
		Результат.Вывести(ОбластиМакета.Шапка);
		ВывестиВРезультатНегативныеСписки(
			Результат,
			ОбластиМакета,
			ИнформацияОФакторахРиска.НегативныеСписки);
		Если ИнформацияОФакторахРиска.ВидКонтрагента = ОбщегоНазначения.ПредопределенныйЭлемент(
					"Перечисление.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо") Тогда
			ВывестиВРезультатФакторыРиска(
				Результат,
				ОбластиМакета,
				ИнформацияОФакторахРиска.ФакторыРискаЮридическогоЛица,
				НСтр("ru='Факторы риска'"));
			ВывестиВРезультатФакторыРиска(
				Результат,
				ОбластиМакета,
				ИнформацияОФакторахРиска.ФакторыРискаЮридическогоЛицаПо115ФЗ,
				НСтр("ru='Факторы риска по 115-ФЗ'"));
		Иначе
			ВывестиВРезультатФакторыРиска(
				Результат,
				ОбластиМакета,
				ИнформацияОФакторахРиска.ФакторыРискаФизическогоЛица,
				НСтр("ru='Факторы риска'"));
			ВывестиВРезультатФакторыРиска(
				Результат,
				ОбластиМакета,
				ИнформацияОФакторахРиска.ФакторыРискаФизическогоЛицаПо115ФЗ,
				НСтр("ru='Факторы риска по 115-ФЗ'"));
		КонецЕсли;
...
Процедура ВывестиВРезультатНегативныеСписки(
		Результат,
		ОбластиМакета,
		НегативныеСписки)
		
	ОбластьШапкаФакторРиска = ОбластиМакета.ШапкаФакторРиска;
	ОбластьШапкаФакторРиска.Параметры.Заголовок = НСтр("ru='Вхождение в негативные списки'");
	Результат.Вывести(ОбластьШапкаФакторРиска);
	Если НегативныеСписки = Неопределено
		Или НегативныеСписки.Количество() = 0 Тогда
		
		ОбластьНетИнформации = ОбластиМакета.НетИнформации;
		Результат.Вывести(ОбластьНетИнформации);
		Возврат;
	КонецЕсли;
	
	
	Для Каждого НегативныйСписок Из НегативныеСписки Цикл
		
		ОбластьСтрокаФакторРиска = ОбластиМакета.СтрокаФакторРиска;
		ОбластьСтрокаФакторРиска.Параметры.Наименование = НегативныйСписок.Название;
		Результат.Вывести(ОбластьСтрокаФакторРиска);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ВывестиВРезультатФакторыРиска(
		Результат,
		ОбластиМакета,
		ФакторыРиска,
		ИмяФактора)
	...
	ОбластьШапкаФакторРиска = ОбластиМакета.ШапкаФакторРиска;
	ОбластьШапкаФакторРиска.Параметры.Заголовок = ИмяФактора;
	Результат.Вывести(ОбластьШапкаФакторРиска);
	Если ФакторыРиска = Неопределено
		Или ФакторыРиска.Количество() = 0 Тогда
	...
	Для Каждого ФакторРиска Из ФакторыРиска Цикл
		...
		ОбластьСтрокаФакторРиска = ОбластиМакета.СтрокаФакторРиска;
		ОбластьСтрокаФакторРиска.Параметры.Наименование = ФакторРиска.Название;
		Результат.Вывести(ОбластьСтрокаФакторРиска);
...
Процедура ВывестиРезультатДанныеОбАрбитражныхДелах(
	ДанныеФакторыРиска,
	ИнформацияОФакторахРиска)
	
	ДанныеОбАрбитражныхДелах = ИнформацияОФакторахРиска.СводныеДанныеОбАрбитражныхДелах;
	Если ДанныеОбАрбитражныхДелах = Неопределено
		Или Не ДанныеФакторыРиска.КарточкаФакторыРиска.ЕстьДанныеОбАрбитражныхДелах Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеФакторыРиска.РезультатДанныеОбАрбитражныхДелах = Новый ТабличныйДокумент;
	Результат = ДанныеФакторыРиска.РезультатДанныеОбАрбитражныхДелах;
	
	ОбластиМакета = ВсеОбластиМакета("ДанныеОбАрбитражныхДелах");
	Результат.Вывести(ОбластиМакета.Шапка);
	
	ОбластьМакетаСводнаяИнформация = ОбластиМакета.СводнаяИнформация;
	ОбластьМакетаСводнаяИнформация.Параметры.СводнаяИнформация =
		ДанныеФакторыРиска.КарточкаФакторыРиска.ДанныеОбАрбитражныхДелах;
	
	Результат.Вывести(ОбластьМакетаСводнаяИнформация);
	
	ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоГодам(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах);
	ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеОтветчика(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах);
	ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеИстца(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах);
...
Процедура ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоГодам(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах)
	
	Результат.Вывести(ОбластиМакета.ЗаголовокСводнаяПоГодам);
	
	СводныеДанныеПоГодам = ДанныеОбАрбитражныхДелах.СводныеДанныеПоГодам;
	Для Каждого СводныеДанныеПоГоду Из СводныеДанныеПоГодам Цикл
		ОбластьМакетаСводнаяИнформация = ОбластиМакета.СтрокаСводнаяПоГоду;
		ОбластьМакетаСводнаяИнформация.Параметры.Год = СводныеДанныеПоГоду.Год;
		Если СводныеДанныеПоГоду.ДанныеОДелахВКачествеИстца = Неопределено Тогда
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецСумма = 0;
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецДел   = 0;
		Иначе
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецСумма =
				СводныеДанныеПоГоду.ДанныеОДелахВКачествеИстца.СуммаПоДелам;
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецДел =
				СводныеДанныеПоГоду.ДанныеОДелахВКачествеИстца.КоличествоДел;
		КонецЕсли;
		Если СводныеДанныеПоГоду.ДанныеОДелахВКачествеОтветчика = Неопределено Тогда
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецСумма = 0;
			ОбластьМакетаСводнаяИнформация.Параметры.ИстецДел   = 0;
		Иначе
			ОбластьМакетаСводнаяИнформация.Параметры.ОтветчикСумма =
				СводныеДанныеПоГоду.ДанныеОДелахВКачествеОтветчика.СуммаПоДелам;
			ОбластьМакетаСводнаяИнформация.Параметры.ОтветчикДел =
				СводныеДанныеПоГоду.ДанныеОДелахВКачествеОтветчика.КоличествоДел;
		КонецЕсли;
		Если СводныеДанныеПоГоду.ДанныеОДелахВКачествеТретьихЛиц = Неопределено Тогда
			ОбластьМакетаСводнаяИнформация.Параметры.ИноеДел = 0;
		Иначе
			ОбластьМакетаСводнаяИнформация.Параметры.ИноеДел =
				СводныеДанныеПоГоду.ДанныеОДелахВКачествеТретьихЛиц.КоличествоДел;
		КонецЕсли;
...
Процедура ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеОтветчика(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах)
		
	ОбластьМакетаСводнаяИнформацияВКачестве = ОбластиМакета.СводнаяИнформацияВКачестве;
	ОбластьМакетаСводнаяИнформацияВКачестве.Параметры.Тип = НСтр("ru='Ответчик'");
	Результат.Вывести(ОбластьМакетаСводнаяИнформацияВКачестве);
	
	ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеПодробно(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах.ДанныеОДелахВКачествеОтветчика);
	
КонецПроцедуры

Процедура ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеИстца(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах)
		
	ОбластьМакетаСводнаяИнформацияВКачестве = ОбластиМакета.СводнаяИнформацияВКачестве;
	ОбластьМакетаСводнаяИнформацияВКачестве.Параметры.Тип = НСтр("ru='Истец'");
	Результат.Вывести(ОбластьМакетаСводнаяИнформацияВКачестве);
	
	ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеПодробно(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах.ДанныеОДелахВКачествеИстца);
	
КонецПроцедуры

Процедура ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияВКачествеПодробно(
		Результат,
		ОбластиМакета,
		ДанныеОбАрбитражныхДелах)
	
		Если ДанныеОбАрбитражныхДелах.КоличествоРассматриваемыхДел > 0 Тогда
			ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоСостоянию(
				Результат,
				ОбластиМакета,
				Новый Структура(
					"Состояние, СуммаПоДелам, КоличествоДел",
					НСтр("ru='Рассматривается'"),
					ДанныеОбАрбитражныхДелах.СуммаПоРассматриваемымДелам,
					ДанныеОбАрбитражныхДелах.КоличествоРассматриваемыхДел));
		КонецЕсли;
		
		Если ДанныеОбАрбитражныхДелах.КоличествоОбжалуемыхДел > 0 Тогда
			ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоСостоянию(
				Результат,
				ОбластиМакета,
				Новый Структура(
					"Состояние, СуммаПоДелам, КоличествоДел",
					НСтр("ru='Обжалуются'"),
					ДанныеОбАрбитражныхДелах.СуммаПоОбжалуемымДелам,
					ДанныеОбАрбитражныхДелах.КоличествоОбжалуемыхДел));
		КонецЕсли;
		...
		Если ДанныеОбАрбитражныхДелах.КоличествоРешенийИПостановлений > 0 Тогда
			ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоСостоянию(
				...
				Новый Структура(
					"Состояние, СуммаПоДелам, КоличествоДел",
					НСтр("ru='Принято решений'"),
					ДанныеОбАрбитражныхДелах.СуммаПоРешениямИПостановлениям,
		...
		Если ДанныеОбАрбитражныхДелах.КоличествоЗавершенныхДел > 0 Тогда
			ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоСостоянию(
				...
				Новый Структура(
					"Состояние, СуммаПоДелам, КоличествоДел",
					НСтр("ru='Завершено'"),
					ДанныеОбАрбитражныхДелах.СуммаПоЗавершеннымДелам,
					ДанныеОбАрбитражныхДелах.КоличествоЗавершенныхДел));
		КонецЕсли;
КонецПроцедуры

Процедура ВывестиВРезультатДанныеОбАрбитражныхДелахСводнаяИнформацияПоСостоянию(
		Результат,
		ОбластиМакета,
		Параметры)
		
	ОбластьМакетаСводнаяИнформацияПоТипу = ОбластиМакета.СводнаяИнформацияПоТипу;
	ОбластьМакетаСводнаяИнформацияПоТипу.Параметры.Заполнить(Параметры);
	Результат.Вывести(ОбластьМакетаСводнаяИнформацияПоТипу);
	
КонецПроцедуры

Процедура ВывестиРезультатДанныеГосударственныеКонтракты(
	ДанныеФакторыРиска,
	ИнформацияОФакторахРиска)
	
	ДанныеГосударственныеКонтракты = ИнформацияОФакторахРиска.ДанныеГосударственныеКонтракты;
	Если ДанныеГосударственныеКонтракты = Неопределено
		Или Не ДанныеФакторыРиска.КарточкаФакторыРиска.ЕстьДанныеГосударственныеКонтракты Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеФакторыРиска.РезультатДанныеГосударственныеКонтракты = Новый ТабличныйДокумент;
	Результат = ДанныеФакторыРиска.РезультатДанныеГосударственныеКонтракты;
	
	ОбластиМакета = ВсеОбластиМакета("ДанныеГосударственныеКонтракты");
	Результат.Вывести(ОбластиМакета.Шапка);
	
	ОбластьМакетаСводнаяИнформация = ОбластиМакета.СводнаяИнформация;
	ОбластьМакетаСводнаяИнформация.Параметры.СводнаяИнформация =
		ДанныеФакторыРиска.КарточкаФакторыРиска.ДанныеГосударственныеКонтракты;
	
	Результат.Вывести(ОбластьМакетаСводнаяИнформация);
	
	ВывестиВРезультатДанныеГосударственныеКонтракты94ФЗИ44ФЗ(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты);
	ВывестиВРезультатДанныеГосударственныеКонтракты223ФЗ(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты);
...
Процедура ВывестиВРезультатДанныеГосударственныеКонтракты94ФЗИ44ФЗ(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты)
		
	Если ДанныеГосударственныеКонтракты.ДанныеОГосКонтрактахПо94ФЗИ44ФЗ = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	ОбластьМакетаСводнаяИнформацияЗаголовокФЗ = ОбластиМакета.СводнаяИнформацияЗаголовокФЗ;
	ОбластьМакетаСводнаяИнформацияЗаголовокФЗ.Параметры.ФЗ = НСтр("ru='Сводные данные о госконтрактах по 94-ФЗ и 44-ФЗ'");
	Результат.Вывести(ОбластьМакетаСводнаяИнформацияЗаголовокФЗ);
	
	ВывестиВРезультатДанныеГосударственныеКонтрактыПоГодам(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты.ДанныеОГосКонтрактахПо94ФЗИ44ФЗ);
	
КонецПроцедуры

Процедура ВывестиВРезультатДанныеГосударственныеКонтракты223ФЗ(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты)
		
	Если ДанныеГосударственныеКонтракты.ДанныеОГосКонтрактахПо223ФЗ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОбластьМакетаСводнаяИнформацияЗаголовокФЗ = ОбластиМакета.СводнаяИнформацияЗаголовокФЗ;
	ОбластьМакетаСводнаяИнформацияЗаголовокФЗ.Параметры.ФЗ = НСтр("ru='Сводные данные о госконтрактах по 223-ФЗ'");
	Результат.Вывести(ОбластьМакетаСводнаяИнформацияЗаголовокФЗ);
	
	ВывестиВРезультатДанныеГосударственныеКонтрактыПоГодам(
		Результат,
		ОбластиМакета,
		ДанныеГосударственныеКонтракты.ДанныеОГосКонтрактахПо223ФЗ);
	
КонецПроцедуры

Процедура ВывестиВРезультатДанныеГосударственныеКонтрактыПоГодам(
		Результат,
		ОбластиМакета,
		ДанныеОГосКонтрактахПоГодам)
	
	Результат.Вывести(ОбластиМакета.ЗаголовокСводнаяПоГодам);
	
	Для Каждого СводныеДанныеПоГоду Из ДанныеОГосКонтрактахПоГодам Цикл
		
		Тендеры = СводныеДанныеПоГоду.Тендеры;
		Контракты = СводныеДанныеПоГоду.Контракты;
		ОбластьМакетаСводнаяИнформация = ОбластиМакета.СтрокаСводнаяПоГоду;
		ОбластьМакетаСводнаяИнформация.Параметры.Год = СводныеДанныеПоГоду.Год;
		ОбластьМакетаСводнаяИнформация.Параметры.КоличествоТендеровСУчастием = Тендеры.КоличествоТендеровСУчастием;
		ОбластьМакетаСводнаяИнформация.Параметры.КоличествоНедопусковКТендеру = Тендеры.КоличествоНедопусковКТендеру;
		ОбластьМакетаСводнаяИнформация.Параметры.КоличествоВыигранныхТендеров = Тендеры.КоличествоВыигранныхТендеров;
		ОбластьМакетаСводнаяИнформация.Параметры.КоличествоКонтрактов = Контракты.КоличествоКонтрактов;
		ОбластьМакетаСводнаяИнформация.Параметры.СуммаКонтрактов = Контракты.СуммаКонтрактов;
