...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьОтображениеБлокировкиИнтернетСервисов();
...
&НаКлиенте
Процедура НадписьБлокировкаРаботыИнтернетСервисовОбработкаНавигационнойСсылки(
		Элемент,
		НавигационнаяСсылкаФорматированнойСтроки,
		СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "action:unlock" Тогда
		СтандартнаяОбработка = Ложь;
		НадписьБлокировкаРаботыИнтернетСервисовОбработкаНавигационнойСсылкиНаСервере();
	КонецЕсли;
...
Процедура РазрешитьДоступКИнтернетСервисамПриИзменении(Элемент)
	РазрешитьДоступКИнтернетСервисамПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ИспользоватьПроверкуКонтрагентовПриИзменении(Элемент)
	...
	МодульПроверкаКонтрагентовВызовСервера.ПриВключенииВыключенииПроверки(ИспользоватьПроверкуКонтрагентов);
...
Процедура ИспользоватьОнлайнОплатыПриИзменении(Элемент)
	...
	МодульОнлайнОплатыВызовСервера.УстановитьИспользованиеИнтеграции(ИспользоватьОнлайнОплаты);
	
	Элементы.ГруппаПолучениеУведомлений.Доступность                           = ИспользоватьОнлайнОплаты;
	Элементы.ГруппаНастройкиОнлайнОплатыПраво.Доступность                     = ИспользоватьОнлайнОплаты;
...
&НаКлиенте
Процедура НастройкаОтображенияКомандПлатежныхСистемНажатие(Элемент)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		МодульНастройкиПлатежныхСистемКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"НастройкиПлатежныхСистемКлиент");
		МодульНастройкиПлатежныхСистемКлиент.НастройкаОтображенияКоманд(ЭтотОбъект);
	КонецЕсли;
...
Процедура ДлительностьОперацииПлатежнойСистемыПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		...
		МодульНастройкиПлатежныхСистемВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"НастройкиПлатежныхСистемВызовСервера");
		МодульНастройкиПлатежныхСистемВызовСервера.УстановитьДлительностьОперации(
...
Процедура ДосьеКонтрагента(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКонтрагентами") Тогда
		ОткрытьФорму(
			"Отчет.ДосьеКонтрагента.Форма.Форма",,,
...
&НаКлиенте
Процедура НастройкиПодключенияКПлатежнымСистемам(Команда)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		МодульНастройкиПлатежныхСистемКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"НастройкиПлатежныхСистемКлиент");
		МодульНастройкиПлатежныхСистемКлиент.НастройкиПодключения(ЭтотОбъект);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьШаблоныСообщенийПлатежныхСистем(Команда)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		МодульНастройкиПлатежныхСистемКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"НастройкиПлатежныхСистемКлиент");
		МодульНастройкиПлатежныхСистемКлиент.ОткрытьШаблоныСообщений(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбновитьСостояниеИнтернетПоддержки()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаСКонтрагентами") Тогда
		...
		Если Элементы.ГруппаПроверкаКонтрагентов.Видимость Тогда
			...
			ИспользоватьПроверкуКонтрагентов = МодульПроверкаКонтрагентов.ПроверкаКонтрагентовВключена();
		КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.НастройкиПлатежныхСистем") Тогда
		...
		МодульНастройкиПлатежныхСистемСлужебный = ОбщегоНазначения.ОбщийМодуль(
			"НастройкиПлатежныхСистемСлужебный");
		ПараметрыПодсистемыНастройкиПлатежныхСистем =
			МодульНастройкиПлатежныхСистемСлужебный.ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы(ЭтотОбъект);
		
		Если ПараметрыПодсистемыНастройкиПлатежныхСистем.НастройкаИнтеграцияДоступна Тогда
			
			Элементы.ГруппаПлатежныеСистемы.Видимость = Истина;
			Элементы.ГруппаПлатежныеСистемы.Заголовок =
				ПараметрыПодсистемыНастройкиПлатежныхСистем.ПредставлениеПлатежныхСистем;
				
			Элементы.НастройкиПодключенияКПлатежнымСистемамРасширеннаяПодсказка.Заголовок =
				ПараметрыПодсистемыНастройкиПлатежныхСистем.ПредставлениеПлатежныхСистемПодсказка;
				
			ДлительностьОперацииПлатежныхСистем =
				ПараметрыПодсистемыНастройкиПлатежныхСистем.ДлительностьОперацииПлатежныхСистем;
			
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы,
				"ГруппаНастройкаШаблоновПлатежныхСистем",
				"Видимость",
				ПараметрыПодсистемыНастройкиПлатежныхСистем.ДоступнаНастройкаШаблонов);
			
			Элементы.НастройкаОтображенияКомандПлатежныхСистем.Видимость =
				ПараметрыПодсистемыНастройкиПлатежныхСистем.ЕстьДокументыСКомандой;
			
		Иначе
			Элементы.ГруппаПлатежныеСистемы.Видимость = Ложь;
		КонецЕсли;
...
Процедура ОтобразитьСостояниеПодключения()
	...
	ЗаголовокПодключения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		ЗаголовокНадписиПодключенияИнтернетПоддержки(
			ДанныеАутентификации));
...
Функция ЗаголовокНадписиПодключенияИнтернетПоддержки(ДанныеАутентификации)
...
Процедура ОбновитьСостояниеПодсистемыОнлайнОплаты()
	...
	ИспользоватьОнлайнОплаты = ПараметрыПодсистемы.ИспользоватьОнлайнОплаты;
	Элементы.ГруппаПолучениеУведомлений.Доступность       = ИспользоватьОнлайнОплаты;
	Элементы.ГруппаНастройкиОнлайнОплатыПраво.Доступность = ИспользоватьОнлайнОплаты;
...
&НаСервере
Процедура НадписьБлокировкаРаботыИнтернетСервисовОбработкаНавигационнойСсылкиНаСервере()
	
	ОбщегоНазначения.НастроитьРаботуСИнтернетСервисами(Истина);
	РазрешитьДоступКИнтернетСервисам = Истина;
	НастроитьОтображениеБлокировкиИнтернетСервисов(
...
&НаСервере
Процедура РазрешитьДоступКИнтернетСервисамПриИзмененииНаСервере()
	
	ОбщегоНазначения.НастроитьРаботуСИнтернетСервисами(
		РазрешитьДоступКИнтернетСервисам);
	НастроитьОтображениеБлокировкиИнтернетСервисов(
		РазрешитьДоступКИнтернетСервисам);
...
&НаСервере
Процедура НастроитьОтображениеБлокировкиИнтернетСервисов(Знач РазрешитьДоступКИнтернетСервисам = Неопределено)
	
	Если РазрешитьДоступКИнтернетСервисам = Неопределено Тогда
		РазрешитьДоступКИнтернетСервисам = ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
	КонецЕсли;
	
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Элементы.НадписьБлокировкаРаботыИнтернетСервисов.Заголовок =
			НСтр("ru = 'В программе заблокирована работа с Интернет-сервисами. Для разблокировки обратитесь к администратору.'")
	КонецЕсли;
	
	Элементы.ГруппаБлокировкаИнтернетСервисов.Видимость = (Не РазделениеВключено
		И Не РазрешитьДоступКИнтернетСервисам);
	Элементы.ГруппаНастройкаДоступаКСервисам.Видимость = (Не РазделениеВключено
