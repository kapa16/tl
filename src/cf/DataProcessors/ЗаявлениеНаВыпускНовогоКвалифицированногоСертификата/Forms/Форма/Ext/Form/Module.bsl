...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодсказкаОбслуживающейОрганизации = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Узнать контакты партнера можно <a href = ""%1"">на сайте</a>.';
			|en = 'View the partner''s contact details <a href = ""%1"">here</a>.'"), СсылкаНаКонтактыПартнеров());
	...
	НСтр("ru = 'Для создания запроса на сертификат используется приложение электронной подписи и шифрования <b>%1</b> или <b>%2</b> или активный ключевой носитель (<b>Рутокен</b> или <b>JaCarta</b>).
		...
		| To manage JaCarta, download and install the library from the developer''s website and insert the token.
		|
...
Процедура ИнструкцияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭтоФизическоеЛицо Тогда
		...
		|en = 'Document set:
		|1. Certificate application prepared on the previous step.
		|2. Copy of the individual''s ID.
		|3. Copy of the individual''s SNILS (personal insurance policy number) or notification of registration in the personal record-keeping system (Form ADI-REG).'");
	ИначеЕсли ЭтоИндивидуальныйПредприниматель Тогда
		...
		|en = 'Document set:
		|1. Certificate application prepared on the previous step.
		|2. Copy of the certificate owner''s ID.
		|3. Copy of the certificate owner''s SNILS (personal insurance policy number) or notification of registration in personal record-keeping system (Form ADI-REG).'");
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|1. Certificate application prepared on the previous step.
			|2. Copy* of the ID of the requester''s representative.
			|3. Copy* of the SNILS (personal insurance policy number) of the requester''s representative.
			|4. Copy* of the document authorizing requester''s representative: owner''s resolution, founders meeting protocol, USRLE, or <a href=""%1"">letter of authority</a>.
			|
			|* All copies must be signed and stamped by the requester''s authorized person.'"),
	...
	Если СертификатОснованиеДействуетДо < ОбщегоНазначенияКлиент.ДатаУниверсальная() Тогда
		...
		ОписаниеОшибки.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Срок действия сертификата ""%1"" истек. Электронная расписка об ознакомлении с информацией квалифицированного сертификата не может быть отправлена.';
				|en = 'Certificate expired: %1. Cannot send the digital acknowledgment of the qualified certificate information.'"),
...
Процедура УстановитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) И Не ЗначениеЗаполнено(ПрограммаИдентификатор) Тогда
		ТекстОшибки = НСтр("ru = 'Приложение или токен: Поле не заполнено.';
							|en = 'App or token: Field is required.'");
	...
	ИначеЕсли ДанныеСертификата.СостояниеЗаявления <> Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено
		...
		Если СообщатьОбОшибках Тогда
			ШаблонСообщения = НСтр("ru = 'Для ""%1"" электронное продление невозможно, статус заявления продлеваемого сертификата ""%2""';
									|en = 'Cannot renew certificate online: %1. Application status is %2.'");
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ДанныеСертификата.ДействителенДо) Тогда
			...
			ШаблонСообщения = НСтр("ru = '""%1"" не является действующим сертификатом, электронное продление невозможно.';
									|en = 'Cannot renew certificate online: %1. It is not a valid certificate.'");
		ИначеЕсли ДанныеСертификата.ДействителенДо < ТекущаяДата Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Сертификат ""%1"" просрочен, электронное продление невозможно.';
									|en = 'Cannot renew certificate online: %1. Certificate expired.'");
		ИначеЕсли ДанныеСертификата.ДействителенДо >= ТекущаяДата + 30 * 24 * 60 * 60 И СообщатьОбОшибках Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Срок действия сертификата ""%1"" истекает более чем через 30 дней.';
									|en = 'Certificate expires in more than 30 days: %1.'");
			ОбщегоНазначения.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
...
Процедура УстановитьВидимостьИДоступность()
	...
	Если ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отправлено Тогда
		Элементы.ДекорацияСостояние.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Отправлено. Уточнить срок отработки заявки можно у <a href = ""%1"">обслуживающей организации</a>.';
				|en = 'The application is sent! For further inquiries, contact the <a href = ""%1"">service provider</a>.'"), СсылкаНаКонтактыПартнеров());
	Иначе
		...
		Если ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.ИсполненоРаспискаНеОтправлена
			...
			Если Не ТолькоПросмотрЗаявления Тогда
				Элементы.ДекорацияСостояние.Заголовок = НСтр(
					...
					|en = 'Application is fulfilled. Sign the information acknowledgment and send it to the service provider.'");
			КонецЕсли;
		Иначе
			...
			Если ДанныеЗаявления.СостояниеЗаявления
				...
				И Не ТолькоПросмотрЗаявления Тогда
					Элементы.ДекорацияСостояние.Заголовок = НСтр(
						...
						|en = 'Application is fulfilled. Before installing the certificate, send the information acknowledgment to the service provider.'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Элементы.УстановитьСертификат.Видимость = Ложь;
...
Функция ПроверитьЗаполнениеДополнительныхРеквизитов()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) И ПустаяСтрока(ПрограммаИдентификатор) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Приложение:';
							|en = 'App:'") + " " + ОбработкаМодуль.ТекстОшибкиПолеНеЗаполнено();
...
Функция ПодготовленныеДокументы()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("ru = 'от';
				|en = 'dated'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("ru = 'выданный';
...
Функция ПечатьИнформацииСертификата()
	...
	Если ЭлектроннаяПодписьСлужебныйКлиентСервер.ДоступныДополнительныеСвойстваСертификата() Тогда
		...
		Если ЗначениеЗаполнено(СертификатКриптографии.УниверсальнаяДатаНачалаЗакрытогоКлюча) И ЗначениеЗаполнено(
			...
			НСтр("ru = '%1 по %2';
				|en = 'from %1 to %2'"), Формат(СертификатКриптографии.УниверсальнаяДатаНачалаЗакрытогоКлюча, НСтр(
				"ru = 'ДФ=''dd MMMM yyyy ""г."" H:mm:ss''';
				|en = 'DF=''dd MMMM yyyy H:mm:ss'''")), Формат(
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СвойстваСертификата.ДатаНачалаЗакрытогоКлюча) И ЗначениеЗаполнено(
			...
			Макет.Параметры.ПериодДействияЗакрытогоКлюча = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 по %2';
					|en = 'from %1 to %2'"), Формат(СвойстваСертификата.ДатаНачалаЗакрытогоКлюча
		...
		Если РасширенныеДанныеДляПечатиСертификата.ИспользованиеКлюча = "030203F8" Тогда
			Макет.Параметры.ОбластьИспользования = НСтр("ru = 'Цифровая подпись, Неотрекаемость, Шифрование ключей, Шифрование данных, Согласование ключей';
														|en = 'Digital signature, Irrevocable, Key encryption, Data encryption, Key establishment'");
		Иначе
...
Асинх Процедура СоздатьКлючИЗапросНаСертификатНаТокене()
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для создания ключа электронной подписи требуется установка внешней компоненты %1.';
			|en = 'To create a digital signature key, install the add-in %1.'"), "ExtraCryptoAPI");
...
Процедура СоздатьКлючИЗапросНаСертификатНаТокенПослеОшибкиПомещенияФайлов(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru = 'Не удалось передать файл запроса на сертификат на сервер по причине:';
			|en = 'Cannot send the certificate request file to the server due to:'"));
	...
	Если Не ЭтоФизическоеЛицо И Не ЭтоИндивидуальныйПредприниматель
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В соответствии с Федеральным законом от 06.04.2011 N 63-ФЗ ""Об электронной подписи""
					...
					|en = 'According to Federal Law of the Russian Federation No. 63-FZ ""On electronic signatures"" as of April 6, 2011,
					|private organizations cannot issue qualified certificates to individuals acting as authorized representatives without requiring a LOA.
					|Request a certificate from %1.
					|(Certificates can only be requested for employees authorized by a LOA).'"),
	...
	ИначеЕсли ЭтоИндивидуальныйПредприниматель И Не ЗаявлениеДляИндивидуальныхПредпринимателейДоступно Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'According to Federal Law of the Russian Federation No. 63-FZ ""On electronic signatures"" as of April 6, 2011,
				|private organizations cannot issue qualified certificates to individual entrepreneurs.
				|Request a new certificate from %1.'"), СсылкаНаСайтФНС(НСтр("ru = 'Удостоверяющем центре ФНС';
																			|en = 'the FTS CA'"))));
	...
	ИначеЕсли Не ЭтоФизическоеЛицо И Не ЭтоИндивидуальныйПредприниматель 
		...
		ТекстСообщения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|Получите новый сертификат в %1.';
					|en = 'According to Federal Law of the Russian Federation No. 63-FZ ""On electronic signatures"" as of April 6, 2011,
					|private organizations cannot issue qualified certificates to legal entities.
					|Request a new certificate from %1.'"), СсылкаНаСайтФНС(НСтр("ru = 'Удостоверяющем центре ФНС';
																				|en = 'the FTS CA'"))));
			ПоказатьПредупреждение(, ТекстСообщения);
			Возврат;
...
Процедура ПроверитьЗаявлениеВБумажномВидеПередОтправкой()
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДокументыПартнерИНН)
		...
		Текст = Текст + Символы.ПС  + Символы.ПС
			...
			|Заявление может обрабатываться дольше обычного.';
			|en = 'You didn''t specify the service provider''s TIN or KPP.
			|The application processing might take longer than usual.'");
...
Процедура ПроверитьЗаявлениеВЭлектронномВидеПередОтправкой()
	...
	Если Не ДокументыПечатались Тогда
		Если ДанныеЗаявления.ОформленоВЭлектронномВиде Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Перед подписанием и отправкой Заявления необходимо создать ключ электронной подписи.';
															|en = 'Before signing and sending out the application, create a digital signature key.'"),,,, Отказ);
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Документы еще не печатались. Заявление не будет принято пока не будут получены печатные документы.';
															|en = 'Documents have not been printed yet. The application will not be accepted until the hard copy documents are received.'"),,,, Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(АдресФайлаСНИЛС) И ПродлениеСертификатаСтороннегоУЦ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран файл скана СНИЛС';
														|en = 'File with a scanned SNILS is required'"),,,, Отказ);
	КонецЕсли;
...
Процедура СформироватьИОтправитьФайлыЗаявления(Результат, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных.Представление.Представление = НСтр("ru = 'Заявление на подключение (файл сформирован)';
														|en = 'Connection application (file generated)'");
	ОписаниеДанных.Представление.Значение = Новый ОписаниеОповещения(
...
Процедура СформироватьИОтправитьФайлРасписки()
	...
	ОписаниеДанныхДляПодписания.Вставить("Операция",            НСтр("ru = 'Подписание расписки об ознакомлении с информацией сертификата';
																	|en = 'Sign information acknowledgment'"));
	ОписаниеДанныхДляПодписания.Вставить("ЗаголовокДанных",     НСтр("ru = 'Файл';
																	|en = 'File'"));
	ОписаниеДанныхДляПодписания.Вставить("ОтборСертификатов",   ОтборСертификатов);
	...
	Если ОписаниеДанных.Успех Тогда
		...
		Иначе
			ОписаниеОшибки.Описание = НСтр("ru = 'Не удалось отправить заявление в электронном виде.';
											|en = 'Couldn''t send the digital application for certificate issuance.'");
...
Процедура ОтправитьРаспискуПослеПодписанияФайлов(ОписаниеДанных, ДополнительныеПараметры) Экспорт
	...
	Если ОписаниеДанных.Успех Тогда
		...
		Иначе
			...
			ОписаниеОшибки.Описание = НСтр("ru = 'Не удалось отправить электронную расписку.';
											|en = 'Couldn''t send the information acknowledgment.'");
	...
	Иначе
		...
		ОписаниеОшибки.Описание = НСтр("ru = 'Не удалось подписать электронную расписку.';
										|en = 'Couldn''t sign the information acknowledgment.'");
		ДанныеЗаявления.СостояниеОбработкиЗаявления = ОписаниеОшибки.Описание + Символы.ПС + ОписаниеОшибки.Текст;
		ОткрытьФормуИнформацииСертификата(ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПроверитьИнтернетПоддержку(Оповещение)
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Невозможно подключиться к порталу интернет-поддержки по причине:
			...
			|Online Support Library is not integrated.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ИнтернетПоддержкаПользователейПодключена Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Для отправки заявления подключите Интернет-поддержку пользователей.';
											|en = 'To send your request, enable online support.'"));
			Возврат;
		КонецЕсли;
	...
	Иначе // Расписка.
		...
		ДанныеЗаявления.СостояниеОбработкиЗаявления = 
			НСтр("ru = 'Расписка об ознакомлении с информацией сертификата отправлена в электронном виде.';
				|en = 'The digital information acknowledgment has been sent.'");
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Механизмы) Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Токен %1 предназначен только для хранения ключей электронной подписи, для создания ключа выберите %2 или %3.';
				|en = 'Token %1 is intended solely for storing signature keys. To create a key, select %2 or %3.'"),
			Представление,
			ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыКриптоПро(),
			ЭлектроннаяПодписьКлиентСерверЛокализация.ИмяПрограммыVipNet());
	КонецЕсли;
...
Процедура ПоказатьПодсказкуКриптопровайдеры(Результат)
	...
	Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		...
		|en = '<a href = ""%1"">Click here</a> to find all apps and tokens for digital signatures installed on the computer.'"),
		...
		Если Не Элементы.ГруппаПодсказкаКриптопровайдеры.Видимость Тогда
			...
			Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = '<a href = ""%1"">Нажмите здесь</a>, чтобы установить расширение для работы с электронной подписью.';
					|en = '<a href = ""%1"">Click here</a> to install the digital signing app.'"),
				"ПроверитьУстановкуПрограммКриптографии");
		КонецЕсли;
	...
	Попытка
		...
		Если СвойстваФункции = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В веб-сервисе ""%1"" не определена обязательная функция ""%2"".';
					|en = 'In the web service ""%1"", the required function is not defined: ""%2"".'"),
		...
		Если ВариантАлгоритмов <> "2001"
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2';
				|en = 'The server response is unexpected for the element ""%1"":
				|%2'"), "algorithm", Ответ);
				...
				Если ПолученныйВариантУдостоверяющегоЦентра <> "1C"
				   ...
				   И ПолученныйВариантУдостоверяющегоЦентра <> "TK" Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						|%2';
						|en = 'The server response is unexpected for the attribute ""%1"":
						|%2'"), "CA", Ответ);
				КонецЕсли;
		...
		Иначе
			ЗаголовокОшибки =
				НСтр("ru = 'Не удалось получить требуемый вариант удостоверяющего центра от сервера 1С:Подпись по причине:';
					|en = 'Cannot get the required certificate authority option from 1C:Signature server due to:'")
				+ Символы.ПС;
		КонецЕсли;
...
Функция ТекстФайлаЗапросаНаСертификат()
	...
	ТекстОшибки = НСтр("ru = 'Не удалось прочитать ранее созданный запрос на сертификат.
		...
		|en = 'Cannot read the certificate request.
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат установите расширение для работы с 1С:Предприятием.';
				|en = 'To create a digital signature key and a certificate request, install the 1C:Enterprise Extension.'"),
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат установите внешнюю компоненту.';
				|en = 'To create a digital signature key and a certificate request, install the add-in.'"));
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "%1")
	...
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Запрос на сертификат не создан.';
										|en = 'Certificate request is not created.'"));
	КонецЕсли;
...
Процедура ПоказатьОшибкуСозданияЗапросаНаСертификат(ИнформацияОбОшибке, ЗаголовокОшибки = "%1")
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru = 'Создание запроса на сертификат';
			|en = 'Create certificate request'"), "", Ошибка,
		Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиУдаленияФайлов(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		...
		|%1';
		|en = 'Cannot delete the certificate request file on the computer due to:
		|%1'"));
				...
				ПоказатьПредупреждение(
					Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке", ЭтотОбъект, Контекст),
						...
						НСтр("ru = 'Приложение электронной подписи не доступно по причине:
							 |%1';
							 |en = 'Cannot access digital signature app due to:
			...
			ПоказатьПредупреждение(
				Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке", ЭтотОбъект, Контекст),
					...
					НСтр("ru = 'Не установлено приложение %1(%2).';
						|en = 'App not installed: %1(%2).'"), ПредставлениеПрограммы, ПрограммаИдентификатор));
			Возврат;
		КонецЕсли;
...
Асинх Процедура УстановитьСертификатНаТокен()
	...
	Если ЗначениеЗаполнено(СертификатыНаТокене.Ошибка) Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: %2.';
				|en = '%1: %2.'"), ПредставлениеПрограммы, СертификатыНаТокене.Ошибка);
		ПослеУстановкиСертификата(Ложь, Контекст);
	Иначе
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ТекстШаблона = ?(Доверенный,
			НСтр("ru = 'Не удалось установить корневой сертификат в доверенные корневые сертификаты: %1';
				|en = 'Couldn''t add the root certificate to the trusted root certificates: %1'"),
			НСтр("ru = 'Не удалось установить сертификат в личное хранилище: %1';
				|en = 'Couldn''t add the certificate to the personal store: %1'"));
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись.УстановкаСертификата';
				|en = 'Digital signature.InstallingTheCertificate'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура УстановитьСертификатПослеПроверкиСуществованияКонтейнераКлючей(Результат, Контекст) Экспорт
	...
	Иначе
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Couldn''t find the local key container.
			|Name: %1
			|Path: %2
			|Do either of the following:
			|- If the container was installed on a storage device, plug in the device.
			|- Install the certificate on the computer where the container was created when printing the application.'"),
			КонтейнерКлючаИмя,
			КонтейнерКлючаПуть);
...
Процедура УстановитьСертификатПослеУточненияОшибкиВызоваУстановитьСертификатВКонтейнерИХранилище(ТекстОшибки, Контекст) Экспорт
	...
	ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|%1';
		|en = 'Couldn''t install the personal certificate into the key container and OS store due to:
		|%1'"),
		ТекстОшибки);
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилищеПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не удалось установить личный сертификат в контейнер ключей и хранилище ОС.';
											|en = 'Couldn''t install the personal certificate into the key container and OS store.'");
	КонецЕсли;
		...
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru = 'Не удалось найти личный сертификат в папке ""Личное""
				           ...
				           |en = 'Couldn''t find the personal certificate in the local ""Personal""
				           |folder of the current user certificate store.'");
		Иначе
			...
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти личный сертификат в папке ""Личное""
				           ...
				           |%1';
				           |en = 'Couldn''t find the personal certificate in the local ""Personal""
				           |folder of the current user certificate store. Reason:
				           |%1'"),
...
Процедура УстановитьСертификатПослеУточненияОшибкиВызоваИмпортироватьСертификат(ТекстОшибки, Контекст) Экспорт
	...
	ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|%1';
		|en = 'Couldn''t install the root certificate to the OS store due to:
		|%1'"),
		ТекстОшибки);
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификатПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не удалось установить корневой сертификат в хранилище ОС.';
											|en = 'Couldn''t install the root certificate to the OS store.'");
	КонецЕсли;
...
Процедура ПослеУстановкиСертификатаНаСервере(Установлен)
	...
	Иначе
		ДанныеЗаявления.СостояниеОбработкиЗаявления = НСтр("ru = 'Исполнено. Сертификат не установлен.';
															|en = 'Application is fulfilled. Certificate is not installed.'");
		ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.ИсполненоСертификатНеУстановлен;
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Регион РФ с кодом ""%1"" не существует.';
				|en = 'Invalid Russian federal subject code: %1.'"), КодРегиона);
		Возврат "";
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.';
				|en = 'The Russian federal subject with code %1 is not assigned a name recommended for use in digital signatures.'"), КодРегиона);
		Возврат "";
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		ПоказатьОшибкуВзаимодействияСОблачнымСервисом(
			НСтр("ru = 'Создание ключа электронной подписи и запроса на сертификат';
				|en = 'Creating a digital signature key and certificate request'"), ОписаниеОшибки);
...
Процедура УстановитьСертификатОблачногоСервисаПослеУстановки(ОтветСервиса, ДополнительныеПараметры) Экспорт
	...
	Если Не ОтветСервиса.Выполнено Тогда
		...
		ЗаголовокОшибки = НСтр("ru = 'Установка сертификата в контейнер ключа и хранилище сертификатов';
								|en = 'Installing certificate to key container and certificate store'");
	...
	Иначе
		...
		Элементы.ВключатьЛицензиюКриптоПроВСертификат.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Включить лицензию КриптоПро CSP (%1)';
				|en = 'Add CryptoPro CSP license (%1)'"),
	...
	Если ЗаявлениеОтправлено И Не СостояниеОбработкиЗаявленияАктуально Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Для сбора технической информации о возникшей проблеме обновите состояние заявления.';
				|en = 'To gather information on the issue, update the application status.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьФайл(ПомещаемыйФайл, ОтказОтПомещенияФайла, ДополнительныеПараметры) Экспорт
	...
	Если РазмерФайлаВБайтах > 2*1024*1024 Тогда 
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'The file size exceeds the limit of 2MB: %1 (%2 byte).
			|Complress this file or choose another one.'"),
	...
	Если СтрНайти(ПоддерживаемыеРасширения, 
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'The file format is not supported: %1.
			|Save it as %2 or choose another file.'"),
...
Функция СвойстваУЦ(ВариантУдостоверяющегоЦентра)
	...
	Если ВариантУдостоверяющегоЦентра = "KA" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '248023, г. Калуга, пер. Теренинский, д. 6';
													|en = 'Tereninsky Lane, 6, Kaluga, 248023'");
	...
	ИначеЕсли ВариантУдостоверяющегоЦентра = "RT" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '115114, Москва, ул. Кожевническая, д. 14, стр. 5';
													|en = 'Kozhevnicheskaya St, 14, bldg 5, Moscow, 115114'");
	...
	ИначеЕсли ВариантУдостоверяющегоЦентра = "TK" Тогда
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '119034, г. Москва, Барыковский пер., д. 4, стр. 2';
													|en = 'Barykovsky Lane, 4, bldg 2, Moscow, 119034'");
	...
	Иначе
		...
		СвойстваУЦ.ПолноеНаименованиеУЦ     = НСтр("ru = 'ООО «Научно-производственный центр ""1С""»';
													|en = 'OOO «Nauchno-proizvodstvernniy centr ""1C""»'");
		...
		СвойстваУЦ.ПолныйЮридическийАдресУЦ = НСтр("ru = '119285, г. Москва, ул. Мосфильмовская, д. 42 стр. 1, эт/пом/ком 1/1/7';
													|en = 'Mosfilmovskaya St, 42, bld. 1, Floor/Office/Room 1/1/7, Moscow, 119285'");
		СвойстваУЦ.ЮридическийАдресУЦ       = НСтр("ru = '119285, г. Москва, ул. Мосфильмовская, д. 42 стр. 1';
													|en = 'Mosfilmovskaya St, 42, bld. 1, Moscow, 119285'");
...
Процедура УстановитьЗаголовокДекорацияТекстЗаявления(ЗаявлениеОтправлено, ЭлектронноеПродление)
	...
	Если ЗаявлениеОтправлено Тогда
		Элементы.ДекорацияТекстЗаявления.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Текст заявления</a>';
				|en = '<a href = ""%1"">Application text</a>'"), "ТекстЗаявления");
	Иначе
		Элементы.ДекорацияТекстЗаявления.Заголовок = ?(ЭлектронноеПродление,
			СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<a href = ""%1"">Создать ключ электронной подписи</a>';
														|en = '<a href = ""%1"">Create digital signature key</a>'"), "ТекстЗаявления"),
			СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = 'Печать заявления: <a href = ""%1"">текст заявления</a>';
														|en = 'Print application: <a href = ""%1"">application text</a>.'"), "ТекстЗаявления"));
	КонецЕсли;
