...
Процедура ПодключитьДляПилотов(Команда)
	...
	Иначе
		...
		Кнопки.Добавить("Отключить", НСтр("ru = 'Отключить'; en = 'Disable'"));
		...
		ПараметрыВопроса.Заголовок = НСтр("ru = 'Подключение в пилотной программе'; en = 'Enable in the pilot application'");
...
"Отключить?'; en = 'To connect, in the pilot application, disable 1C:ITS Portal"
...
Процедура Отключить(Команда)
	...
	ПараметрыВопроса = РаспознаваниеДокументовСлужебныйКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	ПараметрыВопроса.Заголовок = НСтр("ru = 'Отключить распознавания документов'; en = 'Disable document recognition'");
	ПараметрыВопроса.КнопкаПоУмолчанию = "Отмена";
	
	РаспознаваниеДокументовСлужебныйКлиент.ПоказатьВопросПользователю(
		Обработчик,
		НСтр("ru = 'Отключить распознавание документов?"
"(можно восстановить подключение в любое время)'; en = 'Disable document recognition?"
...
Процедура ПолучитьСостоянияПараметровПодключенияРаспознавания()
	...
	Если АвторизацияПереопределена Тогда 
		СостояниеПодключенияРаспознавания = НСтр("ru = 'Сервис распознавания документов готов к использованию.'; en = 'The document recognition service is ready for use.'");
	ИначеЕсли ТипАутентификации = "ПоТикетуИТС" Тогда
		Если АккаунтАктивирован Тогда
			
			Если ОбщегоНазначения.РазделениеВключено() И ТипАутентификации = "ПоТикетуИТС" Тогда
				СостояниеПодключенияРаспознавания = НСтр("ru = 'Используется учетная запись Портала 1С:ИТС.'; en = '1C:ITS Portal account is used.'");
			Иначе
				СостояниеПодключенияРаспознавания = СтрШаблон(НСтр("ru = 'Используется учетная запись Портала 1С:ИТС для пользователя ""%1"".'; en = 'For the ""%1"" user, 1C:ITS Portal account is used.'"), Логин);
			КонецЕсли;
...
"Укажите свой e-mail и мы уведовим вас по факту подключения.'; en = '1C:ITS Portal account is used."
""
...
""
"Заявка на подключение сервиса распознавания документов отправлена и будет обработана в ближайшие дни."
"Укажите свой e-mail и мы уведовим вас по факту подключения.'; en = 'For the ""%1"" user, 1C:ITS Portal account is used."
""
"The request to activate the document recognition service has been sent and will be processed soon."
"Specify your e-mail and we will notify you of activation.'"),
					Логин);
		...
		КонецЕсли;
	ИначеЕсли ТипАутентификации = "ПоЛогинуПаролю" Тогда
		СостояниеПодключенияРаспознавания = СтрШаблон(НСтр("ru = 'Выполнен вход в пилотную программу для пользователя ""%1"".'; en = 'The ""%1"" user logged in to the pilot application.'"), Логин);
	Иначе
		Если ЗаполненыДанныеАутентификацииБИП Тогда
			СостояниеПодключенияРаспознавания = НСтр("ru = 'Сервис распознавания документов готов к использованию.'; en = 'The document recognition service is ready for use.'");
		Иначе
			СостояниеПодключенияРаспознавания = НСтр("ru = 'Для использования распознавания документов необходимо подключить интернет-поддержку пользователей.'; en = 'To use document recognition, connect to online user support.'");
		КонецЕсли;
	КонецЕсли;
