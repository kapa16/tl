...
&НаКлиенте
Перем СвойстваЯчеекТаблицы Экспорт; // Свойства ячеек таблицы
...
Процедура ДоговорСоздание(Элемент, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ИсточникПоляДоговорКонтрагент) ИЛИ
		...
		Сообщение.Текст = НСтр("ru = 'Перед созданием договора необходимо заполнить поля ""Контрагент"" и ""Организация""'; en = 'Before you create a contract, fill the ""Counterparty"" and ""Company"" fields'");
		Сообщение.Сообщить();
...
Процедура СоздатьАвансовыйОтчет(Команда)
	...
	ОткрытьФормуДокументаСДобавлениемЧека("Документ.уатАвансовыйОтчет.Форма.ФормаДокумента");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВАвансовыйОтчет(Команда)
	...
	ОткрытьФормуВыбораДокумента("Документ.уатАвансовыйОтчет.Форма.ФормаДокумента", "Документ.уатАвансовыйОтчет.ФормаВыбора", Отбор);
...
Процедура ОткрытьФормуДокументаСДобавлениемЧека(ИмяФормыДокумента, ДокументСсылка = Неопределено)
	...
	ПараметрыФормы = ОткрытьФормуДокументаСДобавлениемЧекаСервер(ДокументСсылка); 
	НайтиИЗаполнитьСозданныеДокументы();
	ПрикрепитьСканНаСервереИОтметитьОбработанным(ПараметрыФормы.Ключ);

	ОписаниеОповещения = Новый ОписаниеОповещения("АвансовыйОтчетПослеЗакрытия", ЭтотОбъект);
	
	ОткрытьФорму(ИмяФормыДокумента, ПараметрыФормы,,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаСервере
Функция ОткрытьФормуДокументаСДобавлениемЧекаСервер(ДокументСсылка)
	ПараметрыФормы = Новый Структура();
	ЭлементыОтбора = Новый Структура();
	
	МассивТовары     = Новый Массив;
	текТаблицаДокумента = РаспознаваниеДокументовСлужебный.ЗаполненнаяТаблицаДокумента(Объект);
	Для Каждого СтрокаТаблицы Из текТаблицаДокумента Цикл
		ДанныеСтрокиТоваров = Новый Структура("Номенклатура,ЕдиницаИзмерения,Количество,Сумма,Цена,Контрагент,СтавкаНДС,СуммаНДС,
		|НомерВходящегоДокумента,ДатаВходящегоДокумента,НаименованиеВходящегоДокумента");
		ЗаполнитьЗначенияСвойств(ДанныеСтрокиТоваров, СтрокаТаблицы);  
		ДанныеСтрокиТоваров.Контрагент              = Объект.РеквизитыДокумента[2].Значение;
		ДанныеСтрокиТоваров.НомерВходящегоДокумента = Объект.РеквизитыДокумента[0].Значение;
		ДанныеСтрокиТоваров.ДатаВходящегоДокумента  = Объект.РеквизитыДокумента[1].Значение;
		ДанныеСтрокиТоваров.НаименованиеВходящегоДокумента = НСтр("ru = 'Кассовый чек'; en = 'Cash receipt'");
		
		Если НЕ ЗначениеЗаполнено(ДанныеСтрокиТоваров.ЕдиницаИзмерения)
			И ТипЗнч(ДанныеСтрокиТоваров.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			ДанныеСтрокиТоваров.ЕдиницаИзмерения = ДанныеСтрокиТоваров.Номенклатура.ЕдиницаХраненияОстатков;
		КонецЕсли;
		МассивТовары.Добавить(ДанныеСтрокиТоваров);
		
	КонецЦикла; 
	ЭлементыОтбора.Вставить("Материалы", МассивТовары);
	
	Если ДокументСсылка <> Неопределено Тогда
		ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЭлементыОтбора);
		ПараметрыФормы.Вставить("Ключ", ДокументСсылка); 
	Иначе 
		ДокументОбъект = Документы.уатАвансовыйОтчет.СоздатьДокумент(); 
		ДокументОбъект.Заполнить(ЭлементыОтбора); 
		ДокументОбъект.Записать(); 
		
		ПараметрыФормы.Вставить("Ключ", ДокументОбъект.Ссылка);
	КонецЕсли;
...
&НаСервере
Функция ПрикрепитьСканНаСервереИОтметитьОбработанным(Кандидат = Неопределено)
	
	Если Кандидат = Неопределено Тогда
		Кандидат = СозданныеДокументы.АвансовыйОтчет.Ссылка;
	КонецЕсли;  
...
Процедура Подключаемый_СозданиеТипового(ПараметрыОперации) Экспорт
	...
	Если СозданныйДокумент <> Неопределено Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Создан документ'; en = 'Document is created'"), НавигационнаяСсылка, Строка(СозданныйДокумент));
...
Процедура ЗаполнитьТаблицуДокумента()
	...
	Для Каждого Запись Из Объект.РеквизитыТабличныхЧастей Цикл
		...
		Если Запись.ИмяРеквизита = "Номенклатура" Тогда
			...
			Элемент.Оформление.УстановитьЗначениеПараметра("Текст", СтрШаблон(НСтр("ru = 'Не сопоставлен: %1'; en = 'Not mapped: %1'"), Запись.РаспознанныйТекст));
		КонецЕсли;
	КонецЦикла;
...
Процедура НастроитьИзменяемыйЭлемент(РеквизитШапки, ИзменяемыйЭлемент, ДанныеДляВыбораРеквизитов)
	...
	Если Не ЗначениеЗаполнено(РеквизитШапки.Значение) Тогда
		...
		Если РеквизитШапки.ИмяРеквизита <> "ИтогоСумма" И РеквизитШапки.ИмяРеквизита <> "ИтогоСуммаНДС"
			...
			ИзменяемыйЭлемент.ПодсказкаВвода = СтрШаблон(НСтр("ru = 'Не сопоставлен: %1'; en = 'Not mapped: %1'"), РаспознанныйТекст);
		КонецЕсли;
...
Функция СоздатьДокумент(ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Исключение
		...
		УдалосьПровести = Ложь;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка проведения основного документа:'; en = 'Main document posting error:'") + Символы.ПС
	...
	Если Не УдалосьПровести Тогда
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Ошибка создания документа:'; en = 'Document creation error:'") + Символы.ПС
				+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Сообщение.Сообщить();
...
Процедура ИнициализироватьСтруктуруСозданныеДокументы()
	...
	СозданныеДокументы.Вставить("АвансовыйОтчет", Новый Структура("Ссылка", Неопределено));
...
Процедура НайтиИЗаполнитьСозданныеДокументы()
	...
	СозданныеДокументы.АвансовыйОтчет.Ссылка = Неопределено;
	...
	Для Каждого ЭтотСвязанный Из ВсеСвязанные Цикл
		...
		Для Каждого ЭтотТип Из ПодходящиеТипы Цикл
			...
			Если КомплектныеСтроки.Количество() <> 0 Тогда
				...
				Если ТипСвязанного = Тип("ДокументСсылка.уатАвансовыйОтчет") Тогда
					СозданныеДокументы.АвансовыйОтчет.Ссылка = ЭтотСвязанный;
				КонецЕсли;
...
#Область ДокументыКомплектные
	...
	Для Каждого СтрокаТаблицы Из ДокументыКомплектные Цикл
		...
		ВидимыйЭлемент.Заголовок = СтрШаблон(НСтр("ru = '%1 № %2 от %3'; en = '%1 No. %2 of %3'"),
...
#Область ДокументыДубли
		...
		Иначе
			...
			Для Каждого СтрокаДубля Из НовыеДубли Цикл
				...
				Элементы["ОткрытьДубльДокумента_" + ИдДубляДокумента].Заголовок = СтрШаблон(НСтр("ru = '%1 № %2 от %3'; en = '%1 No. %2 of %3'"),
...
#КонецОбласти
	...
	Если КоличествоСвязанных = 0 Тогда
		...
		Элементы.ГруппаВсплывающаяСвязанныеДокументы.Заголовок = НСтр("ru = 'Нет связаных документов'; en = 'No related documents'");
	Иначе
		...
		Элементы.ГруппаВсплывающаяСвязанныеДокументы.Заголовок =
			СтрШаблон(НСтр("ru = 'Связанные документы (%1)'; en = 'Related documents (%1)'"), КоличествоСвязанных);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненныйРеквизитЦветБЭД);
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<заполняется автоматически>'; en = '<filled in automatically>'"));
