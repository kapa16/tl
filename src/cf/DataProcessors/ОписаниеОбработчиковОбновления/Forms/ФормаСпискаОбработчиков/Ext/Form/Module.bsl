...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, НСтр("ru = 'Данные были изменены. Сохранить изменения в репозиторий?';
												|en = 'The data has been changed. Do you want to save the changes to the repository?'"), РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
...
Процедура ЗаполнитьБыстрыеОтборы(Знач ОписаниеОбработчиков = Неопределено)
	...
	ДобавитьПрочийБыстрыйОтбор(ОписаниеОбработчиков, "НачальноеЗаполнение", НСтр("ru = 'Начальное заполнение';
																				|en = 'Initial population'"), "НачальноеЗаполнение");
...
Процедура ЗаписатьОшибку(ИмяОбъекта, ОписаниеОшибки = "",
	...
	Ошибка.ТипОшибки              = НСтр("ru = 'Построение очереди отложенных обработчиков обновления';
										|en = 'Build a queue of deferred update handlers'");
