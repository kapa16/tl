...
Процедура УстановитьДоступностьРегистрации(Форма, ОбъектXDTO) Экспорт
	...
	Если Форма.Элементы.Зарегистрировать.Видимость Тогда
		Форма.Элементы.ГруппаРегНомерДата.Подсказка =
			НСтр("ru = 'Для регистрации документа используйте кнопку ""Зарегистрировать"" в командной панели.'; en = 'Use ""Register"" button in the command bar to register the document.'");
	Иначе
		Форма.Элементы.ГруппаРегНомерДата.Подсказка = "";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидДокументаID) И ЗначениеЗаполнено(ВидДокументаТип) Тогда
		...
		Исключение
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Вид документа ""%1"" не найден в 1С:Документообороте'; en = 'The ""%1"" document kind is not found in 1C:Document Management '"),
				Форма.ВидДокумента);
		КонецПопытки;
...
Функция ПроверитьВозможностьСозданияДокументаНеПоШаблону(Форма, documentType)
	...
	Если documentType <> Неопределено
		...
		Сообщение.Текст = СтрШаблон(
			НСтр("ru = 'В настройках вида документа ""%1"" установлен запрет на создание документов не по шаблону.'; en = 'The prohibition on creating documents not based on the template is set in the settings for the document kind ""%1"".'"),
			Форма.ВидДокумента);
