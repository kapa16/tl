...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПоказыватьPDFСредствами1С Тогда
		...
		МодульИнтеграцияС1СДокументооборот3.ВывестиPDFДокументНаФорму(ЭтотОбъект, "ГруппаPDF");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ДвоичныеДанные) = Тип("ДвоичныеДанные") Или ЗначениеЗаполнено(ПредставлениеHTML) Тогда
		...
		HTMLПредпросмотрОбъекта.Вставить("Расширение", Расширение);
	...
	Иначе
		Элементы.ГруппаСтраницыПревью.Видимость = Ложь;
	КонецЕсли;
...
Асинх Процедура ЯвляетсяОригиналомПриИзменении(Знач Элемент)
	...
	Если ЗначениеЗаполнено(Шаблон) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Нельзя отметить как оригинал файл, созданный из шаблона.'; en = 'You cannot mark a file created from the template as an original.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЯвляетсяОригиналом
		...
		ПараметрыВопросаДаНет.Заголовок = НСтр("ru = 'Отметка оригинала'; en = 'Original mark'");
...
"Вы действительно хотите отметить его как оригинал?'; en = 'The selected file might not be a scanned copy."
"Do you want to mark it as an original document?'"),
...
Процедура ФайловыеОперацииЕще(Команда)
	...
	Если ДоступенЗахватИРедактирование Тогда
		Подменю.Добавить("ОткрытьКаталогФайла", НСтр("ru = 'Открыть каталог файла'; en = 'Open file directory'"),,
	...
	Если (Не Редактируется Или РедактируетсяТекущимПользователем) И РазрешеноРедактирование И Не ЕстьПодписи Тогда
		Подменю.Добавить("ОбновитьИзФайлаНаДиске", НСтр("ru = 'Обновить из файла на диске'; en = 'Update from a file on a hard drive.'"),,
	...
	Если ДоступенЗахватИРедактирование И РедактируетсяТекущимПользователем И Не ЕстьПодписи Тогда
		Подменю.Добавить("ОтменитьРедактирование", НСтр("ru = 'Отменить редактирование'; en = 'Cancel editing'"),,
			БиблиотекаКартинок.ОсвободитьФайл);
		Если РазрешеноРедактирование Тогда
			Подменю.Добавить("СохранитьИзменения", НСтр("ru = 'Сохранить изменения'; en = 'Save changes'"),,
...
Процедура ОбновитьПредпросмотр(ИгнорироватьМаксРазмер = Ложь)
	...
	Если ИспользоватьИнтеграциюДО3 Тогда
		...
		ВернутьДвоичныеДанные = ПоказыватьPDFСредствами1С И НРег(Расширение) = "pdf";
...
Процедура ЗаполнитьФорму(ОписаниеФайла)
	...
	Если ОписаниеФайла.Свойство("Владелец") И ЗначениеЗаполнено(ОписаниеФайла.Владелец) Тогда
		...
		Если ВладелецТип = "DMDocument" Тогда
			ТипВладельца = НСтр("ru = 'Документ'; en = 'Document'");
			
		ИначеЕсли ВладелецТип = "DMDocumentFolder" Или ВладелецТип = "DMInternalDocumentFolder" Тогда
			ТипВладельца = НСтр("ru = 'Папка'; en = 'Folder'");
			
		ИначеЕсли ВладелецТип = "DMIncomingEMail" Тогда
			ТипВладельца = НСтр("ru = 'Вход. письмо'; en = 'Incoming email'");
			
		ИначеЕсли ВладелецТип = "DMOutgoingEMail" Тогда
			ТипВладельца = НСтр("ru = 'Исх. письмо'; en = 'Outgoing email'");
			
		ИначеЕсли ВладелецТип = "DMInternalDocument" Тогда
			ТипВладельца = НСтр("ru = 'Внутр. документ'; en = 'Internal document'");
			
		ИначеЕсли ВладелецТип = "DMIncomingDocument" Тогда
			ТипВладельца = НСтр("ru = 'Вход. документ'; en = 'Incoming document'");
			
		ИначеЕсли ВладелецТип = "DMOutgoingDocument" Тогда
			ТипВладельца = НСтр("ru = 'Исх. документ'; en = 'Outgoing document'");
	...
	Иначе
		...
		Элементы.ДекорацияРедактируется.Заголовок = ?(Редактируется, НСтр("ru = 'Редактируется'; en = 'Edited'"), "");
	КонецЕсли;
	...
	ТекстЗаголовка = НСтр("ru = 'ЭП'; en = 'EP'");
	...
	Иначе
		...
		Если Зашифрован Тогда
			Если КолВоПодписей = 0 Тогда
				СтатусЭП = 1;
				Элементы.КартинкаЭП.Подсказка = НСтр("ru = 'Зашифрован. Не подписан электронной подписью'; en = 'Encrypted. Not digitally signed'");
			Иначе
				СтатусЭП = 3;
				Элементы.КартинкаЭП.Подсказка = НСтр("ru = 'Зашифрован. Подписан электронной подписью. Подпись не проверена.'; en = 'Encrypted. Digitally signed. The signature is not verified.'");
			КонецЕсли;
		Иначе
			Если КолВоПодписей = 0 Тогда
				СтатусЭП = 0;
				Элементы.КартинкаЭП.Подсказка = НСтр("ru = 'Не подписан электронной подписью'; en = 'Not digitally signed'");
			Иначе
				СтатусЭП = 2;
				Элементы.КартинкаЭП.Подсказка = НСтр("ru = 'Подписан электронной подписью. Подпись не проверена.'; en = 'Digitally signed. The signature is not verified.'");
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если ЕстьЗадачи Тогда
			...
			Элементы.КартинкаВзаимодействие.Подсказка = НСтр("ru = 'По файлу выполняются задачи'; en = 'Tasks are being executed by the file'");
		Иначе
			СтатусВзаимодействия = 0;
			Элементы.КартинкаВзаимодействие.Подсказка = НСтр("ru = 'По файлу нет выполняющихся задач'; en = 'There are no tasks that are being executed by the file'");
		КонецЕсли;
...
Процедура ПолучитьОписаниеФайлаИЗаполнитьФорму()
	...
	ИначеЕсли ЗначениеЗаполнено(ОригиналID) Тогда
		ОписаниеФайла = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ОписаниеФайла(
			...
			АдресВременногоХранилищаФайла);
	Иначе
		ВызватьИсключение НСтр("ru = 'Не указан идентификатор файла.'; en = 'File ID is not specified.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ID) Тогда
		Заголовок = СтрШаблон(НСтр("ru = '%1 (Файл)'; en = '%1 (File)'"), Наименование);
	Иначе
		Заголовок = НСтр("ru = 'Файл (создание)'; en = 'File (creation)'");
	КонецЕсли;
...
Процедура ЗаписатьВыполнитьКлиент(Отказ)
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru = 'Наименование'; en = 'Name'")),,
			"Наименование");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
