...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресСервиса = ИнтеграцияС1СДокументооборотБазоваяФункциональность.АдресВебСервиса1СДокументооборот(
		ВызовДляПользователяЗаданияОбмена Или ВызовДляНастройкиДоступа Или Параметры.ПодключениеКЦентральномуУзлу);
	...
	Если ВызовДляПользователяЗаданияОбмена Или ЭтоПользовательЗаданияОбмена Тогда
		ИмяПользователя = ИмяПользователяДляОбмена;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ВызовДляПользователяЗаданияОбмена И Не ЭтоПользовательЗаданияОбмена Тогда
...
Процедура ОК(Команда)
	...
	Если Не ЗначениеЗаполнено(ИмяПользователя)
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не заполнено имя пользователя 1С:Документооборота.'; en = '1C:Document Management username is required.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура АдресВебСервисаДокументооборота(Команда)
	
	ПоказатьПредупреждение(, АдресСервиса,, НСтр("ru = 'Подключение будет выполнено по адресу:'; en = 'Connection will be performed at:'"));
...
Процедура ПроверитьИспользованиеАутентификацииБезЛогина()
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		...
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьЗапросАсинхронно(
			...
			Истина,
			НСтр("ru = 'Установка соединения с 1С:Документооборот.'; en = 'Connecting to 1C:Document Management.'"));
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПрочитатьНастройкиАвторизации()
	
	НастройкиАвторизации =
		Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПрочитатьНастройкиАвторизацииАсинх();
...
Процедура УстановитьВидимостьЭлементов(УстановитьОтметкаНезаполненного = Ложь)
	...
	ВидимостьПоясненияАвторизации = ВызовДляПользователяЗаданияОбмена
		Или (ВызовДляНастройкиДоступа И Не ЗначениеЗаполнено(ИмяПользователяДляОбмена));
	...
	Элементы.ДекорацияИспользуетсяАутентификацияОС.Видимость = ИспользуетсяАутентификацияОС
		И Не ВидимостьПоясненияАвторизации;
	Элементы.ДекорацияИспользуетсяАутентификацияJWT.Видимость = ИспользуетсяАутентификацияJWT
		И Не ВидимостьПоясненияАвторизации;
	Элементы.ПояснениеАвторизация.Видимость = ВидимостьПоясненияАвторизации;
	Элементы.ПустаяДекорация.Видимость = Не ВызовДляНастройкиДоступа
		И Не ВидимостьПоясненияАвторизации
		И Не ИспользуетсяАутентификацияОС
...
Асинх Процедура ЗакончитьАвторизацию()
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПроверитьПодключение(
		...
		Если ВызовДляНастройкиДоступа
				И ЗначениеЗаполнено(ИмяПользователя)
				И Не ЗначениеЗаполнено(ИмяПользователяДляОбмена) Тогда
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СохранитьНастройкиАвторизацииДляОбмена(
				ИмяПользователя,
				Пароль);
		КонецЕсли;
	...
	Иначе
		...
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Подробнее...'; en = 'Details...'");
...
"именем пользователя и паролем. Обратитесь к администратору.'; en = 'Cannot connect to 1C:Document Management"
"with the specified username and password. Contact the administrator.'"),
