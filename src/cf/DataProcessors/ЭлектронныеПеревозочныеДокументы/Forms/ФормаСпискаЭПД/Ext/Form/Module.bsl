...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеДокументовЭПД") Тогда
		Элементы.ГруппаСоздать.Видимость = Ложь;
		Элементы.СписокГруппаКомандыСписка.Доступность = Ложь;
	КонецЕсли;
	...
	ОбменСГИСЭПДПереопределяемый.ПриСозданииНаСервереФормыСпискаЭПД(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Файл"
		...
		Или ИмяСобытия = "МППривязаноКДокументу" 
		// {УАТ}
		ИЛИ ИмяСобытия = "ФормаПодбораЭПД_Выбор" Тогда
		// {/УАТ}
	...
	Иначе
		...
		ОбменСГИСЭПДКлиентПереопределяемый.ОбработкаОповещенияНаФормеСпискаЭПД(ЭтотОбъект, ИмяСобытия, Параметр, Источник);
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, СтрокиСписка)
	...
	Если СтрокаСписка.Данные.Свойство("ДокументОснование") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|	ЭлектронныйЗаказНарядДокументыОснования.ДокументОснование.Представление
			|ИЗ
			...
			|	ЭлектроннаяСопроводительнаяВедомостьДокументыОснования.ДокументОснование.Представление
			|ИЗ
			...
			|ГДЕ
			|	ЭлектроннаяСопроводительнаяВедомостьДокументыОснования.Ссылка В (&Документы)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ЭлектронныйДоговорФрахтованияДокументыОснования.Ссылка,
			|	ЭлектронныйДоговорФрахтованияДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектронныйДоговорФрахтования.ДокументыОснования КАК ЭлектронныйДоговорФрахтованияДокументыОснования
			|ГДЕ
			|	ЭлектронныйДоговорФрахтованияДокументыОснования.Ссылка В (&Документы)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ЭлектронныйЗаказЗаявкаДокументыОснования.Ссылка,
			|	ЭлектронныйЗаказЗаявкаДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектронныйЗаказЗаявка.ДокументыОснования КАК ЭлектронныйЗаказЗаявкаДокументыОснования
			|ГДЕ
			|	ЭлектронныйЗаказЗаявкаДокументыОснования.Ссылка В (&Документы)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ЭлектронныйПутевойЛистДокументыОснования.Ссылка,
			|	ЭлектронныйПутевойЛистДокументыОснования.ДокументОснование.Представление
			|ИЗ
			|	Документ.ЭлектронныйПутевойЛист.ДокументыОснования КАК ЭлектронныйПутевойЛистДокументыОснования
			|ГДЕ
			|	ЭлектронныйПутевойЛистДокументыОснования.Ссылка В (&Документы)";
	...
	ОбменСГИСЭПДПереопределяемый.СписокПриПолученииДанныхНаСервереФормыСпискаЭПД(ИмяЭлемента, Настройки, СтрокиСписка);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОбменСГИСЭПДКлиентПереопределяемый.СписокВыборНаФормеСпискаЭПД(ЭтотОбъект, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
	
	Если СтандартнаяОбработка = Истина 
		И Поле = Элементы.ПредставлениеСостояния Тогда
	...
	Если СтандартнаяОбработка = Истина 
		И ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД.СервисВзаимодействияМПЭПД") Тогда
...
// {УАТ}
&НаКлиенте
Процедура ПодобратьДокументыЭПД(Команда)
	ВыбраннаяСтрока = Элементы.Список.ТекущаяСтрока;
	Если ВыбраннаяСтрока <> Неопределено Тогда
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ВыбраннаяСтрока);
		Если ТипЗнч(ДанныеСтроки.Ссылка) = Тип("ДокументСсылка.ЭлектронныйЗаказЗаявка")
			ИЛИ ТипЗнч(ДанныеСтроки.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
			ПараметрыЭПД = Новый Структура("ЭПД", ДанныеСтроки.Ссылка);
			ОткрытьФорму("Обработка.ЭлектронныеПеревозочныеДокументы.Форма.уатФормаПодбораЭПД", ПараметрыЭПД); 
		Иначе
