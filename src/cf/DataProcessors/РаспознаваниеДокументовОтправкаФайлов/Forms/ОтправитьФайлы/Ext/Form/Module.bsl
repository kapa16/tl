...
Процедура Распознать(Команда)
	...
	ТекстОписанияОтправки = НСтр("ru = 'Подготовка файлов к передаче...'; en = 'Preparing files to transfer...'");
...
Процедура ЗагрузитьПревью()
	...
	Если Не ФокусНаСправку Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			ТекстНевыбраннойКартинки = НСтр("ru = 'Выберите файл для предварительного просмотра'; en = 'Select file to preview'");
		ИначеЕсли ПустаяСтрока(ТекущиеДанные.Адрес) Тогда 
			ТекстНевыбраннойКартинки = НСтр("ru = 'Файл в процессе загрузки. Пожалуйста подождите...'; en = 'Importing the file. Please wait...'");
		...
		ИначеЕсли ЭтоРасширениеPDF(ТекущиеДанные.Расширение) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстНевыбраннойКартинки = НСтр("ru = 'Предварительный просмотр файлов данного типа не поддерживается'; en = 'Cannot preview files of this type'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуФайловИнтерактивно()
	...
	ПараметрыДиалога = Новый ПараметрыДиалогаПомещенияФайлов(НСтр("ru = 'Выберите файлы для распознавания'; en = 'Select files for recognition'"));
...
Процедура ПриВыполненииПомещенияФайлов(ПомещаемыйФайл, Помещено, ОтказОтПомещенияФайла, ПомещеноВсего, ОтказОтПомещенияВсехФайлов, Контекст) Экспорт
	...
	ТекстСообщенияЗагрузки = СтрШаблон(
		НСтр("ru = 'Загрузка %1 (%2)'; en = 'Import %1 (%2)'"),
		ПомещаемыйФайл.Имя,
...
Функция ЗапуститьФоновуюОтправкуФайлов()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Отправка файлов в сервиса распознавания документов'; en = 'Send files to the document recognition service'");
