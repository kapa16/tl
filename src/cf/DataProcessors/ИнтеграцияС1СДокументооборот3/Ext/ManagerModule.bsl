...
Процедура ВыполнитьПоискПоРеквизитам(ТипОбъекта, РеквизитыПоиска, АдресВоВременномХранилище,
	...
	Если Представление = "" Тогда
		Представление = НСтр("ru = 'Условия не заданы.'; en = 'Conditions are not specified.'");
	Иначе
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(Результат, "tooManyObjects")
		...
		Представление = Представление
			+ " " + НСтр("ru = 'Перетащите сюда любой реквизит, чтобы уточнить условия.'; en = 'Drag an attribute here to refine the criteria.'");
	Иначе
	...
	Для Каждого Предмет Из МассивПредметов Цикл
		...
		Если СвязанныйОбъектДО = Неопределено Тогда
			...
			Если КомандаПечати = "ApprovalSheet" Или КомандаПечати = "ApprovalSheetWithHistory" Тогда
				ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Лист согласования'; en = 'Approval sheet '");
				ОбластьШапка.Параметры.Пояснение = НСтр("ru = 'Согласование в 1С:Документообороте не запускалось.'; en = 'Coordination in 1C:Document Management was not started.'");
			ИначеЕсли КомандаПечати = "ProtocolOfDisagreements" Тогда
				ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Протокол разногласий'; en = 'Protocol of disagreements'");
				ОбластьШапка.Параметры.Пояснение = НСтр("ru = 'Согласование в 1С:Документообороте не запускалось.'; en = 'Coordination in 1C:Document Management was not started.'");
			ИначеЕсли КомандаПечати = "ExaminationSheet" Тогда
				ОбластьШапка.Параметры.Заголовок = НСтр("ru = 'Лист ознакомления'; en = 'Preview sheet'");
				ОбластьШапка.Параметры.Пояснение = НСтр("ru = 'Ознакомление в 1С:Документообороте не запускалось.'; en = 'Preview in 1C:Document Management was not started.'");
			КонецЕсли;
