...
"Переход к ""Документооборот"" возможен только после записи данных.'; en = 'The data is not saved."
"To go to ""Document management"", save the data.'");
...
Процедура ДекорацияОбъектВыбратьНажатие(Элемент)
	...
	Иначе // Тип неизвестен, предложим выбор пользователю.
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Выбрать в 1С:Документообороте'; en = 'Select from 1C:Document Management'"));
		ПараметрыФормы.Вставить("ЗаголовокКоманды", НСтр("ru = 'Выбрать'; en = 'Select'"));
...
"%1?'"), Строка(ВнешнийОбъект));
	...
	ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Очистить'; en = 'Clear'");
	ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Не очищать'; en = 'Do not clean'");
...
Процедура ПредставлениеHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если СтрНайти(ДанныеСобытия.Href, "v8doc:status") И Элементы.СтраницаОбработка.Видимость Тогда
...
Процедура ДеревоСвязейПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекущиеДанные = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	Если ТекущиеДанные.ЭтоВеткаСвязи Или ТекущиеДанные.ЭтоРольФайла Тогда
		ТекущиеДанные = ТекущиеДанные.ПолучитьРодителя();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТекущиеДанные.СвязанныйОбъектID)
			Или Не ЗначениеЗаполнено(ТекущиеДанные.СвязанныйОбъектТип) Тогда
		Возврат;
	КонецЕсли;
	
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ОткрытьОбъект(
		ТекущиеДанные.СвязанныйОбъектТип,
	...
	Если СтрокаРодитель <> Неопределено
		...
		Если ПодчиненнаяСтрока.СлужебнаяСтрока Тогда
			Если ДеревоСвязейРазворачиваемаяСтрока <> 0 Тогда
				...
				Возврат;
			КонецЕсли;
			ПодчиненнаяСтрока.СвязанныйОбъект = НСтр("ru = 'Ожидается получение данных из 1С:Документооборота...'; en = 'Expecting data from 1C:Document Management...'");
...
Процедура ДеревоСвязейПриАктивизацииСтроки(Элемент)
	...
	ЭтоСсылочныйОбъект = ЗначениеЗаполнено(ТекущиеДанные.СвязанныйОбъектID)
		И ЗначениеЗаполнено(ТекущиеДанные.СвязанныйОбъектТип);
	ЭтоФайл = (ТекущиеДанные.СвязанныйОбъектТип = "DMFile");
...
"Сформировать протокол разногласий можно на стороне 1С:Документооборот.'; en = 'The %1 application does not support printing of a protocol of disagreements."
...
#Если ВебКлиент Тогда
	ПоказатьПредупреждение(,НСтр("ru = 'В Веб-клиенте печать файлов не поддерживается.'; en = 'Printing of files is not supported in the web client.'"));
	Возврат;
#КонецЕсли
...
"Сформировать лист ознакомления можно на стороне 1С:Документооборот.'; en = 'The %1 application does not support the preview sheet printing."
...
"Сформировать лист согласования с историей можно на стороне 1С:Документооборот.'; en = 'The %1 application does not support printing of the approval sheet with history."
...
Процедура ОбновитьПредпросмотрСвязи(ОбновитьДанные = Ложь, ИгнорироватьМаксРазмер = Ложь)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ВернутьДвоичныеДанные = ПоказыватьPDFСредствами1С И (НРег(ТекущиеДанные.РасширениеФайла) = "pdf");
		...
		Если (ТекущиеДанные.СвязанныйОбъектТип = "DMFile"
					Или ИнтеграцияС1СДокументооборот3КлиентПовтИсп.ЭтоИзвестныйОбъектОбзора(ТекущиеДанные.СвязанныйОбъектТип))
				И (ОбновитьДанные
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ПредставлениеHTML) Тогда
			ПредставлениеСвязиHTML = ТекущиеДанные.ПредставлениеHTML;
		...
		СвязьРедактируетсяТекущимПользователем = ТекущиеДанные.РедактируетсяТекущимПользователем;
		СвязьПредпросмотрУрезан = ТекущиеДанные.ПредпросмотрУрезан;
...
&НаКлиенте
Процедура РазвернутьУровеньДереваСвязей()
	
	Если ДеревоСвязейРазворачиваемаяСтрока = 0 Тогда
		КоллекцияСтрокУровеньТиповСвязей = ДеревоСвязей.ПолучитьЭлементы();
	Иначе
		КоллекцияСтрокУровеньТиповСвязей = ДеревоСвязей.НайтиПоИдентификатору(ДеревоСвязейРазворачиваемаяСтрока).ПолучитьЭлементы();
	КонецЕсли;
	
	Для Каждого СтрокаТипСвязи Из КоллекцияСтрокУровеньТиповСвязей Цикл
		ИдентификаторСтроки = СтрокаТипСвязи.ПолучитьИдентификатор();
		Элементы.ДеревоСвязей.Развернуть(ИдентификаторСтроки, Ложь);
		КоллекцияСтрокУровеньСвязанныхОбъектов = СтрокаТипСвязи.ПолучитьЭлементы();
		Для Каждого СтрокаСвязанныйОбъект Из КоллекцияСтрокУровеньСвязанныхОбъектов Цикл
			ИдентификаторСтроки = СтрокаСвязанныйОбъект.ПолучитьИдентификатор();
			Элементы.ДеревоСвязей.Развернуть(ИдентификаторСтроки, СтрокаСвязанныйОбъект.ЭтоРольФайла);
			КоллекцияСтрокУровеньФайлов = СтрокаСвязанныйОбъект.ПолучитьЭлементы();
			Для Каждого СтрокаФайл Из КоллекцияСтрокУровеньФайлов Цикл
				Если Не СтрокаФайл.ЭтоВеткаСвязи Тогда
					ИдентификаторСтроки = СтрокаФайл.ПолучитьИдентификатор();
					Элементы.ДеревоСвязей.Развернуть(ИдентификаторСтроки, СтрокаФайл.ЭтоРольФайла);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Процедура ОбновитьВидимостьСтраниц(Знач ТипОбъектаДО)
	...
	Если ИнтеграцияС1СДокументооборот3.ЭтоИзвестныйОбъектОбзора(ТипОбъектаДО) Тогда
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.9.18") Тогда
			ДоступенОбзор = Ложь;
			Элементы.ДекорацияОписаниеОбзорНедоступен.Заголовок = НСтр("ru = 'Просмотр обзора не поддерживается в данной версии 1С:Документооборота."
"Требуется 1С:Документооборот версии 3.0.9 или выше.'; en = 'Overview is not supported in this version of 1C:Document Management."
"Version 3.0.9 or later is required.'");
		КонецЕсли;
	Иначе
		Элементы.СтраницаОбзор.Видимость = Ложь;
		ДоступенОбзор = Ложь;
		Элементы.ДекорацияОписаниеОбзорНедоступен.Заголовок = НСтр("ru = 'Интегрированный объект не поддерживает просмотр обзора.'; en = 'The integrated object does not support overview.'");
	КонецЕсли;
	...
	Если ИспользоватьОбработкуОбъектов1СДокументооборота Тогда
		Если ИнтеграцияС1СДокументооборот3.ОбъектДОПоддерживаетОбработку(ТипОбъектаДО) Тогда
			Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.12.22") Тогда
				...
				Элементы.ДекорацияОписаниеОбработкаНедоступна.Заголовок = НСтр("ru = 'Просмотр обработки не поддерживается в данной версии 1С:Документооборота."
"Требуется 1С:Документооборот версии 3.0.12 или выше.'; en = 'Viewing the data processor is not supported in this version of 1C:Document Management."
"Version 3.0.12 or later is required.'");
			КонецЕсли;
		Иначе
			Элементы.СтраницаОбработка.Видимость = Ложь;
			ДоступенПросмотрОбработки = Ложь;
			Элементы.ДекорацияОписаниеОбработкаНедоступна.Заголовок = НСтр("ru = 'Интегрированный объект не поддерживает функционал обработки.'; en = 'The integrated object does not support the data processor functionality.'");
		КонецЕсли;
	Иначе
		Элементы.СтраницаОбработка.Видимость = Ложь;
		ДоступенПросмотрОбработки = Ложь;
		Элементы.ДекорацияОписаниеОбработкаНедоступна.Заголовок = НСтр("ru = 'Функционал обработки выключен в настройках интеграции.'; en = 'The data processor functionality is disabled in the integration settings.'");
	КонецЕсли;
	...
	Если ИнтеграцияС1СДокументооборот3.ЭтоДокументДО3(ТипОбъектаДО) Или ТипОбъектаДО = "DMMeeting" Тогда
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.17.36") Тогда
			...
			Элементы.ДекорацияОписаниеСвязиНедоступны.Заголовок = НСтр("ru = 'Просмотр дерева связей не поддерживается в данной версии 1С:Документооборота."
"Требуется 1С:Документооборот версии 3.0.17 или выше.'; en = 'Viewing the relation tree is not supported in this version of 1C:Document Management."
"Version 3.0.17 or later is required.'");
		КонецЕсли;
	Иначе
		Элементы.СтраницаСвязи.Видимость = Ложь;
		ДоступноДеревоСвязей = Ложь;
		Элементы.ДекорацияОписаниеСвязиНедоступны.Заголовок = НСтр("ru = 'Интегрированный объект не поддерживает функционал связей.'; en = 'The integrated object does not support the relationship functionality.'");
	КонецЕсли;
...
Процедура ОбновитьЗаголовокСвязанногоОбъекта()
	...
	Иначе
		...
		Иначе
			...
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru = 'Соответствие в 1С:Документообороте:'; en = 'Correspondence in 1C:Document Management:'");
	...
	Если Элементы.ГруппаИнфо.Видимость Тогда
		...
		Если КоличествоПодходящихПравилИнтеграции = 0 Тогда
			...
			Если ПолноеИмяПотребителяПоЧастям[0] = "Документ" Тогда
				ПредставлениеТипаОбъектаИС = НСтр("ru = 'документа'; en = 'document'");
			ИначеЕсли ПолноеИмяПотребителяПоЧастям[0] = "Справочник" Тогда
				ПредставлениеТипаОбъектаИС = НСтр("ru = 'справочника'; en = 'catalog'");
			Иначе
				ПредставлениеТипаОбъектаИС = НСтр("ru = 'объекта'; en = 'object'");
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ОбъектДОТип) Тогда
				...
				Попытка
					ПредставлениеТипаОбъектаДО = НРег(ПолучитьСклоненияСтроки(
						...
						"ПД=Родительный")[0]);
				Исключение
					ПредставлениеТипаОбъектаДО = НСтр("ru = 'объекта'; en = 'object'");
				КонецПопытки;
				Инфо = СтрШаблон(НСтр("ru = 'Не настроено правило интеграции для заполнения %1 1С:Документооборота ""%2"" из %3 ""%4"".'; en = 'The integration rule to fill %1 of 1C:Document Management %2 from %3 %4 is not configured.'"),
					ПредставлениеТипаОбъектаДО,
					ОбъектДО,
			...
			Иначе
				...
				Инфо = СтрШаблон(НСтр("ru = 'Не настроено правило интеграции для создания объекта 1С:Документооборота из %1 ""%2"".'; en = 'The integration rule for creating a 1C:Document Management object from %1 ""%2"" is not configured.'"),
					ПредставлениеТипаОбъектаИС,
					ПредставлениеОбъектаИС);
			КонецЕсли;
...
"Измените правила или удалите лишние.'; en = 'Filling of different 1C:Document Management object types from ""%1"" is configured."
...
Процедура НачатьОбновлениеФормы(ТекущаяСтраница, ВыводитьОкноОжидания = Истина)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьЗапросАсинхронно(
		...
		ОповещениеОЗавершении,
		ВыводитьОкноОжидания,
		НСтр("ru = 'Получение данных из 1С:Документооборота.'; en = 'Get data from 1C:Document Management.'"),
		Ложь);
...
Функция ПолучитьДанныеСвязанногоОбъектаДО(ВыборкаИдентификаторОбъектаДО, ВыборкаТипОбъектаДО)
	...
	ИначеЕсли КоличествоСвязей = 1 Тогда
		...
		Если ДоступноДеревоСвязей И ТекущаяЗакладка = "СтраницаСвязи" Тогда
			...
			ЗапросыПакета.Добавить(
				ИнтеграцияС1СДокументооборот3.СвязиОбъектаЗапрос(Прокси, РеквизитыОбъекта));
		КонецЕсли;
		...
		Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьПакетныйЗапросАсинхронно(
			...
			ЗапросыПакета,
			УникальныйИдентификатор,
			НСтр("ru = 'Открытие формы Документооборот'; en = 'Open the Document Management form'"));
		
	ИначеЕсли КоличествоСвязей > 1 Тогда
		
		ВызватьИсключение СтрШаблон(НСтр("ru = 'С объектом ""%1"" связано более одного объекта в 1С:Документооборот."
"Общее количество связей: %2'; en = 'More than one object is linked to the ""%1"" object in 1C:Document Management."
...
Процедура ПолучитьДанныеСвязанногоОбъектаДОЗавершение(Знач ОтветНаЗапросыПакетаСтрока, Знач ПараметрыОповещения)
	...
	Если ДоступноДеревоСвязей И ПараметрыОповещения.ТекущаяСтраницаИмя = "СтраницаСвязи" Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОтветСвязи, "DMError") Тогда
			ТекстОшибкиПриОбновлении = ОтветСвязи.description;
			Элементы.ГруппаСтраницыПодключения.ТекущаяСтраница = Элементы.СтраницаОшибкаПриОбновлении;
		Иначе
...
Процедура ДействиеСОбработкой(ТипОперации, ПараметрыДействияСОбработкой = Неопределено) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьЗапросАсинхронно(
		...
		ОповещениеОЗавершении,
		Истина,
		НСтр("ru = 'Выполняется действие с обработкой.'; en = 'Performing a data processor action.'"));
