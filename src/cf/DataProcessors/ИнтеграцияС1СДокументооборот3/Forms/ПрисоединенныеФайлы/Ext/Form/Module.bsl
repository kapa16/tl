...
Процедура НачатьОбновлениеФайлов(ПараметрыОбновления)
	...
	ТекстСообщения = НСтр("ru = 'Получение списка файлов из 1С:Документооборот.'; en = 'Get a file list from 1C:Document Management.'");
		...
		Иначе
			Если ЗначениеЗаполнено(ПредставлениеВладельца) Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Просмотр списка файлов объекта ""%1"" доступен только на стороне 1С:Документооборот.'; en = 'You can only preview the file list of the ""%1"" object in 1C:Document Management.'"),
					ПредставлениеВладельца);
			Иначе
				ТекстОшибки = НСтр("ru = 'Владелец файлов не поддерживается бесшовной интеграцией."
"Просмотр списка файлов доступен только на стороне 1С:Документооборот.'; en = 'Seamless integration does not support the file owner."
...
""
"Показать файлы, хранящиеся на стороне %3?'; en = 'The ""%2"" %1 has attachments that were added to %3 before configuring file storage in 1C:Document Management."
...
"Show files stored in %3?'"),
					...
					ПараметрыПредупреждения.Заголовок = НСтр("ru = 'Архивные файлы'; en = 'Archived files'");
...
Процедура ДобавитьПечатнуюФормуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ФормаДобавления = Неопределено Тогда
		ТекстПредупреждения = СтрШаблон(НСтр("ru = '%1 не имеет печатных форм.'; en = '%1 has no print forms.'"), Строка(Владелец));
...
Процедура ОбновитьЭлементыФормы()
	...
	Если ЕстьПрисоединенныеФайлыВИС
		...
		ЗаголовокКоманды = СтрШаблон(
			НСтр("ru = 'Файлы %1 из %2...'; en = 'The %1 files from %2...'"),
			ПредставлениеТипаВладельца,
			СокращенноеНаименованиеКонфигурации);
	...
	Если ЕстьВладелец Тогда
		ЗаголовокКоманды = СтрШаблон(НСтр("ru = 'Печатную форму из %1...'; en = 'Print form from %1...'"), ПредставлениеТипаВладельца);
...
Процедура ОтправитьЧерезИС()
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ru = 'Отправка файлов не поддерживается'; en = 'File sending is not supported'"));
	КонецЕсли;
...
Функция ПараметрыИсходящегоПисьма(Знач Владелец, Знач Вложения, Знач УникальныйИдентификаторФормы)
	...
	Тема = СтрШаблон(НСтр("ru = '%1, %2 к документу %3 %4 от %5'; en = '%1, %2 to document %3 %4 from %5'"),
		?(ОдноВложение, НСтр("ru = 'Файл'; en = 'File'"), НСтр("ru = 'Файлы'; en = 'Files'")),
		?(ОдноВложение, НСтр("ru = 'присоединенный'; en = 'attached file'"), НСтр("ru = 'присоединенные'; en = 'attached files'")),
		ТипЗнч(Владелец),
		НомерДокумента,
		ДатаДокумента);
	...
	// Текст.
	Текст = СтрШаблон(НСтр("ru = 'К письму %1 %2:'; en = 'To email %1 %2:'"),
		?(ОдноВложение, НСтр("ru = 'присоединен'; en = 'attached'"), НСтр("ru = 'присоединены'; en = 'are attached'")),
		?(ОдноВложение, НСтр("ru = 'файл'; en = 'file'"), НСтр("ru = 'файлы'; en = 'files'")));
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		Текст = Текст + Символы.ПС + СтрШаблон(НСтр("ru = '- %1'; en = '- %1'"), Вложение.Представление);
	КонецЦикла;
