...
Процедура СрокПриИзменении(Элемент)
	...
	СрокПредставление = Формат(Срок, НСтр("ru = ""ДФ='dd.MM.yy HH:mm';""; en = ""DF='MM/dd/yy hh:mm tt';"""));
	Срок = Неопределено;
	...
	Если ЗначениеЗаполнено(ПараметрыИзмененияСрока.ТекстСообщенияПредупреждения) Тогда
		ИнтеграцияС1СДокументооборот3Клиент.ПоказатьПредупреждениеИзДО(,ПараметрыИзмененияСрока.ТекстСообщенияПредупреждения);
	КонецЕсли;
...
Процедура ИсполнителиИсполненияПриАктивизацииСтроки(Элемент)
	...
	Элементы.ИсполнителиИсполненияКонтекстноеМенюУдалить.Доступность = Не СостояниеЗавершено
		И Не ТекущиеДанные.Недоступно И РазныеСрокиДоступность;
...
Процедура ИсполнителиИсполненияПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные <> Неопределено
			И (ТекущиеДанные.Состояние = "Completed" Или ТекущиеДанные.Недоступно Или Не РазныеСрокиДоступность) Тогда
...
// Поле ИсполнителиИсполненияОписаниеОсобое
...
"Изменить признак ""Ответственный"" нельзя.'; en = 'The employee completed their task."
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если ПустаяСтрока(НомерПункта) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Поле ""№ пункта"" не заполнено'; en = 'The ""Point No."" field is not filled'"),,
			"НомерПункта",, Отказ);
	КонецЕсли;
	Если ПустаяСтрока(ОписаниеИсполнения) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Поле ""Содержание пункта"" не заполнено'; en = 'The ""Point content"" field is not filled'"),,
			"ОписаниеИсполнение",, Отказ);
	КонецЕсли;
	// Проверка наличия одного ответственного исполнения в списке.
	Если ИсполнителиИсполнения.Количество() = 1 И ИсполнителиИсполнения[0].Ответственный Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Единственный исполнитель не может быть ответственным.'; en = 'A single assignee cannot be responsible.'"),,
			"ИсполнителиИсполнения[0].Исполнитель",,
			Отказ);
	КонецЕсли;
	Если ИсполнителиИсполнения.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не введено ни одной строки в список ""Исполнители""'; en = 'The ""Assignees"" list is empty'"),,
	...
	Для Каждого Строка Из ИсполнителиИсполнения Цикл
		Если Не ЗначениеЗаполнено(Строка.ИсполнительID) Тогда
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не заполнена колонка ""Исполнитель"" в строке %1 списка ""Исполнители""'; en = 'The ""Assignee"" column is not filled in line %1 of the ""Assignees"" list'"),
				Индекс + 1);
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				ТекстОшибки,,
...
// Продолжение процедуры ИсполнителиИсполненияОписаниеОсобоеНачалоВыбора.
//
// Параметры:
//   СтруктураОписания - Структура - возвращается из формы редактирования описания:
//     * ОписаниеОсобое - Строка
//   СтрокаУчастника - ДанныеФормыЭлементКоллекции - строка участника, имеющая поля:
//     * ОписаниеОсобое - Строка
//
&НаКлиенте
...
Процедура ПереместитьИсполнителя(Смещение)
	...
	Текст = НСтр("ru = 'Перемещение невозможно. Ответственным может быть назначен только первый исполнитель в списке.'; en = 'Cannot transfer. Only the first assignee in the list can be set as responsible.'");
...
Процедура НастроитьВидимостьЭлементовУправленияПриСозданииФормы()
	...
	ПодсказкаВвода = НСтр("ru = 'Содержание пункта'; en = 'Point content'");
	...
	Заголовок = НСтр("ru = 'Пункт'; en = 'Point'");
