...
Процедура НаправитьНовымУчастникам(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("ru = 'Результаты новых участников не повлияют на результат действия и маршрут обработки, т.к. согласование уже пройдено.'; en = 'The results of new participants will not affect the action result and the route processing as the approval is already completed.'"));
...
Процедура ОбновитьДеревоУчастниковПоОбъекту(ДействиеXDTO)
	...
	Иначе
		ЭтапУчастников = ЭтапыУчастников.Добавить();
		ЭтапУчастников.Функция = "ApprovingPerson";
		ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Согласовать'; en = 'Approve'");
		ЭтапУчастников.ЭтоЭтап = Истина;
	...
	ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Ознакомиться с результатом согласования'; en = 'Review the approval result'");
	ЭтапУчастников.ЭтоЭтап = Истина;
...
"Удалите дубли, укажите особое описание или установите согласующему признак ""Защищенный""'; en = 'In the approval stages, the %1 approver is duplicated."
"Remove the duplicates, specify the special details, or select the ""Protected"" checkbox for the approver'"),
	...
	ИначеЕсли Не ЕстьОбрабатывающийРезультат Тогда
		ВызватьИсключение НСтр("ru = 'Отсутствует участник ""ознакомления с результатом согласования""'; en = 'The ""Approval result review"" participant is missing'");
	КонецЕсли;
...
"Для запуска нового согласования добавьте участников.'; en = 'New participants are not added."
...
Процедура УстановитьВидимостьДоступностьЭлементов(Форма)
	...
	ЗапрещеноМенятьПоНастройке = ЗначениеЗаполнено(Форма.НастройкаДействия) И Не Форма.ЕстьПравоАрбитража;
	...
	Форма.Элементы.МаксимальноеКоличествоЦиклов.Заголовок =
		СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';циклом;;циклами;циклами;циклами'; en = ';loop;;;;loops'"),
