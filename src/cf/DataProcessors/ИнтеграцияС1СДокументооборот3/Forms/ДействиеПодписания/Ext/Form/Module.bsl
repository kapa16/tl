...
Процедура НаправитьНовымУчастникам(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("ru = 'Результаты новых участников не повлияют на результат действия и маршрут обработки, т.к. подписание уже пройдено.'; en = 'The results of new participants will not affect the action result and the processing route as signing is already completed.'"));
...
Процедура ДобавитьОбеспечивающегоПодписание(Команда)
	...
	Если СтрокиПодписывающего.Количество() <> 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'У подписывающего может быть только один обеспечивающий подписание.'; en = 'A signer can have only one person who ensures signing.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДеревоУчастниковПоОбъекту(ДействиеXDTO)
	...
	Иначе
		ЭтапУчастников = ЭтапыУчастников.Добавить();
		ЭтапУчастников.Функция = "Signer";
		ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Подписать'; en = 'Sign'");
		ЭтапУчастников.ЭтоЭтап = Истина;
	...
	ЭтапУчастников.Порядок = "OneByOne";
	ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Ознакомиться с результатом подписания'; en = 'Review the signing result'");
	ЭтапУчастников.ЭтоЭтап = Истина;
...
"Удалите дубли, укажите особое описание или установите подписывающему признак ""Защищенный""'; en = 'In the stages, the %1 signer is duplicated."
...
"Remove the duplicates, specify the special details, or select the ""Protected"" checkbox for the participant'"),
	...
	ИначеЕсли Не ЕстьОбрабатывающийРезультат Тогда
		ВызватьИсключение НСтр("ru = 'Отсутствует участник ""ознакомления с результатом подписания""'; en = 'The ""Signing result review"" participant is missing'");
	КонецЕсли;
...
Процедура УстановитьСпособыПодписанияИзВидаПредмета(ДействиеXDTO)
	...
	Если ДействиеXDTO.target.objectID.type = "DMDocument" Тогда
		...
		Если ДействиеXDTO.targetForm = "Paper" Тогда
			...
			Элементы.СпособПодписания.СписокВыбора.Добавить("OnPaper", НСтр("ru = 'На бумаге'; en = 'On paper'"));
		...
		ИначеЕсли ДействиеXDTO.targetForm = "Electronic" Тогда
			Элементы.СпособПодписания.РежимВыбораИзСписка = Истина;
			Элементы.СпособПодписания.СписокВыбора.Очистить();
			Элементы.СпособПодписания.СписокВыбора.Добавить("BasicDS", НСтр("ru = 'Простая ЭП'; en = 'Simple digital signature'"));
			Если ИспользоватьЭлектронныеПодписи Тогда
				Элементы.СпособПодписания.СписокВыбора.Добавить("EnhancedDS", НСтр("ru = 'Усиленная ЭП'; en = 'Enhanced digital signature'"));
			КонецЕсли;
...
"Для запуска нового подписания добавьте участников.'; en = 'New participants are not added."
