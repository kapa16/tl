...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоДокументДО3 Тогда
		Заголовок = НСтр("ru = 'Условия поиска документа предприятия'; en = 'Enterprise document search conditions'");
		
	ИначеЕсли ТипОбъектаВыбора = "DMCorrespondent" Тогда
		Заголовок = НСтр("ru = 'Условия поиска контрагента'; en = 'Search criteria for counterparty'");
		
	ИначеЕсли ТипОбъектаВыбора = "DMMeeting" Тогда
		Заголовок = НСтр("ru = 'Условия поиска мероприятия'; en = 'Event search conditions'");
...
Асинх Процедура ОбработатьРезультатПоиска()
	...
	Если КоличествоРезультатов = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Заданные условия поиска не дали ни одного результата.'; en = 'No files found with the specified search criteria.'"));
	...
	ИначеЕсли ПредельноеКоличествоРезультатов <> 0 Тогда
		...
		ПараметрыВопросаДаНет.ТекстКнопкиДа = СтрШаблон(
			НСтр("ru = 'Да, показать первые %1'; en = 'Yes, show first %1'"),
				Формат(ПредельноеКоличествоРезультатов, "ЧГ=0"));
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Нет, уточнить условия'; en = 'No, please clarify the terms'");
		Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
			НСтр("ru = 'Заданные условия поиска дали слишком много результатов. Показать результаты?'; en = 'The specified search terms returned too many results. Show results?'"),
...
Функция СтруктураОтбора()
	...
	Если ЗначениеЗаполнено(СокрЛП(Наименование)) Тогда
		...
		Значение.Вставить("Представление", НСтр("ru = 'Наименование'; en = 'Name'"));
		...
		Значение.Вставить("ПредставлениеУсловия", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'содержит ""%1""'; en = 'contains ""%1""'"), СокрЛП(Наименование)));
		Отбор.Вставить("name", Значение);
	КонецЕсли;
	Если ЗначениеЗаполнено(ВидДокументаID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Вид документа'; en = 'Document kind'"));
	...
	Если ЗначениеЗаполнено(ВидМероприятияID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Вид мероприятия'; en = 'Event type'"));
	...
	Если ЗначениеЗаполнено(Сумма) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Сумма'; en = 'Amount'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(РегистрационныйНомер)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Рег. №'; en = 'Reg. No.'"));
	...
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Дата начала'; en = 'Start date'"));
	...
	Если ЗначениеЗаполнено(ДатаОкончания) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Дата окончания'; en = 'End date'"));
	...
	Если ЗначениеЗаполнено(ЛюбаяДата) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Начиная с'; en = 'Starting from'"));
	...
	Если ЗначениеЗаполнено(ОрганизацияID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Организация'; en = 'Company'"));
	...
	Если ЗначениеЗаполнено(КонтрагентID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Контрагент'; en = 'Counterparty'"));
	...
	Если ЗначениеЗаполнено(ПапкаID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'Папка'; en = 'Folder'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(ИНН)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'ИНН'; en = 'TIN'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(КПП)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("ru = 'КПП'; en = 'KPP'"));
