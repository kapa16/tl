...
Процедура ПерезаполнитьДеревоУзлыКОД(УзлыXDTO)
	...
	ДеревоУзлыКОД.Строки.Добавить().ПредставлениеУзла = НСтр("ru = 'Все пользователи'; en = 'All users'");
...
Процедура УстановитьУзелПользователей(МассивПользователей, ИдентификаторУзла)
	...
	Если МассивПользователей.Количество() = 1 Тогда
		Пояснение = СтрШаблон(
			НСтр("ru = 'Пользователь ""%1"" перемещен в узел ""%2""'; en = 'The ""%1"" user is moved to the ""%2"" node'"),
			МассивПользователей[0],
			ИмяУзла);
	Иначе
		Пояснение = СтрШаблон(
			НСтр("ru = 'Пользователи перемещены в узел ""%1""'; en = 'The users are moved to the ""%1"" node'"),
			ИмяУзла);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("ru = 'Узел закреплен'; en = 'Node is pinned'"),, Пояснение);
