...
Процедура ОбновитьДеревоУчастниковПоОбъекту(ДействиеXDTO)
	...
	ЭтапУтверждение.НаименованиеЭтапа = НСтр("ru = 'Утвердить'; en = 'Confirm'");
	...
	ЭтапОзнакомление.НаименованиеЭтапа = НСтр("ru = 'Ознакомиться с результатом утверждения'; en = 'Review the confirmation result'");
...
Процедура ОбновитьОбъектПоДеревуУчастников(Прокси, ДействиеXDTO)
	...
	ИначеЕсли Не ЕстьОбрабатывающийРезультат Тогда
		ВызватьИсключение НСтр("ru = 'Отсутствует участник ""ознакомления с результатом утверждения""'; en = 'The ""Confirmation result review"" participant is missing'");
	КонецЕсли;
