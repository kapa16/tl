...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	БазовоеНаименованиеЭтапа = НСтр("ru = '<Не введен>'; en = '<Not entered>'");
...
&НаКлиенте
Процедура ОписаниеОсобоеПриИзменении(Элемент)
...
Процедура СрокОбработкиРезолюцииПредставлениеПриИзменении(Элемент)
	...
	РезультатЗаполнения = ИнтеграцияС1СДокументооборот3ВызовСервера.ИзменитьСрокИсполненияПоПредставлению(
		ТекущиеДанные.СрокОбработкиРезолюции,
		ТекущиеДанные.СрокОбработкиРезолюцииДни,
		ТекущиеДанные.СрокОбработкиРезолюцииЧасы,
		ТекущиеДанные.СрокОбработкиРезолюцииМинуты,
		ТекущиеДанные.ВариантУстановкиСрокаОбработкиРезолюции,
		ТекущиеДанные.СрокОбработкиРезолюцииПредставление,
		ПараметрыИзмененияСрока);
	
	Если РезультатЗаполнения Тогда
		ЗаполнитьПредставлениеСроковОбработкиРезолюции();
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыИзмененияСрока.ТекстСообщенияПредупреждения) Тогда
		ИнтеграцияС1СДокументооборот3Клиент.ПоказатьПредупреждениеИзДО(,
...
Процедура СрокОбработкиРезолюцииПредставлениеРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ТекущиеДанные.СрокОбработкиРезолюции = Дата(1,1,1);
...
Процедура УчастникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрыватьОсобоеНаименованиеИОписание = Не ТекущиеДанные.ЭтоЭтап
		...
		И ТекущиеДанные.ВышестоящийУчастник = "00000000-0000-0000-0000-000000000000";
	...
	ИначеЕсли Поле = Элементы.ПроектРезолюции И Не ТекущиеДанные.ЭтоЭтап И ТекущиеДанные.Функция = "ReviewingPerson" Тогда
		...
		Если ТекущиеДанные.ЕстьРезолюция Тогда
			ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Резолюция'; en = 'Resolution'"));
			ПараметрыФормы.Вставить("Комментарий", ТекущиеДанные.ПредставлениеРезолюции);
			ПараметрыФормы.Вставить("ТолькоПросмотр", Истина);
		Иначе
			ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Проект резолюции'; en = 'Draft resolution '"));
...
// Заполняет представление сроков обработки резолюции в карточке действия.
//
&НаКлиенте
Процедура ЗаполнитьПредставлениеСроковОбработкиРезолюции() Экспорт
	
	Для Каждого ЭтапУчастников Из Участники.ПолучитьЭлементы() Цикл
		
		Если ЭтапУчастников.СрокОбщий
				Или ЗначениеЗаполнено(ЭтапУчастников.СрокОбработкиРезолюции)
			...
			ЭтапУчастников.СрокОбработкиРезолюцииПредставление =
				ИнтеграцияС1СДокументооборот3КлиентСервер.ПредставлениеСрокаИсполнения(
					...
					ИспользоватьДатуИВремяВСрокахЗадач,
					ЭтапУчастников.ВариантУстановкиСрокаОбработкиРезолюции);
		Иначе
			ЭтапУчастников.СрокОбработкиРезолюцииПредставление = "";
		КонецЕсли;
		...
		Для Каждого УчастникЭтапа Из ЭтапУчастников.ПолучитьЭлементы() Цикл
			...
			УчастникЭтапа.СрокОбработкиРезолюцииПредставление =
				ИнтеграцияС1СДокументооборот3КлиентСервер.ПредставлениеСрокаИсполнения(
					...
					ИспользоватьДатуИВремяВСрокахЗадач,
					УчастникЭтапа.ВариантУстановкиСрокаОбработкиРезолюции);
...
Процедура ОбновитьСрокОбработкиРезолюции()
	...
	ТекущиеДанные = Элементы.Участники.ТекущиеДанные;
	Если ТекущиеДанные.Функция <> "ProcessingResolution" Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовФормы() Экспорт
	...
	ЭтоОбрабатывающийРезолюцию = (ТекущиеДанные.Функция = "ProcessingResolution");
	...
	Иначе
		...
		ТолькоПросмотрСтроки = ТекущиеДанные.Недоступно
			Или (ТекущиеДанные.ЭтоДочернийУчастник И (ТекущиеДанные.Функция <> "ProcessingResolution"))
			Или (ТекущиеДанные.Состояние = "Completed");
		...
		Элементы.СрокОбработкиРезолюцииПредставление.ТолькоПросмотр =
			(ТекущиеДанные.Функция <> "ReviewingPerson") Или ТолькоПросмотрСтроки;
...
Процедура ОбновитьДеревоУчастниковПоОбъекту(ДействиеXDTO)
	...
	Иначе
		...
		ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Рассмотрение'; en = 'Review'");
		ЭтапУчастников.ЭтоЭтап = Истина;
		...
		ЭтапУчастников = ЭтапыУчастников.Добавить();
		ЭтапУчастников.Функция = "Performer";
		ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Исполнение'; en = 'Execution'");
		ЭтапУчастников.ЭтоЭтап = Истина;
		ЭтапУчастников.Идентификатор = "00000000-0000-0000-0000-000000000000";
		ЭтапУчастников.ИдентификаторПункта = "00000000-0000-0000-0000-000000000000";
		...
		ЭтапУчастников.НаименованиеЭтапа = НСтр("ru = 'Обрабатывающий результат'; en = 'Person who processes the result'");
		ЭтапУчастников.ЭтоЭтап = Истина;
		ЭтапУчастников.Идентификатор = "00000000-0000-0000-0000-000000000000";
		ЭтапУчастников.ИдентификаторПункта = "00000000-0000-0000-0000-000000000000";
...
"Удалите дубли или установите участнику признак ""Защищенный""'; en = 'In the ""%1"" point, the %2 participant is duplicated."
...
"Удалите дубли, укажите особое описание или установите участнику признак ""Защищенный""'; en = 'In the ""%1"" stage, the %2 participant is duplicated."
...
Процедура ОбновитьПолеЕстьОсобоеНаименованиеОписание(Строка)
	...
	Строка.ЕстьОсобоеНаименованиеОписание = ЗначениеЗаполнено(Строка.НаименованиеЗадачи)
		...
		Или ЗначениеЗаполнено(Строка.ОписаниеОсобое);
	...
	Для ИндексЭтапа = 0 По ЭтапыУчастников.Количество() - 1 Цикл
		...
		Для ИндексУчастника = 0 По УчастникиЭтапа.Количество() - 1 Цикл
			...
			Если ПоПунктам И СтрокаУчастника.Функция = "PerformanceResultProcessor" Тогда
				СтрокаУчастника.ПрефиксСтроки = НСтр("ru = '(Провер.)'; en = '(Check.)'") + " ";
			КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьПолейПоПунктам(ДействиеXDTO)
	...
	Элементы.СрокОбработкиРезолюцииПредставление.Видимость = ДоступнаНастройкаСрокаОбработкиРезолюции И Не ПоПунктам;
	
	Если ПоПунктам Тогда
		Элементы.ПредставлениеСтроки.Заголовок = НСтр("ru = 'Пункты, участники'; en = 'Points, participants'");
	Иначе
		Элементы.ПредставлениеСтроки.Заголовок = НСтр("ru = 'Участники'; en = 'Participants'");
	КонецЕсли;
...
Процедура УстановитьДоступностьИзмененияУчастников()
	...
	Для Каждого ЭтапУчастников Из ЭтапыУчастников Цикл
		...
		Для Каждого УчастникЭтапа Из УчастникиЭтапа Цикл
			...
			Иначе
				УчастникЭтапа.Недоступно = ЭтапУчастников.Недоступно Или УчастникЭтапа.Состояние = "Completed";
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ОбработатьФормуСогласноВерсииСервиса() Экспорт
	...
	Попытка
		...
		ДоступнаНастройкаСрокаОбработкиРезолюции = Истина;
		Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.17.36") Тогда
			ДоступнаНастройкаСрокаОбработкиРезолюции = Ложь;
			Элементы.ОписаниеОсобое.Видимость = Ложь;
		КонецЕсли;
		Элементы.СрокОбработкиРезолюцииПредставление.Видимость = ДоступнаНастройкаСрокаОбработкиРезолюции;
	...
	Если ПоПунктам Тогда
		...
		Для Каждого ЭлементЭтап Из ЭлементыЭтапы Цикл
			...
			Если ЭлементЭтап.НаименованиеЭтапа = БазовоеНаименованиеЭтапа Тогда
				ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'Укажите содержание пункта ""%1""'; en = 'Specify the ""%1"" point content'"), ЭлементЭтап.Номер));
				Отказ = Истина;
				Возврат;
			КонецЕсли;
			...
			Если ПунктБезУчастников(ЭлементЭтап) Тогда
				ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'Заполните участников пункта ""%1""'; en = 'Fill the ""%1"" point participants'"), ЭлементЭтап.НаименованиеЭтапа));
				Отказ = Истина;
				Возврат;
			КонецЕсли;
		КонецЦикла;
...
"Для запуска нового исполнения добавьте участников.'; en = 'New participants are not added."
