...
Процедура УстановитьДоступностьЭлементовФормы() Экспорт
	...
	ТолькоПросмотрСтроки = (ТекущиеДанные.Состояние = "Completed")
		...
		Или (МожноТолькоДобавлятьУчастников И ТекущиеДанные.Недоступно)
		Или ТекущиеДанные.НедоступноГруппа;
...
&НаСервере
Процедура ЗаполнитьСостоянияОзнакомления(ДействиеXDTO)
	
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
			ДействиеXDTO, "examinationResultRows") Тогда
		Возврат;
	КонецЕсли;
	
	РезультатыОзнакомлений = Новый Соответствие;
	Для Каждого СтрокаXDTO Из ДействиеXDTO.examinationResultRows.rows Цикл
		СтруктураРезультата = Новый Структура("СколькоОзнакомлено, СколькоВсегоНадоОзнакомить, ДатаЗавершенияИтоговая");
		СтруктураРезультата.СколькоОзнакомлено = СтрокаXDTO.numberOfExaminedParticipants;
		СтруктураРезультата.СколькоВсегоНадоОзнакомить = СтрокаXDTO.numberOfParticipantsToBeExamined;
		СтруктураРезультата.ДатаЗавершенияИтоговая = СтрокаXDTO.finalCompletionDate;
		РезультатыОзнакомлений.Вставить(СтрокаXDTO.id, СтруктураРезультата);
	КонецЦикла;
	...
	Для Каждого ЭлементУчастник Из ЭлементыУчастники Цикл
		...
		Если СтруктураОзнакомления <> Неопределено Тогда
			...
			Если СтруктураОзнакомления.СколькоОзнакомлено < СтруктураОзнакомления.СколькоВсегоНадоОзнакомить Тогда
				ЭлементУчастник.Состояние = "Performing";
				ЭлементУчастник.Результат = "";
			Иначе
				ЭлементУчастник.Состояние = "Completed";
				ЭлементУчастник.Результат = "Examined";
			КонецЕсли;
			
			ЭлементУчастник.НомерКартинки =
				ИнтеграцияС1СДокументооборот3КлиентСервер.НомерКартинкиРезультатаВыполненияДействия(
					ЭлементУчастник.Результат,
					ЭлементУчастник.Состояние);
	...
	Для Каждого Участник Из ДействиеXDTO.participantRows.rows Цикл
		...
		Если ТолькоПросмотр И ТекущееКоличество >= МаксимальноеЧислоУчастниковДляПоказа Тогда
			...
			ДопОписаниеОзнакомиться = СтрШаблон(
				НСтр("ru = 'и др., всего %1, см ""Ход ознакомления"".'; en = 'and others, total %1, see ""Preview progress"".'"),
				ПолноеКоличество);
...
"Удалите дубли, укажите особое описание или установите участнику признак ""Защищенный""'; en = 'The %1 preview participant is duplicated."
...
Процедура ОбработатьОтветВебСервисаНаПолучениеДействия(ДействиеXDTO)
	...
	ЗаполнитьСостоянияОзнакомления(ДействиеXDTO);
...
"Для запуска нового ознакомления добавьте участников.'; en = 'New participants are not added."
