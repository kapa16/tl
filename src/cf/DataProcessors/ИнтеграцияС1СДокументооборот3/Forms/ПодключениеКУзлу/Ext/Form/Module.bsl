...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(ИмяУзла) Тогда
		Заголовок = СтрШаблон(НСтр("ru = 'Подключение к узлу %1'; en = 'Connect to the %1 node'"), ИмяУзла);
		Элементы.ПояснениеАдресВебСервиса.Заголовок =
			СтрШаблон(НСтр("ru = 'Адрес, по которому опубликован веб-сервис узла %1.'; en = 'Address where the web service of the %1 node is published.'"), ИмяУзла);
	КонецЕсли;
...
Функция ПодключитьсяНаСервере()
	...
	Если Ответ.node.objectID.id <> ИдентификаторУзла Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'По адресу ""%1"" опубликован узел ""%2"". Требуется указать адрес публикации узла ""%3"".'; en = 'At ""%1"", the ""%2"" node is published. Specify a publication URL for the ""%3"" node.'"),
			АдресВебСервиса,
			Ответ.node.name,
			ИмяУзла);
	КонецЕсли;
