...
Процедура КомандаСоздатьДокумент(Команда)
	...
	Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияРаспознанногоДокумента.Исходящий") Тогда
		ПараметрыОперации.ТипДокумента = "уатСчетНаОплатуПокупателю";
	Иначе
		ПараметрыОперации.ТипДокумента = "уатСчетНаОплатуПоставщика";
	КонецЕсли;
...
Функция СоздатьДокумент(ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Попытка
		...
		Если ТребуетсяПересчетСкидкиВЦену(ЭтотОбъект) Тогда
			ВызватьИсключение НСтр("ru = 'Документ требует ручной проверки пересчета скидки в цену.'; en = 'The document requires manual verification of the discount conversion into the price.'");;
		КонецЕсли;
	...
	Для Каждого ЭтотСвязанный Из ВсеСвязанные Цикл
		...
		Для Каждого ЭтотТип Из ПодходящиеТипы Цикл
			...
			Если КомплектныеСтроки.Количество() <> 0 Тогда
				...
				Если ТипСвязанного = Тип("ДокументСсылка.уатСчетНаОплатуПоставщика") Тогда
					СозданныеДокументы.ДокументВходящий.Ссылка = ЭтотСвязанный;
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.уатСчетНаОплатуПокупателю") Тогда
