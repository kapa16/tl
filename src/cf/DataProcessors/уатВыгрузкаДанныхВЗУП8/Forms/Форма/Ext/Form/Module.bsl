...
Процедура ВыполнитьВыгрузкуЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если ИспользуемыеПравила = 1 Тогда 
		...
		ФайлПравил.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ВыполнитьВыгрузкуЗавершениеСуществует", ЭтотОбъект));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьВыгрузкуЗавершениеСуществует(Существует1, ДополнительныеПараметры1) Экспорт
...
Процедура ВыполнитьВыгрузкуЗавершениеФрагмент()
	...
	#Иначе
		ТекстНСТР = НСтр("en='Выгрузка УАТ в ЗУП';ru='Выгрузка УАТ в ЗУП'") + ".xml";
...
Процедура ВыполнитьВыгрузкуНаСервере()
	...
	Если ИспользуемыеПравила = 0 Тогда 
		...
		МакетПравилОбмена = ОбработкаОбъект.ПолучитьМакет("ПравилаОбменаТекст31");
...
Процедура ЗагрузитьСтандартныеПравилаОбмена()
	...
	ТекстПравил = ОбработкаОбъект.ПолучитьМакет("ПравилаОбменаТекст31");
