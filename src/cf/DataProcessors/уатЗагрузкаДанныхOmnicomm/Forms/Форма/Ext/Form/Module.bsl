...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьOmnicomm.Получить() Тогда
		ТекстСообщения = НСтр("en='Opening is possible only when use of satellite monitoring system - Omnicomm is enabled.';ru='Открытие возможно только тогда, когда включено использование системы спутникового мониторинга - Omnicomm.'");
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	Если ДобавитьМесяц(Объект.ДатаС, 1) < Объект.ДатаПо  Тогда 
		ТекстОшибки = НСтр("ru = 'Для системы мониторинга Omnicomm не допускается запрос данных за период больше 1 месяца'; en = 'The Omnicomm monitoring system does not allow data requests for a period longer than 1 month.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,"Объект.ДатаПо",,Отказ);
	КонецЕсли;
	
	Если НЕ ЗагрузитьПробегИРасходГСМ
		И НЕ ЗагрузитьДополнительныеСведения
		И НЕ ЗагрузитьКоординаты Тогда 
...
Функция ВыполнениеОбработкиНаСервере()
	...
	МассивТС = ТС.Выгрузить().ВыгрузитьКолонку("Ссылка");
	...
	ПараметрыПроцедуры.ВнешняяСистема         = Справочники.уатВнешниеСистемы.Omnicomm;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Загрузка данных Omnicomm'; en = 'Loading data Omnicomm'");
...
Процедура ВосстановитьНастройки()
	...
	Если ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда 
		СтруктураНастроек = ХранилищеНастроекДанныхФорм.Загрузить(
			"Обработка.уатЗагрузкаДанныхOmnicomm", 
			"НастройкиЗагрузки"
		);
	КонецЕсли;
...
Процедура ЗаполнитьВсемиТСЗавершение()
	...
	|	И уатТС.ИспользуемаяСистемаGPS = ЗНАЧЕНИЕ(Справочник.уатВнешниеСистемы.Omnicomm)
...
Процедура РасширенноеЗаполнениеУстановитьОтбор()
	...
	ОткрытьФорму(
		...
		Новый Структура("ИсточникПодбора", "Обработка_уатЗагрузкаДанныхOmnicomm"), 
		ЭтотОбъект,
		,
		,
		,
