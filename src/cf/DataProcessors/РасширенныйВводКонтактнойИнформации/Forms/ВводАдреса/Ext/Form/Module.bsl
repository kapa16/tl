	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		Если ЕстьПравоЗагружатьКлассификатор Тогда
			ТексСообщения = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Для автоподбора и проверки адресов загрузите адресные сведения. <a href=load>Загрузить</a>';
					|en = 'To enable address autocomplete and validation, import the address details. <a href=load>Import</a>'"));
		...
		Иначе
			Элементы.ИнформацияОЗагрузкеАдресныеСведений.Заголовок = НСтр("ru = 'Для автоподбора и проверки адресов обратитесь к администратору для загрузки адресных сведений.';
																			|en = 'The address autocomplete and validation service requires importing address details. To import them, contact your administrator.'");
			Элементы.ЗагрузитьКлассификатор.Видимость = Ложь;
		КонецЕсли;
	Иначе
		Элементы.ЗагрузитьКлассификатор.Видимость = Ложь;
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.НаселенныйПункт.ПодсказкаВвода      = НСтр("ru = 'Выберите или введите название населенного пункта';
															|en = 'Type or select a settlement'");
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("ru = 'Автоподбор и проверка адреса недоступны:';
											|en = 'Address autocomplete and validation are unavailable:'") + Символы.ПС 
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.УточнениеОшибки = НСтр("ru = 'Автоподбор и проверка адреса недоступны:';
											|en = 'Address autocomplete and validation are unavailable:'");
	...
	Если ВидКонтактнойИнформации.МожноИзменятьСпособРедактирования И ВидКонтактнойИнформации.ПроверятьКорректность Тогда
		ДополнительныйТекст = ДополнительныйТекст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для этого предварительно снять флажок ""Запрещать ввод некорректных адресов"" в виде контактной информации ""%1"".';
				|en = 'To allow users to enter free-text addresses, clear the ""Forbid invalid address entries"" check box for the ""%1"" contact information kind.'"),
