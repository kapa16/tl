...
Процедура ВебСодержимоеПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	Если ДанныеСобытия.Свойство("href") И ЗначениеЗаполнено(ДанныеСобытия.href) Тогда
		...
		// {УАТ}
		Если ТипЗнч(ДанныеСобытия.href) = Тип("Строка")
			И СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/1c-rarus-mobilnoe-prilozhenie-voditel-uat/") Тогда
			ИмяОткрываемойСтраницы = "e1cib/command/РегистрСведений.пр_ОбращенияВСлужбуПоддержки.Команда.уатОтправитьЗаявкуНаПодключениеВодительУАТ";
		КонецЕсли;
		// {/УАТ}
		...
		// {УАТ}
		// Сбор статистики
		Если ТипЗнч(ДанныеСобытия.href) = Тип("Строка") Тогда
			Если СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/servis-parkomatika/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.Паркоматика";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/publications/20210905-udobnaya-oplata-parkovok-s-servisom-parkomatika-v-transportnoy-lineyke-1c-rarus-494050/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.ПаркоматикаСтатья";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/servis-yandeks-marshrutizatsiya/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.Яндекс";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/1c-rarus-mobilnoe-prilozhenie-voditel-uat/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.МобильноеПриложениеВодителя";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://ati.su/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.АТИ";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/1c-predpriyatie-8-transportnaya-logistika-ekspedirovanie-i-upravlenie-avtotransportom-korp/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.ТЛЭКОРП";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/publications/20221115-novovvedeniya-v-1c-tle-po-konteynernym-i-zh-d-gruzoperevozkam-560779/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.КонтейнерныеПеревозки";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://business.multigo.ru/") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.MultiGo";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://rarus.ru/1c-transport/article-epd-uat-tle") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.ЭПД";
			ИначеЕсли СтрНачинаетсяС(ДанныеСобытия.href, "https://kontur.ru/lp/logistika-rarus") Тогда
				ИмяКлючевойОперации = "уатОперации.ИнформацияПриЗапуске.ПереходПоСсылке.Контур";
			Иначе
				ИмяКлючевойОперации = "";
			КонецЕсли;
			Если ИмяКлючевойОперации <> "" Тогда
...
// {УАТ}
&НаСервереБезКонтекста
Процедура ЗаписатьОперациюБизнесСтатистики(ИмяОперации, Значение)
