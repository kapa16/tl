...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	КонецЕсли;
	// {/УАТ} Конец ПодключаемоеОборудование
	...
	//		ПараметрыОтбора.Вставить("Штрихкод", Товар.Штрихкод);
	...
	//		Иначе
	//			СтрокиПоиска[0].Количество = СтрокиПоиска[0].Количество + Товар.Количество;
	//		КонецЕсли;
	//		
	//	КонецЦикла;
	//КонецЕсли;
	...
	ПечатьЭтикетокИЦенниковБПОКлиентПереопределяемый.ОбработкаОповещения(ИмяСобытия, Параметр, Источник, ЭтотОбъект);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Обработка.уатПодборНоменклатуры.Форма.Форма"
		ИЛИ ИсточникВыбора.ИмяФормы = "Обработка.ПодборНоменклатуры.Форма.Форма" Тогда
		уатОбщегоНазначенияКлиент_проф.ОбработкаПодбораПечатиЭтикеток(Объект.Товары, ВыбранноеЗначение);
	КонецЕсли;
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		ТекущиеДанные.ОрганизацияБПО = уатОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	Элементы.ТоварыШтрихкод.СписокВыбора.ЗагрузитьЗначения(уатЗащищенныеФункцииСервер_проф.ПолучитьШтрихкодыОбъекта(ТекущиеДанные.НоменклатураБПО));
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущиеДанные.НоменклатураБПО) Тогда
		мсвШтрихкоды = уатЗащищенныеФункцииСервер_проф.ПолучитьШтрихкодыОбъекта(ТекущиеДанные.НоменклатураБПО);
		Если мсвШтрихкоды.Количество() > 0 Тогда
			ТекущиеДанные.Штрихкод = мсвШтрихкоды[0];
		Иначе
...
Процедура Печать(Команда)
	...
	Для Каждого ТекСтрока Из Товары Цикл
		Если ТекСтрока.Выбран И (НЕ ЗначениеЗаполнено(ТекСтрока.НоменклатураБПО)
			ИЛИ НЕ ЗначениеЗаполнено(ТекСтрока.ШаблонЦенникаЭтикетки)) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'В одной или нескольких строках не указана Номенклатура или Шаблон этикетки'; en = 'One or more lines do not have a Item or Label Template specified.'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	// {/УАТ}
...
Процедура ОповещениеПоискаПоШтрихкодуЭтикетокИЦенниковБПО(Результат, ПараметрыОперации) Экспорт
	...
	Если Не ПустаяСтрока(Результат) Тогда
		...
		// {УАТ}
		уатОбщегоНазначенияКлиент_проф.ДобавитьНоменклатуруВТЧПечатьЭтикеток(Объект.Товары, 
			уатЗащищенныеФункцииСервер_проф.ПолучитьОбъектПоШтрихкоду(Результат));
		Возврат;
		// {/УАТ}
...
Процедура ДобавитьНоменклатуруВТЧПоШтрихкоду(Результат, ДопПараметры = Неопределено) Экспорт
	...
	МассивСтрок = Объект.Товары.НайтиСтроки(Новый Структура("НоменклатураБПО", Номенклатура));
	...
	Иначе
		...
		мсвШтрихкоды = уатЗащищенныеФункцииСервер_проф.ПолучитьШтрихкодыОбъекта(Номенклатура);
		Если мсвШтрихкоды.Количество() > 0 Тогда
			СтрокаТабличнойЧасти.Штрихкод = мсвШтрихкоды[0];
		КонецЕсли;
		СтрокаТабличнойЧасти.Количество = 1;
		СтрокаТабличнойЧасти.ОрганизацияБПО = уатОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
			ПользователиКлиентСервер.ТекущийПользователь(), "ОсновнаяОрганизация");
	КонецЕсли;
