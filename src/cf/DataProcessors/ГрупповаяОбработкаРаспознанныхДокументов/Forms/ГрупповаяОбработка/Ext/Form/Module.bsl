...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru = 'Закрыть групповую обработку распознанных документов?'; en = 'Close batch processing of recognized documents?'");
...
Процедура УдалитьКомплект(Команда)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru = 'Удалить комплект?'; en = 'Delete set?'");
...
Процедура ОбработатьДокументы(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Обработать документы?'; en = 'Process documents?'");
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
"Создать новых контрагентов и заполнить реквизиты можно сразу для нескольких распознанных документов с помощью действия «Заполнить для выделенных строк».'; en = 'Check and fill in the main document details. Fill in the blanks and match those highlighted in red."
"You can create new counterparties and fill in the details for multiple recognized documents at once using the ""Fill for selected rows"" action.'"),
			ШрифтПодсказок
		);
...
"Удалите всю группу дублей, если она была сформирована ошибочно.'; en = 'Review the documents in each group and select the highest-quality copies. The remaining duplicates in the group will be marked as processed."
"If a document in the group is not a duplicate, click ""Remove from Duplicates."""
"Delete the entire duplicate group if it was created in error.'"),
			ШрифтПодсказок
		);
	ИначеЕсли ИмяШага = "Комплекты" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Комплект документов – это группа документов, отражающих одну хозяйственную операцию. Объедините в комплект документы, которые относятся к одной операции, тем самым вы сможете сократить число правок номенклатуры и других реквизитов на следующих шагах группового ввода документов. Учетные документы могут быть созданы для каждого отдельного комплекта, либо для всех сразу на последнем шаге обработки.'; en = 'A document kit is a group of documents recording one business transaction. Merge the documents that belong to one transaction into a kit, so you can reduce the number of changes to the item and other attributes at the next steps of bulk document input. You can create accounting documents either for each individual kit, or for all kits at once at the last processing step.'"),
			ШрифтПодсказок
		);
...
"Серые поля в таблице не доступны для редактирования.'; en = 'Check and fill in additional document details. Fill in the blanks and match the highlighted values."
"You can fill in details for multiple recognized documents at once using the ""Fill for selected rows"" action."
"Gray fields in the table are not editable.'"),
			ШрифтПодсказок
		);
	ИначеЕсли ИмяШага = "Номенклатура" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Проверьте и заполните номенклатуру и связанные с ней реквизиты. Заполняйте одинаковые значения реквизитов сразу для нескольких выделенных строк. По кнопке «Создать выделенные» можно быстро добавить новую номенклатуру для строк, где нет сопоставления с номенклатурой из базы.'; en = 'Check and complete the item and related attributes. Fill the same attribute values for several selected lines at once. Click the ""Create selected items"" button to quickly add a new item for the lines that are not mapped with the item from the database.'"),
			ШрифтПодсказок
		);
	ИначеЕсли ИмяШага = "Суммы" Тогда
		Результат.Текст = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Проверьте и заполните суммы из таблиц документов, перепроверьте выделенные красным значения.'; en = 'Check and fill in the amounts from the document tables, double-check the values ​​highlighted in red.'"),
			ШрифтПодсказок
		);
	ИначеЕсли ИмяШага = "Итого" Тогда
		Результат.Текст = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Проверьте изменения, которые будут внесены в базу данных. При необходимости вы можете вернуться на предыдущие шаги обработки.'; en = 'Check the changes that will be made to the database. If required, you can return to the previous processing steps.'"),
			ШрифтПодсказок
		);
	КонецЕсли;
	...
	Если Вперед Тогда
		...
		ИначеЕсли ИмяШага = "Дубли" Тогда
			...
			Если КоличествоНеЗаполнено > 0 Тогда
				...
				Если КоличествоНеЗаполнено = 1 Тогда
					ТекстДокумент = НСтр("ru = ' распознанном документе '; en = ' recognized document'");
				Иначе
					ТекстДокумент = НСтр("ru = ' распознанных документах '; en = ' recognized documents'");
				КонецЕсли;
				
				ТекстВопроса = Новый ФорматированнаяСтрока(НСтр("ru = 'В '; en = 'To'"),
					Новый ФорматированнаяСтрока(Строка(КоличествоНеЗаполнено), ЖирныйШрифт),
					ТекстДокумент + НСтр("ru = 'есть незаполненные поля, поэтому они не будут обработаны. Продолжить?'; en = 'there are blank fields, so they will not be processed. Continue?'")
				);
		...
		ИначеЕсли ИмяШага = "Комплекты" Тогда
			...
			Если КоличествоНеЗаполнено > 0 Тогда
				...
				Если КоличествоНеЗаполнено = 1 Тогда
					ТекстДубль = НСтр("ru = ' дубля '; en = ' duplicate'");
				Иначе
					ТекстДубль = НСтр("ru = ' дублей '; en = ' duplicates'");
				КонецЕсли;
				
				ТекстВопроса = Новый ФорматированнаяСтрока(НСтр("ru = 'Для '; en = 'For'"),
					Новый ФорматированнаяСтрока(Строка(КоличествоНеЗаполнено), ЖирныйШрифт),
					ТекстДубль + НСтр("ru = 'не выбран основной экземпляр. Их можно выбрать автоматически. Продолжить?'; en = 'The primary instance is not selected. They can be selected automatically. Continue?'")
				);
...
Процедура ОбновитьВидимостьНадписиПроЗаполненность()
	...
	Если Элементы.СтраницыШаги.ТекущаяСтраница = Элементы.СтраницаДляТаблиц Тогда
		Если ТекущийШаг = "Шапка" Тогда
			...
			ТекстНадписи = НСтр("ru = 'Все основные реквизиты заполнены корректно. Нажмите кнопку ""Далее""'; en = 'All the basic details are filled in correctly. Click ""Next""'");
		...
		ИначеЕсли ТекущийШаг = "ДопРеквизиты" Тогда
			ВсеСтрокиНаШагеЗаполнены = Истина;
			ТекстНадписи = НСтр("ru = 'Все дополнительные реквизиты заполнены корректно. Нажмите кнопку ""Далее""'; en = 'All additional details are filled in correctly. Click ""Next.""'");
			Для Каждого СтрокаТаблицы Из ТаблицаДопРеквизитов Цикл
				Если Не СтрокаТаблицы.ДанныеЗаполнены Тогда
					ВсеСтрокиНаШагеЗаполнены = Ложь;
					Прервать;
		...
		ИначеЕсли ТекущийШаг = "Номенклатура" Тогда
			ВсеСтрокиНаШагеЗаполнены = Истина;
			ТекстНадписи = НСтр("ru = 'Вся номенклатура заполнена корректно. Нажмите кнопку ""Далее""'; en = 'All items are filled in correctly. Click ""Next""'");
		...
		ИначеЕсли ТекущийШаг = "Суммы" Тогда
			ВсеСтрокиНаШагеЗаполнены = Истина;
			ТекстНадписи = НСтр("ru = 'Все данные таблиц документов заполнены корректно. Нажмите кнопку ""Далее""'; en = 'All document table data is filled in correctly. Click ""Next.""'");
			Для Каждого СтрокаТаблицы Из ТаблицаСумм Цикл
				Если СтрокаТаблицы.ЕстьОшибка Тогда
					ВсеСтрокиНаШагеЗаполнены = Ложь;
					Прервать;
	...
	ИначеЕсли ТекущийШаг = "Дубли" Тогда
		...
		ВсеСтрокиНаШагеЗаполнены = Ложь;
		ТекстНадписи = НСтр("ru = 'Все дубли проверены. Нажмите кнопку ""Далее""'; en = 'All duplicates have been verified. Click ""Next.""'");
...
Функция ТаблицаРеквизитовДляДоговоров(ПараметрыСоздания, Запрос)
	...
	ИтоговаяТаблица.Колонки.Добавить("ВидДоговора", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыДоговоровКонтрагентов"));
	ИтоговаяТаблица.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка"));
	ИтоговаяТаблица.Колонки.Добавить("ХэшНаименования", Новый ОписаниеТипов("Строка"));
	ИтоговаяТаблица.Колонки.Добавить("Номер", Новый ОписаниеТипов("Строка"));
	...
	Пока Выборка.Следующий() Цикл
		...
		РаспознаваниеДокументовПереопределяемый.ПриЗаполненииПараметровВыбораДоговора(Выборка, ПараметрыВыбораДоговора);
		...
		Иначе
			...
			СтрокаИтога.Номер = Выборка.Номер;
			СтрокаИтога.Дата = Выборка.ДатаЗначение;
			СтрокаИтога.ИдентификаторРезультата = Выборка.ИдентификаторРезультата;
			СтрокаИтога.РаспознанныеДокументы = Новый Массив;
			СтрокаИтога.ИдСтрокДокументов = Новый Массив;
		КонецЕсли;
...
Функция СоздатьНовыйЭлементСправочника(ПараметрыСоздания, ДанныеЭлемента)
	...
	Если Не ЗначениеЗаполнено(НовыйЭлементСправочника) Тогда
		...
		Исключение
			...
			Сообщение.Текст = НСтр("ru = 'Описание ошибки:'; en = 'Error description:'") + Символы.ПС
...
Функция СоздатьОставшихсяКонтрагентовНаСервере(ПараметрыСоздания)
	...
	Пока КоличествоСозданий > ТекущийИдСоздания Цикл
		...
		Если Кандидаты.Количество() <> 0 И
			Кандидаты[0].Коэффициент * 100 >= РаспознаваниеДокументовСлужебныйКлиентСервер.ГраницаУверенныхЗначений() Тогда
			...
			НовоеСообщение.Текст = СтрШаблон(НСтр("ru = 'Новый контрагент ""%1"" не создан. В информационной базе уже есть контрагент ""%2""'; en = 'The new counterparty ""%1"" was not created. The database already contains a counterparty named ""%2.""'"),
...
Функция СоздатьОставшиесяДоговорыНаСервере(ПараметрыСоздания)
	...
	Пока КоличествоСозданий > ТекущийИдСоздания Цикл
		...
		РаспознаваниеДокументовПереопределяемый.ПриЗаполненииПараметровВыбораДоговора(ДанныеДоговора, ПараметрыВыбораДоговора);
		...
		Если Кандидаты.Количество() <> 0 И
			...
			НовоеСообщение = Новый СообщениеПользователю;
			НовоеСообщение.Текст = СтрШаблон(НСтр("ru = 'Новый договор ""%1"" не создан. В информационной базе уже есть договор ""%2""'; en = 'The new contract ""%1"" was not created. The database already contains contract ""%2.""'"),
...
Функция ИзменитьНаправлениеНаСервере(ДокументСсылка)
	...
	Для Каждого КлючЗначение Из ИменаРеквизитов Цикл
		...
		Результат.Вставить(КлючЗначение.Ключ + "Распознано", НСтр("ru = 'Не сопоставлен: '; en = 'Not mapped:'") + ДанныеРеквизита.РаспознанныйТекст);
	КонецЦикла;
...
Функция ЗаполнениеСписковВыбораШапкиНаСервере(ИменаРеквизитов, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод: обновление списков выбора шапки'; en = 'Bulk input: header selection list update'");
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовШапкиНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
		ПараметрыВыполнения.КлючФоновогоЗадания = КлючЗадания;
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод реквизитов шапки'; en = 'Bulk input of header attributes'");
...
Процедура ПерейтиНаСтраницуКомплектыНаСервере(АдресДанныеКомплектов)
	...
	Если НужноОбновитьКомплекты Тогда
		...
		НадписьВсеЗаполнено = НСтр("ru = 'Сформированы все комплекты, которые можно определить автоматически. Нажмите кнопку ""Далее""'; en = 'All sets that can be automatically detected have been generated. Click the ""Next"" button.'");
...
Процедура ОбновитьДанныеДокументаВКомплекте(РаспознанныйДокумент)
	...
	ИначеЕсли СтрокиТаблицы.Количество() > 1 Тогда
		...
		ВызватьИсключение НСтр("ru = 'В основной таблице появились дубли документов.'; en = 'Duplicate documents are found in the main table.'");
	КонецЕсли;
...
Функция НазваниеКомплекта(СтрокаДанных)
	...
	Если СтрокаДанных.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
		СтрокаТипДокумента = НСтр("ru = 'Поступление'; en = 'Income'");
		Если ЗначениеЗаполнено(СтрокаДанных.Контрагент) Тогда
			ВОтКонтрагента = СтрШаблон(НСтр("ru = ' от ""%1""'; en = ' from ""%1""'"), СокрЛП(СтрокаДанных.Контрагент));
		Иначе
			ВОтКонтрагента = "";
		КонецЕсли;
	ИначеЕсли СтрокаДанных.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда
		СтрокаТипДокумента = НСтр("ru = 'Реализация'; en = 'Implementation'");
		Если ЗначениеЗаполнено(СтрокаДанных.Контрагент) Тогда
			ВОтКонтрагента = СтрШаблон(НСтр("ru = ' в ""%1""'; en = ' to ""%1""'"), СокрЛП(СтрокаДанных.Контрагент));
		Иначе
			ВОтКонтрагента = "";
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаДанных.ДатаДокумента) Тогда
		ДатаВДекорации = НСтр("ru = ' от '; en = 'from'") + Формат(СтрокаДанных.ДатаДокумента, "ДФ=dd.MM.yyyy");
	Иначе
		ДатаВДекорации = "";
	КонецЕсли;
	Результат = СтрШаблон(НСтр("ru = '%1%2%3'; en = '%1%2%3'"), СтрокаТипДокумента, ДатаВДекорации, ВОтКонтрагента);
...
Процедура ПолученРезультатСозданияКомплекта(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ДанныеОбработки <> Неопределено Тогда
		...
		Если Результат.ДанныеОбработки.КомплектовНеУдалосьОбработать <> 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Не удалось обработать комплект. Повторите операцию после исправления ошибок:'; en = 'Cannot process the kit. Correct the following errors and try again:'");
...
Функция ПроверитьПравилаКомплекта(ДанныеДокументов, НомерКомплекта)
	...
	РезультатПроверки = Новый Структура("ПроверкиПройдены, ТипКомплекта, ТекстСообщения",
		Ложь, "", НСтр("ru = 'Выбранные типы документов не могут входить в один комплект. Уберите лишние документы из комплекта.'; en = 'The selected document types cannot be included in one kit. Remove unnecessary documents from the kit.'"));
	СтрокиКомплекта = ДанныеДокументов.НайтиСтроки(Новый Структура("НомерКомплекта", НомерКомплекта));
	
	Если СтрокиКомплекта.Количество() = 0 Тогда
		РезультатПроверки.ПроверкиПройдены = Ложь;
		РезультатПроверки.ТекстСообщения = НСтр("ru = 'Нельзя создать комплект без документов'; en = 'Cannot create a kit without documents'");
...
Процедура ПроверитьКомплектАктОбОказанииУслугСчетНаОплатуСчетФактураТОРГ12(СтрокиКомплекта, ТипКомплекта, РезультатПроверки)
	...
	Для Каждого ЭтотДокументКомплекта Из СтрокиКомплекта Цикл
		...
		Если ПервыйДокументКомплекта.Направление <> ЭтотДокументКомплекта.Направление Тогда
			РезультатПроверки.ПроверкиПройдены = Ложь;
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Комплект не может одновременно содержать документы поступления и списания. Убедитесь, что либо все документы комплекта относятся к списанию, либо все – к поступлению.'; en = 'A kit cannot contain both receipt and write-off documents. Make sure all documents in the kit are write-off documents or receipt documents.'");
			Возврат;
		КонецЕсли;
		
		Если ПервыйДокументКомплекта.Организация <> ЭтотДокументКомплекта.Организация Тогда
			РезультатПроверки.ПроверкиПройдены = Ложь;
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Данные документы принадлежат разным организациям и не могут быть обработаны в комплекте. Проверьте корректность заполнения данных документов или обработайте их по отдельности.'; en = 'These documents belong to different organizations and cannot be processed together. Please check that these documents are filled out correctly or process them separately.'");
			Возврат;
		КонецЕсли;
		
		Если ПервыйДокументКомплекта.Контрагент <> ЭтотДокументКомплекта.Контрагент Тогда
			РезультатПроверки.ПроверкиПройдены = Ложь;
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Данные документы принадлежат разным контрагентам и не могут быть обработаны в комплекте. Проверьте корректность заполнения данных документов или обработайте их по отдельности.'; en = 'These documents belong to different counterparties and cannot be processed together. Please check that these documents are filled out correctly or process them separately.'");
			Возврат;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуНоменклатурыНаСервере()
	...
	|	ОсновнаяТаблица.Номенклатура.СтавкаНДС КАК ВидСтавкиНДС,
...
Процедура ВыполнитьСозданиеНоменклатуры(ПараметрыСоздаваемойНоменклатуры)
	...
	НастройкиОжидания.ТекстСообщения = НСтр("ru = 'Создание номенклатуры для заполнения...'; en = 'Creating an item to fill...'");
...
Функция ЗапуститьФоновоеСозданиеНоменклатуры(ПараметрыСоздаваемойНоменклатуры, УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Фоновое создание номенклатуры'; en = 'Background item creation'");
...
Процедура ЗаполнитьТаблицуСозданнойНоменклатуройНаСервере(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Данные Из ИнформацияОНоменклатуре Цикл
		...
		Если Данные.ЭтоДубль Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон(НСтр("ru = 'Новая номенклатура ""%1"" не создана. Реквизит сопоставлен с имеющейся в базе номенклатурой ""%2""'; en = 'The new item ""%1"" was not created. The attribute is associated with an existing item ""%2"" in the database.'"),
...
Процедура ОбновитьВидимостьИСуммыДляОшибкиТЧ(ДанныеЭтойСтроки, ИмяРеквизита);
	...
	Если ДанныеЭтойСтроки.ОшибкаЦены И (ИмяРеквизита = "Количество"
		...
		Если ДанныеЭтойСтроки.ВидСкидки = РаспознаваниеДокументовСлужебныйКлиентСервер.ВидыСкидок().НаОтдельныеПозиции Тогда
			НадписьОшибкаТЧ = СтрШаблон(НСтр("ru = 'Количество * Цена - Скидка (= %1) не совпадает с Сумма (= %2)'; en = 'Quantity * Price - Discount (= %1) does not match Amount (= %2)'"),
		...
		Иначе
			...
			НадписьОшибкаТЧ = СтрШаблон(НСтр("ru = 'Количество * Цена (= %1) не совпадает с Сумма (= %2)'; en = 'Quantity * Price (= %1) does not match Amount (= %2)'"),
	...
	ИначеЕсли ДанныеЭтойСтроки.Количество = 0 И ИмяРеквизита = "Количество" Тогда
		...
		Элементы.НадписьОшибкаТЧ.Видимость = Истина;
		НадписьОшибкаТЧ = НСтр("ru = 'Количество не может быть равно 0'; en = 'The quantity cannot be equal to 0'");
		
	ИначеЕсли ДанныеЭтойСтроки.Цена = 0 И ИмяРеквизита = "Цена" Тогда
		
		Элементы.НадписьОшибкаТЧ.Видимость = Истина;
		НадписьОшибкаТЧ = НСтр("ru = 'Цена не может быть равна 0'; en = 'The price cannot be equal to 0'");
		
	ИначеЕсли ДанныеЭтойСтроки.Сумма = 0 И ИмяРеквизита = "Сумма" Тогда
		
		Элементы.НадписьОшибкаТЧ.Видимость = Истина;
		НадписьОшибкаТЧ = НСтр("ru = 'Сумма не может быть равна 0'; en = 'The sum cannot be equal to 0'");
		
	ИначеЕсли ДанныеЭтойСтроки.ОшибкаНДС И (ИмяРеквизита = "СтавкаНДС" Или ИмяРеквизита = "СуммаНДС") Тогда
		
		Элементы.НадписьОшибкаТЧ.Видимость = Истина;
		НадписьОшибкаТЧ = СтрШаблон(НСтр("ru = 'Рассчитанная сумма НДС (= %1) не совпадает с заполненной суммой НДС (= %2)'; en = 'The calculated VAT amount (= %1) does not match the filled in VAT amount (= %2)'"),
	...
	ИначеЕсли ДанныеЭтойСтроки.ОшибкаВсего И (ИмяРеквизита = "Сумма"
		...
		Элементы.НадписьОшибкаТЧ.Видимость = Истина;
		НадписьОшибкаТЧ = СтрШаблон(НСтр("ru = 'Сумма + Сумма НДС (= %1) не совпадает с Всего (= %2)'; en = 'Amount + VAT Amount (= %1) does not match Total (= %2)'"),
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовТаблицыНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
		ПараметрыВыполнения.КлючФоновогоЗадания = КлючЗадания;
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод реквизитов таблицы'; en = 'Bulk input of table attributes'");
...
"Не будут обработаны %1 документа, поскольку в них'; en = ';%1 document will not be processed because it contains;;"
"%1 documents will not be processed because they contain;"
"%1 documents will not be processed because they contain;"
"%1 documents will not be processed because they contain'"), КоличествоНеОбработано)
			+ НСтр("ru = ' остались нераспознанные реквизиты. Для корректировки вернитесь на предыдущие шаги.'; en = 'There are some unrecognized details left. To correct them, please return to the previous steps.'");
	КонецЕсли;
			...
			Если Документ.ПотенциальныйКандидат Тогда
				Если Не РаспознаваниеДокументовСлужебный.СканУжеЕстьУВладельца(СтрокаТаблицы.Ссылка, Документ.Ссылка, АдресКартинки) Тогда
					...
					Если СтрокиВКомплектах.Количество() <> 0 И СтрокиВКомплектах[0].НомерКомплекта <> 0 Тогда
						Если РаспознаваниеДокументовКомплекты.НужноПрикрепитьДополнительныйСкан(
...
" - документы должны принадлежать одному комплекту.'; en = 'Some documents contain errors and cannot be processed."
" Cannot process: %1"
""
...
"В базу данных было добавлено %1 документа'; en = ';%1 document was added to the database;;"
"%1 document was added to the database;"
"%1 documents were added to the database;"
"%1 documents were added to the database'"), ДанныеОбработки.СозданоДокументов)
				+ " " + СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = ';и %1 новый скан.;;"
"и %1 новых скана.;"
"и %1 новых сканов.;"
"и %1 новых скана.'; en = ';and %1 new scan.;;"
"and %1 new scans.;"
"and %1 new scans.;"
"and %1 new scans.'"), ДанныеОбработки.ПрикрепленоСканов);
...
"%1 document was added to the database.;"
"%1 documents were added to the database.;"
"%1 document was added to the database.'"), ДанныеОбработки.СозданоДокументов);
			ИначеЕсли ДанныеОбработки.СозданоДокументов = 0 И ДанныеОбработки.ПрикрепленоСканов <> 0 Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
...
"В базу данных было добавлено %1 новых скана.'; en = ';%1 new scans were added to the database.;;"
"%1 new scans were added to the database.;"
"%1 new scans were added to the database.;"
"%1 new scans were added to the database.'"), ДанныеОбработки.ПрикрепленоСканов);
			Иначе
				ТекстСообщения = "";
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИтогоОдиночныеДокументы Цикл
		...
		Если СтрокаТаблицы.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.ТОРГ12
			...
			Если СтрокаТаблицы.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
				ТипСоздаваемого = Тип("ДокументСсылка.уатПоступлениеТоваровУслуг");
			Иначе
				ТипСоздаваемого = Тип("ДокументСсылка.уатРеализацияУслуг");
			КонецЕсли;
		
		ИначеЕсли СтрокаТаблицы.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.СчетНаОплату Тогда
			Если СтрокаТаблицы.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
				ТипСоздаваемого = Тип("ДокументСсылка.уатСчетНаОплатуПоставщика");
			Иначе
				ТипСоздаваемого = Тип("ДокументСсылка.уатСчетНаОплатуПокупателю"); 
