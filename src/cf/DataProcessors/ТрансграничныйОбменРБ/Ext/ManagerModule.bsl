...
Функция ПолучитьGLN(Идентификатор) Экспорт
	...
	ДлинаИдентификатораСGLN = 17;
	Если СтрДлина(Идентификатор) <> ДлинаИдентификатораСGLN Тогда
		Возврат "";
	КонецЕсли;
	
	ДлинаПрефикса = 3;
	Префикс = ВРег(Лев(Идентификатор, ДлинаПрефикса));
	
	Для Каждого Оператор Из Операторы() Цикл
		Если Префикс = Врег(Оператор.Префикс) Тогда
			ДлинаGLN = 13;
			Возврат Прав(Идентификатор, ДлинаGLN);
		КонецЕсли;
	КонецЦикла;
...
Функция ФорматыЭД() Экспорт
	...
	ФорматЭД.Код = "BLRDLN";
	ФорматЭД.Наименование = НСтр("ru = 'Товарная накладная (Республика Беларусь)'");
	...
	ФорматЭД.Код = "BLRDNR";
	ФорматЭД.Наименование = НСтр("ru = 'Товарная накладная (Республика Беларусь), титул покупателя'");
	...
	ФорматЭД.Код = "BLRAPN";
	ФорматЭД.Наименование = НСтр("ru = 'Уведомление об уточнении (Республика Беларусь)'");
...
Функция СодержаниеТоварнойНакладной(ДвоичныеДанные)
	...
	Если ТрансграничныйЭДО.ТипыДокументовИспользуются() Тогда
		Результат.ТипДокумента = ТипыДокументов.ТоварнаяНакладнаяРБ;
	Иначе
		Результат.ТипДокумента = ТипыДокументов.ТоварнаяНакладная;
	КонецЕсли;
...
Функция ПрочитатьТоварнуюНакладную(ДвоичныеДанные)
	...
	Исключение
		...
		Операция = НСтр("ru = 'Чтение товарной накладной (Республика Беларусь)'");
...
Функция УведомлениеОбУточнении_Данные(ДанныеЭД)
	...
	Исключение
		...
		Операция = НСтр("ru = 'Формирование уведомления об уточнении накладной (Республика Беларусь)'");
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработкаНеисправностейБЭД.ОбработатьОшибку(Операция,
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ТЭДО, ТекстОшибки, ТекстОшибки);
...
Функция ПрочитатьТоварнуюНакладную_ТитулПокупателя(ДвоичныеДанные)
	...
	Исключение
		...
		Операция = НСтр("ru = 'Чтение титула покупателя товарной накладной (Республика Беларусь)'");
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработкаНеисправностейБЭД.ОбработатьОшибку(Операция,
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ТЭДО, ТекстОшибки, ТекстОшибки);
		Возврат Неопределено;
