...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВариантОбработки = Перечисления.ВариантыОбработкиЗаданияРаспознавания.РеализацияТоваров Тогда
...
Процедура Подключаемый_ОбработкаВыбораПоля(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИмяРеквизита = "ПродавецОрганизация"
		ИЛИ ИмяРеквизита = "ПокупательОрганизация" Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуСозданнойНоменклатуройНаСервере(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Данные Из ИнформацияОНоменклатуре Цикл
		...
		Попытка
			Если НЕ ЗначениеЗаполнено(СтрокаЗаполнения.СтавкаНДС)
				ИЛИ СтрокаЗаполнения.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС Тогда
				СтрокаЗаполнения.СтавкаНДС                 = Данные.Номенклатура.СтавкаНДС;
				СтрокаЗаполнения.СтавкаНДСЗаполненоВручную = Истина; 
				СтрокаЗаполнения.СтавкаНДСЭлементСоздан = Истина;
			Конецесли;
		Исключение
		КонецпоПытки;
		РаспознаваниеДокументовПереопределяемый.ПриИзмененииКолонкиНаСервере(
	...
	Для Каждого ЭтотСвязанный Из ВсеСвязанные Цикл
		...
		Для Каждого ЭтотТип Из ПодходящиеТипы Цикл
			...
			Если КомплектныеСтроки.Количество() <> 0 Тогда
				...
				Если ТипСвязанного = Тип("ДокументСсылка.уатПоступлениеТоваровУслуг") Тогда
					СозданныеДокументы.Поступление.Ссылка = ЭтотСвязанный;
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.уатРеализацияУслуг") Тогда
...
#КонецОбласти
	...
	Элементы.ГруппаКнопкиКомплект.Видимость       = Ложь;
...
Процедура УстановитьВидимостьСчетовУчета()
	...
	Для Каждого ЭлементСчет Из ЭлементыСчетов Цикл
		Если Элементы.Найти(ЭлементСчет) <> Неопределено Тогда
			Элементы[ЭлементСчет].Видимость = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДополнительныеРеквизиты()
	...
	Организация = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(
		Объект, 
		уатРаспознаваниеДокументов.ОсновныеРеквизиты(Объект)["Организация"], 
