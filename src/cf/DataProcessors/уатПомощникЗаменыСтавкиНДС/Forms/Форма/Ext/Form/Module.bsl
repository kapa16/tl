...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.СтараяСтавкаНДС) Тогда
		Объект.СтараяСтавкаНДС = Перечисления.СтавкиНДС.НДС20;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Объект.НоваяСтавкаНДС) Тогда
		Объект.НоваяСтавкаНДС = Перечисления.СтавкиНДС.НДС22;
	КонецЕсли;
	ТОНастройкаПорции          = 1;
	...
	Если НЕ уатОбщегоНазначенияПовтИсп.ВариантПоставкиСТД() Тогда
		ИнициализироватьНастройкиНСИ(1, "Справочник.уатГрузовыеМеста_уэ");
	Иначе
		Элементы.ГрузовыеМеста.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.ФормаНазад.Видимость = Ложь;
	ЭтоВнешняяОбработка = СтрНайти(ТипЗнч(РеквизитФормыВЗначение("Объект")), "Внешняя") = 0;
...
Процедура НастройкаОтбораНажатие(Элемент)
	...
	ПараметрыФормы = Новый Структура("ВыбранныеТипы, Настройки, СтараяСтавкаНДС",
		КомпоновщикиНастроек[Индекс].Представление,
		КомпоновщикиНастроек[Индекс].Значение.Настройки,
		Объект.СтараяСтавкаНДС);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбранныхОбъектов", ЭтотОбъект);
	Если ЭтоВнешняяОбработка Тогда
		ОткрытьФорму("Обработка.уатПомощникЗаменыСтавкиНДС.Форма.ВыбранныеЭлементы", ПараметрыФормы,
		,,,, ОписаниеОповещения);
	Иначе
		ОткрытьФорму("ВнешняяОбработка.уатПомощникЗаменыСтавкиНДС.Форма.ВыбранныеЭлементы", ПараметрыФормы,
		,,,, ОписаниеОповещения);
	Конецесли;
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.НоваяСтавкаНДС) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не выбрана ставка НДС, которую требуется установить'"));
			Возврат;
		КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	ОчиститьСообщения();
	ТекущееСостояниеИзменения = ТекущееСостояниеИзмененияПоУмолчанию();
	...
	ТекущееСостояниеИзменения.ЕстьЭлементыДляОбработки = Истина;
	// Позиция последнего обработанного элемента. 1 - первый элемент.
	ТекущееСостояниеИзменения.КоличествоОбъектовДляОбработки = КоличествоОбъектовДляОбработки;
	ТекущееСостояниеИзменения.РазмерПорции = РазмерПорции;
	ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных = ПоказыватьПроцентОбработанных;
...
Процедура ОбновитьНадписьКоличествоВыбранных(Индекс)
	...
	Если ЕстьНастроенныеОтборы(КомпоновщикиНастроек[Индекс].Значение) Тогда
		...
		ТекстНадписи = СтрокаСЧислом(НСтр("ru = ';%1 элемент;;%1 элемента;%1 элементов;%1 элемента'"),
			КоличествоВыбранных, ВидЧисловогоЗначения.Количественное, "ЧН=0");
	Иначе
...
Процедура ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	ПараметрыЗадания.Вставить("СтараяСтавкаНДС", Объект.СтараяСтавкаНДС);
	ПараметрыЗадания.Вставить("НоваяСтавкаНДС", Объект.НоваяСтавкаНДС);
	...
	Если ЭтоВнешняяОбработка Тогда
		ИмяПроцедуры = "Обработка.уатПомощникЗаменыСтавкиНДС.МодульОбъекта.ИзменитьОбъекты";
	Иначе
		ИмяПроцедуры = "ВнешняяОбработка.уатПомощникЗаменыСтавкиНДС.МодульОбъекта.ИзменитьОбъекты";
	КонецЕсли;
...
&НаКлиенте
Функция ТекущееСостояниеИзмененияПоУмолчанию()
	
	ТекущееСостояниеИзменения = Новый Структура;
	
	ТекущееСостояниеИзменения.Вставить("ЕстьЭлементыДляОбработки", Ложь);
	// Позиция последнего обработанного элемента. 1 - первый элемент.
	...
	ТекущееСостояниеИзменения.Вставить("КоличествоОбъектовДляОбработки", 0);
	ТекущееСостояниеИзменения.Вставить("РазмерПорции", 0);
	ТекущееСостояниеИзменения.Вставить("ПоказыватьПроцентОбработанных", Ложь);
	ТекущееСостояниеИзменения.Вставить("ПрерватьИзменение", Ложь);
	
	Возврат ТекущееСостояниеИзменения;
