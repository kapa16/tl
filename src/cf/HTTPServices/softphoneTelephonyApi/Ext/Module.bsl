...
Функция rtEventPOST(Запрос)
	...
	Если ПодписьЗапроса = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В заголовках не передана подпись ""%1""'; en = 'The signature ""%1"" was not passed in the headers.'"), ПодписьЗапросаЗаголовок);
...
Функция calltrackingWebhook(Запрос)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("сфпИспользоватьCoMagic") Тогда
		Возврат СообщениеОбОшибке(
			...
			500,
			ИмяСобытияДляЖурналаРегистрации,
			НСтр("ru = 'Использование коллтрекинга отключено в настройках'; en = 'Call tracking is disabled in the settings.'"));
	КонецЕсли;
