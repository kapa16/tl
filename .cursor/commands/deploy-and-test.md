...
| Key | Purpose |
|---|---|
| `PLATFORM_PATH` | Platform **`bin`** directory (`1cv8.exe`, `ibcmd.exe`). Example: `C:\Program Files\1cv8\8.3.27.1859\bin`. Repo shortcut: `scripts/deploy-extension.ps1` |
...
```powershell
& '{PLATFORM_PATH}\ibcmd.exe' infobase config import `
    --config='{IBCMD_CONFIG}' `
    --user='{IB_USER}' `
...
```powershell
& '{PLATFORM_PATH}\ibcmd.exe' infobase config apply `
    --config='{IBCMD_CONFIG}' `
    --user='{IB_USER}' `
    --password='{IB_PASSWORD}' `
    --force `
...
```powershell
& '{PLATFORM_PATH}\1cv8.exe' DESIGNER `
    /F '{INFOBASE_PATH}' `
    /N '{IB_USER}' `
    /P '{IB_PASSWORD}' `
...
On a locked test infobase, `-SessionTerminate force` is required (dev/test only). Shortcut: `powershell -File scripts/deploy-extension.ps1` (reads `.dev.env`, same flags).
