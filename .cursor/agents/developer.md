...
**Development standards:** Follow `.cursor/rules/dev-standards-core.mdc` (project parameters, code style, modification comments, naming, documentation) and `.cursor/rules/dev-standards-architecture.mdc` (architecture patterns, extensions, platform standards).
...
- **Search discipline** — follow `.cursor/rules/mcp-first-search.mdc`: MCP project-index tools first; `Grep` / `Glob` only as a justified last resort on 1C project source
...
When working with form modules, follow `.cursor/rules/form-module.mdc`:
