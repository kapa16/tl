...
Check compliance with the `## Persona` section in `AGENTS.md`, `.cursor/rules/dev-standards-core.mdc` (project parameters, code style, modification comments, naming, documentation) and `.cursor/rules/dev-standards-architecture.mdc` (architecture patterns, extensions, platform standards):
- Query formatting
- Common module usage
...
**Search discipline:** Follow `.cursor/rules/mcp-first-search.mdc` — MCP project-index tools first (graph → code-metadata → `grep=true` retry); `Grep` / `Glob` are not in this agent's toolset by design (see frontmatter) — request a search via the parent or `1c-explorer` if needed.
...
**SDD Integration:** If the project has an `openspec/` workspace, read `.cursor/rules/sdd-integrations.mdc` for OpenSpec integration guidance.

## Review Checklist

See `.cursor/rules/anti-patterns.mdc` for detailed patterns.
...
### Code Quality (HIGH)
- Method length — see `.cursor/rules/dev-standards-core.mdc §2 → "Quality Metrics"` (review trigger >100 lines, hard limit >200 lines, exception: query texts)
- Deep nesting (≥5 hard limit; 5 levels review — see `.cursor/rules/dev-standards-core.mdc` §2 → "Quality Metrics")
...
See `.cursor/rules/anti-patterns.mdc → "Confidence Scoring (for Reviews)"` for scale details.
...
**File:** `Module.bsl:45`
**Issue:** [Description]
**Rule:** See the relevant section of `.cursor/rules/anti-patterns.mdc`, `.cursor/rules/coding-standards.mdc`, or `AGENTS.md → Development Procedure`
**Fix:** [Correction]
