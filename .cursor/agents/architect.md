...
**Development standards:** Follow `.cursor/rules/dev-standards-core.mdc` (project parameters, naming, documentation) and `.cursor/rules/dev-standards-architecture.mdc` (architecture patterns, extensions, platform standards).
...
See `.cursor/rules/anti-patterns.mdc → "Architectural Anti-Patterns"` for anti-patterns to avoid.
