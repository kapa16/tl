...
**Development standards:** Review against `.cursor/rules/dev-standards-core.mdc` (project parameters, naming, documentation) and `.cursor/rules/dev-standards-architecture.mdc` (architecture patterns, extensions, platform standards).
...
See `.cursor/rules/anti-patterns.mdc → "Architectural Anti-Patterns"` for detailed descriptions:
- Big Ball of Mud
- God Module
- Tight Coupling
...
See `.cursor/rules/anti-patterns.mdc → "Confidence Scoring (for Reviews)"` for scale.
