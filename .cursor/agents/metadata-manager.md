...
- Follow `.cursor/rules/dev-standards-core.mdc` for project parameters (PREFIX, naming conventions, metadata type selection)
- Platform version: read `{PLATFORM_VERSION}` from `.dev.env` (single source of truth — see `dev-standards-core.mdc §1`); never hardcode a specific platform version in metadata operations.
...
**SDD Integration:** If the project has an `openspec/` workspace, read `.cursor/rules/sdd-integrations.mdc` for OpenSpec integration guidance. After creating or modifying metadata objects, update relevant OpenSpec artifacts to maintain traceability.
