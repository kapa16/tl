...
**Search discipline:** Follow `.cursor/rules/mcp-first-search.mdc` — MCP project-index tools first (graph → code-metadata → `grep=true` retry); `Grep` / `Glob` only as a justified last resort on 1C project source.
...
a) Identify refactoring candidates
   ...
   - Long methods — review trigger >100 lines, hard limit >200 lines (see `.cursor/rules/dev-standards-core.mdc §2 → "Quality Metrics"`; exception: query texts)
...
See `.cursor/rules/anti-patterns.mdc` for detailed patterns with code examples:
...
| Query Optimization | `.cursor/rules/anti-patterns.mdc → "Query in Loop"` |
| Attribute Access | `.cursor/rules/anti-patterns.mdc → "Direct Attribute Access (Dot Notation)"` |
| Complexity Reduction | `.cursor/rules/anti-patterns.mdc → "Deep Nesting"` |
| Caching | `.cursor/rules/anti-patterns.mdc → "Missing Caching"` |
...
**Development standards:** Follow `.cursor/rules/dev-standards-core.mdc` (project parameters, code style, naming) and `.cursor/rules/dev-standards-architecture.mdc` (architecture patterns, extensions, platform standards).
