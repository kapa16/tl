...
- **Изоляция в git**: для каждой новой задачи — отдельная ветка; не смешивать несвязанные доработки в одной ветке и не коммитить в `main` / `master` напрямую
...
**Действия**:
1. **Создай отдельную git-ветку для задачи** (до любых правок кода и метаданных):
   - Проверь текущую ветку: `git branch --show-current` и `git status`
   - Если уже на ветке этой же задачи — продолжай в ней
   - Иначе переключись на актуальную базовую ветку (`main` / `master` / ветка, указанная пользователем), подтяни изменения (`git pull`, если есть remote), создай и переключись на новую ветку
   - Имя ветки: `feature/<краткое-имя-задачи>` (латиница, kebab-case; согласуй с пользователем при неоднозначности). Пример: задача «очередь отправки в Госключ» → `feature/goskey-send-queue`
   - **Не начинай Phase 1+**, пока ветка не создана и рабочее дерево не на ней
   - Коммиты и push — только по явной просьбе пользователя (см. правила проекта)
2. Создай список задач со всеми фазами
3. Создай директорию `.tasks/task-[feature-name]/` для хранения артефактов
...
5. Запиши в `.tasks/task-[feature-name]/phase0-complexity.md`:
   - оценку сложности
   - имя git-ветки и базовую ветку, от которой она отведена
...
2. Запусти агентов `1c-explorer` согласно своей стратегии:
...
2. **Для каждого этапа реализации последовательно**:
   ...
   b. Запусти агента `1c-developer` для реализации этапа:
   ...
   c. Проверь критерии приемки этапа:
      ...
      - Код соответствует правилам из `.cursor/rules/coding-standards.mdc` и `AGENTS.md`?
...
1. Запусти ревьюеров согласно размеру задачи:
   ...
   - **Средняя/сложная**:
     ...
     - `1c-code-reviewer` для проверки качества кода:
       - Укажи путь к правилам: `.cursor/rules/coding-standards.mdc` и `AGENTS.md`
...
2. Сохрани резюме в файл `.tasks/task-[feature-name]/phase8-summary.md`:
   ...
   - Git-ветка задачи и изменённые файлы (`git diff --stat` относительно базовой ветки)
