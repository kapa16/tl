...
if (-not $V8Path) {
    ...
    $exeDirect = Join-Path $V8Path "1cv8.exe"
    $exeBin = Join-Path $V8Path "bin\1cv8.exe"
...
try {
    ...
    # --- Server-side load for client-server infobase (/S) ---
    if ($InfoBaseServer -and $InfoBaseRef) {
        $arguments += "-Server"
    }
    ...
    # --- UpdateDB ---
    if ($UpdateDB) {
        $arguments += "/UpdateDBCfg", "-Dynamic+", "-SessionTerminate", "force"
    }
