...
export function findClickTargetScript(formNum, text, { tableName, gridSelector } = {}) {
  ...
  return `(() => {
    const norm = s => (s?.trim().replace(/\\u00a0/g, ' ') || '').replace(/ё/gi, 'е');
    const lc = v => String(v ?? '').toLowerCase().replace(/ё/gi, 'е');
    const target = ${JSON.stringify(String(text).toLowerCase().replace(/ё/g, 'е'))};
    const p = ${JSON.stringify(p)};
    ...
    [...document.querySelectorAll('[data-content]')].filter(el => {
      ...
      items.push({ id: el.id, name: String(el.dataset.content || ''), label: '', kind: 'tab',
    ...
    if (gridSelector) {
      ...
      if (gridEl) {
        ...
        let cf = containerItems.find(i => lc(i.name) === target);
        if (!cf) cf = containerItems.find(i => i.label && lc(i.label) === target);
        if (!cf && target.length >= 4) cf = containerItems.find(i => lc(i.name).includes(target));
        if (!cf && target.length >= 4) cf = containerItems.find(i => i.label && lc(i.label).includes(target));
        ...
        if (gridName) {
          ...
          let pf = prefixItems.find(i => lc(i.name) === target);
          if (!pf && target.length >= 4) pf = prefixItems.find(i => i.label && lc(i.label).includes(target));
          if (!pf && target.length >= 4) pf = prefixItems.find(i => lc(i.name).includes(target));
    ...
    let found = items.find(i => lc(i.name) === target);
    if (!found) found = items.find(i => i.label && lc(i.label) === target);
    if (!found) found = items.find(i => i.tooltip && lc(i.tooltip) === target);
    if (!found) found = items.find(i => lc(i.name).startsWith(target));
    if (!found) found = items.find(i => i.label && lc(i.label).startsWith(target));
    if (!found && target.length >= 4) found = items.find(i => lc(i.name).includes(target));
    if (!found && target.length >= 4) found = items.find(i => i.label && lc(i.label).includes(target));
    if (!found && target.length >= 4) found = items.find(i => i.tooltip && lc(i.tooltip).includes(target));
