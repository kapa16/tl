...
import {
  isWebClientLoggedIn,
  submitWebClientLogin,
  waitForWebClientMainUI,
...
/** System Chrome/Edge when Playwright browsers cannot be downloaded (corporate proxy). */
function chromiumExecutableOptions() {
  const executablePath = process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH?.trim();
  if (executablePath && fsExistsSync(executablePath)) {
...
/**
 * Log in to the 1C web client when the login form is shown (outside formN_ DOM).
 * No-op if the main UI (sections with text labels) is already visible.
 *
 * @param {{ user?: string, password?: string }} [options]
 * @returns {Promise<{ status: 'already' | 'logged_in' }>}
 */
export async function ensureLoggedIn({ user = '', password = '' } = {}) {
  ensureConnected();
  if (await isWebClientLoggedIn(page)) {
    return { status: 'already' };
  }
  if (!user) {
    throw new Error('ensureLoggedIn: login form visible but user name is empty');
  }
  await submitWebClientLogin(page, user, password);
  const ok = await waitForWebClientMainUI(page);
  if (!ok) {
    throw new Error('ensureLoggedIn: main UI did not appear after login');
  }
  await closeModals();
...
/**
 * Click a visible hyperlink / command link without requiring detectForm().
 * Use for hub forms that only contain Type=Hyperlink buttons (no editInput fields).
 */
export async function clickHyperlink(text) {
  ensureConnected();
  await dismissPendingErrors();
  const re = text instanceof RegExp ? text : new RegExp(String(text), 'i');
  const formBefore = await page.evaluate(detectFormScript());
  const target = await page.evaluate(({ pattern, flags }) => {
    const norm = s => (s?.trim().replace(/\\u00a0/g, ' ') || '').replace(/ё/gi, 'е');
    const re = new RegExp(pattern, flags);
    const els = [...document.querySelectorAll(
      'a.press[id], a.hyperlink[id], [class*="staticTextHyper"][id]',
    )].filter(el => el.offsetWidth > 0);
    const match = els.find(el => {
      const label = norm(el.innerText) || norm(el.title || '');
      const idTail = el.id.replace(/^form\\d+_/, '');
      return re.test(label) || re.test(idTail);
    });
    if (!match) {
    ...
    return {
      ok: true,
      name: norm(match.innerText) || match.id,
      x: Math.round(r.x + r.width / 2),
      y: Math.round(r.y + r.height / 2),
    };
  }, { pattern: re.source, flags: re.flags.replace(/g/g, '') || 'i' });
  if (target?.error) {
    throw new Error(`clickHyperlink: "${re}" not found. Available: ${target.available?.join(', ') || 'none'}`);
  }
  await page.mouse.click(target.x, target.y);
  await waitForStable(formBefore);
  const state = await getFormState();
  state.clicked = { kind: 'hyperlink', name: target.name };
  const err = await checkForErrors();
  if (err) state.errors = err;
  return state;
}
...
export async function clickElement(text, { dblclick, table, toggle, expand, modifier, timeout } = {}) {
  ...
  if (pending?.confirmation) {
    const btnResult = await page.evaluate(`(() => {
      ...
      const target = ny(${JSON.stringify(String(text).toLowerCase())});
  ...
  if (Array.isArray(popupItems) && popupItems.length > 0) {
    const target = normYo(String(text ?? '').toLowerCase());
    const popupName = i => String(i?.name ?? '');
    let found = popupItems.find(i => normYo(popupName(i).toLowerCase()) === target);
    if (!found) found = popupItems.find(i => normYo(popupName(i).toLowerCase()).includes(target));
    if (found) {
...
export async function selectValue(fieldName, searchText, { type } = {}) {
  ...
  if (Array.isArray(popupItems) && popupItems.length > 0) {
    ...
    if (searchText) {
      const target = normYo(String(searchText).toLowerCase());
      const itemName = i => String(i?.name ?? '');
      // Try to find match among regular dropdown items
      let match = regularItems.find(i => normYo(itemName(i).toLowerCase()) === target);
      if (!match) match = regularItems.find(i => normYo(itemName(i).toLowerCase()).includes(target));
      if (!match) match = regularItems.find(i => {
        const name = normYo(itemName(i).replace(/\s*\([^)]*\)\s*$/, '').toLowerCase());
...
export async function highlight(text, opts = {}) {
  ...
  if (Array.isArray(popupItems) && popupItems.length > 0) {
    ...
    const popupName = i => String(i?.name ?? '');
    let found = popupItems.find(i => normYo(popupName(i).toLowerCase()) === target);
    if (!found) found = popupItems.find(i => normYo(popupName(i).toLowerCase()).startsWith(target));
    if (!found) found = popupItems.find(i => normYo(popupName(i).toLowerCase()).includes(target));
    if (found) {
