...
For basic SSL usage (attribute access, user messages) — see `dev-standards-architecture.mdc §4 → "Data Access — Reference Attribute Access"` and `dev-standards-core.mdc §2`.
